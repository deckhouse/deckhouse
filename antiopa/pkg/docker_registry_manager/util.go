...
func FindImageDigest(imageId string) (image string, err error) {
	if strings.Contains(imageId, "://") {
		if !DockerPullableDigestRe.MatchString(imageId) {
			err = fmt.Errorf("Pod status contains image_id and not digest. Antiopa update process not working in clusters with Docker 1.11 or earlier. imageId='%s', regex='%s'", imageId, DockerPullableDigestRe)
			return "", err
		}
	} else {
		if !KubeDigestRe.MatchString(imageId) {
			err = fmt.Errorf("Pod status contains image_id and not digest. Antiopa update process not working in clusters with Docker 1.11 or earlier. imageId='%s', regex='%s'", imageId, KubeDigestRe)
			return "", err
		}
	}
