...
STATIC_IMAGE=$(werf stage image website-docs/web/static 2>/dev/null | tail -1)
MODULES_IMAGE=$(werf stage image website-docs/modules-embedded/static-artifact 2>/dev/null | tail -1)
PDF_BUILDER_IMAGE=$(werf stage image website-docs/pdf-builder 2>/dev/null | tail -1)
echo "PDF_BUILDER_IMAGE='${PDF_BUILDER_IMAGE}'" >> "${_TMPDIR}/env"
...
docker stop "$MODULES_CONTAINER" &>/dev/null
