...
function run_hugo() {
    docker run --rm -p 1313:1313 -v $BUILDER_TEMPLATE_PATH:/src -w /src --entrypoint hugo ghcr.io/gohugoio/hugo:v0.161.1 server --disableFastRender --environment production
}
