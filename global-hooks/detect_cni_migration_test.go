...
`))
		...
		It("global.internal.cniMigrationEnabled should be true", func() {
			...
			Expect(f.ValuesGet("global.internal.cniMigrationValidationIgnore").Bool()).To(BeTrue())
		})
	})
...
`))
		...
		It("global.internal.cniMigrationEnabled should be true and validation ignore removed", func() {
			...
			Expect(f.ValuesGet("global.internal.cniMigrationValidationIgnore").Exists()).To(BeFalse())
		})
	})
	...
	Context("CNIMigration resource deleted", func() {
		BeforeEach(func() {
			...
			f.ValuesSet("global.internal.cniMigrationValidationIgnore", true)
			f.BindingContexts.Set(f.KubeStateSet(``))
			f.RunHook()
		})
