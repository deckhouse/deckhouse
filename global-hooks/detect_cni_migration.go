...
func detectCNIMigration(_ context.Context, input *go_hook.HookInput) error {
	...
	for _, s := range snapshots {
		...
		if !found || info.Created < activeMigration.Created || (info.Created == activeMigration.Created && info.Name < activeMigration.Name) {
	...
	input.Logger.Info(fmt.Sprintf("Active CNI migration detected: %s (Succeeded: %v)", activeMigration.Name, activeMigration.IsSucceeded))
