...
func detectCNIMigration(_ context.Context, input *go_hook.HookInput) error {
	...
	if len(snapshots) == 0 {
		...
		input.Values.Remove("global.internal.cniMigrationValidationIgnore")
		return nil
	}
	...
	if activeMigration.IsSucceeded {
		...
		// We keep 'cniMigrationEnabled' true so the Manager and RBAC remain active to show the success status.
		input.Values.Remove("global.internal.cniMigrationValidationIgnore")
	} else {
		...
		input.Values.Set("global.internal.cniMigrationValidationIgnore", true)
	}
