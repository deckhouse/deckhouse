...
const (
	...
	defaultGatewayConfigmapNamespace = "d8-alb"
...
func setDiscoveredDefaultGateway(_ context.Context, input *go_hook.HookInput) error {
	if len(input.Snapshots.Get(configmapSnapshot)) == 1 {
		...
		if len(defaultGW[0].Name) == 0 || len(defaultGW[0].Namespace) == 0 {
			input.Logger.Warn(fmt.Sprintf("could not detect default gateway - configmap %s/%s doesn't contain any meaningful information", defaultGatewayConfigmapNamespace, defaultGatewayConfigmapName))
		}
	...
	input.Values.Set(discoveryDefaultGatewayPath, GatewayDesc{})
