...
func (n *kubeNgGetter) NodeGroups(ctx context.Context) ([]*v1.NodeGroup, error) {
	...
	err := retry.NewSilentLoop("get machine failed events", 6, 1500*time.Millisecond).RunContext(ctx, func() error {
...
func (n *clusterIsBootstrapCheck) hasBootstrappedCM(ctx context.Context) (bool, error) {
	hasCm := false
	err := retry.NewSilentLoop("get is-bootstrapped cm", 6, 1500*time.Millisecond).RunContext(ctx, func() error {
		kubeCl, err := n.kubeProvider.KubeClientCtx(ctx)
		if err != nil {
			return err
		}
