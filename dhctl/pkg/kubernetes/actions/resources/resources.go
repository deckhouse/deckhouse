...
func (c *Creator) ensureRequiredNamespacesExist(ctx context.Context) (map[int]struct{}, error) {
	...
	err := retry.NewSilentLoop("Ensure that required namespaces exist", 20, 5*time.Second).RunContext(ctx, func() error {
...
func (c *Creator) createSingleResource(ctx context.Context, resource *template.Resource, apires metav1.APIResource) error {
	...
	return retry.NewSilentLoop(fmt.Sprintf("Create %s resources", resource.GVK.String()), 120, 5*time.Second).RunContext(ctx, func() error {
...
func (c *Creator) runSingleMCTask(ctx context.Context, task actions.ModuleConfigTask) error {
	// Wait up to 10 minutes
	return retry.NewLoop(task.Title, 120, 2500*time.Millisecond).RunContext(ctx, func() error {
