...
`
	...
	t.Run("By default not ready", func(t *testing.T) {
		...
		for _, resource := range resources {
			...
			resourceIsReadyCheckerFactory, err := newKubedogResourceIsReadyCheckerFactory(fakeClient)
			require.NoError(t, err)

			checker, err := resourceIsReadyCheckerFactory.getChecker(resource)
			require.NoError(t, err)
