...
func checkAndRestartDeployment(ctx context.Context, kubeClProvider kubernetes.KubeClientProviderWithCtx, deploymentName string) error {
	...
	err := retry.NewLoop(fmt.Sprintf("Check deployment %s/%s exists", global.D8SystemNamespace, deploymentName), 20, 2500*time.Millisecond).
	...
	err = retry.NewLoop(fmt.Sprintf("Restart deployment %s/%s with adding annotation", global.D8SystemNamespace, deploymentName), 20, 2500*time.Millisecond).
		RunContext(ctx, func() error {
			kubeCl, err := kubeClProvider.KubeClientCtx(ctx)
			if err != nil {
				return err
