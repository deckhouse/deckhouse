...
func testAPIServerConfiguration(t *testing.T) {
	...
	t.Run("Audit Configuration - File Output", func(t *testing.T) {
		...
		if !strings.Contains(result, "/var/log/audit/audit.log") {
	...
	t.Run("Bind Address Configuration", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				data := getBaseTemplateData("1.31")
				data["apiserver"] = tt.apiserver
				if tt.nodeIP != "" {
					data["nodeIP"] = tt.nodeIP
				}
				...
				if matches[1] != tt.expectedAddr {
					t.Errorf("Expected bind address %s, got %s", tt.expectedAddr, matches[1])
				}
			})
		}
	})
	...
	t.Run("Secret Encryption", func(t *testing.T) {
		data := getBaseTemplateData("1.30") // v1beta3 supports encryption config
		...
		// v1beta3 format should include encryption-provider-config
...
func testServiceAccountConfiguration(t *testing.T) {
	...
	t.Run("Custom Service Account Issuer", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
		data["apiserver"] = map[string]interface{}{
	...
	t.Run("Additional API Issuers", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
		data["apiserver"] = map[string]interface{}{
			"serviceAccount": map[string]interface{}{
	...
	t.Run("Additional API Audiences", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
		data["apiserver"] = map[string]interface{}{
			"serviceAccount": map[string]interface{}{
				"issuer": "https://primary.issuer.com",
...
func testETCDConfiguration(t *testing.T) {
	t.Run("No ETCD Configuration", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
	...
	t.Run("Existing Cluster ETCD", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
	...
	t.Run("ETCD with Quota", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
		data["etcd"] = map[string]interface{}{
			"existingCluster":   true,
...
func testOptionalArguments(t *testing.T) {
	t.Run("Node Monitor Arguments", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
	...
	t.Run("Pod Eviction Timeout", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
		data["arguments"] = map[string]interface{}{
...
func testEdgeCases(t *testing.T) {
	t.Run("Complex Configuration Combination", func(t *testing.T) {
		data := getBaseTemplateData("1.31")
...
func testMissingCoverage(t *testing.T) {
	t.Run("Runtime Config Version Condition", func(t *testing.T) {
		// Test >= 1.28 condition for runtime-config in v1beta4
		data := getBaseTemplateData("1.31")
		result, err := renderKubeadmConfig(data)
		if err != nil {
			t.Fatalf("Failed to render kubeadm config: %v", err)
	...
	t.Run("No NodeIP Configuration", func(t *testing.T) {
		// Test case when nodeIP is not provided
		data := getBaseTemplateData("1.31")
		delete(data, "nodeIP")
		...
		// Should still render successfully without nodeIP
		if !strings.Contains(result, "kind: ClusterConfiguration") {
			t.Error("Expected ClusterConfiguration not found")
		}

		// InitConfiguration and JoinConfiguration should not have advertiseAddress
		if strings.Contains(result, "advertiseAddress:") {
			t.Error("Unexpected advertiseAddress found when nodeIP is not set")
		}
	})

	t.Run("Patches Without NodeIP", func(t *testing.T) {
		// Test patches rendering when nodeIP is not set
		data := getBaseTemplateData("1.31")
		delete(data, "nodeIP")
		data["images"] = map[string]interface{}{
			"controlPlaneManager": map[string]interface{}{
		...
		// kube-apiserver patch should still render without nodeIP-specific configurations
		apiserverPatch, exists := patches["kube-apiserver.yaml"]
		if !exists {
			t.Error("Expected kube-apiserver patch not found")
		}

		// Should not contain nodeIP-specific host configurations
	...
	t.Run("Service Account Edge Cases", func(t *testing.T) {
		// Test service account without issuer
		data := getBaseTemplateData("1.31")
		data["apiserver"] = map[string]interface{}{
			"serviceAccount": map[string]interface{}{
				"additionalAPIIssuers": []string{
		...
		// Should use default issuer when custom issuer is not set
	...
	t.Run("ETCD Without Existing Cluster", func(t *testing.T) {
		// Test etcd configuration with existingCluster = false
		data := getBaseTemplateData("1.31")
		data["etcd"] = map[string]interface{}{
		...
		// Should not contain etcd local configuration
		if strings.Contains(result, "initial-cluster-state: existing") {
			t.Error("Unexpected etcd existing cluster configuration found")
		}
	})

	t.Run("Audit Volume Mount Edge Cases", func(t *testing.T) {
		// Test audit configuration without file output
		data := getBaseTemplateData("1.31")
		data["apiserver"] = map[string]interface{}{
			"auditPolicy": "some-policy",
			// No auditLog specified, should not create volume mount
		}
		...
		// Should not contain audit log volume mount
