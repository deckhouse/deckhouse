...
func (f *SSHFile) Upload(srcPath, remotePath string) error {
	...
	if fType != "DIR" {
		...
		for _, file := range files {
			err = f.Upload(srcPath+"/"+file.Name(), remotePath+"/"+file.Name())
			if err != nil {
				return err
			}
		}
...
func (f *SSHFile) UploadBytes(data []byte, remotePath string) error {
	...
	err = f.Upload(srcPath, remotePath)
	return err
}

func (f *SSHFile) Download(remotePath, dstPath string) error {
	...
	if fType != "directory" {
		...
		for _, file := range files {
			f.Download(remotePath+"/"+file, dstPath+"/"+file)
		}
	}
...
func (f *SSHFile) DownloadBytes(remotePath string) ([]byte, error) {
	...
	err = f.Download(remotePath, dstPath)
	if err != nil {
