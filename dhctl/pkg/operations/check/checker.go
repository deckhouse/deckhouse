...
func (c *Checker) Check(ctx context.Context) (*CheckResult, error) {
	...
	metaConfig, err := commander.ParseMetaConfig(c.StateCache, c.ClusterConfigurationData, c.ProviderClusterConfigurationData)
	if err != nil {
...
func (c *Checker) checkConfiguration(ctx context.Context, kubeCl *client.KubernetesClient, metaConfig *config.MetaConfig) (CheckStatus, error) {
	...
	if bytes.Equal(clusterConfigurationData, inClusterConfigurationData) && bytes.Equal(providerClusterConfigurationData, inClusterProviderClusterConfigurationData) {
