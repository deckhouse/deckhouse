	...
	checkers := []hook.NodeChecker{
		...
		NewKubeProxyChecker().
			...
			WithSSHCredentials(session.Input{
				User:        kubeCl.SSHClient.Settings.User,
				Port:        kubeCl.SSHClient.Settings.Port,
				BastionHost: kubeCl.SSHClient.Settings.BastionHost,
				BastionPort: kubeCl.SSHClient.Settings.BastionPort,
				BastionUser: kubeCl.SSHClient.Settings.BastionUser,
				ExtraArgs:   kubeCl.SSHClient.Settings.ExtraArgs,
				BecomePass:  kubeCl.SSHClient.Settings.BecomePass,
			}, kubeCl.SSHClient.PrivateKeys...),
...
func (h *HookForUpdatePipeline) AfterAction(runner terraform.RunnerInterface) error {
	...
	h.kubeCl.SSHClient.Settings.AddAvailableHosts(outputs.MasterIPForSSH)
