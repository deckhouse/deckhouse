...
func (c *MasterNodeGroupController) addNodes(ctx *context.Context) error {
	...
	if len(masterIPForSSHList) > 0 {
		log.DebugF("Updating master hosts cache with %d new masters", len(masterIPForSSHList))

		// Get current master hosts from cache
		stateCache := ctx.StateCache()
		currentHosts, err := state.GetMasterHostsIPs(stateCache)
		if err != nil {
			log.DebugF("Could not load current master hosts from cache (this is OK for first master): %v", err)
		...
		for _, newHost := range masterIPForSSHList {
			...
			log.DebugF("Adding new master to cache: %s -> %s", newHost.Name, newHost.Host)
		}

		log.DebugF("Saving updated master hosts to cache: %v", hostsMap)

		state.SaveMasterHostsToCache(stateCache, hostsMap)

		log.DebugF("Successfully updated master hosts cache with %d new masters. hostsMap: %v", len(masterIPForSSHList), hostsMap)
	}
...
func (c *MasterNodeGroupController) updateNode(ctx *context.Context, nodeName string) error {
	...
	if outputs.MasterIPForSSH != "" {
		log.DebugF("Updating master hosts cache: node %s got IP %s", nodeName, outputs.MasterIPForSSH)
		...
		log.DebugF("Successfully updated master hosts cache with node %s IP %s. hostsMap: %v", nodeName, outputs.MasterIPForSSH, hostsMap)
	} else {
...
func (c *MasterNodeGroupController) deleteNodes(ctx *context.Context, nodesToDeleteInfo []nodeToDeleteInfo) error {
	...
	return log.Process("converge", title, func() error {
		...
		if err == nil && len(nodesToDelete) > 0 {
			log.DebugF("Updating master hosts cache after deleting %d masters: %v", len(nodesToDelete), nodesToDelete)

			// Get current master hosts from cache
			stateCache := ctx.StateCache()
			currentHosts, cacheErr := state.GetMasterHostsIPs(stateCache)
			if cacheErr != nil {
				log.DebugF("Could not load current master hosts from cache: %v", cacheErr)
				return err
			}
			...
			for _, deletedNode := range nodesToDelete {
				if _, exists := hostsMap[deletedNode]; exists {
					...
					log.DebugF("Removed deleted master from cache: %s", deletedNode)
				}
			}

			log.DebugF("Saving updated master hosts to cache after deletion: %v", hostsMap)

			state.SaveMasterHostsToCache(stateCache, hostsMap)

			log.DebugF("Successfully updated master hosts cache after deleting %d masters. hostsMap: %v", len(nodesToDelete), hostsMap)
		}
