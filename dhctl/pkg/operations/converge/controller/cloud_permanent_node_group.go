...
func (c *CloudPermanentNodeGroupController) addNodes(ctx *context.Context) error {
	...
	err = log.Process("terraform", fmt.Sprintf("Pipelines %s for %s-%s-%v", c.layoutStep, metaConfig.ClusterPrefix, c.name, nodesIndexToCreate), func() error {
		var err error
		nodesToWait, err = operations.ParallelBootstrapAdditionalNodes(ctx.KubeClient(), metaConfig, nodesIndexToCreate, c.layoutStep, c.name, c.cloudConfig, true, ctx.Terraform())
		return err
	})
	if err != nil {
		return err
...
func (c *CloudPermanentNodeGroupController) updateNode(ctx *context.Context, nodeName string) error {
	...
	err = entity.SaveNodeTerraformState(ctx.KubeClient(), nodeName, c.name, outputs.TerraformState, nodeGroupSettingsFromConfig)
	if err != nil {
		return err
	}
