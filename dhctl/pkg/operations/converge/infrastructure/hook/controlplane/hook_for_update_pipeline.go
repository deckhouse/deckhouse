...
func (h *HookForUpdatePipeline) AfterAction(ctx context.Context, runner infrastructure.RunnerInterface) error {
	...
	err = retry.NewLoop("Check Stronghold readiness after node converge", 90, 5*time.Second).RunContext(ctx, func() error {
	...
	return retry.NewLoop(fmt.Sprintf("Check control-plane is ready on node '%s'", h.nodeToConverge), 90, 5*time.Second).RunContext(ctx, func() error {
...
func (h *HookForUpdatePipeline) saveKubernetesDataDevicePath(ctx context.Context, devicePath string) error {
	...
	return retry.NewLoop(fmt.Sprintf("Save Kubernetes data device path for node '%s'", h.nodeToConverge), 90, 5*time.Second).
