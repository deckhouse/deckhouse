...
func CheckPreventBreakAnotherBootstrappedCluster(
	...
	return retry.NewSilentLoop("Check to prevent breaking another bootstrapped cluster", 30, 1500*time.Millisecond).RunContext(ctx, func() error {
...
func WaitForFirstMasterNodeBecomeReady(ctx context.Context, kubeCl *client.KubernetesClient) error {
	...
	var nodeName string
	err := retry.NewSilentLoop("Get master node name", 90, 1500*time.Millisecond).RunContext(ctx, func() error {
