...
func (d *ClusterDestroyer) DestroyCluster(autoApprove bool) error {
	...
	if !d.skipResources {
		if shouldStop, err := d.PhasedExecutionContext.Start(phases.DeleteResourcesPhase, false); err != nil {
			return err
		} else if shouldStop {
			return nil
		}
		...
		if err := d.PhasedExecutionContext.Stop(d.stateCache); err != nil {
			return err
		}
	}
	...
	d.state.Clean()
	return d.PhasedExecutionContext.Finalize()
}
