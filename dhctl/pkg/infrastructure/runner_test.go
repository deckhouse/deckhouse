...
func TestCheckPlanDestructiveChanges(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Has destructive changes but master is untouched",
			plan:    "./mocks/checkplan/destructively_changed.json",
			changes: nil,
			err:     nil,
		},
		{
			name:    "Has destructive changes with master_vm_marker",
			plan:    "./mocks/checkplan/destructively_changed_master.json",
...
var destructivelyChanged = &PlanDestructiveChanges{
	...
	ResourcesRecreated: []ValueChange{
		...
		},
		{
			Type: "null_resource",
			CurrentValue: map[string]any{
				"id": "3133818792804416154",
				"triggers": map[string]any{
					"instance_id": "fv4tjo7velb1tho5p4rr",
				},
				"timeouts": nil,
			},
			NextValue: map[string]any{
