...
func (pc *Checker) CheckCloudMasterNodeSystemRequirements() error {
	...
	if err = validateIntegerPropertyAtPath(configObject, rootDiskPropertyPath, minimumRequiredRootDiskSizeGB, externalDiskSizeDefault); err != nil {
		return fmt.Errorf("Root disk capacity: %v", err)
	}
	if err = validateIntegerPropertyAtPath(configObject, ramAmountPropertyPath, minimumRequiredMemoryMB, 0); err != nil {
		return fmt.Errorf("RAM amount: %v", err)
	}
	if err = validateIntegerPropertyAtPath(configObject, coreCountPropertyPath, minimumRequiredCPUCores, 0); err != nil {
...
func validateIntegerPropertyAtPath(configObject map[string]any, propertyPath []string, minimalValue, externalDefault int) error {
