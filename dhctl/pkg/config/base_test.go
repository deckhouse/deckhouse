	...
	t.Run("Registry", func(t *testing.T) {
		t.Run("InitConfiguration -> always unmanaged && legacy", func(t *testing.T) {
			t.Run("Without CRI (module disable)", func(t *testing.T) {
				metaConfig, err := ParseConfigFromData(t.Context(), initConfig, DummyPreparatorProvider(), &options.New().Global)
			...
			t.Run("With CRI (module enable)", func(t *testing.T) {
				metaConfig, err := ParseConfigFromData(t.Context(), initConfig+clusterConfig, DummyPreparatorProvider(), &options.New().Global)
				require.NoError(t, err)
				require.Equal(t, true, metaConfig.Registry.LegacyMode)
				require.Equal(t, registry_const.ModeUnmanaged, metaConfig.Registry.Settings.Mode)
				registry := metaConfig.Registry.Settings.RemoteData
		...
		t.Run("Default -> CE edition registry", func(t *testing.T) {
			t.Run("Without CRI (module disable) -> unmanaged && legacy", func(t *testing.T) {
				metaConfig, err := ParseConfigFromData(t.Context(), "", DummyPreparatorProvider(), &options.New().Global)
				require.NoError(t, err)
				require.Equal(t, true, metaConfig.Registry.LegacyMode)
				require.Equal(t, registry_const.ModeUnmanaged, metaConfig.Registry.Settings.Mode)
				registry := metaConfig.Registry.Settings.RemoteData
			...
			t.Run("With CRI (module enable) -> direct && not legacy", func(t *testing.T) {
				metaConfig, err := ParseConfigFromData(t.Context(), ""+clusterConfig, DummyPreparatorProvider(), &options.New().Global)
				require.NoError(t, err)
...
`
			t.Run("Without CRI (module disable) -> error", func(t *testing.T) {
				_, err := ParseConfigFromData(t.Context(), moduleConfigDeckhouse, DummyPreparatorProvider(), &options.New().Global)
				require.Error(t, err)
			})
			t.Run("With CRI (module enable) -> from moduleConfig && not legacy", func(t *testing.T) {
				metaConfig, err := ParseConfigFromData(t.Context(), moduleConfigDeckhouse+clusterConfig, DummyPreparatorProvider(), &options.New().Global)
				require.NoError(t, err)
				require.Equal(t, false, metaConfig.Registry.LegacyMode)
				require.Equal(t, registry_const.ModeUnmanaged, metaConfig.Registry.Settings.Mode)
				registry := metaConfig.Registry.Settings.RemoteData
	...
	t.Run("Standard Static", func(t *testing.T) {
		metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig, DummyPreparatorProvider(), &options.New().Global)
		require.NoError(t, err)
	...
	t.Run("Static with StaticClusterConfig", func(t *testing.T) {
		metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig, DummyPreparatorProvider(), &options.New().Global)
		require.NoError(t, err)
	...
	t.Run("Module config", func(t *testing.T) {
		t.Run("Global valid", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+moduleConfigGlobalValid, DummyPreparatorProvider(), &options.New().Global)
			require.NoError(t, err)
		...
		t.Run("Global invalid", func(t *testing.T) {
			_, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+moduleConfigGlobalInvalid, DummyPreparatorProvider(), &options.New().Global)
			require.Error(t, err)
		})

		t.Run("Module valid", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+moduleConfigCommonValid, DummyPreparatorProvider(), &options.New().Global)
		...
		t.Run("Module invalid", func(t *testing.T) {
			_, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+moduleConfigCommonInvalid, DummyPreparatorProvider(), &options.New().Global)
			require.Error(t, err)
		})

		t.Run("Module without enabled field", func(t *testing.T) {
			_, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+moduleConfigCommonWithoutEnabled, DummyPreparatorProvider(), &options.New().Global)
			require.Error(t, err)
		})

		t.Run("Module without settings", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+moduleConfigCommonWithoutSettings, DummyPreparatorProvider(), &options.New().Global)
			require.NoError(t, err)
		...
		t.Run("Unknown module should move into resources", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+unknownModuleConfig, DummyPreparatorProvider(), &options.New().Global)
			require.NoError(t, err)
	...
	t.Run("Config with another k8s resources eg configMap", func(t *testing.T) {
		t.Run("Should move another resources into resourcesYAML", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+configMapAndInstanceClass, DummyPreparatorProvider(), &options.New().Global)
			require.NoError(t, err)
		...
		t.Run("Should move resourcesYAML", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(t.Context(), clusterConfig+initConfig+staticConfig+ngWithTemplating, DummyPreparatorProvider(), &options.New().Global)
			require.NoError(t, err)
...
func TestParseConfigFromFiles(t *testing.T) {
	t.Run("parse wildcard", func(t *testing.T) {
		metaConfig, err := LoadConfigFromFile(t.Context(), []string{"./mocks/*.yml", "./mocks/3-ModuleConfig.yaml"}, DummyPreparatorProvider(), &options.New().Global)
		require.NoError(t, err)
		require.Equal(t, "Static", metaConfig.ClusterType)
...
`
		...
		_, err := ParseConfigFromData(t.Context(), configWithCommentedSeparator, DummyPreparatorProvider(), &options.New().Global)
		require.Error(t, err)
...
`
		...
		_, err := ParseConfigFromData(t.Context(), configWithoutSeparator, DummyPreparatorProvider(), &options.New().Global)
		require.Error(t, err)
		require.Contains(t, err.Error(), "missing '---' separator")
	})
...
`
		...
		metaConfig, err := ParseConfigFromData(t.Context(), validConfig, DummyPreparatorProvider(), &options.New().Global)
		require.NoError(t, err)
		require.NotNil(t, metaConfig)
...
`
		...
		metaConfig, err := ParseConfigFromData(t.Context(), configWithComment, DummyPreparatorProvider(), &options.New().Global)
		require.NoError(t, err)
		require.NotNil(t, metaConfig)
	})
}
