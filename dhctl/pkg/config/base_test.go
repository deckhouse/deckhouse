...
`
	...
	t.Run("Standard Static", func(t *testing.T) {
		metaConfig, err := ParseConfigFromData(clusterConfig+initConfig, DummyPreparatorProvider())
	...
	t.Run("Without init configuration", func(t *testing.T) {
		metaConfig, err := ParseConfigFromData(clusterConfig, DummyPreparatorProvider())
		require.NoError(t, err)
	...
	t.Run("Static with StaticClusterConfig", func(t *testing.T) {
		metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig, DummyPreparatorProvider())
		require.NoError(t, err)
	...
	t.Run("Module config", func(t *testing.T) {
		t.Run("Global valid", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+moduleConfigGlobalValid, DummyPreparatorProvider())
			require.NoError(t, err)
		...
		t.Run("Global invalid", func(t *testing.T) {
			_, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+moduleConfigGlobalInvalid, DummyPreparatorProvider())
			require.Error(t, err)
		})

		t.Run("Module valid", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+moduleConfigCommonValid, DummyPreparatorProvider())
		...
		t.Run("Module invalid", func(t *testing.T) {
			_, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+moduleConfigCommonInvalid, DummyPreparatorProvider())
			require.Error(t, err)
		})

		t.Run("Module without enabled field", func(t *testing.T) {
			_, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+moduleConfigCommonWithoutEnabled, DummyPreparatorProvider())
			require.Error(t, err)
		})

		t.Run("Module without settings", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+moduleConfigCommonWithoutSettings, DummyPreparatorProvider())
			require.NoError(t, err)
		...
		t.Run("Unknown module should move into resources", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+unknownModuleConfig, DummyPreparatorProvider())
			require.NoError(t, err)
	...
	t.Run("Config with another k8s resources eg configMap", func(t *testing.T) {
		t.Run("Should move another resources into resourcesYAML", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+configMapAndInstanceClass, DummyPreparatorProvider())
			require.NoError(t, err)
		...
		t.Run("Should move resourcesYAML", func(t *testing.T) {
			metaConfig, err := ParseConfigFromData(clusterConfig+initConfig+staticConfig+ngWithTemplating, DummyPreparatorProvider())
			require.NoError(t, err)
...
func TestParseConfigFromFiles(t *testing.T) {
	...
	t.Run("parse wildcard", func(t *testing.T) {
		metaConfig, err := LoadConfigFromFile([]string{"./mocks/*.yml", "./mocks/3-ModuleConfig.yaml"}, DummyPreparatorProvider())
		require.NoError(t, err)
		require.Equal(t, "Static", metaConfig.ClusterType)
