...
`,
	...
	// These use a directly-constructed MetaConfig (bypassing YAML parsing and
	// schema validation) rather than generateMetaConfigForDeckhouseConfigTest:
	// PrepareDeckhouseInstallConfig's global.modules.ingress.enabled defaulting
	// operates purely on already-parsed *ModuleConfig values, so exercising it
	// this way keeps the test independent of the "deckhouse"/"global" schemas
	// being available on disk (parsing them from YAML is covered elsewhere,
	// e.g. TestModuleDeckhouseConfigRegistryOverrides). A "deckhouse" MC is
	// always included so the deckhouse-defaulting block above doesn't need to
	// build one via buildModuleConfig, which does require the schema.
	newDeckhouseMc := func() *ModuleConfig {
		mc := &ModuleConfig{
	...
	findGlobalMc := func(mcs []*ModuleConfig) *ModuleConfig {
		for _, mc := range mcs {
	...
	t.Run("Default global.modules.ingress.enabled to false when global mc omits it", func(t *testing.T) {
		globalMcIn := &ModuleConfig{
			Spec: ModuleConfigSpec{
				Settings: map[string]any{
		...
		globalMcIn.SetName("global")

		metaConfig := &MetaConfig{ModuleConfigs: []*ModuleConfig{newDeckhouseMc(), globalMcIn}}

		iCfg, err := PrepareDeckhouseInstallConfig(t.Context(), metaConfig, &options.New().Global)
		require.NoError(t, err)

		globalMc := findGlobalMc(iCfg.ModuleConfigs)
		require.NotNil(t, globalMc)

		modules, ok := globalMc.Spec.Settings["modules"].(map[string]any)
		require.True(t, ok)
		ingress, ok := modules["ingress"].(map[string]any)
		require.True(t, ok)
		require.Equal(t, false, ingress["enabled"])

		// The rest of the user's settings must be left untouched.
		require.Equal(t, "%s.example.com", globalMc.Spec.Settings["publicDomainTemplate"])
	})

	t.Run("Add ingress.enabled: false when modules is set but ingress is absent", func(t *testing.T) {
		globalMcIn := &ModuleConfig{
			Spec: ModuleConfigSpec{
				Settings: map[string]any{
		...
		require.True(t, ok)
		require.Equal(t, false, ingress["enabled"])
		require.Equal(t, "%s.example.com", modules["publicDomainTemplate"])
	})

	t.Run("Keep user's global.modules.ingress.enabled when already set", func(t *testing.T) {
		globalMcIn := &ModuleConfig{
			Spec: ModuleConfigSpec{
				Settings: map[string]any{
					"modules": map[string]any{
		...
		ingress, ok := modules["ingress"].(map[string]any)
		require.True(t, ok)
		require.Equal(t, true, ingress["enabled"])
	})

	t.Run("Do not synthesize a global mc when none was provided", func(t *testing.T) {
		metaConfig := &MetaConfig{ModuleConfigs: []*ModuleConfig{newDeckhouseMc()}}

		iCfg, err := PrepareDeckhouseInstallConfig(t.Context(), metaConfig, &options.New().Global)
		require.NoError(t, err)

		require.Nil(t, findGlobalMc(iCfg.ModuleConfigs))
	})
