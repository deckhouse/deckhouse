	...
	cmd.Action(func(context *kingpin.ParseContext) error {
		if app.MirrorRegistry != "" {
			...
			return log.Process("mirror", "Push Modules to registry", func() error {
				return operations.PushModulesToRegistry(app.MirrorModuleDirectory, app.MirrorRegistry, authProvider, app.MirrorInsecure, app.MirrorTLSSkipVerify, app.MirrorJobs)
			})
		}

		return log.Process("mirror", "Pull Modules to local filesystem", func() error {
			return operations.PullExternalModulesToLocalFS(app.MirrorModuleSourcePath, app.MirrorModuleDirectory, app.MirrorTLSSkipVerify, app.MirrorJobs)
		})
	})
