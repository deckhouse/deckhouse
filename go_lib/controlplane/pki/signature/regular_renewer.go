...
func generateNewSignatureCerts(filePath string, k8sInterface kubernetes.Interface, isInitial bool) error {
	...
	return replaceSignatureFiles(filePath, jwkJSON, jwksJSON)
}
...
func generateTLSCertificate(serviceName, clusterDomain string, keyType CertKeyType, durationInDays int) (*tls.Certificate, error) { //nolint:unparam
	now := time.Now()
