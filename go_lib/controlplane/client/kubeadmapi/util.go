...
func SetDefaultTimeouts(t **Timeouts) {
	...
	if (*t).ControlPlaneComponentHealthCheck == nil {
		(*t).ControlPlaneComponentHealthCheck = &metav1.Duration{Duration: 4 * time.Minute}
	}
	if (*t).KubeletHealthCheck == nil {
		(*t).KubeletHealthCheck = &metav1.Duration{Duration: 4 * time.Minute}
	}
	if (*t).KubernetesAPICall == nil {
		(*t).KubernetesAPICall = &metav1.Duration{Duration: 1 * time.Minute}
	}
	if (*t).EtcdAPICall == nil {
		(*t).EtcdAPICall = &metav1.Duration{Duration: 2 * time.Minute}
	}
	if (*t).TLSBootstrap == nil {
		(*t).TLSBootstrap = &metav1.Duration{Duration: 5 * time.Minute}
	}
	if (*t).Discovery == nil {
		(*t).Discovery = &metav1.Duration{Duration: 5 * time.Minute}
	}
	if (*t).UpgradeManifests == nil {
		(*t).UpgradeManifests = &metav1.Duration{Duration: 5 * time.Minute}
	}
}
