...
func TestDeployDelayReason(t *testing.T) {
	...
	require.Equal(t, "noDelay", reason.Message(nil, zeroTime))
	...
	require.Equal(t, "Release is waiting for the update window", reason.Message(nil, zeroTime))
	...
	require.Equal(t, "Release is waiting for the update window, waiting for the 'cannot find approval annotation: unsupported release type <nil>: \"true\"' annotation", reason.Message(nil, zeroTime))
