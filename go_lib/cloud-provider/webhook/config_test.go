...
func TestInitFlagsNilConfig(t *testing.T) {
	...
	InitServerFlags(pflag.NewFlagSet("test", pflag.ExitOnError), nil)
}
...
func TestInitFlagsPopulatesConfig(t *testing.T) {
	...
	InitServerFlags(fs, &cfg)

	if err := fs.Parse([]string{"--webhook-port=9443", "--webhook-cert-dir=/tmp/certs", "--metrics-bind-address=:9090", "--health-probe-bind-address=:9091"}); err != nil {
