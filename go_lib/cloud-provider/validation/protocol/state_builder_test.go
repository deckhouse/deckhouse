...
func TestStateBuilderBuildDhctlSettingsMap(t *testing.T) {
	...
	if state.MigrationStatus.MigrationPending {
		t.Fatalf("Build() migration status = %#v, want zero value when no ProviderClusterConfig is set", state.MigrationStatus)
	}
}

func TestStateBuilderBuildWithCompleteResources(t *testing.T) {
	t.Parallel()
