...
// The alert (D8CloudProviderDVPMigrationPending and etc) must not fire in clusters managed by
...
func RegisterHook(
	migrationConfigMapName string,
	migrationConfigMapNamespace string,
	migrationPendingMetricName string,
	migrationPendingMetricGroup string,
) bool {
		...
		Kubernetes: []go_hook.KubernetesConfig{
			{
				...
				NamespaceSelector: &types.NamespaceSelector{
					NameSelector: &types.NameSelector{
						MatchNames: []string{migrationConfigMapNamespace},
					},
				},
				NameSelector: &types.NameSelector{
					MatchNames: []string{migrationConfigMapName},
				},
				ExecuteHookOnEvents:          ptr.To(true),
				ExecuteHookOnSynchronization: ptr.To(true),
				FilterFunc:                   filterMigrationMarkerConfigMap(migrationConfigMapName),
			},
			{
				Name:       "commander_uuid",
				ApiVersion: "v1",
	...
	}, handleMigrationPendingMetric(migrationPendingMetricName, migrationPendingMetricGroup))
}

// filterMigrationMarkerConfigMap reports the presence of the migration marker ConfigMap.
func filterMigrationMarkerConfigMap(migrationConfigMapName string) func(*unstructured.Unstructured) (go_hook.FilterResult, error) {
	return func(obj *unstructured.Unstructured) (go_hook.FilterResult, error) {
		if obj.GetName() != migrationConfigMapName {
			return nil, nil
		}
...
func handleMigrationPendingMetric(
	migrationPendingMetricName string,
	migrationPendingMetricGroup string,
) func(context.Context, *go_hook.HookInput) error {
