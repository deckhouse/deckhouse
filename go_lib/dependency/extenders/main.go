...
func CheckModuleReleaseRequirements(moduleRelease string, requirements *v1alpha1.ModuleReleaseRequirements) error {
	...
	if len(requirements.ModuleReleasePlatformRequirements.Deckhouse) > 0 {
		if err := deckhouseversion.Instance().ValidateRelease(moduleRelease, requirements.ModuleReleasePlatformRequirements.Deckhouse); err != nil {
			return err
		}
	}

	if len(requirements.ModuleReleasePlatformRequirements.Kubernetes) > 0 {
		if err := kubernetesversion.Instance().ValidateRelease(moduleRelease, requirements.ModuleReleasePlatformRequirements.Kubernetes); err != nil {
			return err
		}
	}
