...
func (e *Extender) AddConstraint(name, rawConstraint string) error {
	if err := e.versionMatcher.AddConstraint(name, rawConstraint); err != nil {
		e.logger.Debugf("adding installed constraint for '%s' module failed", name)
		return err
	}
	e.logger.Debugf("installed constraint for '%s' module is added", name)
	return nil
}
...
func (e *Extender) Filter(name string, _ map[string]string) (*bool, error) {
	...
	if err := e.versionMatcher.Validate(name); err != nil {
		e.logger.Errorf("requirements of '%s' module are not satisfied: current deckhouse version is not suitable: %s", name, err.Error())
...
func (e *Extender) ValidateBaseVersion(baseVersion string) (string, error) {
	if name, err := e.versionMatcher.ValidateBaseVersion(baseVersion); err != nil {
		e.logger.Errorf("requirements of '%s' module are not satisfied: %s deckhouse version is not suitable: %s", name, baseVersion, err.Error())
		return name, fmt.Errorf("requirements of '%s' module are not satisfied: %s deckhouse version is not suitable: %s", name, baseVersion, err.Error())
	}
...
func (e *Extender) ValidateRelease(releaseName, rawConstraint string) error {
	...
	if err := e.versionMatcher.ValidateConstraint(rawConstraint); err != nil {
		e.logger.Errorf("requirements of '%s' module release are not satisfied: current deckhouse version is not suitable: %s", releaseName, err.Error())
