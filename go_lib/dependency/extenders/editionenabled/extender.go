...
type Extender struct {
	edition string
	bundle  string
	modules map[string]*v1alpha1.ModuleAccessibility
	logger  *log.Logger
}
...
func New(edition, bundle string, logger *log.Logger) *Extender {
	return &Extender{
		edition: edition,
		bundle:  bundle,
		modules: make(map[string]*v1alpha1.ModuleAccessibility),
		logger:  logger,
	}
}
...
func (e *Extender) Filter(name string, _ map[string]string) (*bool, error) {
	...
	return ptr.To(false), fmt.Errorf("disable by the '%s' bundle", e.bundle)
}

func (e *Extender) AddModule(name string, access *v1alpha1.ModuleAccessibility) {
