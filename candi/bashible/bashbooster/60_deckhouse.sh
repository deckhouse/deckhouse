...
bb-deckhouse-get-disruptive-update-approval() {
    ...
    until
        node_data="$(
          bb-kubectl --kubeconfig=/etc/kubernetes/kubelet.conf get node $(bb-d8-node-name) -o json | jq '
          {
    ...
    attempt=0
    until
      bb-kubectl --kubeconfig=/etc/kubernetes/kubelet.conf get node $(bb-d8-node-name) -o json | \
