...
{{- $desired_version_docker := "" }}
{{- $allowed_versions_docker_pattern := "" }}
{{- $desired_version_containerd := "" }}
{{- $allowed_versions_containerd_pattern := "" }}
...
if bb-is-ubuntu-version? {{ $ubuntuVersion }} ; then
    {{- $desired_version_docker = $value.docker.desiredVersion }}
    {{- $allowed_versions_docker_pattern = $value.docker.allowedPattern }}
  ...
  allowed_versions_docker_pattern={{ $allowed_versions_docker_pattern | quote }}
    {{- if or $value.docker.containerd.desiredVersion $value.docker.containerd.allowedPattern }}
      {{- $desired_version_containerd = $value.docker.containerd.desiredVersion }}
      {{- $allowed_versions_containerd_pattern = $value.docker.containerd.allowedPattern }}
...
if [[ "$should_install_containerd" == true ]]; then
  ...
  if bb-is-ubuntu-version? 16.04 ; then
    containerd_tag="{{- index .images.registrypackages (printf "containerdUbuntu%sXenial" ($desired_version_containerd | replace "containerd.io=" "" | replace "." "" | replace "-" "")) }}"
  elif bb-is-ubuntu-version? 18.04 ; then
    containerd_tag="{{- index .images.registrypackages (printf "containerdUbuntu%sBionic" ($desired_version_containerd | replace "containerd.io=" "" | replace "." "" | replace "-" "")) }}"
  elif bb-is-ubuntu-version? 20.04 ; then
    containerd_tag="{{- index .images.registrypackages (printf "containerdUbuntu%sFocal" ($desired_version_containerd | replace "containerd.io=" "" | replace "." "" | replace "-" "")) }}"
  else
...
if [[ "$should_install_docker" == true ]]; then
  ...
  docker_tag="{{- index .images.registrypackages (printf "dockerUbuntu%s" ($desired_version_docker | replace "docker-ce=" "" | replace "." "_" | replace ":" "_" | replace "~" "_" | camelcase )) }}"
