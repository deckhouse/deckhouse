{{- $nvidia_docker := false }}
{{- if hasKey .nodeGroup "docker" }}
  {{- if .nodeGroup.docker.nvidia }}
    {{- $nvidia_docker = true }}
  {{- end }}
{{- end }}

{{- if $nvidia_docker }}
if bb-apt-repo? https://nvidia.github.io/nvidia-docker/ubuntu; then
  exit 0
fi

bb-apt-key-add <<END
-----BEGIN PGP PUBLIC KEY BLOCK-----


=lSYX
-----END PGP PUBLIC KEY BLOCK-----
END

bb-apt-repo-add deb https://nvidia.github.io/nvidia-docker/$(. /etc/os-release;echo $ID$VERSION_ID)/amd64 /
bb-apt-repo-add deb https://nvidia.github.io/nvidia-container-runtime/$(. /etc/os-release;echo $ID$VERSION_ID)/amd64 /
bb-apt-repo-add deb https://nvidia.github.io/libnvidia-container/$(. /etc/os-release;echo $ID$VERSION_ID)/amd64 /
{{- end }}
