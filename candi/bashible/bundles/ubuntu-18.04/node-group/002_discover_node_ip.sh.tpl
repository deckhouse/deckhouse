...
if [ -f /etc/kubernetes/kubelet.conf ] ; then
  if node="$(kubectl --kubeconfig=/etc/kubernetes/kubelet.conf get node $HOSTNAME -o json 2> /dev/null)" ; then
    ...
    >&2 echo "ERROR: Unable to discover node IP for node object: No access to API server"
    exit 1
  fi
fi
{{- end }}
...
# No .nodeGroup.internalNetworkCIDRs in Static node
echo "" > /var/lib/bashible/discovered-node-ip
  {{- else }}
# For Static node we use .nodeGroup.internalNetworkCIDRs
...
for cidr in {{ .nodeGroup.internalNetworkCIDRs | join " " }}; do
...
if [ -z "$(cat /var/lib/bashible/discovered-node-ip)" ] ; then
  >&2 echo "ERROR: Failed to discover node_ip but it's required for cluster bootstrap"
  exit 1
fi
  {{- end }}
{{- end }}
