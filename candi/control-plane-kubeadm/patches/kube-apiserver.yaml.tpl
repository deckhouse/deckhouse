...
spec:
  containers:
    - name: kube-apiserver
      image: {{ printf "%s@%s" .registry.imagesBase (index $.images.controlPlaneManager $imageWithVersion) }}
    {{- end }}
  {{- end }}
{{- end }}
...
spec:
  ...
  - name: kube-apiserver
    ...
    {{- end }}
        ...
        scheme: HTTP
  - name: healthcheck
    image: {{ printf "%s@%s" .registry.imagesBase (index $.images.controlPlaneManager "kubeApiserverHealthcheck") }}
    resources:
      requests:
