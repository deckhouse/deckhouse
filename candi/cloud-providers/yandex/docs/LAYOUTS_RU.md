...
Эта отдельная подсеть необходима для избежания петли маршрутизации. Это сеть не должна перескаться с другими сетями кластера.

Для использования уже существующей сети, необходимо использовать параметр `withNATInstance.internalSubnetID` — NAT-инстанс будет создан в зоне этого subnet.

При использовании параметра `withNATInstance.internalSubnetCIDR` будет создана новая внутренняя подсеть. NAT-инстанс будет создан в этой подсети.
