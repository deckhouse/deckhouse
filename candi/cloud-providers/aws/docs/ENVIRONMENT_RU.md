...
Затем проверьте есть ли у созданного пользователя доступ к нужным вам регионам. Для этого выберите регион в селекторе в верхнем правом углу и если увидите сообщение подобное этому

![enable region](REGION_ENABLE.png)

нажмите `Continue` для того, чтобы разрешить использование региона.
