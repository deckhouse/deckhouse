...
Схема размещения (layout) описывается структурой `AzureClusterConfiguration`:
* `apiVersion` — deckhouse.io/v1
* `kind` — AzureClusterConfiguration
...
* `masterNodeGroup` — спека для описания NG мастера.
    ...
    * `instanceClass` — частичное содержимое полей [AzureInstanceClass](cr.html#azureinstanceclass).
        * `machineSize`
        * `diskSizeGb`
        * `urn`
