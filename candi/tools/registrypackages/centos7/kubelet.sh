...
for VERSION in $(yq r /deckhouse/candi/version_map.yml -j | jq -r '.k8s | keys[] as $key | "\($key).\(.[$key] | .patch)"'); do
  mkdir package
  pushd package
  RPM_PACKAGE="https://packages.cloud.google.com/yum/repos/kubernetes-el7-x86_64/$(grep "kubelet-${VERSION}-0" <<< ${YUMDATA})"
  wget ${RPM_PACKAGE}
  RPM_PACKAGE_FILE="$(ls *kubelet*)"
  popd
...
EOF
  ...
  docker build -t ${REGISTRY_PATH}/kubelet:${VERSION}-centos7 .
  docker push ${REGISTRY_PATH}/kubelet:${VERSION}-centos7
