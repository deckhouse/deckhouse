...
for VERSION in $(yq r /deckhouse/candi/version_map.yml -j | jq -r '.k8s | keys[] as $key | "\(.[$key] | .cni_version)"' | sort | uniq); do
  ...
  wget ${DEB_PACKAGE}
  DEB_PACKAGE_FILE="$(ls kubernetes-cni*)"
  popd
...
#!/bin/bash
set -Eeo pipefail
apt-mark unhold kubernetes-cni
dpkg -r kubernetes-cni
EOF
...
EOF
  ...
  docker build -t ${REGISTRY_PATH}/kubernetes-cni:${VERSION}-ubuntu .
  docker push ${REGISTRY_PATH}/kubernetes-cni:${VERSION}-ubuntu
