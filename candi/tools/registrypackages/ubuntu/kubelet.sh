...
for VERSION in $(yq r /deckhouse/candi/version_map.yml -j | jq -r '.k8s | keys[] as $key | "\($key).\(.[$key] | .patch)"'); do
  mkdir package
  pushd package
  DEB_PACKAGE="https://packages.cloud.google.com/apt/$(curl https://packages.cloud.google.com/apt/dists/kubernetes-xenial/main/binary-amd64/Packages | grep kubelet_${VERSION}-00  | awk '{print $2}')"
  wget ${DEB_PACKAGE}
  DEB_PACKAGE_FILE="$(ls kubelet*)"
  popd
...
EOF
  ...
  docker build -t ${REGISTRY_PATH}/kubelet:${VERSION}-ubuntu .
  docker push ${REGISTRY_PATH}/kubelet:${VERSION}-ubuntu
