...
1. Выпуск версии
    1. Предварительный выкат
        ...
        1. Если при выкате на `alpha` произошли ошибки:
            ...
            1. Рассылаем клиентам уведомления об окончании выката (#TODO описание процесса рассылки).
...
1. Смена версии (релиза) на `beta`, `early-access`, `stable` и `rock-solid`.
    1. Уведомить в Slack.
        1. `beta` – создать отдельное сообщение в канале `#deckhouse-releases` в котором вставить ссылку на русскоязычный пост. Дернуть только дежурных команд. Сделать рассылку в клиентские каналы проектов, кластеры которых будут затронуты при выкате (#TODO описание процесса рассылки).
        1. `early-access`:
