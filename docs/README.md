...
* `allocatableResources` - количество CPU и memory, выделяемые для работы служебных компонентов.
  ...
  * `masterNode` - системные компоненты (control plane и системные компоненты на мастер-узлах).
    * `cpu` – сумма CPU, выделяемая для компонентов control-plane на мастер-узлах сверх `everyNode`. Значение по умолчанию определяется автоматически: `allocatableResources` минимального мастер-узла (но не более 4 ядер), минус `everyNode`.
    * `memory` – суммарный объем памяти, выделяемый для компонентов control-plane на мастер-узлах сверх `everyNode`. Значение по умолчанию определяется автоматически: `allocatableResources` минимального мастер-узла (но не более 8 Гб), минус `everyNode`.
