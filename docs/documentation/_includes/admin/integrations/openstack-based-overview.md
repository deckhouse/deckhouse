В этом разделе рассматривается настройка интеграции Deckhouse Platform (DP) с облачными ресурсами в частном облаке на базе [{{ site.data.admin.cloud-types.types[page.cloud_type].name }}]({{ site.data.admin.cloud-types.types[page.cloud_type].link }}).
