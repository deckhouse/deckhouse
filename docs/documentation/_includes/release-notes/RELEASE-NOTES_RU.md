...
- Детализирован [список портов, используемых при сетевом взаимодействии](https://deckhouse.ru/products/kubernetes-platform/documentation/v1.71/network_security_setup.html):
  - Добавлено и обновлено:
    ...
    - `4295–4299/UDP` — порты, используемые Cilium для VXLAN-инкапсуляции трафика между подами. Используемый порт зависит от **первоначально установленной** версии DKP (после обновления DKP порт не изменяется), а начиная с версии 1.71 — от уровня вложенности DVP (DKP внутри DVP, см. подробности ниже):
