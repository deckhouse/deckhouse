...
- Детализирован [список портов, используемых при сетевом взаимодействии](https://deckhouse.ru/products/kubernetes-platform/documentation/v1.71/network_security_setup.html):
  - Добавлено и обновлено:
    ...
    - `4295–4299/UDP` — порты, используемые Cilium для VXLAN-инкапсуляции межподового трафика в статичных кластерах (облачные кластеры в данном описании не рассматриваются). Используемый порт зависит от версии DKP и, начиная с 1.71, от уровня вложенности DVP:
      - DKP ≤ 1.63 — используется `8472/UDP`;
      - DKP 1.64–1.70 — используется `4299/UDP`, при включении модуля `virtualization` порт меняется на `4298/UDP`;
      - DKP ≥ 1.71 — используется `4298/UDP`, включение модуля `virtualization` порт не меняет;
      - DKP внутри DVP (вложенные кластеры), начиная с 1.71 — порт назначается по уровню вложенности:
        - уровень 0 — базовый DVP, развёрнутый на физическом кластере: `4298/UDP`;
        - уровень 1 — DVP, развёрнутый внутри другого DVP: `4297/UDP`;
        - уровень 2 — DVP, развёрнутый внутри DVP уровня 1: `4296/UDP`;
        - уровень 3 — DVP, развёрнутый внутри DVP уровня 2: `4295/UDP`;

      Как проверить текущий порт VXLAN:

      ```bash
      kubectl -n d8-cni-cilium  get cm cilium-config  -o yaml | grep tunnel
      ```
