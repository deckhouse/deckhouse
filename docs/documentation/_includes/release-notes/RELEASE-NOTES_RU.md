...
- Детализирован [список портов, используемых при сетевом взаимодействии](https://deckhouse.ru/products/kubernetes-platform/documentation/v1.71/network_security_setup.html):
  - Добавлено и обновлено:
    ...
    - `4295–4299/UDP` — порты, используемые Cilium для VXLAN-инкапсуляции межподового трафика в статичных кластерах (облачные кластеры в данном описании не рассматриваются). Используемый порт зависит от версии DKP и, начиная с 1.71, от уровня вложенности DVP (DKP внутри DVP, см. подробности ниже):
