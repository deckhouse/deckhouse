...
## Список впросов ко встрече:

Поддержка версий Kubernetes
  Обновление кластеров Kubernetes - надо? Только третий вопрос Лены: *Для обновления Куба в кластере нужно просто изменить параметр kubernetesVersion.*
...
Для обновления Куба в кластере нужно просто изменить [параметр `kubernetesVersion`](https://kubernetes.io/docs/tasks/administer-cluster/cluster-upgrade/). 

<!-- Обзор - доклад Давида ТРАНСКРИБАТОР! Кусок про обновление кластера Куба: Обновление кластера Kubernetes до версии 1.16 требует перезапуска всех узлов, что занимает некоторое время. У нас есть кластеры, созданные в версиях 17 и 16, которые успешно обновлены до версии 1.16. Мы также обновили наши критически важные кластеры, отвечающие за работу сервисов на железе, с версии 1.17 до 1.16 и планируем обновить их до версии 1.19.

Хотя у нас нет экспериментальных доказательств возможности обновления кластера, мы демонстрируем этот процесс на практике, используя патчи релизов для автоматического обновления кластеров. Наши кластеры разделены по каналам обновления, и после загрузки обновлений в эти каналы, все кластеры обновляются автоматически.

Переход на версию 1.15 был сложным, но сейчас мы ожидаем обновления последних кластеров. Мы решили пропустить версии 1.17 и 1.18 и обновили напрямую версии 1.13, 1.14 и 1.12. Сейчас мы находимся на версии 1.16 и планируем перейти на версию 1.19 без проблем, так как у нас уже готов код для этого перехода.

Через месяц мы планируем перейти на версию 1.19, а затем на версии 1.20 и 1.21. Мы провели тестирование на тестовом стенде и уверены в работоспособности этого процесса. Мы предлагаем услуги по управлению кластерами Kubernetes, которые могут быть реализованы в вашей инфраструктуре, в облаке или выполнены нашими силами.. 

Обновление кластера Kubernetes зависит от первоначального развертывания и последующих изменений.  Это включает обновление плоскости управления, узлов в кластере и клиентов, таких как `kubectl`. Необходимо иметь существующий кластер для обновления с Kubernetes версий 1.28 до 1.29. 
Существуют различные подходы к модернизации кластера, включая использование `kubeadm` и ручное обновление. После обновления кластера необходимо установить последнюю версию `kubectl`. Для каждого узла в кластере требуется обновление или замена узла на версию 1.29 `kubelet`. Обновление до новой версии Kubernetes может привести к появлению новых API, которые требуют преобразования манифестов. Если в кластере запущены плагины для устройств, необходимо обновить их для поддержки обеих версий до обновления узла. -->
