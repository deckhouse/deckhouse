...
## Обновление кластеров Kubernetes (сделать в виде сценария)

В Deckhouse Kubernetes Platform эти этапы выполняются автоматически.

Для обновления достаточно изменить значение *kubernetesVersion* на желаемое. 

Внутри подсистемы `candi` есть два модуля, которые отвечают за управление [control-plane](ссылка) и [управление узлами](ссылка).

В сontrol-plane-manager автоматически отслеживаются изменения.

Вычисляется текущая версия Kubernetes в кластере на основании версии control-plane и узлов кластера. 

Например, если `kubelet` на всех узлах версии 1.27 и все `control-plane` компоненты версии 1.27, то можно запускать обновление на следующую версию 1.28 — и так далее, пока версия не будет той, что указана в конфигурации.

Сценарий обновления кластера Kubernetes с версии 1.X до 1.Z:
1. Убедитесь, что все kubelet на узлах и control-plane-компоненты имеют версию 1.X.
2. Определите следующую версию, на которую будет выполнено обновление, например, 1.Z.
3. Проверьте, что все компоненты кластера обновлены до версии 1.X и готовы к обновлению.
Обновление происходит последовательно, обновляется control-plane до версии 1.Y, kubelet узлов кластера до версии 1.Y. Затем они обновляются, пока не достигнут версии, указанной в конфигурации (например, 1.Z).
После обновления всех компонентов убедитесь, что кластер Kubernetes работает корректно на указанной версии (1.Z) и все узлы и control-plane функционируют корректно.
...
## Повышенные требования к безопасности обновлений
