...
> РЕМАРКА: Тут должно быть общее описание. В том числе ответы на вопросы "зачем это надо", но глубже чем в глоссарии
>
...
В шаблонах проектов можно использовать [переменные](../architecture/projects.html#переменные-шаблонов-проектов) и функции Go templating, что позволяет создавать проекты с различными настройками и ограничениями, не изменяя сам шаблон.
...
В случае удаления Deckhouse Kubernetes Platform, пространства имён остаются в кластере Kubernetes, что позволяет сохранить группировку объектов и обеспечить работоспособность полезной нагрузки в кластере Kubernetes.
...
- [Настройка проектов](../admin/projects.html).
