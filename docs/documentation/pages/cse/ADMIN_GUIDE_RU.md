...
<tr>
<td>4.2</td>
<td>Подготовка инфраструктуры установки</td>
<td>
<ul>
<li>Kubernetes+Virtualization</li>
<li>Virtualization</li>
...
<tr>
<td>4.11</td>
<td>Механизмы аутентификации и авторизации</td>
<td>
<ul>
<li>Kubernetes+Virtualization</li>
<li>Kubernetes</li>
...
1. Общие требования для узлов кластера:
   ...
   - Адресные пространства подов (`podSubnetCIDR`), сервисов (`serviceSubnetCIRD`) и внутренней сети кластера (`internalNetworkCIDRs`) не пересекаются;
   - На узлах отсутствует пользователь DKP CSE;
   - На узлах установлен поддерживаемый container runtime (`containerd`);
   ...
   - На узлах установлено корректное системное время. Рекомендуется синхронизация времени с доверенным NTP-сервером.

1. Для узлов, предназначенных для запуска виртуальных машин (при использовании виртуализации), необходимо дополнительно обеспечить:

   - доступ к репозиториям пакетов используемой ОС;
   - HTTPS-доступ к хранилищу образов контейнеров DKP CSE;
   - SSH-доступ от машины для запуска установки;
   - в случае с РЕД ОС — убедитесь, что установлены yum и which (по умолчанию могут отсутствовать).

1. Дополнительные требования для статического кластера:

   - В команде запуска установки указывается только один параметр `--ssh-host`, обозначающий IP первого master-узла (аргумент команды `dhctl bootstrap`);
   - Доступно подключение по SSH с использованием указанными ключами;
   - Доступна установка SSH-туннеля до первого master-узла;
   ...
   - Между хостом запуска установщика и сервером открыт порт 22/TCP;
   - Отсутствуют конфликты по портам, которые используются процессом установки;
   - DNS разрешает имя `localhost` в IP-адрес 127.0.0.1;
   - Пользователь, от имени которого выполняется установка (например, пользователь `caps`), имеет доступ к команде `sudo`;

   - На сервере (ВМ) установлено время, синхронизированное с доверенным NTP-сервером.

   - При использовании ContainerdV2 в качестве container runtime должны выполняться следующие условия:

     - поддерживается CgroupsV2;
     - используется `systemd` версии не ниже 244;
     - поддерживается модуль ядра `erofs`.

1. Требования для узлов с Московской серверной операционной системой (Мос.ОС 15 Arbat):

   - добавить параметр `systemd.unified_cgroup_hierarchy=1` в `GRUB_CMDLINE_LINUX` в файле `/etc/default/grub`, после чего выполнить команду:
...
1. Требования к машине для запуска установки DKP CSE (ЭВМ, которую не планируется добавлять в кластер):
...
При необходимости отдельные предварительные проверки можно отключить с помощью параметров командной строки:

- `--preflight-skip-all-checks` — пропуск всех предварительных проверок;
- `--preflight-skip-ssh-forward-check` — пропуск проверки SSH-туннеля;
- `--preflight-skip-availability-ports-check` — пропуск проверки доступности необходимых портов;
- `--preflight-skip-resolving-localhost-check` — пропуск проверки разрешения имени `localhost`;
- `--preflight-skip-deckhouse-version-check` — пропуск проверки версии DKP;
- `--preflight-skip-registry-through-proxy` — пропуск проверки доступа к хранилищу образов через прокси;
- `--preflight-skip-public-domain-template-check` — пропуск проверки параметра `publicDomainTemplate`;
- `--preflight-skip-ssh-credentials-check` — пропуск проверки учетных данных SSH;
...
- `--preflight-skip-python-checks` — пропуск проверки Python;
- `--preflight-skip-sudo-allowed` — пропуск проверки прав `sudo`;
...
- `--preflight-time-drift-check` — пропуск проверки синхронизации времени;
- `--preflight-skip-cidr-intersection` — пропуск проверки пересечения CIDR;
- `--preflight-skip-deckhouse-user-check` — пропуск проверки отсутствия пользователя DKP CSE;
- `--preflight-skip-yandex-with-nat-instance-check` — пропуск проверки конфигурации Yandex Cloud с NAT Instance;
...
Замените `<SSH_PRIVATE_KEY_FILE>` на имя используемого приватного ключа (например, `id_rsa` или `id_ed25519`).
...
В этом разделе описана подготовка инфраструктурных компонентов, которые должны быть готовы до заполнения файла конфигурации и запуска установки DKP CSE:

- TLS-сертификаты и ключи для HTTPS-доступа к registry и выпуска сертификатов сервисов DKP CSE. Подготовленные значения используются в `deckhouse.registryCA`, секрете `internal-ca-key-pair` и `ClusterIssuer` в файле конфигурации установки (п. 4.4.1).
- Хранилище образов контейнеров DKP CSE (container registry), путь к загруженным образам и данные аутентификации. Эти значения используются в ресурсе `InitConfiguration` (п. 4.3.1) и при запуске контейнера установщика (п. 4.4.2).
- SSH-доступ к узлам кластера. Подготовленные пользователь, публичный ключ и приватный ключ используются в ресурсе `SSHCredentials` файла конфигурации (п. 4.4.1) и в команде запуска установки (п. 4.4.2).

Если registry или сервисы DKP CSE будут использовать самоподписанные TLS-сертификаты, сначала подготовьте сертификаты и ключи (п. 4.2.1). Если у вас нет локального registry, разверните его и загрузите в него образы DKP CSE (п. 4.2.2) до подготовки YAML-файла конфигурации установки.
...
Подготовленные в этом разделе файлы используются в следующих шагах установки:

- `rootCA.crt` — корневой сертификат УЦ. Укажите его в `deckhouse.registryCA`, если registry работает по HTTPS с сертификатом, выпущенным этим УЦ.
- `intermediateCA.crt` и `intermediateCA.key` — сертификат и ключ промежуточного УЦ. Их можно использовать как значения `tls.crt` и `tls.key` в секрете `internal-ca-key-pair`, если сертификаты сервисов DKP CSE выпускаются через внутренний `ClusterIssuer`.
- `wildcard.crt` и `wildcard.key` — серверный сертификат и ключ для домена кластера и registry.
- `/etc/docker/registry/private.key` и `/etc/docker/registry/public.crt` — файлы, в которые нужно поместить ключ и сертификат registry. Можно использовать `wildcard.key` и `wildcard.crt`, если они подходят для DNS-имени registry, или отдельный сертификат registry.
...
#### 4.2.2. Подготовка registry и загрузка образов DKP CSE

В случае отсутствия registry, выполните шаги по его установке:
...
При отсутствии TLS-сертификатов, можно сгенерировать самоподписанные сертификаты (п. 4.2.1).
...
Также вы можете скопировать образы из публичного хранилища образов контейнеров следующей командой:
...
--license="${LICENSE_KEY}" \
--gost-digest \
--deckhouse-tag="v1.73.4" \
--include-module=stronghold
```

Где:
- `${PACKAGE_DIR_PATH}` — директория для скачанных образов
- `${LICENSE_KEY}` — лицензионный ключ для доступа в публичное хранилище образов контейнеров DKP CSE.
...
После загрузки образов используйте адрес registry, путь к образам, данные аутентификации и сертификат registry в ресурсе `InitConfiguration` (п. 4.3.1):

- `deckhouse.imagesRepo` — `<REGISTRY_URL>/<REGISTRY_PATH>`;
- `deckhouse.registryDockerCfg` — данные аутентификации в registry, закодированные в Base64;
- `deckhouse.registryScheme` — протокол доступа к registry (`HTTP` или `HTTPS`);
- `deckhouse.registryCA` — корневой сертификат, если registry работает по HTTPS с самоподписанным сертификатом.

#### 4.2.3. Создание SSH-доступа к узлам кластера

SSH-доступ используется установщиком DKP CSE для подключения к master-узлу и другим узлам кластера. Подготовьте приватный ключ на хосте установки и добавьте публичную часть ключа на все узлы кластера. Эти данные потребуются при заполнении ресурса `SSHCredentials` в примере конфигурации установки (п. 4.4.1) и при запуске команды `dhctl bootstrap` (п. 4.4.2).
...
### 4.3. Файл конфигурации установки DKP CSE

Для установки DKP CSE нужно подготовить YAML-файл конфигурации установки. Пример заполненного файла конфигурации приведен в п. 4.4.1.

Если в кластере будет использоваться внутренний TLS-сертификат, то необходимо также добавить его в конфигурацию установки, поместив в манифест секрета в пространстве имен d8-cert-manager.
...
#### 4.3.1. Ресурс InitConfiguration
...
#### 4.3.2. Ресурс ClusterConfiguration
...
#### 4.3.3. Ресурс StaticClusterConfiguration
...
#### 4.3.4. Список внутренних сетей узлов кластера
...
### 4.4. Установка DKP CSE

#### 4.4.1. Пример конфигурации установки
...
Перед заполнением `config.yml` подготовьте следующие значения:

- адрес registry и путь к образам DKP CSE: см. п. 4.2.2;
- данные аутентификации в registry в формате Base64: см. описание `deckhouse.registryDockerCfg` в п. 4.3.1;
- корневой сертификат registry, если используется HTTPS с самоподписанным сертификатом: см. п. 4.2.1;
- сертификат и ключ внутреннего CA для `internal-ca-key-pair`, если сертификаты сервисов DKP CSE выпускаются через `cert-manager`: см. п. 4.2.1;
- SSH-ключ и пользователь для подключения к узлам кластера: см. п. 4.2.3.

```yaml
apiVersion: deckhouse.io/v1
kind: InitConfiguration
deckhouse:
 # Адрес и путь к образам, в используемом registry.
 imagesRepo: registry.company.my/deckhouse/cse
 # Строку с данными аутентификации в registry можно получить командой:
 ...
 registryScheme: HTTPS
 # Корневой сертификат УЦ, которым подписан TLS-сертификат registry.
 # Укажите параметр, если registry использует самоподписанный сертификат.
 registryCA: |
   -----BEGIN CERTIFICATE-----
...
apiVersion: v1
data:
 # Сертификат и ключ внутреннего CA для выпуска сертификатов сервисов DKP CSE.
 # Если используются файлы из п. 4.2.1, укажите содержимое intermediateCA.crt и intermediateCA.key.
   ...
   modules:
     https:
       ...
       certManager:
         ...
         # который будет использоваться для выпуска сертификатов сервисов DKP CSE.
...
spec:
 # Пользователь и приватный ключ для подключения к узлам кластера.
 # Подготовка пользователя и SSH-ключа описана в п. 4.2.3.
 user: caps
...
Замените параметры в примере:

- `<REGISTRY_HOST>` — DNS-имя или IP-адрес registry из п. 4.2.2;
- `<REGISTRY_USER>` и `<REGISTRY_PASSWORD>` — учетные данные registry, созданные при подготовке registry;
- `<ДАННЫЕ_АУТЕНТИФИКАЦИИ_В_ХРАНИЛИЩЕ_ОБРАЗОВ>` — строка `registryDockerCfg`, закодированная в Base64;
- `<INTERNAL_CA_CERT>` — содержимое сертификата внутреннего CA для `ClusterIssuer internal-ca`;
- `<INTERNAL_CA_KEY>` — приватный ключ этого CA;
- `<NODES_NETWORK>` — CIDR внутренней сети узлов кластера;
- `<SSH_PRIVATE_KEY>` — приватный SSH-ключ пользователя, который будет подключаться к узлам;
- `<MASTER_2_NODE_IP>`, `<MASTER_3_NODE_IP>`, `<WORKER_1_NODE_IP>`, `<WORKER_2_NODE_IP>`, `<WORKER_3_NODE_IP>` — IP-адреса узлов кластера.

Если registry использует сертификат доверенного УЦ, поле `deckhouse.registryCA` можно не указывать.

Если registry использует самоподписанный сертификат, укажите в `deckhouse.registryCA` корневой сертификат УЦ, которым подписан TLS-сертификат registry.

Если для сервисов DKP CSE используется внутренний CA, заполните Secret `internal-ca-key-pair` и укажите `clusterIssuerName: internal-ca`.

#### 4.4.2. Установка DKP CSE
...
Перед запуском установки убедитесь, что у вас есть:

- файл `config.yml`, заполненный по примеру из п. 4.4.1;
- доступный registry с загруженными образами DKP CSE;
- приватный SSH-ключ для подключения к master-узлу;
- пользователь на master-узле с правами sudo.

Запустите контейнер установщика:

```bash
docker run --pull=always -it [<MOUNT_OPTIONS>] <REGISTRY_URL>/<REGISTRY_PATH>/install:v1.73.4 bash
```
...
docker run -it --pull=always \
  ...
  -v "$HOME/.ssh/:/tmp/.ssh/" registry.company.int/deckhouse/cse/install:v1.73.4 bash
```
...
#### 4.4.3. Дополнительные общие настройки кластера
...
#### 4.4.4. Настройка балансировки входящего трафика с помощью виртуальных IP-адресов
...
#### 4.5.1. Локальное хранилище Local Path Provisioner
...
#### 4.5.2. Локальное хранилище на основе Logical Volume Manager
...
#### 4.5.3. Распределённая система хранения Ceph
...
#### 4.5.4. Сетевое файловое хранилище NFS
...
#### 4.5.5. Хранилище данных на основе протокола SCSI
...
Проверить создание объекта можно командой (Параметр Phase должен быть Created):
...
#### 4.5.6. Хранилище данных Yadro Tatlin Unified Storage
...
### 4.6. Откат установки и удаление DKP CSE
...
- [Astra Linux Special Edition](https://wiki.astralinux.ru/pages/viewpage.action?pageId=158601444).

- [ОС РЕД ОС](https://redos.red-soft.ru/base/arm/arm-other/docker-install/).

- [Альт 8 СП](https://www.altlinux.org/Docker).

При включенном и настроенном operator-trivy (п. 5.1), развёртывание контейнеров из уязвимых образов будет запрещено.

### 4.8. Настройка DKP CSE
...
### 4.10. Включение и отключение модуля
...
### 4.11. Механизмы аутентификации и авторизации

#### 4.11.1 Локальная аутентификация
...
##### 4.11.1.1 Создание статического пользователя
...
##### 4.11.1.2 Добавление пользователя в группу
...
После создания группы и добавления в неё пользователей, необходимо настроить авторизацию (п. 4.11.3).
...
##### 4.11.1.3 Настройка парольной политики
...
##### 4.11.1.4 Настройка двухфакторной аутентификации
...
- `enabled` — включает или отключает двухфакторную аутентификацию для всех статических пользователей;
- `issuerName` — имя, которое будет отображаться в приложении-аутентификаторе при добавлении аккаунта.

После включения двухфакторной аутентификации каждый пользователь должен пройти процесс регистрации в приложении-аутентификаторе при первом входе.

#### 4.11.2 Подключение провайдера аутентификации
...
Перед подключением, провайдер аутентификации необходимо настроить с учетом ролевой модели доступа, используемой в DKP CSE. В каталоге, доступ к которому обеспечивает провайдер аутентификации, пользователям, которые должны иметь доступ в DKP CSE, должны быть присвоены группы, в соответствии с необходимым уровнем прав в рамках ролевой модели доступа DKP CSE (Технические условия, Приложение 1. Список ролей).
...
Обратите внимание на особенности настройки комбинированного доступа и совместного использования RoleBinding и ClusterAuthorizationRule (CAR) для одного и того же пользователя при включенном в кластере режиме мультитенантности (параметр [`enableMultiTenancy: true`](https://deckhouse.ru/modules/user-authz/configuration.html#parameters-enablemultitenancy)).
Если в кластере включён режим мультитенантности и для указанного в RoleBinding пользователя или его группы существует ClusterAuthorizationRule (CAR) с правилами для другого пространства имён, отличного от целевого (указанного в RoleBinding), правила из ClusterRole, указанного в RoleBinding, работать не будут.
Это связано с особенностями работы вебхука модуля `user-authz`. Он проверяет принадлежность запроса к разрешённым пространствам имён на уровне группы. Если группа пользователя привязана к CAR с селектором только на определенное пространство имён, все запросы в пространства имён, не указанные в CAR, будут отвергнуты, независимо от наличия RoleBinding с этими пространствами имён для пользователя.
...
Настройка этих хранилищ подробно описана в п. 4.5.
...
3. Осуществить миграцию данных.
   ...
   **Внимание.** Продолжительность процедуры миграции зависит от размера и нагруженности кластера. В случае сбоя связности или отключения SSH-сессии процедура будет прервана. Для длительных миграций или случаев, когда перезапуск миграции нежелателен, рекомендуется запуск в эмуляторе терминала по типу screen или tmux.
...
- обновить базы уязвимостей (п. 5.1.1)
- установить на пространствах имен, содержащих подлежащие сканированию поды, аннотацию security-scanning.deckhouse.io/enabled.
  ...
  Просмотр отчетов о сканировании осуществляется в веб-интерфейсе Grafana в дашбордах CIS Kubernetes Benchmark и Trivy Image Vulnerability Overview, сгруппированных в директории Security (п. 5.5):
...
Для такого сканирования включите функцию в `ModuleConfig/operator-trivy`:
...
Целевой реестр контейнеров для сканирования описывается ресурсом уровня кластера `RegistryScanTarget`:
...
Результаты сканирования сохраняются в ресурсах уровня кластера `RegistryImageVulnerabilityReport`, по одному на каждый сканируемый тег образа.
...
Политика по умолчанию определяется в параметре `podSecurityStandards.defaultPolicy` в ModuleConfig `admission-policy-engine` (п. 5.2.1).
...
Глобальный режим работы политик определяется в параметре `podSecurityStandards.enforcementAction` в ModuleConfig `admission-policy-engine` (п. 5.2.1). В случае если необходимо переопределить глобальный режим политик для конкретного пространства имен, на него нужно установить лейбл `security.deckhouse.io/pod-policy-action =<РЕЖИМ_ПОЛИТИКИ_В_НИЖНЕМ_РЕГИСТРЕ>` на соответствующем namespace. Список допустимых режимом политик состоит из: `Dryrun`, `Warn`, `Deny`.
...
- `settings.denyVulnerableImages.enabled` — булевый
  ...
  Включает запрет на использование уязвимых образов в помеченных пространствах имён.

  По умолчанию: `false`.

- `settings.denyVulnerableImages.allowedSeverityLevels` — массив строк

  Уровни серьёзности уязвимостей, наличие которых не приводит к блокировке образа.
...
- `settings.useVEXFromOCI` булевый

  Использовать VEX-аттестации OCI для фильтрации найденных уязвимостей при сканировании рабочих нагрузок и реестров контейнеров. Уязвимости, помеченные в VEX-аттестации как not_affected или fixed, исключаются из отчётов.

  По умолчанию: false
- `settings.additionalRegistryCA` массив объектов

  Список корневых CA-сертификатов для приватных реестров контейнеров.

  По умолчанию: []

- `settings.additionalRegistryCA.name` строка

  ОБЯЗАТЕЛЬНЫЙ ПАРАМЕТР

  Произвольное имя сертификата.

- `settings.additionalRegistryCA.ca` строка

  ОБЯЗАТЕЛЬНЫЙ ПАРАМЕТР

  Корневой CA-сертификат или цепочка сертификатов в формате PEM.

- `settings.insecureRegistries` массив строк

  Список реестров контейнеров, для которых разрешено использовать HTTP или HTTPS без проверки TLS-сертификата.
  
  Параметр применяется при сканировании рабочих нагрузок, проверке denyVulnerableImages и сканировании реестров, настроенных с помощью RegistryScanTarget.
...
- Admission policy engine. Содержит информацию, связанную с работой политик безопасности. В том числе: количество событий запрета выполнения действий из-за нарушения политики безопасности; разбивку запретов выполнения действий по типу запрета; журнал событий.
  ...
  ![Дашборд Admission policy engine с журналом событий OPA Violations](images/admin-guide/image12.png)

  *Рисунок 1. Пример дашборда Admission policy engine.*

- CIS Kubernetes Benchmark. Дашборд с информацией о результатах работы сканера проверок конфигурации кластера на соответствие принятым подходам (лучшим практикам). Содержит сводную информацию о результатах проверки, без возможности детализации. Дашборд доступен при включенном модуле `operator-trivy` (п. 5.1).

  ![Дашборд CIS Kubernetes Benchmark с результатами проверок](images/admin-guide/image14.png)
...
- Kubernetes audit logs. Журнал регистрации обращений к API-серверу. Содержит записи о всех обращениях к API-серверу кластера в JSON-формате.
  ...
  ![Дашборд Kubernetes audit logs с журналом обращений к API-серверу](images/admin-guide/image13.png)
...
- Runtime audit engine logs. Журнал регистрации событий безопасности аудита работы ядра Linux и API-сервера кластера.
  ...
  ![Дашборд Runtime audit engine logs с событиями безопасности](images/admin-guide/image16.png)

  *Рисунок 4. Пример дашборда Runtime audit engine logs.*

- Trivy Image Vulnerability Overview. Дашборд со сводной и детализированной информацией о сканировании образов контейнеров подов в пространствах имен, отмеченных аннотацией security-scanning.deckhouse.io/enabled (п. 5.1).

![Дашборд Trivy Image Vulnerability Overview со сведениями об уязвимостях образов](images/admin-guide/image15.png)
...
В DKP CSE контроль целостности при запуске системных контейнеров реализован на уровне модифицированной версии контейнерного рантайма `containerd` v2, поставляемой в составе DKP CSE. Для работы механизма на узлах кластера должен использоваться `ContainerdV2` (подробнее — в пункте 4.2.1.4 [«Обновление версии containerd»](update.html#обновление-версии-containerd) руководства по обновлению DKP CSE).

Механизм применяется только к образам системных компонентов DKP CSE, поставляемым в составе платформы и запускаемым в защищаемых пространствах имен. Это не все пространства имен с префиксом `d8-`, а только пространства имен, включенные при сборке DKP CSE в перечень, приведенный ниже, а также пространство имен `kube-system`. Этот механизм не является общей проверкой любых образов, запускаемых через `ContainerdV2`; для пользовательских образов используйте механизмы проверки целостности, описанные в разделе 5.7.4.

Перечень защищаемых пространств имен:
...
- обеспечение неизменяемости контейнеров при запуске.

Подписывание образов системных контейнеров DKP CSE работает по принципу прикреплённой подписи (attached signature). Подпись добавляется в манифест образа в аннотации `io.deckhouse.delivery-kit.signature`. Это не контрольная сумма, а криптографическая подпись манифеста образа.

При загрузке образа и старте контейнера подпись проверяется с использованием встроенного в `containerd` набора публичных сертификатов. Если подписи на локально скачанном образе нет, образ считается повреждённым и его необходимо заново скачать из registry.

Неизменяемость системных контейнеров при запуске достигается за счёт следующих мер:

- использование в контейнерном рантайме `containerd` v2 снапшоттера EROFS-snapshotter вместо OverlayFS. EROFS-snapshotter преобразует каждый слой OCI-образа в формат EROFS: слои становятся не директориями, а файлами, содержащими контент. Это делает каждый слой неизменяемым (immutable);
- отказ от подключения слоя `upperdir` с правами на чтение и запись (RW, read-and-write). При сборке образа создаётся слой с общеупотребимыми точками монтирования (`/tmp`, `/etc/resolv.conf` и т. д.), доступный только для чтения (RO, read-only). Он подключается автоматически ко всем создаваемым из образа контейнерам;
- использование механизма контроля DM-Verity. DM-Verity — компонент ядра Linux, который выполняет проверки «на лету», чтобы убедиться, что данные на диске не были изменены за пределами контролируемого процесса. На этапе сборки DKP CSE для каждого слоя образа рассчитывается корневой хеш DM-Verity (root hash) по алгоритму SHA-256 с помощью утилиты `veritysetup` и добавляется в аннотацию `io.deckhouse.delivery-kit.dm-verity-root-hash` в манифесте OCI. `containerd` при разворачивании образа включает контроль DM-Verity и сверяет вычисленный root hash с хешем из аннотации.

Последовательность контроля целостности при запуске:

1. Kubelet передаёт запрос на создание или запуск контейнера в `containerd` v2.
2. `containerd` определяет пространство имен пода и проверяет, входит ли оно в список защищаемых пространств имен.
3. Для контейнеров из защищаемых пространств имён `containerd` проверяет подпись образа, контрольные суммы DM-Verity и целостность данных контейнера на узле.
4. Если проверки проходят успешно, контейнер запускается. Если проверка завершается ошибкой, контейнер не запускается.

Ошибки проверки записываются в журнал службы `containerd` с компонентом `integrity`. В DKP CSE такие события дополнительно сохраняются в файл `/var/log/containerd/containerd-integrity.log` и могут передаваться в подсистему аудита событий безопасности.
...
Для работы проверки целостности образа по умолчанию включен модуль `gost-integrity-controller` (модуль устарел и будет удалён в следующем минорном релизе). Если модуль не включен, то для включения модуля используйте команду:
...
![Экран выбора пространства имён в Hubble](images/admin-guide/image7.png)
...
- ![Интерфейс визуализации сетевого стека с панелью фильтров и графиком потоков](images/admin-guide/image10.png)
...
- для выбора пространства имен (выпадающий список в левой части панели); ![Фильтр для выбора пространства имён](images/admin-guide/image2.png)

  *Рисунок 8. Фильтр для выбора пространства имён.*

- для выбора ресурсов пространства имен, для которых нужно отобразить потоки (поле ввода в центральной части панели); ![Фильтр для выбора ресурсов пространств имён](images/admin-guide/image9.png)

  *Рисунок 9. Фильтр для выбора ресурсов пространств имён.*

- для выбора сетевых потоков на основе решения («вердикта»), принятого по ним Cilium; ![Фильтр для выбора сетевых потоков по вердикту](images/admin-guide/image3.png)

  *Рисунок 10. Фильтр для выбора сетевых потоков по «вердикту».*

- для выбора элементов схемы анализируемого пространства имен. ![Фильтр для выбора элементов схемы пространства имён](images/admin-guide/image6.png)
...
![Схема сетевых потоков пространства имён](images/admin-guide/image11.png)
...
![Таблица сетевых потоков и событий с информацией о взаимодействиях](images/admin-guide/image5.png)
...
![Интерфейс настройки столбцов в таблице сетевых потоков](images/admin-guide/image4.png)
...
![Детальная информация о записи сетевого потока](images/admin-guide/image1.png)
...
| loki                             | `securityEvents.alerts`                                                        | Настройки отвечающие за включение уведомлений о событиях безопасности.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Параметр `securityEvents.alerts.events` отвечает за уведомления о несистемных событиях безопасности. Должен содержать значение `true`. Параметр `securityEvents.alerts.systemEvents` отвечает за уведомления о системных событиях безопасности. Должен содержать значение `true`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
Все события разделены на две категории:
- Системные события;
- Несистемные события.

Системными событиями называются действия произведенные (на выбор, достаточно одного условия):
- В пространствах имен:
  - `d8-*`:
  - `kube-system`.
- От имени следующих сервисных аккаунтов:
  - `system:serviceaccount:d8-*`;
  - `system:serviceaccount:kube-*`;
  - `system:serviceaccount:node-*`.
...
<tr>
  ...
  &quot;node_network_address&quot;: &quot;unknown&quot;,
  &quot;virtualization_version&quot;: &quot;v1.73.4&quot;,
