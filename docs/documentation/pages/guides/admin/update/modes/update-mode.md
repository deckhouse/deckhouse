...
В DKP можно в автоматическом режиме включить ручное подтверждение опасных (disruptive) обновлений [для кластера](#для-кластера) или [для группы узлов](#для-группы-узлов).
