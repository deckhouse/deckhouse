...
2. **cloud-provider** - для взамодействия с IaaS облака необходим установленный и настроенный облачный провайдер для этого облака (cloud-provider-* на схеме). Включает также **csi-driver** и **cloud-controller-manager**.
