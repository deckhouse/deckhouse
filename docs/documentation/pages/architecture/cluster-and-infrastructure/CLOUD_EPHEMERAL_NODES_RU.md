...
![Архитектура модуля node-manager для CloudEphemeral-узлов](../../../images/architecture/cluster-and-infrastructure/c4-l2-cloud-ephemeral-nodes.png)
...
7. **Standby-holder** (Deployment) — под для резервирования узлов. При включенном параметре [`spec.cloudinstances.standby`](/modules/node-manager/cr.html#nodegroup-v1-spec-cloudinstances-standby) кастомного ресурса NodeGroup в соответствующей группе узлов во всех [зонах](/modules/node-manager/cr.html#nodegroup-v1-spec-cloudinstances-zones) создаются резервные узлы.
   ...
   Под состоит из одного контейнера:

   * **reserve-resources** — такой же контейнер типа **pause**, который используется во всех подах для резервирования сетевого неймспейса (в подах он скрыт от пользователя).
...
{% alert level="info" %}
Модуль взаимодействует с модулем `cloud-provider` через kube-apiserver, используя секрет `kube-system/d8-node-manager-cloud-provider`, для получения всех необходимых настроек подключения к облаку и создания CloudEphemeral-экземпляров. Также `cloud-provider` предоставляет модулю `node-manager` шаблоны для создания кастомных ресурсов Cluster API, специфичных для определенных провайдеров.
{% endalert %}
