...
permalink: ru/architecture/cluster-and-infrastructure/cloud-static-nodes/
lang: ru
search: cloudstatic nodes, cloudstatic узлы
---
...
![c4 l2 cloud-static-nodes](../../../images/architecture/cluster-and-infrastructure/c4-l2-cloud-static-nodes.png)
...
2. **capi-controller-manager** - компонент обеспечивающий жизненный цикл самого кластера и его узлов. Самостоятельно не заказывает узлы в облаке, работает с Custom Resources более абстрактного уровня, не привязанного к инфраструктуре. Генерирует инфраструктурные Custom Resources, оставляя всю работу для инфраструктурного провайдера, для статических узлов это Cluster API Provider Static.
3. **caps-controller-manager** - Cluster API Provider Static, компонент обеспечивающий управление статическими узлами (ограниченно, без заказа узлов).
4. **csi-driver** - используется для заказа дисков в облачном IaaS.
5. **cloud-controller-manager** - используется для заказа балансировщиков и прочих ресурсов IaaS согласно своей спецификации.
