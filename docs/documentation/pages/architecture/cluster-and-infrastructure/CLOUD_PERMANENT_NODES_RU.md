...
![Архитектура модуля node-manager для CloudPermanent-узлов](../../../images/architecture/cluster-and-infrastructure/c4-l2-cloud-permanent-nodes.png)
...
3. **Fencing-agent** (DaemonSet) — разворачивается на определенной группе узлов при включённом параметре [`spec.fencing`](/modules/node-manager/cr.html#nodegroup-v1-spec-fencing) кастомного ресурса NodeGroup.
