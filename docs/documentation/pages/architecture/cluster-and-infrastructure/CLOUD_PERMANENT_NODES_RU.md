...
2. **terraform-manager** - [модуль](/modules/terraform-manager/), который используется для автоматического управления ресурсами IaaS (проверяет состояние Terraform’а и применяет недеструктивные изменения к ресурсам IaaS). С архитектурой модуля **terraform-manager** можно ознакомиться на соответстующей [странице][../terraform-manager/]
