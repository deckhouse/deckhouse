...
1. Модуль `cloud-provider-yandex` предоставляет модулю [`node-manager`](/modules/node-manager/) следующие артефакты:
   ...
   * шаблоны для создания кастомных ресурсов MCM (Machine Controller Manager) для конкретного провайдера, которые `cloud-provider-yandex` использует для создания виртуальных машин в облаке;
