...
{% alert level="info" %}
Для Deckhouse Virtualization Platform (DVP) ресурс DVPClusterConfiguration не используется. Конфигурация CloudPermanent-узлов складывается из [NodeGroup](/modules/node-manager/cr.html#nodegroup), [DVPInstanceClass](/modules/cloud-provider-dvp/cr.html#dvpinstanceclass) и ModuleConfig [`cloud-provider-dvp`](/modules/cloud-provider-dvp/configuration.html).
{% endalert %}
