...
1. Модуль `cloud-provider-aws` предоставляет модулю [`node-manager`](/modules/node-manager/) следующие артефакты:
   ...
   * шаблоны для создания кастомных ресурсов MCM (Machine Controller Manager) для конкретного провайдера, которые `cloud-provider-aws` использует для создания виртуальных машин в облаке;
