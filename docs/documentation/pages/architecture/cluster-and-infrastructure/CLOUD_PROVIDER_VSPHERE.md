...
For more details about the module configuration, refer to the [corresponding documentation section](/modules/cloud-provider-vsphere/).
...
2. **Cloud-data-discoverer**: It is responsible for collecting data from the cloud provider's API and providing it as a `kube-system/d8-cloud-provider-discovery-data` Secret. This secret contains the parameters of a specific cloud used by other components of the `cloud-provider-vsphere` module.
...
1. The `cloud-provider-vsphere` module provides [`node-manager`](/modules/node-manager/) with following artifacts:
   ...
   * Provider-specific MCM (Machine Controller Manager) custom resource templates to be used by `cloud-provider-vsphere` to create VMs in the cloud.
...
2. The `cloud-provider-vsphere` module provides Terraform/OpenTofu components for VMware vSphere cloud used when building the [dhctl](https://github.com/deckhouse/deckhouse/tree/main/dhctl) executable file for the [`terraform-manager`](/modules/terraform-manager/) module, such as:
