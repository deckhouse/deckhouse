...
- предоставляет на порту `4282` отдельный HTTP-эндпоинт только для скачивания `rpp-get` (утилиты для работы с основным эндпоинтом модуля) при бутстрапе узлов, при этом запросы на этот порт не проходят через TLS и kube-rbac-proxy, в отличие от основного прокси на `4219`;
