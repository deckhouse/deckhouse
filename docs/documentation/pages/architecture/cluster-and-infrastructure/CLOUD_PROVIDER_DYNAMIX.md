...
For more details about the module configuration, refer to the [corresponding documentation section](/modules/cloud-provider-dynamix/).
...
1. **Capd-controller-manager**: Kubernetes Cluster API Provider for Basis Dynamix. [Cluster API](https://github.com/kubernetes-sigs/cluster-api) is an extension for Kubernetes that allows you to manage Kubernetes clusters as custom resources inside another Kubernetes cluster. Cluster API Provider allows clusters running the Cluster API to order virtual machines in the cloud provider's infrastructure, Basis Dynamix in this case.  Capd-controller-manager works with the following custom resources:

   * DynamixCluster: Description of a Basis Dynamix-based cluster.
   * DynamixMachineTemplate: Template describing the characteristics of the machines created in the cloud.
   * DynamixMachine: Description of the characteristics of a machine created based on DynamixMachineTemplate.

   It consists of a single container:

   * **capd-controller-manager**.

2. **Cloud-controller-manager**: It is an implementation of [Cloud controller manager](https://kubernetes.io/ru/docs/concepts/architecture/cloud-controller/) for Basis Dynamix. It provides interaction with the Basis Dynamix cloud and performs the following functions:
...
4. **CSI driver (dynamix)**: It is an implementation of the CSI driver for Basis Dynamix. To study the `cloud-provider-*` CSI driver typical architecture, refer to the [corresponding documentation page](../../storage/csi-drivers/csi-driver.html).

   CSI driver (dynamix) does not support snapshots. For this reason, the `csi-controller` Pod does not include the snapshotter ([external-snapshotter](https://github.com/kubernetes-csi/external-snapshotter )) sidecar container.
...
1. **Kube-apiserver**:
    ...
    * Reconciles DynamixCluster, DynamixMachineTemplate, and DynamixMachine custom resources.
...
1. The `cloud-provider-dynamix` module provides [`node-manager`](/modules/node-manager/) with following artifacts:

   * Provider-specific Cluster API custom resource templates to be used by `cloud-provider-dynamix` to create VMs in the cloud.
...
2. The `cloud-provider-dynamix` module provides Terraform/OpenTofu components for Basis Dynamix cloud used when building the [dhctl](https://github.com/deckhouse/deckhouse/tree/main/dhctl) executable file for the [`terraform-manager`](/modules/terraform-manager/) module, such as:
