...
Модуль `cloud-provider-gcp` управляет взаимодействием с облачными ресурсами [Google Cloud Platform](https://cloud.google.com/). Он позволяет модулю [`node-manager`](/modules/node-manager/) использовать ресурсы GCP при заказе узлов для описанной [группы узлов](/modules/node-manager/cr.html#nodegroup).
...
1. **Сloud-controller-manager** — реализация [Сloud сontroller manager](https://kubernetes.io/ru/docs/concepts/architecture/cloud-controller/) для GCP. Обеспечивает взаимодействие с облаком GCP и выполняет следующие функции:
...
2. **Cloud-data-discoverer** — отвечает за сбор данных из API облачного провайдера и предоставление их в виде секрета `kube-system/d8-cloud-provider-discovery-data`. Этот секрет содержит параметры конкретного облака, которые используется другими компонентами модуля `cloud-provider-gcp`.
...
3. **CSI-драйвер (gcp)** — реализация CSI-драйвера для GCP. С типовой архитектурой CSI-драйвера, используемого в модулях `cloud-provider-*` DKP, можно ознакомиться на [соответствующей странице документации](../infrastructure/csi-driver.html).
...
1. Модуль `cloud-provider-gcp` предоставляет модулю [`node-manager`](/modules/node-manager/) следующие артефакты:
   ...
   * шаблоны для создания кастомных ресурсов MCM (Machine Controller Manager) для конкретного провайдера, которые `cloud-provider-gcp` использует для создания виртуальных машин в облаке;
...
2. Модуль `cloud-provider-gcp` предоставляет компоненты Terraform/OpenTofu для GCP, которые используются при сборке исполняемого файла утилиты [dhctl](https://github.com/deckhouse/deckhouse/tree/main/dhctl) в модуле [`terraform-manager`](/modules/terraform-manager/), такие как:
