...
* The mechanisms for managing all node types used in DKP, as well as [hybrid node groups and clusters](hybrid-nodegroups-and-clusters.html).
* The reference [CSI driver](csi-driver.html) architecture used in DKP.
* [Bashible](bashible.html) service, which is a key component of the Cluster & Infrastructure subsystem. Bashible is used by the [`node-manager`](/modules/node-manager/) module to manage node configuration.
...
Only [`cloud-provider-dvp`](/modules/cloud-provider-dvp/) is currently described in this section. Documentation for the remaining cloud provider modules will be added as it becomes available.
