...
This section describes the mechanisms for managing all node types used in DKP, as well as [hybrid node groups and clusters](hybrid-nodegroups-and-clusters.html).
...
This subsection also describes the [Bashible](bashible.html) service, which is a key component of the Cluster & Infrastructure subsystem. Bashible is used by the [`node-manager`](/modules/node-manager/) module to manage node configuration.
