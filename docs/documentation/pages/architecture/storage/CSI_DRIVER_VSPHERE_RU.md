...
CSI-драйвер `csi-vsphere` основан на [vSphere CSI Driver](https://github.com/kubernetes-sigs/vsphere-csi-driver) и используется в модулях [`csi-vsphere`](/modules/csi-vsphere/) и [`cloud-provider-vsphere`](/modules/cloud-provider-vsphere/).

Архитектура CSI-драйвера `csi-vsphere` отличается от типовой [архитектуры CSI-драйвера](../../storage/csi-drivers/csi-driver.html) наличием компонента *syncer*, специфичного для VMware vSphere.
