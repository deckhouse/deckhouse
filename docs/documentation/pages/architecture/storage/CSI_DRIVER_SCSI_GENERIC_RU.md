...
CSI-драйвер `csi-scsi-generic` — это реализация стандарта [Container Storage Interface (CSI)](https://github.com/container-storage-interface/spec/blob/master/spec.md) для работы с томами на СХД с подключением по SCSI в Deckhouse Kubernetes Platform (DKP).
