...
1. **Csi-node** (DaemonSet) — Node Plugin, работающий на узлах кластера с установленным лейблом `storage.deckhouse.io/csi-nfs-node` и отвечающий за локальное монтирование и размонтирование томов. В случае если `nodeSelector` не задан, компонент запускается на всех узлах кластера.
