...
The CSI driver `csi-vsphere` is based on [vSphere CSI Driver](https://github.com/kubernetes-sigs/vsphere-csi-driver). It used in [`csi-vsphere`](/modules/csi-vsphere/) and [`cloud-provider-vsphere`](/modules/cloud-provider-vsphere/) modules.

The architecture of the CSI driver `csi-vsphere` differs from [the reference architecture of CSI driver](../../storage/csi-drivers/csi-driver.html) by the presence of a *syncer* component specific to VMware vSphere.
