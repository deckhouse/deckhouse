...
description: Architecture of the sds-local-volume module in Deckhouse Platform.
---
...
1. **CSI driver (`sds-local-volume`)**: It is an implementation of the CSI driver for `local.csi.storage.deckhouse.io`. To study the CSI driver typical architecture used in DP, refer to [the CSI-driver architecture documentation section](../storage/csi-drivers/csi-driver.html). CSI driver (`sds-local-volume`) is developed by Flant.
