...
The `sds-node-configurator` manages block devices and LVM on Kubernetes cluster nodes through Kubernetes custom resources, performing following operations:
...
2. **Controller** (Deployment): A controller that monitors custom resources related to block devices and LVM. The controller works with the metadata of resources and updates their statuses.
...
2. **Prometheus-main**: Collects sds-node-configurator metrics.
