...
The `csi-scsi-generic` CSI driver is an implementation of the [Container Storage Interface (CSI)](https://github.com/container-storage-interface/spec/blob/master/spec.md) standard for working with volumes on storage systems connected via SCSI in the Deckhouse Kubernetes Platform (DKP). The CSI driver supports both iSCSI and Fibre Channel (FC) connection technologies.
...
1. **Csi-controller** (Deployment): Controller Plugin responsible for global volume operations such as creating and deleting volumes, and attaching and detaching volumes from nodes.
   ...
   * **controller sidecar containers**: Kubernetes community-maintained external controllers.
     ...
     * **Provisioner** ([external-provisioner](https://github.com/kubernetes-csi/external-provisioner)): Watches PersistentVolumeClaim resources and calls the RPC methods `CreateVolume` or `DeleteVolume`.
