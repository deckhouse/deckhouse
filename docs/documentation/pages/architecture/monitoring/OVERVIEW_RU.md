...
## Установка дополнительного экземпляра prometheus-operator в кластер

Иногда может понадобиться установить в кластер еще один prometheus-operator, чтобы добавить отличные от предоставляемых DKP Prometheus или AlertManager в кластер.

Чтобы не пересекаться с prometheus-operator DKP, prometheus-operator, который не входит в состав DKP необходимо указать флаг `--deny-namespaces=d8-monitoring`. Prometheus-operator из DKP следит за ресурсами правил и мониторов только в пространствах имен
