...
1. **Dex** — федеративный провайдер OpenID Connect, поддерживающий работу со статическими пользователями и интеграцию с внешними провайдерами аутентификации, такими как SAML, GitLab или GitHub. Модуль использует модифицированную версию [Dex](https://github.com/dexidp/dex) для поддержки:

   * групп для статических учетных записей пользователей;
   * двухфакторной аутентификации (2FA);
   * политики паролей;
   * принудительного изменения пароля;
   * SPNEGO (Kerberos) для LDAP‑коннектора. При использовании этого механизма Dex принимает билеты по заголовку `Authorization: Negotiate`, валидирует их по keytab, и завершает вход без отображения формы ввода логина/пароля и т.д.

   Актуальный список изменений в Dex приведен [в репозитории модуля](https://github.com/deckhouse/deckhouse/blob/main/modules/150-user-authn/images/dex/patches/README.md).
