...
{% alert level="warning" %}
This feature is available in the following DP editions: SE+, EE, Ultimate.
{% endalert %}

Image signature verification at the CRI level is not implemented in DP.
...
At the node level, DP also protects the integrity of unpacked image layers:
...
- The mechanism is available in all DP editions and does not require to be enabled separately.
