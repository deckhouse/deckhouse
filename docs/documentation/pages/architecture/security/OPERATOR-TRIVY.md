...
The `operator-trivy` module works with custom resources from the `spdx.softwarecomposition.kubescape.io` and `trivy.deckhouse.io` API groups.
...
- ClusterRbacAssessmentReport: Stores cluster-level RBAC settings assessment results.
- ClusterSbomReport: Stores an aggregated cluster-level SBOM for software components.
...
- [NodeVulnerabilityReport](/modules/operator-trivy/cr.html#nodevulnerabilityreport): Stores vulnerability scanning results for node `rootfs`.
...
- CollapseConfigurations: Defines cluster-wide aggregation thresholds for dynamic paths, endpoints, and network addresses when profiles are generated.
...
1. **Trivy-server** (StatefulSet): This component implements a security scanning service and is based on the Open Source [Trivy](https://github.com/aquasecurity/trivy) project.
   ...
   Trivy-server updates its vulnerability database from an OCI image at startup and then periodically.
...
1. **Security-storage** (Deployment): A controller that includes a single `apiserver` container, extends Kubernetes API, and processes CRUD operations, watch requests, and list requests for custom resources from the `spdx.softwarecomposition.kubescape.io` and `trivy.deckhouse.io` API groups.
...
1. **Node-agent** (DaemonSet): An optional component that includes a single `node-agent` container, runs on all cluster nodes in privileged mode, observes container behavior through eBPF, and builds runtime profiles.
   ...
   - IPC_LOCK
   - NET_RAW
   This is required to observe container behavior through eBPF.
...
1. **Scan-vulnerabilityreport-&lt;hash&gt;** (Job): A component that launches tasks for security scanning of Pod, ReplicaSet, ReplicationController, StatefulSet, DaemonSet, CronJob, and Job resources by using the `trivy-server` component. The task is created and managed by the `operator` component.
   ...
   - **&lt;dynamic-name&gt;**: Optional init container with a dynamic name used to authenticate to the image storage.
   - **&lt;container-name&gt;**: Set of containers where each container is responsible for scanning the corresponding workload container. The Trivy image is used as the base image, and the target container image from its specification is passed in arguments.
...
1. **Kube-apiserver**:
   ...
   - Creates, updates, and deletes Secret and Job resources.

1. **Distribution**: Pulls OCI images with the vulnerability database and the BDU database.

The following external components interact with the module:

1. **Kube-apiserver**:

   - Processes Kubernetes resources referenced in custom resources from the `spdx.softwarecomposition.kubescape.io/*` and `trivy.deckhouse.io` API groups.
