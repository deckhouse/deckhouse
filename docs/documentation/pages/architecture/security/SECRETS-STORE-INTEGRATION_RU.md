...
- инъекция точки входа (entrypoint) для приложений, которые нельзя модифицировать для прямого чтения секретов из хранилища;
...
Режим работы (`Manual` или `DiscoverLocalStronghold`) определяется в параметре модуля [`settings.connectionConfiguration`](/modules/secrets-store-integration/configuration.html#parameters-connectionconfiguration) кастомного ресурса [ModuleConfig](../../reference/api/cr.html#moduleconfig).
...
1. **Csi-secrets-store** (DaemonSet) — компонент работает на всех узлах кластера и реализует стандарт [Container Storage Interface (CSI)](https://github.com/container-storage-interface/spec/blob/master/spec.md) для обеспечения доставки секретов в под в виде примонтированных файлов.
   ...
   - **injector-puller** — init-контейнер, который выполняет однократный запуск файл-инжектора (исполняемого файла для безопасного внедрения секретов
   ...
   - **csi-livenessprobe** — сайдкар-контейнер, проверяющий CSI Unix socket и экспортирующий эндпоинт состояния CSI-драйвера;
...
1. **Vault-csi-provider** (DaemonSet) — компонент состоит из одного контейнера **vault-csi-provider**. Vault-csi-provider выполняет авторизацию подключения к хранилищу секретов, получает данные из хранилища секретов и предоставляет данные для csi-secrets-store. Компонент запускается на всех узлах кластера для взаимодействия с csi-secrets-store через Unix socket.
...
1. **Webhook** (Deployment) — компонент реализует mutating-вебхуки для ресурсов Pod через механику [Mutating Admission Controllers](https://kubernetes.io/docs/reference/access-authn-authz/admission-controllers/).

   Компонент выполняет мутацию пода, если задана аннотация пода `secrets-store.deckhouse.io/role`. При этом компонент выполняет следующие модификации манифеста пода:
   ...
   - если в манифесте пода задана аннотация `secrets-store.deckhouse.io/env-from-path` или в спецификации контейнера определено использование секретов из хранилища секретов, то для каждого такого контейнера, включая init-контейнеры, компонент заменяет оригинальную команду запуска на запуск файл-инжектора.

     В качестве аргумента файл-инжектор получает оригинальную команду запуска. Если в манифесте пода у контейнера отсутствует команда запуска, выполняется получение команды запуска из образа в хранилище образов контейнеров.
...
- **User-app** — компонент представляет типовое приложение пользователя, в которое требуется доставить секреты в переменные окружения.
  ...
  - **&lt;CONTAINER_NAME&gt;** — один или несколько контейнеров (в том числе init-контейнеры) оригинального приложения пользователя, команда запуска которых изменена компонентом webhook на запуск файл-инжектора.
    ...
    Файл-инжектор выполняет авторизацию и получение данных из хранилища секретов, размещает секреты в виде переменных окружения и запускает оригинальную команду пользовательского контейнера. Если указана аннотация `secrets-store.deckhouse.io/restart-on-secret-change` со значением `watch-for-lease` или `watch-for-data`, файл-инжектор выполняет ротацию секретов при их изменении в хранилище.
...
1. **Kube-apiserver** — запрос на выполнение mutating-вебхука для ресурса Pod при его создании.
...
1. **Prometheus-main** — собирает метрики контейнеров secrets-store и vault-secrets-webhook.
