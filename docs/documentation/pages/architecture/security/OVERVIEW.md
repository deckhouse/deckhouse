...
description: Architecture of the Security subsystem in Deckhouse Platform.
extractedLinksOnlyMax: 0
extractedLinksMax: 0
---

This subsection describes the architecture of the Security subsystem of Deckhouse Platform (DP).
...
* [`runtime-audit-engine`](/modules/runtime-audit-engine/): Implements runtime audit (threat detection based on Linux kernel events and Kubernetes API audit events using [Falco](https://falco.org/)). The architecture is described in ["Runtime audit"](runtime-audit.html).
* [`security-events-manager`](/modules/security-events-manager/): Performs declarative collection, processing, normalization, and delivery of security events from logs of applications and Kubernetes infrastructure components. The architecture is described in ["Security events"](security-events.html).
* [`operator-trivy`](/modules/operator-trivy/): Performs periodic vulnerability scanning of the DP cluster.
