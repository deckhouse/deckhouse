...
1. **Cert-manager** — контроллер, обеспечивающий полный цикл управления сертификатами в Deckhouse Kubernetes Platform (DKP). **Cert-manager** управляет следующими кастомными ресурсами:
    ...
    {% alert level="info" %}
    **Cert-manager** поддерживает интеграцию с внешними компонентами для работы с DNS-провайдерами, которые не имеют нативной поддержки в контроллере, с помощью [webhook](https://cert-manager.io/docs/configuration/acme/dns01/webhook/). Такие интеграционные компоненты не входят в состав модуля и должны быть установлены и обслуживаться пользователем самостоятельно.
    {% endalert %}
