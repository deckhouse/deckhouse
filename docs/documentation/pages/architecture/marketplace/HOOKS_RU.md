...
Именам, переданным в `ObjectPatcher`, рантайм добавляет префикс `d8a-<instance>-` — тот же, что шаблоны пакета проставляют вручную. Хук, создающий `config` в инстансе `myapp`, получает объект `d8a-myapp-config`, а объект, отрендеренный собственными шаблонами, хук может адресовать коротким именем. Полное имя эквивалентно — префикс не добавляется дважды.

Префикс зарезервирован: политика `d8a-prefix.deckhouse.io` запрещает создание, изменение и удаление объектов с именем `d8a-*` всем, кроме компонентов платформы, поэтому объекты приложения нельзя подменить и они не конфликтуют с пользовательскими объектами в том же неймспейсе.

Подписки на ресурсы Kubernetes работают с реальными именами в кластере. `NameSelector` не префиксуется, а снапшоты возвращают `metadata.name` как есть, поэтому подписка на объект, который хук создаёт сам, должна указывать полное имя с `d8a-<instance>-`.
