...
- Когда меняются настройки установленного приложения и `spec.packageVersion` совпадает с установленной версией, проверка выполняется в validating-вебхуке. Отказ отклоняет запрос с указанной причиной. Предупреждения возвращаются клиенту, например, `d8 k` выводит их.
...
Проверка не выполняется в validating-вебхуке при создании Application и при изменении `spec.packageVersion`: в этих случаях запрос проверяется только по схеме настроек. Описывайте критичные ограничения в схеме, например, с помощью [правил CEL](settings.html#правила-cel-x-deckhouse-validations), чтобы отклонять некорректные настройки ещё до установки.
