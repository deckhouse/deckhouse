...
Одно из встроенных правил — обязательное именование объектов: каждый объект, который рендерит шаблон, должен называться `d8a-{{ .Application.Instance.Name }}-<суффикс>`. Префикс `d8a-` зарезервирован за объектами приложений и защищает их от изменения пользователями кластера. Объектам, которые создают хуки, тот же префикс проставляет рантайм, см. раздел [«Хуки»](hooks.html).
