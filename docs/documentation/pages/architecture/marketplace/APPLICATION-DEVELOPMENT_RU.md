...
Канал обновлений — это тег пути `<PACKAGE_NAME>/release-channel`, указывающий на копию образа метаданных рекомендуемой версии. `d8 package build` не создаёт теги каналов обновлений — их публикует пайплайн CI/CD.

DP не обновляет приложения по каналам обновлений: версия приложения задаётся в `spec.packageVersion` и меняется, только когда её меняет пользователь.
