...
`d8 package bootstrap application <APPLICATION_NAME>` создаёт директорию `<APPLICATION_NAME>/` в текущей рабочей директории с заготовкой пакета и инициализирует Git-репозиторий с первым коммитом.
...
Основной манифест пакета Application. Определяет метаданные, тип, требования и совместимость.
...
Поле `settings` типа `type: string` можно связать с ресурсом кластера, которым управляет
[`multitenancy-manager`](/modules/multitenancy-manager/) и который доступен проекту по гранту (например, StorageClass).

Когда поле связано и пользователь оставляет его пустым, в `values` подставляется имя ресурса, заданное для проекта как значение по умолчанию. При указании собственного значения оно проверяется по списку имён, доступных проекту. Значение, которого в этом списке нет, отклоняется.

Добавьте к полю расширение `x-deckhouse-grantable-resource` и укажите имя ресурса, доступного проекту по гранту
...
Некоторые настройки не следует менять после применения: например, смена `storageClass` после создания томов либо ни на что не влияет, либо может привести к ошибкам в работе приложения. Чтобы зафиксировать значение такого поля после успешного применения, добавьте `x-deckhouse-immutable: true`.
...
- Проверка неизменяемости начинает действовать после первого успешного применения приложения. До этого ограничение на изменение поля не действует.
- Если поле с расширением ранее не имело применённого значения и для него не задано значение по умолчанию, его можно задать позже. После успешного применения заданного значения последующие изменения отклоняются.
- Если `x-deckhouse-immutable` добавлено к объекту, неизменяемым становится весь объект. После первого успешного применения изменение любого вложенного поля будет отклонено, даже если для него отдельно не указано `x-deckhouse-immutable`. Указывайте расширение на уровне объекта, только если необходимо запретить изменение всего объекта. Чтобы запретить изменение только одного поля, добавьте `x-deckhouse-immutable` непосредственно к нему, как к `postgres.storageClass` в примере выше. В этом случае ограничение не распространяется на `postgres.volumeSize`, и его значение можно изменять.
...
- Если поле одновременно использует `x-deckhouse-grantable-resource` и `x-deckhouse-immutable`, а пользователь не задаёт значение явно, оно берётся из значения по умолчанию для проекта. Пока поле остаётся пустым, изменение этого значения не блокируется `x-deckhouse-immutable`. После успешного применения явно задать другое значение уже не получится.
- Расширение не распространяется на элементы массивов и записи словарей, добавленные при обновлении, поскольку у нового элемента нет предыдущего значения, с которым его можно сравнить.

## Локальная сборка

Чтобы собрать пакет и опубликовать его в реестре, выполните команду:
...
Пайплайн CI/CD публикует релизы пакета в реестре. Для публикации релиза необходимо настроить учётные данные реестра, а затем создать Git-тег в формате SemVer и отправить его в репозиторий.

### Переменные окружения

Для аутентификации в реестре пакетов пайплайн использует следующие переменные:

| Переменная | Описание |
|---|---|
| `PACKAGES_REGISTRY_LOGIN` | Имя пользователя для публикации в реестр |
...
registry.deckhouse.io/deckhouse/<EDITION>/packages:<PACKAGE_NAME>
    Тег с именем пакета — используется для получения списка пакетов
...
registry.deckhouse.io/deckhouse/<EDITION>/packages/<PACKAGE_NAME>/version:<RELEASE_CHANNEL>
    Рекомендуемая версия для канала обновлений
```
