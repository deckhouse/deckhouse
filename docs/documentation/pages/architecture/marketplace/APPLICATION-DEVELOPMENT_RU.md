...
- Полем считается только значение `true`. Любое другое значение метки игнорируется.
...
- Сравнение выполняется после подстановки дефолтов из схемы: если убрать помеченный ключ из манифеста, останется
  значение, с которым приложение уже работает, поэтому само удаление ключа изменением не считается.
