...
### Поля, которые нельзя изменить после установки (`x-deckhouse-immutable`)

Часть настроек имеет смысл только на момент установки: смена `storageClass` после того, как тома уже созданы, либо
ни на что не влияет, либо ломает приложение. Пометьте такое поле `x-deckhouse-immutable: true` — его можно будет
задать при создании приложения и больше никогда.
...
- Метка действует только при значении `true`. Любое другое значение метки игнорируется.
- Метка на объекте фиксирует весь блок как одно значение: любое изменение внутри отклоняется и сообщается как
  изменение блока, включая поля, которые сами по себе менять можно. Ставьте метку на объект, только если блок
  осмыслен как единый выбор; чтобы заморозить одно поле, помечайте само поле — как `postgres.storageClass` выше,
  при котором `postgres.volumeSize` остаётся изменяемым.
- Обновление, меняющее помеченное поле, отклоняется валидирующим вебхуком с указанием имени поля.
- В веб-консоли помеченное поле доступно только для чтения в форме редактирования установленного приложения и
  доступно для изменения в форме установки.
- Сравнение выполняется после подстановки дефолтов из схемы, поэтому убрать помеченный ключ из манифеста можно,
  только если у ключа есть `default`, возвращающий значение, с которым приложение уже работает. Удаление ключа без
  дефолта отклоняется, как и удаление блока с помеченными полями: внутрь блока, которого в манифесте больше нет,
  дефолты не подставляются, и зафиксированные значения были бы потеряны.
- Метка не распространяется на элементы массивов и записи карт, которые обновление добавляет: у нового элемента нет
  предыдущего значения, относительно которого его можно фиксировать.
