...
- When the Application of an installed application is changed, the validating webhook runs the check of the installed version. A rejection denies the request with the specified reason. Warnings are returned to the client, for example, `d8 k` prints them.
...
The check doesn't run in the validating webhook when an Application is created: in this case, the request is checked against the settings schema only. When `spec.packageVersion` changes, the webhook runs the check of the installed version, not of the new one. Describe critical constraints in the schema, for example, with [CEL rules](settings.html#cel-rules-x-deckhouse-validations), to reject invalid settings before installation.
