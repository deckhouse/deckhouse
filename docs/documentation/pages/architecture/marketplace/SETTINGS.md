...
| The Application of an installed application is changed | Additionally, the checks of the installed version: CEL rules that reference `oldSelf`, availability of grantable resources, and the [settings validation hook](hooks.html#settings-validation-hook) | The request is rejected |
