...
A release channel is a tag of the `<PACKAGE_NAME>/release-channel` path that points to a copy of the version metadata image of the recommended version. `d8 package build` doesn't create release channel tags; they are published by the CI/CD pipeline.

DP doesn't update applications by release channels: the version of an application is set in `spec.packageVersion` and changes only when the user changes it.
