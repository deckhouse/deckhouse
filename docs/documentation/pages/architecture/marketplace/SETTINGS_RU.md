...
| Изменяются настройки установленного приложения (`spec.packageVersion` совпадает с установленной версией) | Дополнительно: правила CEL с `oldSelf`, доступность ресурсов по грантам и [хук валидации настроек](hooks.html#хук-валидации-настроек) | Запрос отклоняется |
...
Правила с `oldSelf` сравнивают новые настройки с ранее применёнными. Такие правила проверяются только при изменении настроек установленного приложения: при установке и при изменении `spec.packageVersion` они пропускаются. Чтобы запретить изменение поля после установки, используйте [`x-deckhouse-immutable`](#неизменяемые-поля-x-deckhouse-immutable) — это расширение проверяется при каждом обновлении.
