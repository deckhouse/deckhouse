...
- Прежнее значение того же уровня доступно через переменную `oldSelf`. Правила, использующие `oldSelf`, считаются transition rule и проверяются только при обновлении (когда на этом уровне есть прежнее значение); при первичном создании или для впервые появившегося поддерева такие правила автоматически пропускаются. Поведение совпадает с `x-kubernetes-validations`.
...
- Объявление поля неизменяемым (immutable) через переменную `oldSelf` (transition rule):
  ...
  Правило проверяется только на обновлении: при первичном создании (когда прежнего значения ещё нет) оно пропускается автоматически — так же, как transition rule в `x-kubernetes-validations`.
