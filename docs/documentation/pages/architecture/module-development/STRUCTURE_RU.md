...
> **Примечание.** Если CRD вашего модуля содержат поля, ссылающиеся на кластерные ресурсы (напр. `StorageClass`, `ClusterIssuer`, `ClusterRole`), рассмотрите также поставку CRD `GrantableClusterResourceReference`, чтобы эти поля валидировались и (опционально) подставлялись по умолчанию механизмом грантов `multitenancy-manager`. Подробности — в [руководстве для разработчиков модулей](/modules/multitenancy-manager/usage_ru.html#для-разработчиков-модулей).
