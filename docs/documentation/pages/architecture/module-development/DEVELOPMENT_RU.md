...
lang: ru
description: Инструменты и подходы для разработки и отладки модулей Deckhouse Platform, включая локальное тестирование и поиск ошибок.
---
...
Его применение в production-кластерах **не рекомендуется**. Поддержка ресурса может быть исключена в следующих версиях Deckhouse Platform.
{% endalert %}
...
Необязательный параметр `spec.scanInterval` устанавливает интервал времени для проверки образов в хранилище образов. По умолчанию задан интервал в 15 секунд. Для принудительного обновления можно изменить интервал, либо установить на ModulePullOverride аннотацию `modules.deckhouse.io/renew=true`. Аннотация заставляет заново скачать и развернуть модуль, даже если digest образа не изменился, и снимается после успешного развёртывания.
...
В данном примере модуль будет доступен в редакции `ee` (DP Enterprise Edition)
...
1. В [ModuleSource](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#modulesource) присутствуют два модуля `echo` и `hello-world`. Для них определена политика обновления, они загружаются и устанавливаются в DP:
...
Механизм `from-to` позволяет пропускать [пошаговые обновления модуля](../development/#логика-автообновления-модулей). Если текущая установленная версия модуля (в статусе `Deployed`) попадает в диапазон `from-to`, DP пропускает промежуточные релизы и устанавливает последнюю доступную версию в пределах `to`.
