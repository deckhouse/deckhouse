...
Deckhouse Kubernetes Platform (DKP) использует хранилище образов контейнеров для загрузки и обновления модуля. В хранилище образов хранятся артефакты модуля. Артефакты модуля появляются в результате сборки модуля, после чего их можно загрузить (опубликовать) в хранилище образов.
