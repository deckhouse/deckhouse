...
lang: ru
description: Виды артефактов модуля (образы контейнеров, образ модуля, релиз) и процесс их сборки и публикации в хранилище образов контейнеров.
---

Deckhouse Platform (DP) использует хранилище образов контейнеров для загрузки и обновления модуля. В хранилище образов хранятся артефакты модуля. Артефакты модуля появляются в результате сборки модуля, после чего их можно загрузить (опубликовать) в хранилище образов.

## Состав артефактов модуля

В результате сборки модуля создаются три типа артефактов, которые впоследствии загружаются в хранилище образов:
...
- **Релиз**. Артефакт версии модуля. На основе данных релиза DP принимает решение об обновлении модуля в кластере. У релизов есть два типа тегов: тег [семантического версионирования](https://semver.org/lang/ru/) (как у образа модуля) и тег, соответствующий каналу обновлений (например, `alpha`, `beta` и т. д.). [В шаблоне модуля](https://github.com/deckhouse/modules-template/) есть пример workfklow для GitHub Actions, где релиз создается автоматически при сборке.

## Сборка артефактов модуля и публикация в хранилище образов

Для сборки артефактов модуля и загрузки их в хранилище образов в рамках процесса CI/CD мы предлагаем воспользоваться подготовленными [GitHub Actions](https://github.com/deckhouse/modules-actions).

[В репозитории шаблона модуля](https://github.com/deckhouse/modules-template/) представлен пример модуля, который содержит простой workflow для GitHub Actions и использует [GitHub Packages](https://github.com/features/packages) (ghcr.io) в качестве хранилища образов. В представленном примере workflow используется следующая логика:
- Сборка артефактов модуля при изменениях в рамках PR и при слиянии изменений в ветку main.
- Сборка артефактов модуля из тегов с использованием продуктивного хранилища образов.
- Публикация модуля в хранилище образов GitHub Packages в выбранный [канал обновлений](../versioning/#каналы-обновлений) из тега.  
...
Вы также можете модифицировать workflow, предусмотреть использование своего хранилища образов и более сложный процесс сборки и публикации (например, с использованием отдельных хранилищ образов для разработки и промышленной эксплуатации).
...
1. Опубликуйте изменения в коде модуля в ветке проекта на GitHub. Это запустит сборку артефактов модуля и их публикацию в хранилище образов.
...
1. После успешного выполнения workflow в хранилище образов появится новая версия модуля. Опубликованную версию модуля можно [использовать в кластере](../run/).
