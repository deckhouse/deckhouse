...
**Пример:** Модули `prometheus` и `test` включены в кластере + для `prometheus` задана условная зависимость `test: "=v0.22.1 !optional"` + попытка обновления `test` до версии `0.23.1` → `test` не будет обновлен, т.к. требуемая версия не соответствует `requirements` для `prometheus`.
