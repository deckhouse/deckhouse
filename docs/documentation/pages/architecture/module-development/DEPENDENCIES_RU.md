...
Модуль в Deckhouse Kubernetes Platform может иметь обязательные и условные зависимости от версий других модулей.
...
{% alert level="danger" %}
Условные зависимости для модулей доступны для Deckhouse Kubernetes Platform, начиная с версии 1.73.
...
Условная зависимость используется, когда модуль работает самостоятельно, но может использоваться совместно с другим модулем, **если он включён**.

{% alert level="info" %}
Условные зависимости могут влиять на возможность включения, выключения и обновления обоих модулей: зависимого и того, от которого он зависит.
{% endalert %}

Чтобы указать, что зависимость является условной, добавьте `!optional` к строке ограничения версии модуля, от которого может зависеть целевой модуль:
...
#### Ограничения по включению и выключению prometheus при наличии условной зависимости от test

При наличии условной зависимости от версии другого модуля целевой имеет следующие ограничения по включению и выключению:

1. Если в кластере не включен `test`, `prometheus` может быть включен.

   **Пример:** `test` выключен + для `prometheus` задана условная зависимость `test: ">v0.22.1 !optional"` → `prometheus` будет включен, требование пропускается.
...
1. Если в кластере уже включен `test`, включение `prometheus` с `requirements` возможно только, если в `requirements` указаны требования, которым соответствует текущая версия `test`.
   ...
   **Пример:** в кластере включен `test` версии `v0.21.1` + для `prometheus` задана условная зависимость `test: ">v0.22.1 !optional"` → установка/включение `prometheus` завершится ошибкой о несоответствии зависимости (текущая версия `test` не соответствует `requirements`).

1. Если `test` будет выключен в кластере, `prometheus` останется включенным.

   **Пример:** в кластере включен `test` + для `prometheus` задана условная зависимость `test: ">v0.22.1 !optional"` → выключение `test` пройдет успешно, `prometheus` выключен не будет.

#### Ограничения по обновлению prometheus при наличии условной зависимости от test

При наличии условной зависимости от версии другого модуля целевой модуль имеет следующие ограничения по обновлению:

1. `prometheus` может быть обновлен, даже если в кластере нет модуля `test`.

   **Пример:** `test` выключен + для `prometheus` задана условная зависимость `test: ">v0.22.1 !optional"` → `prometheus` будет обновлен.

1. Обновление для `prometheus` будет заблокировано, пока в кластере не обновится `test` указанной в `requirements` версии.

   **Пример:** `test` включен + для `prometheus` задана условная зависимость `test: ">v0.22.1 !optional"` → `prometheus` не будет обновлен, пока не обновится `test`.
...
**Пример:** `prometheus` включен + для `prometheus` задана условная зависимость `test: ">v0.22.1 !optional"` → попытка включения `test v0.21.1` завершится ошибкой о несоответствии зависимости (версия `test v0.21.1` не соответствует условию в `requirements` для `prometheus`).
...
**Пример:** Модули `prometheus` и `test` включены в кластере + для `prometheus` задана условная зависимость `test: "=v0.22.1 !optional"` + попытка обновления `test` до версии `0.23.1` → `test` не будет обновлен, так как требуемая версия не соответствует `requirements` для `prometheus`.

{% alert level="warning" %}
- Включение и отключение модулей может занимать больше времени из‑за дополнительных проверок экстендера.
- Известное ограничение: во время обработки модулей список включённых модулей может кратковременно быть пустым. В редких случаях это позволяет ошибочно пройти проверку условной зависимости. Если столкнулись, повторите операцию.
{% endalert %}
