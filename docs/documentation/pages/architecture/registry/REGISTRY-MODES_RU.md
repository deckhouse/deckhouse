...
{% alert level="warning" %}
Рекомендуется использовать разные диски для хранения данных registry (`/opt/deckhouse/registry`) и etcd. Использование одного диска может привести к деградации производительности etcd при операциях с хранилищем образов контейнеров.
{% endalert %}
