...
В DKP поддерживаются последние пять минорных версий Kubernetes. Полный список актуальных версий доступен [в разделе Справка](TODO).
...
Для автоматического обновления минорной версии control plane (например, с `1.28.*` на `1.30.*`) укажите [`kubernetesVersion: Automatic`](../reference/cr/clusterconfiguration/#clusterconfiguration-kubernetesversion) в ресурсе ClusterConfiguration. Будет выбрана минорная версия Kubernetes, используемая в DKP по умолчанию на момент обновления.

### Ручное обновление минорных версий

Для ручного обновления минорной версии control plane (например, с `1.28.*` до `1.30.*`), укажите нужную версию в параметре [`kubernetesVersion`](../reference/cr/clusterconfiguration/#clusterconfiguration-kubernetesversion) ресурса ClusterConfiguration. Например: `kubernetesVersion: 1.30`.
