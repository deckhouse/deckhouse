...
description: Виды релизов, каналы обновлений и процесс обновления управляющего слоя и узлов в Deckhouse Platform.
---

## Релизы

Релизом считается любая опубликованная версия Deckhouse Platform (DP). Каждый релиз распространяется по каналам обновлений с указанными задержками. DP публикует два вида релизов:
...
Deckhouse Platform использует **пять каналов обновлений**, обеспечивающих поэтапное развертывание новых версий.
Каждая новая версия DP сначала публикуется в канале **Alpha** и постепенно продвигается к **Rock Solid**.
...
1. DP загружает данные о релизах из канала `EarlyAccess`.
1. Сравнивает их с данными из существующих в кластере кастомных ресурсов DeckhouseRelease.
   ...
   - Если более поздние релизы уже перешли в статус `Deployed` (успешно установлены), переход на новый релиз произойдёт не сразу. DP останется на текущем релизе до тех пор, пока на канале `EarlyAccess` не выйдет более поздняя версия, которую можно будет установить.

### Процесс переключения на менее стабильный канал

При переключении на менее стабильный канал (например, с `EarlyAccess` на `Alpha`):

1. DP скачивает данные о релизах из канала `Alpha`.
1. Сравнивает их с данными из существующих в кластере кастомных ресурсов DeckhouseRelease.
...
Для автоматического обновления минорной версии control plane (например, с `1.28.*` на `1.30.*`) укажите [`kubernetesVersion: Automatic`](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#clusterconfiguration-kubernetesversion) в ресурсе ClusterConfiguration. Будет выбрана минорная версия Kubernetes, используемая в DP по умолчанию на момент обновления.
...
Команда запускает обновление до минорной версии Kubernetes, используемой в DP по умолчанию на момент обновления. Чтобы отследить ход обновления, ориентируйтесь на версию Kubernetes, указанную в выводе команды описания узлов:
...
При откате до предыдущей версии (**downgrade**, не поддерживается для редакций CSE):
...
При каждом выпуске новой версии Deckhouse Platform (DP) формируется *changelog* — подробный список изменений, включающий новые возможности, исправления ошибок, обновления компонентов, а также важные замечания по совместимости.

Changelog для конкретной версии DP можно найти [в общем списке релизов проекта Deckhouse на GitHub](https://github.com/deckhouse/deckhouse/releases).

Сводная информация о ключевых изменениях, обновлениях версий компонентов, а также о том, какие компоненты в кластере будут перезапущены в процессе обновления, доступна в описании нулевой патч-версии релиза: [пример для релиза DP v1.68](https://github.com/deckhouse/deckhouse/releases/tag/v1.68.0).
...
1. При необходимости внесите корректировки в конфигурацию кластера.

## Проверка зависимостей при обновлении

Перед применением нового релиза Deckhouse Platform проверяет состояние кластера. При обнаружении одной из следующих несовместимостей, процесс обновления прерывается:
