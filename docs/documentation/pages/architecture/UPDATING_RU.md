...
Релизом считается любая опубликованная версия платформы. Каждый релиз распространяется по каналам обновлений с указанными задержками. Deckhouse Kubernetes Platform (DKP) публикует два вида релизов:
...
- **Stable** — стабильный канал обновлений для кластеров, в которых закончена активная работа и преимущественно осуществляется эксплуатация. Обновления публикуются не ранее чем через 14 дней после выхода релиза.
- **Early Access** — рекомендуемый канал обновлений, если вы не уверены в выборе. Подойдет для кластеров, в которых идет активная работа (запускаются, дорабатываются новые приложения и т. п.). Обновления публикуются не ранее чем через 7 дней после выхода релиза.
...
Для автоматического обновления минорной версии control plane (например, с `1.26.*` на `1.28.*`) укажите `kubernetesVersion: Automatic` в ресурсе [ClusterConfiguration](#TODO). При этом для обновления будет использоваться минорная версия Kubernetes, используемая в DKP по умолчанию на момент обновления (#TODO).

### Ручное обновление минорных версий

Обновить минорную версию control plane (например, с `1.26.*` на `1.28.*`) вручную можно несколькими способами:

- C помощью параметра `kubernetesVersion` в ресурсе [ClusterConfiguration](#TODO). Для этого явно укажите необходимую версию в параметре, например, `kubernetesVersion: 1.30`.
- С помощью команды редактирования ресурса [ClusterConfiguration](#TODO):
...
## Учёт зависимостей при обновлении
...
- Текущая версия Kubernetes не входит в список поддерживаемых новым релизом платформы.
