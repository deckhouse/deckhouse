...
For components such as [`operator-trivy`](/modules/operator-trivy/), `image-availability-exporter`, `deckhouse-controller`, and others that access the registry directly, requests go through the in-cluster proxy located on the master nodes.
