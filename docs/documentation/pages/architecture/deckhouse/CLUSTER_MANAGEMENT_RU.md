...
* собирает метрики управляемого кластера:
  * `CPU cores` — количество ядер узлов;
  * `Memory` — общий объём памяти узлов, ГиБ;
  * `Filesystem` — общий объём корневых дисков на узлах, ГиБ;
...
1. [**Prometheus**](/modules/prometheus/) — получение метрик управляемого кластера.

1. [**Upmeter**](/modules/upmeter/) — получение информации о доступности управляемого кластера.

1. **Dex** — синхронизация токенов доступа для запросов в API Kubernetes API из управляющего кластера в управляемый.
