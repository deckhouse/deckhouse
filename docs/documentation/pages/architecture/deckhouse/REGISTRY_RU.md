...
Модуль `registry` отвечает за управление настройками хранилища образов компонентов DKP.

Модуль работает в следующих режимах:

* `Direct` — использование прямого доступа к внешнему хранилищу образов по фиксированному адресу `registry.d8-system.svc:5001/system/deckhouse`. Фиксированный адрес, при изменении параметров хранилища образов, позволяет избежать повторного скачивания образов и перезапуска компонентов при смене параметров хранилища образов.

* `Proxy` — использование внутреннего кеширующего прокси хранилища образов с обращением к внешнему хранилищe образов, с запуском кеширующего прокси хранилища образов на control plane (master) узлах. Режим позволяет сократить количество запросов к внешнему хранилищe образов за счёт кеширования образов. Кешируемые данные хранятся на control plane (master) узлах. Обращение к внутреннему хранилищу образов выполняется по фиксированному адресу `registry.d8-system.svc:5001/system/deckhouse` аналогично `Direct` режиму.
...
1. **Registry-incluster-proxy** (режим `Direct`) — хранилище образов на базе [Distribution](https://github.com/distribution/distribution). Distribution — это Open Source-проект, который является основой для хранения и распределения контейнерных образов и другого контента с использованием спецификации [OCI Distribution Specification](https://github.com/opencontainers/distribution-spec). Registry-incluster-proxy устанавливается в кластере как Deployment, не хранит и не кеширует образы, а пересылает запросы к внешнему хранилищу образов, указанному в [настройках режима Direct в ModuleConfig `deckhouse`](/modules/deckhouse/configuration.html#parameters-registry-direct).
   ...
   * **auth** — сайдкар-контейнер, реализующий функционал аутентификации и авторизации при обращении к хранилищу образов. Является [Open Source-проектом](https://github.com/cesanta/docker_auth). 
     ...
     1. **Получение токена**. Клиент отправляет запрос к сервису аутентификации auth (например, `registry.d8-system.svc:5051/auth`), используя значения `service` и `scope` из заголовка `WWW-Authenticate`. Сервис auth возвращает непрозрачный Bearer-токен, который представляет авторизованный доступ клиента.
     ...
     1. **Проверка доступа**. Distribution валидирует токен и содержащиеся в нём `claims`, после чего начинает сессию загрузки/выгрузки образов.

     Подробнее с описанным выше принципом работы можно ознакомиться [в документации Docker](https://docs.docker.com/reference/api/registry/auth/).
...
1. **Registry-nodeservices-manager** (режимы `Proxy` или `Local`) — контроллер, управляющий жизненным циклом компонентов registry-nodeservices. Registry-nodeservices-manager выполняет следущие операции:
   ...
   * синхронизирует набор сертификатов PKI в каталоге `/etc/kubernetes/registry` на control plane (master) узлах;
   * удаляет манифесты, конфигурационные файлы и сертификаты из каталога `/etc/kubernetes/registry`, тем самым удаляя локальное хранилище образов, в случае переключения модуля в режим `Direct` или `Unmanaged`.
...
1. **Kube-apiserver**:
   ...
   * проверяет наличие лейбла `node-role.kubernetes.io/master` у ресурса Node;
   * проверяет, что статический под registry-nodeservices находится в статусе `Ready`;
   * следит за изменениями секрета `registry-node-config-<Node Name>` с конфигурацией локального хранилища образов.

1. **Внешнее хранилище образов** — перенаправляет запросы к хранилищу образов.
