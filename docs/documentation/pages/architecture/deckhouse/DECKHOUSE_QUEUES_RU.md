...
| Parallel      | `parallel_queue_0` … `parallel_queue_19`  | Параллельный ModuleRun с учётом зависимостей модулей (20 очередей)                                 |
...
- задачи могут вставляться как в конец (`AddLast`), так и в начало (`AddFirst`) очереди;
- выполнение задач происходит с начала очереди;
...
![Диаграмма последовательности при установке функциональных модулей](../../images/architecture/deckhouse/DECKHOUSE_QUEUE_MODULES_FUNCTIONAL.ru.svg)
...
| `{packageName}/{hookQueue}`         | Выполнение хуков, запускаемым по Kubernetes- или schedule-событиям (очередь из binding-хука)    |
