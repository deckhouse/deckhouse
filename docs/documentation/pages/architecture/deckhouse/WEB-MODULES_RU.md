...
description: Архитектура модулей Deckhouse Platform, обеспечивающих веб-интерфейс системы.
---

## Модуль console

Модуль [`console`](/modules/console/) реализует веб-интерфейс Deckhouse Platform (DP), который упрощает управление платформой и позволяет отслеживать состояние системы.
...
1. **Frontend** — состоит из одного контейнера **frontend** и предоставляет веб-интерфейс для пользователей и администраторов DP.

1. **Backend** — состоит из одного контейнера **backend** и реализует API-интерфейс, обеспечивающий следующие возможности:

   * получение, создание, удаление и изменение ресурсов DP в соответствии с правами пользователя;
   * генерация kubeconfig с профилем пользователя;
   * определение окружения, в котором развернут кластер DP;
   * обнаружение версий DP и Kubernetes;
...
1. **Kube-apiserver**:
   ...
   - создание, удаление, изменение и отслеживание ресурсов DP.

1. [**Upmeter**](/modules/upmeter/) — получение информации о доступности DP.
...
Модуль [`documentation`](/modules/documentation/) — это реализация веб-интерфейса с документацией, соответствующей запущенной версии DP.
...
- **Documentation** — компонент, реализующий веб-интерфейс документации.
  ...
  * **builder** — сайдкар-контейнер, который динамически расширяет документацию при установке новых модулей DP. Для рендеринга и генерации актуального содержимого сайта используется генератор статических сайтов [Hugo](https://github.com/gohugoio/hugo).
