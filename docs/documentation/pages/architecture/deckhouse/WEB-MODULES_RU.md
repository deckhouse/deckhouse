...
{% alert level="info" %}
Номерами на схеме отмечен порядок прохождения запроса пользователя к компоненту `web`:
- нна шагах 1, 2 и 3 запрос проходит через Ingress NGINX Controller, где выполняется обязательная аутентификация пользователя с использованием модуля [`user-authn`](/modules/user-authn).
{% endalert %}
