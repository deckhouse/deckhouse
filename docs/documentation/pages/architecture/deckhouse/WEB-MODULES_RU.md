...
- на шагах 1, 2 и 3 реализуется обработка запроса Ingress NGINX Controller с обязательной аутентификацией пользователя, реализуемой модулем [`user-authn`](/modules/user-authn). Подробнее с описанием архитектуры модуля `user-authn` можно ознакомиться [в соответствующем разделе документации](../iam/user-authn.html).
{% endalert %}
...
- на шагах 1, 2 и 3 реализуется обработка запроса Ingress NGINX Controller с обязательной аутентификацией пользователя, реализуемой модулем [`user-authn`](/modules/user-authn);
