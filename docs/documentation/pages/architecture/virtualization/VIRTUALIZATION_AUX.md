...
1. **Pre-delete-hook** (Job) — задание, удаляющее ресурсы InternalVirtualizationKubeVirt/config и InternalVirtualizationCDI/config перед деактивацией модуля [`virtualization`](/modules/virtualization/).
...
1. **Host network/Linux kernel** — обновляет маршруты и правила маршрутизации на узле.
...
1. **Kube-apiserver** — удаляет ресурсы InternalVirtualizationKubeVirt/config и InternalVirtualizationCDI/config.
