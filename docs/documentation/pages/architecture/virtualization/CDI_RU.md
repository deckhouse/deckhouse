...
- **Данные KubeVirt** — импортируемый файл следует рассматривать как диск ВМ KubeVirt. При необходимости CDI автоматически распакует и преобразует файл из поддерживаемых форматов в формат `raw` или `qcow2` (в зависимости от режима тома (volume mode)). Также будет изменён размер диска, чтобы использовать всё доступное пространство.
- **Архивные данные** — этот тип означает, что данные хранятся в TAR-архиве. Сжатие архивов не поддерживается. CDI извлекает содержимое архива в том, который затем можно использовать либо с обычным подом, либо с ВМ, используя функцию файловой системы KubeVirt.
...
1. **Cdi-apiserver** — [Kubernetes Extension API Server](https://kubernetes.io/docs/tasks/extend-kubernetes/setup-extension-api-server/), который используется для проверки и изменения ресурсов Kubernetes API через механизмы [Validating/Mutating Admission Controllers](https://kubernetes.io/docs/reference/access-authn-authz/admission-controllers/). Cdi-apiserver реализует validating- и mutating-вебхуки для следующих типов ресурсов:
   ...
   - InternalVirtualizationDataVolume — абстракция поверх стандартных PVC для создания на их основе дисков ВМ;
   - InternalVirtualizationCDI — кастомный ресурс, используемый cdi-operator, для установки и настройки компонентов CDI;
   - InternalVirtualizationDataImportCron — определяет задание cron для импорта образов дисков в виде PVC;
...
1. **Cdi-deployment** (он же **cdi-controller**) — контроллер, выполняющий следующие операции с кастомными ресурсами InternalVirtualizationDataVolumes:
   ...
   - импорт образов и дисков ВМ в PVC-тома для использования их в виртуальных машинах, управляемых KubeVirt;
   ...
   - **cdi-importer** — для импорта образов и дисков ВМ. Cdi-importer также конвертирует образы в зависимости от типа целевого PVC:

     - в формат `raw`, если для PVC установлен режим тома (volume mode) `Block`;
     - в формат `qcow2`, если установлен режим `Filesystem`.
...
1. **KubeVirt** — предоставляет возможность импортировать образы и диски ВМ в PVC-тома для использования в ВМ, управляемых KubeVirt.

1. **DVCR (Deckhouse Virtualization Container Registry)** — использует хранилище образов в качестве источника для импорта образов и дисков ВМ.

С CDI взаимодействуют следующие внешние компоненты:
