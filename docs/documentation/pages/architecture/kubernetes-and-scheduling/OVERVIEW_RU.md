...
description: Архитектура подсистемы Kubernetes & Scheduling в Deckhouse Platform.
extractedLinksOnlyMax: 0
extractedLinksMax: 0
---

В данном подразделе описывается архитектура модулей, входящих в подсистему Kubernetes & Scheduling платформы Deckhouse Platform (DP).
...
* [`pod-reloader`](/modules/pod-reloader/) — позволяет автоматически перезапускать нагрузку в случае изменения определенных ConfigMap или Secret;
...
* [`adaptive-resource-management`](/modules/adaptive-resource-management/) — автоматически подбирает оптимальные resource requests и limits для контейнеров на основе рекомендаций VPA (Vertical Pod Autoscaler).
