...
* На схеме показан под `pod-common`, который представляет собой любой под, управляемый kubelet.
{% endalert %}
...
1. **common-pod** — контролирует состояние контейнеров пода, выполняя пробы Startup, Liveness и Readiness в соответствии со спецификацией пода. Подробнее о пробах можно узнать в [документации Kubernetes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes);
