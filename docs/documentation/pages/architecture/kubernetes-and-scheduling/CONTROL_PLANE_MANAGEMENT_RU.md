...
Мониторинг control plane кластера осуществляется этим же модулем, который обеспечивает безопасный сбор метрик и предоставляет базовый набор правил мониторинга следующих компонентов кластера:
...
Метрики **kube-apiserver** собираются **prometheus-main** напрямую. Этот модуль добавляет правила сбора этих метрик в конфигурацию **prometheus-main**.
