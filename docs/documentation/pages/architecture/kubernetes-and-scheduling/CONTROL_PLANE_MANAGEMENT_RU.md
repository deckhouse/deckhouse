...
Мониторинг control plane кластера осуществляется модулем `control-plane-manager`, который обеспечивает безопасный сбор метрик и предоставляет базовый набор правил мониторинга следующих компонентов кластера:
...
Взаимодействие компонента control-plane-proxy с control plane кластера изображено на приведенной выше схеме архитектуры модуля `control-plane-manager`.
