...
1. **Managed-kafka-operator** (Deployment) — оператор Kubernetes, состоящий из одного контейнера **manager** и выполняющий следующие операции:
   ...
   - согласование состояния кастомных ресурсов [Kafka](/modules/managed-kafka/cr.html#kafka) во всех пользовательских неймспейсах. Кастомные ресурсы Kafka и KafkaClass определяют настройки экземпляра Kafka;

   - создание и управление кастомным ресурсом [Certificate](https://cert-manager.io/docs/usage/certificate/) и ресурсами StatefulSet, Service, Secret и PersistentVolumeClaim, относящимися к экземпляру Kafka.
...
1. **D8ms-kfk-\<INSTANCE_NAME>** (StatefulSet) — компонент, состоящий из одного контейнера **kafka** и отвечающий за запуск и работу экземпляра Kafka. Создаётся компонентом managed-kafka-operator.
