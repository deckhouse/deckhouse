...
![Архитектура модуля extended-monitoring](../../images/architecture/observability/c4-l2-extended-monitoring.ru.svg)
...
1. **X509-certificate-exporter** — экспортер Prometheus, который обеспечивает контроль сертификатов:
   ...
   * Модуль также содержит соответствующие Prometheus-правила, на основе которых Prometheus создает алерты. Это позволяет не пропускать критические моменты и вовремя обновлять сертификаты, избегая простоя приложений из-за просроченных сертификатов.
