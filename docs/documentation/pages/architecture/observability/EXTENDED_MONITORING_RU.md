...
1. **Image-availability-exporter** — экспортер Prometheus, который осуществляет мониторинг контейнерных образов запущенных в кластере подов. Image-availability-exporter добавляет метрики о недоступности образов в хранилищах образов контейнеров для всех типов рабочей нагрузки (Deployments, StatefulSets, DaemonSets, CronJobs). Модуль также содержит соответствующие Prometheus-правила, на основе которых Prometheus создает алерты. Это помогает заранее узнать о возможных проблемах с запуском или обновлением подов.
...
1. **Prometheus-main** — собирает метрики с экспортеров модуля. В модуле также содержатся правила ObservabilityMetricsRulesGroups, которые модуль [`observability`](/modules/observability/) конвертирует в Prometheus-правила. Prometheus создает алерты по этим правилам (при достижении пороговых значений).
