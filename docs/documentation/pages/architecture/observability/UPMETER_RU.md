...
Модуль [`upmeter`](/modules/upmeter/) проверяет доступность Deckhouse Kubernetes Platform (DKP) и состояние компонентов кластера в реальном времени. Результаты проверок отображаются на дашбордах.
...
Архитектура модуля [`upmeter`](/modules/upmeter/) на уровне 2 модели C4 и его взаимодействия с другими компонентами DKP изображены на следующей диаграмме:
...
1. **Upmeter** (StatefulSet) — контроллер, выполняющий следующие операции:
   ...
   - отслеживает кастомный ресурс [Downtime](/modules/upmeter/cr.html#downtime) и рассчитывает доступность компонентов DKP на основе интервалов простоя в этом ресурсе;
