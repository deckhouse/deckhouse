...
description: Архитектура модуля upmeter в Deckhouse Platform.
---

Модуль [`upmeter`](/modules/upmeter/) проверяет доступность Deckhouse Platform (DP) и состояние компонентов кластера в реальном времени. Результаты проверок отображаются на дашбордах.
...
Архитектура модуля [`upmeter`](/modules/upmeter/) на уровне 2 модели C4 и его взаимодействия с другими компонентами DP изображены на следующей диаграмме:
...
1. **Upmeter** (StatefulSet) — контроллер, выполняющий следующие операции:
   ...
   - отслеживает кастомный ресурс [Downtime](/modules/upmeter/cr.html#downtime) и рассчитывает доступность компонентов DP на основе интервалов простоя в этом ресурсе;
   - хранит метрики доступности компонентов DP в локальной базе SQLite;
   - принимает и обрабатывает данные проверок компонентов DP;
...
1. **Upmeter-agent** (DaemonSet) — компонент, работающий на master-узлах кластера и выполняющий на регулярной основе следующие группы проверок:
   ...
   - deckhouse — проверка состояния кластера DP, а также проверка работоспособности deckhouse-контроллера [модуля `deckhouse`](/modules/deckhouse);
...
1. **Status** (Deployment) — компонент, состоящий из одного контейнера **status** и реализующий веб-страницу с текущим статусом доступности всех компонентов DP.

1. **Webui** (Deployment) — компонент, состоящий из одного контейнера **webui** и реализующий дашборд для просмотра истории доступности каждого компонента DP.
