...
Архитектура модуля **ingress-nginx** на уровне 2 модели C4 и его взаимодействия с другими компоненами платформы изображены на следующей диаграмме:
...
1. **controller-nginx** ([Advanced DaemonSet](https://openkruise.io/docs/user-manuals/advanceddaemonset)) - нестандартный DaemonSet с продвинутыми возможностями, управляемый **kruise-controller-manager**, размещается на Frontend-нодах. В свою очередь состоит из следующих контейнеров:
   ...
   * **kube-rbac-proxy** - sidecar-контейнер с авторизирующим прокси на основе Kubernetes RBAC для организации защищенного доступа к метрикам и heathcheck контроллера и protobuf-exporter. [Open-source проект](https://github.com/brancz/kube-rbac-proxy).
...
3. **kruise-controller-manager** (Deployment) - контроллер, управляющий Custom Resource [Advanced DaemonSet](https://openkruise.io/docs/user-manuals/advanceddaemonset). Данное расширение DaemonSet позволяет использовать продвинутые возможности при обновлении **Ingress-NGINX controller**, которых нет в стандартной реализации DaemonSet-контроллера Kubernetes. Состоит из следующих контейнеров:
   ...
   * **kruise-state-metrics** - sidecar-контейнер в поде **kruise-controller-manager**, cледит за состоянием объектов API OpenKruise и предоставляет соответствующие метрики (но не метрики работы самого **kruise-controller-manager**).
...
4. **failover-cleaner** (DaemonSet) - деплоится на нодах кластера, отмеченных меткой `ingress-nginx-controller.deckhouse.io/need-hostwithfailover-cleanup=true`. Представляет собой bash-скрипт, который выполняет актуализацию правил iptables в зависимости от используемого инлета контроллера. При штатной работе ingress-controller компонент **failover-cleaner** не запущен ни на одной ноде.
...
1. **kube-apiserver**:
   ...
   * авторизация запросов на получение метрик, статистики и проверки healtchcheck контроллера.
...
3. Cлужебными сервисами платформы (Console, Dashboard, Grafana и прочие) - пересылка авторизованных через **Dex** HTTP-запросов.
...
Для типов inlet **HostPort**, **HostPortWithProxyProtocol**, **HostPortWithSSLPassthrough** и **HostWithFailover** **балансировщик нагрузки** разворачивается самостоятельно пользователем платформы, либо он вообще может отсутствовать. Пользователь настраивает бэкенды балансировщика или (в случае его отсутствия) обеспечивает сетевую связность до ingress-controller. Точкой входа в ingress-controller в этом случае являются порты на нодах кластера, на которых запущен контроллер.
