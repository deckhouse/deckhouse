...
* [node-local-dns](/modules/node-local-dns/) - разворачивает кэширующий DNS-сервер на каждом узле кластера, экспортирует данные в **Prometheus** для удобного анализа работы DNS в кластере [на доске](/modules/node-local-dns/#grafana-dashboard) Grafana. Архитектура кэширующего DNS-сервера описана в данном подразделе на соответствующей [странице](dns-caching.html).
