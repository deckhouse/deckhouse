...
* [`sdn`](/modules/sdn/) — предоставляет функции программно-определяемых сетей (SDN) в кластере: настройку сетевых интерфейсов на узлах, дополнительные сети для подов и ВМ, underlay-сети и системные сети. Архитектура модуля описана на [соответствующей странице](sdn.html) данного подраздела;
* [`istio`](/modules/istio/) — реализует Service Mesh на основе Istio для централизованного управления сетевым трафиком в кластере. Архитектура модуля описана [на соответствующей странице](istio.html) данного подраздела.
