...
description: Архитектура модуля cni-cilium в Deckhouse Platform.
---
...
Архитектура модуля [`cni-cilium`](/modules/cni-cilium/) на уровне 2 модели C4 и его взаимодействия с другими компонентами Deckhouse Platform (DP) изображена на следующей диаграмме:
...
1. **Agent** (DaemonSet) — это агент, работающий на каждом узле Kubernetes-кластера, основной компонент управления в архитектуре Cilium. Он отвечает за перевод высокоуровневых конфигураций из Kubernetes API в низкоуровневые настройки для сетевых интерфейсов и eBPF-программ, которые реально обрабатывают трафик. Agent выполняет следующие функции:
   ...
   * **Управление конечными точками (Endpoints)** — отслеживает состояние всех подов (endpoints) на своём узле. Каждому поду присваивается уникальный IP-адрес и security identity (числовой идентификатор), который формируется на основе лейблов Kubernetes;
   ...
   * **Обработка событий кластера**. Агент работает по событийной модели: он постоянно наблюдает за изменениями в Kubernetes API (создание подов, обновление сервисов, изменение Endpoints, обновление кастомных ресурсов) и на лету обновляет конфигурацию eBPF-программ;

   * **Обеспечение наблюдаемости (Observability)**. Агент запускает локальный сервер Hubble (встроен в процесс агента), который собирает метрики и данные о сетевых потоках. Это позволяет мониторить состояние сети, выявлять проблемы и анализировать трафик между подами. Сервер Hubble запускается при включении модуля [`cilium-hubble`](/modules/cilium-hubble/). Компонент hubble-relay модуля [`cilium-hubble`](/modules/cilium-hubble/) подключается к агентам Cilium для приема событий через gRPC-поток.
   ...
   * **handle-vxlan-offload** — init-контейнер, исправляющий настройки UDP-сегментации (если [режим работы туннеля](/modules/cni-cilium/configuration.html#parameters-tunnelmode) настроен на `VXLAN`);
...
1. **Safe-agent-updater** (DaemonSet) — специальное приложение, предназначенное для исключения сбоев, которые могут возникнуть при обновлении версии агента Cilium и привести к проблемам с сетевой доступностью компонентов DP.
   ...
   * Набор init-контейнеров для предварительного скачивания новых версий образов соответствующих контейнеров компонента agent:
     ...
     * **prepull-image-vxlan-offloading-fixer** (только в режиме VXLAN);
   ...
   * **pause-cilium** — единственный рабочий контейнер пода. Под не может состоять из одних init-контейнеров, поэтому здесь запускается `pause`, который ничего не делает. Заодно контейнер удерживает образ агента на узле: образ считается используемым, пока на него ссылается любой контейнер.
...
1. **Kube-apiserver**:
   ...
   * Следит за изменениями стандартных ресурсов Kubernetes API и кастомных ресурсов Cilium;
   * Отслеживает состояние подов на узле, читает сетевые политики;
