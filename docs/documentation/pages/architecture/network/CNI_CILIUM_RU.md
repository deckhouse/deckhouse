...
1. **cilium-cni** — исполняемый файл, запускаемый компонентом containerd, который передает в качестве аргумента определенные команды в соответствии со спецификацией CNI, например ADD при запуске контейнера и DEL при его удалении. Cilium-cni взаимодействует с агентом Сilium через Unix-сокет.
