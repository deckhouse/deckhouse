...
На странице приведен список алертов мониторинга в Deckhouse Platform.

{% alert %}
Список не содержит алертов подключаемых модулей.
{% endalert %}

Алерты сгруппированы по модулям. Справа от названия алерта указаны иконки минимальной редакции DP в которой доступен алерт и уровня критичности алерта (severity).
