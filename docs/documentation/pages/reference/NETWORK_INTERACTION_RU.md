...
description: |
  Подробная информация о настройке сетевых политик для Deckhouse Kubernetes Platform, в частно в средах с ограничениями на сетевое взаимодействие между узлами. Описывает необходимые условия для включения режимов туннелирования для трафика подов с использованием CNI Cilium и Flannel, а также требования к сетевым задержкам между master-узлами для работы etcd.
lang: ru
...
При планировании кластера рекомендуется добиваться задержки не более 50 мс между будущими master-узлами, то есть двукратного запаса к требованию. Запас необходим потому, что измерения до установки выполняются на незагруженной сети и могут занизить реальные значения, а также потому, что нужно учитывать всплески и разброс задержки, а также возможные операции с инфраструктурой — например, миграцию виртуальной машины в зону с более высокой задержкой.
