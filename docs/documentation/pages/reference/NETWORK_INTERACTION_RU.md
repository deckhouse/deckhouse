...
description: |
  Подробная информация о настройке сетевых политик для Deckhouse Platform в средах с ограничениями на сетевое взаимодействие между узлами. Описывает необходимые условия для включения режимов туннелирования для трафика подов с использованием CNI Cilium и Flannel, а также требования к сетевым задержкам между master-узлами для работы etcd.
lang: ru
...
Если на площадке, где работает Deckhouse Platform (DP), есть требования для ограничения сетевого взаимодействия между серверами на уровне инфраструктуры, то необходимо соблюсти следующие условия:
...
перечислены в подразделе «Сеть» соответствующей версии DP [на странице «История изменений»](../release-notes.html).
{% endalert %}

## Требования к сетевым задержкам между master-узлами

{% alert level="info" %}
В DP поддерживается режим HA с использованием [arbiter-узлов](../../admin/configuration/high-reliability-and-availability/enable.html#настройка-режима-ha-c-двумя-master-узлами-и-arbiter-узлом). Приведенные ниже требования к сетевым задержкам также применимы и к arbiter-узлам.
{% endalert %}
...
DP использует стандартные значения etcd:
...
При этом задержки влияют на скорость работы всего, что запущено на узлах, — в том числе компонентов самой платформы. Для компонентов, обращающихся к Kubernetes API (операторы DP, Prometheus, Ingress-контроллеры), задержка до master-узлов добавляется к каждому запросу. Задержка между узлами добавляется к каждому сетевому запросу между подами и к разрешению DNS-имён, а при использовании реплицируемого хранилища — ко времени записи на диск.
