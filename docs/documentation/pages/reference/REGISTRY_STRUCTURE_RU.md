---
title: Структура хранилища образов контейнеров
permalink: ru/reference/container-registry-structure.html
toc: false
lang: ru
search: хранилище контейнеров, container registry
---

На странице представлено общее описание хранилищ образов контейнеров, из которых производится установка Deckhouse Kubernetes Platform.

## Хранилище образов сертифицированной версии

Хранилище образов сертифицированной ФСТЭК России версии расположено по адресу `registry-cse.deckhouse.ru/deckhouse/cse`. Оно одержит все [модули и компоненты](https://deckhouse.ru/products/kubernetes-platform/features/), включенные в сертифицированный комплект поставки.

Ключевые особенности:

* проходит сертификационные испытания;
* входит в комплект поставки DKP CSE;
* реализует все заявленные функциональные возможности и функции безопасности.

К образам предоставляется [техническая поддержка](https://deckhouse.ru/tech-support/), включающая консультации по вопросам эксплуатации.

Особенности сборки и обновления образов:

* сборка производится из исходных кодов открытого ПО, [входящего в состав DKP](./oss_info.html):
  * из upstream («как есть»);
  * с доработками к применению в DKP CSE;
  * с глубокой модернизацией (fork).
* в состав входят собственные разработки компании;
* устранение уязвимостей, разработка компенсирующих мер и обновление пакетов осуществляется в сроки, установленные в Формуляре на DKP CSE;
* предоставляется доступ к обновлениям:
  * c новой функциональностью;
  * с исправлениями ошибок и уязвимостей;
  *	с решением инцидентов безопасности.

## Хранилище образов коммерческих версий

Хранилище коммерческих редакций DKP доустпно по адресам `registry.deckhouse.ru/deckhouse/ee`, `registry.deckhouse.ru/deckhouse/se` и `registry.deckhouse.ru/deckhouse/ce`. Оно содержит [модули и компоненты](https://deckhouse.ru/products/kubernetes-platform/features/), доступные в редакциях ЕЕ, SE, CE.

Ключевые особенности:

* реализует функциональные возможности согласно редакции;
*	для аналогичных модулей из CSE редакции проводится тестирование безопасности в рамках цикла безопасной разработки ПО;
*	поставляется в электронном виде.

К образам предоставляется [техническая поддержка](https://deckhouse.ru/tech-support/), включающая консультации по вопросам эксплуатации.

Особенности сборки и обновления образов:

* сборка производится из исходных кодов открытого ПО, [входящего в состав DKP](./oss_info.html):
  * из upstream («как есть»);
  * с доработками к применению в DKP CSE;
  * с глубокой модернизацией (fork).
* в состав входят собственные разработки компании;
*	устранение уязвимостей и обновление пакетов выполняется только по мере обновления исходных пакетов открытого ПО;
* предоставляется доступ к обновлениям:
  * c новой функциональностью;
  * с исправлениями ошибок и уязвимостей;
  *	с решением инцидентов безопасности.
