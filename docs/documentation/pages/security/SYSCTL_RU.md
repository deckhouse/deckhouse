...
В ходе установки Deckhouse автоматически настраивает ряд параметров работы ядра сервера, используя утилиту `sysctl`.
...
{% alert level="warning" %}
Чтобы избежать неполадок в работе кластера Deckhouse, не меняйте перечисленные параметры вручную.
{% endalert %}
...
| `net.netfilter.nf_conntrack_max` | `$NF_CONNTRACK_MAX` | Максимальное количество отслеживаемых соединений в таблице conntrack. |
| `net.nf_conntrack_max` | `$NF_CONNTRACK_MAX` | Максимальное количество отслеживаемых соединений в таблице conntrack для старых версий ядра. |
