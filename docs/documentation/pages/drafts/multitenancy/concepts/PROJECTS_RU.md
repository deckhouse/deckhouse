...
Проекты, в отличие от пространств имён, дают возможность безопасно разделить кластер Deckhouse Platform на логические части.
...
В Deckhouse Platform доступно для использования три встроенных шаблона проектов, покрывающих типичные сценарии использования. Эти шаблоны недоступны для изменения, но могут быть использованы для создания новых шаблонов проектов под конкретные задачи и требования.
...
Администратор платформы может управлять тем, какие возможности Deckhouse Platform доступны в проекте.

Возможности проекта (Project Features) определяют, какие типы нагрузок и API доступны пользователям проекта. На текущий момент предусмотрены две возможности:
- Kubernetes — запуск контейнерных приложений и связанные с ними ресурсы;
- Virtualization — запуск виртуальных машин и связанные с ними ресурсы.

Возможности проектных пространств имён не могут превышать возможности проекта. Дополнительные возможности будут появляться в новых версиях платформы.

### Полная обратная совместимость

Deckhouse Platform использует [пространства имён](glossary.html#пространства-имен) при работе с проектами.
...
Имя проекта в Deckhouse Platform имеет следующие ограничения:
...
- Нельзя создавать проект с именем, начинающимся на имя существующего проекта и символ `-` (например, если есть проект `foo`, нельзя создать `foo-bar`).
