...
Глоссарий содержит список терминов, используемых в Deckhouse Platform (DP).

Глоссарий предназначен для того, чтобы помочь дать нужный контекст, и преимущественно раскрывает назначение терминов и функций DP. Технические детали и детали реализации раскрываются в разделе [архитектура](../architecture/).

## API

_API Deckhouse Platform_ — это интерфейс взаимодействия пользователей и компонентов DP с кластером. Все операции в DP выполняются через API, что обеспечивает единообразие и стандартизацию взаимодействия. API Deckhouse Platform полностью совместим с API Kubernetes.

## Кластер

_Кластер (cluster)_ — группа взаимосвязанных узлов, управляемая Deckhouse Platform и обеспечивающая единую среду для развёртывания, масштабирования и обслуживания [полезной нагрузки](#полезная-нагрузка).
...
_Подсистема (subsystem)_ — логическая часть системы, объединяющая набор взаимосвязанных [модулей](#модуль) и их [системных компонентов](#системный-компонент). Подсистемы используются для группировки компонентов системы и разграничения доступа к ним.

### Модуль

_Модуль (module)_ — набор ресурсов и приложений, добавляющий новые возможности в Deckhouse Platform. Модули выполняют взаимосвязанные функции (например, сбор метрик, балансировка трафика, работа с секретами).
...
Пространства имён в Kubernetes часто применяются для изоляции, хотя, без дополнительной настройки, они этого не обеспечивают, в отличие от [проектов](#проект) в Deckhouse Platform.
...
Системные пространства имён, используемые в Deckhouse Platform, имеют следующие префиксы:
- `d8-`
- `kube-`
...
В Deckhouse Platform реализовано две ролевые модели: [системная ролевая модель](#системное-пространство-имён) и [проектная ролевая модель](#проектная-ролевая-модель).
...
_Возможности проекта (project features)_ — набор функциональности платформы, доступный в рамках проекта. Возможности определяют, какие API и типы ресурсов могут использоваться в проекте, и не являются частью RBAC. Примеры возможностей: Kubernetes, Virtualization.

## Возможности проектного пространства имён

_Возможности проектного пространства имён (project namespace features)_ — ограничения возможностей на уровне отдельного пользовательского пространства имён внутри проекта. Набор возможностей пространства имён не может превышать набор возможностей проекта.
