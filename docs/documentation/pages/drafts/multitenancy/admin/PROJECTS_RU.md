...
В данном разделе описаны процессы работы **администратора [системы](../concepts/glossary.html#система)** с [проектами](../concepts/glossary.html#проект) в Deckhouse Platform (DP). Процессы работы **администратора [проекта](glossary.html#проект)** описаны в разделе [Использование](../user/projects.html).

> РЕМАРКА: администратора **системы**. Да?  (админ проекта читает раздел использование)

## Шаблоны проектов

Шаблоны проектов в DP описывают конфигурацию проекта, которая автоматически применяется ко всем создаваемым в рамках проекта объектам в кластере — [пространствам имён](../concepts/glossary.html#пространство-имён), квотам, ролям, политикам и др. DP включает несколько [встроенных шаблонов проектов](#встроенные-шаблоны-проектов), которые охватывают типовые сценарии их применения. Также на основе встроенных шаблонов можно [создавать собственные](#создание-собственного-шаблона-проекта).
...
В DP доступны следующие шаблоны проектов:
...
DP позволяет создавать собственные [шаблоны проектов](#шаблоны-проектов) на основе [встроенных](#встроенные-шаблоны-проектов).
...
1. Создаётся неймспейс с именем, которое совпадает c именем [Project](#TODO cr.html#project);
...
{% alert level="info" %}
Указание `ProjectTemplate` не является обязательным. Если шаблон не задан, создаётся только основное неймспейс проекта без применения шаблонизации.
{% endalert %}
...
1. В появившемся окне заполните следующие поля:
   ...
   - Шаблон проекта;
   - Настройки для неймспейса (ProjectTemplate);
...
1. На вкладке «Конфигурация» внесите изменения в необходимые поля:
   ...
   - Настройки для неймспейса (ProjectTemplate);
   - Запросы и лимиты (ЦП, Память, Хранилище).
1. При необходимости откройте вкладку Meta для добавления лейблов и аннотаций или вкладку YAML для ручного редактирования всего манифеста.
...
DP поддерживает совместимость с Kubernetes, расширяя механизм неймспейса при использовании [проектов](concepts.html#проект), и позволяет автоматически мигрировать существующие неймспейсы в проекты.
...
DP может автоматически создавать новые [проекты](../concepts/glossary.html#проект) из существующих [неймспейсов](../concepts/glossary.html#пространство-имён). Для этого пометьте необходимые неймспейсы аннотацией `projects.deckhouse.io/adopt`. Например:
...
При `allowNamespacesWithoutProjects: true` автоматически созданные проекты помечаются лейблом `multitenancy.deckhouse.io/project-managed-by-namespace`. Для таких проектов:
- разрешено менять лейблы/аннотации namespace с зеркалированием в `Project.spec.parameters.namespace`;
- разрешено удалять namespace с автоматическим удалением проекта;
- запрещено менять связку namespace ↔ project до снятия аннотации `projects.deckhouse.io/adopt`.
