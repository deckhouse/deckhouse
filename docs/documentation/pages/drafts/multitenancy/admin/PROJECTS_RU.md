...
{% alert level="info" %}
Указание `ProjectTemplate` не является обязательным. Если шаблон не задан, создаётся только основное пространство имён проекта без применения шаблонизации.
{% endalert %}
