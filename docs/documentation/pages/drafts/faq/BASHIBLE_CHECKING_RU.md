...
Если на узле кластера не обновляются компоненты Kubernetes, не применяется конфигурация NodeGroup, не все узлы NodeGroup синхронизированы (имеют статус `UPTODATE`), выполните следующие шаги:
...
1. Проверьте состояние синхронизации узлов кластера с помощью команды:
   ...
   Количество узлов в статусе `UPTODATE` должно совпадать с общим количеством узлов в каждой группе. Пример ответа:
