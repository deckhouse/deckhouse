...
* Модули *monitoring* (`operator-prometheus`, `prometheus` и `vertical-pod-autoscaler`):
  ...
  * Порядок поиска узлов (для определения [longtermNodeSelector](modules/300-prometheus/configuration.html#parameters-longtermretentiondays)):
    * Наличие узла с лейблом `node-role.deckhouse.io/monitoring-longterm`.
