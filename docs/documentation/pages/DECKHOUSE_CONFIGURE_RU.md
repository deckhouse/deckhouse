...
1. При отсутствии выделенных узлов и включенном автоматическом выборе `nodeSelector`, в ресурсах модуля не будет указан `nodeSelector`. Поведение модуля в таком случае будет использование любого узла с не конфликтующими `taints`.   
