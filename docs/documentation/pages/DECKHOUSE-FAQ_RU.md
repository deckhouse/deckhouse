...
### Как сгенерировать корректный self-signed сертификат?
...
2. Корректно заполнять поля `keyUsage`, `basicConstraints`, `extendedKeyUsage`, а именно:
   - `basicConstraints = CA:FALSE`  
     ...
     Данное поле определяет, является ли сертификат сертификатом конечного пользователя (`End-Entity Certificate`) или сертификатом центра сертификации (`CA Certificate`).  CA-сертификат не может использоваться в качестве сертификата сервиса.

   - `keyUsage = digitalSignature, keyEncipherment`  

     Поле `keyUsage` ограничивает допустимые использования данного ключа.  
     `digitalSignature` - позволяет использовать ключ для подписи цифровых сообщений, обеспечения целостности соединения.  
...
3. Не указывать домены в поле `CN` (`Common Name`)  
   ...
   Исторически поле `CN` использовалось для указания основного доменного имени, для которого выдается сертификат. Однако современные стандарты, такие как `RFC 2818`, рекомендуют использовать поле `Subject Alternative Name (SAN)` для этой цели.  
   Если сертификат предполагается распространяется на несколько доменных имен, указанных в поле `SAN`, то при дополнительном указании одно из доменов в `CN` в некоторых сервисах может возникнуть ошибка валидации при обращении к домену, не указанному в `CN`.  
...
4. Сгенерируйте self-signed сертификат
