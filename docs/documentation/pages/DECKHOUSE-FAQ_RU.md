...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
   ...
   По умолчанию `dhctl mirror` скачивает только последние доступные патч-версии актуальных релизов Deckhouse и актуальный набор официально поставляемых модулей.
   ...
   Начиная с версии 1.59.0, `dhctl mirror` поддерживает разделение пакета образов на фрагменты произвольного размера вместо упаковки всех образов в один tar-файл.
   Чтобы использовать эту функцию, передайте желаемый размер фрагмента в гигабайтах флагом `--images-bundle-chunk-size=N` или установите переменную `$DHCTL_CLI_MIRROR_IMAGES_BUNDLE_CHUNK_SIZE` при загрузке образов.
   В результате будет создан набор из последовательных фрагментов `.chunk` вместо одного пакета tar.
   Чтобы загрузить такой фрагментированный пакет в свой частный registry, используйте `dhctl mirror` как указано далее, передав флаг `--images-bundle-path`, как если бы вы загружали пакет в виде одного tar-файла.
   Укажите файл `.tar`, как если бы он был в каталоге пакета, а не какой-либо из файлов фрагментов. `dhctl mirror` автоматически распознает фрагментированный пакет.
...
1. Скачайте образы модулей из их источника, описанного в виде ресурса `ModuleSource` в выделенную директорию, используя команду `dhctl mirror-modules`.
   ...
   `dhctl mirror-modules` скачивает только версии модулей, доступные в каналах обновлений модуля на момент копирования если не передан параметр `--modules-filter`.
