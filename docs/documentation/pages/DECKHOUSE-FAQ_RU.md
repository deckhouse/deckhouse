...
Конфигурация Deckhouse, все ключевые настройки, включая параметры модулей, хранятся в ConfigMap `deckhouse` namespace `d8-system`. 
...
> Документация в кластере доступна при включенном модуле [deckhouse-web](modules/810-deckhouse-web/) (включен по умолчанию, кроме [bundle](modules/020-deckhouse/configuration.html#parameters-bundle) `Minimal`). 
...
При появлении нового релиза на канале обновлений, Deckhouse скачивает его в кластер и создает CustomResource `DeckhouseRelease`. 

После появления ресурса `DeckhouseRelease` в кластере, Deckhouse выполняет обновление на соответствующую версию, согласно установленным [параметрам обновления](modules/020-deckhouse/configuration.html#parameters-update) (по-умолчанию — автоматически, в 
любое время).
...
* При смене канала обновлений на **более стабильный** (например с `Alpha` на `EarlyAccess`), Deckhouse скачивает данные о релизе (в примере — из канала `EarlyAccess`) и сравнивает их с данными из существующих в кластере CR `DeckhouseRelease`:
...
* При смене канала обновлений на **менее стабильный** (например с `EarlyAcess` на `Alpha`):
  - Deckhouse скачивает данные о релизе (в примере — из канала `Alpha`) и сравнивает их с данными из существующих в кластере CR `DeckhouseRelease`;
  - Deckhouse выполняет обновление согласно установленным [параметрам обновления](modules/020-deckhouse/configuration.html#parameters-update).
