...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
  ...
  > Если вы хотите начать загрузку сначала, используйте флаг `--no-pull-resume`.
   ...
   `dhctl mirror` поддерживает рассчет контрольных сумм итогового набора образов Deckhouse в формате ГОСТ Р 34.11-2012 (Стрибог), используйте параметр `--gost-digest` для ее рассчета.
