...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `d8 mirror pull`.
   ...
   где:
   - `<EDITION>` — код редакции Deckhouse Platform Certified Security Edition (например, `ee`, `se`, `cse`);
   - `<LICENSE_KEY>` — лицензионный ключ Deckhouse Platform Certified Security Edition.
