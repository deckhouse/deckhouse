...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
   ...
   Чтобы скачать все версии Deckhouse начиная с конкретной версии, укажите ее в параметре `--min-version` в формате `X.Y` или `X.Y.Z`.

   Пример загрузки всех версий Deckhouse, начиная с версии 1.45:

   ```shell
   dhctl mirror --license="<DECKHOUSE_LICENSE_KEY>" --images /tmp/d8-images --min-version=1.45
   ```

1. Загрузите директорию с загруженными образами Deckhouse на хост с доступом к изолированному registry.

1. **(FIXME Это надо переписать когда поймем как доставлять dhctl mirror без контейнера)**
   Запустите установщик Deckhouse аналогично пунктам 1 и 2 на хосте с доступом к изолированному registry. Не забудьте смонтировать директорию с загруженными образами Deckhouse в контейнер установщика.

1. Загрузите образы Deckhouse с помощью команды `dhctl mirror` в изолированный registry.

   Пример команды для загрузки образов из директории `/tmp/d8-images`:
   ...
   > Обратите внимание, что образы будут выгружены в registry по пути `/deckhouse/ee`. Перед запуском команды загрузки убедитесь, что путь `/deckhouse/ee` существует и у используемой учетной записи есть права на запись.
