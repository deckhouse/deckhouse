...
1. Загрузите образы с помощью скрипта `d8-push.sh` в изолированный приватный registry.
   ...
   > Внимание! Обратитесь к документации вашего registry, чтобы правильно указать значение флага `--path`. Например, для `Harbor` значение может быть `harbor.registry.com/deckhouse/deckhouse`, но не `harbor.registry.com/deckhouse`.
...
* Если вы хотите установить Deckhouse с отключенным автоматическим обновлением:
  * Используйте тэг образа установщика соответствующей версии. Например, если вы хотите установить релиз `v1.44.3`, используйте образ `your.private.registry.com/deckhouse/install:v1.44.3`.
