...
1. Загрузите и запустите образ контейнера установщика Deckhouse. Установщик должен иметь версию не ниже 1.54:

   ```shell
   docker run --pull=always -v $(pwd)/d8-images:/tmp/d8-images registry.deckhouse.io/deckhouse/ce/install:v1.54.0 bash
   ```

   > Обратите внимание: мы монтируем в контейнер папку с файловой системы хоста, в нее будут загружены образы Deckhouse.

1. Скачайте образы Deckhouse используя команду `dhctl mirror`.

   Эта команда скачает образы Deckhouse, необходимые для установки и запуска любого из текущих каналов обновлений Deckhouse и переключения между ними.

   ```shell
   dhctl mirror --license="<DECKHOUSE_LICENSE_KEY>" --images /tmp/d8-images
   ```

   Если у вас уже есть работающие кластеры Deckhouse и вы хотите их обновить,
   вы можете добавить флаг `--min-version=""`, указав в нем вашу текущую версию Deckhouse, к приведенной выше команде
   чтобы загрузить все выпуски Deckhouse, вышедшие с момента вашего последнего обновления.

   Например, эта команда загрузит все выпуски Deckhouse, начиная с версии 1.45:

   ```shell
   dhctl mirror --license="<DECKHOUSE_LICENSE_KEY>" --images /tmp/d8-images --min-version="1.45"
   ```

1. Передайте каталог с загруженными образами Deckhouse из предыдущего шага на хост с доступом к изолированному registry..

1. (FIXME Это надо переписать когда поймем как доставлять dhctl mirror без контейнера)
   Загрузите и запустите образ контейнера установщика Deckhouse, как вы это делали в пункте 1 на хосте из прошлого пункта.

   Не забудьте смонтировать каталог с загруженными образами Deckhouse в контейнер установщика.

1. Выгрузите образы с помощью команды `dhctl mirror` в изолированный registry.

   Пример команды для выгрузки образов из каталога `/tmp/d8-images`:
   ...
   > Внимание! Образы будут выгружены в репозиторий по пути `/deckhouse/ee` в вашем registry.
   > Перед выгрузкой убедитесь что он создан и у учетной записи которую вы используете есть права на запись.

1. После выгрузки образов в изолированный registry можно переходить к установке Deckhouse из него. Следуйте руководству по быстрому старту для Enterprise Edition.

   Убедитесь, что вы указали адрес вашего registry и его строку с ключом для доступа в ресурсе `InitConfiguration` во время настройки установщика на шаге №4.
   При запуске Docker-контейнера установщика скачайте его образ из вашего registry, заменив
   `registry.deckhouse.io/deckhouse/ee/install:stable` на `your.private.registry.com:5000/deckhouse/ee/install:stable`.
