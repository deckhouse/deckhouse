...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
  ...
  > Если загрузка будет прерервана до ее окончания, повторный вызов той же команды проверит, какие образы уже были загружены, и загрузка продолжится с момента ее остановки. Это произойдет только если с момента остановки прошло не более суток.
  > Если вы хотите начать загрузку сначала, используйте параметр `--no-pull-resume`.
  >
  > Если вы хотите загружать образы Deckhouse только с зеркал registry, находящихся на территории РФ, добавьте параметр `--source="registry.deckhouse.ru/deckhouse/ee"`.
   ...
   > Параметр `--license` действует как шорткат для параметров `--source-login` и `--source-password` и предназначен для использования с официальным registry Deckhouse.
   > Если вы укажете и параметр `--license`, и пару логин + пароль одновременно, будет использована последняя.

   `dhctl mirror` поддерживает рассчет контрольных сумм итогового набора образов Deckhouse в формате ГОСТ Р 34.11-2012 (Стрибог), добавьте параметр `--gost-digest` к команде для её рассчета после загрузки образов.
