...
- под `deckhouse` не в статусе `Ready`. Если под долго не переходит в статус `Ready`, это может говорить о проблеме в работе Deckhouse. Необходима диагностика.

### Как понять, что обновление прошло успешно?

Если алерт `DeckhouseUpdating` погас, значит, обновление завершено.

Также можете проверить состояние [релизов](modules/002-deckhouse/cr.html#deckhouserelease) Deckhouse.
...
Статус `Deployed` у соответствующей версии говорит о том, что переключение на соответствующую версию было выполнено (но это не равнозначно успешному завершению).
...
* Отображается алерт `DeckhouseReleaseIsWaitingManualApproval`, если кластер использует ручной режим обновлений (параметр [update.mode](modules/002-deckhouse/configuration.html#parameters-update-mode) установлен в `Manual`).
...
Информацию о версиях Deckhouse и их связях с каналами обновлений можно найти на <https://flow.deckhouse.io>.
...
При появлении нового релиза Deckhouse скачает его в кластер и создаст custom resource [DeckhouseRelease](modules/002-deckhouse/cr.html#deckhouserelease).

После появления custom resource'а `DeckhouseRelease` в кластере Deckhouse выполнит обновление на соответствующую версию согласно установленным [параметрам обновления](modules/002-deckhouse/configuration.html#parameters-update) (по умолчанию — автоматически, в любое время).
...
* При смене канала обновлений на **более стабильный** (например, с `Alpha` на `EarlyAccess`) Deckhouse скачивает данные о релизе (в примере — из канала `EarlyAccess`) и сравнивает их с данными из существующих в кластере custom resouce'ов `DeckhouseRelease`:
  ...
  * Если более *поздние* релизы применены (в статусе `Deployed`), смены релиза не происходит. В этом случае Deckhouse останется в релизе до тех пор, пока на канале обновлений `EarlyAccess` не появится новый релиз.
...
С учетом параметра `search`, указанного в файле `/etc/resolv.conf`, при обращении на адрес `registry.deckhouse.ru` на узле, система попытается получить IP-адрес для имени `registry.deckhouse.ru` (так как считает его определенным, учитывая настройку по умолчанию параметра `options ndots:1`).

При обращении на адрес `registry.deckhouse.ru` **из пода** Kubernetes, учитывая параметры `options ndots:5` (используется в Kubernetes по умолчанию) и `search`, система первоначально попытается получить IP-адрес для имени `registry.deckhouse.ru.company.my`. Имя `registry.deckhouse.ru.company.my` будет разрешено для IP-адреса `10.0.0.100`, так как в DNS-зоне `company.my` настроено разрешение wildcard-записей `*.company.my` для адреса `10.0.0.100`. В результате к хосту `registry.deckhouse.ru` будет невозможно подключиться и скачать информацию о доступных обновлениях Deckhouse.  
{% endofftopic %}
...
* Включен параметр `Allow anonymous docker pull`.
...
* Параметр `Maximum metadata age` созданного репозитория установлен в 0.

Настройка:

1. Включите `Docker Bearer Token Realm`:
  ![Включение `Docker Bearer Token Realm`](images/registry/nexus/nexus-realm.png)

2. Создайте проксирующий репозиторий Docker, указывающий на [Deckhouse registry](https://registry.deckhouse.ru/):
...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
   ...
   Чтобы загрузить образы Deckhouse из определенного репозитория registry, можно указать репозиторий с помощью флага `--source`.
   ...
   Например, образы из стороннего registry можно загрузить следующей командой:
...
1. Передайте директорию с загруженными образами Deckhouse на хост с доступом к изолированному registry.
   Для продолжения установки используйте утилиту `dhctl` (скопированную ранее) или запустите установщик Deckhouse, аналогично шагам 1 и 2 на хосте с доступом к изолированному registry.
   > Не забудьте смонтировать директорию с загруженными образами Deckhouse в контейнер установщика.
...
1. После загрузки образов в изолированный registry можно переходить к установке Deckhouse (доступно только в Enterprise Edition). Воспользуйтесь [руководством по быстрому старту](/gs/bm-private/step2.html).
   ...
   При использовании ресурса `InitConfiguration` необходимо также указать адрес вашего registry и авторизационные данные (параметры [imagesRepo](/documentation/v1/installing/configuration.html#initconfiguration-deckhouse-imagesrepo), [registryDockerCfg](/documentation/v1/installing/configuration.html#initconfiguration-deckhouse-registrydockercfg) или [шаг 3](/gs/bm-private/step3.html) руководства по быстрому старту).
...
1. Cкачайте образы модулей из источника, описанного в ресурсе `ModuleSource`, находящемся в файле `$HOME/module_source.yml`:
...
1. Обратитесь к документации разработчика модулей для получения дальнейших инструкций по их настройке и использованию.
...
1. Выполните команду `deckhouse-controller helper change-registry` из пода Deckhouse с параметрами нового registry. Пример запуска представлен ниже:
...
1. Если registry использует самоподписные сертификаты, положите корневой сертификат соответствующего сертификата registry в файл `/tmp/ca.crt` в поде Deckhouse и добавьте к вызову опцию `--ca-file /tmp/ca.crt`, или вставьте содержимое CA в переменную, как в примере ниже:
...
1. Дождитесь перехода пода Deckhouse в статус `Ready`. Если под будет находиться в статусе `ImagePullBackoff`, перезапустите его.
...
1. Если вы хотите отключить автоматические обновления у уже установленного Deckhouse (включая обновления patch-релизов), удалите параметр [releaseChannel](modules/002-deckhouse/configuration.html#parameters-releasechannel) из конфигурации модуля `deckhouse`.
...
1. Если под находится в статусе `ImagePullBackoff`, перезапустите его:
...
1. Дождитесь перезапуска Deckhouse и выполнения всех задач в очереди:
   ...
   Пример вывода, когда в очереди присутствуют задания (`length 38`):
...
1. Проверьте, не осталось ли в кластере подов с адресом registry для Deckhouse CE:
   ...
   Иногда некоторые static поды (например, `kubernetes-api-proxy-*`) могут оставаться запущенными. Это связано с тем, что kubelet не перезапускает под несмотря на изменение соответствующего манифеста, так как используемый образ одинаков для Deckhouse CE и EE.

1. Выполните на любом master-узле следующую команду, чтобы убедиться, что соответствующие манифесты были изменены:
...
Чтобы запустить Deckhouse на определенном узле, установите параметр `nodeSelector` в модуле `deckhouse` равным нужному [параметру](modules/002-deckhouse/configuration.html) и не устанавливайте значения `tolerations`.
Необходимые значения `tolerations` в этом случае будут проставлены автоматически.
