...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
   ...
   Укажите файл `.tar`, как если бы он был в каталоге пакета, не какой-либо из файлов фрагментов. `dhctl mirror` автоматически распознает фрагментированный пакет.
