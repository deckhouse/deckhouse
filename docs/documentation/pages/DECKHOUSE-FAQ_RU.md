...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `d8 mirror pull`.
   ...
   Например, для Deckhouse 1.60 будет скачана только одна версия `1.60.4`, т. к. этого достаточно для обновления Deckhouse с версии `1.59`.
   ...
   > Не забудьте изменить путь на соответствующий вашей редакции платформы — замените последнюю часть ссылки на `se` или `cse`. Например, для редакции SE ссылка будет выглядеть так: `registry.deckhouse.ru/deckhouse/se`.
   >
   ...
   Также возможна загрузка только одного конкретного релиза Deckhouse посредством использования флага `--release=X.Y.Z`.
...
1. После загрузки образов в изолированный registry отредактируйте YAML-манифест `ModuleSource`:
   ...
   * Измените поле `.spec.registry.dockerCfg` на Base64-строку с данными для авторизации в вашем registry в формате `dockercfg`. Обратитесь к документации вашего registry для получения информации о том, как сгенерировать этот токен.
   ...
   > Не забудьте изменить путь на соответствующий вашей редакции платформы — замените в ссылке `ee` на `se` или `cse`. Например, для редакции SE ссылка будет выглядеть так: `registry.deckhouse.ru/deckhouse/se/modules`.
