...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
  ...
  > Если загрузка будет прерервана до ее окончания, повторный вызов команды проверит какие образы уже были загружены и загрузка продолжится с момента ее остановки. Это произойдет только если с момента остановки прошло не более суток.
  > Если вы хотите начать загрузку сначала, используйте флаг `--no-pull-resume`.
   ...
   `dhctl mirror` поддерживает рассчет контрольных сумм итогового набора образов Deckhouse в формате ГОСТ Р 34.11-2012 (Стрибог), используйте параметр `--gost-digest` для её рассчета.
