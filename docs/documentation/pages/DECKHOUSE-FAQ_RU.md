...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `dhctl mirror`.
   ...
   Чтобы использовать эту функцию, передайте желаемый размер фрагмента в гигабайтах флагом `--images-bundle-chunk-size=N` или установите переменную `$MIRROR_IMAGES_BUNDLE_CHUNK_SIZE` при загрузке образов.
