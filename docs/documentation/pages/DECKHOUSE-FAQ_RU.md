...
1. Скачайте образы Deckhouse в выделенную директорию, используя команду `d8 mirror pull`.
   ...
   - `SSL_CERT_DIR` — Список каталогов, разделенный двоеточиями; определяет, в каких каталогах искать файлы сертификатов SSL. Если переменная установлена, системные сертификаты не используются. См. <https://www.openssl.org/docs/man1.0.2/man1/c_rehash.html>;
