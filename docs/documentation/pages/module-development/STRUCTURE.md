...
## Key module control parameters

This section describes parameters that affect module behavior in the cluster:

- [managementState](#managementstate) — management mode;
- [disable](#disable) — protection from disabling;
- [phase](#phase) — current resource phase;
- [rollback](#rollback) — module rollback after deleting ModulePullOverride.

### managementState

The `managementState` field defines the management mode of the module and can be set in the ModuleConfig resource:

- `Unmanaged` — standalone mode for development or debugging.
  
  In this mode:
  - Changes in module configuration or hooks are not synchronized, so resources are not updated automatically.
  - Module resource monitoring is disabled, so deleted resources are not restored.
  - All module resources are labeled with `managementState: Unmanaged`.
  - The ModuleIsInUnmanagedMode alert is triggered.

  When Deckhouse restarts, the module applies its last known state once and then continues to operate in standalone mode.

  To return the module to managed mode, remove the `managementState` parameter from the ModuleConfig object using the following command:

  ```console
  kubectl patch moduleconfig <module-name> --type=json -p='[{"op": "remove", "path": "/spec/managementState"}]'
  ```

The `managementState` field can be added manually or through Deckhouse automation mechanisms. When set to `Unmanaged`, Deckhouse stops the reconciler behavior for the corresponding module, but continues to consider its presence during platform startup.

### disable

The `disable` parameter enables protection against accidental disabling of a module. It is specified in the `module.yaml` file and is an object with the following fields:

- `confirmation` — boolean. Requires explicit confirmation when attempting to disable the module. If set to `true`, the module can only be disabled if the corresponding ModuleConfig object has the label `modules.deckhouse.io/allow-disabling=true`.

- `message` — string. A message explaining the consequences of disabling the module. Displayed to the user upon attempt to disable. Can be used to warn about critical actions that will be performed during module deactivation.

Example usage in `module.yaml`:
...
If the module is protected from disabling but the label `modules.deckhouse.io/allow-disabling=true` is missing, Deckhouse will prevent disabling the module and display the message specified in `message`.

### phase

The `phase` field is used in the status of ModuleRelease and ModuleSource objects to reflect their current lifecycle phase. It is a string and can have one of the following values:

- `Pending` — the object is created but not yet deployed;
- `Deployed` — the module is successfully deployed and active;
- `Superseded` — the object was replaced by a newer release or source;
- `Suspended` — the release is suspended;
- `Skipped` — the release was skipped;
- `Terminating` — the object is being deleted.

The `phase` field is available in the resource status and can be displayed using `kubectl get` with jsonPath:
...
The value of `phase` is set automatically by the Deckhouse system and is used to monitor the state of the object. Manual modification is not supported.

### rollback

The `rollback` parameter is used in the ModulePullOverride object and determines whether to automatically restore the module from the current release when the ModulePullOverride object is deleted.

`rollback` — boolean. If set to `true`, then upon deletion of the ModulePullOverride object:

- the corresponding symlinks and downloaded module artifacts will be removed;
- Deckhouse will restart to roll back to the previous stable version of the module.

The field is displayed in the `ROLLBACK` column when listing the resource. Example:
...
This parameter has no effect if ModulePullOverride is deleted manually but rollback was not enabled. In this case, the module will not be restored automatically.
