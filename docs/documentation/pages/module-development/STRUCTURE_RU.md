...
- `namespace` — *Строка.* Пространство имен, где будут развернуты компоненты модуля.
...
- `disable` — *Объект.* Параметры, связанные с поведением при отключении модуля.
  ...
  Если для отключения модуля требуется подтверждение (параметр `confirmation` установлен `true`), то отключение модуля будет возможно только если на соответствующем объекте ModuleConfig будет установлен лейбл `modules.deckhouse.io/allow-disabling=true`. Если такого лейбла нет, то при попытке отключить модуль будет выведено предупреждение, с добавлением сообщения из параметра `message`.
- `name` — *Строка, обязательный параметр.* Имя модуля в Kebab Case. Например, `echo-server`.
- `exclusiveGroup` — *Строка.* Если несколько модулей принадлежат к одной и той же `exclusiveGroup`, то только один из них может быть активен в системе в любой момент времени. Это поведение предотвращает конфликты между модулями, выполняющими схожие или несовместимые задачи.
- `requirements` — *Объект.* [Зависимости](../dependencies/) модуля — набор условий, которые должны выполняться чтобы Deckhouse Kubernetes Platform (DKP) мог запустить модуль.
  - `bootstrapped` — *Булевый.* Зависимость модуля [от статуса установки кластера](../dependencies/#зависимость-от-статуса-установки-кластера) (только для встроенных модулей DKP).
...
## Ключевые параметры управления модулем

В этом разделе описаны параметры, влияющие на поведение модуля в кластере:

- [managementState](#managementstate) — режим управления;
- [disable](#disable) — защита от отключения;
- [phase](#phase) — текущая фаза ресурса;
- [rollback](#rollback) — откат модуля при удалении ModulePullOverride.

### managementState

Поле `managementState` задаёт режим управления работой модуля и может быть указано в ресурсе ModuleConfig:

- Unmanaged — автономный режим для разработки или отладки модуля.
  В этом режиме:
  ...
  - Ко всем ресурсам модуля применяется лейбл `managementState: Unmanaged`.
  - Срабатывает алерт ModuleIsInUnmanagedMode.
...
Чтобы вернуть модуль в управляемый режим, удалите параметр `managementState` у объекта ModuleConfig следующей командой:

```console
kubectl patch moduleconfig <имя-модуля> --type=json -p='[{"op": "remove", "path": "/spec/managementState"}]'
```

Поле `managementState` может быть добавлено вручную, либо через автоматические механизмы Deckhouse. При установке значения Unmanaged Deckhouse прекращает reconciler-поведение для соответствующего модуля, но продолжает учитывать его наличие при старте платформы.

### disable

Параметр `disable` позволяет задать защиту от случайного отключения модуля. Он указывается в файле `module.yaml` и представляет собой объект со следующими полями:

- `confirmation` — булево значение. Требует явного подтверждения при попытке отключить модуль. Если значение установлено в `true`, отключение модуля возможно только в том случае, если на соответствующем объекте ModuleConfig присутствует лейбл `modules.deckhouse.io/allow-disabling=true`.

- `message` — строка. Сообщение, объясняющее последствия отключения модуля. Показывается пользователю при попытке отключения. Может использоваться для предупреждения о критичных действиях, которые будут выполнены при деактивации модуля.

Пример использования в module.yaml:
...
Если модуль защищён от отключения, но лейбл `modules.deckhouse.io/allow-disabling=true` отсутствует, Deckhouse не позволит отключить модуль и выведет сообщение, указанное в `message`.

### phase

Поле `phase` используется в статусе объектов типа ModuleRelease и ModuleSource для отображения текущей фазы их жизненного цикла. Это поле является строкой и может принимать одно из следующих значений:

- Pending — объект создан, но ещё не задеплоен;
- Deployed — модуль успешно задеплоен и активен;
- Superseded — объект заменён новым релизом или источником;
- Suspended — релиз приостановлен;
- Skipped — релиз пропущен;
- Terminating — объект находится в процессе удаления.

Поле `phase` доступно в статусе ресурса и отображается, например, в `kubectl get` с помощью jsonPath:
...
Значение `phase` задаётся автоматически системой Deckhouse и служит для наблюдения за состоянием объекта. Ручное изменение не предусмотрено.

### rollback

Параметр `rollback` используется в объекте ModulePullOverride и определяет, нужно ли автоматически восстанавливать модуль из текущего релиза, если объект ModulePullOverride будет удалён.

`rollback` — булево значение. Если параметр установлен в `true`, то при удалении объекта ModulePullOverride:

- будут удалены соответствующие симлинки и загруженные артефакты модуля;
- произойдёт рестарт Deckhouse с целью отката к предыдущей стабильной версии модуля.

Поле отображается в колонке `ROLLBACK` при выводе ресурса. Пример:
...
Этот параметр не влияет на поведение, если ModulePullOverride удалён вручную, но `rollback` не был включён. В таком случае модуль не будет восстановлен автоматически.
