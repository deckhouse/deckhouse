...
- При запуске с аргументами `hook config`, должна выводиться свою конфигурация хуков в формате JSON.
- При запуске с аргументами `hook list`, должен выводиться список всех хуков с их порядковым номером.
- При запуске с аргументами `hook run 0`, хук под номером 0 должен выполнять свое основное действие.
