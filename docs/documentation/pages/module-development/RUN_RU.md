...
- **Релиз назначения**. Правило читается из релиза, на который нужно перейти — значение `to` должно совпадать с версией самого этого релиза (constrained release).
- **Порог `from`**. Текущая установленная версия модуля (`Deployed`) не ниже значения `from`. Если текущая версия ниже `from`, переход по правилу не выполняется — обновление идёт по порядку.
...
Нужна ли аннотация зависит от политики обновления модуля, в режиме `Manual` — нужна, в `Auto` — не нужна (обновится автоматически в разрешённое окно), в `Disabled — обновлений нет. Подробнее — [Политика обновления модуля](#политика-обновления-модуля).
