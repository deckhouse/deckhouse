...
DKP видит, что `0.3.33 ≥ 0.3`, и предлагает установить последнюю доступную версию в пределах `0.7 — p-o-test-v0.7.25` (в статусе `Pending`). После подтверждения `p-o-test-v0.7.25` промежуточные версии `0.4.1`, `0.5.27` и `0.6.11` помечаются как `Skipped`, а `p-o-test-v0.7.25` становится `Deployed`.
