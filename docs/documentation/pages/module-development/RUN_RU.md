...
{% alert level="warning" %}
Если модуль требует обязательной конфигурации, но включён без неё, произойдёт ошибка валидации конфигурации. В этом случае сработает alert D8DeckhouseModuleValidationError, а модуль не будет успешно активирован.

Для получения подробностей используйте следующую команду:
