...
Затем вам нужно пересоздать политику обновления с новой версии и без селектора, пересоздавать обязательно так как kubernetes хранит старые версии и селектор останется в кластере и алерт будет продолжать гореть.
