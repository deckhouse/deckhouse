...
Deckhouse Kubernetes Platform (DKP) может работать как со встроенными модулями (идут в составе DKP), так и с модулями, которые можно получить из [источника модулей](run/). В этом разделе подробно рассмотрим, что представляет собой модуль Deckhouse Kubernetes Platform и как он функционирует.

Жизненный цикл модуля можно разделить на четыре ключевых этапа:
...
* **Зависимости**: этап настройки зависимостей модуля, включая версии Deckhouse Kubernetes Platform, Kubernetes и другие критичные компоненты — описывается в разделе [**Зависимости модуля**](dependencies/).
