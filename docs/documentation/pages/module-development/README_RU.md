...
Deckhouse Kubernetes Platform (DKP) может работать как со встроенными модулями (идут в составе DKP), так и с модулями, которые можно получить из [источника модулей](../cr.html#modulesource). В этом разделе подробно рассмотрим, что представляет собой модуль Deckhouse Kubernetes Platform и как он функционирует.

Жизненный цикл модуля содержит четыре этапа:
