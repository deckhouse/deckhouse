...
{% alert level="info" %}
Промежуточные релизы можно пропустить — для этого в `module.yaml` задайте правила [`update.versions`](../../cr.html#modulerelease-v1alpha1-spec-update) (`from-to`). Если текущая установленная версия не ниже значения `from`, DKP обновляет модуль сразу до значения `to` — устанавливается последняя доступная версия в пределах значения `to`. Подробности и пример см. в разделе [Пропуск промежуточных релизов (from-to)](../run/#пропуск-промежуточных-релизов-from-to).
{% endalert %}
