...
-Текущая установленная версия модуля (`Deployed`) не ниже значения `from`. Если текущая версия ниже `from`, переход по правилу не выполняется — обновление идёт по порядку.
