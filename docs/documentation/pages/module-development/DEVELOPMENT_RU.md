...
Чтобы определить, в каких редакциях модуль должен быть доступен, а в каких бандлах — включён по умолчанию, используйте поле `accessibility` в файле `module.yaml`:
...
Эта конфигурация означает, что модуль доступен в DKP Enterprise Edition (может быть включён). Также, модуль будет включён по умолчанию в бандле `Default`.
...
{% alert level="warning" %}
Модуль по-прежнему можно отключить.
{% endalert %}

{% alert level="warning" %}
Модуль останется на последнем доступном релизе, если в следующем релизе он будет отключён (например, при установке `available: false` в соответствующей редакции).
{% endalert %}
