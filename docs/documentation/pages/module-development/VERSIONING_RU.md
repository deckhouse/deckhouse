...
**Важно**: Модули на стадии `Experimental` не устанавливаются по умолчанию. Для их установки необходимо установить флаг `allowExperimentalModules` в true. Этот флаг задаётся через переменную окружения `DECKHOUSE_ALLOW_EXPERIMENTAL_MODULES`.
