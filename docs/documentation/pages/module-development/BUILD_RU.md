...
- В склонированном репозитории модуля на странице _Settings -> Secrets and variables -> Actions_ создайте Secret с названием `TOKEN` содержащий PAL.
- Замените переменную `GITHUB_TOKEN` на `TOKEN`:
    ```shell
