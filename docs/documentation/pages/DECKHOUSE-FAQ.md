...
1. Pull Deckhouse images using the `d8 mirror pull` command.
   ...
   Additional configuration options for the `d8 mirror` family of commands are available as environment variables:
    - `HTTP_PROXY / HTTPS_PROXY` — Proxy URLs for HTTP(S) requests unless overridden by `$NO_PROXY`;
    ...
    - `SSL_CERT_DIR` — List of directories to search for SSL certificate files, separated by a colon. If set, system certificates are not used. See <https://www.openssl.org/docs/man1.0.2/man1/c_rehash.html>;
