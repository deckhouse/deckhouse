...
{% alert %}
- The instruction implies using the public address of the container registry: `registry-cse.deckhouse.ru`. If you use a different container registry address, modify the commands accordingly or follow the [instructions to switch Deckhouse to using a third-party registry](#how-do-I-switch-a-running-Deckhouse-cluster-to-use-a-third-party-registry).
- Deckhouse CSE does not support cloud clusters and certain modules.
- Currently, you can only migrate to the CSE edition from Deckhouse EE version `1.58`.
- The CSE edition supports only Kubernetes version 1.27.
- Cluster components may become temporarily unavailable during the switch to the CSE edition.
  {% endalert %}

Perform the following steps to switch a Deckhouse Enterprise Edition cluster to Community Edition (all commands should be executed on the cluster’s master node by a user with a configured `kubectl` context or by a superuser):

1. Configure the cluster to use Kubernetes version `1.27`. To do this:
   1. Run:
   ...
   1. Change the kubernetesVersion parameter to `"1.27"` (in quotes).
   1. Save the changes. The cluster nodes will begin updating sequentially.
   1. Wait for the update to finish. You can track its progress by running `kubectl get no`.
   The update is considered complete when the updated version appears in the `VERSION` column for each node in the cluster.

1. Prepare variables with your license token and create a NodeGroupConfiguration for transitional authorization in `registry-cse.deckhouse.io`:
     ...
     content: |
       ...
       [plugins]
         [plugins."io.containerd.grpc.v1.cri"]
           ...
           [plugins."io.containerd.grpc.v1.cri".registry.mirrors."registry-cse.deckhouse.io"]
             endpoint = ["https://registry-cse.deckhouse.io"]
           [plugins."io.containerd.grpc.v1.cri".registry.configs]
             [plugins."io.containerd.grpc.v1.cri".registry.configs."registry-cse.deckhouse.io".auth]
               auth = "$AUTH_STRING"
       EOF_TOML
   EOF
   ```

   Wait for the file to appear on the nodes and for bashible synchronization to complete:

   ```shell
   /etc/containerd/conf.d/cse-registry.toml
   ```

   You can monitor the synchronization status by the `UPTODATE` value (the displayed number of nodes in this status must match the total number of nodes (`NODES`) in the group):
   ...
   In the systemd service log for bashible, you should see the message `Configuration is in sync, nothing to do`.
...
1. Run the following commands to launch a temporary pod of the CSE edition to obtain current digests and a list of modules:

   ```console
   kubectl run cse-image --image=registry-cse.deckhouse.io/deckhouse/cse/install:v1.58.2 --command sleep -- infinity
   ```
...
1. Ensure that the modules used in the cluster are supported in Deckhouse CSE edition.  
   For example, at the moment the cert-manager module is not available in the CSE edition. Therefore, before disabling it, you need to switch the `https.mode` for related components (e.g., [user-authn](https://deckhouse.io/products/kubernetes-platform/documentation/v1.58/modules/150-user-authn/configuration.html#parameters-https-mode) or [prometheus](https://deckhouse.io/products/kubernetes-platform/documentation/v1.58/modules/300-prometheus/configuration.html#parameters-https-mode)) to alternative options, or change the [global parameter](deckhouse-configure-global.html#parameters-modules-https-mode) responsible for the HTTPS operating mode in the cluster

   To display the list of modules that are not supported in the CSE edition and will be disabled, use:

   ```shell
   echo $MODULES_WILL_DISABLE
   ```

   > Verify the list and make sure that the functionality of these modules is not being used in your cluster and that you are ready to disable them.

   Disable the modules not supported in the CSE edition:
   ...
   Currently, the 1.58 CSE edition does not support the earlyOOM component. Disable it with the [configuration](modules/node-manager/configuration.html#parameters-earlyoomenabled).
   ...
   Ensure the disabled modules have transitioned to `Disabled` status:
...
1. Create a `NodeGroupConfiguration` resource:
   ...
   spec:
     ...
     content: |
        ...
        [plugins]
          [plugins."io.containerd.grpc.v1.cri"]
            sandbox_image = "registry-cse.deckhouse.io/deckhouse/cse@$CSE_SANDBOX_IMAGE"
        EOF_TOML

        sed -i 's|image: .*|image: registry-cse.deckhouse.io/deckhouse/cse@$CSE_K8S_API_PROXY|' /var/lib/bashible/bundle_steps/051_pull_and_configure_kubernetes_api_proxy.sh
        sed -i 's|crictl pull .*|crictl pull registry-cse.deckhouse.io/deckhouse/cse@$CSE_K8S_API_PROXY|' /var/lib/bashible/bundle_steps/051_pull_and_configure_kubernetes_api_proxy.sh
   EOF
   ```

   Wait for bashible synchronization to finish on all nodes.

   You can track the synchronization status by the `UPTODATE` value (the displayed number of nodes in this status must match the total number of nodes (`NODES`) in the group):

   ```shell
   kubectl get ng -o custom-columns=NAME:.metadata.name,NODES:.status.nodes,READY:.status.ready,UPTODATE:.status.upToDate -w
   ```

   In the systemd service log for bashible on the nodes, you should see the message `Configuration is in sync, nothing to do`.
...
1. Update the Deckhouse registry access secret by running:

   ```console
   kubectl -n d8-system create secret generic deckhouse-registry \
     --from-literal=".dockerconfigjson"="{\"auths\": { \"registry-cse.deckhouse.io\": { \"username\": \"license-token\", \"password\": \"$LICENSE_TOKEN\", \"auth\": \"$AUTH_STRING\" }}}" \
     --from-literal="address"=registry-cse.deckhouse.io \
...
1. Apply the CSE image:

   ```console
   kubectl -n d8-system set image deployment/deckhouse deckhouse=registry-cse.deckhouse.io/deckhouse/cse:v1.58.2
   ```

1. Wait until the Deckhouse pod transitions to `Ready` and [all tasks in the queue are completed](#how-to-check-the-job-queue-in-deckhouse). If an `ImagePullBackOff` error occurs during the process, wait for the pod to automatically restart.
   ...
   Check the Deckhouse queue status:
...
1. Check if there are any pods with the Deckhouse EE registry address left in the cluster:
   ...
   If the output shows pods of the chrony module, re-enable that module (in the CSE edition, this module is disabled by default):
