...
Приветствуем вас на главной странице документации Deckhouse Platform Certified Security Edition — платформы для управления Kubernetes-кластерами. Если вы еще не использовали платформу, рекомендуем начать с раздела {% if site.mode == 'module' %}[Быстрый старт🔗]({{ site.urls[page.lang] }}/gs/){% else %}[Быстрый старт](/gs/){% endif %}, где вы найдете пошаговые инструкции по развертыванию платформы на любой инфраструктуре.

Как быстро найти то, что нужно:
- Документация по Deckhouse Platform Certified Security Edition разных версий может отличаться. Выберите нужную версию в выпадающем списке вверху страницы. В списке доступны актуальные версии документации по Deckhouse.
