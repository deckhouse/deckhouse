...
Помимо предустановленных в Deckhouse Kubernetes Platform (DKP) классов приоритета администратор может создать пользовательский PriorityClass. В примере ниже используется класс `my-app-critical` — порядок его создания описан [в разделе «Создание PriorityClass»](/products/kubernetes-platform/documentation/v1/admin/configuration/app-scaling/pod-eviction/priority-classes.html#создание-priorityclass).
...
{% alert level="warning" %}
Параметр [`priorityClassName`](/products/kubernetes-platform/documentation/v1/admin/configuration/app-scaling/pod-eviction/priority-classes.html#создание-priorityclass) нельзя изменить у работающего пода. Это поле является неизменяемым. Единственный способ изменить приоритет — удалить под и создать его заново с новым классом.
{% endalert %}
...
Подробнее о механизмах защиты от вытеснения смотрите [в разделе «Защита от вытеснения»](/products/kubernetes-platform/documentation/v1/admin/configuration/app-scaling/pod-eviction/priority-classes.html#защита-от-вытеснения). Ниже — практический сценарий, в котором эти механизмы работают вместе.
...
Описание типичных проблем и решений на уровне кластера смотрите [в разделе «Эксплуатация и диагностика»](/products/kubernetes-platform/documentation/v1/admin/configuration/app-scaling/pod-eviction/priority-classes.html#эксплуатация-и-диагностика).

[В «Пошаговой демонстрации вытеснения»](#пошаговая-демонстрация-вытеснения) под `high-priority-pod` переходит в `Running`. Ниже тот же под разбирается в двух случаях, когда он остаётся в `Pending`: не хватает CPU или памяти, и планировщик не находит поды для вытеснения.
...
Ниже — [после «Демонстрации защиты Stateful-приложения»](#демонстрация-защиты-stateful-приложения): на узле `mock-stateful` (`6000`) и `emergency-task` (`9000`), системные поды ещё выше, поэтому `mock-stateful-0` некого вытеснять.
