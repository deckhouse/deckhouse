...
description: "Настройка вертикального автомасштабирования (VPA) для пользовательской нагрузки в Deckhouse Platform. Управление ресурсами контейнеров, работа с лимитами, мониторинг и примеры конфигурации VerticalPodAutoscaler."
...
VPA управляет запросами ресурсов (requests) контейнера, но не управляет лимитами (limits), если это явно не указано в его политике.

VPA рассчитывает рекомендуемые значения на основе данных о потреблении ресурсов контейнером. Это поведение может повлиять на соотношение запросов ресурсов и лимитов:

- Если запросы ресурсов и лимиты равны, то VPA изменяет только запросы, оставляя лимиты неизменными.
- Если лимиты не указаны, VPA обновляет только запросы ресурсов.
- Если лимиты заданы, но не управляются VPA, соотношение запросов ресурсов и лимитов может измениться.
...
1. Пример 1. В кластере имеется:
   ...
   - Под с ресурсами:
     ...
     Если контейнер будет потреблять 1 CPU, VPA порекомендует 1,168 CPU. В данном случае, соотношение между запросами ресурсов и лимитами будет равно 100%. При пересоздании пода VPA изменит ресурсы на следующие:
...
1. Пример 2. В кластере имеется:
   ...
   - Под с ресурсами:
     ...
     В данном случае соотношение запросов ресурсов и лимитов будет 25%. Если VPA порекомендует 1,168 CPU, ресурсы контейнера будут изменены на:
