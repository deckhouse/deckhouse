...
VPA управляет запрашиваемыми ресурсами (requests) контейнера, но не управляет лимитами (limits), если явно не указано в его политике.

VPA рассчитывает рекомендуемые значения на основе данных о потреблении ресурсов контейнером. Это поведение может повлиять на соотношение запрашиваемых ресурсов и лимитов:

- Если запрашиваемые ресурсы и лимиты равны, то VPA изменяет только ресурсы, оставляя лимиты неизменными.
- Если лимиты не указаны, VPA обновляет только запрашиваемые ресурсы.
- Если лимиты заданы, но не управляются VPA, соотношение запрашиваемых ресурсов/лимитов может измениться.
...
1. Пример 2. В кластере имеется:
   ...
   - Под с ресурсами:
     ...
     В данном случае соотношение запрашиваемых ресурсов и лимитов будет 25%. Если VPA порекомендует 1,168 CPU, ресурсы контейнера будут изменены на:
