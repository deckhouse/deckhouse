---
title: "Веб-интерфейс Deckhouse Platform"
description: "Веб-интерфейс Deckhouse Platform. Мониторинг, настройка параметров, управление узлами и модулями, конфигурация безопасности и сети."
permalink: ru/user/web/ui.html
search: веб-интерфейс
lang: ru
---

Веб-интерфейс Deckhouse Platform (DP) предоставляет доступ к управлению кластером, включая мониторинг, настройку параметров, управление узлами и модулями, а также настройку безопасности и сети. Большинство операций, доступных в командной строке через утилиты `d8` ([Deckhouse CLI](../../cli/d8/)) или `kubectl`, также можно выполнять через веб-интерфейс.
...
В подразделе «Обзор» представлена ключевая информация о кластере Deckhouse Platform (DP) и его компонентах.
...
В подразделе «Обновления» содержится информация о релизах DP.
...
Подраздел предоставляет доступ к критически важным настройкам кластера DP:
...
Подраздел предоставляет сводную информацию обо всех узлах кластера DP с возможностью фильтрации и сортировки.
...
Шаблоны проектов по умолчанию включают базовые сценарии использования и демонстрируют возможности DP. Для добавления нового шаблона используется кнопка «Создать» в правом верхнем углу.
...
В подразделе отображается информация о текущих Ingress-контроллерах, обеспечивающих маршрутизацию трафика внутри кластера DP.
...
Подраздел предназначен для проверки контейнерных образов на наличие уязвимостей (CVE) в кластере DP.
...
Список всех доступных алертов системы мониторинга DP приведён на [отдельной странице документации](../../reference/alerts.html).
