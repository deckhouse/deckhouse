...
Веб-интерфейс Deckhouse предоставляет доступ к управлению кластером, включая мониторинг, настройку параметров, управление узлами и модулями, а также настройку безопасности и сети. Большинство операций, доступных в командной строке через утилиты `d8` ([Deckhouse CLI](../../cli/d8/)) или `kubectl`, также можно выполнять через веб-интерфейс.
...
Под заголовком «Конфигурация» указывается имя создаваемого класса, а в блоке «Ресурсы» настраиваются следующие параметры:
...
В блоке «Сети» можно задать основную подсеть, выбрать тип сети и включить использование публичного IP.
Также предусмотрена возможность добавления дополнительных подсетей и лейблов для более гибкой настройки инфраструктуры.

![Форма добавления класса машин](../../images/console/add_class.png)
