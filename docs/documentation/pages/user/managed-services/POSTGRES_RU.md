...
- [Настроить параметры PostgreSQL](#настройка-параметров-postgresql);
- [Настроить TLS](#настройка-tls);
- [Настроить наблюдаемость](#настройка-наблюдаемости).

Ниже представлен типовой пример манифеста ресурса Postgres `app-postgres`, который можно применить сразу, а затем донастроить под свои задачи, последовательно проходя каждый из этих шагов.
...
Для подключения из кластера используйте соответствующий Service и учётные данные из Secret пользователя. В [основном примере](#основной-пример-создание-postgres) приложение с правами на запись подключается к Service `d8ms-pg-app-postgres-rw` от имени пользователя `app-rw` к базе данных `app`.
