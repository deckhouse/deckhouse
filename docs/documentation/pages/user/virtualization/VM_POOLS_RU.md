...
Число реплик в пуле меняется вручную или автоматически, средствами автомасштабирования. Пул поддерживает стандартный субресурс `scale` и публикует `status.selector`, поэтому HPA читает метрики CPU и памяти прямо с реплик без дополнительной обвязки.

Помимо процессора и памяти пул работает и с кастомными метриками (`Pods`/`External` через `custom.metrics.k8s.io`/`external.metrics.k8s.io`) и с KEDA, например для масштабирования по длине внешней очереди.
...
При `scaleDownPolicy: Explicit` автомасштабирование может только увеличивать число реплик, а безадресное сжатие через субресурс `scale` отклоняется.
