...
В архив попадает следующая информация:
...
- XML-конфигурация домена ВМ.

Собранные данные лежат в архиве в формате YAML (для ресурсов) и в текстовых файлах (для логов). Такой архив можно передать в службу технической поддержки для анализа проблемы.
...
Команда `collect-debug-info` собирает диагностические данные о виртуальной машине и всех связанных ресурсах в сжатый архив (tar.gz), который выводится в stdout, поэтому вывод нужно перенаправить в файл.
...
Без перенаправления вывода в файл команда завершится с ошибкой, а при успешном выполнении вы получите архив `debug-info.tar.gz`.
