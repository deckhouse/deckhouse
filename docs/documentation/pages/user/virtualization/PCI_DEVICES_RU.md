...
Если устройство извлечено из узла, машина не запускается и остаётся в фазе `Pending`. В параметре [`.status.pciDevices`](/modules/virtualization/cr.html#virtualmachine-v1alpha2-status-pcidevices) такое устройство перестаёт быть готовым, но остаётся у машины, пока указано в её спецификации. Работающую машину DP при этом не останавливает, в том числе когда устройство стало недоступно проекту.
