...
Устройство подключается к машине и отключается от неё без её остановки. USB-устройство автоматически пробрасывается по сети (USBIP) на узел, где запущена машина, поэтому размещать машину на узле с устройством вручную не нужно.

{% alert level="warning" %}
Во время миграции ВМ USB-устройство ненадолго отключится и подключится на новом узле в момент переключения машины. При сбое миграции устройство останется на старом узле.
{% endalert %}
...
Добавьте устройство в параметр [`.spec.usbDevices`](/modules/virtualization/cr.html#virtualmachine-v1alpha2-spec-usbdevices) ресурса [VirtualMachine](/modules/virtualization/cr.html#virtualmachine):
