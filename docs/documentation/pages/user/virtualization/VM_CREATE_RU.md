...
Ниже показано, как запустить виртуальную машину с Ubuntu 24.04 на диске, [созданном ранее](../storage/vm-disks.html#создание-диска-из-образа). Сценарий cloud-init устанавливает агента `qemu-guest-agent` и сервис `nginx`, а также создаёт пользователя `cloud` с паролем `cloud`.
