...
> В процессе клонирования для виртуальной машины и всех её дисков автоматически создаются временные снимки. Именно из этих снимков затем собирается новая ВМ. После завершения клонирования временные снимки удаляются автоматически, поэтому в списке ресурсов их не будет. При этом в спецификации клонированных дисков остаётся ссылка (`dataSource`) на соответствующий снимок, даже если самого снимка уже не существует. Это ожидаемое поведение, а не признак проблемы, ведь к моменту запуска клона все необходимые данные уже перенесены на новые диски.
