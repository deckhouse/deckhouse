...
Клонирование ВМ выполняется с использованием ресурса [VirtualMachineOperation](/modules/virtualization/cr.html#virtualmachineoperation) с типом операции `Clone` и поддерживается как для выключенных, так и для работающих машин. При клонировании работающей ВМ автоматически создаётся консистентный снимок, из которого затем формируется клон.
...
Параметры `nameReplacements` и `customization` задаются в блоке [`.spec.clone`](/modules/virtualization/cr.html#virtualmachineoperation-v1alpha2-spec-clone).
