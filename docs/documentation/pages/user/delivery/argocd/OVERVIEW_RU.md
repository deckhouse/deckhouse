...
Пользовательская работа с Argo CD обычно включает:
...
[AppProject](/modules/operator-argo/cr.html#appproject) — это кастомный ресурс Argo CD, который задаёт логические границы проекта. С его помощью можно определить:
...
Каждый объект [Application](/modules/operator-argo/cr.html#application) должен ссылаться на проект через параметр [`spec.project`](/modules/operator-argo/cr.html#application-v1alpha1-spec-project).
...
- декларативно — с помощью объекта [Application](/modules/operator-argo/cr.html#application);
- интерактивно — через веб-интерфейс Argo CD;
- с помощью CLI-утилиты `argocd`.

### Создание приложения с помощью объекта Application

Для описания приложения используется объект [Application](/modules/operator-argo/cr.html#application). В нём указываются:

- проект Argo CD ([`spec.project`](/modules/operator-argo/cr.html#application-v1alpha1-spec-project));
- источник манифестов или чарта ([`spec.source`](/modules/operator-argo/cr.html#application-v1alpha1-spec-source));
- целевой кластер и неймспейс ([`spec.destination`](/modules/operator-argo/cr.html#application-v1alpha1-spec-destination));
- политика синхронизации ([`spec.syncPolicy`](/modules/operator-argo/cr.html#application-v1alpha1-spec-syncpolicy)).
