...
### Excluding resources from the quota

The platform provides the ability to exclude pod or PVC resources (requests, limits) from the project quota calculation. For example, some pods or PVCs may consume resources that should not count against the available project quota.

The following special labels are available for pods only:

- `resources.k8s.io/discount-memory` — reduces the pod's memory counted in the quota by the specified amount;
- `resources.k8s.io/discount-cpu` — reduces the pod's CPU counted in the quota by the specified amount;
- `resources.k8s.io/discount-storage` — reduces the pod's storage (ephemeral-storage) counted in the quota by the specified amount;
- `resources.deckhouse.io/cap-cpu` — limits the maximum CPU value counted in the project quota to the specified value;
- `resources.deckhouse.io/cap-memory` — limits the maximum memory value counted in the project quota to the specified value;

For pods and PVCs:

- `resources.k8s.io/ignore` — when set to `true`, completely excludes the pod or PVC resources from the project quota calculation;

Labels can be used together. However, if a pod has the `resources.k8s.io/ignore="true"` label, all other labels are ignored.
