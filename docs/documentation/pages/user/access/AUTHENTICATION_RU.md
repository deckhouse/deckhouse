...
description: "Настройка аутентификации для пользовательских приложений в Deckhouse Platform. Интеграция с внешними провайдерами аутентификации LDAP, GitLab, GitHub..."
...
Аутентификация — это процесс проверки подлинности пользователя. Deckhouse Platform (DP) поддерживает сквозную аутентификацию пользователей при доступе к интерфейсам платформы и ресурсам кластера. Этот механизм также может использоваться для включения аутентификации в собственных приложениях, развёрнутых в кластере.

DP позволяет настраивать аутентификацию как на основе внутренней базы данных, так и с использованием внешних провайдеров — например, LDAP, GitLab или GitHub. Это даёт возможность использовать единую аутентификацию сразу в нескольких кластерах DP.

С точки зрения пользователя кластера или разработчика приложения, не имеет значения то, как администратор настроил аутентификацию в DP — интерфейс аутентификации для пользователя и способы включения аутентификации для приложения будут одинаковые.

{% alert level="info" %}

Чтобы использовать аутентификацию в DP, необходима [настройка](../../admin/configuration/access/authentication/).

{% endalert %}

## Интерфейс

Интерфейс аутентификации открывается при первом обращении к ресурсу, для которого включена аутентификация — DP перенаправляет пользователя на страницу аутентификации. Если пользователь уже прошёл аутентификацию (например во внешнем провайдере аутентификации), то DP перенаправит запрос обратно к ресурсу, к которому первоначально обращался пользователь, обогатив запрос данными аутентификации. Если аутентификация не пройдена, пользователь увидит интерфейс аутентификации.

Пример интерфейса аутентификации в DP:

![Пример интерфейса аутентификации](../../images/user/access/authentication/web-auth-example.png)

Интерфейс аутентификации предлагает выбрать метод аутентификации, если их настроено несколько. Если настроен только один внешний провайдер аутентификации, то пользователь сразу попадет на страницу аутентификации этого провайдера. Если в DP созданы [локальные пользователи](../../admin/configuration/access/authentication/local.html), то DP предложит ввести логин и пароль.

Пример интерфейса аутентификации в DP с вводом логина и пароля:

![Пример интерфейса аутентификации с вводом логина и пароля](../../images/user/access/authentication/web-auth-example2.png)

## Смена и сброс пароля локального пользователя

Если в кластере настроена [локальная аутентификация](../../admin/configuration/access/authentication/local.html), пользователь может изменить или сбросить пароль самостоятельно.

### Смена пароля по требованию политики или администратора

При входе DP может перенаправить локального пользователя на форму смены пароля, если:

- сложность пароля не соответствует [настроенной политике](../../admin/configuration/access/authentication/local.html#настройка-парольной-политики);
- истёк срок действия пароля согласно [настроенной политике](../../admin/configuration/access/authentication/local.html#настройка-парольной-политики);
- администратор инициировал принудительную смену пароля.

В форме нужно указать текущий пароль, новый пароль и подтверждение. Новый пароль должен соответствовать парольной политике кластера.

### Самостоятельный сброс пароля

Локальный пользователь может сбросить свой пароль в интерфейсе аутентификации. Операция выполняется через ресурс [UserOperation](/modules/user-authn/cr.html#useroperation) с `initiatorType: self`.

Самостоятельный сброс доступен только для локальных учётных записей. Если вы входите через LDAP, GitHub или другой внешний провайдер, обратитесь к администратору соответствующей системы.

После сброса пароля активные сессии завершаются — потребуется повторный вход с новым паролем.

## Включение аутентификации в веб-приложении

> Для работы аутентификации в приложении, аутентификация должна быть настроена на уровне Deckhouse Platform.

В DP можно включить аутентификацию для приложения двумя способами
В зависимости от того, умеет приложение обрабатывать запросы на аутентификацию (выступать OIDC-клиентом) или нет, в DP можно включить аутентификацию для приложения двумя способами. Оба способа рассматриваются далее.
...
Чтобы включить аутентификацию для приложения, развернутого в DP выполните следующие шаги:
...
1. Создайте объект [DexAuthenticator](/modules/user-authn/cr.html#dexauthenticator) в пространстве имен приложения.
   ...
   * Секреты (Secret), необходимые для доступа к системе аутентификации DP.
...
Приложения, которые умеют самостоятельно обрабатывать запросы на аутентификацию и выступать OIDC-клиентом, могут напрямую взаимодействовать с системой аутентификации DP. В этом случае приложение самостоятельно перенаправляет пользователя на страницу входа и обрабатывает полученные OIDC-токены.
...
1. Создайте объект [DexClient](/modules/user-authn/cr.html#dexclient) в пространстве имен приложения.
   ...
   * В системе аутентификации DP будет зарегистрирован OIDC-клиент с идентификатором (`clientID`) вида:  
