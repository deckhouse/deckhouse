...
Кроме основной сети кластера виртуальную машину (ВМ) можно подключить к дополнительным сетям модуля [`sdn`](/modules/sdn/) и выдать её интерфейсам адреса.

## Дополнительные сетевые интерфейсы

Кроме основной сети кластера машину можно подключить к дополнительным сетям, проектным (Network) и кластерным (ClusterNetwork). Нужные сети перечисляют в блоке [`.spec.networks`](/modules/virtualization/cr.html#virtualmachine-v1alpha2-spec-networks), а если этот блок не задан, машина работает только в основной сети кластера.
...
Кластерный ресурс [VirtualMachineMACAddressLease](/modules/virtualization/cr.html#virtualmachinemacaddresslease) (`vmmacl`) управляет арендой адресов из общего пула, а проектный ресурс [VirtualMachineMACAddress](/modules/virtualization/cr.html#virtualmachinemacaddress) (`vmmac`) отвечает за резервирование арендованных адресов и их привязку к машинам. Адрес закрепляется за машиной до её удаления.
...
Перечислите нужные сети в блоке [`.spec.networks`](/modules/virtualization/cr.html#virtualmachine-v1alpha2-spec-networks) спецификации машины.
...
Адреса в дополнительной сети Deckhouse Platform (DP) может выдавать сам, если администратор настроил для этой сети пул адресов ([`spec.ipam.ipAddressPoolRef`](/modules/sdn/cr.html#clusternetwork-v1alpha1-spec-ipam-ipaddresspoolref) в модуле [`sdn`](/modules/sdn/)). Тогда DP выделяет адреса дополнительным интерфейсам ВМ и доставляет их в гостевую ОС через DHCP.
