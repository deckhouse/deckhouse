...
В этом разделе вы узнаете о работе с дашбордами для анализа состояния Deckhouse Platform (DP)
...
В DP доступны предустановленные дашборды, а также пользовательские дашборды, которые можно создавать несколькими способами.

| Вид дашбордов | Описание |
| ------------- | -------- |
| [Предустановленные](#предустановленные-дашборды) | Готовые дашборды, поставляемые вместе с DP. Предназначены для мониторинга состояния запущенных приложений. |
...
Пользователи DP могут создавать собственные дашборды несколькими способами,
...
- Администраторы DP получают доступ ко всем системным метрикам:
...
Поддержка этого способа будет прекращена в следующих версиях DP.
{% endalert %}
