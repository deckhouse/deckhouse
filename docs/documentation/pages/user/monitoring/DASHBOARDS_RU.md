...
а также о состоянии подов, контроллеров, узлов и пространств имён.
...
- **Namespaces** — совокупные метрики Ingress-ресурсов по пространствам имён;
- **Namespace Detail** — детальная информация по Ingress-ресурсам в выбранном пространстве имён;
...
Предназначены для оценки нагрузки, поиска проблем с ресурсами и анализа состояния рабочих нагрузок.

Доступные дашборды:

- **Namespaces** — сводная информация по всем пространствам имён;
- **Namespace** — основные показатели использования ресурсов в выбранном пространстве имён;
- **Namespace / Controller** — статистика использования ресурсов контроллерами в рамках выбранного пространства имён;
...
Пользователи могут создавать собственные дашборды двумя способами:

| Способ | Описание |
| ------ | -------- |
| [С помощью модуля `observability`](#с-помощью-модуля-observability) | Рекомендуемый способ, при котором дашборды создаются с помощью ресурса ObservabilityDashboard на уровне пространств имен и с возможностью разграничения прав доступа. |
| [С помощью GrafanaDashboardDefinition](#с-помощью-grafanadashboarddefinition) | Устаревший способ, при котором дашборды создаются с помощью ресурса GrafanaDashboardDefinition на уровне кластера. Требует расширенных прав доступа и не позволяет управлять разделением прав доступа. Поддержка этого способа будет прекращена в следующих версиях DKP. |
...
Модуль добавляет новые типы дашбордов, включая ресурсы, ограниченные пространством имён.
Это позволяет пользователям создавать и управлять собственными дашбордами
без требования к наличию прав на объекты кластерного уровня.
Модуль также упрощает редактирование дашбордов, позволяя пользователям настраивать их напрямую в веб-интерфейсе,
без необходимости работы с ресурсами вручную.
...
- [ObservabilityDashboard](/modules/observability/cr.html#observabilitydashboard) — дашборды в рамках пространства имён.
...
- [ClusterObservabilityPropagatedDashboard](/modules/observability/cr.html#clusterobservabilitypropagateddashboard) — дашборды, расширяющие список дашбордов из двух предыдущих категорий.
  ...
  Они становятся доступны пользователям, обладающим правами на соответствующее пространство имён или системный раздел.
...
- `observabilitydashboards.observability.deckhouse.io` — дашборды в рамках пространств имён;
...
- Пользователи пространств имён получают доступ только к метрикам своего пространства имён.
...
- Метрики из пользовательских пространств имён также могут быть доступны администраторам
...
- Системные дашборды и дашборды, добавленные через GrafanaDashboardDefinition, нельзя изменить через интерфейс Grafana.
