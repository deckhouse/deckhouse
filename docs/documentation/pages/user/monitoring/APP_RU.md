...
description: "Настройка мониторинга приложений в Deckhouse Platform. Подключение через лейблы, ServiceMonitor, PodMonitor, ScrapeConfig. Интеграция с Prometheus и blackbox-exporter для проверки доступности."
permalink: ru/user/monitoring/app.html
lang: ru
---

Deckhouse Platform (DP) поддерживает четыре способа подключения приложения к системе мониторинга:
...
1. Убедитесь, что включен [модуль `monitoring-custom`](/modules/monitoring-custom/).
   При необходимости обратитесь к администратору DP.
...
DP поддерживает подключение приложений через два схожих по функциональности ресурса:
...
1. Создайте ресурс PodMonitor или ServiceMonitor, указав обязательный лейбл `prometheus: main`,
   ...
   Поле `spec.selector.matchLabels` определяет, какие объекты будут использоваться для сбора метрик.
   Для [PodMonitor](/modules/operator-prometheus/cr.html#podmonitor-v1-spec-selector-matchlabels) указываются лейблы целевых подов, а для [ServiceMonitor](/modules/operator-prometheus/cr.html#servicemonitor-v1-spec-selector-matchlabels) — лейблы целевого сервиса.
   ...
   spec:
     selector:
       matchLabels:
         # Лейбл подов, с которых необходимо собирать метрики.
         app: example-app
     podMetricsEndpoints:
       - port: metrics
         interval: 30s
   ...
   spec:
     selector:
       matchLabels:
         # Лейбл сервиса, через который доступны метрики приложения.
         app: example-app
     endpoints:
       - port: web
         interval: 30s
...
Если вам нужно добавить дополнительные лейблы непосредственно в собираемые метрики,
воспользуйтесь правилами переписывания метрик (с помощью параметра [`metricRelabelings`](/modules/operator-prometheus/cr.html#podmonitor-v1-spec-podmetricsendpoints-metricrelabelings)).
...
DP поддерживает сбор метрик доступности с [blackbox-exporter](https://github.com/prometheus/blackbox_exporter/),
который не входит в состав DP и должен быть установлен отдельно в кластере.
...
Чтобы подключить Probe к системе мониторинга DP, выполните следующие шаги:
