...
description: "Обзор системы мониторинга в Deckhouse Platform. Готовые дашборды Grafana, алерты Prometheus, сбор метрик приложений. Настройка мониторинга пользовательских приложений и создание собственных дашбордов."
...
Данный раздел предназначен для пользователей проектов Deckhouse Platform (DP).

В состав DP включена система мониторинга,
которая предоставляет пользователям удобные инструменты для наблюдения за состоянием инфраструктуры и приложений.

По умолчанию в DP доступен готовый набор дашбордов и алертов,
...
После установки DP пользователям по умолчанию доступен базовый набор инструментов для наблюдения за кластером.
...
По умолчанию в кластере DP включены уведомления о следующих видах событий:
...
Пользователям доступны следующие настройки системы мониторинга DP:
...
- **настройка собственных алертов** — можно задать новые правила уведомлений одним из двух способов:
  ...
  - **устаревший способ** — с помощью ресурса [CustomPrometheusRules](/modules/prometheus/cr.html#customprometheusrules) модуля `prometheus`. Пример этого способа доступен [в документации модуля](/modules/prometheus/faq.html#как-добавить-алерты-иили-recording-правила-для-вашего-проекта).
