...
* `modules` — параметры для служебных компонентов;
  ...
  * `placement` — настройки, определяющие расположение компонентов Deckhouse.
    * `customTolerationKeys` — список ключей пользовательских taint'ов, необходимо указывать, чтобы позволить выезжать на выделенные узлы критическим add-on'ам, таким как например cni и csi.
      * Пример:
  ...
  * `resourcesRequests` - количество CPU и memory, выделяемые для работы служебных компонентов.
    ...
    * `masterNode` - системные компоненты (control plane и системные компоненты на master-узлах).
      * `cpu` – сумма CPU, выделяемая для системных компонентов на master-узлах сверх `everyNode`.
      ...
      * `memory` – суммарный объем памяти, выделяемый для системных компонентов на master-узлах сверх `everyNode`.
