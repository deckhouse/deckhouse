...
Для версионирования модулей используется [`semver`][semver]. Какую версию поднимать, выбирается следующим образом:

**Patch-версия** — исправление дефекта (бага).

**Minor-версия** — добавление новой функции.

**Major-версия** — добавление функции, которая кардинально меняет возможности модуля.
Масштабное изменение интерфейса. Завершение крупного этапа работы.

> Перед номером версии в теге git и контейнере registry всегда добавляется буква "v".
...
Выпущенная версия модуля перемещается по каналам стабильности, от менее стабильных к более стабильным: `Alpha`, `Beta`, `EarlyAccess`, `Stable`, `RockSolid`.
...
Во время разработки модуль может находиться на следующих стадиях (stage) готовности:

**Alpha** - это начальная стадия для каждого нового модуля. На этом этапе модуль можно включить и использовать, но у него еще нет большого количества установок.

**Beta** - на этой стадии у модуля уже есть достаточное количество установок и отзывов, чтобы убедиться в его работоспособности для большинства пользователей.

**Stable** - эта стадия является финальной и стабильной. На ней модуль считается достаточно стабильным для массового использования.
...
Выводы:
- Модуль на стадии `Alpha` в канале `Stable` не рекомендуется использовать в production-средах.
- Модуль на стадии `Stable` в канале `Alpha`, также не рекомендуется использовать в production-средах.
...
*ModuleConfig* позволяет управлять дополнительными возможностями модуля. Эти опции могут быть помечены как `Alpha`, `Beta`, `Stable` или `Deprecated` в параметре `x-feature-stage` в схеме OpenAPI `x-feature-stage: Alpha|Beta|Stable|Deprecated` (значение по умолчанию - `Stable`).

При включении функций на стадии, отличной от Stable, выдается предупреждение.

В настройках Deckhouse Kubernetes Platform можно задать глобальные правила, определяющие, какие функции и на каком этапе могут быть включены в кластере. Это помогает предотвратить случайное использование Alpha-функций в рабочих средах.

## Версионирование API

Модули в Deckhouse используют пользовательские ресурсы (Custom Resources) для взаимодействия с пользователями. Поле apiVersion (версия API) этих ресурсов обновляется в соответствии со следующими правилами:
...
* если накопилось много устаревших параметров;
