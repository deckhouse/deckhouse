...
* **Сборка**: во время сборки создается артефакт модуля и загружается в registry контейнера. В разделе **Анатомия модуля** --> **Registry контейнера** разбирается, где хранятся образы в registry и какие образы доступны.
* **Запуск в кластере**: как модуль устанавливается в кластер Deckhouse Kubernetes Platform, как его включить, настроить и проверить, что он работает правильно. В разделе **Как опубликовать модуль в кластере DKP?** разбираются все доступные кастомные ресурсы _Definition_ и содержатся подсказки, где искать ошибки в случае проблем.
