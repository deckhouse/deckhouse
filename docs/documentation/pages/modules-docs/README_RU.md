...
* **Разработка**: создание кода модуля и его структуры в git-репозитории. В разделе [**Анатомия модуля** → **Папка с модулем**](module-anatomy/module-folder/) описывается, какие папки существуют и что в них находится.
* **Сборка**: создание артефакта модуля во время сборки и загрузка его в container registry. В разделе [**Анатомия модуля** → **Container registry**](module-anatomy/registry/) разбирается, где хранятся образы в registry и какие образы доступны.
* **Запуск в кластере**: установка модуля в кластер Deckhouse Kubernetes Platform, его включение, настройка и проверка, что он работает правильно. В разделе [**Как опубликовать модуль в кластере DKP?**](modules-docs/deckhouse/) описываются все доступные _СustomResourceDefinition_ (CRD) и содержатся подсказки, где искать ошибки в случае проблем.
