...
* **Разработка**: создание кода модуля и его структуры в git-репозитории. В разделе [**Папка с модулем**](module-anatomy/module-folder/) описывается, какие папки существуют и что в них находится.
* **Сборка**: создание артефакта модуля во время сборки и загрузка его в container registry. В разделе [**Container registry**](module-anatomy/registry/) разбирается, где хранятся образы в registry и какие образы доступны.
* **Запуск в кластере**: установка модуля в кластер Deckhouse Kubernetes Platform, его включение, настройка и проверка, что он работает правильно. В разделе [**Как опубликовать модуль в кластере DKP?**](modules-docs/deckhouse/) разбираются все доступные _СustomResourceDefinition_ (CRD) и содержатся подсказки, где искать ошибки в случае проблем.
...
> Хук позволяет сохранять значения в памяти и использовать их позже при рендеринге шаблонов Helm. Об этой особенности и цикле работы модуля, рекомендуем прочитать в [документации Addon operator](https://flant.github.io/addon-operator/OVERVIEW.html#hooks-and-helm-values).
...
* 🎬 [рекомендуем посмотреть видео](https://www.youtube.com/watch?v=1_55KPHjVTU), где подробно рассказано о shell-operator и addon-operator.
