...
* `README.md` — описание, для чего нужен модуль, какую проблему он решает и общие архитектурные принципы.
  ...
  - `menuTitle` - **(желательно)** Название модуля в меню слева на странице документации. Пример - "Deckhouse Admin". Если отсутствует, то используется название папки (репозитория) |прим. Лена: Все таки папки или репозитория?|, например `deckhouse-admin`.
...
Необходим для проверки параметров модуля, которые пользователь может настроить через [_ModuleConfig_](deckhouse_ru.md#ресурс-moduleconfig).
...
* Доступы в registry из ресурса [_ModuleSource_](deckhouse_ru.md/#ресурс-modulesource) доступны по пути `.Values.<имяМодуля>.registry.dockercfg`.
