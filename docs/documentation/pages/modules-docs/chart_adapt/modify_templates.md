...
> Команда Deckhouse Kubernetes Platform старается тщательно подходить к выбору параметров, которые могут настраивать пользователи. Мы считаем, что важно предоставить пользователям возможность настроить только те параметры, которые действительно имеют значение для их работы.
