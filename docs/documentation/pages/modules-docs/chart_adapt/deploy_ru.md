...
2.  Для установки и последующего обновления, определите *ModuleUpdatePolicy* для *ModuleSource* (обратите внимание, что канал обновления теперь указывается в политике обновления в параметре `.spec.releaseChannel`), например:
...
3. Проверьте *ModuleSource* (в статусе не должно содержаться ошибок и должны быть перечислены доступные модули):

   ```sh
   kubectl get ms hello-world -o yaml
   ```

4.  Убедитесь, что были созданы новые *ModuleReleases* для модулей, подпадающих под *ModuleUpdatePolicy*, и что они имеют статус Pending (если режим обновления ручной или автоматический с указанием окна обновления за пределами текущей даты/временного интервала) или Deployed (если обновление автоматическое без указания окна обновления, или окно обновления совпадает с текущей датой/временным интервалом):
...
5. В случае успешной установки релизов, дождитесь перезапуска пода Deckhouse Kubernetes Platform.

   ```sh
   kubectl -n d8-system get pod -l app=deckhouse
   ```

6. Включите модуль при помощи создания объекта в Deckhouse Kubernetes Platform.
