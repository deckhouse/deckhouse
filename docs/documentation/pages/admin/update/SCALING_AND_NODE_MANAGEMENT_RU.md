...
При добавлении узлов всех типов в кластер используется один и тот же подход: на сервере выполняется специально сгенерированный скрипт bootstrap.sh — в результате сервер добавляется как узел в одну из групп кластера.
Способы доставки и запуска скрипта различаются в зависимости от типа узла. Для CloudEphemeral- и CloudPermanent-узлов скрипт bootstrap.sh добавляется в cloud-init-скрипты ОС. Для Static-узлов скрипт доставляется и запускается по SSH вручную или автоматически.

Первые и основные задачи скрипта bootstrap.sh — определить операционную систему, на которой он запущен, связаться с API кластера и получить от него набор скриптов на исполнение — бандл (bundle). Бандл генерируется с помощью bashible API server. Он индивидуальный для каждой группы узлов и для каждой поддерживаемой операционной системы. Список доступных бандлов можно изменить в настройках модуля node-manager.

В процессе установки на узле будут созданы systemd-сервис и таймер bashible. Всякий раз, когда таймер истекает, bashible будет обращаться к API и проверять, изменилась ли контрольная сумма скриптов бандл, и если она изменилась, то выполнять их.

Бандл разбит на отдельные скрипты-шаги, которые выполняются последовательно. Администраторы кластера могут добавлять собственные скрипты-шаги в бандлы и определять, на каких группах узлов они будут выполняться.

Собственные шаги конфигурации узлов описываются специальным ресурсом NodeGroupConfiguration, например:
...
В cloud-кластеры DKP можно добавлять узлы трёх типов — в зависимости от варианта их создания в облаке.

Static- и CloudStatic-узлы создаются вручную. Способы их добавления в кластер такие же, как и для bare metal. Однако, чтобы добавить эти узлы в кластер, может потребоваться дополнительная конфигурация (в зависимости от облачного провайдера).
Узлы CloudStatic позволяют использовать облачные диски провайдера.

CloudPermanent-узлы заказывает Terraform, встроенный в DKP.
Узлы этого типа создаются и настраиваются в (Provider)ClusterConfiguration.
Они позволяют использовать зарезервированные IP-адреса в облачном провайдере и не поддерживают автомасштабирование.
Именно к этому типу относятся мастер-узлы cloud-кластеров DKP.

CloudEphemeral-узлы автоматически создаёт и удаляет Machine Controller Manager (MCM) в настроенном облачном провайдере.
Группа с узлами этого типа поддерживает горизонтальное автомасштабирование.
Такие узлы составляют основу cloud-кластеров DKP.

1. Добавление CloudPermanent-узлов в cloud-кластер.

Конфигурация CloudPermanent-узлов находится в (Provider)ClusterConfiguration. С этим конфигом рекомендуется работать извне кластера, в установочном контейнере, с помощью утилиты dhctl.

Сначала необходимо запустить контейнер установщика той же версии, что и у DKP в кластере. Посмотреть версию DKP можно в консоли администратора или с помощью команды kubectl:

```console
kubectl -n d8-system get deployment/deckhouse -ojsonpath="{.metadata.annotations.core\.deckhouse\.io/version}"
```

Далее нужно запустить контейнер установщика:

```console
docker run --pull=always -it -v "$HOME/.ssh/:/tmp/.ssh/" registry.deckhouse.ru/deckhouse/ee/install:v1.57.5 bash
```

Версия DKP передаётся в качестве тега образа установщика.

Обратите внимание: в команде запуска контейнера нужно примонтировать директорию с приватным SSH-ключом, который используется для доступа в кластер.

Прежде чем начинать работу с (Provider)ClusterConfiguration, рекомендуется проверить состояние Terraform кластера с помощью dhctl:

```console
dhctl terraform check --ssh-host 158.160.137.209 --ssh-user ubuntu --ssh-agent-private-keys /tmp/.ssh/education_id_rsa
```

В этой команде мы добавляем адрес мастер-узла кластера, имя пользователя и ключ для подключения по SSH.
При выполнении этой команды dhctl подключится по SSH к мастер-узлу, скачает state Terraform, выполнит проверки и выведет отчёт.
...
nodes:
- group: master
  name: stand-0-master-0
  status: ok
```

Если по результатам проверки везде статус ok, можно приступать к работе с (Provider)ClusterConfiguration. В противном случае нужно сначала исправить несоответствия state Terraform и объектов в облаке.

Чтобы отредактировать (Provider)ClusterConfiguration, необходимо запустить dhctl-команду:

```console
dhctl config edit provider-cluster-configuration --ssh-host 158.160.137.209 --ssh-user ubuntu --ssh-agent-private-keys /tmp/.ssh/education_id_rsa
```

При выполнении этой команды dhctl по SSH загрузит из кластера текущий (Provider)ClusterConfiguration и откроет его на редактирование.

Конфигурации CloudPermanent-узлов у разных облачных провайдеров будут отличаться.
...
В этой конфигурации мы указываем:
name: cloud-permanent — имя группы узлов. Узлы в этой группе будут именоваться так же, как и группа, с добавлением суффикса — номера узла, например cloud-permanent-0, cloud-permanent-1, …, cloud-permanent-n;
instanceClass — параметры виртуальных машин группы;
replicas: 2 — количество виртуальных машин в группе.

После того как в конфиг внесены изменения, он сохраняется локально в контейнере. Чтобы применить новую конфигурацию, необходимо выполнить команду dhctl converge:

```console
dhctl converge --ssh-host 158.160.137.209 --ssh-user ubuntu --ssh-agent-private-keys /tmp/.ssh/education_id_rsa
```

В результате выполнения этой команды dhctl создаст (или удалит) виртуальные машины в облаке, создаст группу узлов в кластере в соответствии с конфигурацией. 

Далее на вновь созданных виртуальных машинах выполнится bootstrap-скрипт. В результате машины добавятся в группы.

Команда dhctl converge дождётся, пока все заказанные узлы перейдут в статус Ready, и завершит работу.

1. Добавление CloudEphemeral-узлов в cloud-кластер

Виртуальные машины узлов в группах CloudEphemeral создаются с помощью Machine Controller Manager (MCM), который входит в состав модуля node-manager в DKP.

MCM — это группа контроллеров, которые управляют жизненным циклом рабочих машин. Он вдохновлён Kube Controller Manager, в котором различные подконтроллеры управляют примитивами Kubernetes.

MCM согласует между собой пользовательские ресурсы — MachineDeployment, MachineSet и Machines, — которыми управляют и которые мониторят их собственные одноимённые контроллеры и совместный контроллер Safety Controller.

Параметры виртуальных машин, которые создаются в облаке, задаются в специальном ресурсе (Provider)InstanceClass. В нём перечислены количество CPU и памяти, объём корневого диска, операционная система и прочие настройки, специфичные для каждого облачного провайдера.

Количество узлов, зоны размещения (если применимо), параметры масштабирования и автомасштабирования для узлов типа CloudEphemeral задаются в самом объекте NodeGroup.

В каждом объекте группы узлов необходимо задать ссылку на один из объектов (Provider)InstanceClass.

MCM отследит появление группы узлов, обратится к API облака и создаст необходимое количество виртуальных машин в соответствии с конфигурацией из объекта (Provider)InstanceClass, который указан в группе узлов.

Машины, созданные в облаке, после запуска автоматически сконфигурируются и добавятся в качестве узлов в соответствующую группу.
...
- просто злы (клауды и прочее)
- автомасштабирование (гле возможно и как работает)
- hpa и vpa (относится к приложению)