...
Класс виртуальной машины (ВМ) задаёт, какой процессор и какой набор инструкций увидит гостевая система.
...
- `universal` — ВМ запускаются на любом узле и мигрируют между всеми четырьмя. Deckhouse Platform (DP) возьмёт набор инструкций, общий для обоих процессоров, поэтому совместимость максимальная, а часть возможностей «CPU Y» останется неиспользованной;
