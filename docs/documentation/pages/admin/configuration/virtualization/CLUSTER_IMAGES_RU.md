...
Чтобы DP сверил скачанный файл с контрольной суммой, добавьте в источник блок [`checksum`](/modules/virtualization/cr.html#clustervirtualimage-v1alpha2-spec-datasource-http-checksum). Если файл не совпал ни с одной из указанных сумм, образ перейдёт в фазу `Failed`.
...
DP работает только с теми хранилищами, где включён TLS. Если хранилище использует собственный центр сертификации, передайте цепочку сертификатов в параметре [`caBundle`](/modules/virtualization/cr.html#clustervirtualimage-v1alpha2-spec-datasource-containerimage-cabundle), а учётные данные для доступа к закрытому хранилищу возьмите из секрета, указанного в параметре `imagePullSecret`.
