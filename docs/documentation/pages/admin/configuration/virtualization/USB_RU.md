...
1. DRA-драйвер обнаруживает USB-устройства на узлах и публикует сведения о них в API Kubernetes как [ResourceSlice](https://kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/). DP создаёт ресурсы [NodeUSBDevice](/modules/virtualization/cr.html#nodeusbdevice) по этим данным.
...
1. После назначения неймспейса DP создаёт в нём ресурс [USBDevice](/modules/virtualization/cr.html#usbdevice).
