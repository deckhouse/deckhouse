...
Политика состоит из списка правил, каждое из которых действует на свой диапазон ядер. Диапазон задаётся обязательным блоком `cores`, и диапазоны разных правил пересекаться не могут, иначе Deckhouse Platform (DP) отклонит такой класс.
