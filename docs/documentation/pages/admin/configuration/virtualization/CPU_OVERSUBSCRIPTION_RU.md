...
Размещая ВМ на узле, Deckhouse Platform (DP) складывает гарантированные доли всех ВМ узла по формуле `Σ(cores × coreFraction / 100)`. Если сумма превысит число физических ядер, на этом узле ВМ не запустится.
