...
DP обнаруживает устройства на подходящих узлах и создаёт для каждого из них ресурс [NodePCIDevice](/modules/virtualization/cr.html#nodepcidevice). Шину PCI компонент сканирует при запуске и далее каждые пять минут, поэтому только что установленное устройство появляется в списке через несколько минут.
...
- DP обнаруживает не все устройства узла, потому что оборудование, от которого зависит работа самого узла, остаётся в его распоряжении. В списке не появятся:
...
- проброшенная сетевая карта работает в обход сетевой подсистемы кластера. Она не отражается в параметре [`.spec.networks`](/modules/virtualization/cr.html#virtualmachine-v1alpha2-spec-networks), а адреса IP и MAC на ней DP не выдаёт и не учитывает.
