...
{% alert level="warning" %}
Отключите vApp/Guest OS Customization (Guest Customization, vApp Customization и аналогичные механизмы) для шаблона и виртуальных машин кластера в VMware Cloud Director. DKP выполняет первичную настройку узлов через `cloud-init` (datasource OVF/VMware GuestInfo). Если customization включена, она может конфликтовать с `cloud-init` и приводить к некорректной инициализации узла.
{% endalert %}
