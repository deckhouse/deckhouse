...
Данный раздел описывает схемы размещения Kubernetes-кластера в инфраструктуре AWS и связанные с ними параметры. Выбор схемы (layout) влияет на способ назначения публичных IP-адресов, работу NAT и возможность подключения к узлам.
...
![Схема размещения Standard в Amazon](../../../../images/cloud-provider-amazon/layout-withoutnat.png)
...
{% alert level="danger" %} В данной схеме NAT Gateway всегда создается в зоне `a`. Если узлы кластера размещены в других зонах, то при сбое зоны a кластер может стать недоступным. Bastion-хост обязателен для подключения к узлам. {% endalert %}
...
![Схема размещения Standard в Amazon](../../../../images/cloud-provider-amazon/layout-withnat.png)
