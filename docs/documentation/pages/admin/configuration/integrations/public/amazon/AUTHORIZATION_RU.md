...
description: "Настройка подключения и авторизации AWS для Deckhouse Platform. Роли IAM, настройка учетных данных и требования к интеграции AWS для облачного развертывания."
lang: ru
---
...
Перед началом работы необходимо подготовить облачное окружение и обеспечить доступ к AWS-ресурсам для компонентов Deckhouse Platform (DP), взаимодействующих с API AWS.
...
Эти службы необходимы для корректной инициализации экземпляров EC2 и взаимодействия с инфраструктурными модулями DP, такими как [`cloud-provider-aws`](/modules/cloud-provider-aws/) и `machine-controller-manager`.
...
{% alert level="info" %}
Сохраните полученные `Access key ID` и `Secret access key`. Эти данные потребуются для настройки DP.
{% endalert %}
...
Параметр `iamNodeRole` в ресурсе AWSClusterConfiguration позволяет переопределить IAM-роль по умолчанию, которую DP назначает всем EC2-инстансам узлов кластера.

По умолчанию DP создаёт и назначает IAM-роль с именем `<prefix>-node`, где `<prefix>` — это значение параметра `prefix` в ModuleConfig `global`. Эта роль включает минимально необходимый набор прав для работы узлов.
...
{% alert level="info" %}
Указанная роль должна включать все права, требуемые DP, иначе узлы не смогут полноценно функционировать. В частности, она обязательно должна включать права роли `<prefix>-node>`, даже если они назначаются вручную.
{% endalert %}
