...
{% alert level="info" %}
Сохраните полученные `Access key ID` и `Secret access key`. Эти данные потребуются для настройки DKP.
{% endalert %}
...
По умолчанию DKP создаёт и назначает IAM-роль с именем `<prefix>-node`, где `<prefix>` — это значение параметра `prefix` в ModuleConfig `global`. Эта роль включает минимально необходимый набор прав для работы узлов.
