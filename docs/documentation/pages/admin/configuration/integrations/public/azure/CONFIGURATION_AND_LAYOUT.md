...
When a cluster is created, Deckhouse Platform (DP) creates a Network Security Group (NSG) named after the cluster prefix and associates it with the node subnet.
...
You cannot attach a pre-created custom NSG to nodes through the DP parameters. To restrict SSH access, use [`sshAllowList`](/modules/cloud-provider-azure/cluster_configuration.html#azureclusterconfiguration-sshallowlist).
