...
Чтобы мониторинг среды выполнения с KCS работал в кластере DKP, назначьте лейбл `security.deckhouse.io/pod-policy=privileged` пространству имен, в котором размещаются компоненты KCS.
