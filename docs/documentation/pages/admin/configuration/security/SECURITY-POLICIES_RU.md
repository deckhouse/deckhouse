...
3. Подпишите образ в хранилище образов контейнеров с помощью сгенерированного приватного ключа:
...
4. Чтобы включить проверку подписи образов контейнеров в кластере DKP,
...
5. Создайте ресурс OperationPolicy, ограничивающий запуск подов со сторонних registry:
...
6. Добавьте метку на пространство имен, где необходимо включить проверку подписи командой (укажите нужное пространство имён):

    ```shell
    kubectl label ns <NAMESPACE> security.deckhouse.io/verify-image-test=
    ```

7. Для проверки работы механизма подписи образов разверните поды в пространстве имён, с подписанным и неподписанным образами (укажите нужное пространство имен и ):
