...
В Deckhouse Kubernetes Platform (DKP) по умолчанию создается базовый набор политик аудита, которые фиксируют следующие события:

**Общие события:**
- Создание и удаление узлов кластера (объектов `Node`);
- Операции создания, изменения и удаления ресурсов во всех системных пространствах имен (`kube-system`, `d8-*`);
...
- Операции, связанные с ресурсами модуля `Deckhouse Virtualization Platform`;
- Действия в пространстве имён `d8-virtualization`
- Операции с объектами `moduleconfigs`;
- Запросы от пользователей, не прошедших аутентификацию (фиксируются только события уровня `Metadata`).

**События, подлежащие регистрации согласно требованиям ФСТЭК к средствам контейнеризации:**
- Создание, изменение, удаление объектов `Pod`;
- Использование контейнерных тегов, отличных от `@sha256`;
- Создание и удаление объектов `ServiceAccount`, в т.ч. в системных пространствах имён;
- Создание, изменение и удаление объектов `Role` и `ClusterRole`;
- Создание, изменение и удаление объектов `ClusterRoleBinding`
- Использование команд `attach` и `exec` к `Pod`, а также добавление ephemeral-контейнеров (запрос и модификация объектов `pods/exec`, `pods/attach`, `pods/ephemeralcontainers`);

**События, которые принудительно не фиксируются политикой аудита:** (связи с повышенной частотой обновления объектов)
- Операции над объектами `Endpoints`, `Endpointslices` и `Events;
- Операции над объектами `Leases` (выбор лидера raft в компонентах платформы);
...
- Операции `PATCH` для объектов `VerticalPodAutoscaler` от сервисного аккаунта `d8-vertical-pod-autoscaler-recommender`;
- Действия над объектами `UpmeterHookProbes`;
