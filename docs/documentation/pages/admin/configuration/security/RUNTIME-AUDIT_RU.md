...
Подробности об архитектуре реализации аудита можно найти в разделе Архитектура — Безопасность(#TODO).
...
Все доступные параметры аудита безопасности доступны в разделе Справка(#TODO).
...
- **встроенные правила**, включая:
  ...
  - правила, удовлетворяющие требованиям приказа ФСТЭК России №118 от 4 июля 2022 г.
    ...
    (`fstec`, в формате кастомного ресурса FalcoAuditRules(#TODO));

  Чтобы настроить список встроенных правил,
  используйте параметр `settings.builtInRulesList`(#TODO) модуля `runtime-audit-engine`.

- **пользовательские правила**, которые задаются через кастомный ресурс FalcoAuditRules(#TODO).

Подробности о работе правил аудита безопасности можно найти в разделе Архитектура — Безопасность(#TODO).

### Добавление пользовательского правила

Чтобы добавить правило, создайте ресурс FalcoAuditRules(#TODO) с необходимыми условиями.
...
сторонние правила из интернета необходимо сконвертировать в ресурс FalcoAuditRules(#TODO) перед применением.
...
по которым можно настроить оповещения через ресурс CustomPrometheusRules(#TODO).
Это позволяет:
...
Для сбора и отправки событий в хранилище логов создайте ресурс ClusterLoggingConfig(#TODO), следуя примеру:
...
Для включения установите параметр `spec.settings.debugLogging`(#TODO) в `true`, следуя примеру
