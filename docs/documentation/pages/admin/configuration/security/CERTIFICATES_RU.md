...
description: "Управление TLS-сертификатами в Deckhouse Platform с интеграцией Let's Encrypt, HashiCorp Vault и Venafi. Автоматическое обновление, мониторинг и управление жизненным циклом сертификатов."
lang: ru
...
Deckhouse Platform (DP) предоставляет встроенные средства управления TLS-сертификатами в кластере
и поддерживает:
...
На этой странице описаны доступные в DP возможности управления сертификатами,
...
DP экспортирует метрики в Prometheus, что позволяет отслеживать:
...
В DP предопределены несколько ролей для доступа к ресурсам:

| Роль       | Права доступа |
| ---------- | ------------- |
| `User`     | Просмотр ресурсов Certificate и Issuer в доступных пространствах имён, а также глобальных ресурсов ClusterIssuer. |
| `Editor`   | Управление ресурсами Certificate в доступных пространствах имён. |
| `Admin`    | Управление ресурсами Issuer в доступных пространствах имён. |
| `ClusterEditor` | Управление ресурсами Certificate и Issuer во всех пространствах имён, а также ресурсами ClusterIssuer. |
| `SuperAdmin` | Управление внутренними служебными объектами. |

## Работа с издателями сертификатов

В DP по умолчанию поддерживаются следующие издатели сертификатов (ClusterIssuer):
...
для всех компонентов DP или конкретного компонента.

Например, чтобы использовать ClusterIssuer для получения сертификатов для всех компонентов DP,
