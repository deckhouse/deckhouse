...
1. Скопируйте нужные файлы из директории с резервной копией обратно в исходное расположение. Например, чтобы восстановить манифест `kube-apiserver`:
   ...
   Восстановления манифеста статического пода достаточно, чтобы kubelet перезапустил под — вручную перезапускать ничего не нужно. Файлы PKI и kubeconfig (`admin.conf`, сертификаты и т. д.) восстанавливаются так же, с сохранением исходного относительного пути внутри `/etc/kubernetes`.
