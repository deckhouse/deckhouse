...
### Восстановление манифестов компонентов control plane из резервной копии CPM

Перед применением любого изменения к компоненту control plane (`etcd`, `kube-apiserver`, `kube-controller-manager` или `kube-scheduler`) модуль `control-plane-manager` (CPM) автоматически создаёт резервную копию файлов, которые он собирается перезаписать на узле. Резервные копии сохраняются отдельно для каждого компонента и каждой операции по пути:

```text
/etc/kubernetes/deckhouse/backup/<Компонент>/<имя-операции>/
```

CPM хранит 7 последних операций для каждого компонента, более старые резервные копии удаляются автоматически. Содержимое директории с резервной копией зависит от компонента. Например, операция для `kube-apiserver` сохраняет манифест статического пода, TLS-сертификаты и `admin.conf`:
...
Рядом с резервной копией сохраняется unified diff внесённого CPM изменения — в директории `/etc/kubernetes/deckhouse/diffs/<Компонент>/<имя-операции>/` (также хранятся только 7 последних операций):
...
Эти резервные копии можно использовать, чтобы вернуть компонент control plane к предыдущему состоянию — например, если изменение, применённое CPM (в том числе из-за правки `ModuleConfig`), привело к неработоспособности компонента.

Чтобы восстановить файл компонента control plane из резервной копии CPM:

1. На нужном master-узле посмотрите список доступных резервных копий для компонента, отсортированных по времени, и выберите операцию, из которой нужно восстановить файлы:

   ```shell
   ls -lt /etc/kubernetes/deckhouse/backup/<КОМПОНЕНТ>/
   ```

   При необходимости посмотрите, что именно изменилось в этой операции, изучив соответствующий diff:

   ```shell
   cat /etc/kubernetes/deckhouse/diffs/<КОМПОНЕНТ>/<ИМЯ_ОПЕРАЦИИ>/manifests/*.diff
   ```

1. Включите maintenance-режим для ресурса `ControlPlaneNode` этого узла. Это остановит создание CPM новых операций на узле, пока вы вручную восстанавливаете файлы (уже выполняющаяся операция при этом не прерывается):

   ```shell
   d8 k label cpn <ИМЯ_УЗЛА> control-plane-manager.deckhouse.io/maintenance=""
   ```

1. Скопируйте нужные файлы из директории с резервной копией обратно в исходное расположение, сохраняя исходный относительный путь внутри `/etc/kubernetes`. Например, чтобы восстановить манифест `kube-apiserver`:
   ...
   Восстановления манифеста статического пода достаточно, чтобы kubelet автоматически перезапустил под — вручную перезапускать ничего не нужно.

1. Если вы восстановили сертификаты, ключи или kubeconfig (`admin.conf` и другие файлы из `pki/`), а не сам манифест, перезапустите под соответствующего компонента вручную. kubelet реагирует только на изменения самого файла манифеста и не отслеживает файлы, которые под монтирует, поэтому уже запущенный контейнер продолжит использовать старый, ранее загруженный сертификат, пока его не перезапустят. Достаточно удалить под — kubelet пересоздаст его из (не изменившегося) статического манифеста:
   ...
   Если сам API-сервер недоступен (например, вы восстанавливаете сертификат единственного control-plane узла и именно из-за него API недоступен), перезапустите контейнер напрямую на узле:

   ```shell
   crictl stopp $(crictl pods --name=<ИМЯ_КОМПОНЕНТА> -q)
   ```

1. Убедитесь, что компонент поднялся и работает корректно:

   ```shell
   d8 k -n kube-system wait pod --timeout=10m --for=condition=ContainersReady -l app=d8-control-plane-manager --field-selector spec.nodeName=<ИМЯ_УЗЛА>
   ```
...
{% alert level="warning" %}
Восстановление файлов из резервной копии откатывает только их состояние на диске. Если изменение, которое вы откатываете, по-прежнему требуется согласно текущей конфигурации кластера (например, `ModuleConfig`), CPM обнаружит расхождение и создаст новую операцию, которая повторно применит это изменение сразу после отключения maintenance-режима. Сначала исправьте исходную конфигурацию, если не хотите, чтобы изменение было применено повторно.
{% endalert %}
