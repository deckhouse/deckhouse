...
1. Подготовьте утилиту `etcdctl`. Найдите и скопируйте исполняемый файл на узле:
   ...
   Убедитесь, что результат команды `etcdctl version` отображается корректно (без ошибок).
...
Чтобы автоматически выгрузить объекты кластера из резервной копии etcd, воспользуйтесь представленным ниже скриптом. Перед его запуском обратите внимание на следующие настройки:  
