---
title: "Мониторинг в Deckhouse Platform"
permalink: ru/admin/configuration/monitoring/
description: "Настройка комплексного мониторинга для Deckhouse Platform с Prometheus и Grafana. Сбор метрик, алертинг, дашборды и мониторинг SLA для здоровья кластера."
lang: ru
extractedLinksMax: 0
relatedLinks:
...
Deckhouse Platform (DP) предоставляет решение для мониторинга Kubernetes на базе **Prometheus** и **Grafana**.
...
В DP реализовано несколько видов мониторинга:
...
В DP предусмотрена [система оповещений (алертинг)](#алерты), поддерживающая отправку уведомлений о событиях, в том числе [во внешние системы](#отправка-алертов-во-внешние-системы).
...
По умолчанию DP собирает подробную статистику со всех Ingress-ресурсов в кластере. Этот процесс может приводить к высокой нагрузке системы мониторинга.
...
Мониторинг control plane осуществляется модулем [`control-plane-manager`](/modules/control-plane-manager/). Он организует безопасный сбор метрик и предоставляет базовый набор правил мониторинга следующих компонентов кластера:
...
DP безопасно собирает метрики мониторинга и настраивает правила.

Возможности мониторинга DP:

- мониторинг текущей версии container runtime (containerd) на узле и ее соответствия версиям, разрешенным для использования в DP;
...
В DP поддерживается режим расширенного мониторинга с помощью [модуля `extended-monitoring`](/modules/extended-monitoring/), который позволяет настроить:
...
Мониторинг в составе DP включает уведомления о событиях. Стандартная поставка включает набор базовых предупреждений, охватывающих состояние кластера и его компоненты. Также остается возможность добавлять кастомные алерты.
...
DP поддерживает отправку алертов с помощью Alertmanager:
...
Примеры интеграции мониторинга DP с внешними системами доступны в разделе [Настройка интеграций](alerts-integrations.html).
