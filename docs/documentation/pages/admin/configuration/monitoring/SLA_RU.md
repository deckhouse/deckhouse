...
description: "Настройка мониторинга SLA в Deckhouse Platform. Отслеживание доступности кластера, мониторинг соответствия SLA и сбор статистики времени работы компонентов платформы."
lang: ru
---

DP может собирать статистику о доступности компонентов кластера и компонентов самого DP. Эти данные позволяют оценить степень выполнения SLA, а также получить информацию о доступности в веб-интерфейсе.
...
[Модуль `upmeter`](/modules/upmeter/) настраивается с помощью ModuleConfig `upmeter`:
...
DP предоставляет два веб-интерфейса для оценки доступности:
- Страница статуса.
...
Пример конфигурации [UpmeterRemoteWrite](/modules/upmeter/cr.html#upmeterremotewrite) для экспорта метрик статуса по протоколу [Prometheus Remote Write](https://docs.sysdig.com/en/docs/installation/prometheus-remote-write/):
