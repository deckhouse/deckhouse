...
description: "Настройка сбора и доставки логов в Deckhouse Platform. Централизованное логирование из подов и узлов во внутренние или внешние системы хранения с фильтрацией и маршрутизацией."
lang: ru
---

В Deckhouse Platform (DP) предусмотрен сбор и доставка логов из узлов и подов кластера во внутреннюю или внешние системы хранения.
...
Администраторам DP доступна настройка сбора и отправки логов с помощью трёх кастомных ресурсов:
...
На основе этих ресурсов формируется *pipeline*, который используется в DP для чтения логов
...
Ниже приведён вариант базовой конфигурации DP,
...
Вы можете настроить DP на работу с внешними системами хранения и анализа логов,
...
1. Настройте DP, добавив ресурс [ClusterLogDestination](/modules/log-shipper/cr.html#clusterlogdestination) для отправки логов в Splunk:
...
DP может отправлять логи в формате CEF через использование `codec: CEF`,
...
В DP предусмотрены фильтры, позволяющие исключить лишние сообщения для оптимизации процесса сбора логов:
...
Пример конфигурации для сбора системных логов DP, находящихся в файле `/var/log/syslog`.
...
[Модуль `log-shipper`](/modules/log-shipper/) в DP собирается на основе Vector с ограниченным набором [cargo-функций](https://doc.rust-lang.org/cargo/reference/features.html),
...
В DP предусмотрен лейбл `log-shipper.deckhouse.io/exclude=true` для исключения определенных подов и пространств имён.
