...
| Колонка | Описание |
|---|---|
| `Phase` | Текущее состояние репозитория: `Active` после успешного сканирования, `Error`, если последнее сканирование завершилось ошибкой |
...
| Поле | Описание |
|---|---|
| `status.phase` | Текущая фаза репозитория: `Active` после успешного сканирования, `Error`, если последнее сканирование завершилось ошибкой |
| `status.message` | Причина ошибки последнего сканирования; пустое после успешного |
...
Если сканирование завершилось ошибкой, поле `reason` условия называет причину:

| Причина | Значение |
|---|---|
| `AccessDenied` | Хранилище образов отклонило учётные данные из `spec.registry` |
| `RepositoryNotFound` | В хранилище нет репозитория по пути `spec.registry.repo` |
| `RegistryUnavailable` | Хранилище образов недоступно |
| `ScanFailed` | Любая другая ошибка; подробности в сообщении |
