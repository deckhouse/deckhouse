...
{% alert level="warning" %}
На узлах с Ubuntu в AWS модуль ядра erofs, необходимый для containerd v2, поставляется отдельным пакетом `linux-modules-extra`. Deckhouse устанавливает его автоматически, но для части ядер, преимущественно устаревших, этот пакет больше не публикуется в APT-репозитории. В этом случае установка пропускается с предупреждением в логах узла, узел получает лейбл `node.deckhouse.io/containerd-v2-unsupported`, и по нему срабатывает алерт `D8NodeContainerdV2NotSupported`. Проблема подтверждена, в частности, на ядрах `5.15.0-1028-aws`, `6.8.0-1024-aws` и `6.8.0-1029-aws`. Чтобы мигрировать такие узлы, переключите группу узлов на более свежий AMI или ядро, в котором есть erofs.

Перед миграцией сверьтесь с алертами и списком узлов с этим лейблом (команды приведены ниже), чтобы понять, затронуты ли ваши узлы.
{% endalert %}
