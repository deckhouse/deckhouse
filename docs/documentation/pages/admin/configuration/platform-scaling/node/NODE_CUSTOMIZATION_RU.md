...
{% alert level="warning" %}
Если DKP развернут на одном узле с GPU и используется стратегия `MIG`, после каждой перезагрузки узла необходимо вручную повторно применить MIG-конфигурацию.
Это связано со спецификой инсталляции кластера из одного узла: после перезагрузки MIG-конфигурация не приводится в рабочее состояние автоматически, поэтому требуется выполнить несколько дополнительных действий.
{% endalert %}
