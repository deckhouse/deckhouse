...
- `priority` — целочисленный приоритет группы. При масштабировании `Cluster Autoscaler` сначала выбирает группы с большим значением `priority`. Используется для задания порядка масштабирования между несколькими группами узлов. Скоростью переключения на заказ узлов из групп с меньшим приоритетом можно управлять (подробнее — в разделе [«FAQ»](../../../../faq.html#что-делать-если-переключение-на-заказ-узлов-в-менее-приоритетных)).
