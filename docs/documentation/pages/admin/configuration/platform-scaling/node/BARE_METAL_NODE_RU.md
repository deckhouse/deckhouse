...
1. Создать объект [SSHCredentials](/modules/node-manager/cr.html#sshcredentials) с доступом к серверу. DKP использует объект SSHCredentials для подключения к серверам по SSH. В нём указывается:
   ...
   - (опционально) пароль для `sudo`, если требуется.
     ...
     Подробнее о защите конфиденциальных данных SSHCredentials (маскирование `<omitted>`, права доступа, редактирование и шифрование в etcd) — в [FAQ модуля `node-manager`](/modules/node-manager/faq.html#почему-ssh-ключ-и-sudo-пароль-в-sshcredentials-отображаются-как-omitted).
