...
В DP возможно автоматическое добавление физических (bare-metal) серверов в кластер без ручного запуска установочного скрипта на каждом узле. Для этого необходимо:
...
1. Создать объект [SSHCredentials](/modules/node-manager/cr.html#sshcredentials) с доступом к серверу. DP использует объект SSHCredentials для подключения к серверам по SSH. В нём указывается:
...
1. Создать [NodeGroup](/modules/node-manager/cr.html#nodegroup) с описанием, как DP будет использовать эти серверы:
...
После создания группы узлов появится скрипт для добавления серверов в группу. DP будет ожидать появления требуемого числа объектов StaticInstance, подходящих по лейблам. Как только такой объект появится, DP использует указанный IP и параметры для подключения по SSH, выполнит скрипт `bootstrap.sh` и добавит сервер в группу.
