...
1. **Миграция multi-master (3 master-узла) → 2 master-узла и 1 arbiter-узел в облачном кластере**:
   ...
   - В настройках облачного провайдера для параметра `masterNodeGroup.replicas` укажите `2` и создайте Создайте NodeGroup для arbiter-узла. Аналогично добавлению/удалению узлов, но чаще всего выполняется с помощью команды `dhctl converge` или вручную через облачные инструменты.

   [Подробнее](../../high-reliability-and-availability/enable.html#настройка-в-облачном-кластере).
...
1. **Миграция multi-master (3 master-узла) → 2 master-узла и 1 arbiter-узел в статическом кластере**:
   ...
   - для **bare-metal кластеров**:
     - чтобы корректно исключить узел из etcd:
       - выполните команду `d8 k delete node <имя-узла>`;
       - выключите соответствующую виртуальную машину или сервер.

   [Подробнее](../../high-reliability-and-availability/enable.html#настройка-в-статическом-кластере).
