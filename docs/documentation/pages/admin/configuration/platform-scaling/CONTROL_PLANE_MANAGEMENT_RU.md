...
1. Через [веб-интерфейс Deckhouse](https://deckhouse.ru/products/kubernetes-platform/modules/console/stable/):
   ...
   - Найдите модуль `control-plane-managerr` и нажмите на него;
...
1. Определите узел, который будет выполнять роль master.
1. Установите лейбл:
   ...
   Для облачных кластеров необходимо обновить `provider-cluster-configuration` соответствующим образом, а затем выполнить `dhctl converge ...`.

1. Убедитесь, что в кластере в итоге будет нечётное количество master-узлов. Это необходимо для обеспечения кворума etcd.
1. Проверьте запуск компонентов control plane:
   - Убедитесь, что на новом узле запущены поды kube-apiserver, etcd и другие (через `kubectl get pods -n kube-system`).
   - Убедитесь, что сам узел находится в статусе Ready (через `kubectl get nodes`).
...
- Если число узлов уменьшается с двух до одного, etcd может перейти в статус `readonly`. В этом случае требуется запуск с параметром `--force-new-cluster`.
