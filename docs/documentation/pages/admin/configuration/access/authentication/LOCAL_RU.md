...
Операции сброса пароля, сброса 2FA и блокировки выполняются через ресурс [UserOperation](/modules/user-authn/cr.html#useroperation). В поле `initiatorType` указывается, кто инициировал операцию: администратор (`admin`), система (`system`) или сам пользователь (`self`).
