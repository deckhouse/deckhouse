...
Токен ServiceAccount используется напрямую для аутентификации в API. Внешний IdP не требуется. Для ServiceAccount можно создавать [долгоживущие токены](#создание-serviceaccount-и-долгоживущего-токена) (без TTL, с возможностью отзыва или ротации) и [короткоживущие токены](#короткоживущие-токены).
