...
Токен ServiceAccount используется напрямую для аутентификации в API. Внешний IdP не требуется. Для ServiceAccount можно создавать [долгоживущие токены](#создание-serviceaccount-и-долгоживущего-токена) (без TTL, с возможностью отзыва или ротации) и [короткоживущие токены](#короткоживущие-токены-tokenrequest).
...
Для создания ServiceAccount и токена используйте команду:
...
Пример генерации kubeconfig для созданного ранее ServiceAccount:
...
Пример настройки kubeconfig для Basic Auth:
...
Пример использования Basic Auth в GitLab CI:
...
{% alert level="warning" %}
Token exchange не работает с коннекторами `type: GitLab` или `type: GitHub`. Настройте GitLab/GitHub как OIDC-провайдеры.
{% endalert %}

### Создание DexClient

Для создания DexClient используйте команду:
...
{% alert level="warning" %}
Поскольку эта аннотация выдаёт доступ к Kubernetes API на уровне кластера, поставить её на объект может только субъект, которому разрешено обновлять ModuleConfig `user-authn`, — например обладатель роли `d8:manage:permission:module:user-authn:edit`. Роли уровня пространства имён, позволяющей создать DexClient, недостаточно: admission отклонит запрос с сообщением, содержащим имя аннотации.

Проверяется само появление аннотации, независимо от её значения, а также изменение значения на более разрешительное у объекта, который её уже содержит. Значение `"false"` тоже запрещено вводить, поскольку в более старых выпусках грант выдаётся по одному наличию ключа. Объект, который аннотацию уже содержит, остаётся доступным для редактирования владельцу, включая сужение значения. Удаление и повторное создание объекта считается повторной установкой аннотации, поэтому GitOps-контроллеру, который пересоздаёт объекты вместо их обновления, требуется право `update` на ресурс `moduleconfigs` с именем `user-authn`.
{% endalert %}
...
Для получения URL Dex и API используйте команды:
...
Пример запроса токена для GitLab CI:
...
**Флаг `--token` может не работать** — kubeconfig с `auth-provider`, `exec` или `tokenFile` переопределяет токен из флага. Для отладки используйте `-v=8`:

```shell
d8 k --server="https://${API_HOST}" --token="${DEX_TOKEN}" auth whoami -v=8
```

В выводе будет видно какой Authorization заголовок отправляется. Для надёжной работы с токеном используйте минимальный kubeconfig только с `server` и `token`, без `auth-provider`/`exec`.
...
- Аннотация `dexclient.deckhouse.io/allow-access-to-kubernetes` на DexClient. Если её нет, проверьте, не отклонил ли admission запрос, который должен был её добавить, и не пересоздал ли GitOps-контроллер объект без неё: добавить аннотацию может только субъект, которому разрешено обновлять ModuleConfig `user-authn`.
