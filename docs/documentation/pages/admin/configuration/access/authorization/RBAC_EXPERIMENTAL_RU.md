...
Системная или подсистемная роль сама по себе не даёт права раздавать доступ другим людям: создание User на email, на котором уже есть грант, запись или удаление ClusterAuthorizationRule, либо сброс такой идентичности через UserOperation — это раздача ролей, и запрос пройдёт, только если у запрашивающего уже есть покрывающие права или ему явно разрешено назначать эти роли.

Подключение DexProvider проверяется так же, но относительно идентичностей, которые провайдер способен утвердить: [`spec.allowedIdentities`](/modules/user-authn/cr.html#dexprovider-v1-spec-allowedidentities) (email, домены email, группы) и собственные фильтры групп провайдера ограничивают это множество, и учитываются только роли, уже выданные внутри него. Провайдер без ограничителя по email способен утвердить любой адрес, включая те, у которых есть `SuperAdmin`, поэтому его запись требует `SuperAdmin`. Подробности и список изменений, проходящих без проверки, — в [документации модуля user-authn](/modules/user-authn/usage.html).
{% endalert %}
