...
description: "Узнайте, как настроить платформу Deckhouse Platform с помощью глобальных настроек, конфигураций модулей и пользовательских ресурсов. Руководство по настройке DP."
lang: ru
---
...
- **[Кастомных ресурсов](../../reference/api/cr.html).** Некоторые модули настраиваются с помощью дополнительных кастомных ресурсов.
...
{% alert level="warning" %}
Для применения изменений конфигурации узлов необходимо выполнить команду `dhctl converge`, запустив инсталлятор DP. Эта команда синхронизирует состояние узлов с указанным в конфигурации.
{% endalert %}
...
После сохранения изменений DP автоматически приведёт кластер в соответствие с новой конфигурацией. В зависимости от размеров кластера это может занять некоторое время.
...
Используйте утилиту `dhctl` из контейнера инсталлятора DP с флагом `--yes-i-am-sane-and-i-understand-what-i-am-doing`.
...
1. Получите текущую версию и редакцию DP из вашего кластера. Для этого **на локальной машине** запустите контейнер установщика DP соответствующей редакции и версии:
...
1. Запустите контейнер инсталлятора DP (при необходимости измените адрес registry):
...
DP управляется с помощью глобальных настроек, настроек модулей и различных кастомных ресурсов.
...
{% alert level="warning" %}
Таблица ниже описывает наборы модулей только для встроенных модулей Deckhouse Platform.  
...
Deckhouse Platform с набором модулей `Minimal` без включения базовых модулей сможет работать только в уже развернутом кластере.
{% endalert %}

Для установки DP с набором модулей `Minimal` включите как минимум следующие модули, указав их в конфигурационном файле установщика:
