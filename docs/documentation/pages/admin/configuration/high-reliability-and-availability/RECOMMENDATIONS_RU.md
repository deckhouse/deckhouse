...
{% alert level="info" %}
Доступно в редакции EE, Ultimate.
{% endalert %}

В DP поддерживается механизм включения задержки выключения или перезагрузки узла (далее просто «выключение узла»), если на нем запущены stateful-приложения или виртуальные машины (ВМ). Механизм реализуется сервисом `d8-shutdown-inhibitor` модуля [`node-manager`](/modules/node-manager/).

Задержка срабатывает, если на узле запущены поды с лейблом `pod.deckhouse.io/inhibit-node-shutdown`. DP откладывает выключение узла, чтобы пользователь смог перенести критичные поды на другой узел или завершить их работу штатным образом. Это полезно, например, для stateful-нагрузок и других случаев, когда необходимо избежать потери данных. Подробная информация о принципах работы и особенностях механизма представлена в [документации модуля `node-manager`](/modules/node-manager/#задержка-перезагрузки-или-выключения-узла-пока-на-нём-работают-критичные-поды).

### Включение задержки выключения узла
...
Чтобы включить механизм задержки выключения узла, добавьте на под лейбл `pod.deckhouse.io/inhibit-node-shutdown` (для Deployment укажите лейбл в шаблоне пода).
