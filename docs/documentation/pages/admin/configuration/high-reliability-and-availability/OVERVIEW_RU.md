...
Deckhouse Platform (DP) обеспечивает высокую доступность (High Availability, HA) и отказоустойчивость за счёт встроенных механизмов и модулей.
...
Виртуальные машины тоже переживают отказы и перегрузку узлов. Перебалансировка переносит их живой миграцией с загруженных узлов, а механизм ColdStandby возвращает машины в работу после отказа узла. Подробнее читайте в разделе [Отказоустойчивость и балансировка виртуальных машин](./vm-reliability.html).
