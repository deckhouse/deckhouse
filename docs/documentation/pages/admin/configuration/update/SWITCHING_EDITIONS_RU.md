...
1. Определите список внешних модулей, запущенных из `moduleSource/deckhouse`, которые не поддерживаются в DKP новой редакции. Для этого выполните следующие шаги:
   ...
   1. Отключите модули которые не были найдены в новой редакции, если это допустимо. Иначе, **прервите процесс переключения.**

{% endcapture %}

{% capture take_care_deckhuse_imagepullbackoff %}
1. Убедитесь, что поды `deckhouse` запустились после смены хранилища образов:
   ...
   1. Если под находится в ImagePullBackoff с ошибкой скачивания сайдкар-контейнеров, укажите актуальные образы:

      Получить актуальные дайджесты образов сайдкаров:
...
Каждый узел один раз перезагрузится с очисткой кеша образов containerd. После перезагрузки узел заново скачает все образы из хранилища образов контейнеров.
...
conditions:
  - lastTransitionTime: "2026-05-05T13:53:23Z"
    message: |-
      Mode: Default
      <REGISTRY_HOST>: all 1 items are checked
    reason: Ready
    status: "True"
    type: RegistryContainsRequiredImages
  - lastTransitionTime: "2026-05-05T13:54:49Z"
