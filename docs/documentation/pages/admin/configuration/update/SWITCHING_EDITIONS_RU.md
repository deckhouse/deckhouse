...
1. Выполните команду, для задания данных аутентификации в хранилище образов::
   ...
   {{ ngc_auth_registry | regex_replace: "^", "   " }}

   {{ bashible_sync_wait | regex_replace: "^", "   " }}

1. Переключите registry:

   {{ change_registry_helper_commercial | regex_replace: "^", "   " }}
...
1. Выполните очистку:
   ...
   {{ ngc_cleanup_registry | regex_replace: "^", "   " }}
{% endtab %}
{% tab "DKP SE" %}
1. Выполните команду, для задания данных аутентификации в хранилище образов:
