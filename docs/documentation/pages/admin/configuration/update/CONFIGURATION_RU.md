...
## Обновление виртуализации

Если в кластере включена виртуализация, её компоненты делятся на две категории, которые обновляются по-разному:

- компоненты управления ресурсами виртуализации (управляющий слой);
- компоненты запуска виртуальных машин («прошивка»).

Обновление управляющего слоя не влияет на работу уже запущенных виртуальных машин, но на время перезапуска компонента разрывает открытые соединения по VNC и через серийную консоль.

Обновление «прошивки» может потребовать миграции виртуальных машин на новую версию. DKP мигрирует машину один раз, и если миграция не удалась, владельцу машины придётся перенести или перезагрузить её самостоятельно. Выбрать подходящее время для такого обновления помогают [окна обновлений](#окна-обновлений) и [ручной режим](#ручной-режим-manual).
