...
description: "Настройка параметров обновлений Deckhouse Platform включая каналы релизов, режимы обновлений и политики обновлений. Автоматическое и ручное управление обновлениями."
lang: ru
---

Deckhouse Platform (DP) поддерживает гибкий механизм обновлений. Вы можете выбрать [канал обновлений](../../../architecture/updating.html#каналы-обновлений) и настроить режим установки новых версий. Каналы обновлений помогают сбалансировать скорость получения новых функций и стабильность.
...
DP поддерживает два режима обновления, которые [определяют](/modules/deckhouse/configuration.html#parameters-update-mode) порядок перехода на новую версию:
...
Чтобы DP обновлялся в пределах текущей минорной версии (устанавливались патч-версии), используйте режим `AutoPatch`.

Например, при установленной версии `v1.70.1` DP сможет автоматически обновиться до `v1.70.2`,
...
указав необходимую версию DP вместо `<DECKHOUSE-VERSION>`:
...
В режиме `Auto` DP автоматически применяет и патч-версии, и минорные версии
...
В режиме `Manual` DP получает информацию о новых версиях в кластер,
...
1. DP раз в минуту проверяет канал обновлений на наличие нового релиза.
1. При появлении нового релиза DP скачивает его в кластер и создает [кастомный ресурс DeckhouseRelease](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#deckhouserelease).
1. После появления кастомного ресурса DeckhouseRelease в кластере
   DP выполняет обновление на соответствующую версию согласно установленным параметрам обновления
...
{% alert level="warning" %}
Начиная с версии DP 1.70, патч-версии обновлений (например, обновление на версию `1.70.2` при установленной версии `1.70.1`) устанавливаются с учетом окон обновлений. До версии DP 1.70 патч-версии обновлений устанавливаются без учета режима и окон обновлений.
{% endalert %}
...
- Установить режим ручного подтверждения обновлений.
  ...
  В этом случае DP будет получать обновления в кластер,
  ...
  Чтобы подтвердить обновление, выполните следующую команду, указав необходимую версию DP вместо `<DECKHOUSE-VERSION>`:
...
- Установить режим автоматического обновления для патч-версий.
  ...
  Например, если у вас установлена версия DP `v1.70.1`,
...
- Установить тег необходимой версии DP для Deployment `deckhouse`
...
В этом случае DP останется на указанной версии
...
Пример установки версии `v1.66.3` для DP EE и удаления параметра `releaseChannel` из конфигурации модуля `deckhouse`:
...
Ручное подтверждение обновления версии DP предусмотрено в следующих случаях:

- Включен режим подтверждения обновлений DP.
  ...
  (подтверждение как патч-версии, так и минорной версии DP) или `AutoPatch` (подтверждение минорной версии DP).

  Для подтверждения обновления выполните следующую команду, указав необходимую версию DP вместо `<DECKHOUSE-VERSION>`:
...
DP позволяет задавать *окна обновлений* — временные интервалы,
...
- Если окна обновлений настроены, DP будет устанавливать новые версии только в указанные временные интервалы.
...
Управлять окнами обновлений DP можно следующими способами:
