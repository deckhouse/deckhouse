...
Чтобы применить обновление DKP немедленно, установите в соответствующем ресурсе DeckhouseRelease (#TODO) аннотацию `release.deckhouse.io/apply-now: "true"`.

{% alert level="warning" %}
В этом случае будут проигнорированы окна обновления, настройки canary-release (#TODO) и режим ручного обновления кластера[LINK]. Обновление применится сразу после установки аннотации.
{% endalert %}
...
- под `deckhouse` находится не в статусе `Ready`. Если под долго не переходит в статус `Ready`, это может говорить о наличии проблем в работе Deckhouse. Необходима диагностика.

## Как понять, что обновление прошло успешно?

Если алерт `DeckhouseUpdating` погас, это означает, что обновление завершено.

Также вы можете проверить состояние релизов Deckhouse в кластере, используя следующую команду:
...
- Если под находится в статусе `Running`, но в колонке `READY` указано `0/1`, это означает, что обновление еще не завершено. Если так продолжается более 20–30 минут, это может говорить о наличии проблем в работе Deckhouse. Необходима диагностика.
- Если под не находится в статусе `Running`, это может говорить о наличии проблем в работе Deckhouse. Необходима диагностика.
...
- Соберите отладочную информацию(#TODO) и свяжитесь с технической поддержкой DKP.
...
- Загорается алерт `DeckhouseReleaseIsWaitingManualApproval`, если кластер использует ручной режим обновлений [LINK].
- Появляется новый custom resource DeckhouseRelease(#TODO). Используйте команду `sudo -i d8 k get deckhousereleases`, чтобы посмотреть список релизов. Если `DeckhouseRelease` новой версии находится в состоянии `Pending`, указанная версия еще не установлена. Возможные причины, при которых `DeckhouseRelease` может находиться в `Pending`:
  - Установлен ручной режим обновлений [LINK].
  - Установлен автоматический режим обновлений и настроены окна обновлений[LINK], интервал которых еще не наступил.
  ...
  - Установлен параметр update.notification.minimalNotificationTime[LINK] и заданный в нем интервал еще не прошел.
...
- Настроить ручной режим обновлений[LINK]. В этом случае при появлении новой версии на канале обновлений загорится алерт `DeckhouseReleaseIsWaitingManualApproval`, и в кластере появится новый custom resource DeckhouseRelease(#TODO).
- Настроить автоматический режим обновлений[LINK] и указать минимальное время в параметре minimalNotificationTime(#TODO), на которое будет отложено обновление. В этом случае при появлении новой версии на канале обновлений в кластере появится новый custom resource DeckhouseRelease(#TODO). Если указать URL в параметре update.notification.webhook[LINK], через указанный вебхук будет отправлено уведомление об предстоящем обновлении.

## Как узнать версию Deckhouse на каждом из каналов обновлений?

Актуальная информация о версиях Deckhouse на разных каналах обновлений доступна на сайте [releases.deckhouse.ru](https://releases.deckhouse.ru).

## Что делать, если Deckhouse не получает обновления из настроенного канала?

- Проверьте, что настроен нужный канал обновлений[LINKtoПроверкатекущегоканалаобновлений].
