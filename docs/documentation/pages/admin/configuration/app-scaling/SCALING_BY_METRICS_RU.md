...
description: "Настройка масштабирования на основе метрик в Deckhouse Platform. Масштабирование по пользовательским метрикам, интеграция с Prometheus и настройка HPA для динамической корректировки ресурсов."
lang: ru
---
...
Для упрощения настройки масштабирования по метрикам, DP предлагает использовать специальные ресурсы (`Cluster*Metric` и `*Metric`). Также, можно настроить масштабирование [по собственным правилам расчета метрик](#настройка-метрик-через-ресурсы-модуля-observability).
...
- `sum(...) by (<<.GroupBy>>)` — суммирует результат по некоторому набору лейблов (например, по имени Ingress, пространству имён и т. д.);
...
Таким образом, этот PromQL-запрос даёт «число текущих запросов в секунду» по указанным лейблам (Ingress, namespace и т. д.).
...
В Deckhouse Platform можно настраивать параметры автомасштабирования узлов через [веб-интерфейс Deckhouse](/modules/console/). Это позволяет динамически изменять количество узлов в зависимости от нагрузки.
