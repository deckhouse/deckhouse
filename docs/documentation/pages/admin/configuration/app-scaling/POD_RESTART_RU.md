...
Указывает, что нужно отслеживать только те Secret или ConfigMap, которые имеют аннотацию `pod-reloader.deckhouse.io/match: "true"`. Это удобно, если в поде используется множество ресурсов, но перезапуск требуется только при изменении определённых.
