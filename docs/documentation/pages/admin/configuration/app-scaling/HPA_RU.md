...
1. Кастомные метрики (Pods, Object):
   ...
   - Требуют регистрации метрик в Custom Metrics API (см. [Масштабирование по метрикам](todo)).
...
HPA — это базовый функционал Kubernetes, встроенный в `kube-controller-manager`, и не требует отдельного включения в Deckhouse.

Однако для работы с кастомными и внешними метриками нужен модуль `prometheus-metrics-adapter`. Как включить модуль [см. в документации](todo).
