...
{% alert level="danger" %}
Запрещено использовать классы приоритета `system-node-critical`, `system-cluster-critical`, `cluster-medium`, `cluster-low`, так как они зарезервированы для ключевых компонентов кластера.
{% endalert %}
...
### Включение или отключение приоритетов

Для включения приоритетов необходимо включить модуль priority-class`. Это можно сделать следующими способами:

1. Через ресурс ModuleConfig (например, ModuleConfig/priority-class). Установите параметр `spec.enabled` в значение `true` или `false`:

   Для **включения** установите параметр `spec.enabled` в значение `true` в файле `ModuleConfig`/`priority-class` (создав этот ресурс при необходимости).
   ...
   Для **выключения** установите параметр `spec.enabled` в значение `false` в файле `ModuleConfig`/`priority-class`.
...
1. Через команду deckhouse-controller (в поде `d8-system/deckhouse`):
   ...
   Для **включения** воспользуйтесь командой deckhouse-controller в поде `d8-system/deckhouse` (требуется kubectl, настроенный на работу с кластером):

   ```console
   kubectl -ti -n d8-system exec svc/deckhouse-leader -c deckhouse -- deckhouse-controller module enable priority-class
   ```

   Для **выключения** воспользуйтесь командой:

   ```console
   kubectl -ti -n d8-system exec svc/deckhouse-leader -c deckhouse -- deckhouse-controller module disable priority-class
   ```

1. 1. Через веб-интерфейс [Консоль (UI)](https://deckhouse.ru/products/kubernetes-platform/modules/console/stable/):

   - Перейдите в раздел «Deckhouse - «Модули»;
   - Найдите модуль `priority-class` и нажмите на него;
