...
permalink: ru/admin/configuration/delivery/argocd/setup.html
...
Перед созданием экземпляров Argo CD включите модуль [operator-argo](/modules/operator-argo/):
...
Дождитесь, пока модуль перейдёт в состояние `Ready`:
...
После включения модуля `operator-argo` в кластере DKP станут доступны следующие определения кастомных ресурсов (CustomResourceDefinition, CRD):

| CRD | Область использования | Назначение |
|---|---|---|
| [`argocds.argoproj.io`](/modules/operator-argo/cr.html#argocd) | **Модуль `operator-argo`** | Декларативный ресурс, по которому модуль `operator-argo` настраивает и сопровождает экземпляр Argo CD в целевой среде. Через этот ресурс задаются параметры установки, состав компонентов, интеграции, политики доступа и другие настройки экземпляра. |
...
| [`applications.argoproj.io`](/modules/operator-argo/cr.html#application) | **Экземпляр Argo&nbsp;CD** | Основная прикладная CRD Argo CD для описания приложения, которое должно синхронизироваться из декларативного источника (Git, Helm, Kustomize и др.) в Kubernetes. Определяет источник, целевое окружение и параметры синхронизации. |
| [`applicationsets.argoproj.io`](/modules/operator-argo/cr.html#applicationset) | **Экземпляр Argo&nbsp;CD** | Используется для автоматизированного создания набора ресурсов `Application` по шаблону. Подходит для сценариев массового управления приложениями: в нескольких кластерах, окружениях, директориях, командах или ветках репозитория. |
...
Чтобы запустить экземпляр Argo CD, создайте ресурс [`ArgoCD`](/modules/operator-argo/cr.html#argocd) (см. раздел ниже). Работа с кастомными ресурсами, относящимися к области экземпляра Argo CD, описана в [руководстве пользователя](/products/kubernetes-platform/documentation/v1/user/delivery/argocd/).

## Развёртывание экземпляра Argo CD

Параметры, доступные для конфигурации экземпляра Argo CD, приведены в [документации](/modules/operator-argo/cr.html#argocd) модуля `operator-argo`.

Чтобы развернуть экземпляр Argo CD в неймспейсе `argocd` и опубликовать веб-интерфейс Argo CD через Ingress, используйте следующий пример:
...
После создания ресурса `ArgoCD` из примера выше в неймспейсе `argocd` будут запущены компоненты Argo CD:
...
Когда все компоненты перейдут в статус `Running`, веб-интерфейс экземпляра Argo CD станет доступен по адресу `https://<argocd-domain>`. Для настройки аутентификации и получения учётных данных см. раздел [«Настройка аутентификации и авторизации»](authentication.html).

Доставка пользовательских приложений с помощью Argo CD описана в [руководстве пользователя](/products/kubernetes-platform/documentation/v1/user/delivery/argocd/).
...
Если в кластере требуется несколько экземпляров Argo CD, создайте для каждого из них отдельный неймспейс и отдельный ресурс `ArgoCD`.
...
{% alert level="warning" %}
В одном неймспейсе поддерживается создание не более одного ресурса `ArgoCD`.
{% endalert %}
...
Чтобы включить режим высокой доступности, установите параметр `spec.ha.enabled: true` в ресурсе `ArgoCD`.
...
При включённом режиме высокой доступности изменения в `.spec.redis.resources` не применяются. Ограничения и запросы ресурсов для `Redis` настраивайте через `.spec.ha.resources`.
{% endalert %}
...
Чтобы разрешить создание cluster-wide-ресурсов, укажите в параметре `clusterConfigNamespaces` настроек модуля `operator-argo` имя неймспейса, в котором запущен целевой экземпляр Argo CD. Если экземпляров несколько, перечислите их через запятую:
...
{% alert level="info" %}
Если полномочия, описанные в кластерных ролях по умолчанию, избыточны, в настройках ресурса `ArgoCD` укажите параметр `spec.defaultClusterScopedRoleDisabled` со значением `true`. В этом случае кластерные роли не будут созданы автоматически, и вы сможете самостоятельно описать необходимый уровень привилегий для ServiceAccount, используемого экземпляром Argo CD.
{% endalert %}

{% alert level="info" %}
При необходимости уровень доступа к кластерным ресурсам можно переопределить на уровне ресурса [`AppProject`](/modules/operator-argo/cr.html#appproject), используя параметры `spec.clusterResourceBlacklist` и `spec.clusterResourceWhitelist`.
{% endalert %}

### Использование собственного домена кластера

Если в кластере используется домен, отличный от `cluster.local`, укажите его в параметре `spec.clusterDomain` ресурса `ArgoCD`.
