...
description: "Запуск Argo CD в Deckhouse Platform."
lang: ru
relatedLinks:
...
В этом разделе описаны шаги, необходимые для запуска экземпляра Argo CD в кластере DP:
...
1. Дождитесь, пока модуль перейдёт в состояние `Ready`.
   ...
   Проверить состояние модуля можно в веб-интерфейсе DP или с помощью команды:
...
После включения модуля `operator-argo` в кластере DP станут доступны кастомные ресурсы Argo CD.
...
{% offtopic title="Список кастомных ресурсов Argo CD, создаваемых модулем <code>operator-argo</code>..." %}
...
Если в кластере требуется несколько экземпляров Argo CD, создайте для каждого из них отдельный неймспейс (или проект DP) и отдельный объект ArgoCD.
