---
title: Управление хранилищем образов в кластерах, полностью управляемых DKP
...
Управление настройками хранилища образов компонентов платформы для кластеров, полностью управляемых Deckhouse Kubernetes Platform (DKP), реализуется с помощью модуля [`registry`](/modules/registry/).
...
Архитектура режимов описана в разделе [«Модуль registry»](../../../architecture/deckhouse/registry.html).
...
- Бутстрап кластера DKP поддерживается в режимах `Direct`, `Unmanaged`, `Proxy` и `Local`. Параметры доступа к хранилищу образов компонентов DKP во время установки кластера настраивается через [ModuleConfig `deckhouse`](/modules/deckhouse/configuration.html#parameters-registry).
...
- Кластер полностью управляется DKP. В кластерах Managed Kubernetes настройка хранилища образов компонентов DKP с помощью модуля `registry` недоступна.
...
- Изменение параметров хранилища образов компонентов DKP и переключение режимов доступны только после полного завершения этапа бутстрапа.
...
1. Если необходимо отключить автоматическое обновление DKP до новых версий, загружая их из настроенного хранилища образов, удалите параметр `releaseChannel` из конфигурации модуля `deckhouse`.
...
1. После применения дождитесь в [статусе переключения](#просмотр-статуса-переключения-режима-хранилища-образов) сообщения:
...
| `DeckhouseRegistrySwitchReady`    | Состояние переключения DKP и его компонентов на использование нового хранилища образов контейнеров. Значение `True` указывает, что DKP успешно переключился на сконфигурированное хранилище образов и готов к работе                          |
