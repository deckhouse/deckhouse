...
Управление настройками хранилища образов компонентов платформы для кластеров, полностью управляемых Deckhouse Platform (DP), реализуется с помощью модуля [`registry`](/modules/registry/).

## Режимы управления настройками хранилища образов компонентов DP

Режимы определяют, как DP получает образы своих компонентов.

В DP поддерживается несколько режимов управления настройками хранилища образов компонентов платформы: `Unmanaged`, `Direct`, `Proxy` и `Local`. В режимах `Direct`, `Proxy` и `Local` для обращения к хранилищу образов компонентов DP используется фиксированный виртуальный адрес. Это позволяет избежать перезапуска всех компонентов control plane и повторного скачивания образов в случае каких-либо изменений в хранилище образов компонентов платформы.
...
Неконфигурируемый режим `Unmanaged` — устаревший (deprecated) режим управления настройками хранилища образов компонентов DP. В этом случае не используется модуль `registry` (например, в Managed Kubernetes-кластерах). Параметры конфигурации хранилища образов компонентов DP задаются при установке кластера, или при изменении в развёрнутом кластере с помощью утилиты `helper change registry` (deprecated).
{% endalert %}

## Ограничения по настройкам хранилища образов компонентов DP

Имеется ряд ограничений и особенностей, касающихся установки кластера, условий использования модуля `registry` и переключения режимов управления настройками хранилища образов компонентов DP.
...
Для управления настройками хранилища образов компонентов DP необходимо соблюдение следующих условий:
...
Статус переключения режима хранилища образов компонентов DP можно получить с помощью следующей команды:
