---
title: Управление настройками хранилища образов компонентов платформы в кластере, полностью управляемом DKP
permalink: ru/admin/configuration/registry/managing-interaction.html
description: "Управление настройками хранилища образов компонентов платформы в Deckhouse Kubernetes Platform. Режимы взаимодействия с хранилищем компонентов DKP"
lang: ru
---

Управление настройками хранилища образов компонентов платформы для кластеров, полностью управляемых DKP, реализуется с помощью модуля [`registry`](/modules/registry/).

## Режимы управления настройками хранилища образов компонентов DKP

Режимы определяют, как DKP получает образы своих компонентов.

В DKP поддерживается несколько режимов управления настройками хранилища образов компонентов платформы: `Unmanaged`, `Direct`, `Proxy` и `Local`. В режимах `Direct`, `Proxy` и `Local` для обращения к хранилищу образов компонентов DKP используется фиксированный виртуальный адрес. Это позволяет избежать перезапуска всех компонентов Control plane и повторного скачивания образов в случае каких-либо изменений в хранилище образов компонентов платформы.

В режиме `Unmanaged` не используется виртуальный адрес. Обращение выполняется напрямую к внешнему хранилищу: в случае изменения хранилища происходит перезапуск всех компонентов DKP.
...
Архитектура режимов описана в разделе [«Архитектура режимов управления настройками хранилища образов»](../../../architecture/deckhouse/registry-modes.html).

Особенности режимов управления настройками хранилища образов компонентов DKP:

- `Direct` — использование прямого доступа к **внешнему** хранилищу по фиксированному виртуальному адресу `registry.d8-system.svc:5001/system/deckhouse`. Фиксированный адрес позволяет избежать повторного скачивания образов и перезапуска компонентов при изменении параметров хранилища. Архитектура режима описана в разделе [«Архитектура режима Direct»](../../../architecture/deckhouse/registry-modes.html#архитектура-режима-direct).
- `Proxy` — использование **внутреннего кеширующего прокси-хранилища** с обращением к **внешнему** хранилищу. Кеширующее прокси-хранилище запускается на control-plane (master) узлах. Режим позволяет сократить количество запросов к внешнему хранилищу за счёт кеширования образов. Обращение к внутреннему хранилищу образов выполняется по фиксированному виртуальному адресу `registry.d8-system.svc:5001/system/deckhouse` аналогично режиму `Direct`. Архитектура режима описана в разделе [«Архитектура режима Proxy»](../../../architecture/deckhouse/registry-modes.html#архитектура-режима-proxy).
- `Local` — использование локального **внутреннего** хранилища образов, с запуском хранилища на control-plane (master) узлах. Режим позволяет кластеру работать в изолированной среде. Обращение к внутреннему хранилищу образов выполняется по фиксированному виртуальному адресу `registry.d8-system.svc:5001/system/deckhouse` аналогично `Direct` и `Proxy` режимам. Архитектура режима описана в разделе [«Архитектура режима Local»](../../../architecture/deckhouse/registry-modes.html#архитектура-режима-local).
...
{% alert level="warning" %}
В этом документе фигурируют конфигурируемый режим `Unmanaged` и неконфигурируемый режим `Unmanaged`. Конфигурируемый режим `Unmanaged` — это один из режимов работы модуля `registry`.

Неконфигурируемый режим `Unmanaged` — устаревший (deprecated) режим управления настройками хранилища образов компонентов DKP. В этом случае не используется модуль `registry` (например, в Managed Kubernetes кластерах). Параметры конфигурации хранилища образов компонентов DKP задаются при установке кластера, или при изменении в развёрнутом кластере с помощью утилиты `helper change registry` (deprecated).
{% endalert %}

## Ограничения по настройкам хранилища образов компонентов DKP

Имеется ряд ограничений и особенностей, касающихся установки кластера, условий использования модуля `registry` и переключения режимов управления настройками хранилища образов компонентов DKP.

### Ограничения при установке кластера

Ограничения по настройке хранилища образов компонентов DKP при установке кластера следующие:

- Bootstrap кластера DKP поддерживается в режимах `Direct`, `Unmanaged`, `Proxy` и `Local`. Параметры доступа к хранилищу образов компонентов DKP во время установки кластера настраивается через [ModuleConfig `deckhouse`](/modules/deckhouse/configuration.html#parameters-registry).
- Bootstrap кластера в режимах `Local` и `Proxy` поддерживаются только на статичных кластерах.
...
Для управления настройками хранилища образов компонентов DKP необходимо соблюдение следующих условий:
...
- Кластер полностью управляется DKP. В кластерах Managed Kubernetes управление настройками управления хранилищем образов компонентов DKP с помощью модуля `registry` недоступно.
...
- Изменение параметров хранилища образов компонентов DKP и переключение режимов доступны только после полного завершения этапа bootstrap.
...
## Примеры переключения режимов управления настройками хранилища образов
...
При необходимости переключения на старый метод управления хранилищем образов (без модуля `registry`), ознакомьтесь с [инструкцией](#миграция-на-устаревший-формат-управления-настройками-хранилища-образов-компонентов-dkp-без-модуля-registry).

{% alert level="warning" %}
Управление хранилищем образов компонентов DKP без модуля `registry` — устаревший (deprecated) формат.
{% endalert %}
...
Чтобы посмотреть тип container runtime, используемый на узлах кластера (в NodeGroup’ах) по умолчанию, используйте команду:
...
- Во время переключения containerd v1 будет переведен на новую схему конфигурации хранилища образов компонентов DKP.
...
1. После удаления старых конфигураций убедитесь, что переключение продолжается. Пример [статуса переключения](#просмотр-статуса-переключения-режима-хранилища-образов):
...
## Миграция на устаревший формат управления настройками хранилища образов компонентов DKP (без модуля registry)

{% alert level="danger" %}

- Это устаревший (deprecated) формат управления хранилищем образов компонентов DKP.
...
1. Проверьте статус переключения, используя [инструкцию](#просмотр-статуса-переключения-режима-хранилища-образов). Пример вывода:
...
1. Переведите хранилище образов компонентов DKP в неконфигурируемый режим `Unmanaged` (без модуля `registry`). Пример конфигурации:
...
1. После удаления дождитесь завершения переключения. Для отслеживания используйте [инструкцию](#просмотр-статуса-переключения-режима-хранилища-образов). Пример вывода:
...
Статус переключения режима хранилища образов компонентов DKP можно получить с помощью следующей команды:
