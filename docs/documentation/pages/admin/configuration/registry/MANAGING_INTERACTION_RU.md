---
title: Управление настройками взаимодействия с хранилищем образов компонентов платформы в кластере, полностью управляемом DKP
permalink: ru/admin/configuration/registry/managing-interaction.html
description: "Управление настройками взаимодействия с хранилищем образов в Deckhouse Kubernetes Platform. Режимы взаимодействия с хранилищем компонентов DKP"
lang: ru
---

Управление настройками взаимодействия с хранилищем образов компонентов платформы для кластеров, полностью управляемых DKP, реализуется с помощью модуля [`registry`](/modules/registry/).

## Режимы взаимодействия с хранилищем оразов компонентов DKP

В DKP поддерживается несколько режимов взаимодействия с хранилищем образов компонентов платформы. В режимах `Direct`, `Proxy` и `Local` используется фиксированный виртуальный адрес. Это позволяет избежать перезапуска всех компонентов Control plane и повторного скачивания образов в случае каких-либо изменений в хранилище образов компонентов платформы.
...
Особенности режимов взаимодействия с хранилищем образов компонентов DKP:
...
- `Unmanaged` — работа без использования внутреннего хранилища образов. Виртуальный адрес не используется. Обращение выполняется напрямую к **внешнему** хранилищу.
  Существует два вида режима `Unmanaged`:
  - Конфигурируемый — режим, управляемый с помощью модуля `registry`.
  - Неконфигурируемый (deprecated) — режим без использования модуля `registry`. Параметры конфигурации задаются [при установке кластера](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#initconfiguration-deckhouse-imagesrepo), или при [изменении в развёрнутом кластере](/products/kubernetes-platform/documentation/v1/admin/configuration/registry/third-party.html) с помощью утилиты `helper change registry` (deprecated).
...
## Ограничения по настройкам взаимодействия с хранилищем образов

Имеется ряд ограничений и особенностей, касающихся установки кластера, условий использования модуля `registry` и переключения режимов взаимодействия с хранилищем.
...
- Bootstrap кластера DKP поддерживается только в `Direct`, `Unmanaged` и `Proxy` режимах (bootstrap в `Local` режиме не поддерживается). Параметры доступа к хранилищу во время установки кластера настраивается через [ModuleConfig `deckhouse`](/modules/deckhouse/configuration.html#parameters-registry).
...
Для управления настройками взаимодействия DKP с хранилищем необходимо соблюдение следующих условий:
...
- Кластер полностью управляется DKP. В кластерах Managed Kubernetes управление настройками взаимодействия с хранилищем образов с помощью модуля `registry` недоступно.
...
- Изменение параметров хранилища и переключение режимов доступны только после полного завершения этапа bootstrap.
...
## Примеры переключения режимов взаимодействия DKP с хранилищем образов
...
1. Если в кластере используется неконфигурируемый `Unmanaged` режим взаимодействия с хранилищем образов, перед переключением выполните [миграцию на формат управления хранилищем образов с использованием модуля `registry`](#миграция-на-формат-управления-хранилищем-образов-с-использованием-модуля-registry).
...
1. Если в кластере используется неконфигурируемый `Unmanaged` режим взаимодействия с хранилищем образов (без модуля `registry`), перед переключением выполните [миграцию на формат управления хранилищем образов с использованием модуля `registry`](#миграция-на-формат-управления-хранилищем-образов-с-использованием-модуля-registry).
