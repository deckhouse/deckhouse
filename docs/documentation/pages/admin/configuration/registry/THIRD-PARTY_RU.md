---
title: Переключение Managed Kubernetes-кластера на использование стороннего хранилища образов контейнеров
...
1. Выполните команду `deckhouse-controller helper change-registry` из пода DKP с параметрами нового хранилища образов.
   Пример запуска:
...
1. Если хранилище образов контейнеров использует самоподписанные сертификаты, положите корневой сертификат соответствующего CA (центра сертификатов) в файл `/tmp/ca.crt` в поде DKP и добавьте к вызову опцию `--ca-file /tmp/ca.crt` или вставьте содержимое сертификата CA в переменную, как в примере ниже:
...
1. Если необходимо отключить автоматическое обновление DKP до новых версий через стороннее хранилище образов, удалите параметр `releaseChannel` из конфигурации модуля `deckhouse`.
   После этого автоматическое обновление платформы будет отключено, и управлять версией DKP потребуется вручную.

1. Проверьте, не осталось ли в кластере подов с оригинальным адресом хранилища образов контейнеров:
