...
1. Проверьте, нет ли в неймспейсах `d8-*` подов в ошибочном состоянии, которые не могут загрузить образы. Это необходимо сделать вручную, так как в настоящий момент модули Deckhouse не переинициализируются автоматически после изменений, описанных выше.
