...
1. Чтобы корректно загрузить новые дайджесты образов после смены редакции, обратите внимание на то, что в настоящий момент модули Deckhouse не переинициализируются автоматически после такого изменения. Поэтому необходимо вручную проверить, нет ли в `d8-*` неймспейсах подов в ошибочном состоянии, которые не могут загрузить образы:
...
Если такие поды есть, повторно загрузите соответствующие им модули, выполнив на всех мастер-нодах:

```
rm -rf /var/lib/deckhouse/downloaded/<module-name>/
```

Для получения `<module-name>`, выполните:
