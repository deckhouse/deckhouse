...
Каждый узел один раз перезагрузится с очисткой кеша образов containerd. После перезагрузки узел заново скачает все образы из container registry, поэтому доступ к нему обязателен.
...
Если узлы кластера используют ContainerdV2, каждый из них будет поочерёдно перезагружен с полной очисткой кеша образов containerd. Чтобы управлять моментом перезагрузки, заранее переведите все NodeGroup, включая `master`, в ручной режим подтверждения простоя.

Сначала сохраните текущий режим подтверждения простоя всех NodeGroup, чтобы вернуть исходные значения после переключения:
...
Переведите все NodeGroup в ручной режим:
...
1. **Только для узлов на ContainerdV2** — примените конфигурацию миграции состояния containerd:
   ...
   Сборка containerd в DKP CSE проверяет целостность образов и распаковывает каждый слой вместе с файлом `.verity`. Слои, оставшиеся от предыдущей редакции, такого файла не имеют и переиспользуются по совпадающему дайджесту, из-за чего поды не запускаются с ошибкой `checking hash image: no such file or directory`. Конфигурация обнаруживает такие слои и очищает состояние containerd на узле.
