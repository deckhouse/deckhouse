...
1. Выполните scale всех операторов (redis, clickhouse, kafka и т.д) в ноль реплик. В момент миграции операторы в кластере работать не должны. Единственное исключение — [оператор `prometheus`](/modules/prometheus/) в составе Deckhouse, который будет автоматически отключен в процессе миграции.
1. Выключите модуль `ceph-csi` и включите [модуль `csi-ceph`](/modules/csi-ceph/).
...
Для подключения Ceph-кластера в Deckhouse необходимо включить [модуль `csi-ceph`](/modules/csi-ceph/). Для этого примените ресурс ModuleConfig:
