...
Deckhouse Platform (DP) поддерживает управление хранилищами, подключёнными через iSCSI или Fibre Channel, обеспечивая возможность работы с томами на уровне блочных устройств. Это позволяет интегрировать системы хранения данных с Kubernetes и управлять ими через CSI-драйвер.

На этой странице представлены инструкции по подключению SCSI-устройств в DP, созданию SCSITarget, StorageClass, а также проверке работоспособности системы.
...
- Для подключения по Fibre Channel:
  ...
  - необходимые LUN должны быть заранее созданы в СХД. DP не создаёт LUN;
  - на узлах должен быть установлен пакет `multipath-tools`.
...
1. Создайте ресурс SCSITarget. В поле [`spec.fibreChannel.WWNs`](/modules/csi-scsi-generic/cr.html#scsitarget-v1alpha1-spec-fibrechannel-wwns) укажите World Wide Port Name (WWPN) портов СХД, через которые доступны необходимые LUN. DP обнаруживает устройства, сопоставляя указанные WWPN с путями в директории `/dev/disk/by-path/`.
   ...
   При обнаружении и подключении тома DP выполняет сканирование FC-хостов. При этом он не настраивает коммутаторы и не выполняет вход на целевые устройства.
