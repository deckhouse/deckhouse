...
- Configure [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/). Before creating a StorageClass, you must create an [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resource for the `sds-node-configurator` module on the cluster nodes.
- Enable the `sds-node-configurator` module. Ensure that the `sds-node-configurator` module is enabled **before** enabling the `sds-local-volume` module.
- Create the corresponding StorageClasses. Creating a StorageClass for the CSI driver `local.csi.storage.deckhouse.io` by a user is **prohibited**.
...
The placement of `sds-local-volume-csi-node` pods is managed by specific labels (nodeSelector). These labels are set in the [spec.settings.dataNodes.nodeSelector](configuration.html#parameters-datanodes-nodeselector) parameter of the module.

### Configuring Storage on Nodes

To configure storage on nodes, you need to create LVM volume groups using [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resources. This example creates a Thick storage.

{% alert level="warning" %}
Before creating an [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resource, ensure that the `sds-local-volume-csi-node` pod is running on the respective node. This can be checked with the command:
...
1. Retrieve all [BlockDevice](../../../reference/cr/blockdevices/) resources available in your cluster:
...
1. Create an [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resource for the `worker-0` node:
...
1. Wait for the created [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resource to transition to the `Ready` state:
...
1. Create an [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resource for the `worker-1` node:
...
1. Create an [LVMVolumeGroup](../../../reference/cr/lvmvolumegroup/) resource for the `worker-2` node:
...
1. Create a [LocalStorageClass](../../../reference/cr/localstorageclass/) resource:
...
> **Note.** In a [LocalStorageClass](../../../reference/cr/localstorageclass/) with `type: Thick`, you cannot use an LVMVolumeGroup that contains at least one thin pool.
...
- Use stock kernels provided with [supported distributions](https://deckhouse.ru/documentation/v1/supported_versions.html#linux).
