...
permalink: en/admin/configuration/storage/sds/node-configurator/layouts.html
---
...
When creating virtual machines using cloning methods, be sure to replace the UUID of the volume groups (VG) by executing:
...
`<VG_or_LV_NAME>` — the name of the VG to deactivate all volumes in the group, or the name of the LV to deactivate a specific volume.
{% endalert %}
...
- [Storage with Identical Disks](#storage-with-identical-disks): All disks in the node are of the same type/size;
- [Combined Storage](#combined-storage): The node has disks of different types (e.g., SSD + HDD).
...
1. Add the prepared node to the Deckhouse cluster.
   ...
   If the node matches the `nodeSelector` specified in `spec.nodeSelector` of the `sds-replicated-volume` or `sds-local-volume` modules, the `sds-node-configurator` agent will detect the VG `main` and create a [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource. This can be used in the `sds-local-volume` and `sds-replicated-volume` modules.
...
After automatic discovery, three CRD resources of type [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) with automatically generated names will appear in the cluster.

To list the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources, execute the following command:
...
To configure `sds-local-volume` in "Full Mirror" mode, create a [LocalStorageClass](../../../../../reference/cr/localstorageclass/) resource and specify all discovered [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources in it. This ensures that the VG with the label `main` is available on each node in the module:
...
1. Create a [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource and add all [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it, so that the VG `main` is used on all nodes in the `sds-replicated-volume` module:
...
1. Create a [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) resource and specify the name of the previously created [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource in the `storagePool` field:
...
1. Add the prepared node to the Deckhouse cluster.
   ...
   If the node matches the `nodeSelector` specified in `spec.nodeSelector` of the `sds-replicated-volume` or `sds-local-volume` modules, the `sds-node-configurator` agent will run on this node, detect the VGs `main-safe` and `main-unsafe`, and add the corresponding [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to the Deckhouse cluster. These [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources can then be used to create volumes in the `sds-replicated-volume` or `sds-local-volume` modules.

#### Example of Configuring SDS Modules (Identical Disks, "Partial Mirror")

In this example, it is assumed that you have configured three nodes according to the "Partial Mirror" scenario. In the Deckhouse cluster, six [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources with randomly generated names will appear. In the future, it will be possible to specify a name for the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources created during the automatic VG discovery process using the `LVM` tag with the desired resource name.
...
To configure the `sds-local-volume` module according to the "Partial Mirror" scenario, create a [LocalStorageClass](../../../../../reference/cr/localstorageclass/) resource and add the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that only the VG `main-safe` is used on all nodes in the `sds-local-volume` module:
...
1. Create a [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource named `data-safe` and add the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that only the VG `main-safe` is used on all nodes in the `sds-replicated-volume` module for [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) with the parameter `replication: None`:
...
1. Create a [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource named `data-unsafe` and add the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that only the VG `main-unsafe` is used on all nodes in the `sds-replicated-volume` module for [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) with the parameter `replication: Availability` or `replication: ConsistencyAndAvailability`:
...
1. Create a [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) resource and specify the name of the previously created [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resources in the `storagePool` field so that the VGs `main-safe` and `main-unsafe` are used on all nodes:
...
To configure the `sds-local-volume` module according to the "Full Mirror" scenario, create a [LocalStorageClass](../../../../../reference/cr/localstorageclass/) resource and add all [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that the VG `<vg-name>` is used on all nodes in the `sds-local-volume` module:
...
Examples of informative names for the [LocalStorageClass](../../../../../reference/cr/localstorageclass/) resource for additional disks of different types:
...
1. Create a [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource and add all [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that the VG `<vg-name>` is used on all nodes in the `sds-replicated-volume` module:
   ...
   >
   > Examples of informative names for the [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource for additional disks of different types:
   >
...
To configure the `sds-local-volume` module according to the "Partial Mirror" scenario, create a [LocalStorageClass](../../../../../reference/cr/localstorageclass/) resource and add the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that only the VG `<vg-name>-safe` is used on all nodes in the `sds-local-volume` module:
...
1. Create a [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource named `data-<vg-name>-safe` and add the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that only the VG `<vg-name>-safe` is used on all nodes in the `sds-replicated-volume` module for [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) with the parameter `replication: None`:
...
1. Create a [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resource named `data-<vg-name>-unsafe` and add the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources to it so that only the VG `<vg-name>-unsafe` is used on all nodes in the `sds-replicated-volume` module for [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) with the parameter `replication: Availability` or `replication: ConsistencyAndAvailability`:
...
1. Create a [ReplicatedStorageClass](../../../../../reference/cr/replicatedstorageclass/) resource and specify the name of the previously created [ReplicatedStoragePool](../../../../../reference/cr/replicatedstoragepool/) resources in the `storagePool` field so that the VGs `<vg-name>-safe` and `<vg-name>-unsafe` are used on all nodes:
