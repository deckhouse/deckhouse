...
Контроллер оперирует двумя типами пользовательских ресурсов Kubernetes:
- [BlockDevice](../../../../../reference/cr/blockdevices/) — ресурс, представляющий блочное устройство;
- [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) — ресурс, описывающий группу томов Logical Volume Manager.

## Работа с ресурсами BlockDevice

### Создание ресурса BlockDevice

Контроллер периодически сканирует доступные блочные устройства на каждом узле и автоматически формирует ресурс [BlockDevice](../../../../../reference/cr/blockdevices/) при обнаружении устройства, соответствующего правилам. В результате создаётся объект ресурса с уникальным именем, содержащий детальную информацию о характеристиках устройства.
...
- Ёмкость устройства превосходит 1 GiB;
- Для виртуальных дисков обязательно наличие серийного номера.

Сформированный ресурс [BlockDevice](../../../../../reference/cr/blockdevices/) служит источником данных для последующей работы с ресурсами [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/).

### Обновление ресурса BlockDevice

При изменении состояния блочного устройства (например, изменение размера, изменение метаданных или исчезновение устройства из системы) контроллер автоматически обнаруживает эти изменения и обновляет поля ресурса [BlockDevice](../../../../../reference/cr/blockdevices/), обеспечивая актуальность информации. Пользовательская правка ресурса [BlockDevice](../../../../../reference/cr/blockdevices/) запрещена и будет перезаписана контроллером.

### Удаление ресурса BlockDevice

Контроллер удаляет ресурс [BlockDevice](../../../../../reference/cr/blockdevices/) только при выполнении условий:
...
- Блочное устройство не входит в состав группы томов с меткой `storage.deckhouse.io/enabled=true` (такая группа томов управляется [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) и удаление [BlockDevice](../../../../../reference/cr/blockdevices/) не производится).

{% alert level="info" %}
Контроллер выполняет данные операции без участия пользователя. При ручном удалении ресурса [BlockDevice](../../../../../reference/cr/blockdevices/) контроллер повторно создаст его при следующем сканировании, если устройство по-прежнему удовлетворяет критериям.
{% endalert %}

## Работа с ресурсами LVMVolumeGroup

Ресурс [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) агрегирует несколько [BlockDevice](../../../../../reference/cr/blockdevices/) в единую группу томов Logical Volume Manager (LVM) на узле и отображает текущее состояние этой группы. Обратите внимание, что на текущий момент поддерживается только тип `Local` (локальные группы томов).

### Создание ресурса LVMVolumeGroup

Существует два сценария формирования ресурса [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/):

- Автоматическое создание ресурса:
  - Контроллер сканирует список активных групп томов LVM на каждом узле.
  - Если у обнаруженной группы томов присутствует метка `storage.deckhouse.io/enabled=true` и нет соответствующего ресурса [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/), контроллер автоматически создаёт ресурс.
...
- Ручное создание ресурса
  - Пользователь формирует YAML-манифест, указывая минимальный набор полей: `metadata.name` — уникальное имя ресурса [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) и `spec` — имя узла, на котором будет существовать группа томов.
  - После валидации контроллер создаёт группу томов на узле и обновляет пользовательский ресурс актуальной информацией о состоянии созданной LVM Volume Group.
...
Пользователь может использовать любые допустимые селекторы (`matchLabels` или `matchExpressions`) для выбора [BlockDevice](../../../../../reference/cr/blockdevices/):
...
Поле `spec.local.nodeName` обязательно должно совпадать с именем узла, на котором создаётся Volume Group. В противном случае ресурс не будет запущен.
...
Секция `status` ресурса автоматически отражает актуальную информацию о состоянии Volume Group (список physical volumes, данные о thin pool, свободное/занятое пространство и т.д.). Рекомендуется **не вносить** правки в `status`, так как этот раздел управляется контроллером.
...
Контроллер удаляет ресурс автоматически, если на узле больше не существует Volume Group (например, все блочные устройства отключены или удалены). Для ручного удаления Volume Group и связанных с ней логических томов пользователь может вызвать:
...
- Чтобы исключить конкретный [BlockDevice](../../../../../reference/cr/blockdevices/) из состава Volume Group, скорректируйте селектор `spec.blockDeviceSelector` ресурса [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) или удалите соответствующий лейбл у ресурса [BlockDevice](../../../../../reference/cr/blockdevices/).
- На узле вручную выполните операции по переносу данных (если в Volume Group есть тома) следующей командой:

  ```shell
  pvmove <исходное-устройство>
  ```  

- Удалите физический том из группы:

  ```shell
  vgreduce <имя-VG> <исключаемое-устройство>
  ```  

- Окончательно уберите метаданные физического тома:
...
{% alert level="info" %}
Если в процессе удаления остаются логические тома, необходимо их заранее удалить с помощью команды `lvremove`.
Для защиты от непреднамеренного удаления можно использовать аннотацию `storage.deckhouse.io/deletion-protection` — до её снятия ресурс [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) не будет удалён.
{% endalert %}
...
- Параметр не задан — при удалении тома дополнительных операций не выполняется, блоки остаются неинициализированными, и данные могут быть доступны следующему пользователю.
...
- `Discard` — вызовом TRIM все блочные устройства помечаются как свободные; эффективно на большинстве современных твердотельных накопителях, но зависит от поддержки контроллера SSD (DRAT/RZAT).

Большинство современных твердотельных накопителей поддерживает команду `discard` (TRIM), которая помечает освобождённые блоки как «ноль» и при последующем чтении не возвращает старые данные. Поэтому использование опции `Discard` является наиболее надёжным способом предотвратить утечки данных на твердотельных накопителях. Однако следует учитывать следующие нюансы:
- Команда `discard` инициирует очистку ячеек, но фактическое обнуление выполняется контроллером накопителя в фоновом режиме. До завершения этой операции некоторые блоки могут ещё содержать старые данные.
