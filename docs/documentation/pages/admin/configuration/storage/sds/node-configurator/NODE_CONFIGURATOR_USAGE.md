...
- [BlockDevice](../../../../../reference/cr/blockdevices/): A resource representing a block device;
- [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/): A resource describing a Logical Volume Manager volume group.

## Working with BlockDevice Resources

### Creating a BlockDevice Resource

The controller periodically scans available block devices on each node and automatically creates a [BlockDevice](../../../../../reference/cr/blockdevices/) resource when it detects a device that matches the rules. As a result, a resource object with a unique name is created, containing detailed information about the device's characteristics.
...
The created [BlockDevice](../../../../../reference/cr/blockdevices/) resource serves as a data source for subsequent work with [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resources.

### Updating a BlockDevice Resource

When the state of a block device changes (for example, size change, metadata change, or the device disappears from the system), the controller automatically detects these changes and updates the fields of the [BlockDevice](../../../../../reference/cr/blockdevices/) resource, ensuring the information is up-to-date. User edits to the [BlockDevice](../../../../../reference/cr/blockdevices/) resource are prohibited and will be overwritten by the controller.

### Deleting a BlockDevice Resource

The controller deletes a [BlockDevice](../../../../../reference/cr/blockdevices/) resource only when the following conditions are met:
...
- The block device is not part of a volume group with the label `storage.deckhouse.io/enabled=true` (such a volume group is managed by [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/), and the [BlockDevice](../../../../../reference/cr/blockdevices/) is not deleted).

{% alert level="info" %}
The controller performs these operations without user intervention. If a user manually deletes a [BlockDevice](../../../../../reference/cr/blockdevices/) resource, the controller will recreate it during the next scan if the device still meets the criteria.
{% endalert %}

## Working with LVMVolumeGroup Resources

The [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource aggregates several [BlockDevice](../../../../../reference/cr/blockdevices/) resources into a single Logical Volume Manager (LVM) volume group on a node and reflects the current state of this group. Note that currently, only the `Local` type (local volume groups) is supported.

### Creating an LVMVolumeGroup Resource

There are two scenarios for creating an [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource:

- Automatic creation of the resource:
  - The controller scans the list of active LVM volume groups on each node.
  - If a discovered volume group has the label `storage.deckhouse.io/enabled=true` and there is no corresponding [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource, the controller automatically creates the resource.
  - The controller populates the `spec` section, except for the `thinPools` field — all other parameters (block device names, Volume Group name, etc.) are automatically pulled from the system state.
...
- Manual creation of the resource:
  - The user creates a YAML manifest, specifying the minimum set of fields: `metadata.name` — a unique name for the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource, and `spec` — the name of the node where the volume group will exist.
...
The user can use any valid selectors (`matchLabels` or `matchExpressions`) to select [BlockDevice](../../../../../reference/cr/blockdevices/) resources:
...
The `status` section of the resource automatically reflects the current information about the state of the Volume Group (list of physical volumes, thin pool data, free/used space, etc.). It is recommended **not to make** changes to `status`, as this section is managed by the controller.
...
- To exclude a specific [BlockDevice](../../../../../reference/cr/blockdevices/) from the Volume Group, adjust the `spec.blockDeviceSelector` selector of the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource or remove the corresponding label from the [BlockDevice](../../../../../reference/cr/blockdevices/) resource.
- On the node, manually perform data transfer operations (if there are volumes in the Volume Group) using the following command:

  ```shell
  pvmove <source-device>
  ```

- Remove the physical volume from the group:

  ```shell
  vgreduce <VG-name> <excluded-device>
  ```

- Finally, remove the physical volume metadata:
...
{% alert level="info" %}
If logical volumes remain during the removal process, they must be deleted in advance using the `lvremove` command.  
To protect against unintentional deletion, you can use the annotation `storage.deckhouse.io/deletion-protection` — until it is removed, the [LVMVolumeGroup](../../../../../reference/cr/lvmvolumegroup/) resource will not be deleted.
{% endalert %}
...
If the drive is not confirmed to support DRAT and RZAT (i.e., has not passed verification for deterministic cleaning and reading of zeroed blocks), the use of the `Discard` option is **not recommended**, as there remains a risk of recovering old data.
