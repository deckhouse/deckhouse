...
Также поддерживаются CNI Flannel и Simple Bridge.

В облачных кластерах установщик добавляет модуль CNI, рекомендованный облачным провайдером, если в конфигурации установки модуль CNI не задан. Для Azure, Google Cloud и Yandex Cloud это `cni-cilium` с туннелированием трафика подов через VXLAN. Если задан другой модуль CNI, установщик предложит заменить его на рекомендованный, а при отказе оставит вашу конфигурацию. В существующих кластерах CNI не меняется при обновлении DKP.
