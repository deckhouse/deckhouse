...
В модуле возможен выбор режима работы с помощью параметра [bpfLBMode](configuration.html#parameters-bpflbmode), влияющего на поведение `Service` с типом `NodePort` и `LoadBalancer`:
