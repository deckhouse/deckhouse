...
Для настройки режима работы bpfLB (BPF Load Balancer) в Deckhouse Kubernetes Platform можно использовать модуль [`cni-cilium`](../../../../modules/cni-cilium/).
...
В модуле возможен выбор режима работы с помощью параметра [bpfLBMode](../../../../modules/cni-cilium/configuration.html#parameters-bpflbmode), влияющего на поведение `Service` с типом `NodePort` и `LoadBalancer`:
...
spec:
  settings:
    tunnelMode: VXLAN
    bpfLBMode: SNAT # Выбран режим SNAT (Source Network Address Translation).
  version: 1
  enabled: true
```
