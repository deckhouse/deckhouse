...
Служебные домены (веб-интерфейсы компонентов DKP и модулей по `publicDomainTemplate`) и домены приложений (маршруты разработчиков приложения) настраиваются по-разному. Подробности — в разделе [«Публикация служебных доменов»](alb/alb-gateway-api.html#публикация-служебных-доменов).
...
| Способы приёма трафика | [Инлеты](./alb/alb-gateway-api.html#%D0%B8%D0%BD%D0%BB%D0%B5%D1%82%D1%8B) `LoadBalancer`, `HostPort` и `HostWithFailover` | Инлеты [`LoadBalancer`](/modules/alb/cr.html#clusteralbinstance-v1alpha1-spec-inlet-loadbalancer) и [`HostPort`](modules/alb/cr.html#clusteralbinstance-v1alpha1-spec-inlet-hostport) |
