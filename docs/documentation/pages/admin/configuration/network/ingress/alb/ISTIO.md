...
Use this option when you need advanced traffic management in a service mesh (for example, canary routing or mTLS). For configuration details, see the [`istio`](/modules/istio/) module documentation.
...
1. Create a secret that defines the TLS certificate and key for securing HTTPS traffic:
...
1. Create a Gateway resource. In the `spec.selector` field, specify the label that references the ingressGatewayClass and the secret created in the previous steps:
...
* Set up an Ingress that refers to the Service. The following annotations are mandatory for Ingress:
  ...
  * `nginx.ingress.kubernetes.io/upstream-vhost: productpage.bookinfo.svc`: Using this annotation,
