...
На поддержку модуля в составе Deckhouse Kubernetes Platform это не распространяется: модуль сопровождается командой Deckhouse, включая обновления безопасности. Подробнее — в разделе [«Поддержка и безопасность модуля»](#поддержка-и-безопасность-модуля).
{% endalert %}
