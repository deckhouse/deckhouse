...
description: "Миграция с модуля ingress-nginx на модуль alb в Deckhouse Platform: переход на Gateway API, переключение трафика и откат."
lang: ru
extractedLinksMax: 0
relatedLinks:
...
Руководство охватывает различия моделей, подготовку инфраструктуры модуля `alb` с ClusterALBInstance или ALBInstance и управляемым Gateway, перевод приложений на Gateway API, миграцию системных интерфейсов Deckhouse Platform (DP), переключение трафика на модуль `alb` и откат при необходимости.

Порядок работ:

1. [Подготовка инфраструктуры модуля alb](#step-1-preparing-alb-infrastructure).
1. [Миграция интерфейсов DP](#step-2-migrating-dkp-interfaces) — только если системные интерфейсы сейчас на Ingress и их нужно перевести на Gateway API.
...
- Активное сопровождение upstream-проекта Ingress NGINX, используемого в DP, прекращено. Новые функции, исправления и интеграции в нём не ожидаются. Для новых сценариев публикации приложений используйте Gateway API.
...
В этом разделе сравнивается, как описывается инфраструктура обработки трафика, как DP создаёт её и как правила маршрутизации приложений преобразуются в конфигурацию прокси-сервера.
...
1. DP обрабатывает объект и создаёт необходимую инфраструктуру, включая IngressClass. Несколько объектов IngressNginxController могут использовать один IngressClass.
1. По умолчанию ресурсы DP публикуются через IngressClass с именем `nginx`. Другой класс можно выбрать в глобальной конфигурации DP.
...
1. Если настроен объект Gateway DP по умолчанию, модули DP создают для него объекты ListenerSet, HTTPRoute и другие ресурсы Gateway API.
...
| Ссылки между неймспейсами | Ingress, объект Service, указанный в качестве бэкенда, и объект Secret с TLS-сертификатом, как правило, находятся в одном неймспейсе | Доступ к ресурсам в других неймспейсах явно разрешается с помощью [ReferenceGrant](https://gateway-api.sigs.k8s.io/reference/api-types/referencegrant/)                            |
...
- Используйте [ClusterALBInstance](/modules/alb/cr.html#clusteralbinstance) для общего или платформенного объекта Gateway, публикации системных интерфейсов DP, а также если требуется инлет `HostPort`.
- Для публикации системных интерфейсов DP выполните инструкции из раздела [«Публикация служебных доменов»](alb-gateway-api.html#публикация-служебных-доменов).
...
## Шаг 2. Миграция интерфейсов DP {#step-2-migrating-dkp-interfaces}

Если системные интерфейсы DP публикуются через Ingress и их нужно перевести на Gateway API, выполните инструкции из раздела [«Публикация служебных доменов»](alb-gateway-api.html#публикация-служебных-доменов).

{% alert level="info" %}
Не все модули DP пока публикуют служебные HTTPRoute через Gateway API. Команда `jq` из этого раздела покажет, какие модули уже делают это в вашем кластере, — это фактический список опубликованных маршрутов, а не полный перечень возможностей платформы.
{% endalert %}

Если миграция интерфейсов DP не требуется, перейдите к шагу 3.
...
Спецификация Gateway API не охватывает все зависящие от реализации функции управления трафиком, необходимые для работы DP. Поэтому модуль `alb` использует аннотации HTTPRoute для параметров, которые пока нельзя задать стандартными полями Gateway API.
...
1. Удалите ресурсы Ingress и Ingress-контроллеры, которые больше не обслуживают приложения и системные интерфейсы DP.
