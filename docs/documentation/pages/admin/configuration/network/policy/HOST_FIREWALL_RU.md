...
`CiliumClusterwideNetworkPolicy` с `nodeSelector` применяется к специальному endpoint-у узла с лейблом `reserved:host` и фильтрует трафик, входящий на узел и исходящий из него (включая трафик подов в режиме `hostNetwork`). Политики для подов с `endpointSelector` на host-endpoint не действуют — это разные сущности.
