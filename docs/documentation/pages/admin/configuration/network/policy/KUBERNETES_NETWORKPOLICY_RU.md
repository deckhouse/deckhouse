...
Здесь разрешены соединения от подов с лейблом `role=client` в локальном неймспейсе (в том же, где создан NetworkPolicy) или от любых подов в неймспейсе с лейблом `user=alice`.
