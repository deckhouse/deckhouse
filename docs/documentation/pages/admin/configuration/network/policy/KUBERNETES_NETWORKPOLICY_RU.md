...
- `spec.podSelector` — обязательное поле. Определяет, к каким подам политика применяется. Политика действует на поды в том же namespace, в котором она создана. Пустой селектор `{}` выбирает все поды namespace.
- `spec.policyTypes` — список из `Ingress`, `Egress` или обоих значений. Если поле опущено, значение `Ingress` устанавливается всегда, `Egress` — только если политика содержит правила egress.
...
- `podSelector` — поды в том же namespace, что и политика;
- `namespaceSelector` — все поды в выбранных namespace;
- `podSelector` и `namespaceSelector` в одном элементе — поды с указанными лейблами в namespace с указанными лейблами;
...
Эта политика применяется к подам с лейблом `app: db` в namespace `my-app`. На входящий трафик разрешены подключения на TCP-порт 6379 от трёх источников: из CIDR `172.17.0.0/16` за исключением `172.17.1.0/24`, из любых подов в namespace `frontend`, из подов с лейблом `role: backend` в локальном namespace. На исходящий трафик разрешены подключения только на TCP-порт 5978 в подсеть `10.0.0.0/24`.
...
Здесь разрешены соединения от подов с лейблом `role=client` из namespace с лейблом `user=alice`.
...
Здесь разрешены соединения от подов с лейблом `role=client` в локальном namespace или от любых подов в namespace с лейблом `user=alice`.

### Выбор namespace по имени

Поле для прямого указания имени namespace в спецификации отсутствует. Вместо этого используйте лейбл `kubernetes.io/metadata.name`. Kubernetes автоматически устанавливает этот лейбл на каждый namespace, а его значение равно имени namespace.
...
## Default-политики namespace

Для namespace применяют типовые политики по умолчанию, задавая базовое поведение:

- запретить весь входящий трафик к подам namespace:
...
- запретить весь исходящий трафик из подов namespace:
...
Поведение при изменении набора политик во время уже установленного соединения не определено: одни движки разрывают такое соединение, другие оставляют до закрытия. Не меняйте политики и лейблы подов или namespace в моменты, когда это может затронуть рабочий трафик.
...
Поддерживаются только стандартные форматы Kubernetes: `networking.k8s.io/NetworkPolicy API`, V1/GA и beta-семантика. Расширений Cilium (CNP, CCNP, L7-правила, FQDN, deny-правила) этот движок не поддерживает.

Готовые примеры стандартных политик, работающих и через `network-policy-engine`, и через `cni-cilium`, есть в разделе [Типовые примеры политик](examples.html).
