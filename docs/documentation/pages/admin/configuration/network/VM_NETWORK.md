...
The [`.spec.settings.virtualMachineCIDRs`](/modules/virtualization/configuration.html#parameters-virtualmachinecidrs) block lists the subnets in CIDR notation from which Deckhouse Platform (DP) assigns IP addresses to virtual machines, either automatically or on request.
