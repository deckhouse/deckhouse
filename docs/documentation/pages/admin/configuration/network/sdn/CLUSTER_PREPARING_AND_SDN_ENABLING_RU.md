...
description: |
  Подготовка кластера и настройки программно-определяемых сетей
lang: ru
---
...
* **Для создания дополнительных программно-определяемых сетей на основе прямого, нетегированного доступа в сетевой интерфейс:**
...
После включения модуля в кластере автоматически появятся объекты [NodeNetworkInterface](/modules/sdn/cr.html#nodenetworkinterface), которые отражают текущее состояние интерфейсов на узлах.

Чтобы проверить их наличие, используйте команду:
...
{% alert level="info" %}
При обнаружении интерфейсов узлов контроллер устанавливает на них следующие служебные метки:
...
### Разметка интерфейсов под организацию дополнительных программно-определяемых сетей

Чтобы была возможность организовывать [дополнительные программно-определяемые сети](#настройка-и-подключение-дополнительных-виртуальных-сетей-для-использования-в-прикладных-подах), разметьте выделенные интерфейсы, которые планируется использовать для создания сетей (в примере выше — eth1), подходящим лейблом.
...
### Объединение нескольких физических интерфейсов в интерфейс агрегации каналов (Bond)

Для повышения пропускной способности или резервирования возможно объединение нескольких физических интерфейсов в Bond.
   ...
   spec:
     ...
     bond:
       ...
       memberNetworkInterfaces:
         - labelSelector:
             matchLabels:
               network.deckhouse.io/node-name: worker-01 # Служебный лейбл, который необходимо указывать для объединения с интерфейсом Bond на определенном узле.
               nni.example.com/bond-group: bond0 # Пользовательский лейбл, (был добавлен на интерфейсы на предыдущем шаге).
   ```
...
* С точки зрения управления сети бывают двух типов:
  * [Кластерная](#создание-общедоступной-сети-кластерной) (общедоступна) — сеть, общедоступная в каждом проекте, управляется администратором. Пример — публичная WAN-сеть или shared-сеть обмена трафиком между проектами;
  * [Сеть проекта](#создание-сети-проекта-пользовательской-сети) (сеть проекта) — сеть, доступная в рамках неймспейса, управляется пользователем.
...
{% alert level="warning" %}
Перед созданием дополнительной сети [разметьте интерфейсы](#разметка-интерфейсов-под-организацию-дополнительных-программно-определяемых-сетей), которые будут использованы неё.
{% endalert %}

### Создание общедоступной сети (кластерной)
...
* **Поддержка DPDK**: Устройства могут быть привязаны в различных режимах, подходящих для рабочих нагрузок DPDK:
  ...
  * **NetDev**: Внутрь пода пробрасывается только сетевой интерфейс Linux как стандартное сетевое устройство ядра.
...
Чтобы создать Underlay-сеть в режиме Dedicated, выполните следюущие шаги:
...
Чтобы создать Underlay-сеть в режиме Shared, выполните следюущие шаги:
