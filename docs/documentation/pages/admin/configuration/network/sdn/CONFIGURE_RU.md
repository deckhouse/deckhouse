...
Для использования SDN в кластере DP требуется подготовка инфраструктуры к включению модуля [`sdn`](/modules/sdn/), а также некоторые подготовительные действия после его включения.
...
* **Для создания дополнительных программно-определяемых сетей на основе тегированных VLAN:**
  ...
  * Выберите физические интерфейсы на узлах для последующей настройки тегированных VLAN-интерфейсов. Допускается использование интерфейсов, уже задействованных для служебной межузловой сети DP.
...
В примере выше у каждого узла в кластере есть по два сетевых интерфейса — eth0 (DP LAN) и eth1 (выделенный интерфейс для организации дополнительных программно-определяемых сетей).
...
В Deckhouse Platform можно декларативно управлять дополнительными сетями для прикладных нагрузок (поды, виртуальные машины). При этом предусмотрено следующее:
...
#### Принципы и особенности работы IPAM в DP
...
В DP реализованы следующие возможности по работе с Underlay-сетями:
