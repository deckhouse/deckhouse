...
После включения модуля в кластере автоматически появятся объекты [NodeNetworkInterface](cr.html#nodenetworkinterface) (можно использовать сокращение — nni), которые отражают текущее состояние интерфейсов на узлах.
