...
В блоке [`.spec.settings.virtualMachineCIDRs`](/modules/virtualization/configuration.html#parameters-virtualmachinecidrs) перечисляются подсети в формате CIDR, из которых Deckhouse Platform (DP) выдаёт IP-адреса виртуальным машинам автоматически или по запросу.
