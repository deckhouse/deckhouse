...
permalink: ru/admin/configuration/switching-editions.html
...
{% alert level="warning" %}
Инструкция подразумевает использование публичного адреса хранилища образов контейнеров: `registry-cse.deckhouse.ru`.
...
Если в процессе переключения образ какого-либо модуля не был повторно загружен и модуль не был переустановлен, для устранения проблемы воспользуйтесь [инструкцией](../../faq.html#что-делать-если-образ-модуля-не-скачался-и-модуль-не-переустанов).
{% endalert %}
...
1. Дождитесь переключения хранилища образов. Для проверки выполнения переключения воспользуйтесь [инструкцией](/modules/registry/faq.html#как-посмотреть-статус-переключения-режима-registry).
...
1. Проверьте, не осталось ли в кластере подов с адресом хранилища образов контейнеров для Deckhouse EE:
...
1. Подготовьте переменные с лицензионным ключом и создайте [ресурс NodeGroupConfiguration](/modules/node-manager/cr.html#nodegroupconfiguration) для переходной авторизации в `registry-cse.deckhouse.ru`:
   ...
   > Перед созданием ресурса ознакомьтесь с разделом [Как добавить конфигурацию для дополнительного хранилища образов](/modules/node-manager/faq.html#как-добавить-конфигурацию-для-дополнительного-registry)
...
1. Актуализируйте секрет доступа к хранилищу образов контейнеров Deckhouse CSE, выполнив следующую команду:
...
- [с помощью модуля `registry`](#переключение-с-помощью-модуля-registry),
- [без использования модуля `registry`](#переключение-без-использования-модуля-registry).
