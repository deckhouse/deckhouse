...
- Проверки `dhctl`:
	- Общие проверки:
		...
		- Проверка данных аутентификации в хранилище образов контейнеров, указанных в конфигурации установки.
	- Проверки для установки статического и гибридного кластера:
		...
		- Проверка возможности подключения по SSH с использованием указанных данных аутентификации.
		- Проверка возможности установки SSH-туннеля до сервера (ВМ) мастер-узла.
		- Проверки, что сервер (ВМ) удовлетворяет минимальным требованиям для настройки мастер-узла.
		- Проверка установленного python и необходимых библиотек на сервере (ВМ) для мастер-узла.
		- Проверка доступности хранилища образов контейнеров через прокси (если настройки прокси указаны в конфигурации установки).
		- Проверка, что на сервере (ВМ) и хосте инсталлятора свободны порты, необходимые для установки.
		- Проверка, что localhost разрешается в IP 127.0.0.1.
	- Проверки для установки облачного кластера: 
		- Проверки, что сервер (ВМ) удовлетворяет минимальным требованиям для настройки мастер-узла.
		- Проверка, что конфигурация мастер-узлов, заданная в конфигурации установки, удовлетворяет минимальным требованиям.
- Проверки `bashible`:
	- Общие проверки:
		- Проверка, что имя хоста соответствует требованиям: длина <= 63 символов, имя в нижнем регистре, без спецсимволов (допускаются символы `-` и `.`, которые не могут стоять в начале и конце имени).
		- Проверка, что на сервере (ВМ) отсутствует установленный CRI (contanerd).
	- Проверки для настройки узла в статическом и гибридном кластере:
		- Проверка уникальности имени хоста относительно других хостов кластера.
