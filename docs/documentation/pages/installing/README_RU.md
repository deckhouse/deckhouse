...
1. Набор ресурсов [ModuleConfig](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#moduleconfig), содержащих параметры конфигурации встроенных модулей Deckhouse. Некоторые ресурсы ModuleConfig являются обязательными, необходимость использования других зависит от окружения, параметров создаваемого кластера и иных факторов.
   ...
   * [`deckhouse`](/modules/deckhouse/configuration.html) — настройки для доступа к registry, желаемый канал обновлений, набор модулей (bundle), которые будут включены по умолчанию, и другие параметры;
...
Укажите параметры для доступа к стороннему registry в секции [`settings.registry`](/modules/deckhouse/configuration.html#parameters-registry) ресурса ModuleConfig `deckhouse`.
