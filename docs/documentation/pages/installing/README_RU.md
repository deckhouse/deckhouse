...
1. Общие проверки:
   ...
   - Адресное пространство Pod’ов (`podSubnetCIDR`) и сервисов (`serviceSubnetCIRD`) кластера не пересекаются.
...
1. Проверки для установки статического и гибридного кластера:
   ...
   - На сервере (ВМ) должно быть корректное время.
   - Адресное пространство Pod’ов (`podSubnetCIDR`), сервисов (`serviceSubnetCIRD`) и внутренней сети кластера (`internalNetworkCIDRs`) не пересекаются.
