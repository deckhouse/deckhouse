...
1. Проверки для установки статического и гибридного кластера:
   ...
   - На сервере (ВМ) для master-узла и в хосте, на котором запущен установщик, свободны порты, перечисленные в разделе [«Сетевое взаимодействие компонентов платформы»](../reference/network_interaction.html).
