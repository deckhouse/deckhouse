...
1. Общие проверки:
   ...
   - На сервере (ВМ) отсутствует установленный container runtime (например, `containerd`, `docker` и др.).
   - Имя хоста должно быть уникальным в пределах кластера.
   - Проверка корректности времени на сервере.
...
1. Проверки для установки статического и гибридного кластера:
   ...
   - Открыты необходимые порты для установки:
     - между install-хостом и сервером — порт 22322/TCP;
