...
Компоненты распределяются по приоритету: если в кластере есть подходящий тип узлов, компонент попадёт на него. За что отвечает каждый компонент, описано в разделе [«Подсистема Virtualization»](../architecture/virtualization/).
