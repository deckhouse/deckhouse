...
1. Проверки для установки статического и гибридного кластера:
   ...
   - На сервере (ВМ) для master-узла должен быть доступен Python (`python3`, `python2` или `python`). Также должны быть доступны стандартные модули Python, используемые установщиком: `urllib.request` или `urllib2`, `urllib.error` или `urllib2`, `configparser` или `ConfigParser`, `http.server` или `SimpleHTTPServer`, `http.server` или `SocketServer`.
   - Хранилище образов доступно через прокси (если настройки прокси указаны в конфигурации установки).
   - На сервере (ВМ) для master-узла и в хосте, на котором запущен установщик, свободны порты, необходимые для процесса установки. Требования к сетевому взаимодействию и перечень используемых портов приведены в разделе [«Настройка сетевых политик для работы Deckhouse»](../reference/network_interaction.html).
