...
- Скачайте образы DKP в выделенную директорию, используя команду `d8 mirror pull`.
  ...
  Если в ограничении версии для `--include-module` используются операторы `>=` или `<=`, заключайте в кавычки все значение флага. Без кавычек shell воспринимает `>` как перенаправление вывода.
