...
В случае прерывания установки или возникновения проблем во время установки, в облаке могут остаться созданные ресурсы. Для удаления созданных при установке Deckhouse ресурсов в облаке, используйте команду: `dhctl bootstrap-phase abort`. Обратите внимание, что файл конфигурации (передаваемый через параметр `--config`) должен быть тот же, с которым производилась установка.

Чтобы удалить работающий кластер в облаке, развернутый после установки Deckhouse, используйте команду: `dhctl destroy`, передав ей параметры подключения к master-узлу. `dhctl` подключится к master-узлу, получит от него terraform state и корректно удалит созданные в облаке ресурсы.
