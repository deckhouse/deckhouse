...
{% alert level="warning" %}
Сеть провижининга должна быть изолированной. Эндпоинт на порту `50000/TCP` не аутентифицирован: у машины без конфигурации ещё нет секрета, которым она могла бы подтвердить себя. Любой, кто может обратиться к этому порту, установит машину как узел чужого кластера. Инвентарь, который машина отдаёт на том же порту, тоже открыт без проверки подлинности: тот, кто до него дотянется, увидит диски машины, их серийные номера и её интерфейсы. Держите машины в отдельном сегменте сети до окончания установки.
{% endalert %}

Машина, ожидающая конфигурацию, сама рассказывает о себе на том же порту — гадать, что в ней стоит, не нужно:

```shell
curl http://<адрес>:50000/inventory        # чтобы прочитать глазами
curl http://<адрес>:50000/inventory.json   # чтобы разобрать скриптом
```

`/inventory` — краткая форма. По каждому диску она даёт имя в ядре, размер, состояние (`blank`, `formatted` или `system-layout`), ссылку в `/dev/disk/by-path` и разделы с их файловыми системами и метками; для дисков, которые нельзя различить по размеру, добавляется отдельная предупреждающая строка. Под каждым диском печатается строка для вставки в `NodeConfig`: она называет диск однозначно, если машина хоть чем-то его выделяет, и прямо говорит, что не выделяет ничем, если так и есть. `/inventory.json` отдаёт то же самое плюс всё, чего в краткой форме нет: модель, производителя, серийный номер, `wwid`, все ссылки `/dev/disk/by-id`, путь на шине, тип подключения и вращается ли диск. Интерфейсы с их MAC-адресами, состоянием, адресами и шлюзом перечисляют обе формы. Вот как выглядит краткая:
...
Оба эндпоинта отвечают, только пока машина ждёт конфигурацию: как только она её примет, сервер, который их обслуживал, закрывается.
...
Доступ к хранилищу образов должен быть настроен в режиме `Unmanaged`: неизменяемый узел забирает образы control plane и системные расширения из хранилища напрямую, без внутрикластерного прокси, которого во время установки ещё нет.
...
При повторном запуске установщик пропускает машины, которым уже передал конфигурацию, — машина, ставшая узлом, вторую конфигурацию не примет. Эта отметка хранится по файлу на машину: `immutable-control-plane-pushed-payload-<имя-узла>` в каталоге кэша состояния, который установщик печатает при старте (`State cache directory: …`; это подкаталог с хешем в имени внутри `--cache-dir`, по умолчанию `/tmp/dhctl`). Если между попытками переустановили одну машину, удалите файл только этой машины. Повторный запуск на кластере, где DKP уже установлен, больше не останавливается на пространстве имён `d8-system`: установщик видит его на месте и оставляет как есть.

{% alert level="warning" %}
Не используйте для этого `--yes-i-want-to-drop-cache`: он очищает кэш целиком, вместе с отметками остальных машин и путём к учётным данным, собранным с первого мастера. Первому мастеру уже сообщили, что учётные данные сохранены, и он закрыл свой bootstrap-канал — собрать их второй раз нельзя. Сбрасывать кэш целиком уместно, только если переустановлены все машины.
{% endalert %}
...
Образ ОС, собранный до того, как канонический путь стал `/config`, принимает документ только по пути `/nodeconfig.yml`. Установщик сам переходит на него, написанный руками `curl` — нет.
