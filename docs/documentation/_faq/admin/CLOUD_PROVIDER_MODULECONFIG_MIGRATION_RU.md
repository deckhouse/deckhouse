...
{% alert level="warning" %}
Миграция обязательна. Это не опциональный шаг: поддержка `<PROVIDER>ClusterConfiguration` будет прекращена. Пока миграция не выполнена, обновление DKP может быть заблокировано.
{% endalert %}

Миграция безопасна. Она подразумевает применение автоматически подготовленных DKP ресурсов и **не приводит к пересозданию узлов**. При этом требуется осознанное действие администратора — автоматически подготовленные ресурсы нужно просмотреть и применить вручную.
