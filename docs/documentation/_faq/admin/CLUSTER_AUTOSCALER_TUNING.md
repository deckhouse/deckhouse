...
1. Create one or more new node groups ([NodeGroup](/modules/node-manager/cr.html#nodegroup)), in which, in the [`spec.cloudInstances.classReference`](/modules/node-manager/cr.html#nodegroup-v1-spec-cloudinstances-classreference), specify the InstanceClass objects created in the previous step. In the [`spec.cloudInstances.priority`](/modules/node-manager/cr.html#nodegroup-v1-spec-cloudinstances-priority) parameter, set the priority of the node group.
1. If necessary, modify the `max-node-provision-time`, `max-node-group-backoff-duration`, and `initial-node-group-backoff-duration` parameters, which affect the rate at which nodes from lower-priority groups are provisioned.
...
| Time | Event |
|-------|---------|
| 10:00 | Cluster autoscaler orders a node from **A** |
| 10:15 | Failed to provision a node in **A** within 15 minutes (`max-node-provision-time`) → **A** blocked for 5 minutes (`initial-node-group-backoff-duration`) |
| 10:15:xx | Cluster autoscaler requests a node in **B** (next in priority, not blocked)|
| 10:30 | Failed to provision a node in **B** within 15 minutes (`max-node-provision-time`) → **B** blocked for 5 minutes (`initial-node-group-backoff-duration`)|
| 10:30:xx | Cluster autoscaler requests a node in **A** again, as the lockout period (the default value for `initial-node-group-backoff-duration` is 5 minutes) has ended|
| 10:45 | Failed to order a node in **A** → **A** is blocked for 10 minutes (`initial-node-group-backoff-duration` for **A** is doubled) |
| … | And so on, until the lockouts for **A** and **B** exceed 15 minutes (`max-node-provision-time`). In this case, the queue may not reach group **C** for about 1.5 hours |
