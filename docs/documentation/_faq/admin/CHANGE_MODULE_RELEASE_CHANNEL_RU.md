...
Модуль может быть встроенным в DP или подключенным из источника модулей (определяется с помощью [ModuleSource](reference/api/cr.html#modulesource)). Встроенные модули имеют общий с DP релизный цикл и обновляются вместе с DP. **Канал обновлений встроенного модуля всегда соответствует каналу обновлений DP.** Модуль, подключаемый из источника, имеет собственный релизный цикл, который не зависит от релизного цикла DP. **Канал обновлений модуля, подключенного из источника, может быть изменен.**

Далее рассматривается процесс смены канала обновлений для модуля, подключенного из источника.

По умолчанию канал обновлений для модулей наследуется от канала обновлений DP (указывается в параметре [`releaseChannel`](/modules/deckhouse/configuration.html#parameters-releasechannel) ModuleConfig `deckhouse`). Подробнее про каналы обновлений — в разделе [«Каналы обновлений»](architecture/module-development/versioning/#каналы-обновлений).
