...
Jekyll::Hooks.register :site, :pre_render do |site|
  ...
  # Generate search index data in pre_render hook to ensure all generators have run
  puts "Generating search index data in pre_render hook..."
  generate_search_index_data_in_hook(site)
end

# Generate search index data using the same logic as the generator
def generate_search_index_data_in_hook(site)
  puts "Generating search index data in pre_render hook..."
  
  # Generate search indices for all languages
  search_indices = {}
  ['en', 'ru'].each do |lang|
    search_indices[lang] = generate_search_index_in_hook(site, lang)
  end
  ...
  ['en', 'ru'].each do |lang|
    ...
    embedded_documents = embedded_pages.map do |page|
      ...
      
      {
        ...
        "content" => normalize_search_content_in_hook(page.content || '')
      }
    end
      ...
      if site.data['search']['searchItems'][lang]
        site.data['search']['searchItems'][lang].each do |item|
          param = {
            ...
            "content" => normalize_search_content_in_hook(item['content'] || '')
          }
  ...
  puts "Search index data generation completed in pre_render hook."
end

def generate_search_index_in_hook(site, lang = nil)
...
def normalize_search_content_in_hook(text)
  return '' if text.nil?
