    ...
    def format_schema(name, attributes, parent, primaryLanguage = nil, fallbackLanguage = nil, ancestors = [])
        ...
        if name != ""
            ...
            if attributes['x-doc-deprecated']
                name_text = sprintf(%q(<span id="%s" data-anchor-id="%s" class="resources__prop_name anchored deprecated" data-tippy-content="%s">%s<span data-tippy-content="%s">%s</span></span>), linkAnchor, linkAnchor, pathString, name,
