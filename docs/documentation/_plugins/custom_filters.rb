...
module Jekyll
  module CustomFilters
    ...
    STRIP_HTML_TAGS = /<.*?>/m
    STRIP_MD_CODEBLOCKS = Regexp.union(
      /```[\s\S]*?```/m,
      /~~~[\s\S]*?~~~/m,
      /^```[\s\S]*?^```/m,
      /^~~~[\s\S]*?^~~~/m
    )
    STRIP_SHELL_BLOCKS = /<<\s*EOF[\s\S]*?^EOF/m
    STRIP_FRONT_MATTER = /^---\s*\n[\s\S]*?\n---\s*\n/m
    STRIP_HTML_DIV_BLOCKS = /<div[^>]*markdown="0"[^>]*>[\s\S]*?<\/div>/m
    STRIP_HTML_CODE_BLOCKS = /<code[^>]*>[\s\S]*?<\/code>/m
    STRIP_HTML_TABLE_BLOCKS = /<table[^>]*>[\s\S]*?<\/table>/m
    STRIP_INLINE_CODE = /`([^`]+)`/
...
#                        gsub(STRIP_MD_TABLES,' ').
                       gsub(STRIP_HTML_TABLE_BLOCKS,' ').
#                        gsub(STRIP_LIQUID_TAGS, ' ').
#                        gsub(STRIP_MD_CODEBLOCKS, ' ').
                       gsub(STRIP_SHELL_BLOCKS, ' ').
#                        gsub(STRIP_FRONT_MATTER, ' ').
#                        gsub(STRIP_HTML_DIV_BLOCKS, ' ').
                       gsub(STRIP_HTML_CODE_BLOCKS, ' ').
