...
description: Аппаратные требования к узлам кластера под управлением Deckhouse Platform.
lang: ru
layout: sidebar-guides
---

Перед развёртыванием кластера под управлением Deckhouse Platform необходимо определиться с конфигурацией будущего кластера и выбрать параметры для будущих узлов кластера, такие как количество RAM, CPU и так далее.
...
* Какие модули Deckhouse Platform (DP) планируется использовать?
...
{% alert level="info" %}
Все описанное дальше применимо к установке Deckhouse Platform [с набором модулей Default](/products/kubernetes-platform/documentation/v1/admin/configuration/#наборы-модулей).
{% endalert %}
...
* **системные узлы** — предназначены для запуска модулей DP;
...
{% alert level="info" %}
Deckhouse Platform, начиная с версии 1.74, имеет механизм контроля целостности модулей, который защищает их от подмены и изменения. Этот механизм включается автоматически при поддержке операционной системой на узлах, где установлен DP, модуля ядра `erofs`. При отсутствии этого модуля ядра DP продолжит работу без механизма контроля целостности модулей, при этом появится алерт о неработоспособности этой функциональности.
{% endalert %}
  ...
  <tbody>
    <tr>
      <td rowspan="6" style="width: 45%;">
        ...
        <i>Работа кластера на узлах с минимальными требованиями сильно зависит от набора включённых модулей DP.<br>
...
Для развёртывания Deckhouse Platform в редакции Enterprise Edition с набором модулей [Default](/modules/deckhouse/configuration.html#parameters-bundle) необходима следующая конфигурация узлов:
...
{% alert level="info" %}
При необходимости DP в такой же конфигурации можно запустить и на одном узле с 16 CPU, 32 ГБ RAM для виртуальной машины или 10 CPU, 24 ГБ RAM для сервера bare metal.
{% endalert %}
...
Команда Deckhouse Platform знает, каких усилий требует организация работы production-кластера в Kubernetes. Мы будем рады, если DP позволит вам реализовать задуманное. Поделитесь своим опытом и вдохновите других на переход в Kubernetes.
