...
* `/var/lib/containerd` – слои образов компонентов DKP. Чем больше компонентов DKP устанавливается на узел, тем больше свободного пространства в этом каталоге будет затребовано.
...
* `/var/lib/deckhouse/downloaded/` — хранилище данных модулей DKP;
...
| `/run/containerd`                 | 1 GB        |
| `/var/lib/cni`                    | 1 GB        |
| `/opt/local-path-provisioner`     | 40 GB*      |

{% alert level="info" %}
\* Зависит от настроек, заданных пользователем.
{% endalert %}
