...
description: Руководство по выбору объема дисков и разметке файловой системы перед установкой Deckhouse Platform
lang: ru
layout: sidebar-guides
---

В [гайде по выбору минимально требуемого дискового пространства](hardware-requirements.html#выбор-ресурсов-для-узлов) для различных типов узлов Deckhouse Platform (DP) указаны объемы дисков, которые необходимо выделить для успешной установки и работы DP. Важно также корректно сконфигурировать файловую систему — иначе пространство может неожиданно закончиться даже при верно рассчитанном объёме на этапе установки.

{% alert level="info" %}
Причина сбоев может крыться в поведении установщика дистрибутива Linux. Например, Astra Linux при установке может выделить 15 ГБ под корень файловой системы (`/`), 15 ГБ под домашний каталог пользователя (`/home`), а остальное оставить неразмеченным, несмотря на подключённый диск общим объемом в 60 ГБ, рекомендуемых в гайде. При такой конфигурации установка DP завершается ошибкой отсутствия требуемого дискового пространства.
{% endalert %}

Чтобы избежать проблем в будущем, перед установкой лучше убедиться, что разделы файловой системы диска, выделенного для машины, соответствуют требованиям DP по объёму.

## Где и что хранит DP

DP размещает данные разных типов в определённых каталогах файловой системы. Ниже — краткий разбор ключевых каталогов:

* `/etc/kubernetes/`, `/etc/containerd` и т.д. — каталоги с конфигурацией компонентов Kubernetes;
* `/var/lib/containerd` — слои образов компонентов DP и прочих контейнеров на узле. Чем больше компонентов и контейнеров, тем больше свободного места требуется в каталоге.
...
* `/var/lib/deckhouse/downloaded/` — хранилище конфигураций релизов модулей Deckhouse DP ([ModuleRelease](../documentation/v1/reference/api/cr.html#modulerelease));
...
* `/opt/deckhouse/` — служебные компоненты DP, такие как kubelet, containerd, статические утилиты (например, `lsblk`) и т.д.;
...
В таблице представлены рекомендуемые объёмы пространства для каталогов, используемых DP на master-узлах кластера.
...
В таблице представлены рекомендуемые объёмы пространства для каталогов, используемых DP на worker-узлах кластера.
...
Системные узлы (system-узел) — это узлы, на которых запускаются компоненты DP. При добавлении таких узлов в кластер учитывайте, что на них размещается нагрузка мониторинга, включая:
- [Prometheus](/modules/prometheus/);
- [loki](/modules/loki/);
- [upmeter](/modules/upmeter/) и другие компоненты DP.
...
В кластере DP можно настроить [локальное хранилище на узлах](../documentation/v1/admin/configuration/storage/sds/lvm-local.html) с использованием LVM.
