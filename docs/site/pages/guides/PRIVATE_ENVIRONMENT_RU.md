...
В качестве приватного registry используется [Harbor](https://goharbor.io/). Это container registry с открытым исходным кодом, который поддерживает настройку политик и управление доступом на основе ролей, проверяет образы на наличие уязвимостей и помечает их как надёжные. Harbor входит в состав проектов CNCF.
...
{% alert level="info" %}
Обратите внимание, что для доступа по этому доменному имени с рабочего компьютера также добавьте доменное имя `harbor.local` в `/etc/hosts`, указав в качестве назначения IP-адрес сервера Bastion.
{% endalert %}
...
{% alert level="warning" %}
Процесс загрузки образов занимает довольно продолжительное время, поэтому чтобы избежать проблем от прерывания сетевого соединения запустите его в сессии tmux или screen. В случае обрыва соединения по SSH с сервером Bastion можно будет просто подключиться к сессии, а не запускать всё заново. Обе утилиты обычно присутствуют в репозиториях дистрибутивов Linux и устанавливаются с помощью пакетного менеджера.

{% offtopic title="Как работать с tmux..." %}
* Запустите сессию командой `tmux`.
* Выход из сессии осуществляется сочетанием клавиш `Ctrl + b` и затем `d`. Сессия остаётся запущенной, выполняемые в ней процессы продолжают работать. Для полного выхода из сессии используется сочетание `Ctrl + d`, как и для любой терминальной сессии.
...
* Выход из сессии осуществляется сочетанием клавиш `Ctrl + a` и затем `d`, не отпуская `Ctrl`. Сессия остаётся запущенной, выполняемые в ней процессы продолжают работать. Для полного выхода из сессии используется сочетание `Ctrl + d`, как и для любой терминальной сессии.
...
Для подключения по SSH к серверу, к которому нет внешнего доступа, можно воспользоваться сервером Bastion в качестве jump-хоста.
...
1. *Подключение через jump-хост.* Выполните команду:
   ...
   В таком режиме сначала выполняется подключение к серверу Bastion, а затем с него — к целевому серверу с использованием того же SSH-ключа.
2. *Подключение в режиме агента.* Подключитесь к серверу Bastion командой:
...
Выполните команды от root-пользователя (введите публичную часть своего ssh-ключа):
...
Если вход выполнен успешно, значит пользователь создан корректно.
