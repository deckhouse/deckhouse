...
description: Руководство по установке Deckhouse Platform в закрытом окружении
lang: ru
layout: sidebar-guides
---

В этом руководстве описано, как развернуть кластер Deckhouse Platform в закрытом окружении без прямого доступа к хранилищу образов контейнеров DP (`registry.deckhouse.ru`) и внешним репозиториям deb/rpm-пакетов, используемых на узлах [поддерживаемых операционных систем](../documentation/v1/reference/supported_versions.html#linux).

{% alert level="warning" %}
Обратите внимание, что установка DP в закрытом окружении доступна в следующих редакциях: SE, SE+, EE, CSE Lite, CSE Pro.
{% endalert %}
...
```bash
d8 mirror push $(pwd)/d8-bundle 'harbor.example:443/deckhouse/<РЕДАКЦИЯ_DP>' --registry-login='robot$<ROBOT_ACCOUNT_NAME>' --registry-password='<PASSWORD>' --tls-skip-verify
```
...
Для выполнения установки DP добавьте на будущем master-узле пользователю, под которым будет выполняться установка платформы, публичную часть своего SSH-ключа в `.ssh/authorized_keys`.
...
* В секции `InitConfiguration` добавьте параметры доступа к registry:
  ...
  ```yaml
  deckhouse:
    # Адрес Docker registry с образами Deckhouse (укажите редакцию DP).
...
```bash
docker run --pull=always -it -v "$PWD/config.yml:/config.yml" -v "$HOME/.ssh/:/tmp/.ssh/" --network=host -v "$PWD/dhctl-tmp:/tmp/dhctl" harbor.example/deckhouse/<РЕДАКЦИЯ_DP>/install:stable bash
```
...
* Убедитесь, что все узлы кластера находятся в статусе `Ready`:
  ...
  Запуск всех компонентов DP после завершения установки может занять некоторое время.
...
Запуск Ingress-контроллера после завершения установки DP может занять некоторое время. Прежде чем продолжить, убедитесь, что Ingress-контроллер запустился (выполните на master-узле):
