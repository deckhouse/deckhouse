...
**Scope of application:** all levels of the Code platform — from individual projects and groups to instance configuration.  
...
- **UI and API access** — events can be viewed and filtered both in the administrator interface and through the dedicated API.

## Example Use Cases

- Tracking who and when changed a user’s access level in a Code project.
- Recording the creation and deletion of users.
- Detecting suspicious activity — for example, mass changes to employee email addresses or repository deletions.
- Monitoring changes to CI/CD environment variables.
...
Code also provides an API method to retrieve audit events with filtering and sorting options.

**Method:**  
`POST /api/v4/admin/audit_events/search`

**Description:**  
Returns a list of audit events. Filtering by dates, full-text search, and entity types is supported.
...
|-------------------------------|-------------------------------------------------------|-------------------------------------------------------------------------------------------|--------------------|
| 2fa_login_failed              | User 2fa login failed                                | Records a failed attempt to log in with two-factor authentication.                        |                    |
...
| deploy_token_created          | Deploy token created                                 | A deploy token was created for repository or registry access.                             |                    |
...
| member_permissions_updated    | Member access updated                                | A user’s role or membership expiration was updated.                                        |                    |
...
| user_rejected                 | User was rejected                                   | A user account registration was rejected.                                                  |                    |
