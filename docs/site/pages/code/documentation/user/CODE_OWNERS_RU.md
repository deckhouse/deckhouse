...
Функция **CODEOWNERS** позволяет определить пользователей и группы, которые являются ответственными за отдельные части репозитория. Изменения, затрагивающие файлы, указанные в `CODEOWNERS`, должны быть просмотрены и (опционально) апрувнуты владельцами кода.
