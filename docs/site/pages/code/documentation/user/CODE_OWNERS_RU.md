...
Если в проекте сущесвует несколько файлов и вы сейчас просматриваете не приоритетный то увидите соответвующее сообщение

![не приоритетный файл](/images/code/not_primary_code_owners_ru.png)

---

### Валидация файла CODEOWNERS

Валидация помогает на этапе редактирования увидеть ошибки в файле.

Если файл валиден отображается соответвующее сообщение

![Валидный файл](/images/code/code_owners_valid_ru.png)

Если файла невалиден будут отображены проблемные строки и тип ошибки
...
- *Пробелы в пути* - Если путь содержит пробелы нужно их экранировать символо `\`
- *Нет владельцев* - Не указано ни одного владельца
- *Незакрытый раздел* - Не достает символа закрытия раздела `]`
...
Deckhouse Code использует подстановачные символы:
