...
![неприоритетный файл](/images/code/not_primary_code_owners_ru.png)
...
![Валидный файл](/images/code/code_owners_valid_ru.png)

Если файл невалиден, будут отображены проблемные строки и тип ошибки:

![Невалидный файл](/images/code/code_owners_invalid_ru.png)
...
## Отключение проверок Code Owners

Проверки Code Owners можно отключить на уровне проекта. Это удобно, когда в проекте есть файл `CODEOWNERS`, но требовать апрувы от владельцев кода не нужно.

>**Как найти**: «Проект» → «Настройки» → «Запросы на слияние» → раздел дополнительных настроек правил апрува → настройка «Отключить проверки одобрения Code Owners».

Когда настройка включена, проверки Code Owners пропускаются во всём проекте:

- запрос на слияние не блокируется из-за отсутствующих апрувов от владельцев кода;
- владельцы кода не рассчитываются для изменённых файлов — список применимых записей пуст;
...
>**Как найти**: «Проект» → «Настройки» → «Запросы на слияние» → раздел «Автоматическое назначение проверяющих» → настройка «Автоматически назначать владельцев кода проверяющими».

Когда настройка включена, владельцы кода назначаются проверяющими:

- при создании запроса на слияние сразу готовым (не черновиком);
- при снятии с запроса на слияние статуса черновика.
...
- автор запроса на слияние проверяющим не назначается;
- назначаются только пользователи, у которых есть доступ на чтение запроса на слияние;
- назначается только первый владелец кода, если проект не разрешает нескольких проверяющих.
