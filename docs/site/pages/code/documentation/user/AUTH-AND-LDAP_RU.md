...
- `block_auto_created_users` — если `true`, автоматически созданные учётные записи блокируются до одобрения администратором.
  По умолчанию — `true`.
- `auto_link_ldap_user` — связывает учётную запись с учётной записью LDAP при первом входе (см. раздел [«Связывание учётных записей OIDC с LDAP»](#связывание-учётных-записей-oidc-с-ldap)).
  По умолчанию — `false`.
...
Обратите внимание на параметр `uid_field`: поле, указанное в нём, становится значением `uid` учётной записи, и именно это значение используется при поиске пользователя в LDAP. Оно должно совпадать со значением атрибута, указанного в параметре `uid` LDAP-сервера (в примере — `cn`), или с адресом электронной почты. Если параметр не задан, используется поле `sub`, которое обычно не совпадает ни с одним атрибутом LDAP.
