...
> **Примечание:** LDAP Sync не поддерживает транзитивность для вложенных групп. Подробности и решения см. в [FAQ: Вложенные группы и транзитивность](#nested-groups-and-transitivity).
...
### Как работает алгоритм синхронизации? {#sync-algorithm}

Синхронизация LDAP использует плоский, нерекурсивный алгоритм:

1. **Получение групп**: Один LDAP-запрос получает все группы по настроенным параметрам `base`, `filter` и `scope`.
2. **Извлечение участников**: Для каждой группы читаются атрибуты членства (`member`, `uniquemember`, `memberof`, `memberuid`, `submember`).
3. **Сопоставление пользователей**: Каждый DN из атрибутов сопоставляется с `Identity.extern_uid` в базе данных.
4. **Неизвестные DN пропускаются**: Если DN не соответствует известному пользователю (например, это DN вложенной группы), он молча игнорируется.



### Есть ли проблемы с циклами в иерархии групп? {#cycles}

Нет. Поскольку LDAP Sync читает группы плоско по фильтру, он не пытается воспроизвести древовидную структуру LDAP. Рекурсивный обход иерархии групп не выполняется, поэтому циклические зависимости между группами не вызывают проблем.

### Вложенные группы и транзитивность {#nested-groups-and-transitivity}

**LDAP Sync не поддерживает транзитивность.**

Если `Группа1` содержит `Группу2` в качестве члена, участники `Группы2` **не будут** автоматически добавлены в `Группу1`. Процесс синхронизации не разворачивает вложенные группы рекурсивно.

**Почему?** Deckhouse Code синхронизация читает атрибуты членства напрямую, без обхода вложенных групп. Ожидается, что все необходимые DN пользователей будут присутствовать в атрибутах членства группы.

**Решение:** Используйте внешние скрипты или механизмы на стороне LDAP-сервера для «выравнивания» вложенных членств. Например:
- Заполняйте атрибут `submember` всеми транзитивными участниками (включая участников вложенных групп).

Такой подход сохраняет простоту логики синхронизации, избегает проблем с рекурсией и даёт полный контроль над разрешением вложенных групп на стороне LDAP.
