...
| Какое значение ищется | В каком атрибуте LDAP |
| --------------------- | --------------------- |
...
Обратите внимание на параметр `uid_field`: поле, указанное в нём, становится значением `uid` учётной записи, и именно это значение используется при поиске пользователя в LDAP. Оно должно совпадать со значением атрибута, указанного в параметре `uid` LDAP-сервера (в примере — `cn`), или с адресом электронной почты — подробнее в разделе [«Как ищется учётная запись LDAP»](#как-ищется-учётная-запись-ldap).
