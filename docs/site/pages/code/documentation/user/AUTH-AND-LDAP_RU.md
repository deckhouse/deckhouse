...
2. В верхнем правом углу нажмите на кнопку  "Запустить" и подтвердите в диалоговом окне.
  ![Ldap sync worker UI](/images/code/ldap_sync_worker_ru.png)

Чтобы посмотреть информацию о том, как завершился процесс запущенной синхронизации, нужно зайти на страницу метрики задачи синхронизации LDAP
`/admin/sidekiq/metrics?substr=SyncWorker&period=8h`. На графике показана статистика успешных вызовов, а в таблице ниже статистика по количеству успешно завершенных и аварийно завершенных вызовов синхронизации LDAP
![Ldap sync worker metrics](/images/code/ldap_sync_metrics.png)

Чтобы посмотреть полные логи процесса синхронизации, нужно:
1. На странице воркера `/admin/sidekiq/cron/namespaces/default/jobs/ldap_sync_worker`  найти таблицу событий запусков с названием "История". В этой таблице первая строчка будет соответствовать
    последнему запуску. Необходимо выбрать значение в колонке JID (Job ID) первой строки - этот идентификатор понадобится для поиска по логам
   ![Ldap sync history table](/images/code/ldap_sync_history_ru.png)
2. Необходимо зайти на виртуальную машину, далее определить имя пода, с которого нужно собрать логи,
    используя команду `kubectl -n d8-code -l app.kubernetes.io/component=sidekiq get pod -o NAME`
3. Использовав идентификатор процесса синхронизации (JID) и имя pod (POD_NAME) выполнить команду для сбора логов `kubectl -n d8-code logs POD_NAME | jq 'select(.jid=="JID")'` 

> По прошествии некоторого количества времени старые логи удаляются ротацией,  и их сбор  становится невозможен. 
> При необходимости процесс синхронизации можно повторить и собрать актуальные логи.
