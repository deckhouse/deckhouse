...
- OIDC-провайдер, который передаёт email пользователя и список его групп.
- LDAP-каталог с теми же пользователями и с группами, имена которых позволяют определить роль (например, `dh-code-developer-backend`).
...
### Шаг 2. Настройте OIDC-провайдера

Конфигурация размещается в секции `spec.appConfig.omniauth.`. Параметры подключения к провайдеру задаются согласно [документации GitLab](https://docs.gitlab.com/integration/omniauth/), а доступ и административные права — параметрами `allowed_groups` и `admin_groups`:
...
### Шаг 3. Включите связывание с LDAP

Конфигурация размещается в секции `spec.appConfig.`:
...
### Шаг 4. Задайте расписание синхронизации
...
### Шаг 5. Проверьте связывание при первом входе
...
### Шаг 6. Дождитесь синхронизации прав
