...
Для проектов с видимостью `внутренний` авторизованный пользователь без признака внешнего пользователя
может читать ответы внешних проверок статуса в виджете на странице запроса на слияние,
если у него есть доступ на чтение данного запроса.

URL внешней проверки при этом отображается только пользователям с правом просмотра URL внешних проверок
(в стандартных ролях — `Разработчик` или выше), поэтому не отображается обычному пользователю,
который не является участником проекта.
