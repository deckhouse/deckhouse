...
Отсчет тайм-аута начинается в момент отправки запроса во внешний сервис. Если пользователь перезапускает проверку в состоянии `с ошибкой`, отсчет таймаута начинается с момента перезапуска.
...
| `external_status_check_response_updated` | Ответ изменен через обратный вызов внешнего сервиса, перезапуск, ошибку запроса или тайм-аут |
