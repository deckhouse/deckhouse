...
{% alert level="info" %}
Интеграция CodeScoring в Deckhouse Code покрывает SCA/OSA-сценарии: анализ зависимостей, обнаружение уязвимостей, генерацию SBOM и разбор на стороне платформы.
...
- Разбор и политики (пороги severity, игнорирование срабатываний) — на стороне платформы CodeScoring.
...
- В проекте/группе доступен GitLab Runner с executor `docker` — задача сканера выполняется в контейнере `debian:bookworm-slim`.

Подробнее о развёртывании сервера см. официальную документацию: [docs.codescoring.ru](https://docs.codescoring.ru/on-premise/).

{% alert level="info" %}
Агент Johnny **не нужно** устанавливать вручную: задача сканера скачивает его с вашего сервера CodeScoring по API-токену при каждом запуске.
{% endalert %}
...
1. Найдите раздел **CodeScoring** и откройте его.
...
Интеграция автоматически передаёт в пайплайн CI-переменные `FE_SCANS_CODESCORING_URL`, `FE_SCANS_CODESCORING_TOKEN`, `FE_SCANS_CODESCORING_CA_CERT`, `FE_SCANS_CODESCORING_PROJECT`, `FE_SCANS_CODESCORING_SCAN_STAGE` — задавать их вручную в `.gitlab-ci.yml` не нужно.
...
1. Привяжите проект политик к целевому проекту: **Настройки** → **Security policy**.

После этого в каждый пайплайн автоматически добавляется задача **`codescoring_scan`** (стадия `fe-security-scanner`), которая:

- скачивает агент Johnny с сервера CodeScoring (по токену; при самоподписанном — с переданным CA-сертификатом);
...
Одна задача `codescoring_scan` формирует все отчёты за один прогон. Deckhouse Code основан на GitLab FOSS, где часть EE-виджетов отсутствует, поэтому результаты выводятся так:
...
Задача `codescoring_scan` неблокирующая: она всегда завершается успешно и загружает отчёты (в том числе на упавших попытках — `artifacts:when: always`), не приводя к сбою пайплайна.

Настройка политик (40 критериев, пороги severity, разбор) и решение о блокировке выполняются на стороне платформы CodeScoring. Жёсткая блокировка пайплайна по нарушению политики — предмет отдельной настройки scan-execution-политики и в текущем шаблоне не включена.
...
Проверьте, что задача сформировала `gl-code-quality-report.json` и он объявлен в секции `artifacts:reports:codequality`.
