...
- **Автоматическая ротация** — секреты могут автоматически обновляться без изменения пайплайнов.
- **Гранулярный контроль доступа** — доступ к секретам ограничен на основе проекта, ветки, окружения и других параметров.
- **Отсутствие секретов в репозитории** — секреты никогда не попадают в код или переменные CI.

## Примеры использования

В этом разделе приведены типовые сценарии использования секретов из Vault в CI/CD-пайплайнах.

### Деплой приложения с учётными данными базы данных

Получение credentials для подключения к PostgreSQL во время деплоя:
...
Интеграция с внешними API (Slack, Telegram, S3):
...
Использование приватного ключа из Vault для подписания образов:
...
Настройка разных секретов для develop и main с использованием bound claims по ветке (`ref`):
...
Для разграничения доступа настройте роли в Vault с bound claims по ветке:
