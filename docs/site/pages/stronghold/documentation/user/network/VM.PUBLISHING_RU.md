...
permalink: ru/stronghold/documentation/user/network/vm-publishing.html
lang: ru
---
...
![NodePort](/images/stronghold/lb-nodeport.ru.png)
...
![LoadBalancer](/images/stronghold/lb-loadbalancer.ru.png)

### Использование сервисов с активными проверками

{% alert level='warning' %}Находится на стадии тестирования. Будет доступно в ближайших версиях.{% endalert %}
...
где `authSecretName` – это название секрета с учетными данными для доступа к PostgreSQL.
...
1. Ресурс `Ingress` для публикации. Пример:
...
2. Подключите приложение к `Dex`. Для этого добавьте в `Ingress`-ресурс приложения аннотации:
