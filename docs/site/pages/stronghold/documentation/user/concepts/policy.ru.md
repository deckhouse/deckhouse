...
Эти параметры можно использовать для установки минимальных/максимальных значений TTL, задаваемых клиентами при запросе на [обертывание ответа](response-wrapping.html), с точностью до секунды. Они используют строки формата duration.
