...
1. Используйте эндпоитн `/config`, чтобы настроить Stronghold на взаимодействие с новым кластером Kubernetes. Используйте `d8 kubectl cluster-info` для получения адреса хоста Kubernetes и TCP-порта.
