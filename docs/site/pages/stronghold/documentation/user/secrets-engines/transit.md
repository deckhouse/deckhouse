...
permalink: en/stronghold/documentation/user/secrets-engines/transit.html
lang: en
description: >-
  The Stronghold's Transit secrets engine in encrypts and decrypts data on-the-fly.
  It doesn't store secrets.
---
...
{% alert level="warning" %}
Stronghold HTTP API imposes a maximum request size of 32MB to prevent a DoS attack. This can be adjusted in the `listener` block in the [Stronghold configuration](../../admin/standalone/configuration.html#listener).
{% endalert %}
