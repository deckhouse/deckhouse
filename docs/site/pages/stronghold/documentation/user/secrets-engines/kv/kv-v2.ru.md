...
1. Чтение вернет самую свежую версию данных:
...
1. TКоманда `d8 stronghold kv patch` также поддерживает флаг `-method`, который можно использовать чтобы указать, какой метод использовать, `patch` или `rw`.
...
1. Чтение вернет самую новую версию, в которой были обновлены только заданные значения:
...
1. Предыдущие врерсии секретов можно получить используя флаг `-version`:
...
1. Создать секрет, используя политику `example`:
