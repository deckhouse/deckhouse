...
permalink: ru/stronghold/documentation/agent/key-features.html
lang: ru
---
...
1. Рендерит финальный файл, подставляя реальные значения.
1. Сохраняет файл с указанными правами доступа.
1. (Опционально) Выполняет команду для перезагрузки приложения.
...
| `env` | Получение ENV переменной   | `{{ env "HOME" }}` |
...
1. Выполняет аутентификацию (AppRole: читает role-id и secret-id)
1. Получает токен и сохраняет в sink (`/var/run/stronghold-agent/token`)
1. Запрашивает секреты из Stronghold
1. Рендерит шаблон и создает файл (`/etc/myapp/application.properties`)
1. **Завершается с кодом 0** (успех) благодаря флагу `-exit-after-auth`
...
1. Запрашивает секреты из Stronghold.
1. Формирует переменные окружения из шаблона.
1. Запускает приложение как дочерний процесс с этими переменными.
1. Отслеживает изменения секретов.
1. При изменении секретов - перезапускает приложение с новыми значениями.
...
- Простая миграция legacy-приложений на ENV-переменные.
...
1. Формирует новые ENV переменные.
1. Отправляет `SIGTERM` дочернему процессу (приложение должно корректно обрабатывать `SIGTERM`).
1. Перезапускает процесс с обновленными переменными.

## Кеширование и ротация токенов

Кеширование токена (Token Caching):
...
Agent получает токен с ограниченным сроком действия (например, 1 час) и заранее продлевает его. Если продление невозможно — Agent повторно аутентифицируется.
Это нужно для того, чтобы приложение могло работать непрерывно, без ручного вмешательства.

Автоматическое обновление секретов (Lease Renewal):

Динамические секреты (например, database credentials) тоже имеют срок действия.
Agent автоматически продлевает их перед истечением, затем обновляет файлы конфигурации и может перезагрузить приложение.
Таким образом, сredentials всегда актуальны, приложение не падает из-за истекших паролей.
...
1. Автоматически проходит аутентификацию в Stronghold.
1. Получает токен и использует его для своих операций (templating, API proxy).
1. Если настроен sink — записывает токен в файл для использования другими процессами. Sink  — это файл, куда Agent записывает полученный токен.
1. Автоматически обновляет токен перед истечением TTL.
1. При необходимости повторно аутентифицируется

Настройка sink опциональна:
- Если sink настроен – токен записывается в файл (например, `/var/run/stronghold-agent/token`), доступный для чтения другими процессами.
- Если sink не настроен – токен используется Agent только для внутренних операций (templating, caching).
...
- Ограничивайте доступ по CIDR (`secret_id_bound_cidrs`).
- Логируйте все использования Secret ID в Stronghold для аудита.
...
1. Используйте токены с `renewable=true` для автоматического обновления.
2. Установите разумный TTL (например, 30 дней).
3. Настройте explicit-max-ttl для ограничения общего времени жизни.
4. Регулярно проверяйте и отзывайте неиспользуемые токены.
5. Храните токен с минимальными правами доступа (0640).
6. Для production рассмотрите использование AppRole вместо token.
...
1. Настройте bound_audiences для защиты от переиспользования токенов.
