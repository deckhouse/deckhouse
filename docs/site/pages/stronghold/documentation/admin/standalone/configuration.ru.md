...
!!! Внимание
    Поля `tls_disable_client_certs` и `tls_require_and_verify_client_cert` в секции `listener` — взаимоисключающие. Обязательно убедитесь, что они оба не установлены в значение `true`. С настройками по умолчанию проверка клиента TLS не обязательна.  |
