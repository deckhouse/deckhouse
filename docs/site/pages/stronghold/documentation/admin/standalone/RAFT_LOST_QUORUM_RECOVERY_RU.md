...
Если вышедшие из строя серверы **поддаются восстановлению**, лучшим вариантом будет вернуть их в сеть и подключить к кластеру с использованием тех же адресов хостов. Это вернет кластер в полностью рабочее состояние. Для этого в файле `raft/peers.json` должны быть указаны данные: идентификатор сервера, *адрес:порт* и информация о возможности голосовать, для каждого сервера, который вы хотите включить в кластер.
