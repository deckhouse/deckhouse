...
Для включения узла в эту группу нужно назначить ему соответствующий лейбл:
...
Сопоставление политики и виртуальных машин происходит по лейблам.

Чтобы добавить виртуальную машину в политику, необходимо назначить для нее лейбл, например:
...
spec:
  ...
  selectors:
    - podSelector:
        matchLabels:
          # Данная политика будет применена для всех подов с лейблом app=backend в пространстве имён default на всех виртуальных машинах.
          app: backend
