...
1. Создайте диск виртуальной машины из образа, созданного на предыдущем шаге (Внимание: перед созданием убедитесь, что в системе присутствует StorageClass по умолчанию):
   ...
   Статус диска в этом случае `СОЗДАНИЕ 0%`, но диск уже можно будет выбирать при создании ВМ; подробности — в [разделе «Диски»](/products/virtualization-platform/documentation/user/resource-management/disks.html)
   {% endalert %}
...
- `Running` — виртуальная машина запущена.
  ...
  - Особенности:
    ...
    - Условие `type: AwaitingRestartToApplyConfiguration, status: True` отображает информацию о необходимости выполнить вручную перезагрузку ВМ, так как некоторые изменения конфигурации невозможно применить без перезагрузки ВМ.
  - Возможные проблемы:
...
| `.spec.cpu.cores`                       | Может применяться сразу при включённом hotplug (EE, SE+); подробности — в [разделе «Горячее подключение CPU»](#горячее-подключение-cpu); иначе требуется перезапуск ВМ |
...
Чтобы размещать ВМ по зонам доступности (а не привязывать к конкретным узлам), задайте `topologyKey: topology.kubernetes.io/zone` (подробности — в [разделе «Размещение ВМ по зонам доступности»](/products/virtualization-platform/documentation/user/resource-management/virtual-machines.html#размещение-вм-по-зонам-доступности)).
...
{% alert level="info" %}
При настройке сетевых интерфейсов в гостевой ОС используйте стабильные идентификаторы (предсказуемые имена `enpXsY` или привязку по MAC-адресу) вместо имён `ethX`. Подробнее — в разделе [Именование сетевых интерфейсов в гостевой ОС](#именование-сетевых-интерфейсов-в-гостевой-ос).
{% endalert %}
...
Подробнее — в документации [IP sysctl](https://docs.kernel.org/networking/ip-sysctl.html).
{% endalert %}
