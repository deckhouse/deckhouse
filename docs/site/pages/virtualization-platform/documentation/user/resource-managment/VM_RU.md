...
- `false` — используется эмуляция стандартных устройств (SATA для дисков, e1000e для сетевых интерфейсов), что может быть необходимо для совместимости со старыми ОС.
...
Deckhouse Virtualization Platform поддерживает `qemu-guest-agent` версии 5.2.0 и выше. Текущую версию агента можно посмотреть в гостевой ОС, выполнив команду:
...
- Простое связывание по лейблам (`nodeSelector`) — базовый способ выбора узлов с заданными лейблами.
...
- Учитывайте существующие ограничения (`taints`) на узлах. При необходимости можно добавить соответствующие `tolerations` для ВМ.
...
Чтобы размещать ВМ по зонам доступности (а не привязывать к конкретным узлам), задайте `topologyKey: topology.kubernetes.io/zone` (см. [раздел «Размещение ВМ по зонам доступности»](/products/virtualization-platform/documentation/user/resource-management/virtual-machines.html#размещение-вм-по-зонам-доступности)).

Как задавать «предпочтения» и «обязательности» для размещения виртуальных машин в веб-интерфейсе в [разделе «Размещение»](/products/virtualization-platform/documentation/user/resource-management/virtual-machines.html#размещение-вм-по-узлам):
...
Как настроить предотвращение совместного размещения ВМ на узлах в веб-интерфейсе в [разделе «Размещение»](/products/virtualization-platform/documentation/user/resource-management/virtual-machines.html#размещение-вм-по-узлам):
...
- В секции «Загрузочные диски» доступны следующие действия:
  ...
  - `Изменить размер` — изменить размер диска;
  - `Изменить порядок` — изменить порядок загрузки с устройств.
...
Горячее подключение диска невозможно, пока виртуальная машина находится в процессе живой миграции. В этом случае ресурс `VirtualMachineBlockDeviceAttachment` остаётся в фазе `Pending`, а в условии `Attached` указывается причина `BlockedByMigration`. Подключение продолжится автоматически после завершения миграции.
...
- Горячее подключение дисков: Во время живой миграции подключение дисков к ВМ через ресурс `VirtualMachineBlockDeviceAttachment` недоступно. Ресурс остаётся в фазе `Pending` с причиной `BlockedByMigration` в условии `Attached` до завершения миграции.
...
- `Attached` — ресурс [VirtualMachineIPAddress](/modules/virtualization/cr.html#virtualmachineipaddress) подключен к ресурсу [VirtualMachine](/modules/virtualization/cr.html#virtualmachine).
...
- параметры сети (IP-адреса, шлюзы, DNS и т.д.) для дополнительных сетей настраиваются вручную изнутри гостевой ОС (например, с помощью Cloud-Init), если для сети не настроен IPAM (подробнее — в подразделе [«IPAM для дополнительных сетевых интерфейсов»](#ipam-для-дополнительных-сетевых-интерфейсов)).
...
#### Настройка гостевой ОС для работы с hotplug-интерфейсами

При подключении дополнительного сетевого интерфейса через hotplug (после того как ВМ уже запущена) гостевая ОС должна быть настроена на автоматический подъём новых сетевых интерфейсов и запрос DHCP-аренды. Linux по умолчанию не запускает DHCP-клиент на hotplug-интерфейсах.
