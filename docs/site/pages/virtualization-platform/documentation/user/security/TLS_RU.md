...
permalink: ru/virtualization-platform/documentation/user/security/tls.html
lang: ru
---

Deckhouse Virtualization Platform (DVP) предоставляет встроенные средства управления TLS-сертификатами,
упрощающие настройку и управление шифрованием трафика в приложениях, работающих в кластере.

На этой странице описаны следующие аспекты использования сертификатов в DVP:
...
1. Отредактируйте [настройки модуля `cert-manager`](/modules/cert-manager/configuration.html), добавив следующую секцию:
   ...
   После этого DVP автоматически создаст ClusterIssuer и Secret для Cloudflare в пространстве имён `d8-cert-manager`.

1. Создайте ресурс Certificate с проверкой с помощью провайдера Cloudflare.
   Данная возможность появится только при указании настройки `cloudflareGlobalAPIKey` и `cloudflareEmail` в DVP:
...
1. Создайте ресурс Certificate с проверкой с помощью провайдера Route53.
   Данная возможность появится только при указании настроек `route53AccessKeyID` и `route53SecretAccessKey` в DVP:
...
2. Использовать стойкие криптографические алгоритмы, например, алгоритмы на основе эллиптических кривых (в том числе `prime256v1`).
...
Если вы не хотите хранить учётные данные в конфигурации DVP,
...
DVP поддерживает аннотацию `kubernetes.io/tls-acme: "true"` в ресурсах Ingress.
