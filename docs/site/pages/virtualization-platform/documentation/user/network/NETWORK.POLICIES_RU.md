...
Правила сетевых политик применяются одновременно, по принципу их складывания, для всех виртуальных машин, которые соответствуют указанным лейблам.

Дальнейшие примеры будут показаны на примере проекта `test-project` с двумя виртуальными машинами в неймспейсе `test-project`.
...
Виртуальные машины имеют соответствующие им лейблы.
...
Сетевая политика, ограничивающая весь входящий трафик для виртуальных машин с лейблом `vm-a`, в неймспейсе `test-project`:
...
Сетевая политика, разрешающая исходящий трафик от виртуальных машин с лейблом `vm-a` до внешнего адреса 8.8.8.8:
...
## Разрешение входящего трафика между неймспейсами

Сетевая политика разрешает входящий трафик до виртуальных машин с лейблом `vm: a` из неймспейса `another-project`, которое имеет соответствующий лейбл `kubernetes.io/metadata.name: another-project`.
