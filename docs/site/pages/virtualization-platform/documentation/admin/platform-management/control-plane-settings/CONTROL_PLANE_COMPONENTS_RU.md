...
Компоненты управляющего слоя (control plane) отвечают за основные операции кластера (например, планирование), а также обрабатывают события кластера (например, запускают новый под, когда количество реплик в Deployment не соответствует запущенному количеству реплик).
...
Работа с etcd, в том числе, резервное копирование данных, разобрана в разделе [Настройки etcd](etcd.ru).
