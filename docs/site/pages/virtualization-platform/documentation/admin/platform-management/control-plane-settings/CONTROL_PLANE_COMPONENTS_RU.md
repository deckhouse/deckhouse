...
Компоненты управляющего слоя могут быть запущены на любой машине в кластере. Однако для упрощения настройки и поддержки кластера все компоненты управляющего слоя запускаются на выделенных узлах, где запрещают запуск пользовательских контейнеров. Если компоненты запущены на одном узле, это конфигурация `single-master`. Если компоненты запустить на нескольких узлах, то это переводит управляющий слой в режим высокой доступности `multi-master`.
...
Как и kube-apiserver, etcd поддерживает запуск в нескольких экземплярах для обеспечения высокой доступности, образуя etcd-кластер.
...
Компонент запускает контроллеры, которые взаимодействуют с основными облачными провайдерами.
...
- **Обновление или возврат к предыдущей версии компонентов**. Поддерживает в кластере одинаковые версии компонентов.
...
- **Настройка kubeconfig**. Обеспечивает всегда актуальную конфигурацию для работы kubectl. Генерирует, продлевает, обновляет kubeconfig с правами cluster-admin и создает символическую ссылку для пользователя root, чтобы kubeconfig использовался по умолчанию.
...
- Клиентские и серверные сертификаты для подключения компонентов управляющего слоя друг к другу. Сертификаты выпускаются, продлеваются и перевыпускаются, если что-то изменилось (например, список SAN). Следующие сертификаты хранятся только на узлах:
  ...
  - клиентский сертификат для подключения `kubelet` к `etcd` для healthcheck'ов (`etcd/healthcheck-client.crt` и `etcd/healthcheck-client.key`).
