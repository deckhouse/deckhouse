...
Алгоритм принятия решения планировщиком разбит на 2 фазы: Filtering и Scoring (на самом деле их больше, есть еще pre-filtering / post-filtering, но для простоты и без потери смысла можно рассмотреть две фазы).
...
Поду назначается выбранный узел.
...
По умолчанию, если узел в течении 40 секунд не сообщает свое состояние, он помечается как недоступный. Ещё через 5 минут поды такого узла будут назначены планировщиком на другие узлы ("переселены"). Итоговое время недоступности приложений составляет около 6 минут.
