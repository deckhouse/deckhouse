...
1. Установка и обновление и настройка ПО узла (containerd, kubelet и др.), подключение узла в кластер:
   * Установка операционной системы (см. [список поддерживаемых ОС](../../../about/requirements.html#поддерживаемые-ос-для-узлов-платформы)) вне зависимости от типа используемой инфраструктуры (в любом облаке или на физическом оборудовании).
...
Ознакомиться с другими типами узлов и возможностями работы с облачными провайдерами можно [в документации платформы Deckhouse](/products/kubernetes-platform/documentation/v1/modules/node-manager).
...
Разбиение узлов по группам и распределение компонентов по группам узлов зависит от задач кластера. Примеры конфигураций кластера платформы виртуализации можно посмотреть в разделе [Установка платформы](../../install/steps/base-cluster.html).
...
1. **Подготовка ресурсов.**
   ...
   Перед тем, как передать bare-metal-сервер или виртуальную машину под управление CAPS, может понадобиться предварительная подготовка, например:
...
1. **Создание ресурса [SSHCredentials](../../../../reference/cr/sshcredentials.html).**
...
Подробно о всех параметрах можно прочитать в описании Custom Resource [NodeGroup](../../../../reference/cr/nodegroup.html).
...
{% alert level="warning" %}
Запрещено использовать домен `deckhouse.io` в ключах `labels` и `taints` у NodeGroup. Домен `deckhouse.io` зарезервирован для компонентов **Deckhouse**. Используйте ключ `dedicated` или `dedicated.client.com`.
{% endalert %}
