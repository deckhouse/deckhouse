...
- **Отсутствует заказ узлов.** Непосредственное выделение ресурсов (серверов bare metal) выполняется вручную. Дальнейшая настройка ресурсов (подключение узла к кластеру, настройка мониторинга и т.п.) выполняется автоматически или частично.
...
- **[NodeGroup](cr.html#nodegroup)**. Секция параметров [staticInstances](todo cr.html#nodegroup-v1-spec-staticinstances) определяет необходимое количество узлов в группе и фильтр множества ресурсов `StaticInstance` которые могут использоваться в группе.
...
2. **Создание ресурса [SSHCredentials](../../../reference/cr.html#sshcredentials).**
...
3. **Создание ресурса [StaticInstance](../../../reference/cr.html#staticinstance).**
...
4. **Создание ресурса [NodeGroup](../../../reference/cr.html#nodegroup).**
...
**Scaling** — рассчитывается только для облачных провайдеров, для групп узлов с типом ```CloudEphemeral```.
