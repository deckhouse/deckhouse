...
subjects:
- kind: User
  name: joe@example.com # параметр .spec.email для пользователя joe из users.deckhouse.io 
...
kind: RoleBinding
metadata:
  name: project-admin-vms-admins
  namespace: vms
subjects:
- kind: Group
  name: vms-admins # параметр .spec.name для группы  vms-admins из groups.deckhouse.io
  apiGroup: rbac.authorization.k8s.io
roleRef:
  kind: ClusterRole
  name: d8:use:role:admin
...
- `ACCESS_LEVEL` — уровень доступа.
  ...
  - `d8:manage:all:manager` — аналогично роли `d8:manage:all:viewer`, только доступ на уровне `admin`, то есть просмотр/создание/изменение/удаление конфигурации всех модулей DVP (ресурсы `moduleConfig`), их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes во всех системных пространствах имён (начинающихся с `d8-` или `kube-`);
...
roleRef:
  kind: ClusterRole
  name: d8:manage:all:manager # название manage-роли
  apiGroup: rbac.authorization.k8s.io
```
...
Чтобы назначить права менеджера сетевой подсистемы пользователю `joe`, можно использовать следующую настройку:
...
roleRef:
  kind: ClusterRole
  name: d8:manage:networking:manager # название manage-роли
  apiGroup: rbac.authorization.k8s.io
```
...
В случае, если в [ClusterAuthorizationRule](/modules/user-authz/cr.html#clusterauthorizationrule) используется `namespaceSelector`, параметры `limitNamespaces` и `allowAccessToSystemNamespaces` не учитываются.
