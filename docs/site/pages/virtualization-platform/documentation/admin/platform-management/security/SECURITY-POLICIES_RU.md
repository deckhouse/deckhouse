...
Политики применяются к ним иначе, чем к прикладным неймспейсам, и это поведение не настраивается.
Неймспейс с лейблом `heritage: deckhouse` считается системным, даже если назван иначе:
платформа ставит этот лейбл на создаваемые ею неймспейсы, и политика, написанная для прикладных неймспейсов, до него не доходит.

Каждый неймспейс с именем `d8-*` или `kube-*` проверяется по стандарту `restricted` в режиме `warn`.
...
Управляющие ими лейблы и сама нагрузка принадлежат владеющему неймспейсом модулю,
и Deckhouse возвращает их к заявленному состоянию при следующем применении конфигурации.
Модуль сам выводит из-под проверки собственную нагрузку — ресурсом SecurityPolicyException, который поставляет вместе с ней.

Ресурсы OperationPolicy и SecurityPolicy в системных неймспейсах тоже действуют в режиме `warn`.
Политика с `enforcementAction: Deny` блокирует запуск нагрузки в прикладных неймспейсах,
а в системном неймспейсе только сообщает о нарушении.
Модуль может включить принудительное применение политик в собственном неймспейсе:
там политика с `enforcementAction: Deny` блокирует нагрузку так же, как в прикладном неймспейсе.

Политика с `enforcementAction: Deny`, которая доходит до системных неймспейсов,
разворачивается в несколько ограничений Gatekeeper,
...
Префиксы `d8-system-default-`, `d8-system-enforce-`, `d8-system-excluded-` и `d8-pod-security-` зарезервированы:
политика, имя которой начинается с одного из них, отклоняется при создании.
По той же причине имя политики ограничено 234 символами:
имена производных ограничений должны укладываться в 253 символа, отведённые имени объекта Kubernetes.
