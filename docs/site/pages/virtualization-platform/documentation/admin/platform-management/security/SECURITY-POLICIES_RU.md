...
Создаваемые платформой неймспейсы дополнительно помечены лейблом `heritage: deckhouse`. Неймспейс с этим лейблом, но с другим именем не попадает под политики и мутации, написанные для прикладных неймспейсов; Pod Security Standards ниже действуют по именам.
...
Нарушение фиксируется в отчётах безопасности и отображается в веб-интерфейсе Deckhouse, нагрузка при этом запускается.
...
Управляющие ими лейблы и сама нагрузка принадлежат владеющему неймспейсом модулю, и Deckhouse Platform возвращает их к заявленному состоянию при следующем применении конфигурации.
...
Лейблы неймспейса на это не влияют: модуль, ужесточающий проверки в собственном неймспейсе, поднимает там Pod Security Standards, а к политике, написанной для прикладной нагрузки, это отношения не имеет.
...
Префиксы `d8-system-default-` и `d8-pod-security-` зарезервированы: политика, имя которой начинается с одного из них, отклоняется при создании.
По той же причине имя политики ограничено 235 символами: имена производных ограничений должны укладываться в 253 символа, отведённые имени объекта Kubernetes.
...
- перечень неймспейсов использует маску в начале имени, например `*-system`, которую нельзя точно пересечь с `d8-*`, — тогда единственное ограничение сохраняет действие политики и просто исключает системные неймспейсы;
- политика выбирает только системные неймспейсы — тогда единственное ограничение сохраняет имя политики и предупреждает.

Мутации Gatekeeper в системных неймспейсах не применяются, какие бы лейблы на неймспейсе ни стояли.
Платформа задаёт параметры своих компонентов сама, поэтому ресурсам `Assign` и `ModifySet` изменять их там не разрешено.
