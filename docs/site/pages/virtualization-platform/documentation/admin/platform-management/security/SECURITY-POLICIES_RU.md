...
поэтому применяемые к ним политики определяются отдельно от настроек выше.
...
То же правило распространяется на ресурсы OperationPolicy и SecurityPolicy, и важнее всего это
для политики, чей селектор охватывает все неймспейсы. Такая политика с `enforcementAction: Deny`
запрещает запуск нагрузки в обычных неймспейсах, а в системных подчиняется параметру
`systemNamespaces.enforcementAction` — то есть по умолчанию только предупреждает и не может
случайно уронить компонент платформы. Там, где модуль включил применение политик для своего
неймспейса, политика сохраняет собственное действие; там, где оператор исключил неймспейс,
она только предупреждает.

При таком разделении рядом с исходным появляются дополнительные констрейнты, видимые в отчётах
безопасности и в веб-интерфейсе Deckhouse: `d8-system-default-<политика>` для системных неймспейсов,
которые не выбрал ни один модуль, `d8-system-enforce-<политика>` для выбранных модулем
и `d8-system-excluded-<политика>` для перечисленных в `excludeNamespaces`. Первые два сливаются в один,
когда `systemNamespaces.enforcementAction` совпадает с действием самой политики.
...
если все перечисленные системные неймспейсы уже исключены в ней, либо если этот перечень
использует маску в начале имени, например `*-system`, которую нельзя точно пересечь с `d8-*`.

Мутации Gatekeeper в системных неймспейсах не применяются независимо от наличия лейбла.
Параметры своих компонентов платформа задаёт сама, поэтому ресурсам `Assign` и `ModifySet`,
изменяющим эти параметры, работать там не разрешено.
