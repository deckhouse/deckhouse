...
- `dryrun` — не влияет на выполнение действий и используется для отладки. Информацию о событиях можно посмотреть в веб-интерфейсе Deckhouse Platform или с помощью команды `d8 k`.
...
Модуль сам выводит из-под проверки собственную нагрузку — ресурсом SecurityPolicyException, который поставляется вместе с модулем.
...
По той же причине имя политики ограничено 235 символами: имена производных ограничений должны укладываться в 253 символа, отведённые для имени объекта Kubernetes.
...
- политика выбирает только системные неймспейсы — тогда единственное ограничение сохраняет имя политики и только предупреждает о нарушениях.

Мутации Gatekeeper в системных неймспейсах не применяются, какие бы лейблы на неймспейсе ни стояли.
Платформа задаёт параметры своих компонентов сама, поэтому ресурсам `Assign` и `ModifySet` не разрешено изменять их параметры.
