...
Deckhouse Virtualization Platform supports integration with the Zabbix monitoring system. For this purpose, an external script is used that receives alerts from Deckhouse via `kubectl` and passes them to Zabbix using the Zabbix agent.

The following are required for the script to work:
...
1. Check `zabbix` user permissions. Ensure the agent runs the script under a user with the necessary cluster access rights:
