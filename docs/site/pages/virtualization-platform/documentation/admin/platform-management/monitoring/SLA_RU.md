...
- `smoke-mini` — проверка сетевой связности между узлами. Размещаются пять StatefulSet с одной репликой. Тест проверяет связность между подами `smoke-mini` и с подами `upmeter-agent` на master-узлах. Раз в минуту один из подов `smoke-mini` переносится на другой узел.
