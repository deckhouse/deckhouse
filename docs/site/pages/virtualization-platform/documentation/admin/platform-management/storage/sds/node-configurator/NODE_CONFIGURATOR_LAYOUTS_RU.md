...
После запуска автоматического обнаружения в кластере появятся три CRD-ресурса типа [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup) с автоматически сгенерированными именами.

Чтобы вывести список ресурсов [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup), выполните следующую команду:
...
Чтобы сконфигурировать `sds-local-volume` в режиме «Полное зеркало», создайте ресурс [LocalStorageClass](/products/kubernetes-platform/modules/sds-local-volume/stable/cr.html#localstorageclass) и укажите в нём все обнаруженные [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup). Это гарантирует, что VG с меткой `main` будет доступен на каждом узле в модуле:
...
1. Создайте ресурс [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) и добавьте в него все ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
...
1. Создайте ресурс [ReplicatedStorageClass](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstorageclass) и в поле `storagePool` укажите имя созданного ранее ресурса [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool):
...
1. Добавьте подготовленный узел в кластер DVP.
   ...
   который определит VG `main-safe` и `main-unsafe` и добавит соответствующие этим VG ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup) в кластер DVP.
   Дальше ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup) можно использовать для создания томов в модулях `sds-replicated-volume` или `sds-local-volume`.
...
В кластере DVP при этом появятся шесть ресурсов [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup) со случайно сгенерированными именами.
В будущем добавится возможность указывать имя для ресурсов [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
...
Чтобы настроить модуль `sds-local-volume` по сценарию «Частичное зеркало», создайте ресурс [LocalStorageClass](/products/kubernetes-platform/modules/sds-local-volume/stable/cr.html#localstorageclass)
и добавьте в него ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup), чтобы на всех узлах в модуле `sds-local-volume` использовалась только VG `main-safe`:
...
1. Создайте ресурс [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) с именем `data-safe` и добавьте в него ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
   чтобы на всех узлах в модуле `sds-replicated-volume` в [ReplicatedStorageClass](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstorageclass) с параметром `replication: None`
...
1. Создайте ресурс [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) с именем `data-unsafe` и добавьте в него ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
   чтобы на всех узлах в модуле `sds-replicated-volume` в [ReplicatedStorageClass](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstorageclass) с параметром `replication: Availability` или
...
1. Создайте ресурс [ReplicatedStorageClass](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstorageclass) и в поле `storagePool` укажите имя созданных ранее ресурсов [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool),
...
В кластере DVP при этом появятся три ресурса [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup) со случайно сгенерированными именами.
В будущем добавится возможность указывать имя для ресурсов [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
которые создаются в процессе автоматического обнаружения VG, с помощью тега `LVM` с желаемым именем ресурса.
...
Чтобы настроить модуль `sds-local-volume` по сценарию «Полное зеркало», создайте ресурс [LocalStorageClass](/products/kubernetes-platform/modules/sds-local-volume/stable/cr.html#localstorageclass)
и добавьте в него все ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup), чтобы VG `<vg-name>` использовалась на всех узлах в модуле `sds-local-volume`:
...
Примеры информативных имен ресурса [LocalStorageClass](/products/kubernetes-platform/modules/sds-local-volume/stable/cr.html#localstorageclass) для дополнительных дисков разных типов:
...
1. Создайте ресурс [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) и добавьте в него все ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
   ...
   >
   > Примеры информативных имен ресурса [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) для дополнительных дисков разных типов:
   >
...
Чтобы настроить модуль `sds-local-volume` по сценарию «Частичное зеркало», создайте ресурс [LocalStorageClass](/products/kubernetes-platform/modules/sds-local-volume/stable/cr.html#localstorageclass)
и добавьте в него ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup), чтобы на всех узлах в модуле `sds-local-volume` использовалась только VG `<vg-name>-safe`:
...
1. Создайте ресурс [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) с именем `data-<vg-name>-safe` и добавьте в него ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
   чтобы на всех узлах в модуле `sds-replicated-volume` в [ReplicatedStorageClass](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstorageclass) с параметром `replication: None`
...
1. Создайте ресурс [ReplicatedStoragePool](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstoragepool) с именем `data-<vg-name>-unsafe` и добавьте в него ресурсы [LVMVolumeGroup](/products/kubernetes-platform/modules/sds-node-configurator/stable/cr.html#lvmvolumegroup),
   чтобы на всех узлах в модуле `sds-replicated-volume` в [ReplicatedStorageClass](/products/kubernetes-platform/modules/sds-replicated-volume/stable/cr.html#replicatedstorageclass) с параметром `replication: Availability` или
