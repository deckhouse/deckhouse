...
Размещение подов `sds-local-volume-csi-node` управляется специальными лейблами (`nodeSelector`). Эти лейблы задаются в параметре [`spec.settings.dataNodes.nodeSelector`](/modules/sds-local-volume/stable/configuration.html#parameters-datanodes-nodeselector) модуля.
