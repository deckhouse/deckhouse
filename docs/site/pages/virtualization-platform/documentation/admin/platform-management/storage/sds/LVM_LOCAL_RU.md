...
Чтобы включить модуль, примените следующий ресурс `ModuleConfig`:
...
Размещение данных подов по узлам определяется на основе специальных меток (nodeSelector), которые указываются в поле `spec.settings.dataNodes.nodeSelector` в настройках модуля. Для получения более подробной информации о настройке перейдите по [ссылке](todo,mc).

Перед тем как приступить к настройке создания StorageClass’ов, необходимо объединить доступные на узлах блочные устройства в группы томов LVM (в дальнейшем группы томов будут использоваться для размещения PersistentVolume’ов).
...
Создание StorageClass’ов осуществляется через ресурс `LocalStorageClass`, который определяет конфигурацию для желаемого класса хранения. Ручное создание ресурса StorageClass без `LocalStorageClass` может привести к нежелательному поведению.

При создании `LocalStorageClass` важно выбрать тип хранения, который может иметь значение `Thick`, либо `Thin`.

Thick-пул обладает высокой производительностью, сравнимой с производительностью накопителя, но не позволяет использовать снапшоты, в то время как Thin-пул позволит использовать снапшоты и overprovisioning, но производительность будет ниже.
