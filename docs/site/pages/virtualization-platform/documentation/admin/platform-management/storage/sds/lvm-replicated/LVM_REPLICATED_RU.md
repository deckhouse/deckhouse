...
Чтобы проверить, что в неймспейсах `d8-sds-replicated-volume` и `d8-sds-node-configurator` все ВМ в состоянии `Running` или `Completed` и запущены на всех узлах, где планируется использовать ресурсы DRBD, можно использовать команды:
