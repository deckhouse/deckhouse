...
1. Скопируйте или переместите файл резервной копии etcd [`etcd-backup.snapshot`](#создание-резервных-копий-с-помощью-deckhousecli) в домашнюю директорию текущего пользователя (root):
...
В следующем примере `etcd-backup.snapshot` — [файл резервной копии etcd](#резервное-копирование-etcd), `infra-production` — неймспейс, в котором нужно восстановить объекты.
...
1. Установите нужные переменные. В текущем примере:
   ...
   - `infra-production` — неймспейс, в пределах которого выполняется поиск ресурсов.
...
Deckhouse автоматически выполняет ежедневное резервное копирование etcd с помощью CronJob, запускаемого в поде `d8-etcd-backup` в неймспейсе `kube-system`. В рамках задания создаётся снимок базы данных, архивируется и сохраняется локально на узле в директории `/var/lib/etcd/`:
...
- Секреты и ConfigMap'ы, из неймспейсов, начинающихся на `d8-` или `kube-`, если они явно перечислены в файле whitelist.
...
Для успешной выгрузки `d8` обращается к Loki API от имени ServiceAccount `loki` в неймспейсе `d8-monitoring`, используя секрет с токеном.
