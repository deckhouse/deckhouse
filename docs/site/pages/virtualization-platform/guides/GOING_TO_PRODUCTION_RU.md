...
- **Программно-определяемые хранилища (SDS)**:
  - `[sds-local-volume](../../../modules/sds-local-volume/stable/)` — локальное хранилище на основе LVM. Высокая производительность, но без репликации. Подходит для временных данных или при наличии внешнего бэкапа.
  - `[sds-replicated-volume](../../../modules/sds-replicated-volume/stable/)` — реплицируемое хранилище на основе DRBD. Обеспечивает отказоустойчивость за счет репликации между узлами. Рекомендуется для production.
