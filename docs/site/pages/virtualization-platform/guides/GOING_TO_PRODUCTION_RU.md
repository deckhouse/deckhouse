...
Учтите, что при использовании программно определяемых хранилищ, на worker-узлах потребуются дополнительные диски для хранения данных.
{% endalert %}

| Тип узла             | Назначение узлов                                                                                                | Требования                                                                                                                                                                                                                                         |
| -------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Мастер-узел**      | Запуск компонент плоскости уроавления платформы                                                                 | 4 CPU, 8 ГБ RAM, 60 ГБ дискового пространства на быстром диске (400+ IOPS);                                                                                                                                                                        |
| **Системный узел**   | Запуска компонент модулей платформы                                                                             | - 2 CPU, 4 ГБ RAM, 50 ГБ дискового пространства — если в кластере есть выделенные узлы мониторинга;<br>- 4 CPU, 8 ГБ RAM, 60 ГБ дискового пространства на быстром диске (400+ IOPS) — если в кластере нет выделенных узлов мониторинга.            |
| **Worker-узел**      | Запуска рабочей нагрузки пользователей (ВМ)                                                                     | требования аналогичны требованиям к master-узлу, но во многом зависят от характера запускаемой на узле (узлах) нагрузки. При использовании программноопределяемых хранилищ, рекомендуется использовать узлы с дополнительными дисками (400+ IOPS). |
| **Frontend-узел**    | Запуска Ingress-контроллеров, для обработки входящего трафика                                                   | 2 CPU, 4 ГБ RAM, 50 ГБ дискового пространства;                                                                                                                                                                                                     |
| **Узел мониторинга** | Запуска системных компонент подсистемы мониторинга, рекомендуется использовать для высоконагруженных кластеров. | 4 CPU, 8 ГБ RAM; 50 ГБ дискового пространства на быстром диске (400+ IOPS).                                                                                                                                                                        |                                                                                                                                                        |
...
Добавление узлов в кластер, осуществляется согласно следующей [инструкции](/products/kubernetes-platform/documentation/latest/modules/node-manager/#работа-со-статическими-узлами).
