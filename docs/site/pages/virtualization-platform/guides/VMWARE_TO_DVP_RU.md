...
1. [Загрузка образа диска в кластер](#загрузка-образа-диска-в-кластер) как ресурса VirtualDisk.
1. [Создание виртуальной машины](#создание-виртуальной-машины) (ресурс VirtualMachine), которая загружается с этого диска.
...
- машина с Linux (или другая среда), где можно установить `virt-v2v` и `libguestfs`, и достаточно места на диске под распаковку дистрибутива виртуальной машины `OVA` (либо для отдельных файлов дисков) и для размещения каталога с результатом конвертации;
...
Установите инструменты на рабочую станцию для для конвертации:
...
Утилита `virt-v2v` в режиме `-i disk` обрабатывает локальный `VMDK` и сохраняет результат в указанный каталог. Для конвертации выполните команду:
...
После конвертации в каталоге `./out` появится файл вида `./out/machine.qcow2` (точное имя может совпадать с именем исходной ВМ из метаданных). Этот файл далее загружается в кластер.
...
Последний шаг — описание запускаемой ВМ: сколько CPU и памяти нужно, к какой сети подключиться и какой диск считать загрузочным. Конфигурация VMware (`OVF`/`VMX`) напрямую не импортируется. Перенесите параметры вручную по таблице соответствий ниже и с помощью примера YAML-манифеста виртуальной машины (ресурс VirtualMachine).
