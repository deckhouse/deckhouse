...
2. **`replace`** — the first matching regex substitutes the href. Applied to **every** link, relative or absolute. A rule may rewrite an absolute URL into a relative one (e.g. `https://deckhouse.ru/documentation/v1/foo.html` → `/reference/api/foo.html`) — the result then skips the whitelist check.
