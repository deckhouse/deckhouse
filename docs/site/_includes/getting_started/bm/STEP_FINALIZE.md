...
<li><strong>Create DNS records</strong> to organize access to the cluster web-interfaces:
  ...
  <div>
  {% alert type="info" %}
  The command below applies when physical servers are used as cluster nodes. If the cluster nodes are virtual machines, use a different method to identify the public IP address of the node running the Ingress Controller.
  {% endalert %}
  </div>
  <div class="highlight">
