...
<ul>
  <li>
<p>Выполните на <strong>master-узле</strong> следующую команду, для того чтобы снять с него <i>taint</i> и разрешить остальным компонентам Deckhouse работать на master-узле (если taint не был снят автоматически в конфигурации):</p>
<div markdown="1">
```bash
