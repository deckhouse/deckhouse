...
1. **Физический сервер или виртуальная машина для master-узла.**
   ...
   - SSH-доступ от **персонального компьютера** (пункт 1) по ключу;
   - сетевой доступ от **персонального компьютера** (пункт 1) по порту `22/TCP`;
...
1. **Физический сервер или виртуальная машина для worker-узла.**
   ...
   Также на worker-узлах требуются дополнительные диски для развертывания программно определяемого хранилища, как минимум один диск без разметки емкостью не менее 200 ГБ.
