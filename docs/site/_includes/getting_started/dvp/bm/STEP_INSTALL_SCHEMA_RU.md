...
<ol>
  ...
  <li><p><strong>Персональный компьютер</strong> — машина для установки, с которой будет производиться установка Deckhouse Virtualization Platform. Персональный компьютер нужен только для запуска инсталлятора и не будет частью кластера.</p>
  ...
  Некоторые дистрибутивы (например, Astra Linux 1.7.4) не соответствуют этим требованиям, и ОС на узлах необходимо привести в соответствие требованиям перед установкой Deckhouse Virtualization Platform. Подробнее — [в документации](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#clusterconfiguration-defaultcri).
  {% endalert %}
  ...
  <li><p><strong>Сервер NFS</strong> — внешний сервер сетевого файлового хранилища (Network File System), используемый для хранения дисков виртуальных машин и данных компонентов кластера (метрики, логи и т.д.). NFS-сервер обеспечивает централизованное внешнее хранилище, доступное со всех узлов кластера.</p>
    {% offtopic title="Требования..." %}
