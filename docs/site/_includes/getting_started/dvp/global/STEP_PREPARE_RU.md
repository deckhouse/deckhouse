...
Для успешного развертывания кластера DVP необходимо определить подсети подов (`podSubnetCIDR`), подсети сервисов (`serviceSubnetCIDR`), узлов кластера (`internalNetworkCIDRs`), и подсети виртуальных машин (`virtualMachineCIDRs`).
Для подсетей подов и сервисов возможно использовать значения по умолчанию.

{% alert level="warning" %}
Подсети подов, сервисов, узлов кластера и виртуальных машин не должны пересекаться.
{% endalert %}

### Оставшиеся поля для теста

```bash
# Подсеть подов
echo "Pod Subnet CIDR: <POD_SUBNET_CIDR>"

# Подсеть сервисов
echo "Service Subnet CIDR: <SERVICE_SUBNET_CIDR>"

# Подсеть узлов кластера
echo "Internal Network CIDRs: <INTERNAL_NETWORK_CIDRS>"

# Подсеть виртуальных машин
echo "Virtual Machine CIDRs: <VIRTUAL_MACHINE_CIDRS>"
```

## Доменное имя DNS кластера

Используются для формирования доменов системных приложений в кластере. Например, Grafana для шаблона `%s.domain.my` будет доступна как `grafana.domain.my`.
...
Если у вас нет возможности завести wildcard-записи DNS, для тестирования вы можете воспользоваться, например, сервисом [sslip.io](https://sslip.io/) или аналогами.

{% alert level="info" %}
Создание DNS-записи для доступа в веб-интерфейсы кластера описано в [документации](/products/virtualization-platform/gs/bm/step7.html).
{% endalert %}
