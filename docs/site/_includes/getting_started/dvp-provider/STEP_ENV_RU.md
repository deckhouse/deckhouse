...
Чтобы развернуть Deckhouse Platform поверх виртуализации Deckhouse Platform и иметь интеграцию (например, заказ узлов для вложенного кластера в виртуализации), необходимо выполнить предварительную настройку. Для этого создайте пользователя (ServiceAccount), назначьте ему права и получите kubeconfig.
