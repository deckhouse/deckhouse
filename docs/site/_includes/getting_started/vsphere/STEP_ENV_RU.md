...
{% alert level="info" %}
Теги, разметку datastore и сервисную роль Deckhouse также можно настроить через **клиента VMware vSphere** по инструкции [«Настройка через vSphere Client»](/modules/cloud-provider-vsphere/environment.html#настройка-через-vsphere-client) в документации модуля. Ниже описана настройка через **`govc`**.
{% endalert %}
