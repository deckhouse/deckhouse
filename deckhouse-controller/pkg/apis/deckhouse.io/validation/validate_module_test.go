...
// TestModuleV1alpha2ValidationHandler verifies the restrictions on manual changes to
// v1alpha2 Modules: creating and deleting one is forbidden, and editing one is
// forbidden while its module config exists, because the module config reconciler owns
// the module spec. The deckhouse service account is exempt from all three.
func TestModuleV1alpha2ValidationHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:        "user is forbidden from creating a module",
			operation:   "CREATE",
			username:    "kubernetes-admin",
			wantAllowed: false,
			wantMessage: "manual Module creation is forbidden",
		},
		{
			name:        "deckhouse service account is allowed to create a module",
			operation:   "CREATE",
			username:    deckhouseServiceAccount,
			wantAllowed: true,
		},
		{
			name:        "user is forbidden from editing a module managed by a module config",
			operation:   "UPDATE",
			username:    "kubernetes-admin",
			withConfig:  true,
			wantAllowed: false,
		...
		{
			name:        "deckhouse service account is allowed to edit a module managed by a module config",
			operation:   "UPDATE",
			username:    deckhouseServiceAccount,
			withConfig:  true,
			wantAllowed: true,
		...
		{
			name:        "user is forbidden from deleting a module",
			operation:   "DELETE",
			username:    "kubernetes-admin",
			wantAllowed: false,
		...
		{
			name:        "deckhouse service account is allowed to delete a module",
			operation:   "DELETE",
			username:    deckhouseServiceAccount,
			wantAllowed: true,
		},
