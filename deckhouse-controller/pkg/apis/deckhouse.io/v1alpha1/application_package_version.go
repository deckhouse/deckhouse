...
func (a *ApplicationPackageVersion) SetConditionTrue(condType string, time metav1.Time) *ApplicationPackageVersion {
	for idx, cond := range a.Status.Conditions {
		if cond.Type == condType {
			a.Status.Conditions[idx].LastProbeTime = time
			if cond.Status != corev1.ConditionTrue {
				a.Status.Conditions[idx].LastTransitionTime = time
				a.Status.Conditions[idx].Status = corev1.ConditionTrue
			}
			a.Status.Conditions[idx].Reason = ""
			a.Status.Conditions[idx].Message = ""

			return a
		}
	}

	a.Status.Conditions = append(a.Status.Conditions, ApplicationPackageVersionCondition{
		Type:               condType,
...
func (a *ApplicationPackageVersion) SetConditionFalse(condType string, time metav1.Time, reason string, message string) *ApplicationPackageVersion {
	for idx, cond := range a.Status.Conditions {
		if cond.Type == condType {
			a.Status.Conditions[idx].LastProbeTime = time
			if cond.Status != corev1.ConditionFalse {
				a.Status.Conditions[idx].LastTransitionTime = time
				a.Status.Conditions[idx].Status = corev1.ConditionFalse
			}
			a.Status.Conditions[idx].Reason = reason
