...
func TestCalculateVersionLag(t *testing.T) {
	tests := []struct {
		name           string
		currentRelease *v1alpha1.ModuleRelease
		otherReleases  []*v1alpha1.ModuleRelease
		updatePolicy   *v1alpha2.ModuleUpdatePolicy
		expectedLag    int
	}{
		{
			name: "no pending releases",
			currentRelease: &v1alpha1.ModuleRelease{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-module-v1.2.0",
					Labels: map[string]string{
						v1alpha1.ModuleReleaseLabelModule:       "test-module",
						v1alpha1.ModuleReleaseLabelSource:       "test-source",
						v1alpha1.ModuleReleaseLabelUpdatePolicy: "manual-policy",
					},
				},
		...
		{
			name: "auto mode - no lag calculated",
			currentRelease: &v1alpha1.ModuleRelease{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-module-v1.2.0",
					Labels: map[string]string{
						v1alpha1.ModuleReleaseLabelModule:       "test-module",
						v1alpha1.ModuleReleaseLabelSource:       "test-source",
						v1alpha1.ModuleReleaseLabelUpdatePolicy: "auto-policy",
					},
				},
				Spec: v1alpha1.ModuleReleaseSpec{
					ModuleName: "test-module",
		...
		{
			name: "major version difference - no lag calculated",
			currentRelease: &v1alpha1.ModuleRelease{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-module-v1.2.0",
					Labels: map[string]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			objects := []client.Object{tt.currentRelease, tt.updatePolicy}
			for _, release := range tt.otherReleases {
			...
			result, err := r.calculateVersionLag(ctx, tt.currentRelease)
			require.NoError(t, err)
			assert.Equal(t, tt.expectedLag, result)
		})
	}
}
