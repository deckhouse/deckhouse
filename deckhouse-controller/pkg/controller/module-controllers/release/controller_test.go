...
func (suite *ReleaseControllerTestSuite) loopUntilDeploy(dc *dependency.MockedContainer, releaseName string) {
	...
	// Setting hasAppliedReleases field in real code causes the process to reboot.
	...
	for result.Requeue || result.RequeueAfter > 0 || suite.ctr.hasAppliedReleases.Load() {
		suite.ctr.hasAppliedReleases.Store(false)
...
`
		...
		// Initialize restartCooldown as done in controller
		suite.ctr.restartCooldown.Store(1)
		...
		go func() {
			for {
				...
				case <-suite.ctr.delayTicker.C:
					...
					if suite.ctr.restartCooldown.Load() == 0 {
						restartCalled.Store(true)
						suite.ctr.hasAppliedReleases.Store(false)
						return
					}

					if suite.ctr.hasAppliedReleases.Load() && suite.ctr.activeApplyCount.Load() == 0 {
						suite.ctr.restartCooldown.Store(0)
					}
		...
		applyReleaseWithControl := func(releaseName string) {
			...
			defer func() {
				suite.ctr.activeApplyCount.Add(-1)
				suite.ctr.hasAppliedReleases.Store(true)
			}()
		...
		require.False(t, suite.ctr.hasAppliedReleases.Load(), "hasAppliedReleases should be reset after restart")

		// Verify that the cooldown mechanism worked - cooldown should be 0 after restart
		require.Equal(t, int32(0), suite.ctr.restartCooldown.Load(), "cooldown should be 0 after restart triggered")
	})
}
