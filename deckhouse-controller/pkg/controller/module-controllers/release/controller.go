...
func (r *reconciler) handleDeployedRelease(ctx context.Context, release *v1alpha1.ModuleRelease) (ctrl.Result, error) {
	...
	if versionLag, err := r.calculateVersionLag(ctx, release); err == nil && versionLag > 0 {
		metricLabels[releaseUpdater.VersionLag] = strconv.Itoa(versionLag)
	}
...
// calculateVersionLag calculates minor versions delta keeping in mind logic with pre-checks
func (r *reconciler) calculateVersionLag(ctx context.Context, currentRelease *v1alpha1.ModuleRelease) (int, error) {
	logger := r.log.With(
		slog.String("module", currentRelease.GetModuleName()),
		slog.String("current_release", currentRelease.GetName()),
		slog.String("current_version", currentRelease.GetVersion().String()),
		slog.String("current_phase", currentRelease.Status.Phase),
	)

	// ensure manual mode
	updatePolicy, err := r.getUpdatePolicy(ctx, currentRelease.Labels[v1alpha1.ModuleReleaseLabelUpdatePolicy])
	if err != nil {
		logger.Info("failed to get update policy", log.Err(err))
		return 0, fmt.Errorf("get update policy: %w", err)
	}

	if updatePolicy.Spec.Update.Mode != "Manual" {
		logger.Info("skipping version lag calculation: not Manual mode", slog.String("mode", updatePolicy.Spec.Update.Mode))
		return 0, nil
	}

	// fetch all module releases
	moduleReleases := new(v1alpha1.ModuleReleaseList)
	labelSelector := client.MatchingLabels{
		v1alpha1.ModuleReleaseLabelModule: currentRelease.GetModuleName(),
		v1alpha1.ModuleReleaseLabelSource: currentRelease.Labels[v1alpha1.ModuleReleaseLabelSource],
	}

	if err := r.client.List(ctx, moduleReleases, labelSelector); err != nil {
		logger.Info("failed to list module releases", log.Err(err))
		return 0, fmt.Errorf("list module releases: %w", err)
	}

	logger.Info("found module releases", slog.Int("count", len(moduleReleases.Items)))

	// for deployed releases: find latest pending release
	var latestPendingRelease *v1alpha1.ModuleRelease
	var latestDeployedRelease *v1alpha1.ModuleRelease
	currentVersion := currentRelease.GetVersion()

	for _, rel := range moduleReleases.Items {
		relVersion := rel.GetVersion()
		logger.Info("examining release",
			slog.String("release", rel.GetName()),
			slog.String("version", relVersion.String()),
			slog.String("phase", rel.Status.Phase),
		)

		// ignore modules with different major version, this is done on purpose
		if relVersion.Major() != currentVersion.Major() {
			logger.Info("skipping release: different major version",
				slog.Uint64("rel_major", relVersion.Major()),
				slog.Uint64("current_major", currentVersion.Major()),
			)
			continue
		}

		// skip self
		if rel.GetName() == currentRelease.GetName() {
			logger.Info("skipping self")
			continue
		}

		if rel.Status.Phase == v1alpha1.ModuleReleasePhasePending {
			if latestPendingRelease == nil || relVersion.GreaterThan(latestPendingRelease.GetVersion()) {
				relCopy := rel
				latestPendingRelease = &relCopy
				logger.Info("found newer pending release", slog.String("pending_version", relVersion.String()))
			}
		}

		if rel.Status.Phase == v1alpha1.ModuleReleasePhaseDeployed {
			if latestDeployedRelease == nil || relVersion.GreaterThan(latestDeployedRelease.GetVersion()) {
				relCopy := rel
				latestDeployedRelease = &relCopy
				logger.Info("found newer deployed release", slog.String("deployed_version", relVersion.String()))
			}
		}
	}

	// For deployed releases: compare with latest pending
	if currentRelease.Status.Phase == v1alpha1.ModuleReleasePhaseDeployed {
		if latestPendingRelease == nil {
			logger.Info("no pending releases found for deployed release")
			return 0, nil
		}

		latestPendingVersion := latestPendingRelease.GetVersion()
		if latestPendingVersion.LessThan(currentVersion) || latestPendingVersion.Equal(currentVersion) {
			logger.Info("pending release is not newer than deployed",
				slog.String("pending_version", latestPendingVersion.String()),
			)
			return 0, nil
		}

		lag := int(latestPendingVersion.Minor() - currentVersion.Minor())
		logger.Info("calculated version lag for deployed release",
			slog.Int("lag", lag),
			slog.String("pending_version", latestPendingVersion.String()),
		)
		return lag, nil
	...
	if currentRelease.Status.Phase == v1alpha1.ModuleReleasePhasePending {
		logger.Info("skipping version lag calculation for pending release")
		return 0, nil
	}

	logger.Info("release phase not handled", slog.String("phase", currentRelease.Status.Phase))
	return 0, nil
}
