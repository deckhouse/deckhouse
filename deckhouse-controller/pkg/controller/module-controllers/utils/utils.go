...
var ErrClisterIsBootstrappedFieldIsNotFound = errors.New("secret has no clusterIsBootstrapped field")
...
func ParseDeckhouseRegistrySecret(data map[string][]byte) (*DeckhouseRegistrySecret, error) {
	...
	if !ok {
		err = errors.Join(err, ErrClisterIsBootstrappedFieldIsNotFound)
	}
