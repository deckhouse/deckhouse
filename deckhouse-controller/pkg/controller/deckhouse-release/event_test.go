	...
	t.Run("Update Event", func(t *testing.T) {
		tests := []struct {
		...
		}{
			{
				...
				arg: event.UpdateEvent{
					ObjectOld: nil,
					ObjectNew: &v1alpha1.DeckhouseRelease{
						TypeMeta: metav1.TypeMeta{
							Kind:       v1alpha1.DeckhouseReleaseKind,
							APIVersion: v1alpha1.DeckhouseReleaseGVK.GroupVersion().String(),
						},
			...
			{
				// Probably impossible scenario, because ObjectOld can't be created (spec.version: Required value)
				name: "Release without annotations",
				arg: event.UpdateEvent{
					ObjectOld: &v1alpha1.DeckhouseRelease{
						TypeMeta: metav1.TypeMeta{
							Kind:       v1alpha1.DeckhouseReleaseKind,
							APIVersion: v1alpha1.DeckhouseReleaseGVK.GroupVersion().String(),
						},
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				got := eventFilter.Update(test.arg)
				if got != test.expected {
					t.Errorf("Event:\n%v\ngot %t\nexpected %t", string(must(yaml.Marshal(test.arg))), got, test.expected)
				}
			})
