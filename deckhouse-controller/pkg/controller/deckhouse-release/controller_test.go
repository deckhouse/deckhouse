...
func (suite *ControllerTestSuite) TestWebhookNotifications() {
	...
	suite.Run("Notification: patch release uses ReleaseApplyTime instead of ReleaseApplyAfterTime", func() {
		// This test verifies that notification for patch releases uses ReleaseApplyTime (the actual deploy time)
		// instead of ReleaseApplyAfterTime. When window is set, these times can differ:
		// ReleaseApplyTime is adjusted by window, while ReleaseApplyAfterTime may retain the notification period time.
		var httpBody string
		var webhookData updater.WebhookData
		svr := httptest.NewServer(http.HandlerFunc(func(_ http.ResponseWriter, r *http.Request) {
			data, _ := io.ReadAll(r.Body)
		...
		suite.setupControllerSettings("notifier-webhook-patch-release-apply-time.yaml", initValues, ds)
		dr := suite.getDeckhouseRelease("v1.26.0")
		_, err := suite.ctr.createOrUpdateReconcile(ctx, dr)
		require.NoError(suite.T(), err)
		...
		require.Contains(suite.T(), httpBody, "New Deckhouse Release 1.25.1 is available")
		require.Contains(suite.T(), httpBody, `"version":"1.25.1"`)
		require.Contains(suite.T(), httpBody, `"subject":"Deckhouse"`)
	})
}
