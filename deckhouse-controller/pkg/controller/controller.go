...
func (c *DeckhouseController) Start(ctx context.Context) error {
	// give the old module stack its package system objects and fill the
	// module v2 resources before any controller runs; the sync reads through
	// the API reader, so it does not need the manager cache
	if _, err := pkgsync.Sync(ctx, c.runtimeManager.GetAPIReader(), c.runtimeManager.GetClient(), c.dc, app.Version, app.EmbeddedModulesDir, c.log.Named("pkgsync")); err != nil {
