...
func TestComputeAndApplyConditions_InitialInstall(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			appCurrentVersion: "",
			internalConditions: []status.Condition{
				{Name: status.ConditionDownloaded, Status: metav1.ConditionTrue},
				{Name: status.ConditionReadyOnFilesystem, Status: metav1.ConditionTrue},
				{Name: status.ConditionReadyInRuntime, Status: metav1.ConditionTrue},
				{Name: status.ConditionReadyInCluster, Status: metav1.ConditionTrue},
			},
		...
		},
		{
			...
			internalConditions: []status.Condition{
				...
				{Name: status.ConditionReadyOnFilesystem, Status: metav1.ConditionTrue},
				{Name: status.ConditionReadyInRuntime, Status: metav1.ConditionTrue},
				{Name: status.ConditionReadyInCluster, Status: metav1.ConditionFalse},
			},
			expectedVersion:         "",
...
func TestComputeAndApplyConditions_Update(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			internalConditions: []status.Condition{
				{Name: status.ConditionDownloaded, Status: metav1.ConditionTrue},
				{Name: status.ConditionReadyOnFilesystem, Status: metav1.ConditionFalse},
				{Name: status.ConditionReadyInRuntime, Status: metav1.ConditionFalse},
				{Name: status.ConditionReadyInCluster, Status: metav1.ConditionFalse},
			},
			expectedVersion:         "1.0.0",
			expectedVersionCommited: false,
