...
// processing") and only Enable clears it — which never runs for an application
// the scheduler switched off. Reporting "waiting to converge" for an
// application that is not queued behind anything would be a lie. The chain is
// consulted for membership because updatePipeline deliberately omits
// RequirementsMet: the verdict is not re-checked on a version change.
