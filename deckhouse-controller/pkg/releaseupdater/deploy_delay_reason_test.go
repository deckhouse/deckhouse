...
func TestDeployDelayReason(t *testing.T) {
	...
	require.Equal(t, "Release is waiting for the update window until 2019-10-17T15:33:00Z", reason.Message(deckhouseRelease, now))
	...
	require.Equal(t, "Release is waiting for the update window, waiting for the 'release.deckhouse.io/approved: \"true\"' annotation. After approval the release will be delayed until 2019-10-17T15:33:00Z", reason.Message(deckhouseRelease, now))
	require.Equal(t, "Release is waiting for the update window, waiting for the 'modules.deckhouse.io/approved: \"true\"' annotation. After approval the release will be delayed until 2019-10-17T15:33:00Z", reason.Message(moduleRelease, now))
