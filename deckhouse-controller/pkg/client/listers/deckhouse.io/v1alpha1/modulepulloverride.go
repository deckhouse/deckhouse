...
type ModulePullOverrideLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ModulePullOverride, err error)
	// Get retrieves the ModulePullOverride from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ModulePullOverride, error)
...
// List lists all ModulePullOverrides in the indexer.
func (s *modulePullOverrideLister) List(selector labels.Selector) (ret []*v1alpha1.ModulePullOverride, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ModulePullOverride))
	})
	return ret, err
}

// Get retrieves the ModulePullOverride from the index for a given name.
func (s *modulePullOverrideLister) Get(name string) (*v1alpha1.ModulePullOverride, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("modulepulloverride"), name)
	}
	return obj.(*v1alpha1.ModulePullOverride), nil
}
