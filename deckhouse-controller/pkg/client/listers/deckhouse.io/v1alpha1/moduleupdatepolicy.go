...
type ModuleUpdatePolicyLister interface {
	...
	List(selector labels.Selector) (ret []*deckhouseiov1alpha1.ModuleUpdatePolicy, err error)
	// Get retrieves the ModuleUpdatePolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*deckhouseiov1alpha1.ModuleUpdatePolicy, error)
...
type moduleUpdatePolicyLister struct {
	listers.ResourceIndexer[*deckhouseiov1alpha1.ModuleUpdatePolicy]
}
...
func NewModuleUpdatePolicyLister(indexer cache.Indexer) ModuleUpdatePolicyLister {
	return &moduleUpdatePolicyLister{listers.New[*deckhouseiov1alpha1.ModuleUpdatePolicy](indexer, deckhouseiov1alpha1.Resource("moduleupdatepolicy"))}
}
