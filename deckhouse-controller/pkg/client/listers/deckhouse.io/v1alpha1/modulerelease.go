...
type ModuleReleaseLister interface {
	...
	List(selector labels.Selector) (ret []*deckhouseiov1alpha1.ModuleRelease, err error)
	// Get retrieves the ModuleRelease from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*deckhouseiov1alpha1.ModuleRelease, error)
...
type moduleReleaseLister struct {
	listers.ResourceIndexer[*deckhouseiov1alpha1.ModuleRelease]
}
...
func NewModuleReleaseLister(indexer cache.Indexer) ModuleReleaseLister {
	return &moduleReleaseLister{listers.New[*deckhouseiov1alpha1.ModuleRelease](indexer, deckhouseiov1alpha1.Resource("modulerelease"))}
}
