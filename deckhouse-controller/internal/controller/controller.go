...
import (
	...
	"github.com/deckhouse/deckhouse/deckhouse-controller/internal/controller/apps/application"
	apppackageversion "github.com/deckhouse/deckhouse/deckhouse-controller/internal/controller/apps/packageversion"
...
func (c *Controller) Build(ctx context.Context, rest *rest.Config, ms metricsstorage.Storage, logger *log.Logger) (*Controller, error) {
	...
	err = apppackageversion.RegisterController(runtime, dc, logger)
	if err != nil {
		return nil, fmt.Errorf("register application package version controller: %w", err)
	}

	err = modulepackageversion.RegisterController(runtime, dc, logger)
	if err != nil {
	...
	err = moduleconfig.RegisterController(synced, runtime, settingsCh, logger)
	if err != nil {
...
func (c *Controller) Start(ctx context.Context) error {
	...
	if err := c.restoreModulesByOverrides(ctx); err != nil {
		return fmt.Errorf("restore modules by overrides: %w", err)
	}

	if err := c.restoreModulesByReleases(ctx); err != nil {
