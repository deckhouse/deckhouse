...
// Package cel implements Deckhouse-specific extensions to the
// x-deckhouse-validations CEL validator that ships with addon-operator.
//
// The upstream validator (github.com/flant/addon-operator/pkg/values/validation/cel)
// only exposes the current value as the "self" variable. This package adds
// transition rule semantics: it additionally exposes the previous value as
// "oldSelf" and is intended to be invoked alongside the upstream validator,
// so callers can express immutability and other update-time invariants
// without forking addon-operator.
//
// To avoid double-evaluating rules that only reference "self", this package
// only fires rules whose CEL AST references "oldSelf". Self-only rules are
// left to the upstream validator.
package cel
...
// ruleKey is the OpenAPI extension key that carries CEL validation rules for
// Deckhouse schemas. It matches the key used by the upstream addon-operator
// validator so the same schema can be processed by both validators.
...
// stored ModuleConfig settings) and powers transition rules /
// immutability checks.
const (
	selfVar    = "self"
	oldSelfVar = "oldSelf"
...
// rule is a single CEL validation entry in an x-deckhouse-validations
// extension list. Expression is a CEL expression over "self" (and optionally
// "oldSelf"); Message is returned when the expression is false.
type rule struct {
	Expression string `json:"expression" yaml:"expression"`
	Message    string `json:"message" yaml:"message"`
}

// ValidateTransition evaluates x-deckhouse-validations CEL rules attached to
// schema and its nested properties recursively, exposing the current value
// as "self" and the previous value as "oldSelf" inside expressions.
//
// To avoid duplicating evaluation already performed by the upstream
// addon-operator validator, only rules whose AST references "oldSelf"
// (transition rules) are evaluated here; rules that mention only "self"
// are silently skipped and assumed to be handled upstream.
//
// Transition rules are evaluated only when an old value is available at the
// same level (i.e. on updates) and silently skipped otherwise (e.g. on
// create or for newly added properties). This mirrors the semantics of
// x-kubernetes-validations and allows expressing immutability with a rule
// like:
//
...
//
// It returns validation errors for every rule that evaluated to false, and
// a non-nil error for configuration or evaluation failures.
func ValidateTransition(schema *spec.Schema, values, oldValues any) ([]error, error) {
	...
	for _, r := range rules {
		...
		// Only transition rules (rules referencing oldSelf) are evaluated
		// here. Self-only rules are left to the upstream addon-operator
		// validator that is run by the caller.
		if !expressionUsesOldSelf(ast) {
			continue
		}
...
// transition rules so they can be skipped when no previous value is
// available, and so that non-transition rules can be deferred to the
// upstream validator.
