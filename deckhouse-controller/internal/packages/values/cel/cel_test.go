...
func TestValidateTransition_NoRules(t *testing.T) {
	s := schemaFromYAML(t, `
type: object
properties:
  foo:
...
func TestValidateTransition_PlainSelfRuleSkipped(t *testing.T) {
	// Self-only rules are handled by the upstream addon-operator validator,
	// so this local validator must ignore them even when they would fail.
	s := schemaFromYAML(t, `
type: object
properties:
  replicas:
...
`)
	...
	errs, err := ValidateTransition(s, values, nil)
	require.NoError(t, err)
	assert.Empty(t, errs, "self-only rules must be deferred to the upstream validator")
}
...
func TestValidateTransition_OnlyTransitionRulesFireWithOldValues(t *testing.T) {
	// Mixing plain rules and transition rules: this local validator fires
	// only transition rules even when oldValues are provided; plain rules
	// stay the responsibility of the upstream validator.
	s := schemaFromYAML(t, `
type: object
properties:
  replicas:
...
`)
	...
	errs, err := ValidateTransition(s, values, oldValues)
	require.NoError(t, err)
	assert.Equal(t, []string{"mode is immutable"}, joinMessages(errs))
}
...
func TestValidateTransition_InvalidExtensionShape(t *testing.T) {
...
`)
	...
	_, err := ValidateTransition(s, values, nil)
	assert.Error(t, err)
}

func TestValidateTransition_MissingExpression(t *testing.T) {
	s := schemaFromYAML(t, `
type: object
x-deckhouse-validations:
