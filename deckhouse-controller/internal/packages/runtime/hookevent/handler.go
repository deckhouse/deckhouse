...
type TaskBuilder interface {
	BuildScheduleTasks(ctx context.Context, crontab string) map[string][]queue.Task
	BuildKubeTasks(ctx context.Context, kubeEvent shkubetypes.KubeEvent) map[string][]queue.Task
}
		...
		go func() {
			...
			for {
				...
				case crontab := <-h.scheduleManager.Ch():
					...
					res = h.taskBuilder.BuildScheduleTasks(h.ctx, crontab)
				...
				case kubeEvent := <-h.kubeEventsManager.Ch():
					...
					res = h.taskBuilder.BuildKubeTasks(h.ctx, kubeEvent)
				}
