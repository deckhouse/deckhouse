...
// EncodeResponse sends value in the format the output query parameter asks for: JSON when
// it is absent or "json", YAML when it is "yaml". Any other value is refused, so
// a typo does not silently return the default.
func EncodeResponse(w http.ResponseWriter, req *http.Request, value any) {
	...
	// The dumps are read with curl as often as by a program, and a body without a
	// trailing newline runs into the next shell prompt. yaml.Marshal ends with one
	// already, json.Marshal does not.
