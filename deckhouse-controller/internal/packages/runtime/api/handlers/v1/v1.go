...
// NewPublicHandler mounts the subtrees whose responses carry no package values:
// queues, scheduler and requirements.
func NewPublicHandler(deps Deps) chi.Router {
	router := chi.NewRouter()

	mountPublic(router, deps)

	return router
}

// NewPrivateHandler mounts the public subtrees plus the packages subtree, whose
// responses carry registry credentials, rendered Secrets and hook snapshots.
func NewPrivateHandler(deps Deps) chi.Router {
	router := chi.NewRouter()

	mountPublic(router, deps)
	router.Mount("/packages", packages.NewHandler(deps.Packages))

	return router
}

// mountPublic mounts the subtrees both transports serve.
func mountPublic(router chi.Router, deps Deps) {
	router.Mount("/queues", queues.NewHandler(deps.Queues))
	router.Mount("/scheduler", scheduler.NewHandler(deps.Scheduler))
	router.Mount("/requirements", requirements.NewHandler(deps.Requirements))
}
