...
func NewTask(pkg packageI, ns string, nelm nelmI, status *status.Registry, logger *log.Logger) queue.Task {
	return &task{
		pkg:       pkg,
		namespace: ns,
		nelm:      nelm,
...
func (t *task) runPackage(ctx context.Context) error {
	...
	if err := t.nelm.Upgrade(ctx, t.namespace, t.pkg); err != nil && !errors.Is(err, nelm.ErrPackageNotHelm) {
		span.SetStatus(codes.Error, err.Error())
		t.status.HandleError(t.pkg.GetName(), status.ConditionHelmApplied, err)
		return err
	}
	...
	if err := t.pkg.RunHooksByBinding(ctx, addontypes.AfterHelm); err != nil {
		span.SetStatus(codes.Error, err.Error())
		t.status.HandleError(t.pkg.GetName(), status.ConditionHooksReady, newAfterHelmHookErr(err))
		return err
	}
