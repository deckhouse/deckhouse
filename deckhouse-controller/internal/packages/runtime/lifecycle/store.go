...
// is new, the version changed, the settings checksum differs, or the maintenance
// mode changed. Used as a fast-path check before the more expensive Update call.
func (s *Store) NeedUpdate(name, version, checksum, maintenance string) bool {
...
//
// Returns nil when only settings or maintenance changed (no new context needed —
// the new values are stored and will be picked up by the scheduler via
// GetPendingSettings/GetPendingMaintenance on next Reschedule, or by the next
...
func (s *Store) Update(name, version string, settings addonutils.Values, maintenance string) context.Context {
	pkg, ok := s.packages[name]
	if !ok {
