...
// Subscribe declares reactive bindings for an application. APIs lists
// Kubernetes API groups whose changes the runtime should observe; the
// scheduler does not consume these. Values is a list of values-path bindings
// on other packages — each Module becomes a scheduler subscription edge
// through Application.GetSubscriptions.
...
// should rerun this application. Module is the target package name; Path is
// the dotted path into its values document.
...
// Constraints projects the application definition into the scheduler's
// Constraints shape. Applications always run in the functional order tier.
