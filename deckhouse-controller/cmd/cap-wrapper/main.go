...
var (
	runDeckhouseController bool
	runUnprivilegedCommand string
)
...
func main() {
	flag.BoolVar(&runDeckhouseController, "runDeckhouseController", false, "uns deckhouse-controller with the necessary capabilities.")
	flag.StringVar(&runUnprivilegedCommand, "runUnprivilegedCommand", "", "Drops capabilities and executes the provided command.")
	flag.Parse()
	...
	if len(runUnprivilegedCommand) > 0 {
		cmd := strings.Split(runUnprivilegedCommand, " ")
		...
		if err != nil {
			log.Fatalf("executing the command: %s", err)
		}
	}
}
