...
//
// A multi-arch release resolves to its linux/amd64 child unless
// client.WithPlatform is passed. Release images carry metadata only, so the
// choice rarely matters — but the default is the library's, not the host's.
func (s *Service) Fetch(ctx context.Context, tag string, opts ...registry.ImageGetOption) (*Release, error) {
	entry := s.Entry(tag)
...
// the tags that are not concrete versions or digests. It pulls no image.
//
// A release repository holds one tag per published version, so the channels are
// a handful of names among possibly thousands. The tags are therefore streamed
// rather than collected, and the walk stops as soon as every known channel has
// been seen: on a repository whose channels all appear early, that is one page
// instead of the whole history.
func (s *Service) Channels(ctx context.Context) ([]string, error) {
	found := make(map[Channel]bool, len(knownChannels))

	err := s.StreamTags(ctx, func(tags []string) error {
		for _, tag := range tags {
			if channel := Channel(strings.ToLower(tag)); channel.IsValid() {
				found[channel] = true
			}
		}

		if len(found) == len(knownChannels) {
	...
	// Report them least stable first, the order AllChannels fixes, so the
	// result does not depend on how the registry happened to sort its tags.
	channels := make([]string, 0, len(found))
