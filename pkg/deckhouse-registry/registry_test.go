...
func TestPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{"installer", installer.Path(), "registry.deckhouse.io/deckhouse/installer"},
...
func TestRefs(t *testing.T) {
	...
	tests := []struct {
		...
		{"installer", installer.Ref("v0.1.3"), "registry.deckhouse.io/deckhouse/installer:v0.1.3"},
...
func TestNoEdition(t *testing.T) {
	...
	installer, err := reg.Installer()
	require.NoError(t, err)
	assert.Equal(t, "dev-registry.deckhouse.io/sys/deckhouse-oss/installer", installer.Path())
}
...
// TestEditionScopedClientHasNoInstaller covers the one thing an edition-scoped
// client cannot do. A client can only be scoped further down, never back up to
// its parent, so the installer — the single node that is not edition-scoped —
// is out of reach. Root still reports the right path; Installer says why it
// cannot be read rather than handing back a service addressing
// <root>/<edition>/installer, which is nothing.
func TestEditionScopedClientHasNoInstaller(t *testing.T) {
	scoped := func() *dhregistry.Registry {
		return dhregistry.NewForPath(client.New("registry.deckhouse.io").WithSegment("deckhouse", "se-plus"))
	}
	...
	t.Run("edition detected", func(t *testing.T) {
		...
		assert.Equal(t, "registry.deckhouse.io/deckhouse/se-plus", reg.EditionRoot())
		assert.Equal(t, "registry.deckhouse.io/deckhouse", reg.Root(), "Root is the path above the edition")

		_, err := reg.Installer()
		require.Error(t, err)
		assert.ErrorIs(t, err, dhregistry.ErrNoRootClient)
	})

	t.Run("edition passed explicitly", func(t *testing.T) {
		reg := dhregistry.New(
			client.New("registry.deckhouse.io").WithSegment("deckhouse", "se-plus"),
			dhregistry.WithEdition(dhregistry.SEPlusEdition),
		)
	...
	t.Run("root-scoped client reaches it", func(t *testing.T) {
		reg := dhregistry.New(
			client.New("registry.deckhouse.io").WithSegment("deckhouse"),
			dhregistry.WithEdition(dhregistry.SEPlusEdition),
		)

		installer, err := reg.Installer()
		require.NoError(t, err)
		assert.Equal(t, "registry.deckhouse.io/deckhouse/installer", installer.Path())
	})
}
