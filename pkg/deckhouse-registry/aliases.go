...
var (
	...
	// ErrImageNotFound, so IsNotFound matches both.
	ErrRepositoryNotFound = service.ErrRepositoryNotFound
	// ErrAccessDenied is returned when the registry refused the request for
	// lack of rights. It is not an absence: a denied request says nothing about
	// whether the target exists.
	ErrAccessDenied = service.ErrAccessDenied
	// ErrCatalogNotSupported is returned when listing repositories against a
	// registry that does not implement the catalog endpoint.
	ErrCatalogNotSupported = service.ErrCatalogNotSupported
	// ErrStopStreaming ends a StreamTags or StreamRepositories walk early
	// without being reported as a failure. Return it from the visit function.
	ErrStopStreaming = service.ErrStopStreaming
...
// rights, which is worth telling apart from IsNotFound: a wrong or expired
// credential otherwise makes every repository look empty.
func IsAccessDenied(err error) bool { return service.IsAccessDenied(err) }
