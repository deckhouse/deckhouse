...
func Example() {
	...
	// The installer is the one node that is not edition-scoped, so it is only
	// reachable when the tree was built over a root-scoped client.
	installer, err := reg.Installer()
	if err != nil {
		panic(err)
	}

	fmt.Println(installer.Ref("v0.1.3"))
