...
// rather than per field. Only the layers up to the one holding the digests file
// are transferred: the flattened stream is read newest layer first and stops at
// the first match.
//
// A multi-arch bundle resolves to its linux/amd64 child unless
// client.WithPlatform is passed — that default comes from the underlying
// library and is not the host's platform.
func (s *Service) Fetch(ctx context.Context, tag string, opts ...registry.ImageGetOption) (*Bundle, error) {
	entry := s.Entry(tag)

	entry.Debug("Fetching bundle image", slog.String("file", s.digestsPath))

	img, err := s.GetImage(ctx, tag, opts...)
	if err != nil {
		return nil, err
	}
