...
//
// The listing methods come in two forms. ListTags and ListRepositories return
// the complete result; StreamTags and StreamRepositories hand over one page at
// a time, so a repository with very many tags can be walked without buffering
// it, and the walk can stop as soon as the caller has seen enough.
type Reader interface {
	...
	GetManifest(ctx context.Context, tag string, opts ...registry.ManifestGetOption) (registry.ManifestResult, error)
	...
	StreamTags(ctx context.Context, visit func(tags []string) error, opts ...registry.ListTagsOption) error
	ListRepositories(ctx context.Context, opts ...registry.ListRepositoriesOption) ([]string, error)
	StreamRepositories(ctx context.Context, visit func(repos []string) error, opts ...registry.ListRepositoriesOption) error
}
