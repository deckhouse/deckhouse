...
function common_hooks::daemonset_pair_safe_update::delete_all_not_updated_crashloopback_pods_in_ds() {
  namespace=$1
  daemonset_name=$2
...
function common_hooks::daemonset_pair_safe_update::delete_pod_in_ds() {
  namespace=$1
  daemonset_name=$2
...
function common_hooks::daemonset_pair_safe_update::main() {
  ...
  if [[ "${a_need_update}" == "true" && "${a_ready}" == "true" && "${b_ready}" == "true" ]]; then
    common_hooks::daemonset_pair_safe_update::delete_pod_in_ds $namespace $a_name
  elif [[ "${b_need_update}" == "true" && "${b_ready}" == "true" && "${a_ready}" == "true" ]]; then
    common_hooks::daemonset_pair_safe_update::delete_pod_in_ds $namespace $b_name
  fi

  common_hooks::daemonset_pair_safe_update::delete_all_not_updated_crashloopback_pods_in_ds $namespace $a_name
  common_hooks::daemonset_pair_safe_update::delete_all_not_updated_crashloopback_pods_in_ds $namespace $b_name
}
