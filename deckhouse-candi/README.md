...
Существует острая необходимость реагировать на внесение ручных изменений в инфраструктуру кластера при использовании облачных провайдеров.
...
Для проверки наличия изменений и приведения объектов в облаке к начальному состоянию существует команда `converge`. 
Во время выполнения команды deckhouse-candi:
* Подключается к кластеру Kubernetes
* Собирает информацию о текущем состоянии (state) для узлов (node) и базовой инфраструктуры
* Запускает создание базовой инфраструктуры используя terraform и первоначальное состояние из кластера Kubernetes
* Сохраняет новое состояние в кластер Kubernetes
* Обрабатывает master-узлы и статические узлы
    * Если master-узлов или статических узлов в облаке меньше, чем указано в состоянии кластера, deckhouse-candi сначала создает недостающие узлы
...
> При расхождении состояния и при удалении объектов из облака deckhouse-candi запросит подтверждение.

Пример запуска:
```bash
deckhouse-candi converge \
  --ssh-host 8.8.8.8 \
  --ssh-user=ubuntu \
  --ssh-agent-private-keys=/tmp/.ssh/id_rsa
```
...
Для удаления кластера, развернутого в облаке, используется команда `terraform destroy-all`.
Во время выполнения команды deckhouse-candi:
* Подключается к кластеру Kubernetes
* Собирает информацию о текущем состоянии (state) для узлов (node) и базовой инфраструктуры
...
Пример запуска:
```bash
deckhouse-candi terraform destroy-all \
  --ssh-host 8.8.8.8 \
  --ssh-user=ubuntu \
  --ssh-agent-private-keys=/tmp/.ssh/id_rsa
```
