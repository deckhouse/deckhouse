...
2. Для проверки наличия изменений в облаке и в состоянии terraform существуют команды:
    * `deckhouse-candi terraform converge-exporter` - запускает экспортер для Prometheus, который  периодически проверяет схождение состояния в облаке и состояния terraform'а в секретах.
        > Эта команда используется в модуле `040-terraform-manager`
    * `deckhouse-candi terraform check` - запускает проверку один раз и выдает отчет в формате YAML или JSON.
...
Во время выполнения команды deckhouse-candi:
* Подключается к кластеру Kubernetes
* Удаляет ресурсы, которые отвечают за создание объектов в облаке: service'ы с типом LoadBalancer,  PV, PVC и Machines (deckhouse-candi дожидается, пока ресурсы удалятся из кластера).
* Собирает информацию о текущем состоянии (state) для узлов (node) и базовой инфраструктуры
* По очереди вызывает удаление всех компонентов

Пример запуска:
```bash
deckhouse-candi destroy \
  --ssh-host 8.8.8.8 \
  --ssh-user=ubuntu \
  --ssh-agent-private-keys=/tmp/.ssh/id_rsa
```
