...
func BootstrapAdditionalNode(kubeCl *kube.KubernetesClient, index int, providerName, layout, step, nodeGroupName, cloudConfig string, metaConfig *config.MetaConfig) error {
	nodeName := fmt.Sprintf("%s-%v", nodeGroupName, index)
	nodeConfig := metaConfig.PrepareTerraformNodeGroupConfig(nodeGroupName, index, cloudConfig)

	return logboek.LogProcess(fmt.Sprintf("🌿 Bootstrap additional node %v 🌿", nodeName), log.TaskOptions(), func() error {
...
func RunConverge(kubeCl *kube.KubernetesClient, metaConfig *config.MetaConfig) error {
...
func NewConvergeController(kubeCl *kube.KubernetesClient, metaConfig *config.MetaConfig) *ConvergeController {
...
func (c *ConvergeController) addNewNodeGroup(nodeGroup *ConvergeNodeGroupGroupOptions) error {
	return logboek.LogProcess(fmt.Sprintf("🌼 Add new nodes for NodeGroup %s (replicas: %v) 🌼", nodeGroup.Name, nodeGroup.Replicas), log.BoldOptions(), func() error {
		...
		for nodeGroup.Replicas > count {
			candidateName := fmt.Sprintf("%s-%v", nodeGroup.Name, index)
...
func (c *ConvergeController) updateNode(nodeGroup *ConvergeNodeGroupGroupOptions, nodeName string) error {
	...
	return logboek.LogProcess(fmt.Sprintf("🌻 Update node %s 🌻", nodeName), log.TaskOptions(), func() error {
		...
		kubernetesNodeName := fmt.Sprintf("%s-%s", c.config.ClusterPrefix, nodeName)
		...
		return WaitForSingleNodeBecomeReady(c.client, kubernetesNodeName)
	})
}
