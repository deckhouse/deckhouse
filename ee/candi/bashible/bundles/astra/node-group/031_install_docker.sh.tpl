...
{{- range $key, $value := index .k8s .kubernetesVersion "bashible" "astra" }}
  {{- $astraVersion := toString $key }}
  if bb-is-astra-version? {{ $astraVersion }} ; then
    containerd_tag="{{- index $.images.registrypackages (printf "containerdAstra%s%s" ($value.docker.containerd.desiredVersion | replace "containerd.io=" "" | replace "." "" | replace "-" "") (index $astraName $astraVersion)) }}"
  fi
{{- end }}
...
{{- range $key, $value := index .k8s .kubernetesVersion "bashible" "astra" }}
  {{- $astraVersion := toString $key }}
  if bb-is-astra-version? {{ $astraVersion }} ; then
    docker_tag="{{- index $.images.registrypackages (printf "dockerAstra%s" ($value.docker.desiredVersion | replace "docker-ce=" "" | replace "." "_" | replace ":" "_" | replace "~" "_" | camelcase)) }}"
  fi
{{- end }}
