...
{{- range $key, $value := index .k8s .kubernetesVersion "bashible" "redos" }}
  {{- $redosVersion := toString $key }}
  if bb-is-redos-version? {{ $redosVersion }} ; then
    containerd_tag="{{- index $.images.registrypackages (printf "containerdRedos%s" ($value.docker.containerd.desiredVersion | replace "containerd.io-" "" | replace "." "_" | replace "-" "_" | camelcase )) }}"
  fi
{{- end }}
