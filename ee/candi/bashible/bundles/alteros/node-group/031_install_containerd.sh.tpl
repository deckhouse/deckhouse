...
{{- range $key, $value := index .k8s .kubernetesVersion "bashible" "alteros" }}
  {{- $alterosVersion := toString $key }}
  if bb-is-alteros-version? {{ $alterosVersion }} ; then
    containerd_tag="{{- index $.images.registrypackages (printf "containerdAlteros%s" ($value.containerd.desiredVersion | replace "containerd.io-" "" | replace "." "_" | replace "-" "_" | camelcase )) }}"
  fi
{{- end }}
