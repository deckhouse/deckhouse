...
Network (внутренняя сеть) может быть настроена вашим поставщиком услуг VMware Cloud Director, либо вы можете настроить ее самостоятельно. При выборе схемы размещения `StandardWithNetwork` сеть создается автоматически. Далее рассматривается настройка внутренней сети самостоятельно.
...
{% alert level="info" %}
Пункт актуален только для схемы размещения `Standard`
{% endalert %}
