...
* Компоненты DKP поддерживают использование ресурсов Service типа LoadBalancer при установке в VCD.
* Для балансировщиков используется технология VMware NSX Advanced Load Balancer (ALB или Avi Networks).
* Балансировщики поддерживаются только при использовании платформы сетевой виртуализации `NSX-T`.
* Функционал балансировщиков должен быть предоставлен явно в Edge Gateway вашим поставщиком услуг VMware Cloud Director; Проверить статус в Edge Gateway можно в разделе Load Balancer -> General Settings; State должен быть в статусе Active.
* Если активация функционала балансировщиков была выполнена уже после успешного создания кластера DKP, то соответствующие изменения в компонентах будут произведены автоматически в течении часа.
* Балансировщик создается как связка Pool + Virtual Service для каждого из открываемых портов.
* При использовании межсетевого экрана для внешнего адреса и портов должно быть создано разрешающее правило.
