...
> Используйте модуль `csi-vsphere`, когда в кластере отсутствует возможность применять модуль [`cloud-provider-vsphere`](/modules/cloud-provider-vsphere/), но требуется динамический заказ (`provisioning`) томов в инфраструктуре vSphere.
