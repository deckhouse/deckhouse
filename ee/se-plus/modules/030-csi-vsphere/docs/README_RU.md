...
{% alert level="info" %}
Используйте модуль `csi-vsphere`, когда в кластере отсутствует возможность применять модуль [`cloud-provider-vsphere`](/modules/cloud-provider-vsphere/), но требуется динамический заказ томов в инфраструктуре vSphere.
{% endalert %}
...
- В настройках каждой виртуальной машины необходимо включить параметр `disk.EnableUUID:TRUE`. Он обеспечивает корректную работу модуля с дисковыми ресурсами и позволяет DP идентифицировать подключенные тома.
...
Чтобы задать StorageClass по умолчанию для кластера, используйте глобальный параметр [`defaultClusterStorageClass`](/products/kubernetes-platform/documentation/v1/reference/api/global.html#parameters-defaultclusterstorageclass). Чтобы отфильтровать ненужные объекты StorageClass, задайте шаблоны исключений в параметре [`storageClass.exclude`](configuration.html#parameters-storageclass-exclude).
