...
- apiGroups:
  ...
  - update`))
          ...
          "zoneTagCategory": "myzonetagcat"
			...
			capvDeployment := f.KubernetesResource("Deployment", moduleNamespace, "capv-controller-manager")
			Expect(capvDeployment.Exists()).To(BeTrue())
			Expect(capvDeployment.Field("spec.template.metadata.annotations.checksum/config").String()).ToNot(BeEmpty())
			Expect(f.KubernetesResource("Service", moduleNamespace, "capv-controller-manager-webhook-service").Exists()).To(BeTrue())
			Expect(f.KubernetesGlobalResource("ValidatingWebhookConfiguration", "capv-validating-webhook").Exists()).To(BeTrue())
			...
			Expect(csiNodeLegacyPluginDS.Exists()).To(BeTrue())
			Expect(csiNodeLegacyPluginDS.Field("spec.template.spec.serviceAccountName").String()).To(Equal("csi"))
	...
	Context("Hybrid vSphere", func() {
		...
		It("renders resources for hybrid clusters", func() {
          ...
          "zoneTagCategory": "myzonetagcat"
			...
			Expect(f.KubernetesResource("Secret", "kube-system", fmt.Sprintf("d8-cloud-provider-%s-capi", providerID)).Exists()).To(BeTrue())
			...
			Expect(f.KubernetesResource("Deployment", moduleNamespace, "capv-controller-manager").Exists()).To(BeTrue())
...
`))
	...
	Context("Vsphere without provider insecure (default false)", func() {
		...
		It("Omits caBundle and keeps insecure in the node-manager registration secret", func() {
			...
			_, hasCABundle := blob["caBundle"]
			Expect(hasCABundle).To(BeFalse())
			Expect(blob["insecure"]).To(BeFalse())
		})
	})
	...
	Context("Vsphere without NSX-T insecureFlag (default false)", func() {
		...
		It("Sets nsxt insecureFlag in the CCM cloud-config", func() {
			Expect(f.RenderError).ShouldNot(HaveOccurred())
