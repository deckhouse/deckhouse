...
- Provisioning CloudEphemeral nodes via Machine Controller Manager (MCM). Virtual machine parameters are set in the [VsphereInstanceClass](cr.html#vsphereinstanceclass) resource.
- Registering with [node-manager](/modules/node-manager/) so that `VsphereInstanceClass` can be used when describing a `NodeGroup`.
