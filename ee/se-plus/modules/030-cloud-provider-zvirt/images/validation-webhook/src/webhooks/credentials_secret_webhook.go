...
func NewCredentialSecretValidator(factory *zval.AdmissionStateBuilderFactory, object runtime.Object) *CredentialSecretValidator {
...
func (v *CredentialSecretValidator) validate(
	...
	result := zadmission.ValidateCredentialSecret(state, operation)
...
// validateCredentialSecretTypeChange keeps a managed credential Secret from being retyped out of
// the module's reach. Changing the type would take the Secret out of every rule's view while the
// components carry on reading it.
func validateCredentialSecretTypeChange(oldObj, newObj runtime.Object) error {
	...
	if objectNamespace(oldSecret) != zmeta.Namespace {
		return nil
	}
