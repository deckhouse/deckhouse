...
func TestBuildCredentialSecret(t *testing.T) {
	...
	if !ok {
		t.Fatalf("stringData has type %T, want map", secret["stringData"])
	}

	if got, want := secret["type"], cpapi.CredentialsSecretType; got != want {
		t.Errorf("type = %v, want %v", got, want)
	}
	if got, want := stringData["authScheme"], string(cpapi.AuthSchemeUserPassword); got != want {
		t.Errorf("authScheme = %v, want %v", got, want)
	}
	if got, want := stringData["identity"], "admin@internal"; got != want {
		t.Errorf("identity = %v, want %v", got, want)
	}
	if got, want := stringData["secret"], "s3cret"; got != want {
...
func TestMapPCCInstanceClassToSpecFillsTerraformDefaults(t *testing.T) {
	...
	if spec.RootDiskSizeGb == nil || *spec.RootDiskSizeGb != defaultRootDiskSizeGb {
...
func TestMapPCCInstanceClassToSpecKeepsExplicitSizes(t *testing.T) {
	...
	if spec.RootDiskSizeGb == nil || *spec.RootDiskSizeGb != root {
...
func TestMapPCCInstanceClassToSpecSplitsDNSServers(t *testing.T) {
	...
	if spec.CustomNetworkConfig == nil {
		t.Fatal("customNetworkConfig must be carried over")
	}
...
func TestBuildMigrationResources(t *testing.T) {
	...
	kinds := make(map[string]int, len(resources))
	for _, resource := range resources {
		manifest, ok := resource.(map[string]any)
		if !ok {
			t.Fatalf("resource has type %T, want map", resource)
		}
		kind, _ := manifest["kind"].(string)
		kinds[kind]++
	}

	for kind, want := range map[string]int{"Secret": 1, "ModuleConfig": 1, "ZvirtInstanceClass": 2, "NodeGroup": 2} {
		if kinds[kind] != want {
...
func TestBundleInstanceClassNamesMatchTheCompletenessCheck(t *testing.T) {
	resources, err := BuildNodeGroupAndInstanceClassResources(
	...
	instanceClass, ok := resources[0].(map[string]any)
	if !ok {
		t.Fatalf("instance class has type %T, want map", resources[0])
	}
	metadata, ok := instanceClass["metadata"].(map[string]any)
	if !ok {
		t.Fatalf("metadata has type %T, want map", instanceClass["metadata"])
	}

	if got, want := metadata["name"], cpapi.BuildInstanceClassName("worker"); got != want {
