...
// (WhoCan) queries.
func initAuthorizers(init *initResult, configPath string, scopeCache *resolver.ResourceScopeCache) (authorizer.Authorizer, *multitenancy.Engine, *rbacadapter.RBACAuthorizer, error) {
...
func (c completedConfig) New() (*PermissionBrowserServer, error) {
	...
	compositeAuth, mtEngine, rbacAuth, err := initAuthorizers(initRes, c.ExtraConfig.ConfigPath, scopeCache)
	if err != nil {
		cancel()
