...
description: "Кеширующий DNS-сервер на каждом узле кластера Deckhouse Platform."
---
...
- Отсутствие кеширования запросов (в Linux из коробки нет кеша, соответственно, и в подах его тоже нет из коробки).
...
- Добавляет правило в iptables, которое проверяет доступность локального сокета: если сокет открыт – трафик перенаправляется на локальный CoreDNS, если не открыт – обрабатывается стандартным механизмом Kubernetes для clusterIP.

Правило для обеспечения резервного пути обработки DNS (fallback):
