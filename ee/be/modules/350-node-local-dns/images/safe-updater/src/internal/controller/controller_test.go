...
func (suite *ControllerTestSuite) TestCreateReconcile() {
	suite.Run("simple", func() {
		err := suite.setupController(suite.fetchTestFileData("simple.yaml"))
		require.NoError(suite.T(), err)
	...
	suite.Run("daemonset-updated-but-not-pod-and-cilium-is-not-up-to-date", func() {
		err := suite.setupController(suite.fetchTestFileData("daemonset-updated-but-not-pod-and-cilium-is-not-up-to-date.yaml"))
		require.NoError(suite.T(), err)
