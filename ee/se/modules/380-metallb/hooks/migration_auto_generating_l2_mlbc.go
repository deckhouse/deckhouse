...
func migrateMCtoMLBC(input *go_hook.HookInput) error {
	snapsMC := input.NewSnapshots.Get("module_config")
	...
	snapsIAP := input.NewSnapshots.Get("ip_address_pools")
	...
	snapsL2A := input.NewSnapshots.Get("l2_advertisements")
	...
	snapsMLBC := input.NewSnapshots.Get("mlbc_with_label")
