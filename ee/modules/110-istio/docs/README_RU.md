...
* В кластер добавляется сервис, который экспортит метаданные кластера наружу:
  ...
  * Публичный адрес ingress-gateway (доступен только для аутентифицированных запросов из соседних кластеров).
  * Публичные ключи для авторизации к apiserver и закрытым данным (выше).
