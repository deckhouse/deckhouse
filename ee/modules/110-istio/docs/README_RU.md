...
[Istio](https://istio.io/) — это реализация подхода Service Mesh, то есть, это фреймворк для централизованного управления сетевым трафиком.

В частности, Istio решает прозрачно для приложений следующие задачи:
...
Рекомендуется ознакомиться с [видео](https://www.youtube.com/watch?v=9CUfaeT3T-A), где мы раскрываем этот термин, изучаем архитектуру Istio и оцениваем накладные расходы.
...
![Архитектура кластера с включенным Istio](https://docs.google.com/drawings/d/e/2PACX-1vRt0avuNi0cC_PiZmzuvbuYnFbx8rEyi4lUqB2l4pDIq2j1b3MY3HUeNHKhT3S9EeFC0tQdcY3Q8ydw/pub?w=1314&h=702)
...
* Для сбора информации о соседних сервисах, Istio подключается напрямую к apiserver соседнего кластера. Данный модуль Deckhouse берёт на себя организацию соответствующего канала связи.
...
* В кластер добавляется сервис, который экспортит метаданные кластера наружу:
  ...
  * Список публичных адресов сервиса ingressgateway (доступен только для аутентифицированных запросов из соседних кластеров).
  * Публичные ключи сервера для аутентификации запросов к apiserver и к закрытым метаданным данным (выше).
...
Для сборки мультикластера необходимо:
...
Для ограничения потребления ресурсов путём ограничения поля видимости сайдкара, возможно использования ресурса [Sidecar](istio-cr.html#sidecar).
