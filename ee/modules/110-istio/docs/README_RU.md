...
Для ограничения потребления ресурсов путём ограничения поля видимости sidecar, возможно использование ресурса [Sidecar](istio-cr.html#sidecar), а также предусмотрен лейбл для исключения неймспейса из поля зрения Istio — `istio.deckhouse.io/discovery: disabled`.
