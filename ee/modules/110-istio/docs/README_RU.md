...
Для автоматизации обновления istio-sidecar'ов необходимо установить лейбл `istio.deckhouse.io/auto-upgrade="true"` на `Namespace` либо на ресурс. Обновление поддерживается для `Deployment`, `DaemonSet` и `StatefulSet`.
