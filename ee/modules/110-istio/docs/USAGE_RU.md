...
По умолчанию после получения сигнала SIGTERM isiot-proxy завершает свою работу не дожидаясь завершения установленных соединений и закрытия сокетов открытых для прослушивания у основного приложения. 
Исправить данное поведение можно путем установки следующей аннотации на pod приложения: `inject.istio.io/templates: sidecar,d8-hold-istio-proxy-termination-until-application-stops`.
Установка данной аннотаций заставит sidecar-injector вставлять preStop-хук для istio-proxy, который при получении сигнала SIGTERM заставит istio-proxy ждать завершения всех установленных соединений и закрытия сокетов, открытых для прослушивания, но не более чем интервал `terminationGracePeriodSeconds`. 
В некоторых случаях это позволит избежать некорректного завершения приложения.
