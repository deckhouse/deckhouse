...
const istioValues = `
    internal:
      ...
      globalVersion: "1.13.9"
      versionMap:
        "1.13.9":
          revision: "v1x13x9"
          fullVersion: "1.13.9.0"
          imageSuffix: "V1x13x9"
        "1.12.6":
          revision: "v1x12x6"
...
var _ = Describe("Module :: istio :: helm template :: main", func() {
	...
	Context("There are revisions to install, no federations or multiclusters", func() {
		BeforeEach(func() {
			...
			f.ValuesSet("global.modulesImages", GetModulesImages())
			f.ValuesSetFromYaml("istio", istioValues)
			f.ValuesSetFromYaml("istio.internal.versionsToInstall", `["1.13.9","1.12.6"]`)
			f.ValuesSetFromYaml("istio.internal.operatorVersionsToInstall", `["1.13.9","1.12.6"]`)
		...
		It("", func() {
			...
			iopV13 := f.KubernetesResource("IstioOperator", "d8-istio", "v1x13x9")
			iopV12 := f.KubernetesResource("IstioOperator", "d8-istio", "v1x12x6")

			deploymentOperatorV13 := f.KubernetesResource("Deployment", "d8-istio", "operator-v1x13x9")
			...
			Expect(iopV13.Field("spec.revision").String()).To(Equal(`v1x13x9`))
			...
			Expect(deploymentOperatorV13.Field("spec.template.spec.containers.0.image").String()).To(Equal(`registry.example.com:imageHash-istio-operatorV1x13x9`))
			...
			Expect(iopV13.Field("spec.values.global.proxy.image").String()).To(Equal(`registry.example.com:imageHash-istio-proxyv2V1x13x9`))
			Expect(iopV12.Field("spec.values.global.proxy.image").String()).To(Equal(`registry.example.com:imageHash-istio-proxyv2V1x12x6`))

			Expect(iopV13.Field("spec.values.pilot.image").String()).To(Equal(`registry.example.com:imageHash-istio-pilotV1x13x9`))
			Expect(iopV12.Field("spec.values.pilot.image").String()).To(Equal(`registry.example.com:imageHash-istio-pilotV1x12x6`))

			Expect(mwh.Field("webhooks.0.clientConfig.service.name").String()).To(Equal(`istiod-v1x13x9`))
			Expect(mwh.Field("webhooks.0.clientConfig.caBundle").String()).To(Equal(`bXljZXJ0`)) // b64("mycert")
			Expect(serviceGlobal.Field("spec.selector").String()).To(MatchJSON(`{"app":"istiod","istio.io/rev":"v1x13x9"}`))
...
`))
	...
	Context("There are some federations", func() {
		BeforeEach(func() {
			...
			f.ValuesSet("global.modulesImages", GetModulesImages())
			f.ValuesSetFromYaml("istio", istioValues)
			f.ValuesSetFromYaml("istio.internal.versionsToInstall", `["1.13.9"]`)
			f.ValuesSetFromYaml("istio.internal.operatorVersionsToInstall", `["1.13.9"]`)
...
limits: {}
`))
			vpa := f.KubernetesResource("VerticalPodAutoscaler", "d8-istio", "istiod-v1x13x9")
