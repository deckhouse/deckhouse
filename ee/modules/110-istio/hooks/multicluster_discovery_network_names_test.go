...
var _ = Describe("Istio hooks :: multicluster_discovery_network_names ::", func() {
...
status:
  metadataCache:
    networkName: some-network-123
    public:
      clusterUUID: deadbeef-pm1
`))
			_ = os.MkdirAll("/tmp/proper-multicluster-0/private", 0755)
			ioutil.WriteFile("/tmp/proper-multicluster-0/private/multicluster-network-name", []byte(`network-foo-123`), 0644)
			_ = os.MkdirAll("/tmp/proper-multicluster-1/private", 0755)
			ioutil.WriteFile("/tmp/proper-multicluster-1/private/multicluster-network-name", []byte(`network-bar-234`), 0644)
		...
		It("Hook must execute successfully", func() {
			...
			t0, err := time.Parse(time.RFC3339, f.KubernetesGlobalResource("IstioMulticluster", "proper-multicluster-0").Field("status.metadataCache.networkNameLastFetchTimestamp").String())
			Expect(err).ShouldNot(HaveOccurred())
			t1, err := time.Parse(time.RFC3339, f.KubernetesGlobalResource("IstioMulticluster", "proper-multicluster-1").Field("status.metadataCache.networkNameLastFetchTimestamp").String())
			Expect(err).ShouldNot(HaveOccurred())

			Expect(t0).Should(BeTemporally("~", time.Now().UTC(), time.Minute))
			Expect(t1).Should(BeTemporally("~", time.Now().UTC(), time.Minute))

			Expect(f.KubernetesGlobalResource("IstioMulticluster", "proper-multicluster-0").Field("status.metadataCache.networkName").String()).To(Equal("network-foo-123"))
			Expect(f.KubernetesGlobalResource("IstioMulticluster", "proper-multicluster-1").Field("status.metadataCache.networkName").String()).To(Equal("network-bar-234"))
		})
	})
...
`))
		...
		It("Hook must execute successfully with proper warnings", func() {
			...
			stderrBuff := string(f.Session.Err.Contents())
			Expect(stderrBuff).Should(ContainSubstring(`ERROR: Cannot fetch network name metadata endpoint https://some-improper-hostname-0/metadata/private/multicluster-network-name for IstioMulticluster improper-multicluster-0.`))
		})
	})
})
