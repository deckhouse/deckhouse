...
func multiclusterDiscovery(_ context.Context, input *go_hook.HookInput, dc dependency.Container) error {
	...
	// Read remote secrets with their extracted tokens from alliance_metadata_merge hook
	remoteSecrets := input.Values.Get("istio.internal.remoteSecrets")
	if remoteSecrets.Exists() {
		secretCount := len(remoteSecrets.Array())
		if secretCount > 0 {
			input.Logger.Info("validating remote secret tokens", slog.Int("count", secretCount))
		}
		...
		for _, secretValue := range remoteSecrets.Array() {
			...
			if existingToken == "" {
				// Token doesn't exist - this shouldn't happen as alliance_metadata_merge extracts them
				input.Logger.Warn("no token found for remote secret",
			...
			case validationResult.IsExpired:
				input.Logger.Warn("token is expired, will be regenerated by alliance_metadata_merge hook",
					slog.String("secretName", secretName),
