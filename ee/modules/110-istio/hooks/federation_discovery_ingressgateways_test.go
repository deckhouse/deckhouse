...
`))
			...
			ioutil.WriteFile("/tmp/proper-federation-0/private/federation-ingressgateways", []byte(`
{
  "ingressGateways": [
...
`), 0644)
			...
			ioutil.WriteFile("/tmp/proper-federation-1/private/federation-ingressgateways", []byte(`
{
  "ingressGateways": [
...
`), 0644)
			...
			ioutil.WriteFile("/tmp/proper-federation-2/private/federation-ingressgateways", []byte(`
{
  "ingressGateways": [
    {"address": "some-actual.host-2", "port": 111},
    {"address": "some-actual.host-1", "port": 111}
...
`))
			...
			tokenPF0Bytes, errpf0r := ioutil.ReadFile("/tmp/jwt-igs-proper-federation-0")
			Expect(errpf0r).ShouldNot(HaveOccurred())
			tokenPF1Bytes, errpf1r := ioutil.ReadFile("/tmp/jwt-igs-proper-federation-1")
			Expect(errpf1r).ShouldNot(HaveOccurred())
			tokenPF2Bytes, errpf2r := ioutil.ReadFile("/tmp/jwt-igs-proper-federation-2")
			...
			Expect(tokenPF0Payload.Scope).To(Equal("federation-ingressgateways"))
			Expect(tokenPF1Payload.Scope).To(Equal("federation-ingressgateways"))
			Expect(tokenPF2Payload.Scope).To(Equal("federation-ingressgateways"))
...
`))
		...
		It("Hook must execute successfully with proper warnings", func() {
			...
			stderrBuff := string(f.Session.Err.Contents())
			Expect(stderrBuff).Should(ContainSubstring(`ERROR: Cannot fetch ingressgateways metadata endpoint https://some-improper-hostname-0/metadata/private/federation-ingressgateways for IstioFederation improper-federation-0.`))
