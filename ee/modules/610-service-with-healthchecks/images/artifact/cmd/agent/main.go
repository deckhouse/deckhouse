...
func main() {
	...
	flag.StringVar(&probeAddr, "health-probe-bind-address", ":9873", "The address the probe endpoint binds to.")
	flag.BoolVar(&secureMetrics, "metrics-secure", false,
		"If set the metrics endpoint is served securely")
	flag.BoolVar(&enableHTTP2, "enable-http2", false,
		"If set, HTTP/2 will be enabled for the metrics and webhook servers")
	flag.BoolVar(&debugging, "debugging", false, "If set, enables debugging")
	flag.IntVar(&workersCount, "workers-count", 2, "The number of workers to run")
	...
	serviceController := agent.NewServiceWithHealthchecksReconciler(
		mgr.GetClient(),
		workersCount,
		nodeName,
		mgr.GetScheme(),
	...
	if err = serviceController.RunWorkers(context.Background()); err != nil {
