      ...
      <template #content>
        <div class="flex flex-wrap items-start -mx-24 -my-6">
          ...
          <div class="mx-24 my-6">
            ...
            <Field :name="'machineType'" v-slot="{ errorMessage }">
              <InputBlock
                title="Тип машины"
                type="column"
                spec="spec.machineType"
                class="mb-6"
                :disabled="readonly"
          ...
          <div class="mx-24 my-6">
            ...
            <div class="flex flex-col gap-y-6">
              ...
              <Field :name="'disableExternalIP'" v-slot="{ errorMessage }">
                <InputBlock title="Без внешнего IP" spec="spec.disableExternalIP" special>
                  <InputSwitch :disabled="readonly" v-model="values.disableExternalIP" />
                </InputBlock>
              </Field>
              <Field :name="'preemptible'" v-slot="{ errorMessage }">
                <InputBlock title="Preemptible" spec="spec.preemptible" special>
        ...
        <div class="flex flex-wrap items-start -mx-12 -my-6">
          <FieldArray :name="'additionalNetworkTags'" v-slot="{ fields, push, remove }" v-model="values.additionalNetworkTags">
            <InputLabelGroup
              class="mx-12 my-6"
              title="Дополнительные сетевые теги"
              spec="spec.additionalNetworkTags"
              :fields="['value']"
              :disabled="readonly"
              :model="fields"
              @push="push({ value: '' })"
...
const formSchema = z.object({
  ...
  disableExternalIP: z.boolean().optional(),
  preemptible: z.boolean().optional(),
  additionalNetworkTags: z
    .object({
...
const initialValues = computed(() => ({
  ...
  machineType: props.item.spec.machineType,
  diskSizeGb: props.item.spec.diskSizeGb,
  diskType: props.item.spec.diskType,
  image: props.item.spec.image,
  disableExternalIP: props.item.spec.disableExternalIP,
  preemptible: props.item.spec.preemptible,
  additionalNetworkTags: props.item.spec.additionalNetworkTags?.map((asg: string) => ({ value: asg })),
...
const submitForm = handleSubmit(
  (values) => {
    ...
    newSpec.machineType = values.machineType;
    newSpec.diskSizeGb = values.diskSizeGb;
    newSpec.diskType = values.diskType;
    newSpec.image = values.image;
    newSpec.disableExternalIP = values.disableExternalIP;
    newSpec.preemptible = values.preemptible;
    newSpec.additionalNetworkTags = values.additionalNetworkTags?.map((obj: { value: string }) => obj.value);
