<template>
  <CardParamGrid>
    <CardParam title="ЦПУ" :value="item.spec?.cores" />
    <CardParam title="Память" :value="memoryInfo" />
    <CardParam title="Диск" :value="diskInfo" />
    <CardParam title="ГПУ" :value="item.spec?.gpus" />
    <CardParam title="Внешний IP" :value="item.spec?.assignPublicIPAddress ? 'Включено' : '–'" />
    <CardParam title="Основная сеть" :value="item.spec?.mainSubnet" />
    <CardParam title="Допсети" :value="item.spec?.additionalSubnets?.join(', ')" />
  </CardParamGrid>
...
const diskInfo = computed((): string | undefined => {
  return props.item.spec?.diskSizeGB ? `${props.item.spec?.diskSizeGB} G` : undefined;
});
