<template>
  <div class="z-50 fixed bottom-0 left-1/2 -translate-x-1/2 flex flex-col gap-y-3">
    <div class="w-[800px] text-sm text-white rounded-md shadow-lg" :class="type_settings[message.type].styles" role="alert" v-for="message in messages" :key="message.key">
      <div class="flex p-4 gap-x-2">
        ...
        <div class="ml-auto">
          <button type="button" v-if="!message.isReloader" @click="closeMessage(message.key)"
          ...
          <button type="button" v-if="message.isReloader" @click="closeMessage(message.key)" :tooltip="'flash_messages.require_reload'"
            class="inline-flex flex-shrink-0 justify-center items-center h-4 w-4 rounded-md text-white/[.5] hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white/20 transition-all text-sm">
            <span class="sr-only">Close</span>
            <component :is="Icons['IconClose']" class="h-4 w-4 mt-0.5" />
          </button>
...
function addMessage(message: Message): void {
  ...
  if (message.timeout) setTimeout(() => { closeMessage(message.key); }, message.timeout);
}

function idxOfKey(key: string): number {
  return messages.findIndex((message) => { return message.key == key; });
}
