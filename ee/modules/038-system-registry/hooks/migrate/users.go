...
const (
	usersSnap       = "migrate-users"
	usersSecretType = "registry/user"
)
...
var _ = sdk.RegisterFunc(&go_hook.HookConfig{
	...
	Kubernetes: []go_hook.KubernetesConfig{
		{
			...
			FilterFunc: func(obj *unstructured.Unstructured) (go_hook.FilterResult, error) {
				...
				if secret.Type != usersSecretType {
					return secret, nil
				}
	...
	secrets, err := helpers.SnapshotToList[v1core.Secret](input, usersSnap)
	if err != nil {
	...
	for _, secret := range secrets {
		...
		newSecret.Type = usersSecretType
