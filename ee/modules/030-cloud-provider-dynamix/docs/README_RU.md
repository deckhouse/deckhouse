...
description: "Управление облачными ресурсами в Deckhouse Platform с помощью Basis Dynamix."
---

Модуль `cloud-provider-dynamix` обеспечивает интеграцию Deckhouse Platform с платформой [Basis Dynamix](https://basistech.ru/products/basis-dynamix/). Он предоставляет возможность модулю [`node-manager`](/modules/node-manager/) использовать ресурсы Dynamix при заказе узлов для [NodeGroup](/modules/node-manager/cr.html#nodegroup).
