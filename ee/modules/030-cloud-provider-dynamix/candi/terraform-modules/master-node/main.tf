  ...
  lifecycle {
    postcondition {
      ...
      error_message = <<-EOT
        ...
        Set DynamixClusterConfiguration.storagePolicy (or masterNodeGroup.instanceClass.storagePolicy) to the exact name of a storage policy available to the account.
      EOT
    }
  }
}
