...
const ironicProviderNamespace = "d8-cloud-provider-baremetal"
...
func cleanupIronicOnDelete(ctx context.Context, input *go_hook.HookInput, dc dependency.Container) error {
	...
	ironics, err := k8sClient.Dynamic().Resource(ironicGVR).Namespace(ironicProviderNamespace).List(ctx, metav1.ListOptions{})
	...
	for _, ironic := range ironics.Items {
		...
		_, err = k8sClient.Dynamic().Resource(ironicGVR).Namespace(ironicProviderNamespace).Patch(ctx, name, types.MergePatchType, patch, metav1.PatchOptions{})
		if apierrors.IsNotFound(err) {
			continue
		}
		if err != nil {
		...
		input.Logger.Info("Ironic finalizers removed",
			slog.String("namespace", ironicProviderNamespace),
		...
		err = k8sClient.Dynamic().Resource(ironicGVR).Namespace(ironicProviderNamespace).Delete(ctx, name, metav1.DeleteOptions{})
		if apierrors.IsNotFound(err) {
			continue
		}
		if err != nil {
