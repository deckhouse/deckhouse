...
func migrateDiskGBHandler(input *go_hook.HookInput) error {
	...
	if !needMigration {
		input.LogEntry.Info("Skipping migration rootDiskSizeGb because Deckhouse installation version too ok")
		return nil
	}
...
func hasDiskSizeProperty(rawConfig map[string]interface{}, fields []string) (bool, error) {
...
func needMigrateNodeGroupsInstanceClass(rawConfig map[string]interface{}) (bool, error) {
	...
	for _, rawNG := range nodeGroups {
		...
		found, err := hasDiskSizeProperty(ng, fieldForNG)
		if err != nil {
			return false, err
		}
...
func needMigrateMasterInstanceClass(rawConfig map[string]interface{}) (bool, error) {
	...
	found, err := hasDiskSizeProperty(rawConfig, fieldForMaster)
	if err != nil {
		return false, err
	}
