...
const (
	...
	clusterAPIServiceAccountName = "vcd-controller-manager"
)
...
func generateStaticKubeconfigSecret(input *go_hook.HookInput, dc dependency.Container) error {
	...
	clusterAPIStaticClusterName := input.Values.Get("cloudProviderVcd.internal.providerClusterConfiguration.virtualApplicationName").String()
	...
	config, err := New(clusterAPIStaticClusterName, restConfig.Host, restConfig.CAData, string(serviceAccountToken))
	if err != nil {
	...
	secret := GenerateSecret(clusterAPIStaticClusterName, clusterAPINamespace, configYAML)
...
func getSecretForServiceAccountToken(clusterName string, namespace string, k8sClient k8s.Client) (*v1.Secret, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			secret = GenerateSecretForServiceAccountToken(clusterName, namespace, clusterAPIServiceAccountName)
