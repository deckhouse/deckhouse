...
3. Для проверки статуса проекта выполните команду:
   ...
   Успешно созданный проект должен отображаться в статусе `Sync` (синхронизирован). Если состояние `Error` (ошибка), добавьте суффикс `-o yaml` к команде, чтобы получить более подробную информацию о причине.
