...
func Register(ctx context.Context, runtimeManager manager.Manager, helmClient helm.Interface, log logr.Logger, defaultPath string) error {
	r := &reconciler{
		...
		projectManager: projectmanager.New(runtimeManager.GetClient(), helmClient, log, defaultPath),
	}
	...
	if err := runtimeManager.Add(manager.RunnableFunc(r.projectManager.Init)); err != nil {
	...
	return ctrl.NewControllerManagedBy(runtimeManager).
		...
		WithEventFilter(customPredicate[client.Object]{log: log.WithName("filter")}).
...
func (p customPredicate[T]) Update(e event.TypedUpdateEvent[T]) bool {
	p.log.Info("new update event")
	if isNil(e.ObjectNew) {
		p.log.Info("new object is nil, skip updating project", "project", e.ObjectNew.GetName())
		return false
	}
	if isNil(e.ObjectOld) {
		p.log.Info("old object is nil, skip updating project", "project", e.ObjectNew.GetName())
		return false
	}
	if annotations := e.ObjectNew.GetAnnotations(); annotations != nil {
		if val, ok := annotations[helm.ProjectRequireSyncAnnotation]; ok && val == "true" {
			p.log.Info("project requires sync", "project", e.ObjectNew.GetName())
			return true
		}
	}
	if e.ObjectNew.GetGeneration() != e.ObjectOld.GetGeneration() {
		p.log.Info("project generation changed", "project", e.ObjectOld.GetName())
		return true
	}
	return false
}
