...
* Разрешить подключение к портам статического узла для работы приложения (например, HTTP/HTTPS при публичном IP на ВМ).
...
созданные в облаке. Добавлять произвольные правила в группу безопасности, которой управляет Deckhouse, через конфигурацию модуля нельзя.
