...
OpenStack-провайдеры могут использовать разные типы томов, для root-диска. Это может быть как сетевой диск, так и локальный диск. Чтобы быть уверенным, лучше уточните этот момент в документации или в технической поддержке провайдера.
...
* Избегайте использования flavor'ов с указанным объемом диска и одновременным использованием параметра `roodDiskSize`. В этом случае cloud-провайдер может взымать плату за использование диска в соответствии с объемом диска, указанным во flavor, но вы не сможете его использовать.

С учетом изложенного выше:
- Если `rootDiskSize` не установлен, будет использован ephemeral-диск размера, соответствующего указанному во flavor, и типа, указанного в cloud provider'е;
- Если `rootDiskSize` указан, будет использован Cinder volume OpenStack (указанного размера с типом тома по умолчанию, указанного в OpenStack). Но, в этом случае есть несколько нюансов:
  ...
  * При необходимости, можно [переопределить](#как-переопределить-тип-диска-по-умолчанию-cloud-провайдера) тип диска по умолчанию cloud provider.

## Как переопределить тип диска по умолчанию cloud-провайдера?

Если у cloud-провайдера доступно несколько типов, то вы можете указать тип диска по умолчанию для образа, чтобы использовать его при создании ВМ. Для этого необходимо указать название типа диска в метаданных образа.
