...
Существует несколько вариантов - ограничить или расширить входящий и исходящий трафик на виртуальных машинах кластера.
Например:
...
Применяйте дополнительные группы безопасности (security groups). Можно использовать группы безопасности предварительно созданные в облаке.

### Установка дополнительных групп безопасности (security groups) на статических и master-узлах

Установку дополнительных групп безопасности  можно задать:

* при создании кластера;

* в существующем кластере.

В обоих случаях дополнительные группы безопасности указываются в `OpenStackClusterConfiguration`:
...
Для дополнительных групп безопасности пропишите параметр `additionalSecurityGroups` для всех `OpenStackInstanceClass` в кластере, которым нужны дополнительные группы безопасности. Подробнее — [параметры модуля `cloud-provider-openstack`](../../modules/030-cloud-provider-openstack/configuration.html).

## Как поднять гибридный кластер?

Гибридный кластер представляет собой кластер, в котором могут содержаться узлы bare metal и узлы OpenStack. Для создания такого кластера необходимо наличие L2-сети между всеми узлами кластера.
...
> **Важно!** Cloud-controller-manager синхронизирует состояние между OpenStack и Kubernetes, удаляя из Kubernetes те узлы, которых нет в OpenStack. В гибридном кластере подобное не всегда необходимо. Если узел Kubernetes запущен не с параметром `--cloud-provider=external`, он автоматически игнорируется (Deckhouse прописывает `static://` на узлы в `.spec.providerID`, а cloud-controller-manager такие узлы игнорирует).

### Подключение storage в гибридном кластере

Если требуются PersistentVolumes на узлах, подключаемых к кластеру OpenStack, необходимо создать StorageClass с нужным OpenStack volume type. Получить список типов можно с помощью команды `openstack volume type list`.
...
6. Создайте образ и передайте в него тип диска, который будет использоваться (если OpenStack не поддерживает локальные диски или если эти диски не подходят для работы):
...
Чтобы проверить, поддерживает ли провайдер группы безопасности, выполните команду `openstack security group list`. Если в ответе отсутствуют ошибки, значит [группы безопасности](https://docs.openstack.org/nova/pike/admin/security-groups.html) поддерживаются.

## Как настроить работу онлайн-изменения размера дисков?

OpenStack API рапортует об изменении размера диска, но Cinder не оповещает Nova о том, что диск изменился, поэтому диск внутри гостевой ОС остается старого размера.

Для устранения проблемы пропишите в `cinder.conf` параметры доступа к Nova API.
Например:
...
Локальный диск — это ephemeral disk. Удаляется вместе с ВМ.
Сетевой диск — это persistent disk (cinder storage). Остается в облаке, когда ВМ удаляется.

* Для master-узла предпочтительнее сетевой диск, чтобы узел мигрировал между гипервизорами.
* Для ephemeral-узла предпочтительнее локальный диск, чтобы экономить на стоимости.

Не все cloud-провайдеры поддерживают использование локальных дисков. Если локальные диски не поддерживаются, для ephemeral-узлов придется использовать сетевые диски.
...
В `OpenStackInstanceClass` существует параметр `rootDiskSize`, а также в OpenStack flavor существует параметр размера диска.

Зависимость комбинации параметров для диска представлена в таблице:
...
* Проконтролируйте тип диска. Тип диска будет взят из образа ОС, если он [задан](#как-переопределить-тип-диска-по-умолчанию-cloud-провайдера). Если этого не произойдет, тип диска будет взят из [volumeTypeMap](cluster_configuration.html#openstackclusterconfiguration-masternodegroup-volumetypemap).
...
* Проконтролируйте тип диска. Тип диска будет взят из образа ОС, если он [задан](#как-переопределить-тип-диска-по-умолчанию-cloud-провайдера). Если этого не произойдет, будет использоваться тип диска по умолчанию cloud-провайдера.

### Как проверить объем диска в flavor?

Выполите проверку объема диска во flavor. 

В примере, команда `openstack flavor show` примет в качестве аргумента название `flavor` и выведет информацию о flavor.

Параметр `-c disk` укажет информацию о диске.
...
Если у cloud-провайдера доступно несколько типов, можно указать тип диска для образа по умолчанию.
