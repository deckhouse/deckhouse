...
var _ = Describe("Modules :: cloud-provider-openstack :: hooks :: change host ip ::", func() {
...
`, 2))
			...
			It("Should delete the pod", func() {
				Expect(f).To(ExecuteSuccessfully())
				Expect(f.KubernetesResource("Pod", "d8-cloud-provider-openstack", "ccm-test").Exists()).To(BeFalse())
			})
		})
	})
