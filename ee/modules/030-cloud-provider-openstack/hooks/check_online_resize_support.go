...
}, handleCheckOnlineResize)

func handleCheckOnlineResize(input *go_hook.HookInput) error {
	_, ok := input.Values.GetOk("cloudProviderOpenstack.internal.supportsOnlineDiskResize")
	if ok {
		return nil
	}
	...
	input.Values.Set("cloudProviderOpenstack.internal.supportsOnlineDiskResize", isSupported)
