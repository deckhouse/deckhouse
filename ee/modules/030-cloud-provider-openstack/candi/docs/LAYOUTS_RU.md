...
- разрешение входящего трафика по протоколу `TCP` и порту 22 из CIDR, указанных в `sshAllowList` (по умолчанию `0.0.0.0/0`);
...
- для master-узлов — в секции `masterNodeGroup` ресурса `OpenStackClusterConfiguration`;
- для статических узлов — в секции `nodeGroups` ресурса `OpenStackClusterConfiguration`;
- для эфемерных узлов — в ресурсе `OpenStackInstanceClass`.
