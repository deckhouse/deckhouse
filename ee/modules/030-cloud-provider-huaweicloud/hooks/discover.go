...
func handleCloudProviderDiscoveryDataSecret(input *go_hook.HookInput) error {
	if len(input.Snapshots["cloud_provider_discovery_data"]) == 0 {
		...
		if len(input.Snapshots["storage_classes"]) == 0 {
			input.LogEntry.Warn("failed to find storage classes for huaweicloud provisioner")
		...
		input.LogEntry.Infof("Found huaweicloud storage classes using StorageClass snapshots: %v", storageClasses)
...
func handleDiscoveryDataVolumeTypes(
	...
	input.LogEntry.Infof("Found huaweicloud storage classes using StorageClass snapshots, StorageDomain discovery data: %v", storageClasses)
...
func setStorageClassesValues(
	...
	values.Set("cloudProviderHuaweicloud.internal.storageClasses", storageClasses)

	def, ok := values.GetOk("cloudProviderHuaweicloud.storageClass.default")
	if ok {
		values.Set("cloudProviderHuaweicloud.internal.defaultStorageClass", def.String())
		return
	}

	if defaultSCName != "" {
		values.Set("cloudProviderHuaweicloud.internal.defaultStorageClass", defaultSCName)
		return
	}
	values.Remove("cloudProviderHuaweicloud.internal.defaultStorageClass")
}
