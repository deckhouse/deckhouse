...
func handleCloudProviderDiscoveryDataSecret(input *go_hook.HookInput) error {
	if len(input.Snapshots["cloud_provider_discovery_data"]) == 0 {
		...
		if len(input.Snapshots["storage_classes"]) == 0 {
			input.LogEntry.Warn("failed to find storage classes for sample provisioner")
	...
	_, err := config.ValidateDiscoveryData(&discoveryDataJSON, []string{"/deckhouse/ee/candi/cloud-providers/sample/openapi"})
	if err != nil {
		return fmt.Errorf("failed to validate 'discovery-data.json' from 'd8-cloud-provider-discovery-data' secret: %w", err)
	}

	var discoveryData cloudDataV1.SampleCloudProviderDiscoveryData
	...
	input.Values.Set("cloudProviderSample.internal.providerDiscoveryData", discoveryData)
...
func handleDiscoveryDataVolumeTypes(
	...
	volumeTypes []cloudDataV1.SampleStorageProfile,
) {
	var defaultSCName string
	...
	for _, volumeType := range volumeTypes {
		...
		storageClassName := getStorageClassName(volumeType.Name)

		if volumeType.IsDefault {
			defaultSCName = storageClassName
		}
		...
		volumeTypesMap[storageClassName] = storageClass{
			Name: storageClassName,
		}
	}

	excludes, ok := input.Values.GetOk("cloudProviderSample.storageClass.exclude")
	if ok {
...
func setStorageClassesValues(
	...
	values.Set("cloudProviderSample.internal.storageClasses", storageClasses)

	def, ok := values.GetOk("cloudProviderSample.storageClass.default")
	if ok {
		values.Set("cloudProviderSample.internal.defaultStorageClass", def.String())
		return
	}

	if defaultSCName != "" {
		values.Set("cloudProviderSample.internal.defaultStorageClass", defaultSCName)
		return
	}
	values.Remove("cloudProviderSample.internal.defaultStorageClass")
}
