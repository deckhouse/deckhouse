...
- `"InPlaceOrRecreate"` (default) — режим GA, работает в Kubernetes 1.33 и новее. VPA пытается изменить ресурсы пода без пересоздания. Если обновить ресурсы «на месте» (in-place) невозможно, VPA пересоздает под аналогично режиму `Recreate`.
