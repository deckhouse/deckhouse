...
- `InPlaceOrRecreate` — режим по умолчанию. Работает в Kubernetes 1.33 и новее. VPA пытается изменить ресурсы пода без пересоздания. Если обновить ресурсы «на месте» (in-place) невозможно, VPA пересоздает под аналогично режиму `Recreate`.
...
- `Auto` — устаревший режим. Его поддержка будет прекращена в будущих версиях DKP. Рекомендуется перейти на один из поддерживаемых режимов работы.
...
- При использовании режимов, допускающих пересоздание подов, VPA может пересоздать под, если обновить `resource requests` без перезапуска невозможно. Новый под может быть создан на другом узле.
