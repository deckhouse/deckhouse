...
| **Реализованные меры защиты, выявленные по исходным данным** | Запуск от `nobody` (UID 64535); контейнер PSS Restricted (`pss_restricted_flexible`); образ `common/distroless`; бинарники `chmod 0700`; раздельные ServiceAccount'ы; метрики за `kube-rbac-proxy` (`SubjectAccessReview`); авторотация TLS webhook'а (Go-хук: `OnBeforeHelm`/cron `15 10 * * *`/watch Secret; перегенерация за 7 дней); TLS-секрет только для чтения; `failurePolicy: Ignore` (fail-open); `objectSelector` исключает самопроверку; сборка из закреплённого upstream-тега + патчи; приоритетные классы; Eviction API учитывает PDB нагрузок; гард-рейлы updater (`--eviction-rate-limit=1`, `--eviction-tolerance=0.1`, `--min-replicas=2`). Ограничения: PDB защитен только у admission-controller (у recommender/updater `minAvailable: 0`); `automountServiceAccountToken: false` на SA переопределён `true` в подах — токены SA монтируются (не мера подавления) |
