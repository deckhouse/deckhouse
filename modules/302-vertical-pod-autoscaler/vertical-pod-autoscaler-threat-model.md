...
Документ подготовлен в соответствии с «Методикой моделирования угроз и поверхности атаки» (`5.7-threat-modeling.md`).

**Исходные данные, использованные при моделировании:**

| Источник | Состояние | Примечание |
| -------- | --------- | ---------- |
| Локальный git-репозиторий модуля `302-vertical-pod-autoscaler` | Использован | Проанализированы `templates/`, `images/`, `hooks/`, `crds/`, `openapi/`, `monitoring/`, `docs/`, `module.yaml`, `oss.yaml`, `.dmtlint.yaml` |
| Методика моделирования угроз и поверхности атаки (`5.7-threat-modeling.md`) | Использована | Структура и шаблоны разделов 1–6 и приложений |
| Перечень угроз БДУ ФСТЭК России | Использован файл `Угрозы.csv` | Содержит идентификаторы УБИ.1–УБИ.11 (обобщённые классы угроз). Доступ к онлайн-разделу `https://bdu.fstec.ru/threat-section` в среде моделирования отсутствовал; использован предоставленный CSV |
| Файл аббревиатур и терминов `abbr.md` | Использован | Добавлен пользователем в каталог модуля. Терминология модели согласована с определениями файла (`Источник угрозы`, `Поверхность атаки`, `Уровень доверия`, `Доверенный`/`Ограниченно доверенный`/`Недоверенный субъект`, `Потенциал нарушителя`, `БДУ`, `SBOM`, `VEX` и др.); сводка использованных терминов приведена в приложении «Термины и сокращения». **Примечание:** файл содержит незакрытые маркеры git-конфликта (`<<<<<<< app/docs/includes/abbr.md`, строки 24 и 33) — требует уточнения/устранения у сопровождающего |
| Документация модуля `docs/` | Использована | `README.md`, `FAQ.md`, `CONFIGURATION.md`, `EXAMPLES.md`, `docs/internal/README.md` |

**Примечание о границах оценки (раздел 3 Методики):** поверхность атаки не учитывает угрозы от аппаратных уязвимостей и root-пользователя в операционной системе; отсутствие шифрования трафика через loopback внутри модуля не учитывается. По умолчанию предполагается, что модуль лежит на поверхности атаки; данное предположение проверяется по результатам моделирования.
...
| **Неопределённости** | Требует уточнения: фактическая версия upstream VPA (в `oss.yaml` для id `autoscaler` указана `1.6.0`, тег клонирования `vertical-pod-autoscaler-1.6.0`); точные версии общих образов `common/distroless`, `builder/golang-alpine`, `kube-rbac-proxy` (управляются централизованно Deckhouse, в модуле не зафиксированы); наличие/полнота SBOM и VEX для релизных образов (в каталоге модуля не обнаружены); наличие `NetworkPolicy`/сетевой изоляции для компонентов в `kube-system`; фактические `updateMode` и `resourcePolicy` эксплуатируемых VPA-объектов; политика выдачи прав `create VerticalPodAutoscaler`/`create pods` пользователям; отсутствие файла `abbr.md` в рабочем каталоге |
...
| ---------- | -------------------------------- |
| Recommender обращается к `aggregating-proxy` с `--prometheus-insecure=true` (TLS-верификация отключена), bearer-токен SA в запросе. | При MITM на внутрикластерном пути или подмене сервиса возможна подмена истории метрик → искажение рекомендаций. Требует уточнения наличие `NetworkPolicy`/mTLS для `kube-system`↔`d8-monitoring`. |
...
| Updater имеет кластерные права `pods/eviction` create и `pods/resize`+`pods` patch; флаг `--in-place-skip-disruption-budget`. | Потенциальное массовое вытеснение/ресайз чужих подов; для in-place PDB не учитывается. Фактический `updateMode` эксплуатируемых VPA требует уточнения. |
...
| **Используемая уязвимость** | `--prometheus-insecure=true`; требует уточнения наличие `NetworkPolicy`/mTLS между `kube-system` и `d8-monitoring` |
...
| **УБИ.4, УБИ.3, УБИ.9** | AS-02 | Да | Средняя (требует уточнения сетевой модели) | Высокий | Высокий | **Условно актуальна** | `--prometheus-insecure=true` отключает TLS-верификацию; реализуемость зависит от наличия `NetworkPolicy`/mTLS между `kube-system` и `d8-monitoring`. |
...
| TM-02 | Искажение рекомендаций через подмену источника метрик | Условно актуальна | vpa-recommender, `aggregating-proxy`, Metrics API | Высокий при отсутствии сетевой изоляции |
...
| TM-02 | Высокий при отсутствии сетевой изоляции | Обеспечить `NetworkPolicy`/mTLS между `kube-system` и `d8-monitoring`; по возможности отказаться от `--prometheus-insecure=true` в пользу проверяемой CA; контролировать целостность источника метрик; тестировать устойчивость recommender к аномальным метрикам. |
...
| ------ | ----------------- |
| Наличие `NetworkPolicy`/mTLS для трафика `kube-system`↔`d8-monitoring` и между компонентами. | Определяет перевод TM-02 (и частично TM-08) в актуальные или неактуальные. |
| Фактические `updateMode`/`resourcePolicy` эксплуатируемых VPA-объектов и политика выдачи прав `create VerticalPodAutoscaler`/`create pods`. | Влияет на реализуемость TM-01 и TM-07. |
| Полный SBOM и актуальный VEX релизных образов; точная версия upstream VPA и общих образов (`distroless`, `golang-alpine`, `kube-rbac-proxy`). | Необходимы для точной оценки TM-09 и known-CVE. |
| Полнота аудита Kubernetes API (eviction/resize/patch VPA-status, изменения checkpoint и ModuleConfig) и сборочного конвейера. | Нужна для оценки класса Repudiation и остаточного риска. |
| Обоснование хранения закрытого ключа CA в Secret `vpa-tls-certs` и возможность его исключения. | Влияет на остаточный риск TM-04. |
| Целесообразность размещения модуля в `kube-system` против выделенного `d8-*` namespace. | Влияет на последствия TM-05 и возможность сетевой/RBAC-изоляции. |
