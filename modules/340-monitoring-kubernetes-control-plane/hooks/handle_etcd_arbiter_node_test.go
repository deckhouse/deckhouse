...
User-stories:
1. Hook must discover nodes with label node.deckhouse.io/etcd-arbiter and set monitoringKubernetesControlPlane.internal.hasEtcdArbiterNode to true if exactly one such node exists, else — to false.
...
var _ = Describe("Modules :: monitoring-kubernetes-control-plane :: hooks :: handle_etcd_arbiter_node ::", func() {
	const (
		initValuesString       = `{"monitoringKubernetesControlPlane":{"internal":{}},"global":{"enabledModules":[]}}`
...
metadata:
  ...
  labels:
	...
	Context("Empty cluster", func() {
		...
		It("`monitoringKubernetesControlPlane.internal.hasEtcdArbiterNode` must be false", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(f.BindingContexts.Array()).ShouldNot(BeEmpty())

			Expect(f.ValuesGet("monitoringKubernetesControlPlane.internal.hasEtcdArbiterNode").Bool()).To(BeFalse())
		})
	})
	...
	Context("One etcd arbiter node in cluster", func() {
		...
		It("`monitoringKubernetesControlPlane.internal.hasEtcdArbiterNode` must be true", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(f.BindingContexts.Array()).ShouldNot(BeEmpty())

			Expect(f.ValuesGet("monitoringKubernetesControlPlane.internal.hasEtcdArbiterNode").Bool()).To(BeTrue())
		})
	})
