...
- Если у приложения есть безопасный endpoint для запросов проверки состояния, то вы можете воспользоваться дополнительными опциями:
```yaml
      - name: ca-auth-proxy
        image: flant/kube-ca-auth-proxy:v0.5.2
        args:
...
А у контейнера с приложением использовать такую liveness/readiness probe:
```yaml
  readinessProbe:
    httpGet:
      path: /
