...
`
	...
	Context("apiserver tests", func() {
		assertKubeadmConfigServerExtraArgs := func(ff *Config, hasArgs map[ArgV4]bool) {
			...
			for arg, shouldExist := range hasArgs {
				if shouldExist {
					Expect(config.APIServer.ExtraArgs).To(ContainElement(arg))
					continue
				}
				Expect(config.APIServer.ExtraArgs).ToNot(ContainElement(arg))
			}
		}

		Context("Encryption kubeadm extra args", func() {
			getEncryptionArgsAssertMap := func(hasConfigPath, autoReload bool) map[ArgV4]bool {
				return map[ArgV4]bool{
					{
			...
			Context("Without secretEncryptionKey and signature", func() {
				...
				It("should not have args", func() {
					assertKubeadmConfigServerExtraArgs(f, getEncryptionArgsAssertMap(false, false))
				})
			})
			...
			Context("With secretEncryptionKey", func() {
				...
				It("should have both args", func() {
					assertKubeadmConfigServerExtraArgs(f, getEncryptionArgsAssertMap(true, true))
				})
			})
