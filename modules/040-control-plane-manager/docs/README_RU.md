...
- **Управление сертификатами**, необходимыми для работы control-plane, в том числе продление, выпуск при изменении конфигурации и т. п. Автоматически поддерживает безопасную конфигурацию control plane и быстро добавляет дополнительные SAN для организации защищенного доступа к API Kubernetes.
...
Если указанная для обновления версия (с параметром [kubernetesVersion](../../installing/configuration.html#clusterconfiguration-kubernetesversion)) не соответствует текущей версии control plane в кластере, запускается умная стратегия изменения версий компонентов:
* Общие замечания:
...
* При upgrade:
  ...
  - На каждом этапе сначала обновляется версия control plane, после чего происходит обновление kubelet на узлах кластера.  
* При downgrade:
  - Успешный downgrade гарантируется только на одну версию от максимальной минорной версии control plane, когда-либо использовавшейся в кластере.
  - На узлах кластера выполняется понижение версии kubelet, после чего производится снижение уровня контроля plane компонентов.
...
- совершаются с ресурсами в системных пространствах иимен `kube-system`, `d8-*`.
