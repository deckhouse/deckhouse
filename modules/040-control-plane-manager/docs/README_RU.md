...
Маркер разрешено ставить только на листовых полях (строки, числа, логические значения) и на объектах или массивах (в этом случае чувствительным считается всё поддерево). Установка маркера на корне схемы запрещена и будет отклонена API-сервером при применении CRD.
