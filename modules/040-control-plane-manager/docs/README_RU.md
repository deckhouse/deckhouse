...
- **Периодическая дефрагментация etcd**. В кластерах с тремя и более членами etcd функция включена по умолчанию. Подробнее — в описании параметра [`defrag`](configuration.html#parameters-etcd-defrag).
