...
- **Малые кластеры** (нод <100): 1000 завершенных Pod'ов
- **Средние кластеры** (100≤ нод <300): 3000 завершенных Pod'ов  
- **Крупные кластеры** (нод ≥300): 6000 завершенных Pod'ов

*Примечание: Эта функция применяется только в средах, где параметр `terminated-pod-gc-threshold` можно настраивать. В управляемых Kubernetes-сервисах (таких как EKS, GKE, AKS) это значение контролируется провайдером.*
