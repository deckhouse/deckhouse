...
- **Мониторинг control plane**. Модуль организует безопасный сбор метрик и предоставляет базовый набор правил мониторинга компонентов control plane (`kube-apiserver`, `kube-controller-manager`, `kube-scheduler` и `kube-etcd`). См. [Мониторинг control plane](#мониторинг-control-plane).
