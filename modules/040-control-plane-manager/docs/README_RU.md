...
Если измерение (`cpu` / `memory`) **не** задано, Deckhouse раз в сутки автоматически подстраивает requests каждого компонента control plane по среднему потреблению за 7 дней. Задание значения для измерения отключает автоматику этого измерения у всех четырёх компонентов и применяет фиксированный процент от общего бюджета.
