...
Версия Kubernetes в кластере определяется в едином порядке: `kubernetesVersion` в ModuleConfig `control-plane-manager`, затем устаревшее поле [`ClusterConfiguration.kubernetesVersion`](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#clusterconfiguration-kubernetesversion), затем версия по умолчанию текущего релиза Deckhouse. Значение из ModuleConfig имеет приоритет всегда, когда оно задано, включая `Default`; пока оно не задано, версию определяет устаревшее поле, и Deckhouse поднимает алерт `D8UnsetKubernetesVersionInModuleConfig` с просьбой мигрировать.
