...
По умолчанию DKP вычисляет запросы CPU и памяти для каждого статического пода control plane (`kube-apiserver`, `etcd`, `kube-controller-manager`, `kube-scheduler`) автоматически и индивидуально по компонентам. Каждый запрос считается как фиксированная нижняя граница плюс линейный рост от общего числа узлов в кластере с ограничением сверху. Это держит запросы близко к реальному потреблению на небольших кластерах и масштабирует их по мере роста кластера. Результат не зависит от размера master-узла — нагрузка на control plane определяется размером кластера, а не железом мастера. У подов control plane нет лимита CPU, поэтому кратковременные всплески обслуживаются за счёт свободных ядер master-узла.
