...
- маркер запрещён внутри `items` у массивов с `x-kubernetes-list-type: map`, потому что восстановление замаскированных значений выполняется по индексу, а элементы list-map определяются по ключам, а не по позиции.
