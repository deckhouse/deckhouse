...
По умолчанию DKP вычисляет запросы CPU и памяти для каждого статического пода control plane (`kube-apiserver`, `etcd`, `kube-controller-manager`, `kube-scheduler`) автоматически и индивидуально по компонентам. Каждый запрос выбирается из набора дискретных тиров в зависимости от общего числа узлов в кластере. Тиры (вместо непрерывной формулы) сохраняют отрендеренные манифесты статических подов стабильными внутри тира, поэтому поды control plane перезапускаются только на редких границах тиров, а не при каждом изменении числа узлов. Это держит запросы близко к реальному потреблению на небольших кластерах и масштабирует их по мере роста кластера. У подов control plane нет лимита CPU, поэтому кратковременные всплески обслуживаются за счёт свободных ядер master-узла.

Чтобы control plane никогда не занимал весь master-узел, сумма автоматически вычисленных запросов ограничивается 75% от полезного allocatable наименьшего master-узла (allocatable за вычетом резерва kubelet и резерва на каждый узел). Оставшаяся ёмкость остаётся для остальных подов на master-узлах (CNI, CSI, мониторинг, сам control-plane-manager). Если master-узел слишком мал для текущего числа узлов в кластере, запросы урезаются под лимит, и срабатывает алерт `D8ControlPlaneMasterResourcesInsufficient`, сигнализируя, что master-узлы следует увеличить (или задать запросы явно).

Автоматически вычисленные значения можно переопределить, задав фиксированный суммарный бюджет запросов CPU и/или памяти для компонентов control plane на каждом master-узле.
...
Если значения заданы, они используются как общий бюджет запросов для компонентов control plane на каждом master-узле, переопределяя автоматический расчёт по компонентам. Deckhouse Kubernetes Platform (DKP) распределяет этот бюджет между статическими подами control plane при формировании их манифестов. CPU и память можно переопределять независимо — если задать только одно из значений, второе продолжит вычисляться автоматически по компонентам.
