...
Если для измерения (`cpu` / `memory`) **не** задан ручной `resourcesRequests` (и нет глобального fallback), а модуль `prometheus-metrics-adapter` включён, DKP раз в сутки автоматически подстраивает per-component requests по среднему потреблению контейнера за 7 дней. Ручной override для измерения отключает автотюн этого измерения у всех четырёх компонентов и возвращает фиксированный %-сплит общего бюджета.
