...
1. Делаем [бэкап](faq.html#резервное-копирование-etcd-и-восстановление) `etcd` и `/etc/kubernetes`
1. Переносим архив на сервер вне кластера (на локальную машину)
1. Нужно убедится что в кластере нет [алертов](..300-prometheus/faq.html#как-получить-информацию-об-алертах-в-кластере) которые помешают создать новые master
