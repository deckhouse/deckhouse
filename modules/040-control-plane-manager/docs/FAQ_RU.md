...
**Когда узлы возвращаются в онлайн после длительного простоя**: как только master-узлы запустятся и Kubernetes API станет доступен, `control-plane-manager` обнаруживает истёкшие или скоро истекающие сертификаты и автоматически создаёт операции по их обновлению. Дождитесь, пока все поды `d8-control-plane-manager` в пространстве имён `kube-system` перейдут в состояние `Ready`:

```shell
kubectl -n kube-system wait --for=condition=Ready pod -l app=d8-control-plane-manager --timeout=600s
```

**Если кластер не может восстановиться автоматически** (например, если все сертификаты компонентов управляющего слоя истекли и API-сервер не может запуститься): это сценарий аварийного восстановления. Воспользуйтесь процедурами резервного копирования и восстановления кластера, описанными в [официальной документации Deckhouse](https://deckhouse.io/products/kubernetes-platform/documentation/v1/admin/configuration/backup/backup-and-restore.html).
