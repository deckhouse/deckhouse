...
За распределение Pod’ов по узлам отвечает компонент kubernetes `scheduler`.
У него есть 2 фазы - Filtering и Scoring (на самом деле их больше - pre-filtering / post-filtering, но это для гибкости и оптимизации, глобально можно свести к вышеуказанным 2 фазам).

### Общее устройство scheduler
...
По умолчанию функция выбора количества узлов - линейная, для кластера из <=50 узлов будут выбраны 100% узлов, для кластера из 100 узлов - 50%, а для кластера из 5000 узлов - 10%.
Минимальное значение - 5%, при количестве узлов >5000. Поэтому, при настройках по умолчанию, которые мы используем, узел может не попасть в список возможных, даже если вам кажется, что все условия выполняются.
Данную настройку можно переопределить, установив для scheduler флаг _percentageOfNodesToScore_.

После того как выбраны узлы, подходящие под условия, запускается фаза **Scoring**. Каждый плагин анализирует список фильтрованных узлов и каждому узлу назначает свою оценку (score). Оценки от разных плагинов суммируются.
Здесь происходит оценка доступных ресурсов на узлах, pod capacity, affinity, volume provisioning, и так далее.
...
- [Подробности фильтрации нод](https://kubernetes.io/docs/concepts/scheduling-eviction/scheduler-perf-tuning/)
- [Исходники](https://github.com/kubernetes/kubernetes/tree/master/cmd/kube-scheduler)
