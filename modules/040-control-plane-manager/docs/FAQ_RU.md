...
{% alert level="warning" %}
Перед добавлением или удалением мастера, при использовании модуля **Stronghold** обязательно убедитесь, что модуль полностью вернулся в работоспособное состояние.
Также рекомендуется создать [резервную копию данных модуля](https://deckhouse.ru/modules/stronghold/stable/auto_snapshot.html) **Stronghold** перед началом любых изменений.  
{% endalert %}
