...
## Как вручную обновить сертификаты компонентов control plane?

Может возникнуть ситуация, когда мастер ноды кластера находятся выключенными долгое время. За это время сертификаты компонентов control plane могут истечь. После включения нод, сертификаты не обновятся автоматически, поэтому это необходимо сделать вручную.
Обновление сертификатов компонентов control plane происходит с помощью утилиты `kubeadm`
Чтобы обновить сертификаты control plane необходимо:  
