...
1. Отметьте чувствительные поля в схеме CRD маркером `x-kubernetes-sensitive-data: true`.
   Маркер должен находиться на поле типа `string`, `integer`, `number`, `boolean`, `object` или `array` (маркер на `object` или `array` делает чувствительным всё поддерево); также поддерживаются поля с `x-kubernetes-int-or-string: true`. Маркер нельзя ставить на корне схемы (на узле `openAPIV3Schema`) и внутри веток `anyOf`, `oneOf`, `allOf`, `not`.
