...
* Если удаление не нарушает кворум в etcd (в корректно функционирующем кластере это все ситуации, кроме перехода 2 -> 1):
    1. Выполнить drain мастера
    2. Если удаление мастера согласовано, то необходимо удалить виртуальную машину обычным способом.
...
* Если удаление нарушает кворум (переход 2 -> 1):
    1. Выполнить drain мастера
    2. Остановить kubelet на узле (не останавливая контейнер с etcd):
       ```
       ...
       ```
    3. Удалить объект Node из Kubernetes
    4. [Дождаться](#как-посмотреть-список-memberов-в-etcd), пока etcd member будет автоматически удален.
    5. Если удаление мастера согласовано, то необходимо удалить виртуальную машину обычным способом.
