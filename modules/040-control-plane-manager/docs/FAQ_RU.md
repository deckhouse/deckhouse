...
1. Пометьте чувствительные поля в схеме CRD маркером `x-kubernetes-sensitive-data: true`.
   Маркер разрешено ставить только на листовых полях и на объектах или массивах (в этом случае всё поддерево считается чувствительным). Установка маркера на корне схемы запрещена и будет отклонена при применении CRD.
