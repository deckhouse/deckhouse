...
- **TaintToleration** — имплементирует механизм [taints and tolerations](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/). Фазы: **Filtering, Scoring**
...
Сначала идет фаза фильтрации (**Filtering**). В этот момент работают `filter`-плагины, которые из всего списка узлов, выбирают попадающие под условия фильтров (`taints`, `nodePorts`, `nodeName`, `unschedulable`, и т.д.). Если узлы лежат в разных зонах, то выбор идет по очереди между зонами, чтобы не размещать все Pod'ы в одной зоне.

Например, при следующем распределении по зонам:
...
Обратите внимание, что с целью оптимизации выбираются не все попадающие под условия узлы, а только их часть. По умолчанию функция выбора количества узлов — линейная. Для кластера из <=50 узлов будут выбраны 100% узлов, для кластера из 100 узлов — 50%, а для кластера из 5000 узлов - 10%. Минимальное значение - 5%, при количестве узлов >5000. Поэтому, при настройках по умолчанию, которые мы используем, узел может не попасть в список возможных, даже если вам кажется, что все условия выполняются.
Данную настройку можно переопределить, установив для scheduler флаг `percentageOfNodesToScore`.

После того как выбраны узлы, подходящие под условия, запускается фаза **Scoring**. Каждый плагин анализирует список отфильтрованных узлов и каждому узлу назначает свою оценку (score). Оценки от разных плагинов суммируются. На этой фазе происходит оценка доступных ресурсов на узлах, pod capacity, affinity, volume provisioning, и так далее. По итогам этой фазы выбирается узел с наибольшей оценкой. Если узлов с одинаковой оценкой несколько, выбор узла идет случайным образом.

В итоге, на выбранном узле и запускается Pod.
