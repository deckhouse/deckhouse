...
Чтобы проверить механизм запрета обработки данных с некорректной подписью, намеренно измените значение поля `signature`.
...
1. Намеренно измените поле `signature` в секрете:
