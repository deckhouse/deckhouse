...
Forked from https://github.com/kubernetes/kops/blob/master/cmd/kube-apiserver-healthcheck/main.go
*/
package main
...
var (
	apiServerAddress string = "127.0.0.1"
	apiServerPort    int    = 443
)
...
func sanitizeRequest(r *http.Request, allowedQueryParameters []string) *http.Request {
	u := &url.URL{
		Scheme: "https",
		Host:   fmt.Sprintf("%s:%d", apiServerAddress, apiServerPort),
		Path:   r.URL.Path,
	}
...
func run() error {
	...
	flag.StringVar(&apiServerAddress, "api-server-address", apiServerAddress, "Api server address")
	flag.IntVar(&apiServerPort, "api-server-port", apiServerPort, "Api server port")
