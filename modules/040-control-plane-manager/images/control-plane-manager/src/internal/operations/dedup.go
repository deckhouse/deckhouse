...
func Covered(current []controlplanev1alpha1.ControlPlaneOperation, d Decision) bool {
	return hasActiveOperation(current, d.component, d.match)
}

func (d Decision) match(op *controlplanev1alpha1.ControlPlaneOperation) bool {
	switch d.kind {
	case KindObserve:
		return true
	case KindCertRenew:
		return hasRenewalStep(op)
	case KindSignatureRenew:
		return hasSignatureStep(op)
	default:
		return op.Spec.DesiredConfigChecksum == d.intended.Config &&
