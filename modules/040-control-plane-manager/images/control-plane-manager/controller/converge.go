...
func syncExtraFiles() error {
	...
	log.Info(fmt.Sprintf("phase: sync extra files to %s", dstDir))
...
func rejoinEtcdMemberIfNeeded(etcd *Etcd) error {
	...
	if err == nil {
		...
		if !memberExists {
			log.Info(fmt.Sprintf("etcd member folder exists but %s is not a member of the cluster, cleanup etcd folder and re-join member to the cluster", config.NodeName))
...
metadata:
  ...
  annotations:
	...
	log.Info(fmt.Sprintf("write checksum patch for component %s", componentName))
...
func waitPodIsReady(componentName string, checksum string) error {
	tries := 0
	log.Info(fmt.Sprintf("waiting for the %s pod component to be ready with the new manifest in apiserver", componentName))
	for {
		tries++
