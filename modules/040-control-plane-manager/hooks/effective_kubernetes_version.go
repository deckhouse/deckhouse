...
// moduleConfigK8sVersionSecretKey carries the resolved ModuleConfig control-plane-manager
// kubernetesVersion override (empty when unset or "Automatic") into the d8-cluster-configuration
// Secret, so that the update-observer controller (which only watches that Secret, not
// ModuleConfig) can pick up an operator-set version override. Kept in sync with the identically
// named key read in update-observer's cluster/const.go.
const moduleConfigK8sVersionSecretKey = "moduleConfigKubernetesVersion"
...
func ekvFilterSecret(unstructured *unstructured.Unstructured) (go_hook.FilterResult, error) {
	...
	versions.ModuleConfig = string(secret.Data[moduleConfigK8sVersionSecretKey])
...
func handleEffectiveK8sVersion(ctx context.Context, input *go_hook.HookInput, dc dependency.Container) error {
	...
	configVersionRaw := resolveDeclaredKubernetesVersion(input)
	if configVersionRaw == "" {
		return fmt.Errorf("kubernetesVersion required (set it in ModuleConfig control-plane-manager or global.clusterConfiguration)")
	}
	...
	// Mirror the ModuleConfig control-plane-manager kubernetesVersion override (if any) into the
	// Secret, so update-observer (which only watches this Secret, not ModuleConfig) can see it.
	// "Automatic"/unset both mean "no override" and are stored as an empty string.
	mcVersion := input.Values.Get("controlPlaneManager.kubernetesVersion").String()
	if mcVersion == "Automatic" {
		mcVersion = ""
	}
	if mcVersion != versionsInSecret.ModuleConfig {
		encoded := base64.StdEncoding.EncodeToString([]byte(mcVersion))
		addToPatch(moduleConfigK8sVersionSecretKey, encoded)
	}
