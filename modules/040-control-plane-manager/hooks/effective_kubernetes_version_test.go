	...
	Context("Empty cluster", func() {
		...
		DescribeTable("version change",
			...
			Entry("upgrade: Node version lower than control plane, do not allow to bump effective version and max used version",
				input{
					nodeVersions:               []string{"v1.29.3", "v1.29.1", "v1.29.5", "v1.29.2"},
					maxUsedControlPlaneVersion: "1.30",
					configVersion:              "1.31",
					controlPlaneVersions:       []string{"1.30", "1.30", "1.30"},
				},
				output{
					maxUsedControlPlaneVersion: "1.30",
					effectiveVersion:           "1.30",
					minUsedVersion:             "1.29.1",
				},
			),
			Entry("upgrade: control plane and nodes are on the same version, allow bumping effective version and max used version", input{
				nodeVersions:               []string{"v1.29.10", "v1.29.3", "v1.29.5", "v1.29.2"},
				maxUsedControlPlaneVersion: "1.29",
				configVersion:              "1.30",
				controlPlaneVersions:       []string{"1.29", "1.29", "1.29"},
			},
				output{
					maxUsedControlPlaneVersion: "1.30",
					effectiveVersion:           "1.30",
					minUsedVersion:             "1.29.2",
				},
			),
			Entry("upgrade: control plane and nodes are on the same version (but kube-scheduler is on a lower version), do not bump effective version and max used version",
				input{
					nodeVersions:               []string{"v1.30.10", "v1.30.3", "v1.30.5", "v1.30.2"},
					maxUsedControlPlaneVersion: "1.30",
					configVersion:              "1.31",
					controlPlaneVersions:       []string{"1.30", "1.30", "1.29"},
				},
				output{
					maxUsedControlPlaneVersion: "1.30",
					effectiveVersion:           "1.30",
					minUsedVersion:             "1.30.2",
				},
			),
			...
			Entry("downgrade: nodes are downgraded already, lower effective version",
				input{
					nodeVersions:               []string{"v1.30.10", "v1.30.3", "v1.30.5", "v1.30.2"},
					maxUsedControlPlaneVersion: "1.31",
					configVersion:              "1.29",
					controlPlaneVersions:       []string{"1.31", "1.31", "1.31"},
				},
				output{
					maxUsedControlPlaneVersion: "1.31",
					effectiveVersion:           "1.30",
			...
			Entry("downgrade: nodes are downgraded already, but maxUsedControlPlaneVersion does not allow us to downgrade by more than 1",
				input{
					nodeVersions:               []string{"v1.29.4", "v1.29.3", "v1.29.5", "v1.29.2"},
					maxUsedControlPlaneVersion: "1.31",
					configVersion:              "1.29",
					controlPlaneVersions:       []string{"1.30", "1.30", "1.30"},
				},
