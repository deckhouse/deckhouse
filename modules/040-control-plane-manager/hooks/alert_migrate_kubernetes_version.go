...
func checkKubernetesVersionMigration(_ context.Context, input *go_hook.HookInput) error {
	input.MetricsCollector.Expire("D8ObsoleteKubernetesVersionInClusterConfiguration")
	...
	if ccVersion != "" && ccVersion != "Automatic" && (mcVersion == "" || mcVersion == "Automatic") {
		input.MetricsCollector.Set(
			"d8_obsolete_kubernetes_version_in_cluster_configuration", 1,
			map[string]string{},
			metrics.WithGroup("D8ObsoleteKubernetesVersionInClusterConfiguration"),
		)
	}
