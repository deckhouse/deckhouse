...
func generateSelfSignedCA(_ context.Context, input *go_hook.HookInput) error {
	const (
		certPath = "controlPlaneManager.internal.selfSignedCA.cert"
		keyPath  = "controlPlaneManager.internal.selfSignedCA.key"
	)

	publishAPIEnabled := input.Values.Get("controlPlaneManager.apiserver.publishAPI.ingress.enabled").Bool()
	publishAPIMode := input.Values.Get("controlPlaneManager.apiserver.publishAPI.ingress.https.mode").String()
