...
func handleExtenders(input *go_hook.HookInput) error {
	...
	for _, snapshot := range input.Snapshots["kube_scheduler_extenders"] {
		for _, config := range snapshot.([]KubeSchedulerWebhook) {
			urlPrefix, err := url.JoinPath(fmt.Sprintf("https://%s.%s.%s:%d", config.ClientConfig.Service.Name, config.ClientConfig.Service.Namespace, clusterDomain, config.ClientConfig.Service.Port), config.ClientConfig.Service.Path)
			if err != nil {
				return err
			}
