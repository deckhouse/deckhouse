...
Для мониторинга пользовательских приложений, запущенных в кластере, требуется дополнительная настройка. Данный модуль позволяет упростить процесс настройки, сведя её к указанию для нужного приложения определённого лейбла.
