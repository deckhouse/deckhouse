...
const (
	// UserRO is used by the node agents and by replication between replicas.
	UserRO = "registry-ro"
...
func (h Hosts) all() []string {
	...
	// The syncer reaches its own replica over the loopback interface, so the
	// certificate has to cover it too.
...
func (s *State) Complete() bool {
	...
	for _, user := range []User{s.RO, s.RW} {
