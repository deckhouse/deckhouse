...
func handleSwitch(_ context.Context, input *go_hook.HookInput) error {
	...
	// Recorded for the release requirement, so that the release which finally removes
	// the previous implementation cannot install on a cluster still running it.
	if enabled {
