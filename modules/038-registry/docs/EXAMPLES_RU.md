...
{% alert level="danger" %}
- При изменении режима registry или параметров registry, Deckhouse будет перезапущен.
- Недоступно переключение из режима `Local` в `Proxy` . Для переключения из режима `Local` необходимо переключить registry на другой доступный режим (например: `Direct`).
{% endalert %}
...
{% alert level="danger" %}
- При изменении режима registry или параметров registry, Deckhouse будет перезапущен.
- Недоступно переключение из режима `Proxy` в `Local` . Для переключения из режима `Proxy` необходимо переключить registry на другой доступный режим (например: `Direct`).
{% endalert %}
...
1. Подготовте архивы с образами DKP текущей версии. Для этого, воспользуйтесь командой `d8 mirror`.
...
1. Проверьте статус переключения registry в секрете `registry-state`, используя [инструкцию](faq.html#как-посмотреть-статус-переключения-режима-registry). В статусе необходимо дождатся запуска проверки `RegistryContainsRequiredImages`. Условие отобразит отсутствие или наличие образов в запущенном локальном registry.
...
1. Выполните загрузку образов в локальный registry с помощью команды `d8 mirror`. Загрузка образов в локальный registry осуществляется через Ingress по адресу `registry.${PUBLIC_DOMAIN}`.
