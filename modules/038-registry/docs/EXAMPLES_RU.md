...
любой registry, а агент начинает предпочитать кеш, оставляя upstream резервным путём, — поэтому
промах кеша с первой же минуты означает более медленную загрузку, а не неудачную.
...
нуля. Если возвращаться к кешу вы не собираетесь,
[освободите это место](faq.html#как-удалить-с-узла-оставшиеся-данные-кеша).

## Переход в air-gap

У изолированного кластера нет upstream'а: кеш — единственный источник образов, а путь внутрь —
`d8 mirror push`. Поскольку полнота кеша должна быть проверяемой, прежде чем ему можно
...
1. Опишите, что кеш должен держать, и уберите upstream:
...
кеша будет держать весь ожидаемый набор. Это единственный переход, который иначе мог бы оставить
...
Данные кеша на master-узлах намеренно остаются, чтобы повторное включение долилось из того, что
уже есть. Если кеш больше не понадобится,
[освободите это место](faq.html#как-удалить-с-узла-оставшиеся-данные-кеша).
