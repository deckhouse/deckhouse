...
Шаг 1 выполняется ДО обновления на релиз с текущей реализацией. Этот релиз не рендерит ни одного
объекта предыдущей реализации, поэтому кластер, приехавший в `Direct`, `Proxy` или `Local`, их уже
потерял, и шаг 1 ему недоступен: переключение режимов выполняет тот самый код, которого больше нет.
Такой кластер восстанавливается возвратом на предыдущий релиз, выполнением шага 1 там и повторным
обновлением. Изолированный кластер в `Local` не может выполнить шаг 1 в написанном виде, потому что
ему не с чем быть в `Unmanaged`; для него есть отдельная процедура — см.
[как миграровать изолированный кластер в Local](#как-миграровать-изолированный-кластер-в-local).
...
Цена: вторая копия набора образов на время миграции. Временный registry занимает столько же,
сколько занимает хранилище `Local`, поэтому рассчитывайте на двойной объём на диске до последнего
шага. Больше ничего не тратится — образы, уже лежащие на master-узлах, усыновляются, а не
скачиваются заново, см. шаг 6.
...
1. Поднимите OCI-registry в своём пространстве имён. Реализация любая; он должен отдавать TLS с
   сертификатом, который кластер может проверить, и быть доступен с каждого узла. «Внешний» он
   только в том смысле, который здесь важен: Deckhouse им не управляет, поэтому происходящее с
   объектами модуля его не выключает.
...
1. Включите модуль: `mode: Managed`, `storage.cache: true` и временный registry как upstream.
   Внутрикластерное хранилище поднимается на том же пути на хосте, который использовал `Local`,
   поэтому то, что уже лежит на этих дисках, усыновляется: наполнение проверяет его и дозагружает
   только недостающее.
