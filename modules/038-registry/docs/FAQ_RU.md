...
description: "Часто задаваемые вопросы о модуле registry Deckhouse Kubernetes Platform: миграция на модуль, обслуживание кэша и диагностика проблем с registry."
---

## Как устроена миграция на модуль registry?

Управлять путём загрузки образов в DKP можно двумя способами:
...
Кластер записывает, на какой реализации он фактически работает (см.
[следующий вопрос](#на-какой-реализации-работает-мой-кластер)). Пока передача управления
...
согласования и поднимает алерт
...
1. Чтобы модуль начал управлять путём загрузки, задайте `mode: Managed` в ModuleConfig
   ...
   кластера публикуется в секрете `registry-suggested-config` — см.
...
1. В ModuleConfig `deckhouse` задайте `registry.mode: Unmanaged`, сохранив тот же адрес
   ...
   [примеры переключения режимов](examples.html#примеры-для-предыдущей-реализации). Все узлы
   будут перенастроены на загрузку напрямую из внешнего registry, поэтому кэширование, которое
...
1. Чтобы вернуть кэширование внутри кластера, задайте `mode: Managed` с `storage.cache: true` и
   тем же внешним registry. Учтите, что кэш модуля устроен иначе, чем `Proxy`: одно хранилище с
...
кластере, но вне пространств имён Deckhouse; кластер переключается на него и обновляется, а
...
работал, и тот, на который обновлялся) и увеличил хранилище с 13,0 до 21,4 ГиБ, не считая кэша
...
можно загрузить. В тесте такой дедлок продержался 99 минут, и разрешить его удалось только
...
модуля проверяет и принимает на месте, а не скачивает заново (см. шаг 6). Но превратить два
...
1. Запустите временный OCI-registry в собственном пространстве имён. Реализация registry может
   быть любой, но он должен отдавать TLS с сертификатом, который кластер сможет проверить, и
   Deckhouse не должен им управлять: что бы ни происходило с объектами модуля, временный
   ...
   |---|---|---|
   | порт `hostNetwork` на IP самого узла (`<ip узла>:5000`) | работает | работает |
   | имя Service (`<service>.<namespace>.svc:<port>`) | не резолвится | работает |
   ...
   миграция пройдёт на одном адресе. Имя Service выглядит аккуратнее, но ломается на шаге 3,
   потому что container runtime узла не резолвит кластерный DNS.

1. Загрузите набор образов во временный registry: сначала скачайте его командой
   `d8 mirror pull` на машине, у которой есть доступ к registry Deckhouse, а затем отправьте во
...
1. Включите модуль: задайте `mode: Managed` с `storage.cache: true`, укажите временный registry
   ...
   Изменение этих настроек перезапускает процесс registry, поэтому `RegistryStorage` примерно
...
: Кластер всё ещё работает на предыдущей реализации. Ничего не сломано — миграция не завершена.
  См. [как устроена миграция](#как-устроена-миграция-на-модуль-registry).
...
: Часть узлов не может достучаться до API-сервера и маршрутизирует загрузки по копии
...
: Часть реплик кэша не держит весь ожидаемый набор образов. Пока настроен внешний registry, на
...
: Вы убрали внешний registry из конфигурации, но модуль продолжает им пользоваться, потому что
  кэш пока не может обслуживать кластер сам. Это безопасный исход: отключение внешнего registry
  при неполном кэше оставило бы узлы без источника образов. Алерт не разрешится сам, если кэш
...
: Изменение основного внешнего registry отклонено, и кластер продолжает пользоваться последним
  ...
  данные, но не содержит образов Deckhouse (обычно неверный путь репозитория).
...
: Ни одна реплика не выполняла сборку мусора неделю. Сборка — единственный механизм, который
  ...
  См. [что чистит кэш](#кэш-растёт-что-его-чистит).

`D8RegistryStaleCacheData`
: На узле лежат данные кэша, которые никто не использует. См.
  [как их удалить](#как-удалить-с-узла-оставшиеся-данные-кэша).

## Как удалить с узла оставшиеся данные кэша?

При выключении кэша данные в `/opt/deckhouse/registry` намеренно сохраняются: если включить кэш
обратно, он наполнится из того, что уже лежит на диске, а не будет скачивать всё заново — на
медленном канале это экономит часы. Автоматическое удаление сделало бы выключение кэша
...
Если включать кэш обратно вы не собираетесь, удалите каталог на узле:
...
- всё, что новее развёрнутого релиза, — это обновление в процессе или, в изолированном
  кластере, релиз, залитый намеренно;
- все теги, которые не являются версиями: имена каналов обновлений вроде `stable`, плавающие
  теги, всё залитое вручную. Сборщик не может знать, что они означают, поэтому не трогает их.
...
### Когда сборка запускается и почему реплика уходит в read-only
...
- сохранить результат промаха кэша — агент на узле уходит на внешний registry, то есть загрузка
  становится медленнее, но не падает;
- принять `d8 mirror push` — он падает с видимой ошибкой, и его можно повторить.
...
причина — в логе агента. Если файл есть, а загрузки всё равно падают, отказ находится за
...
- `replicas` — единственное место, где сообщается полнота кэша; каждая запись — отчёт реплики о
...
- `leader` — реплика, которая наполняется из внешнего registry и служит источником репликации
  ...
  изолированному кластеру застрять с пустым лидером и полным follower'ом.
