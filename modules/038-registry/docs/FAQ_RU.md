...
1. Следите за ходом передачи:
   ...
   Для ориентира — порядок и масштаб событий: передача фиксируется через пару минут после
   старта новой версии, агент появляется на узлах ещё через несколько минут, а объекты
   предыдущей реализации удаляются вскоре после этого. Загрузка образов работает на каждом
   из этих этапов.
...
когда хранилище модуля наполнится образами, временный registry удаляется.
...
Масштаб можно оценить на примере набора образов в 13 ГиБ (платформа с модулями): он занимает
около 21 ГиБ во временном registry (там лежат два релиза — тот, на котором кластер работает, и
тот, на который обновляется) и увеличивает хранилище с 13,0 до 21,4 ГиБ, не считая кеша образов
самого узла и системы. На master-узле со 100 ГиБ диска пик составляет около 38 ГиБ, а на узле с
50 ГиБ та же миграция упирается в вытеснение подов.
