...
> [!IMPORTANT]
> Переход `Local` → `Unmanaged` **не поддерживается**.
...
Компоненты `registry-nodeservices-<node>` запущены в Local-режиме. Образы берутся из Local хранилища локального реестра.
