...
flowchart TB
  ...
  Distribution -->|"проксирование"| ImageRegistry
...
Обращение `containerd` идёт на `127.0.0.1:5001` в static pod `registry-proxy-<node>`, запущенный на каждом узле. `registry-proxy-<node>` балансирует запросы на компоненты `registry-nodeservices-<node>` (static pod-ы на master-узлах, слушающие на `internal-ip:5001`), работающие в proxy-режиме и кеширующие образы из upstream registry в локальное хранилище (`/opt/deckhouse/registry`).
...
flowchart TB
  ...
  subgraph DKP["Deckhouse Kubernetes Platform"]
    ...
    subgraph RegistryModule["Модуль registry"]
      ...
      subgraph RegistryNodeservicesStaticPod["registry-nodeservices-&lt;master-node&gt; [static pod, кеширующий]<br/>internal-ip:5001"]
        direction TB
        Distribution["distribution"]
        Auth["auth"]
      end
  ...
  RegistryProxy -->|"балансировка, internal-ip:5001"| Distribution
  ...
  Distribution -->|"кеширование образов"| RegistryStorage

  Distribution -->|"кеширующий proxy"| ImageRegistry
...
Топология сети идентична режиму `Proxy`: `containerd` обращается на `127.0.0.1:5001` к `registry-proxy-<node>` на каждом узле, который балансирует запросы на `registry-nodeservices-<node>` (static pod-ы на master-узлах, слушающие на `internal-ip:5001`). Отличие в том, что `registry-nodeservices-<node>` работают в Local-режиме и отдают образы из локального хранилища (`/opt/deckhouse/registry`) — обращений во внешний registry нет.
...
flowchart TB
  ...
  subgraph DKP["Deckhouse Kubernetes Platform"]
    ...
    subgraph RegistryModule["Модуль registry"]
      ...
      subgraph RegistryNodeservicesStaticPod["registry-nodeservices-&lt;master-node&gt; [static pod, локальный]<br/>internal-ip:5001"]
        direction TB
        Distribution["distribution"]
        Auth["auth"]
      end
...
flowchart TB
  ...
  Containerd -->|"прямое обращение"| ImageRegistry

  Deckhouse -->|"прямое обращение"| ImageRegistry

  OperatorTrivy -->|"прямое обращение"| ImageRegistry
