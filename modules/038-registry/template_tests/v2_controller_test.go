...
var _ = Describe("Module :: registry :: helm template :: v2 publication endpoint", func() {
	...
	Context("a pass-through cache", func() {
		...
		It("is published anyway, and points at the write instance", func() {
			Expect(f.RenderError).ShouldNot(HaveOccurred())
...
var _ = Describe("Module :: registry :: helm template :: v2 token service", func() {
	...
	It("gives a replica a write account of its own, separate from the node and publication ones", func() {
		...
		Expect(config).To(ContainSubstring("SYNC-HASH"))
		Expect(config).ToNot(ContainSubstring("SYNC-PASSWORD"))
		Expect(config).To(ContainSubstring(`actions: ["pull", "push"]`))

		// Three distinct identities, and the syncer holding neither of the other two: the node
		// account is on every node, and the publication one guards a path reachable from outside.
		access := f.KubernetesResource("Secret", "d8-system", "registry-storage-access")
		Expect(access.Field("data.sync-username").String()).
			To(Equal(base64.StdEncoding.EncodeToString([]byte("registry-sync"))))
		Expect(access.Field("data.username").String()).
			To(Equal(base64.StdEncoding.EncodeToString([]byte("registry-ro"))))
		...
		for _, env := range syncer.Get("env").Array() {
			switch env.Get("name").String() {
			case "REGISTRY_LOCAL_USER":
				user = env.Get("valueFrom.secretKeyRef.key").String()
			case "REGISTRY_LOCAL_PASSWORD":
				password = env.Get("valueFrom.secretKeyRef.key").String()
			}
		}
		Expect(user).To(Equal("sync-username"),
			"the syncer must not be given the account the nodes pull with; it cannot write")
		Expect(password).To(Equal("sync-password"))
	})

	It("lets the read account list the catalogue, which is how a replica counts what it holds", func() {
		secret := f.KubernetesResource("Secret", "d8-system", "registry-storage-auth-config")
		config := secret.Field("stringData.auth_config\\.yaml").String()

		Expect(config).To(ContainSubstring(`type: "registry", name: "catalog"`))

		// Before the account's own rule, because docker_auth stops at the first match and
		// that one grants "pull" — which is not the action a catalogue read asks for.
		catalogue := strings.Index(config, `name: "catalog"`)
		pullOnly := strings.Index(config, `actions: ["pull"]`)
		Expect(catalogue).To(BeNumerically(">", -1))
		Expect(pullOnly).To(BeNumerically(">", -1))
		Expect(catalogue).To(BeNumerically("<", pullOnly),
			"the catalogue rule must be matched before the rule that grants only pull")
	})
})
...
var _ = Describe("Module :: registry :: helm template :: v2 garbage collection", func() {
	...
	It("lets the syncer take a lease and read what the cluster is running", func() {
		...
		// And which modules the cluster keeps. Without this the syncer cannot enumerate the images
		// its modules run, so every pass ends in an error instead of an accounting — and a store that
		// counted the platform's images alone would call itself complete while missing them, which is
		// what authorizes cutting an air-gapped cluster off from its upstream.
		Expect(cluster.Field("rules").String()).To(ContainSubstring("modulereleases"))
		Expect(cluster.Field("rules").String()).ToNot(ContainSubstring("modulereleases/status"))
	})

	// A release object is not the only way a cluster can be running a version: installed from a
	// development or pull-request image, it has none, and then the fill has nothing to fill towards
	...
	It("lets the syncer read the version the cluster is actually running", func() {
		...
		var deployments []gjson.Result
		for _, rule := range role.Field("rules").Array() {
			for _, resource := range rule.Get("resources").Array() {
				if resource.String() == "deployments" {
					deployments = append(deployments, rule)
				}
			}
		}

		Expect(deployments).To(HaveLen(1), "one rule, so what it grants is legible at a glance")
		rule := deployments[0]

		// One object, by name, and nothing beyond reading it: the syncer asks this Deployment
		// exactly which tag its image carries. Read-only matters here for the same reason it does
		// for the releases — this is an input to deletions, not something to change.
		Expect(rule.Get("resourceNames").String()).To(ContainSubstring("deckhouse"))
		Expect(rule.Get("verbs").String()).To(Equal(`["get"]`))
	})
})
...
// The metrics have to be scrapable, which is a second grant and not the one next to it.
//
// `access-to-registry-prometheus-metrics` says Prometheus may READ these metrics. This one says the
// kube-rbac-proxy beside each component may ASK the API server whether the token it was handed is
// valid — a TokenReview it must be allowed to create. Having only the first is silent: every scrape
// was rejected with "cannot create resource tokenreviews", `up` was 0 for both components, no
// `d8_registry_*` series reached Prometheus, and all nine alerts this module ships were inert,
// because every one of their expressions opens with `max(d8_registry_managed) > 0`.
var _ = Describe("Module :: registry :: helm template :: v2 metrics are scrapable", func() {
	f := SetupHelmConfig(``)

	BeforeEach(func() {
		f.ValuesSetFromYaml("global", globalValues)
		f.ValuesSet("global.modulesImages", GetModulesImages())
		// The PodMonitor is rendered only where the CRD it is an instance of exists, so the
		// module that ships that CRD has to be in the fixture for this block to mean anything.
		f.ValuesSetFromYaml("global.enabledModules",
			`["vertical-pod-autoscaler", "registry", "prometheus", "operator-prometheus-crd"]`)
		f.ValuesSetFromYaml("registry", v2Enabled)
		f.HelmRender()
		Expect(f.RenderError).ShouldNot(HaveOccurred())
	})

	It("lets each component's kube-rbac-proxy authenticate the scraper", func() {
		for _, item := range []struct{ binding, account string }{
			{"d8:registry:controller:rbac-proxy", "registry-controller"},
			{"d8:registry:storage:rbac-proxy", "registry-storage"},
		} {
			binding := f.KubernetesGlobalResource("ClusterRoleBinding", item.binding)
			Expect(binding.Exists()).To(BeTrue(),
				"without this binding the proxy cannot create a TokenReview and every scrape is refused")
			Expect(binding.Field("roleRef.name").String()).To(Equal("d8:rbac-proxy"))
			Expect(binding.Field("subjects.0.name").String()).To(Equal(item.account))
			Expect(binding.Field("subjects.0.namespace").String()).To(Equal("d8-system"))
		}
	})

	// The other half, kept beside it so that removing either one fails here rather than in a
	// cluster three weeks later.
	It("lets Prometheus read them", func() {
		role := f.KubernetesResource("Role", "d8-system", "access-to-registry-prometheus-metrics")
		Expect(role.Exists()).To(BeTrue())

		monitor := f.KubernetesResource("PodMonitor", "d8-monitoring", "registry-controller")
		Expect(monitor.Exists()).To(BeTrue())
		Expect(monitor.Field("metadata.labels.prometheus").String()).To(Equal("main"),
