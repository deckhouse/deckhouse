...
// QuiescedStore makes a replica's own registry refuse writes for the duration of a collection.
//
// Implemented by re-rendering the configuration and signalling the process to re-read it,
// which is the same mechanism a credential change uses. The pod object is never touched, so
// nothing about this depends on the Deckhouse operator being up.
type QuiescedStore struct {
	Log *slog.Logger

	// Applier owns the rendered configuration.
	Applier *distribution.Applier

	// Spec is the desired state to render, read at quiesce time so that the restore puts
	// back what the cluster currently wants rather than what it wanted an hour ago.
	Spec func() *registryv1alpha1.RegistryStorageSpec
}

// Quiesce refuses writes and returns what accepts them again.
func (s *QuiescedStore) Quiesce(context.Context) (func(), error) {
	spec := s.Spec()
	if spec == nil {
		return nil, errors.New("no desired state to render a read-only configuration from")
	}

	if _, err := s.Applier.ApplyReadOnly(spec); err != nil {
		return nil, err
	}

	return func() {
		// The spec is read again rather than captured: a collection takes time, and what the
		// cluster wants may have changed while it ran. Restoring the old one would undo that
		// change until the next pass of the ordinary loop.
		current := s.Spec()
		if current == nil {
			current = spec
		}
		if _, err := s.Applier.Apply(current); err != nil {
			// Loud, because this is the failure that leaves a replica unable to accept
			// writes. The ordinary loop applies the same configuration every pass, so it
			// recovers on its own within a minute — which is why this is not fatal.
			s.Log.Error("cannot let the registry accept writes again; the next pass of the loop will",
				"error", err.Error())
		}
	}, nil
}
