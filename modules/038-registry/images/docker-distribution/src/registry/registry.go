...
type Registry struct {
	...
	// The write endpoint, when the configuration asks for one: the same storage served without the
	// pull-through cache, on its own address. Nil otherwise.
	writeApp    *handlers.App
...
func NewRegistry(ctx context.Context, config *configuration.Configuration) (*Registry, error) {
	...
	writeApp, writeServer, err := newWriteEndpoint(ctx, config)
	if err != nil {
		return nil, err
	}
...
// ListenAndServe runs the registry's HTTP server, and the write endpoint's beside it when one is
// configured.
func (registry *Registry) ListenAndServe() error {
	config := registry.config

	ln, err := registry.newListener(config.HTTP.Addr)
	if err != nil {
		return err
	}

	// Room for both halves: whichever stops first is the answer, and the other's error is not left
	// blocking a goroutine.
	serveErr := make(chan error, 2)

	if registry.writeServer != nil {
		writeLn, err := registry.newListener(config.WriteEndpoint.Addr)
		if err != nil {
			return err
		}

		go func() {
			serveErr <- registry.writeServer.Serve(writeLn)
		}()
	}
	...
	case <-quit:
		...
		// Both halves drain within the same period. A push in flight is a long request and the
		// endpoint that accepts it is the one an operator is watching, so ending it abruptly while
		// the read half drains politely would be the wrong way round.
		var writeErr error
		if registry.writeServer != nil {
			writeErr = registry.writeServer.Shutdown(c)
		}

		if err := registry.server.Shutdown(c); err != nil {
			return err
		}
		return writeErr
	}
...
// newListener opens one of this registry's addresses, wrapped in TLS when the configuration carries a
// certificate. The material is the same for every address: the write endpoint is the same registry,
// reached from another side.
func (registry *Registry) newListener(addr string) (net.Listener, error) {
	config := registry.config
