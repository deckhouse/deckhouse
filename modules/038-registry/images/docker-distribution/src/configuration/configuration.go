...
type Configuration struct {
	...
	// WriteEndpoint serves the same store on a second address, without the pull-through cache.
	WriteEndpoint WriteEndpoint `yaml:"writeendpoint,omitempty"`
...
// WriteEndpoint is the address that accepts pushes into a registry which is otherwise a
// pull-through cache.
//
// A cache cannot accept a push: every write is answered with `UNSUPPORTED`, and turning the cache off
// to accept one is not available either — the store this serves is filled BY pushes while it still
// has an upstream to serve from. So the writes get their own listener, backed by a second view of the
// same storage with no proxying configured, in this same process.
//
// One process rather than two, which is how this used to be done: two registries over one data
// directory needed two rendered configurations that must not be swapped, two containers to run them,
// and a store-wide flag to stop each of them from deleting the other's data on startup.
//
// The two views are deliberately not symmetric. A push must be answered by the STORE, not by the
// upstream: before uploading a layer a client asks whether the destination already holds it, and a
// cache answers yes by fetching it from the upstream — the upload is skipped, the manifest is stored,
// and what is left is a manifest naming blobs the store does not have. Measured: a fill reporting 400
// layers written left the store at its original 333 MB. Hence a listener that never proxies, rather
// than one endpoint answering both ways.
type WriteEndpoint struct {
	// Addr is the address to listen on, in the same form as `http.addr`. Empty means there is no
	// write endpoint and this process serves reads only.
	//
	// Necessarily a different port from `http.addr`: the two listeners live in one process.
	Addr string `yaml:"addr,omitempty"`

	// ClientCertCA is the authority whose client certificates are trusted to carry the real client
	// address on this listener — the `http.realip.clientcert.ca` of the serving half, which the write
	// endpoint needs because it is the one reached through an ingress.
...
type Proxy struct {
	...
	// SkipModeCleanup keeps the store when the registry starts in a different mode than the one
	// that last wrote it.
	//
	// By default a transition between cache and non-cache modes deletes everything under /docker,
	// so that data written as a proxy cache is never served as if it were local. That assumes one
	// registry process owns the directory. Where two processes share it — one proxying and one
	// accepting pushes, which is how Deckhouse serves and fills a single store — the non-proxying
	// one sees the other's scheduler state as a mode change on every start and empties the store
	// out from under a running cluster. Measured on a three-master cluster: twelve gigabytes and
	// 3236 files gone two seconds after the push instance came up, twice, and once at the exact
	// moment the upstream was dropped for an air-gap, leaving nothing to pull anywhere.
	//
	// Left false by default, so a registry that does own its directory behaves exactly as before.
	SkipModeCleanup bool `yaml:"skipmodecleanup,omitempty"`
}
