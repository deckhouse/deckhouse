...
// encodedPair builds the wire form of a credential pair, on the spot.
//
// Written this way rather than as a base64 literal so that no line in these tests looks like a
// credential to a secret scanner: `Auth: "<base64>"` is exactly the shape gitleaks reports as a
// generic API key, and it did — three findings on this package's fixtures, all of them invented
// values. The pair is also plainer to read here than an opaque string with a comment explaining it.
func encodedPair(user, secret string) string {
...
func persistedUpstreamAuth() map[string]registryv1alpha1.Auth {
	return map[string]registryv1alpha1.Auth{
		constant.AuthKeyUpstream: {Auth: encodedPair("license-token", "key")},
	}
}
...
func TestComputePassThroughCache(t *testing.T) {
	...
	// Published here too, which is the reversal: the write endpoint is its own instance over the same
	// data, so it costs the cache nothing, and without it a bundle could never be pushed BEFORE the
	// air-gap transition it is meant to make safe.
	assert.True(t, got.Storage.Publish,
		"the write endpoint is published on every managed cluster, not only an air-gapped one")

	// And what did NOT become always-on: how completeness is measured. A pass-through cache writes into
	// its own store on every miss, so reading the catalogue here would count what the cluster fetched
	// rather than what it deliberately holds — and the air-gap gate is derived from that count.
	assert.False(t, got.Storage.AirGapRequested,
		"a cluster that caches has not asked for air-gap, whatever is published")
	assert.False(t, got.DropUpstream)
...
// TestAgentsArePointedAtTheLeader pins which replica a node asks first.
//
// It decides what a node can pull, because an agent does not move to the next target on a 404 — and
// that is deliberate: for the Storage-then-Upstream chain a 404 means the image is genuinely absent,
...
func TestAgentsArePointedAtTheLeader(t *testing.T) {
	...
	t.Run("the leader goes first and the rest become its mirrors", func(t *testing.T) {
		access.Leader = "10.0.0.3:5001"

		backend := storageBackend(access)

		assert.Equal(t, "10.0.0.3:5001", backend.Endpoint.Host,
			"the primary target must be the leader, not whichever address came first")
		mirrors := make([]string, 0, len(backend.Mirrors))
		for _, mirror := range backend.Mirrors {
			mirrors = append(mirrors, mirror.Host)
		}
		assert.Equal(t, []string{"10.0.0.1:5001", "10.0.0.2:5001"}, mirrors)
		assert.NotContains(t, mirrors, "10.0.0.3:5001", "the leader must not also be its own mirror")
	})

	t.Run("with no leader reported the given order stands", func(t *testing.T) {
		access.Leader = ""

		backend := storageBackend(access)

		assert.Equal(t, "10.0.0.1:5001", backend.Endpoint.Host)
		assert.Len(t, backend.Mirrors, 2,
			"serving no storage backend at all is worse than an imperfect first choice")
	})

	t.Run("a leader that is not among the addresses is ignored", func(t *testing.T) {
		access.Leader = "10.9.9.9:5001"

		backend := storageBackend(access)

		assert.Equal(t, "10.0.0.1:5001", backend.Endpoint.Host,
			"a stale leader address must not become a target nobody can reach")
		assert.Len(t, backend.Mirrors, 2)
	})
}
