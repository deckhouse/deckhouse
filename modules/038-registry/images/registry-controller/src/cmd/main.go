...
// leaderElectionLease is the lease the controller replicas contend for.
const leaderElectionLease = "registry-controller-leader"

// managerOptions is where leader election is switched on, and it is a function of its own so that a
...
func managerOptions(metricsAddr, probeAddr string) ctrl.Options {
	return ctrl.Options{
		...
		LeaderElection:          true,
		LeaderElectionID:        leaderElectionLease,
		LeaderElectionNamespace: registryNamespace,
		// Handed over on shutdown rather than waited out: a rolling update of the controller
		// otherwise leaves the cache un-reconciled for the remainder of the lease.
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), managerOptions(metricsAddr, probeAddr))
	if err != nil {
