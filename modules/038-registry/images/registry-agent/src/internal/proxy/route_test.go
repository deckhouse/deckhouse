...
func TestResolveAuthenticatesAKnownRegistryAskedForByItsOwnAddress(t *testing.T) {
	...
	// than through the intercepted name. Credentials are ours to supply; the repository is not
	// under the primary prefix, so there is nothing to cache and nothing to rewrite.
	decision, err := Resolve(
		"vendor.example.com", "/v2/virt/tool/manifests/sha256:abc", spec, self)
	require.NoError(t, err)
	...
	target := decision.Targets[0]
	assert.Equal(t, "vendor.example.com", target.Host)
	assert.Equal(t, "/v2/virt/tool/manifests/sha256:abc", target.Path,
	...
	assert.Equal(t, "vendor", target.Auth.Username)
}

// TestResolveSendsThePrimarySetThroughTheCacheEvenByItsUpstreamAddress is the ADR requirement
...
//
// The control plane's own guarantee survives, and that is why the order matters rather than the
// destination: etcd and kube-apiserver name the upstream so as not to depend on the in-cluster
// registry, and the upstream is still here, one place behind the cache. A master joining before
// the cache can serve it spends one failed attempt and then pulls exactly as before.
func TestResolveSendsThePrimarySetThroughTheCacheEvenByItsUpstreamAddress(t *testing.T) {
	...
	require.Equal(t, KindPrimary, decision.Kind)
	require.Len(t, decision.Targets, 2, "the cache, and the upstream behind it")

	cache := decision.Targets[0]
	assert.Equal(t, string(registryv1alpha1.BackendStorage), cache.Name)
	assert.Equal(t, constant.Host, cache.Host)
	assert.Equal(t, "/v2/system/deckhouse/etcd/manifests/sha256:abc", cache.Path,
		"the repository is named relative to the image set, so the cache is asked for it under its own prefix")
	require.NotNil(t, cache.Auth)

	upstream := decision.Targets[1]
	assert.Equal(t, "registry.deckhouse.io", upstream.Host)
	assert.Equal(t, "/v2/deckhouse/ee/etcd/manifests/sha256:abc", upstream.Path,
		"behind the cache the request is what it always was")
	require.NotNil(t, upstream.Auth, "without credentials the registry refuses the pull")
	assert.Equal(t, "license-token", upstream.Auth.Username)
}

// TestResolveClaimsForTheCacheOnlyWhatBelongsToTheImageSet is the boundary of the rule above.
//
// Matching on the host alone would drag other content into the cache: an upstream serves the
// platform under one prefix and may serve anything else beside it. The cache is sized for
// Deckhouse and, in air-gap, is the only source it has — so what is claimed for it has to be
// exactly the image set, and everything else keeps its own address and its own path.
func TestResolveClaimsForTheCacheOnlyWhatBelongsToTheImageSet(t *testing.T) {
	t.Run("a repository beside the image set on the same registry", func(t *testing.T) {
		decision, err := Resolve(
			"registry.deckhouse.io", "/v2/somebody/else/manifests/v1", layout(), self)
		require.NoError(t, err)
		assert.Equal(t, KindKnown, decision.Kind)
		require.Len(t, decision.Targets, 1)
		assert.Equal(t, "/v2/somebody/else/manifests/v1", decision.Targets[0].Path)
	})

	t.Run("a name that merely starts like the prefix", func(t *testing.T) {
		decision, err := Resolve(
			"registry.deckhouse.io", "/v2/deckhouse/eee/manifests/v1", layout(), self)
		require.NoError(t, err)
		assert.Equal(t, KindKnown, decision.Kind,
			"'deckhouse/eee' is not under 'deckhouse/ee'")
	})

	t.Run("a backend serving at the root claims nothing", func(t *testing.T) {
		spec := layout()
		spec.Backends[1].Path = ""

		decision, err := Resolve(
			"registry.deckhouse.io", "/v2/deckhouse/ee/etcd/manifests/v1", spec, self)
		require.NoError(t, err)
		assert.Equal(t, KindKnown, decision.Kind,
			"with no prefix there is nothing to tell the image set from the rest of the registry")
	})

	t.Run("the primary set is claimed through a mirror's address too", func(t *testing.T) {
		spec := layout()
		spec.Backends[1].Mirrors = []registryv1alpha1.Endpoint{{
			Scheme: registryv1alpha1.SchemeHTTPS,
		...
		decision, err := Resolve(
			"mirror.example.com", "/v2/mirrored/ee/etcd/manifests/v1", spec, self)
		require.NoError(t, err)
		assert.Equal(t, KindPrimary, decision.Kind,
			"a mirror carries the same image set, so a pull named after it belongs in the cache as well")
	})
}
