...
const deckhouseDeploymentUseNelmTrue = `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: deckhouse
...
const deckhouseDeploymentUseNelmFalse = `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: deckhouse
...
const deckhouseDeploymentNoEnv = `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: deckhouse
...
var _ = Describe("KubeDns hooks :: adopt_kube_dns_resources", func() {
	...
	Context("when USE_NELM is true", func() {
		BeforeEach(func() {
			state := kubeDNSService + "---" + coreDNSDeployment + "---" + deckhouseDeploymentUseNelmTrue
	...
	Context("when USE_NELM is false", func() {
		BeforeEach(func() {
			state := kubeDNSService + "---" + coreDNSDeployment + "---" + deckhouseDeploymentUseNelmFalse
			f.BindingContexts.Set(f.KubeStateSet(state), f.GenerateBeforeHelmContext())
			f.RunHook()
		})
		...
		It("Service kube-dns is NOT modified", func() {
			svc := f.KubernetesResource("Service", "kube-system", "kube-dns")

			Expect(svc.Field("metadata.labels.app\\.kubernetes\\.io/managed-by").Exists()).To(BeFalse())
			Expect(svc.Field("metadata.annotations.meta\\.helm\\.sh/release-name").Exists()).To(BeFalse())
		})
	...
	Context("when USE_NELM is not set", func() {
		BeforeEach(func() {
			state := kubeDNSService + "---" + coreDNSDeployment + "---" + deckhouseDeploymentNoEnv
			f.BindingContexts.Set(f.KubeStateSet(state), f.GenerateBeforeHelmContext())
			f.RunHook()
		})
