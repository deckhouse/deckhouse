...
`))
	...
	Context("AWS", func() {
		...
		It("Everything must render properly", func() {
			...
			clusterAdminAPIGroups := userAuthzClusterRoleClusterAdmin.Field("rules.#.apiGroups").String()
			Expect(clusterAdminAPIGroups).ShouldNot(ContainSubstring(`"machine.sapcloud.io"`))
			Expect(clusterAdminAPIGroups).ShouldNot(ContainSubstring(`"cluster.x-k8s.io"`))
