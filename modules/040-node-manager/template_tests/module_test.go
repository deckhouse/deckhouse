...
`))
			...
			Context("cluster auto-scaler split mode with MCM only", func() {
				...
				It("renders MCM autoscaler target alerts against the MCM job", func() {
					...
					rule := f.KubernetesResource("PrometheusRule", "d8-cloud-instance-manager", "node-manager-cluster-autoscaler")
					Expect(rule.Exists()).Should(BeTrue())
					Expect(rule.Field("spec.groups.0.rules.2.expr").String()).To(Equal(`max by (job) (up{d8_source="dkp", job=~"cluster-autoscaler-mcm", namespace="d8-cloud-instance-manager"} == 0)`))
					Expect(rule.Field("spec.groups.0.rules.3.expr").String()).To(Equal(`absent(up{d8_source="dkp", job="cluster-autoscaler-mcm", namespace="d8-cloud-instance-manager"} == 1)`))
				})
			})
			...
			Context("cluster auto-scaler split mode with MCM and CAPI", func() {
				...
				It("renders target alerts for both autoscaler jobs", func() {
					...
					rule := f.KubernetesResource("PrometheusRule", "d8-cloud-instance-manager", "node-manager-cluster-autoscaler")
					Expect(rule.Exists()).Should(BeTrue())
					Expect(rule.Field("spec.groups.0.rules.2.expr").String()).To(Equal(`max by (job) (up{d8_source="dkp", job=~"cluster-autoscaler|cluster-autoscaler-mcm", namespace="d8-cloud-instance-manager"} == 0)`))
					Expect(rule.Field("spec.groups.0.rules.3.expr").String()).To(Equal(`absent(up{d8_source="dkp", job="cluster-autoscaler", namespace="d8-cloud-instance-manager"} == 1) or absent(up{d8_source="dkp", job="cluster-autoscaler-mcm", namespace="d8-cloud-instance-manager"} == 1)`))
				})
			})
		})
