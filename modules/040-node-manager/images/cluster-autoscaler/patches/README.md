...
and on 1.33+ `go mod tidy` drops the requirement entirely, so no patch in this
directory pins `runc` any more.

Note that `go mod why -m` answers "is this module needed by the module graph",
which is not the same as "is it linked into the binary": it can report a chain
through the kubemark cloud provider, but that provider is not part of the
