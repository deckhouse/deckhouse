...
func (*StaticInstanceCustomValidator) ValidateUpdate(_ context.Context, new, old runtime.Object) (admission.Warnings, error) {
	...
	if oldStaticInstance.Spec.NodeName != staticInstance.Spec.NodeName &&
		!nodeNameStillOpen(oldStaticInstance.Status.CurrentStatus) {
...
// nodeNameStillOpen reports whether spec.nodeName can still be changed. A missing
// status is an instance the controller has not reached yet, so it is as untouched
...
func nodeNameStillOpen(status *StaticInstanceStatusCurrentStatus) bool {
	return status == nil || status.Phase == "" || status.Phase == StaticInstanceStatusCurrentStatusPhasePending
}
