...
func (r *StaticInstanceCustomDefaulter) Default(_ context.Context, obj runtime.Object) error {
	...
	if !ok {
		return fmt.Errorf("expected a StaticInstance object but got %T", obj)
	}
...
func (r *StaticInstanceCustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	staticInstance, ok := obj.(*StaticInstance)
	if !ok {
		return nil, fmt.Errorf("expected a StaticInstance object but got %T", obj)
	}
	staticinstancelog.Info("validate create", "name", staticInstance.Name)
