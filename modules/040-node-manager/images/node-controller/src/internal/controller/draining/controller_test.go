...
// keep the request in place, record the durable failure, and are retried.
func TestReconcile_EvictionEndsBadly(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:            "a deadline keeps the request, so the requeue retries it",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, drained := updated.Annotations[nodecommon.DrainedAnnotation]; drained {
				t.Fatal("a node whose eviction failed must not be marked as drained")
			}
			...
			// Both outcomes have to reach NodeStuckInDraining: the alert is
			// driven by this gauge alone, node_group_node_status only supplies
			// the node_group label, whatever status the node happens to be in.
...
func TestReconcile_FailedDrainRecoversOnNextAttempt(t *testing.T) {
	...
	failed := h.node(t)
	if _, drained := failed.Annotations[nodecommon.DrainedAnnotation]; drained {
		t.Fatal("the failed attempt must not mark the node as drained")
	}
...
// The gauge is never lowered by a node that lacks the marker: the pass that
// starts a retry may read one from a cache older than the failure it retries,
// and lowering it there would silence NodeStuckInDraining for a whole attempt.
