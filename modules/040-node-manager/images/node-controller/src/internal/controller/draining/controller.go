...
func (r *Reconciler) reconcileNode(ctx context.Context, node *corev1.Node) (_ ctrl.Result, resErr error) {
	...
	defer func() {
		// Last, over the annotations as they are being written, so a failure this
		// pass recorded and one left on the node by an earlier process both raise
		// the gauge.
	...
	if state.requestedBy == "" {
		...
		// Nothing is being attempted any more, so the last attempt's failure is
	...
	// write the same drained=user over it, so nothing would tell the two apart.
...
// finishDrain records a successful drain or leaves a failed request in place
// so the next reconcile can retry it.
...
// truncateForAnnotation bounds what an eviction error may write to the Node. A
// drain that names every unevictable pod runs to kilobytes, all annotations of
// an object share a 256 KB budget, and only the head of such an error is read
// anyway. The full text stays in the log and in the DrainFailed event.
func truncateForAnnotation(msg string) string {
	const limit = 1024
	if len(msg) <= limit {
		return msg
	}
	// The cut can land mid-rune, and an annotation value has to survive JSON.
