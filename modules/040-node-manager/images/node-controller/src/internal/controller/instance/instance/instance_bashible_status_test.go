...
func TestBashibleStatusFromConditions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "waiting approval timeout has highest priority",
			conditions: []deckhousev1alpha2.InstanceCondition{
		...
		{
			name: "waiting approval timeout has highest priority over bashible ready true",
			conditions: []deckhousev1alpha2.InstanceCondition{
		...
		{
			name: "waiting approval with another reason does not affect status",
			conditions: []deckhousev1alpha2.InstanceCondition{
				{Type: deckhousev1alpha2.InstanceConditionTypeBashibleReady, Status: metav1.ConditionTrue},
				{
					Type:   deckhousev1alpha2.InstanceConditionTypeWaitingApproval,
					Status: metav1.ConditionTrue,
					Reason: "UpdateApprovalNotRequired",
				},
			},
