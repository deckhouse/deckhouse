...
type machineState struct {
	...
	sourceTransitionTime *metav1.Time
}
...
func buildMachineReadyCondition(state machineState) deckhousev1alpha2.InstanceCondition {
	...
	if state.sourceCondition != nil {
		transitionTime := state.sourceCondition.LastTransitionTime
		cond.LastTransitionTime = &transitionTime
		cond.ObservedGeneration = state.sourceCondition.ObservedGeneration
	} else if state.sourceTransitionTime != nil {
		transitionTime := *state.sourceTransitionTime
		cond.LastTransitionTime = &transitionTime
	}
	return cond
}
