...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.processUpdatedNodes(ctx, ng, nodeInfos, ngChecksum); err != nil {
		return ctrl.Result{}, err
	}

	// Step 2: Approve disruptions
	if err := r.approveDisruptions(ctx, ng, nodeInfos); err != nil {
		return ctrl.Result{}, err
	}

	// Step 3: Approve updates
	if err := r.approveUpdates(ctx, ng, nodeInfos); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *Reconciler) processUpdatedNodes(ctx context.Context, ng *v1.NodeGroup, nodes []nodeInfo, ngChecksum string) error {
	logger := log.FromContext(ctx)
...
// approveDisruptions approves disruptions for nodes in Automatic mode.
func (r *Reconciler) approveDisruptions(ctx context.Context, ng *v1.NodeGroup, nodes []nodeInfo) error {
	logger := log.FromContext(ctx)
	...
	for _, node := range nodes {
		...
		case "Automatic":
			if !isInAllowedWindow(ng.Spec.Disruptions.Automatic.Windows, now) {
				continue
			}

		case "RollingUpdate":
			if !isInAllowedWindow(ng.Spec.Disruptions.RollingUpdate.Windows, now) {
				continue
			}
		}
...
func (r *Reconciler) approveUpdates(ctx context.Context, ng *v1.NodeGroup, nodes []nodeInfo) error {
	logger := log.FromContext(ctx)
	...
	// For CloudEphemeral: only approve if desired <= ready, or for other types: if all ready
	allReady := true
	for _, node := range nodes {
		if !node.IsReady {
			allReady = false
	...
	if ng.Spec.NodeType == v1.NodeTypeCloudEphemeral {
		canApprove = ng.Status.Desired <= ng.Status.Ready
	} else {
		canApprove = allReady
	}

	if canApprove {
		// Approve waiting nodes (prioritize ready nodes first)
		for _, node := range nodes {
			if node.IsWaitingForApproval {
...
func (r *Reconciler) needDrainNode(node *nodeInfo, ng *v1.NodeGroup) bool {
	// Can't drain single control-plane node
	...
	// Check DrainBeforeApproval setting
