...
// TestNodeTemplateControllerEnvtest runs the envtest-backed integration suite. It complements
// the fast fake-client/pure-logic unit tests: here the real node-template controller runs
// inside a manager against a real kube-apiserver, so the merge-patch it issues to Nodes (system
// labels, template labels/annotations/taints, uninitialized-taint removal) is exercised end to
// end and asserted as the resulting Node state. The suite is skipped when envtest assets are
// not available so the unit tests stay runnable without `make envtest`.
func TestNodeTemplateControllerEnvtest(t *testing.T) {
	...
	RunSpecs(t, "NodeTemplate Controller Envtest Suite")
}
