...
func renderSpec(ng *v1.NodeGroup, node *corev1.Node, in clusterInputs) internalv1alpha1.NodeSpec {
	...
	return internalv1alpha1.NodeSpec{
		// Spelled out rather than copied from the group: only an Immutable group
		// reaches this render, and a group that names nothing would leave "" for
		// the API default to differ from on every pass, spending a rollout slot.
		SystemType:           internalv1alpha1.SystemTypeImmutable,
		NodeName:             node.Name,
		OSImage:              in.OSImage,
