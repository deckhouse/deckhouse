...
func TestScopedTemplateInputsValidateOnlyTheirOwnContract(t *testing.T) {
	...
	require.ErrorContains(t, err, "MachineTemplate kind or apiVersion")
	...
	require.ErrorContains(t, err, "cluster name, kind or apiVersion")

	withoutMCM := provider
	withoutMCM.Registration.MachineClassKind = ""
	_, err = source.LoadMCMInputs(t.Context(), withoutMCM)
	require.ErrorContains(t, err, "no MachineClass kind")
}
