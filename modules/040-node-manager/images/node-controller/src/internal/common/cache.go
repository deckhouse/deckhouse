		...
		ByObject: map[client.Object]cache.ByObject{
			...
			&corev1.ConfigMap{}: {
				Namespaces: map[string]cache.Config{
					...
					// the latter on every pass as the primary source of the target Kubernetes
					// version. With the selector still in place that read silently returned
					// NotFound (and the ConfigMap watch never fired), so the target quietly fell
					// back to the deprecated ClusterConfiguration field.
					"kube-system": {},
					"d8-system": {
