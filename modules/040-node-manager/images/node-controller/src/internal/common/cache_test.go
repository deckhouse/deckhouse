...
// caller then silently takes its fallback path and everything looks healthy.
//
// This bit the derived-status service: kube-system ConfigMaps were pinned to d8-cluster-uuid by a
// name FieldSelector, so d8-cluster-kubernetes — the primary source of the cluster's target
// Kubernetes version — was never found, the ConfigMap watch never fired, and the target quietly
// fell back to the deprecated ClusterConfiguration field on every pass.
//
...
// TestModuleConfigCacheScopeStaysNarrow documents why derived_status reads the
// control-plane-manager ModuleConfig through the APIReader instead of the cached client: this
// informer is deliberately pinned to "global" so the controller does not watch every ModuleConfig
// in the cluster. If this scope is ever widened, that live GET can become a cached read.
func TestModuleConfigCacheScopeStaysNarrow(t *testing.T) {
	opts, _ := CacheOptions()

	for obj, byObject := range opts.ByObject {
		if obj.GetObjectKind().GroupVersionKind().Kind != "ModuleConfig" {
			continue
		}
		require.NotNil(t, byObject.Field, "ModuleConfig informer is expected to stay name-scoped")
		assert.Equal(t, "metadata.name=global", byObject.Field.String())
		return
	}
	t.Fatal("no ByObject entry for ModuleConfig")
}
