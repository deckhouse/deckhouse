...
С помощью сервиса `d8-shutdown-inhibitor` в `node-manager` реализован механизм задержки выключения или перезагрузки узла (далее просто «выключение узла»), если на нем запущены stateful-приложения или виртуальные машины (ВМ). Задержка срабатывает, если на узле запущены поды с лейблом `pod.deckhouse.io/inhibit-node-shutdown`. DP откладывает выключение узла, чтобы пользователь смог перенести критичные поды на другой узел или завершить их работу штатным образом. Это полезно, например, для stateful-нагрузок и других случаев, когда необходимо избежать потери данных.
...
#### Особенности механизма в DP

Задержка перезагрузки или выключения узла, на котором работают критичные поды, в DP имеет следующие особенности:
...
* Механизм работает только с модифицированной версией kubelet, входящей в состав DP. На стандартном Kubernetes аналогичное поведение отсутствует.
