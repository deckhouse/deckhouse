...
С помощью сервиса `d8-shutdown-inhibitor` в `node-manager` реализована задержка выключения или перезагрузки узла (shutdown), если на нем запущены stateful-приложения или виртуальные машины (ВМ). Задержка срабатывает, если на узле запущены поды с лейблом `pod.deckhouse.io/inhibit-node-shutdown`. DKP откладывает выключение или перезагрузку узла, чтобы пользователь смог отселить критичные поды или завершить их работу штатным образом. Это полезно, например, для stateful-нагрузок и других случаев, когда необходимо избежать потери данных.

### Принцип работы и особенности

Сервис `d8-shutdown-inhibitor` (ингибитор) блокирует shutdown узла до тех пор, пока на нём остаются поды в статусе `Running`, помеченные лейблом `pod.deckhouse.io/inhibit-node-shutdown`. Для блокировки используется condition `GracefulShutdownPostpone`. Также на время блокировки выполняется cordon узла, чтобы scheduler не планировал на него новые поды. kubelet читает condition `GracefulShutdownPostpone` узла и не начинает выселение помеченных лейблом подов, пока у этого condition `status: True` и `reason: PodsWithLabelAreRunningOnNode`. При наличии на узле подов с лейблом `pod.deckhouse.io/inhibit-node-shutdown` в консоль узла (wall) выводится соответствующее сообщение.

#### Особенности механизма в DKP

Задержка перезагрузки или выключения узла, пока на нём работают критичные поды, в DKP имеет следующие особенности:

* Лейбл необходимо явно задавать на каждый под, который должен блокировать shutdown. Если поды создаются через Deployment лейбл указывается в шаблоне пода.
* Механизм работает только с пропатченным kubelet DKP. На ванильном Kubernetes аналогичное поведение отсутствует.
* Максимальное время блокировки (`InhibitDelayMaxSec`) — `3` суток. По истечении этого срока shutdown может продолжиться независимо от наличия на узле подов с лейблом `pod.deckhouse.io/inhibit-node-shutdown`.
* Механизм не отменяет планирование disruptive-обновлений в `node-manager` — он задерживает только фактическую перезагрузку узла в момент shutdown.
