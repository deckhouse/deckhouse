...
1. Задержка перезагрузки или выключения узла, пока на нём работают критичные поды.
1. Установка/обновление и настройка ПО узла (containerd, kubelet и др.), подключение узла в кластер:
    * Установка операционной системы (смотри [список поддерживаемых ОС](/products/kubernetes-platform/documentation/v1/reference/supported_versions.html#linux)) вне зависимости от типа используемой инфраструктуры (в любом облаке или на любом железе).
    ...
    * Настройка kubernetes-api-proxy (и системы автоматического обновления перечня upstream’ов) для балансировки запросов от узла (kubelet) по API-серверам.
...
## Задержка перезагрузки или выключения узла, на котором работают критичные поды

С помощью сервиса `d8-shutdown-inhibitor` в `node-manager` реализован механизм задержки выключения или перезагрузки узла (далее просто «выключение узла»), если на нем запущены stateful-приложения или виртуальные машины (ВМ). Задержка срабатывает, если на узле запущены поды с лейблом `pod.deckhouse.io/inhibit-node-shutdown`. DKP откладывает выключение узла, чтобы пользователь смог перенести критичные поды на другой узел или завершить их работу штатным образом. Это полезно, например, для stateful-нагрузок и других случаев, когда необходимо избежать потери данных.

### Принцип работы и особенности

Сервис `d8-shutdown-inhibitor` блокирует выключение узла до тех пор, пока на нём остаются поды в статусе `Running`, помеченные лейблом `pod.deckhouse.io/inhibit-node-shutdown`. Для блокировки используется состояние `GracefulShutdownPostpone`. Также на время блокировки выполняется cordon узла, чтобы scheduler не планировал на него новые поды. Kubelet отслеживает состояние `GracefulShutdownPostpone` узла и не начинает вытеснение помеченных лейблом подов, пока для этого состояния сохраняются `status: True` и `reason: PodsWithLabelAreRunningOnNode`. При наличии на узле подов с лейблом `pod.deckhouse.io/inhibit-node-shutdown` в консоль узла (wall) выводится соответствующее сообщение.

#### Особенности механизма в DKP

Задержка перезагрузки или выключения узла, на котором работают критичные поды, в DKP имеет следующие особенности:

* Лейбл необходимо явно задавать на каждый под, который должен блокировать выключение. Если поды создаются через Deployment, лейбл указывается в шаблоне пода.
* Механизм работает только с модифицированной версией kubelet, входящей в состав DKP. На стандартном Kubernetes аналогичное поведение отсутствует.
* Максимальное время блокировки (`InhibitDelayMaxSec`) составляет `3` суток. По истечении этого срока выключение может продолжиться независимо от наличия на узле подов с лейблом `pod.deckhouse.io/inhibit-node-shutdown`.
* Механизм не отменяет планирование [disruptive-обновлений](/products/kubernetes-platform/documentation/v1/admin/configuration/platform-scaling/node/node-management.html#обновления-требующие-прерывания-работы-узла) в `node-manager`. Он задерживает только фактическую перезагрузку узла в момент выключения.
...
Информация о включении механизма задержки представлена в разделе [«Как отложить перезагрузку узла, пока на нём работают критичные поды»](faq.html#как-включить-задержку-выключения-или-перезагрузки-узла-пока-на-нём-работают-критичные-поды).
...
- **Вручную с последующей передачей узла под автоматическое управление** [Cluster API Provider Static](#cluster-api-provider-static).
  ...
  Эта аннотация императивная и одноразовая: CAPS удаляет её сразу после передачи узла под управление, и установить её можно только пока [StaticInstance](cr.html#staticinstance) находится в фазе `Pending`. Поэтому она не подходит для манифестов, управляемых декларативно (GitOps-инструменты, операторы, шаблоны кластеров): такой инструмент будет повторно добавлять аннотацию, только что удалённую CAPS, а обновление будет отклонено, как только инстанс выйдет из фазы `Pending`.

  Для декларативно управляемых манифестов используйте аннотацию `static.node.deckhouse.io/adopt-if-node-exists: ""`. Она задаёт условие, а не действие: узел передаётся под управление, если в кластере уже есть `Node` с тем же `spec.address`, и настраивается обычным образом в противном случае. CAPS не удаляет эту аннотацию и не ограничивает фазу, в которой её можно установить, поэтому она может постоянно оставаться в манифесте и даёт корректный результат и для существующих, и для впервые добавляемых узлов.

  Прежде чем пропустить bootstrap по аннотации `static.node.deckhouse.io/adopt-if-node-exists`, CAPS проверяет, что существующий `Node` пригоден к передаче под управление: он не удаляется, находится в состоянии `Ready`, у него заполнено поле `spec.providerID` и он принадлежит той же группе узлов, что и [StaticInstance](cr.html#staticinstance). Узел, который удаляется или ещё не готов, CAPS дожидается — объект `Node` не принадлежит CAPS и какое-то время может расходиться с остальным состоянием кластера. Узел с пустым `spec.providerID` или принадлежащий другой группе узлов под управление не передаётся: CAPS сообщает о несоответствии в условиях `StaticMachine` и в событиях и никогда не запускает bootstrap поверх узла, который уже входит в кластер. После устранения несоответствия передача под управление продолжится сама.
