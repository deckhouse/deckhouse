...
description: Deckhouse с помощью модуля node-manаger управляет управляет узлами кластера Kubernetes как связанной группой, настраивает и обновляет компоненты узлf кластера Kubernetes, управляет масштабированием кластера в облаке и управляет Linux-пользователями на узлах.
---
...
3. Установка/обновление и настройка ПО узла (containerd, kubelet и др.), подключение узла в кластер:
    ...
    * Базовая настройка операционной системы (отключение автообновления, установка необходимых пакетов, настройка параметров журналирования, настройка iptables и и.т.).
