...
- **Вручную с последующей передачей узла под автоматическое управление** [Cluster API Provider Static](#cluster-api-provider-static).
  ...
  > Функционал доступен начиная с версии Deckhouse 1.63.
...
1. **Создание ресурса [StaticInstance](cr.html#staticinstance).**
   ...
   > Можно отдать существующий узел кластера, заранее введенный в кластер вручную, под управление CAPS с помощью пометки его StaticInstance аннотацией `static.node.deckhouse.io/skip-bootstrap-phase: ""`.
