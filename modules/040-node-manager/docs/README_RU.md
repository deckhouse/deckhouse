...
6. Поддержка GPU на узлах:
   * Автоматическое обнаружение и настройка GPU NVIDIA на узлах (установка необходимых драйверов и плагинов устройства).
   * Настраиваемые режимы совместного использования GPU: эксклюзивный режим, разделение по времени (time-slicing) и разделение на аппаратные экземпляры (NVIDIA MIG).
   * Интеграция с мониторингом — автоматический сбор метрик GPU через DCGM Exporter с готовыми Grafana-дашбордами для визуализации и алертинга.
