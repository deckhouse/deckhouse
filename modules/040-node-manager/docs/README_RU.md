...
1. **Подготовка ресурсов.**
   ...
   Перед тем, как отдать bare-metal-сервер или виртуальную машину под управление CAPS, может понадобиться предварительная подготовка, например:
...
### Автоматическая простановка кастомных лейблов для узлов

На узлах можно автоматически проставить кастомные лейблы. Для этого в каталоге `/var/lib/node_labels` узла нужно создать файлы, содержащие нужные лейблы в формате `key=value`. Названия файлов могут быть любыми, вложенность каталогов с файлами не ограничена.

Такой способ работает как для уже существующих в кластере узлов, так и во время добавления новых узлов.
