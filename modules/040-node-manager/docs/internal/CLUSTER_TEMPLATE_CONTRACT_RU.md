...
Ресурсы провайдера обновляются через server-side apply. Живой `spec.controlPlaneEndpoint`
сохраняется, если шаблон его не рендерит: большинство провайдеров заполняют это поле сами, и
применение объекта без него стёрло бы найденный провайдером адрес. Если шаблон рендерит эндпоинт,
поле принадлежит ему, и при каждом apply побеждает отрендеренное значение. Оно берётся из адресов
apiserver, за которыми следит node-controller, поэтому замена master-узла его обновляет.
...
Необязательный `credentials.yaml` должен собрать ровно один `v1/Secret` с фиксированным именем
`capi-user-credentials`. Имя входит в миграционный контракт: before-Helm hook должен найти и
защитить существующий Secret до удаления старого манифеста. Оба объекта должны
находиться в `d8-cloud-instance-manager`. Если namespace не указан, node-controller добавит его.
Если `credentials.yaml` удалён, ранее созданный по этому шаблону Secret удаляется.
...
ClusterReconciler применяет тот же объект от имени `node-controller` и сохраняет UID. Метаданные
провайдера, аннотация `keep` и метки владения Helm остаются на месте.

Метки Helm сохраняются не ради отката: addon-operator обновляет релизы с `TakeOwnership`, поэтому
Helm принимает живой объект по kind и имени и эти метки не читает. Они остаются потому, что убирать
их незачем и потому, что это честное описание объекта, который Helm может забрать обратно.

Аннотацию `keep` контроллер ставит на каждый apply и никогда не снимает. Helm пропускает
помеченный объект и при prune, и при удалении релиза, а node-controller такие объекты сам не
удаляет. Поэтому отключение модуля оставит их в кластере, в том числе `capi-user-credentials` с
действующими доступами к облаку. Prune сравнивает с манифестом предыдущего релиза, так что
аннотация нужна ровно на том обновлении, где объект уезжает из чарта. Снимать её вместе с
`hooks/set_keep_policy_on_capi_resources.go`, там стоит такая же пометка об удалении.
