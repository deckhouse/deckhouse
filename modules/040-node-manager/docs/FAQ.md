...
1. If there are additional service components (not maintained by Deckhouse, such as `filebeat` DaemonSet) for these nodes, you need to specify percentage of their combined resource consumption relative to the total node resources in the parameter `standbyHolder.overprovisioningRate`.
