...
When the `InstanceClass` or `instancePrefix` parameters are modified, a process similar to updating a Deployment in Kubernetes will occur (in this case, changing `InstanceClass` results in creating a new `MachineSet`, and changing `instancePrefix` results in creating a new `MachineDeployment`). New instances will be created according to the value of the `cloudInstances.maxSurgePerZone` parameter, and as they are created, the old instances will be gradually removed with a drain operation performed beforehand.
