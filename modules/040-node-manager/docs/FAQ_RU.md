...
Например, можно настроить чтобы сначала пытались заказываться *spot-node*, а если они закончились — обычные узлы. Или, чтобы сначала пытались заказаться узлы большего размера, а если ресурсы облака исчерпаны — узлы меньшего размера.
...
В приведенном примере, `cluster-autoscaler` будет пытаться сначала заказать spot-node. Если в течение 15 минут не получается добавить узел в кластер, то NodeGroup `worker-spot` будет поставлена *на паузу* (на 20 минут) и `cluster-autoscaler` начнет заказывать узлы из NodeGroup `worker`.
