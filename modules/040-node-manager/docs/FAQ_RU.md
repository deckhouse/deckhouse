...
При изменении параметров `InstanceClass` или `instancePrefix` будет происходить процесс, аналогичный тому, как как происходит обновление обновление deployment в k8s (в данному случае при изменении `InstanceClass` - будет создан новый `Machineset`, а при изменении `instancePrefix` - `MachineDeployment`). Новые инстансы будут создаваться в соответствии со значением параметра `cloudInstances.maxSurgePerZone` и по мере их создания старые будут постепенно удаляться с предварительным выполнением операции drain.
