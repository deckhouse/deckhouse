...
1. Когда число узлов меньше _желаемого числа узлов в группе, т.е. когда нужно увеличить число узлов в группе._
1. Когда какой-то узел помечается к удалению или число узлов больше _желаемого числа узлов_, т.е. когда нужно уменьшить число узлов в группе.

_Желаемое число узлов_ — это сумма всех реплик входящих в группу узлов.
...
Werf проверяет состояние ```Ready``` у ресурсов и в случае его наличия дожидается пока значение станет ```True```. Если в группе узлов запланировано большое количество узлов, то может потребоваться значительное время на их развертывание. В результате чего Werf может завершить свою работу с ошибкой тайм-аута. Чтобы Werf проигнорировал состояние ```Ready``` у группы узлов необходимо добавить аннотации:
