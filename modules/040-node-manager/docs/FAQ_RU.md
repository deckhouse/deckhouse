...
регистрацией: kubelet отказывается от идентичности, с которой вошёл в кластер, и
регистрируется заново под новым именем. Сама машина остаётся на месте — диски,
container runtime и hostname не трогаются, — а объект Node со старым именем
удаляется после того, как переименованный узел вернулся. Это работает для узлов
типов Static и CloudStatic, включая master-узлы статического и гибридного кластера.
...
1. На самом узле выполните:
   ...
   Скрипт проверит, что новое имя свободно, перерегистрирует узел и дождётся его
   возвращения. На узле без cordon скрипт откажется работать; чтобы это обойти,
   передайте `--skip-drain`.

1. `node-manager` удалит объект Node со старым именем, как только увидит, что
   переименованный узел перешёл в `Ready`. Удаляется только Node, который не в
   состоянии `Ready` и сообщает о той же машине, — поэтому неудавшееся
   переименование оставляет старый объект Node на месте, чтобы к нему можно было
   вернуться.
