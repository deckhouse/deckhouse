...
3. Перед настройкой Kubernetes на узле убедиться, что выполнены все необходимые действия для корректной работы узла в кластере:
  - Добавлены в `/etc/fstab` все необходимые точки монтирования (nfs, ceph, ...);
  - Установлен на узел пакет `ceph-common` нужной версии или другие необходимые пакеты;
  - Настроена сетевая связанность между узлами кластера.
