...
## Как fencing обрабатывает разные типы узлов?

Fencing срабатывает, когда узел теряет связь с кластером, и защищает кластер от «зомби»-узлов с неопределённым состоянием. Чтобы кратковременные сбои не приводили к каскадному срабатыванию, fencing действует в два захода: сначала помечает ноду как подозрительную (на уровне gossip-протокола `memberlist`/Lifeguard), и только затем «строит на ней забор» — выключает узел и (при необходимости) удаляет его объект Node.

Поведение при срабатывании зависит от типа узла (лейбл `node.deckhouse.io/type`) и от режима fencing (лейбл `node-manager.deckhouse.io/fencing-mode`, см. [`spec.fencing.mode`](cr.html#nodegroup-v1-spec-fencing-mode)).

### Облачные узлы (CloudEphemeral, CloudPermanent)

Для облачных узлов при срабатывании fencing в режиме `Watchdog`:

- Fencing-agent перестаёт кормить watchdog (модуль ядра `softdog`), и по истечении таймаута ядро узла уходит в kernel panic. Это гарантирует, что на узле не останется работающей нагрузки.
- Fencing-controller удаляет объект Node из кластера.
- Удаление Node подхватывает cloud-provider-контроллер (MCM/CAPI): он удаляет стоящую за узлом виртуальную машину, и при необходимости заказывает новую. Таким образом, сбойный узел пересоздаётся автоматически.

Теоретически возможен сценарий, когда kubelet на «зависшей» ВМ ожил бы раньше, чем cloud-provider удалил машину, и попытался бы заново зарегистрировать узел в кластере. Вероятность такого события минимальна благодаря нескольким предохранителям:

- kernel panic оставляет узел в нерабочем состоянии — чтобы узел вернулся, кто-то должен вручную перезапустить ВМ;
- автоматическая перезагрузка узла после kernel panic отключена на уровне ОС для всех узлов с включённым fencing;
- между kernel panic и моментом, когда машину успеет подобрать cloud-provider, есть предохранительные тайминги.

### Статические узлы (Static, CloudStatic)

Для статических узлов объект Node **не удаляется**, поскольку его никто не пересоздаст: машину за такой Node в облаке не заказать. Fencing-agent так же выключает узел через kernel panic, чтобы исключить undefined behavior, а fencing-controller только выгоняет нагрузку с узла (удаляет поды). Сам объект Node остаётся, и узел ожидает, пока оператор вручную не введёт его обратно в строй.

### Режим `Notify`

В режиме `Notify` fencing-agent работает и отслеживает состояние кластера, но watchdog не активируется, и узел не перезагружается. Fencing-controller для узлов в этом режиме также не удаляет объект Node — режим предназначен для отладки и наблюдения за работой fencing без реальных воздействий на узлы. Но поды будут удалены с узла.

### Отключение автоматической перезагрузки после kernel panic

Во всех случаях при включённом fencing автоматическая перезагрузка узла после kernel panic отключается на уровне ОС (настройкой ядра). Это исключает возврат узла «сам собой» с неопределённым состоянием до того, как cloud-provider удалит/пересоздаст машину (для облачных узлов) или оператор введёт узел обратно в строй (для статических узлов).

Если у вас встречается сценарий, в котором узел с включённым fencing возвращается в кластер не так, как описано выше, — обратитесь в поддержку: это поможет уточнить цепочку проверок и предохранителей.
