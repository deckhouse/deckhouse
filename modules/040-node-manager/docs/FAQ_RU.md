...
{% alert level="warning" %}
В кластере, где маршруты в сеть подов создаёт облако, имя узла обязано совпадать
с именем его машины.
{% endalert %}
...
Узел кластера, где маршруты в сеть подов создаёт облако, переименовать тоже
нельзя, и по той же причине: маршрут создан для имени, которое у узла сейчас,
...
исчезает, он заново запускает бутстрап машины под именем из
