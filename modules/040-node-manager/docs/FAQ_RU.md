...
пределах парка машин, статическому узлу можно задать собственное имя. Hostname
...
hostname не трогаются. Это работает для узлов типов Static и CloudStatic, включая
...
Узлы, созданные облаком, — `CloudEphemeral` и `CloudPermanent` — переименовать
нельзя, и скрипт `rename_node.sh` на них не устанавливается. Их имя — это то, по
чему остальной кластер находит стоящую за ними машину: `machine-controller-manager`
сопоставляет Machine с Node по имени, а состояние инфраструктуры, с которым
работает `dhctl converge`, хранится в секрете `d8-node-terraform-state-<имя-узла>`.
Переименованный узел выглядел бы как пропавшая машина и взявшийся ниоткуда узел.
...
Если узел находится под управлением CAPS, приведите в соответствие и
`spec.nodeName` в его StaticInstance; сам CAPS находит узел по provider ID, а не
по имени.
