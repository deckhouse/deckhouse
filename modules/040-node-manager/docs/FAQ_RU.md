...
## Почему может не применяться изменение CRI?

Одной из основных причин такого поведения может быть наличие лейблов `node.deckhouse.io/containerd-v2-unsupported` и `node.deckhouse.io/containerd-config=custom`.

Лейбл `node.deckhouse.io/containerd-v2-unsupported` ставится в случае, если нода не удовлетворяет следующим критериям:
1. Версия ядра должна быть не ниже 5.8.
2. Версия systemd должна быть не ниже 244.
3. Cgroup v2 должен быть активирован.
4. EROFS должен быть доступен.

Лейбл `node.deckhouse.io/containerd-config=custom` ставится, если на ноде есть файл в директории `conf.d` или `conf2.d` с расширением `.toml`. В этом случае нужно попробовать их удалить, если это не приведет к критическим последствиям в работе контейнеров, и удалить NGC, с помощью которых они могли быть добавлены.

Если вы используете модуль Deckhouse Virtualization Platform, одной из причин, почему не удается сменить CRI, может стать NGC `containerd-dvcr-config.sh`. Если платформа виртуализации уже установлена и работает, данное NGC можно удалить.
