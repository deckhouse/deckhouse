...
Операции по изменению конфигурации узлов, могут потребовать перезагрузки. Например, изменение некоторых настроек sysctl, в частности при изменении параметра `kernel.yama.ptrace_scope` (например при использовании `astra-ptrace-lock enable/disable` в дистрибутиве Astra Linux).
