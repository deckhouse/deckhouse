...
{% alert level="info" %}
Используется в containerd v1, если Deckhouse не управляется модулем Registry (режим [`Unmanaged`](/modules/deckhouse/v1.72/configuration.html#parameters-registry)).
{% endalert %}
...
Используется в containerd v1, если управление осуществляется через модуль Registry (например, в режиме [`Direct`](/modules/deckhouse/v1.72/configuration.html#parameters-registry)).
{% endalert %}
