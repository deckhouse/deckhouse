...
Порядок конвертирования кластера с одним master-узлом в мультимастерный кластер описан в [FAQ модуля `control-plane-manager`](/modules/control-plane-manager/faq.html#как-добавить-master-узлы-в-облачном-кластере).
...
Порядок конвертирования мультимастерного кластера в кластер с одним master-узлом описан в [FAQ модуля `control-plane-manager`](/modules/control-plane-manager/faq.html#как-уменьшить-число-master-узлов-в-облачном-кластере).
...
## Как задать узлу собственное имя?

По умолчанию узел входит в кластер под hostname своей машины. Если hostname'ы
назначаются не вами — конвейером подготовки образов, DHCP, — или не уникальны в
пределах парка машин, узлу типа `Static` можно задать собственное имя. Hostname
машины при этом не меняется: так называется только объект Node.

Это работает только для узлов типа `Static`. Узел `CloudStatic` инициализирует
cloud controller manager, а у него нет ничего, кроме имени узла, чтобы найти
стоящую за ним машину: `providerID` у такого узла не проставляется вовсе. Под
любым другим именем узел навсегда остаётся с taint
`node.cloudprovider.kubernetes.io/uninitialized`, не получает ни адресов, ни
лейблов зоны и никогда не становится Ready, — поэтому бутстрап отклоняет такое
имя, а не оставляет узел в этом состоянии.

Имя должно быть DNS-именем по RFC 1123 (строчные буквы, цифры, `-` и `.`) и
уникальным в кластере. Оно фиксируется при первом бутстрапе узла и дальше не
меняется — поэтому hostname уже работающего узла можно менять, не трогая кластер.

Для статического узла, добавляемого [вручную](#как-добавить-статический-узел-в-кластер),
задайте `D8_NODE_NAME` при запуске скрипта бутстрапа:

```shell
echo <Base64-CODE> | base64 -d | D8_NODE_NAME=worker-rack3-07 bash
```

Для узла, добавляемого через [CAPS](./#cluster-api-provider-static), задайте
[`spec.nodeName`](cr.html#staticinstance-v1alpha2-spec-nodename) в StaticInstance:
...
Поле можно изменить, только пока StaticInstance находится в фазе `Pending`: дальше
имя уже записано на узел.

Для первого master-узла статического или гибридного кластера передайте
`--node-name` команде `dhctl bootstrap`. В облачном кластере этот параметр
отклоняется: там master — узел, созданный инфраструктурой, и `dhctl converge`
находит его машину именно по имени узла.

{% alert level="warning" %}
В кластере, где сеть подов маршрутизируется средствами облака, имя узла обязано
совпадать с именем его машины.
{% endalert %}

В таком кластере работает `cni-simple-bridge` вместе с контроллером маршрутов
облачного провайдера, а этот контроллер находит стоящую за узлом машину по имени
узла. Узел с любым другим именем никогда не получит маршрут до своей подсети подов,
и ничто на нём не будет доступно остальному кластеру.

Узнать такой кластер можно по причине, с которой узлы сообщают условие
`NetworkUnavailable`: `RouteCreated` — там, где CNI со своей сетью поверх сети
узлов указывает свою (например, `FlannelIsUp`):
...
## Как переименовать узел, который уже в кластере?

Объект Node переименовать нельзя, поэтому узел переименовывается повторной
регистрацией: объект Node со старым именем удаляется, и машина регистрируется
заново под новым. Сама машина остаётся на месте — диски, container runtime и
hostname не трогаются. Это работает для узлов типа `Static`, включая
master-узлы статического и гибридного кластера.

{% alert level="warning" %}
Машина будет перезагружена, а нагрузка на узле переименование не переживёт.
{% endalert %}

Перезагрузка обязательна: переименование обесценивает состояние на узле, которое
больше ничем не сбрасывается, — мост CNI сохраняет подсеть прежней аренды, а всё,
что прочитало имя узла при старте, помнит старое. Перезагрузка снимает это разом.

Порядок тоже обязателен. Объект Node со старым именем удаляется до того, как машина
вернётся. Пока оба объекта Node существуют, это один адрес под двумя именами, и CNI,
который различает соседей по адресу, при удалении одного снимает запись и для
другого — переименованный узел остаётся недоступен.

Вместе со старым объектом Node пропадёт всё, что на нём оставалось запланировано,
поэтому сначала выполните drain узла. Master-узлы переименовывайте по одному,
дожидаясь, пока etcd сообщит, что все члены кластера здоровы, перед переходом к
следующему.

Узлы всех остальных типов — `CloudEphemeral`, `CloudPermanent` и `CloudStatic` —
переименовать нельзя, и скрипт `rename_node.sh` на них не устанавливается. Их имя —
это то, по чему облако находит стоящую за ними машину: `machine-controller-manager`
сопоставляет Machine с Node по имени, состояние инфраструктуры, с которым работает
`dhctl converge`, хранится в секрете `d8-node-terraform-state-<имя-узла>`, а узел
`CloudStatic`, у которого нет `providerID`, только по имени и находит cloud
controller manager. Переименованный узел выглядел бы как пропавшая машина и
взявшийся ниоткуда узел.

Узел кластера, где сеть подов маршрутизируется средствами облака, переименовать
тоже нельзя, и по той же причине: маршрут создан для имени, которое у узла сейчас,
а для нового не будет создан никогда. Скрипт `rename_node.sh` определяет такой
кластер и отказывается работать.

Локальные PersistentVolume закрепляются за узлом через `nodeAffinity` по лейблу
`kubernetes.io/hostname`, значение которого — имя узла. После переименования такие
тома окажутся привязаны к несуществующему узлу; их нужно перенести или пересоздать.
...
1. Выполните drain узла:
   ...
   Здесь регулярно мешают две вещи. Под, который никто не пересоздаст, drain
   отказывается удалять — для него нужен `--force`. А под, у которого в
   PodDisruptionBudget не осталось запаса, — это сам `deckhouse`, когда его реплика
   единственная, — drain будет бесконечно пытаться вытеснить, хотя именно вытеснение
   бюджет и запрещает. Такой под нужно не вытеснять, а удалить: он будет пересоздан
   на другом узле:
...
1. Получите bootstrap-токен группы узлов — по нему kubelet аутентифицируется, чтобы
   зарегистрироваться под новым именем:
...
1. На самом узле выполните:

   ```shell
   sudo /var/lib/bashible/rename_node.sh --new-name <NEW_NAME> --bootstrap-token <TOKEN>
   ```

   Скрипт проверит, что новое имя свободно, остановит kubelet и будет ждать. На узле
   без cordon он откажется работать; чтобы это обойти, передайте `--skip-drain`.

   Объект Node со старым именем удаляется за него: узел просит об этом, проставляя
   себе аннотацию `node.deckhouse.io/rename-to`, а `node-manager` удаляет объект,
   когда узел перестал отвечать. Узлу разрешено изменять только собственный объект
   Node — именно это делает просьбу достоверной; `node-manager` не удаляет ничего,
   о чём его не попросил сам узел, и ничего, что ещё отвечает. Просьбу, которую он
   выполнять не станет, видно как событие `NodeRenameRejected` на узле.

1. Скрипт перезагрузит машину. Она вернётся, зарегистрируется под новым именем и
   поднимет CNI на подсети, выданной новому объекту Node. Наблюдать удобно через
   `d8 k get nodes -w`.

   Master-узел входит в etcd отдельным членом: член со старым именем удаляется, а
   узел добавляется заново — сначала как learner, а затем получает право голоса.
   Минуту-другую `etcd-<новое-имя>` перезапускается, `etcdctl member list` показывает
   член как `unstarted`, а `endpoint health` считает его нездоровым. Это состояние проходит
   само; остальные члены всё это время держат кворум.

Узел под управлением [CAPS](./#cluster-api-provider-static) переименовать таким
образом нельзя — скрипт `rename_node.sh` на нём отказывается работать. Имя такому
узлу даёт его StaticInstance, и CAPS за этим следит: как только объект Node
исчезает, он заново бутстрапит машину под именем из
[`spec.nodeName`](cr.html#staticinstance-v1alpha2-spec-nodename). Переименование
откатывается в течение минуты, оплаченное перезагрузкой. Чтобы дать такой машине
другое имя, удалите её StaticInstance и добавьте её заново с нужным именем.
...
1. Установка лейблов в `NodeGroup` `spec.nodeTemplate.labels` для последующего использования их в `Pod` [spec.nodeSelector](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/) или [spec.affinity.nodeAffinity](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#node-affinity). Указывает, какие именно узлы будут выбраны планировщиком для запуска целевого приложения.
...
- Версия ядра Linux входит в диапазоны 6.12.0–6.12.28 или 6.14.0–6.14.6 (эти версии подвержены уязвимости CVE-2025-37999 в EROFS);
...
Кроме того, для смены CRI может понадобиться снять пользовательский лейбл `node.deckhouse.io/containerd-config=custom`. Сделать это можно с помощью команды:
...
Подробнее о работе fencing-agent и fencing-controller можно почитать [в описании параметра `spec.fencing.mode`](cr.html#nodegroup-v1-spec-fencing-mode) ресурса NodeGroup.

Поведение при срабатывании зависит от типа узла (лейбл `node.deckhouse.io/type`).

### Облачные узлы (CloudEphemeral, CloudPermanent)

При срабатывании механизма fencing в режиме `Watchdog`:

1. Fencing-agent перестаёт сбрасывать таймер watchdog (модуль ядра `softdog`). По истечении таймаута происходит kernel panic — это гарантирует, что на узле не останется работающей нагрузки.
1. Fencing-controller удаляет объект Node из кластера.
1. Cloud-provider-controller (MCM/CAPI) подхватывает удаление Node и удаляет соответствующую виртуальную машину и, при необходимости, заказывает новую. Таким образом, сбойный узел пересоздаётся автоматически.

Теоретически возможен сценарий, когда kubelet на «зависшей» ВМ восстановится раньше, чем cloud-provider удалит машину, и попытается заново зарегистрировать узел в кластере. Вероятность такого события минимальна благодаря следующим механизмам:

- kernel panic оставляет узел в нерабочем состоянии — для возврата требуется ручной перезапуск ВМ;
- автоматическая перезагрузка узла после kernel panic отключена на уровне ОС для всех узлов с включённым `fencing`;
- между kernel panic и моментом, когда машину успеет обработать cloud-provider, есть предохранительные тайминги.

### Статические узлы (Static, CloudStatic)

Для статических узлов объект Node **не удаляется**, поскольку соответствующую машину нельзя автоматически пересоздать. В этом случае:

1. Fencing-agent выключает узел через kernel panic, чтобы исключить неопределённое поведение.
1. Fencing-controller только удаляет поды с узла.
1. Объект Node остаётся в кластере, а сам узел ожидает, пока оператор не восстановит его вручную.

### Отключение автоматической перезагрузки после kernel panic

Во всех случаях при включённом `fencing` автоматическая перезагрузка узла после kernel panic отключается на уровне ОС (настройкой ядра). Это предотвращает возврат узла в кластер с неопределённым состоянием:

- **для облачных узлов** — до момента, когда cloud-provider удалит или пересоздаст виртуальную машину;
- **для статических узлов** — до ручного восстановления оператором.

Если вы наблюдаете сценарий, при котором узел с включённым `fencing` возвращается в кластер не так, как описано выше, [обратитесь в поддержку](https://deckhouse.ru/tech-support/). Это поможет уточнить цепочку проверок и защитных механизмов.
...
1. Установите драйвер NVIDIA и NVIDIA Container Toolkit.
   ...
   Актуальные примеры NodeGroupConfiguration для группы узлов `gpu` (Ubuntu, Debian, CentOS, РЕД ОС, Astra Linux) приведены в [FAQ модуля `gpu`](/modules/gpu/faq.html#как-работать-с-gpu-узлами).
...
1. Проверьте установку на узле, используя команду:
   ...
   {% alert level="warning" %}
   Если GPU-узел был подготовлен заранее, поставлялся с предустановленными драйверами NVIDIA или на нём установлена ОС с графическим окружением, проверьте, что GPU не занята сторонними процессами.

   В выводе `nvidia-smi` в секции `Processes` не должно быть сторонних процессов, использующих GPU. На узлах с графическим окружением это могут быть, например, процессы графической сессии или display manager: `Xorg`, `gnome-shell`, `gdm`, `sddm`, `lightdm` и другие. Такие процессы могут занимать память GPU и мешать корректной работе рабочих нагрузок, а также применению MIG-конфигурации.
   {% endalert %}
...
Примеры приведены в разделе [Управление узлами: примеры](examples.html#пример-gpu-nodegroup).
...
1. Проверьте модель GPU. MIG поддерживается в моделях H100/A100/A30 и **не** поддерживается в моделях V100/T4. Проверить поддержку можно [в таблицах профилей](https://docs.nvidia.com/datacenter/tesla/mig-user-guide/latest/supported-mig-profiles.html) в руководстве NVIDIA MIG.

1. Убедитесь, что GPU не используется процессами ОС или пользовательскими приложениями. Если на узле запущено графическое окружение, display manager или другие процессы, использующие GPU, применение MIG-конфигурации может завершиться ошибкой или не примениться до освобождения GPU. Проверьте это командой:

   ```shell
   nvidia-smi
   ```

1. Проверьте [конфигурацию NodeGroup](cr.html#nodegroup-v1-spec-gpu):
