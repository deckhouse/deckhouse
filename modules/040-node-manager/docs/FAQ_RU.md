...
В режиме `Notify` fencing-agent работает и отслеживает состояние кластера, но watchdog не активируется, и узел не перезагружается. Fencing-controller для узлов в этом режиме также не удаляет объект Node — режим предназначен для отладки и наблюдения за работой fencing без реальных воздействий на узлы.
