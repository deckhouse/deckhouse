...
Параметр [priority](cr.html#nodegroup-v1-spec-cloudinstances-priority) CustomResource `NodeGroup` можно использовать для того, чтобы указывать порядок заказа узлов кластера.
Например, можно настроить чтобы сначала пытались заказываться _spot-node_, а если они закончились — обычные узлы. Или, чтобы сначала пытались заказаться узлы большего размера, а если ресурсы облака исчерпаны — узлы меньшего размера.

Пример создания двух `NodeGroup` с использованием spot-node:
...
В приведенном примере, `cluster-autoscaler` будет пытаться сначала заказать spot-node. Если в течение 15 минут не получается добавить узел в кластер, то NodeGroup `worker-spot` будет поставлена _на паузу_ (на 20 минут) и `cluster-autoscaler` начнет заказывать узлы из NodeGroup `worker`.
Если через 30 минут для нагрузки в кластере понадобится еще один узел, то он снова попытается заказаться сначала из NodeGroup `worker-spot`, а только потом из NodeGroup `worker`.

Если NodeGroup `worker-spot` достигнет своего максимума (в примере — 5 узлов), то узлы будут заказываться из NodeGroup `worker`.

Шаблоны узлов (labels/taints) для NodeGroup `worker` и `worker-spot` должны быть одинаковы, или, по крайней мере, должны быть подходящими для той нагрузки, которая запускает процесс увеличения кластера.
