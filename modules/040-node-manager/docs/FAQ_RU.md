...
Ресурс Instance представляет собой ресурс эфемерной машины, независимый от реализации, например, машины созданные MachineControllerManager или Cluster API Provider Static будут иметь соответcтвующий ресурс Instance.

Объект не содержит спецификации. Статус содержит ссылку на:
1. InstanceClass если он существует для данной реализации.
1. На объект Node kubernetes.
1. Текущий статус машины.
1. На этапе создания машины появляется информация как посмотреть [логи создания машины](#как-посмотреть-что-в-данный-момент-выполняется-на-узле-при-его-создании).

При создании/удалении машины соотвственно создается/удаляется объект Instance.
