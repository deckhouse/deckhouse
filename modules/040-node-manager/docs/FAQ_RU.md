...
- Недоступна файловая система EROFS. В AWS стандартные Ubuntu AMI не содержат модуль `erofs` — для его наличия требуется пакет `linux-modules-extra`. Начиная с Deckhouse v1.76 этот пакет устанавливается автоматически при бутстрапе узла. На более старых версиях установите его вручную: `apt-get install -y linux-modules-extra-$(uname -r) && modprobe erofs`.
