...
### Как добавить статический узел с неизменяемой ОС?

На узле, чья NodeGroup просит `systemType: Immutable`, нет ни sshd, ни bashible,
поэтому `bootstrap.sh` к нему неприменим. Машина загружается с образа olcedar,
не находит конфигурации и ждёт её на `50000/TCP`; вы отдаёте конфигурацию туда,
машина ставится сама и входит в кластер.

{% alert level="warning" %}
Эндпоинт на `50000/TCP` никого не проверяет: у машины без конфигурации ещё нет
секрета, которым она могла бы подтвердить себя. Любой, кто дотянется до этого
порта, введёт машину в свой кластер, а инвентарь, который она отдаёт на том же
порту, покажет её диски и их серийные номера. Держите такие машины в отдельном
сегменте сети до окончания установки.
{% endalert %}

1. Создайте [NodeGroup](cr.html#nodegroup) для таких машин:
...
1. Прочитайте шаблон этой группы. Он собирается на лету и нигде не хранится:
   кластер подставляет всё, что знает сам, включая живой bootstrap-токен, и
...
1. Спросите машину, что у неё есть. Она отвечает в форме того документа,
   которого ждёт, — чтобы вы объединили ответ с шаблоном, а не перепечатывали его.
   ...
   Те же данные отдают ещё два адреса: `/inventory.pretty` — выровненная таблица
   с разделами, их файловыми системами и метками, `/inventory.json` — форма,
   которую читает dhctl.

1. Снимите комментарий с диска, на который ставится узел, и объедините документы.
...
1. Отправьте её машине:
   ...
   ```shell
   curl -X PUT --data-binary @worker-0.yaml http://<адрес-машины>:50000/config
   ```

   Машина разметит названный диск, поставит систему, перезагрузится и войдёт в
   кластер. Больше на неё ничего не загружается.
...
1. Дождитесь узла:
   ...
   Зарегистрировавшись, узел публикует свой `NodeConfig`, и сеть с дисками,
   которые вы отправили, в нём остаются: остальное кластер пересобирает из
   NodeGroup, а известное только машине сохраняет.

Чтобы заменить железо под узлом, удалите его `NodeConfig` и отдайте новой машине
конфигурацию тем же способом; при регистрации она опубликует новый.
