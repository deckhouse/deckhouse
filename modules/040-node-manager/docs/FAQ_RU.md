...
1. При наличии дополнительных служебных компонентов (не обслуживаемых Deckhouse, например DaemonSet `filebeat`) для этих узлов — задать их процентное потребление ресурсов от общего количества ресурсов ноды в параметре `standbyHolder.overprovisioningRate`.
