...
{% alert level="warning" %}
Собственное имя нельзя задать узлу кластера, в котором сеть подов маршрутизируется
средствами облака, — `cni-simple-bridge` вместе с контроллером маршрутов облачного
провайдера. Там контроллер маршрутов находит стоящую за узлом машину по имени узла,
поэтому узел с любым другим именем никогда не получит маршрут до своей подсети
подов, и ничто на нём не будет доступно остальному кластеру. Имя узла в таком
кластере обязано совпадать с именем его машины.
{% endalert %}
...
{% alert level="warning" %}
**Машина будет перезагружена** в ходе переименования, а объект Node со старым
именем удаляется до того, как она вернётся.
...
Порядок тоже обязателен. Пока оба объекта Node существуют, это один адрес под
двумя именами, и CNI, который различает соседей по адресу, при удалении одного
снимает запись и для другого — переименованный узел остаётся недоступен.
{% endalert %}

{% alert level="warning" %}
Нагрузка на узле переименование не переживёт: старый объект Node удаляется, а
вместе с ним и всё, что на нём оставалось запланировано. Сначала выполните drain
узла. Master-узлы переименовывайте по одному, дожидаясь, пока etcd сообщит, что
все члены кластера здоровы, перед переходом к следующему.
{% endalert %}
