...
- **Manually with subsequent transfer of the Node under automatic control** of [Cluster API Provider Static](#cluster-api-provider-static).
  ...
  Before skipping the bootstrap because of `static.node.deckhouse.io/adopt-if-node-exists`, CAPS checks that the existing `Node` is fit to be taken over: it is not being deleted, it is `Ready`, its `spec.providerID` is set, and it belongs to the same node group as the [StaticInstance](cr.html#staticinstance). A node that is being deleted or is not ready yet is waited for, because CAPS does not own the `Node` object and it may be out of sync with the rest of the cluster for a while. A node with an empty `spec.providerID` or one that belongs to another node group is not adopted at all: CAPS reports the mismatch in the `StaticMachine` conditions and in events and never bootstraps over a node that is already part of the cluster. Once the mismatch is resolved, the adoption proceeds on its own.
