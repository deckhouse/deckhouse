...
1. **Автоматизация через `NodeGroupConfiguration`** (подробнее – в разделе [Порядок действий по добавлению GPU-узла в кластер](/modules/node-manager/faq.html#порядок-действий-по-добавлению-gpu-узла-в-кластер)).
...
[инструкцией](/modules/node-manager/faq.html#как-посмотреть-доступные-mig-профили-в-кластере).
...
1. Зайдите на новый узел по SSH и выполните следующую команду, вставив полученную в п. 3 Base64-строку:
   ...
   меняя hostname, задайте `D8_NODE_NAME`:
