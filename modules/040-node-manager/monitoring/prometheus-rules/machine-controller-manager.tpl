- name: d8.machine-controller-manager.availability
  ...
  - alert: D8MachineControllerManagerPodIsNotReady
    expr: min by (pod) (kube_pod_status_ready{d8_source="dkp", condition="false", namespace="d8-cloud-instance-manager", pod=~"machine-controller-manager-.*"}) > 0
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8MachineControllerManagerPodIsNotRunning
    expr: absent(kube_pod_status_phase{d8_source="dkp", namespace="d8-cloud-instance-manager",phase="Running",pod=~"machine-controller-manager-.*"})
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8MachineControllerManagerTargetDown
    expr: max by (job) (up{d8_source="dkp", job="machine-controller-manager", namespace="d8-cloud-instance-manager"} == 0)
    for: 5m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8MachineControllerManagerTargetAbsent
    expr: absent(up{d8_source="dkp", job="machine-controller-manager", namespace="d8-cloud-instance-manager"} == 1)
    for: 5m
    labels:
      severity_level: "8"
      tier: cluster
...
- name: d8.machine-controller-manager.malfunctioning
  ...
  - alert: D8MachineControllerManagerPodIsRestartingTooOften
    expr: max by (pod) (increase(kube_pod_container_status_restarts_total{d8_source="dkp", namespace="d8-cloud-instance-manager", pod=~"machine-controller-manager-.*"}[1h]) and kube_pod_container_status_restarts_total{d8_source="dkp", namespace="d8-cloud-instance-manager", pod=~"machine-controller-manager-.*"}) > 5
    labels:
      severity_level: "9"
      tier: cluster
      d8_module: node-manager
