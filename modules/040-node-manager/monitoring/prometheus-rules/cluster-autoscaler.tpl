...
{{- $clusterAutoscalerTargetDownExpr := printf `max by (job) (up{d8_source="dkp", job=~"%s", namespace="d8-cloud-instance-manager"} == 0)` $clusterAutoscalerJobsRegex -}}
{{- $clusterAutoscalerTargetAbsentExprs := list -}}
{{- range $job := $clusterAutoscalerJobs -}}
  {{- $clusterAutoscalerTargetAbsentExprs = append $clusterAutoscalerTargetAbsentExprs (printf `absent(up{d8_source="dkp", job="%s", namespace="d8-cloud-instance-manager"} == 1)` $job) -}}
{{- end -}}
...
- name: d8.cluster-autoscaler.availability
  ...
  - alert: D8ClusterAutoscalerManagerPodIsNotReady
    expr: min by (pod, namespace) (kube_pod_status_ready{d8_source="dkp", condition="false", namespace="d8-cloud-instance-manager", pod=~"cluster-autoscaler-.*"}) > 0
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8ClusterAutoscalerPodIsNotRunning
    expr: absent(kube_pod_status_phase{d8_source="dkp", namespace="d8-cloud-instance-manager",phase="Running",pod=~"cluster-autoscaler-.*"})
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
...
- name: d8.cluster-autoscaler.malfunctioning
  ...
  - alert: D8ClusterAutoscalerPodIsRestartingTooOften
    expr: max by (pod) (increase(kube_pod_container_status_restarts_total{d8_source="dkp", namespace="d8-cloud-instance-manager", pod=~"cluster-autoscaler-.*"}[1h]) and kube_pod_container_status_restarts_total{d8_source="dkp", namespace="d8-cloud-instance-manager", pod=~"cluster-autoscaler-.*"}) > 5
    labels:
      severity_level: "9"
      tier: cluster
      d8_module: node-manager
  ...
  - alert: D8ClusterAutoscalerTooManyErrors
    expr: sum by(instance) (increase(cluster_autoscaler_errors_total{d8_source="dkp"}[20m]) > 5)
    for: 5m
    labels:
      severity_level: "8"
      tier: cluster
