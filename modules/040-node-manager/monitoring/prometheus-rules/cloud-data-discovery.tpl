- name: cloud-data-discoverer.general
  ...
  - alert: D8CloudDataDiscovererCloudRequestError
    for: 1h
    expr: max by(job, namespace)(cloud_data_discovery_cloud_request_error{d8_source="dkp"} == 1)
    labels:
      severity_level: "6"
  ...
  - alert: D8CloudDataDiscovererSaveError
    for: 1h
    expr: max by(job, namespace)(cloud_data_discovery_update_resource_error{d8_source="dkp"} == 1)
    labels:
      severity_level: "6"
      d8_module: node-manager
      d8_component: cloud-data-discoverer
  ...
  - alert: ClusterHasOrphanedDisks
    for: 1h
    expr: max by(job, id, name, namespace)(cloud_data_discovery_orphaned_disk_info{d8_source="dkp"} == 1)
    labels:
      severity_level: "6"
      d8_module: node-manager
      d8_component: cloud-data-discoverer
  ...
  - alert: UnmetCloudConditions
    for: 1h
    expr: max by(job, id, name, namespace)(cloud_data_discovery_cloud_conditions_error{d8_source="dkp"} == 1)
    labels:
      severity_level: "6"
      d8_module: node-manager
      d8_component: cloud-data-discoverer
