...
func handleUpdateNGStatus(input *go_hook.HookInput) error {
	...
	for _, res := range snap {
		...
		input.PatchCollector.Filter(set_cr_statuses.SetProcessedStatus(applyNodeGroupCrdFilter), "deckhouse.io/v1", "nodegroup", "", ngName, object_patch.FilterWithSubresource("/status"), object_patch.FilterWithIgnoreHookError(true))
	}
