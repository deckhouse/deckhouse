...
func handleUpdateNGStatus(input *go_hook.HookInput) error {
	...
	for _, res := range snap {
		...
		input.PatchCollector.PatchWithMutatingFunc(set_cr_statuses.SetProcessedStatus(applyNodeGroupCrdFilter), "deckhouse.io/v1", "nodegroup", "", ngName, object_patch.WithSubresource("/status"), object_patch.WithIgnoreHookError(true))
	}
