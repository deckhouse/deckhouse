...
func lockHandler(input *go_hook.HookInput) error {
	...
	if deployment.ImageDigestOrTag != valuesDigest {
		...
		input.PatchCollector.MergePatch(annotationsPatch, "v1", "Secret", bashibleNamespace, "bashible-apiserver-context", object_patch.PatchWithIgnoreMissingObject(true))
		return nil
	}
