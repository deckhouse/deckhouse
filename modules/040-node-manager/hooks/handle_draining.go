...
const (
	...
	nodeGroupAnnotationKey = "node.deckhouse.io/group"
...
func drainFilter(obj *unstructured.Unstructured) (go_hook.FilterResult, error) {
	...
	ngName, _ = node.Annotations[nodeGroupAnnotationKey]
...
func getDrainTimeout(input *go_hook.HookInput, client k8s.Client, ngName string) time.Duration {
	...
	for _, group := range nodeGroups.Items {
		...
		spec, _, err := unstructured.NestedMap(group.Object, "spec")
		if err != nil {
			input.Logger.Error("Error extracting 'spec' from node group", "ngName", groupName)
			return defaultDrainTimeout
		}
