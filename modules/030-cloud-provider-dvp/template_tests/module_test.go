...
var _ = Describe("Module :: cloud-provider-dvp :: helm template ::", func() {
	...
	Context("DVP", func() {
		...
		It("Everything must render properly", func() {
			...
			Expect(len(capiSecretData["crds/external/deckhouseclusters.yaml"].String())).To(BeNumerically(">", 0))
			Expect(len(capiSecretData["capi/instance-class.checksum"].String())).To(BeNumerically(">", 0))
		})
	})
})
