...
func (r *DeckhouseMachineReconciler) reconcileDeleteOperation(
	...
	vm, err := r.getVM(ctx, dvpMachine)
	if errors.Is(err, cloudprovider.InstanceNotFound) {
		logger.Info("VM not found in DVP, nothing to do")
		return ctrl.Result{}, nil
	}
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("find DeckhouseMachine: %w", err)
	}

	vmDisksList, err := r.DVP.ComputeService.GetVMAttachedBlockDevices(ctx, vm.Name)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("find DeckhouseMachine disks: %w", err)
	}

	if err = r.DVP.ComputeService.DeleteVM(ctx, vm.Name); err != nil {
		return ctrl.Result{}, fmt.Errorf("delete DeckhouseMachine: %w", err)
	}

	merr := &multierror.Error{}
	for i := range vmDisksList.Items {
		if err = r.DVP.DiskService.RemoveDiskByName(ctx, vmDisksList.Items[i].Name); err != nil {
			merr = multierror.Append(merr, fmt.Errorf("delete DeckhouseMachine: %w", err))
		}
	}
...
func (r *DeckhouseMachineReconciler) getOrCreateVM(
	...
	if err != nil {
		...
		return nil, false, fmt.Errorf("cannot get VirtualMachine: %w", err)
	}
...
func (r *DeckhouseMachineReconciler) getVM(ctx context.Context, dvpMachine *infrastructurev1a1.DeckhouseMachine) (*v1alpha2.VirtualMachine, error) {
...
func (r *DeckhouseMachineReconciler) createVM(ctx context.Context, dvpMachine *infrastructurev1a1.DeckhouseMachine) (*v1alpha2.VirtualMachine, error) {
	vm, err := r.DVP.ComputeService.CreateVM(ctx, &v1alpha2.VirtualMachine{
		...
		Spec: v1alpha2.VirtualMachineSpec{
			...
			// Bootloader:               dvpMachine.Spec.Bootloader, // TODO
