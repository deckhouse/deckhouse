...
func (r *DeckhouseMachineReconciler) reconcileUpdates(
	...
	if err := r.reconcileDisksStorageClass(ctx, logger, dvpMachine); err != nil {
...
func (r *DeckhouseMachineReconciler) reconcileDisksStorageClass(
	...
	bootDiskName := dvpMachine.Name + "-boot"
	if err := r.migrateDiskStorageClassIfNeeded(ctx, logger, bootDiskName, dvpMachine.Spec.RootDiskStorageClass); err != nil {
		return fmt.Errorf("boot disk: %w", err)
	}

	for i, d := range dvpMachine.Spec.AdditionalDisks {
		diskName := fmt.Sprintf("%s-additional-disk-%d", dvpMachine.Name, i)
		if err := r.migrateDiskStorageClassIfNeeded(ctx, logger, diskName, d.StorageClass); err != nil {
...
func (r *DeckhouseMachineReconciler) migrateDiskStorageClassIfNeeded(
	...
	return r.DVP.DiskService.MigrateDiskStorageClass(ctx, diskName, desiredStorageClass)
}
