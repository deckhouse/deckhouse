---
title: "Руководство администратора модуля stronghold: методы аутентификации"
LinkTitle: "Руководство администратора: методы аутентификации"
description: "Руководство администратора по установке и настройке методов аутентификации модуля stronghold."
---
...
Метод `jwt auth` может быть использован для аутентификации в модуле stronghold с помощью OIDC или путем предоставления JWT.
...
Модуль stronghold включает два встроенных потока авторизации OIDC: пользовательский интерфейс модуля stronghold UI и CLI с использованием логина vault.

**Перенаправление URI**

Важной частью конфигурации ролей OIDC является правильная настройка URI перенаправления. Это должно быть сделано как в модуле stronghold, так и в провайдере OIDC, причем эти настройки должны совпадать. URI перенаправления задаются для роли с помощью параметра `allowed_redirect_uris`. Для настройки потоков модуля stronghold UI и CLI существуют различные URI перенаправления, поэтому в зависимости от установки необходимо настроить один или оба.
...
##### Вход в систему OIDC (модуль stronghold UI)
...
Способ аутентификации OIDC успешно протестирован с рядом провайдеров. Полное руководство по настройке приложений OAuth/OIDC не входит в документацию модуля stronghold.

**Устранение неполадок в конфигурации OIDC**

Некоторые советы по настройке OIDC представлены ниже:

* Если параметр роли (например, `bound_claims`) требует значения карты (map), его нельзя установить отдельно с помощью модуля stronghold CLI. В таких случаях запишите конфигурацию в виде одного JSON-объекта:
...
* Убедитесь, что URI перенаправления корректны в модуль stronghold и на провайдере. Они должны точно совпадать. Проверьте: http/https, 127.0.0.1/localhost, номера портов, наличие слэшей в конце.
...
Способ аутентификации для ролей типа "jwt" проще, чем в OIDC, поскольку модуль stronghold требуется только проверить предоставленный JWT.
...
1. Для настройки модуля stronghold используйте конечную точку `/config.` Для поддержки ролей JWT необходимо наличие локальных ключей, URL JWKS или URL OIDC Discovery. Для ролей OIDC необходимо наличие OIDC Discovery URL, OIDC Client ID и OIDC Client Secret.
