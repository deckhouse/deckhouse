...
1. Заходим на мастер-ноду
2. [Подготовить](#как-получить-бинарный-файл-stronghold) инструмент командной строки stronghold для работы
3. Устанавливаем переменные окружения:
  ...
  ```
2. Получаем ключ из Secret `kubectl -n d8-stronghold get secret stronghold-keys -o jsonpath='{.data.unsealKey}' | base64 -d;echo`
3. Выполняем команду `stronghold operator unseal`
4. Вводим unseal ключ.
...
Эта опция подходит, если вы хотите использовать свой самоподписанный Центр сертификации. В качестве примера мы будем использовать уже созданный `ClusterIssuer` ресурс **selfsigned**.
...
1. убедиться в наличии сертификата для домена stronghold.*
    `kubectl -n d8-stronghold get ingress-tls` (либо через Консоль)
...
```
Команда `stronghold` готова к использованию.
