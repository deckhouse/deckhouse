...
В namespaced-ресурсе `Issuer` для ACME, Vault, Venafi и acme-dns допускаются только публичные HTTPS-имена. Частный центр сертификации подключайте через `ClusterIssuer`.
