...
`)
	...
	Context("issuer endpoint ValidatingAdmissionPolicy", func() {
		...
		It("restricts namespaced Issuer ACME URLs and keeps the documented Vault/rfc2136 contracts", func() {
			Expect(f.RenderError).ShouldNot(HaveOccurred())
			...
			Expect(policy.Field("spec.paramKind").Exists()).To(BeFalse())
			...
			Expect(policy.Field("spec.validations").Array()).To(HaveLen(3))
