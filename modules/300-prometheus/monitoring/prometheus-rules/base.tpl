      ...
      annotations:
        ...
        description: |-
          ...
          * `grafana` — by default, all dashboards use aggregating proxy for caching requests to Prometheus. You can retrieve data directly from Prometheus (bypassing the aggregating proxy). However, this may lead to high memory usage by Prometheus and, hence, to unavailability.
          ...
          3. Usually, aggregating-proxy is unavailable due to Prometheus-related issues because the readinessProbe checks the Prometheus availability. Thus, make sure that Prometheus is running: `kubectl -n d8-monitoring describe pod -l app.kubernetes.io/name=prometheus,prometheus=main`.
        summary: >
