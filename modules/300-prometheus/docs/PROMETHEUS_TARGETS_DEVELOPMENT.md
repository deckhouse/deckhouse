    ...
    spec:
      ...
      - name: kube-rbac-proxy
        configMap:
          name: kube-rbac-proxy
```
Экспортер метрик принимает запросы на адресе 127.0.0.1, что ознает, что по незащищенному соединению к нему можно подключиться только изнутри пода.
    ...
    - upstream: http://127.0.0.1:8081/metrics # куда проксируем
      ...
      authorization:
        resourceAttributes:
          ...
          name: my-exporter
```
Согласно конфигурации у пользователя должены быть права на доступ к Deployment с именем `my-exporter` 
и его допонительному ресурсу `prometheus-metrics` в неймспейсе `my-namespace`.
