    ...
    - alert: SentryExporterDown
      annotations:
        description: |-
          ...
          Оповестите команду клиента и клиента.
...
Для достижения аналогичного разультата можно использовать `impact` и `likelihood`:
...
Если инцидент можно описать негативными последствиями и вероятностью их наступления, то лучше применять второй способ — с лейблами `impact` и `likelihood`. Например:
* Отказ одного диска массива RAID5 из четырех дисков, на котором лежит БД.
  ...
  * Скорее всего, второй диск не сломается сразу же, но такой исход возможен, поэтому `likelihood: possible`.
  * Комбинация этих двух лейблов дает уровень S3.

Таблицу соответствия уровней и лейблов можно найти в Polk, нажав на значок критичности любого инцидента.
