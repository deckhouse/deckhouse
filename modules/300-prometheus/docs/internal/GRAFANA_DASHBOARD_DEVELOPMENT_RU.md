...
2. **Основные разрезы** — лейблы, по которым чаще всего анализируют графики:
    - `namespace`
    - `node`
    - `cluster`

3. **Служебные лейблы** (например, `pod`, `container`) редко являются ключевыми и обычно перегружают график.
...
1. **Память (RAM, кеш, буферы, swap) и диск (FS usage, inode usage, volume usage)**  
