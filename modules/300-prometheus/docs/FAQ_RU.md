...
1. Сконфигурируйте Service по аналогии с сервисом [сбора метрик вашего приложения](../../modules/340-monitoring-custom/#пример-service), но без указания параметра `spec.selector`.
1. Создайте Endpoints для этого Service с указанием `IP:PORT`, по которым ваши приложения отдают метрики.
> Имена портов в Endpoints должны совпадать с именами портов в Service.
...
Для обеспечения безопасности рекомендуем использовать `kube-rbac-proxy`.
...
3. Положите получившийся `kubeconfig` на *удаленный сервер*. Необходимо указать путь к `kubeconfig` в настройках `kube-rbac-proxy` (в примере используется путь `${PWD}/.kube/config`).

4. Настройте `node-exporter` на *удаленном сервере*, с доступом только на локальном интерфейсе (слушал `127.0.0.1:9100`).
...
Подробно о всех параметрах можно прочитать в описании custom ресурса [CustomAlertmanager](cr.html#customalertmanager).

## Как добавить внешний дополнительный Alertmanager?

Создайте custom ресурс `CustomAlertmanager` с типом `External`, который может указывать на Alertmanager по FQDN или через сервис в Kubernetes-кластере.
...
* Получатель заведен без параметров.
* Лишние алерты направлены в этого получателя.

Ниже приведены примеры настройки `CustomAlertmanager`:

Чтобы получать только алерты с лейблами `service: foo|bar|baz`, выполните:
...
Чтобы получать все алерты, кроме `DeadMansSwitch`, выполните:
...
Краткое описание:
Разные scrapeInterval принесут следующие проблемы:
...
> Наиболее подходящее значение параметра scrapeInterval, как правило, лежит в интервале от 10 до 60 секунд.

## Как ограничить потребление ресурсов Prometheus?

Чтобы предотвратить ситуации, когда VPA (Variable Policy Agent) запрашивает у Prometheus или долгосрочного Prometheus больше ресурсов, чем доступно на выделенном узле для этих целей, можно явно установить ограничения для VPA с использованием [параметров модуля](configuration.html):
- `vpa.longtermMaxCPU`;
...
После развертывания ресурсов, метрики Prometheus станут доступны по адресу `lens-proxy/prometheus-lens-proxy:8080`.
...
1. Для увеличения размера отредактируйте `PersistentVolumeClaim`, указав новый размер в поле `spec.resources.requests.storage`. Увеличение размера возможно, если в StorageClass поле `allowVolumeExpansion` установлено в `true`.
...
Информацию об активных алертах можно получить не только в веб-интерфейсе Grafana/Prometheus, но и в CLI. Это полезно, если присутствует доступ к API-серверу кластера и нет возможности открыть веб-интерфейс Grafana/Prometheus.
...
> Присутствие специального алерта `DeadMansSwitch` в кластере говорит о работоспособности Prometheus.
