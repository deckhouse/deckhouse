...
1. Создайте `ServiceAccount` с указанными ниже правами:
  ...
  >**Примечание:** используем встроенную в deckhouse `ClusterRole` - `d8:rbac-proxy`
...
2. Создайте конфигурацию для `kube-rbac-proxy`:
  ...
  >**Примечание:** более подробную информацию по атрибутам можной найти [тут](https://kubernetes.io/docs/reference/access-authn-authz/authorization)

3. Создайте `Service` и `Deployment` для вашего приложения, где `kube-rbac-proxy` займёт позицию sidecar-контейнера:
...
После шага 4 метрики вашего приложения должны появится в Prometheus.
