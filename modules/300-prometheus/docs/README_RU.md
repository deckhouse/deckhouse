...
- В комплекте — готовые триггеры и дашборды, поддерживается push и pull-модель сбора метрик. Нагрузка оптимизирована за счет использования кешей и [Deckhouse Prom++](/products/prompp/). Есть возможность хранить исторические данные с помощью даунсемплинга.
...
Возможности системы мониторинга DKP могут быть расширены за счёт использования [модуля Observability](/modules/observability/stable/). С его помощью реализуется:
