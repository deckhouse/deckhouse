...
func updateAmStatus(input *go_hook.HookInput) error {
	...
	for _, am := range addressDeclaredAlertmanagers {
		input.PatchCollector.Filter(set_cr_statuses.SetProcessedStatus(applyAlertmanagerCRDFilter), "deckhouse.io/v1alpha1", "customalertmanager", "", am.Name, object_patch.FilterWithSubresource("/status"), object_patch.FilterWithIgnoreHookError(true))
	}
	...
	for _, am := range serviceDeclaredAlertmanagers {
		...
		input.PatchCollector.Filter(set_cr_statuses.SetProcessedStatus(applyAlertmanagerCRDFilter), "deckhouse.io/v1alpha1", "customalertmanager", "", am.ResourceName, object_patch.FilterWithSubresource("/status"), object_patch.FilterWithIgnoreHookError(true))
	}

	for _, am := range internalDeclaredAlertmanagers {
		name := am["name"].(string)
		input.PatchCollector.Filter(set_cr_statuses.SetProcessedStatus(applyAlertmanagerCRDFilter), "deckhouse.io/v1alpha1", "customalertmanager", "", name, object_patch.FilterWithSubresource("/status"), object_patch.FilterWithIgnoreHookError(true))
	}
