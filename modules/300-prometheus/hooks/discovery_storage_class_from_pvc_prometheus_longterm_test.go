...
var _ = Describe("Modules :: prometheus :: hooks :: discovery_storage_class_from_pvc_longterm ::", func() {
	const (
		initValuesString       = `{"prometheus":{"internal": {"prometheusLongterm": {}}}}`
...
`
	...
	Context("cluster with storage class and pvc", func() {
		...
		It("currentStorageClass must be gp2", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(f.ValuesGet("prometheus.internal.prometheusLongterm.currentStorageClass").String()).To(Equal("gp2"))
		})
	})
