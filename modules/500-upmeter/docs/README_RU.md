...
# Сумма статусов в точке вседа равна 5 минутам
sum by (status) (status_time{group="synthectic", probe="dns"})   # = 300000
```

> **Пример.** Возьмем для простоты только статусы *up* и *down*. В пяти минутах 300'000 миллисекунд, в этих единицах приходит значение. Допустим, в точке метрики пришли значения с лейблами
> >
> ```
> 100000 status=down
> 200000 status=up
...
Апметр оценивает время доступности с участием статуса *Unknown*.

    availability = (up+unknown)/(down+up+unknown)

Статус *Unknown* бывает по двум причинам: штатной недоступности компонента и из-за простоя.

Компонент может быть штатном, но не рабочем состоянии. Например, если обновился релиз Декхаус и его Pod перезапустился. Пока Декхаус инициализирует модули, Pod не в состоянии Ready. Обновление Декхаус может занять несколько минут. В этот период Апметр не считает Декхаус недоступным, если это время не превысило порог.

Когда не работает кластерный API-сервер, проба `control-plane/apiserver` будет в состоянии *Down*, поэтому и вся группа `control-plane` будет иметь статус *Down*. Статус *Unknown* получат те проверки, которые проверяют состояние кластерных ресурсов. Это не говорит о том, доступна или нет проверяемая функциональность. Однако недоступность группы `control-plane` будет зафиксирована в любом случае.
