...
Агенты апметра отправляют статистику доступности за 30 секунд на сервер. Интервалы накоплений определяются по системным часам с границами, кратными 30 секундам текущего времени. 30 секунд — это оперативный интервал. Статистика для групп вычисляется в агенте и отправляется на сервер совместно со статистикой для проб.

Сервер комбинирует статистику от нескольких агентов по принципу самой удачной в интервал 5 минут. 5 минут — это интервал хранения данных и отправки метрик.
