...
func (s *Sender) SendEpisodes(now int64) {
	...
	if err != nil {
		log.Errorf("Fail sending batch of len=%d (%d) to '%s' at %d: %v", len(batchBuf), len(s.Buffer), s.Client.Ip, now, err)
		return
	}
	log.Infof("Send batch of len=%d (%d) to '%s' at %d ts", len(batchBuf), len(s.Buffer), s.Client.Ip, now)
