...
func Test_initVirtualizationTimeouts(t *testing.T) {
	...
	assert.Equal(t, 6*time.Minute, lifecycleConfig.Timeout)
	assert.Equal(t, 30*time.Second, lifecycleConfig.WaitVirtualDiskTimeout)
	assert.Equal(t, 30*time.Second, lifecycleConfig.WaitVirtualMachineTimeout)
	assert.Equal(t, 30*time.Second, lifecycleConfig.WaitVirtualMachineMigrationTimeout)
	assert.Equal(t, 25*time.Second, lifecycleConfig.WaitDeletionTimeout)
	assert.Equal(t, 25*time.Second, lifecycleConfig.WaitNamespaceDeletedTimeout)
}
