...
Модуль имеет возможность включить логирование всех пользовательских соединений, выполненных через VPN, в json-формате. Группировка трафика
происходит по полям `src_ip, dst_ip, src_port, dst_port, ip_proto`. Логи контейнера могут быть собраны и отправлены на хранение или анализ
с помощью модуля [log-shipper](../460-log-shipper/).
