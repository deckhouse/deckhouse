- name: d8.terraform-manager.terraform-state-exporter.availability
  ...
  - alert: D8TerraformStateExporterTargetDown
    expr: max by (job) (up{d8_source="dkp", job="terraform-state-exporter"} == 0)
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8TerraformStateExporterTargetAbsent
    expr: absent(up{d8_source="dkp", job="terraform-state-exporter"}) == 1
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8TerraformStateExporterPodIsNotReady
    expr: |
      min by (pod) (
        kube_controller_pod{d8_source="dkp", namespace="d8-system", controller_type="Deployment", controller_name="terraform-state-exporter"}
        * on (pod) group_right() kube_pod_status_ready{d8_source="dkp", condition="true", namespace="d8-system"}
      ) != 1
    for: 10m
    labels:
      severity_level: "8"
  ...
  - alert: D8TerraformStateExporterPodIsNotRunning
    expr: absent(kube_pod_status_phase{d8_source="dkp", namespace="d8-system",phase="Running",pod=~"terraform-state-exporter-.*"})
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
...
- name: d8.terraform-manager.terraform-state-exporter.checks
  ...
  - alert: D8TerraformStateExporterHasErrors
    expr: |
      increase(candi_converge_exporter_errors{d8_source="dkp", job="terraform-state-exporter"}[5m]) == 3
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8TerraformStateExporterClusterStateChanged
    expr: |
      max by(job, status) (candi_converge_cluster_status{d8_source="dkp", status=~"changed|destructively_changed", job="terraform-state-exporter"} == 1)
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8TerraformStateExporterNodeStateChanged
    expr: |
      max by(node_group, name, status) (candi_converge_node_status{d8_source="dkp", status=~"changed|destructively_changed|absent|abandoned", job="terraform-state-exporter"} == 1)
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
  ...
  - alert: D8TerraformStateExporterClusterStateError
    expr: |
      max by(job) (candi_converge_cluster_status{d8_source="dkp", status="error", job="terraform-state-exporter"} == 1)
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
...
{{- end }}
  ...
  - alert: D8TerraformStateExporterNodeStateError
    expr: |
      max by(node_group, name) (candi_converge_node_status{d8_source="dkp", status="error", job="terraform-state-exporter"} == 1)
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
...
{{- end }}
  ...
  - alert: D8TerraformStateExporterNodeTemplateChanged
    expr: |
      max by(job) (candi_converge_node_template_status{d8_source="dkp", status!="ok", job="terraform-state-exporter"} == 1)
    for: 10m
    labels:
      severity_level: "8"
      tier: cluster
