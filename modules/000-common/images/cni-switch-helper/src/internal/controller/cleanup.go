...
func cleanupFlannel(ctx context.Context) error {
	...
	if err := deleteInterfaces(logger, []string{"flannel.1", "cni0"}); err != nil {
...
func cleanupCilium(ctx context.Context) error {
	...
	// It is baked into the image at /sbin/cilium-dbg
	...
	if err := deleteInterfaces(logger, []string{"cilium_host", "cilium_net", "cilium_vxlan"}); err != nil {
		logger.Error(err, "Failed to delete interfaces")
	}
	...
	// Note: post-uninstall-cleanup handles most of this, but we double check.
...
func cleanupSimpleBridge(ctx context.Context) error {
	...
	// Clean IPAM for bridge
	ipamDir := "/var/lib/cni/networks/cni0"
...
// It uses iptables-save | grep -v ... | iptables-restore approach.
...
func deleteInterfaces(logger logr.Logger, interfaces []string) error {
	for _, iface := range interfaces {
		if err := runCommand(logger, "/sbin/ip", "link", "delete", iface); err != nil {
			...
			logger.V(1).Info("Could not delete interface (likely did not exist)", "interface", iface)
		} else {
