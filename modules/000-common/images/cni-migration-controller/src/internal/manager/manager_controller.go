...
func (r *CNIMigrationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	steps := []struct {
		...
		{
			condition: cnimigrationv1alpha1.ConditionTargetCNIValidated,
			phase:     cnimigrationv1alpha1.PhaseValidatingTargetCNI,
			handler:   r.ensureTargetCNIValidated,
		},
		{
...
func (r *CNIMigrationReconciler) setCondition(ctx context.Context, m *cnimigrationv1alpha1.CNIMigration, condType string, status metav1.ConditionStatus, reason, message string) error {
	...
	for i, c := range m.Status.Conditions {
		if c.Type == condType {
			...
			// Update LTT if Status OR Reason changed.
...
func (r *CNIMigrationReconciler) ensureTargetCNIValidated(_ context.Context, m *cnimigrationv1alpha1.CNIMigration) (bool, string, error) {
	if m.Spec.TargetCNI == "" {
...
func (r *CNIMigrationReconciler) ensureAgentsReady(ctx context.Context, m *cnimigrationv1alpha1.CNIMigration) (bool, string, error) {
	nodeMigrations := &cnimigrationv1alpha1.CNINodeMigrationList{}
	...
	if len(nodeMigrations.Items) < len(nodes.Items) {
		return false, fmt.Sprintf("Waiting for node registrations: %d/%d", len(nodeMigrations.Items), len(nodes.Items)), nil
	}
	...
	if readyAgents < len(nodes.Items) {
		return false, fmt.Sprintf("Waiting for agents to be prepared (pods annotated): %d/%d", readyAgents, len(nodes.Items)), nil
	}
...
func (r *CNIMigrationReconciler) ensureNodesCleaned(ctx context.Context, m *cnimigrationv1alpha1.CNIMigration) (bool, string, error) {
	nodeMigrations := &cnimigrationv1alpha1.CNINodeMigrationList{}
	if err := r.List(ctx, nodeMigrations); err != nil {
		return false, "", err
	}
...
func (r *CNIMigrationReconciler) ensureTargetCNIReady(ctx context.Context, m *cnimigrationv1alpha1.CNIMigration) (bool, string, error) {
	targetCNI := strings.ToLower(m.Spec.TargetCNI)
	...
	if err != nil {
		if errors.IsNotFound(err) {
			return false, "Waiting for target CNI DaemonSet to be created...", nil
		}
		return false, "", err
	}

	if ds.Status.DesiredNumberScheduled == 0 {
		return false, "Waiting for target CNI DaemonSet to schedule pods...", nil
	}

	if ds.Status.NumberReady < ds.Status.DesiredNumberScheduled {
		return false, fmt.Sprintf("Waiting for target CNI pods ready: %d/%d", ds.Status.NumberReady, ds.Status.DesiredNumberScheduled), nil
	}

	return true, "", nil
}

func (r *CNIMigrationReconciler) ensurePodsRestarted(ctx context.Context, _ *cnimigrationv1alpha1.CNIMigration) (bool, string, error) {
	nodeMigrations := &cnimigrationv1alpha1.CNINodeMigrationList{}
	if err := r.List(ctx, nodeMigrations); err != nil {
		return false, "", err
	}
	...
	if restartedNodes < len(nodes.Items) {
		return false, fmt.Sprintf("Waiting for pod restarts: %d/%d nodes completed", restartedNodes, len(nodes.Items)), nil
	}

	return true, "", nil
}

func (r *CNIMigrationReconciler) ensureSucceeded(ctx context.Context, m *cnimigrationv1alpha1.CNIMigration) (bool, string, error) {
