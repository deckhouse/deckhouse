...
func main() {
	...
	flag.StringVar(&migrationName, "migration-name", "", "Name of the CNIMigration resource to process (manager mode only)")
	...
	if mode == "agent" {
		...
		mgr, err := ctrl.NewManager(config, ctrl.Options{
			...
			// This prevents the agent from caching all pods in the cluster, avoiding OOM and network storms.
			Cache: cache.Options{
