...
`
	...
	Context("Cluster has cni secret, key `cni` eq `simple-bridge`, MC exists but explicitly disabled", func() {
		BeforeEach(func() {
			...
			resources := []string{
				cniSecretYAML(cni, ``, nil, nil),
				cniMCYAML(cniName, false, v1alpha1.SettingsValues{}, nil),
			}
			f.KubeStateSet(strings.Join(resources, "\n---\n"))
			f.BindingContexts.Set(f.GenerateBeforeHelmContext())
			f.RunHook()
	...
	Context("Cluster is not bootstrapped and has cni secret with mismatched configuration", func() {
		BeforeEach(func() {
			...
			resources := []string{
				cniSecretYAML(cni, ``, nil, nil),
				cniMCYAML(cniName, true, v1alpha1.SettingsValues{}, nil),
			}
			f.KubeStateSet(strings.Join(resources, "\n---\n"))
			f.BindingContexts.Set(f.GenerateBeforeHelmContext())
			f.RunHook()
	...
	Context("Cluster is bootstrapped, has cni secret created after MC, so MC takes priority", func() {
		BeforeEach(func() {
			...
			resources := []string{
				...
				cniMCYAML(cniName, true, v1alpha1.SettingsValues{}, &mcTime),
			}
			f.KubeStateSet(strings.Join(resources, "\n---\n"))
			f.BindingContexts.Set(f.GenerateBeforeHelmContext())
			f.RunHook()
