...
ENV APP_PATH_FROM=./modules/038-system-registry/images/nodeservices-manager/app \
    APP_PATH_TO=/deckhouse/modules/038-system-registry/images/nodeservices-manager/app \
...
RUN --mount=type=cache,target=/root/.cache/go-build \
    cd ${APP_PATH_TO} && \
    GOOS=${TARGETOS} GOARCH=${TARGETARCH} CGO_ENABLED=0 go build -tags "${BUILD_TAGS}" -o /manager ./cmd/staticpod && \
...
ENV APP_PATH_FROM=./modules/038-system-registry/images/nodeservices-manager
...
# Usage example kubectl port-forward pod/registry-nodeservices-manager-jzw4r 9876:9876 -n d8-system
ENTRYPOINT ["/manager"]
