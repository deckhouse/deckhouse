...
* `EgressGateway` — описывает группу узлов, которые осуществляют функцию egress-шлюза в режиме горячего резерва:
  * Среди группы узлов, попадающих под `spec.nodeSelector`, будут выявлены пригодные к работе и один из них будет назначен активным - [в алфавитном порядке](https://docs.cilium.io/en/latest/network/egress-gateway/egress-gateway/index.html#selecting-and-configuring-the-gateway-node). Признаки пригодного узла:
...
Снимите метку (label) с узла, чтобы исключить его из списка кандидатов для роли egress-шлюза:

```bash
kubectl label node <node-name> <egress-label>-
```
(где egress-label — это метка, указанная в spec.nodeSelector вашего EgressGateway)
...
Трафик продолжит маршрутизироваться через новый шлюз без прерывания.

После завершения работ верните узел в строй:
```bash
...
Повторное добавление метки может привести к тому, что узел снова станет активным (если он первый в алфавитном порядке среди кандидатов).
Чтобы избежать немедленного переключения, временно уменьшите количество реплик EgressGateway или настройте приоритеты через дополнительные метки.
