...
1. `HostPort` pods only bind to [one IP address](https://github.com/deckhouse/deckhouse/issues/3035). If the OS has multiple interfaces/IP, Cilium will choose one, preferring `private` to `public`.
1. In [`masqueradeMode: BPF`](/modules/cni-cilium/latest/configuration.html#parameters-masquerademode), traffic from the PodNetwork to a node's `ExternalIP` does not work if that IP address is not assigned to any network interface on the node but is provided by the underlying infrastructure (for example, via 1:1 NAT). As a result, pods that do not use `hostNetwork` cannot access a port published with `hostPort` through the node's `ExternalIP`; use the node's `InternalIP` instead. To work around this limitation, use [`masqueradeMode: Netfilter`](/modules/cni-cilium/latest/configuration.html#parameters-masquerademode).
...
1. Kernel requirements:
   * Linux kernel version not lower than `5.8` for the `cni-cilium` module to work and work together with the [istio](/modules/istio/), [openvpn](/modules/openvpn/) or [node-local-dns]({% if site.d8Revision == 'CE' %}{{ site.urls.ru}}/modules/{% else %}..{% endif %}/node-local-dns/) modules.
1. OS compatibility:
    * Ubuntu:
