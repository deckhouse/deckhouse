...
1. Для обеспечения стабильной работы cni-cilium на узлах кластера необходимо деактивировать или ограничить функциональность Elastic Agent. Этот агент включает в себя компонент Elastic Endpoint, который взаимодействует с eBPF (Extended Berkeley Packet Filter) на узлах кластера. Elastic Endpoint может удалять программы eBPF, необходимые для корректной работы cni-cilium. Для получения более детальной информации рекомендуется ознакомиться с соответствующими публикациями проектов [Cilium](https://github.com/cilium/cilium/issues/28433) и [Elastic](https://discuss.elastic.co/t/network-disruption-on-kubernetes-node-with-elastic-security-integration-on-debian/354202).
1. Требования к ядру:
