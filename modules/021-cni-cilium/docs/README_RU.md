...
Если Cilium работает в режиме [VXLAN](configuration.html#parameters-tunnelmode) (единственный доступный в AWS), то трафик изнутри кластера (с узла или из пода) не доходит до балансировщика (Load Balancer) типа `internal`, хотя снаружи кластера тот же адрес отвечает. Причина - включенная по умолчанию опция `preserve_client_ip` («Preserve client IP addresses») на целевой группе (Target Group): балансировщик не делает NAT, обратный трафик уходит напрямую через VXLAN-туннель мимо балансировщика, трафик становится асимметричным, и соединение не устанавливается. Чтобы устранить проблему, отключите `preserve_client_ip` для целевой группы одним из двух способов.
