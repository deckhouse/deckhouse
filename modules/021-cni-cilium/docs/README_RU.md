...
3. Сервисам с типом `NodePort` и `LoadBalancer` не подходят эндпоинты в режиме `DSR`, работающие с hostNetwork. Если это необходимо, переключитесь на режим `SNAT`.
4. Поды `HostPort` связываются только [c одним IP](https://github.com/deckhouse/deckhouse/issues/3035). Если операционная система имеет несколько интерфейсов/IP адресов, Cilium выберет один из них и предпочтет использовать «серые» IP адреса («серые» IP - это IP адреса, которые не видны из интернета, в отличие от «белых»).
5. Требования к ядру:
