...
В определенной конфигурации трафик изнутри кластера может не доходить до балансировщика AWS. При этом снаружи кластера балансировщик остается доступен.

Проблема проявляется только при одновременном совпадении трех условий:

* Cilium работает в режиме [VXLAN](configuration.html#parameters-tunnelmode) (в облаке AWS это единственный доступный режим);
* балансировщик имеет тип `internal` (задается на стороне AWS);
* обращение к балансировщику идет изнутри кластера (с узла или из пода).

Пример зависшего запроса с узла кластера:
...
По умолчанию для целевой группы (Target Group) балансировщика AWS включена опция `preserve_client_ip` (в веб-интерфейсе — «Preserve client IP addresses»). В этом режиме балансировщик не выполняет NAT-преобразование и передает трафик, сохраняя исходный IP-адрес клиента.

Когда запрос идет изнутри кластера, входящий пакет проходит через балансировщик на целевой узел с сохранением исходного IP-адреса клиента. Обратный трафик целевой узел отправляет через VXLAN-туннель напрямую исходному узлу, минуя балансировщик. В итоге исходный узел получает ответ с адреса целевого узла, а не с адреса балансировщика. Трафик становится асимметричным. Соединение не устанавливается, и запрос к балансировщику изнутри кластера обрывается.

### Решение

Чтобы балансировщик выполнял NAT-преобразование, отключите опцию сохранения клиентского IP-адреса (`preserve_client_ip`) для целевой группы. Сделать это можно двумя способами.

{% alert level="warning" %}
После отключения `preserve_client_ip` балансировщик выполняет SNAT, и приложение перестает видеть реальный IP-адрес клиента на сетевом уровне. Для HTTP/HTTPS-трафика исходный IP-адрес клиента все равно передается в заголовке `X-Forwarded-For`, поэтому приложения, читающие этот заголовок, продолжат получать адрес клиента.
{% endalert %}

#### Способ 1. Через веб-интерфейс AWS

1. Откройте консоль AWS и перейдите в раздел **EC2** → **Target groups**.
2. Выберите целевую группу, связанную с балансировщиком кластера (например, `k8s-d8ingres-internal-...`).
3. Перейдите на вкладку с атрибутами группы и нажмите **Edit target group attributes**.
4. В блоке **Traffic configuration** отключите переключатель **Preserve client IP addresses**.
5. Сохраните изменения.

  ![Настройка Preserve client IP addresses в веб-интерфейсе AWS](images/preserve-client-ip-disabled.png)

#### Способ 2. Через CLI-утилиту AWS

Выполните следующую команду, подставив свои значения:
...
* `REGION` — регион AWS, в котором развернут кластер (например, `eu-central-1`). Указывается как в ARN целевой группы, так и в параметре `--region`.
* `ACCOUNT` — идентификатор аккаунта AWS (AWS Account ID).
* `NAME` — имя целевой группы (Target Group).
* `ID` — идентификатор целевой группы.

{% alert level="info" %}
Полный ARN целевой группы можно посмотреть в веб-интерфейсе AWS (**EC2** → **Target groups** → выбранная группа) или получить командой `aws elbv2 describe-target-groups`.
{% endalert %}
