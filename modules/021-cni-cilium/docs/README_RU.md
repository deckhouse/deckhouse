...
1. В режиме `masqueradeMode: BPF` трафик из PodNetwork к `ExternalIP` узла не проходит, если этот IP-адрес не назначен ни одному сетевому интерфейсу узла, а предоставляется внешней инфраструктурой, например через NAT 1:1. Поэтому поды, не использующие `hostNetwork`, не смогут обратиться через `ExternalIP` узла к порту, опубликованному с помощью `hostPort` — для такого подключения следует использовать `InternalIP` узла. Чтобы обойти это ограничение, используйте `masqueradeMode: Netfilter`.
