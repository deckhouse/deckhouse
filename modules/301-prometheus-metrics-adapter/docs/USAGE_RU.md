...
Далее рассмотрим только HPA (Horizontal Pod Autoscaling) с [apiVersion: autoscaling/v2](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.23/#objectmetricsource-v2-autoscaling), чья поддержка появилась начиная с Kubernetes v1.12.
...
* Ограничение скорости прироста новых реплик с помощью `spec.behavior.scaleUp.policies`.

## Какой тип масштабирования мне подойдет?

1. Пример [классического типа](#классическое-масштабирование-по-потреблению-ресурсов).
1. Если у вас одно приложение, источник метрик находится внутри namespace и связан с одним из объектов, используйте [кастомные](#масштабирование-по-кастомным-метрикам) Namespace-scoped-метрики.
1. Если у вас много приложений используют одинаковую метрику, источник которой находится в namespace приложения, и она связана с одним из объектов, используйте [кастомные](#масштабирование-по-кастомным-метрикам) Cluster-wide-метрики. Подобные метрики предусмотрены на случай необходимости выделения общих инфраструктурных компонентов в отдельный деплой («infra»).
...
`Pods` — из ресурса, которым управляет HPA, будут выбраны все поды и для каждого пода будут собраны метрики со соответствующими ярлыками (`namespace=XXX`, `pod=YYY-sadiq`, `namespace=XXX`, `pod=YYY-e3adf`, и т. д.). Из этих показателей HPA рассчитает среднее значение и использует его для масштабирования, [например...](#примеры-с-использованием-кастомных-метрик-типа-pods).
...
> Для подключения к SQS необходимо установить экспортер. Для этого следует создать отдельный “служебный” репозиторий Git (или, к примеру, использовать "инфраструктурный" репозиторий), и разместить в нем инсталляцию этого экспортера, а также сценарий для создания требуемого `CustomPrometheusRules`, таким образом объединив кластер. Если необходимо настроить автомасштабирование только для одного приложения (в одном пространстве имен), лучше ставить экспортер вместе с этим приложением и воспользоваться `NamespaceMetrics`.

В представленом примере подразумевается, что в Amazon SQS работает очередь `send_forum_message`. Если количество сообщений в этой очереди превышает 42, выполняется масштабирование. Для получения метрик из Amazon SQS понадобится exporter (например, [sqs-exporter](https://github.com/ashiddo11/sqs-exporter)).
