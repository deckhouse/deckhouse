...
init-контейнеров и ephemeral-контейнеров пода.
...
Лейбл неймспейса читается из кэша Gatekeeper. Пока неймспейс отсутствует в этом кэше, например сразу
после перезапуска подов Gatekeeper, политика запрещает поды, запрашивающие GPU-ресурсы в этом неймспейсе.
