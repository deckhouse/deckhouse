...
### Типовые сценарии
    ...
    1. Динамический выбор по лейблам (`labelSelector + excludeNames`)
    ...
    1. Гибкая фильтрация по выражениям (`labelSelector.matchExpressions`)
    ...
    1. Комбинация `matchNames` и `labelSelector` (пересечение, использовать осторожно)
