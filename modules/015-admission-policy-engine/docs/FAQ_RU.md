...
## Есть ли в DKP шаблоны для создания своих политик?

В DKP есть готовые шаблоны для Gatekeeper, которые можно использовать и для своих политик.
Для просмотра доступных шаблонов используйте команду:

```shell
d8 k get constrainttemplates.templates.gatekeeper.sh
```

Пример ответа со списком шаблонов:
