...
запрещает рабочие нагрузки, запрашивающие GPU-ресурсы, если на неймспейсе не установлен лейбл,
разрешающий использование GPU. Проверка выполняется при создании и изменении пода, в том числе
при добавлении ephemeral-контейнера, а также при создании и изменении контроллеров, создающих поды.
Проверяются `resources.requests` и `resources.limits` всех контейнеров,
...
{% alert level="warning" %}
Политика применяется к подам и к контроллерам, создающим поды,
поэтому Deployment, в шаблоне пода которого запрашиваются GPU-ресурсы, будет отклонён при создании.
Чтобы не проверять контроллеры, задайте для параметра [`controllerValidation`](configuration.html#parameters-podsecuritystandards-controllervalidation) значение `false`.
