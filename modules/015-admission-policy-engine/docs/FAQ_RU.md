...
В `OperationPolicy` и `SecurityPolicy` поле `spec.match` определяет, на какие именно объекты (поды) в кластере будет распространяться политика. Оно обязательно должно присутствовать в конфигурации. Фильтрация выполняется путём комбинирования двух основных критериев: селектора подов (`labelSelector`) и селектора пространств имён (`namespaceSelector`).

Если указаны оба селектора, политика применяется только к тем подам, которые **одновременно** удовлетворяют условиям выбора подов **и** находятся в пространствах имён, прошедших фильтрацию. Если какой-либо из селекторов опущен, соответствующее проверка не производится (будут использоваться все поды или пространства имён).
...
### `spec.match.namespaceSelector` – выбор пространств имён

Позволяет ограничить пространства имён, в которых действует политика. Можно использовать комбинацию трёх фильтров:

* **`matchNames`** – явный список разрешённых namespace. Если задан, политика действует только в перечисленных пространствах имён.
* **`excludeNames`** – список исключаемых namespace. Политика будет действовать во всех пространствах имён, **кроме** указанных.
* **`labelSelector`** – селектор по лейблам самого объекта Namespace. Синтаксис полностью аналогичен селектору подов (`matchLabels` / `matchExpressions`). Фильтруются именно метаданные пространства имён.
...
Наиболее типичный сценарий – ограничение и по подам, и по пространствам имён одновременно:
...
Эта политика сработает для подов с лейблами `app=payments` и `version=v2`, которые находятся в пространствах имён, имеющих лейбл `env=prod`, за исключением пространства имён `legacy-prod`.
...
Здесь политика охватывает поды, у которых значение лейбла `security` не равно `low`, в пространствах имён `frontend` и `backend`.
