...
## Как разрешить одну или несколько политик Pod Security Standards, не исключая весь список?

1. Добавьте на ваш `namespace` метку `security.deckhouse.io/pod-policy: privileged`, чтобы отключить встроенный набор.
1. Создайте ресурс `SecurityPolicy`, соответствующий политикам baseline либо restricted, при этом вы можете отредактировать список `policies` под ваши нужды.
