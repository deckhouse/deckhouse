...
## Пример: запрет удаления Node без метки

> Примечание. Операции DELETE обрабатываются Gatekeeper по умолчанию.

Можно создать собственную политику Gatekeeper, запрещающую удаление Node без специальной метки. Пример ниже использует `oldObject` для проверки меток удаляемого узла:
