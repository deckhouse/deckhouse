...
Пример 1. Добавление лейбла `owner` со значением `admin` во всех неймспейсах:
...
Пример установки `imagePullPolicy` для всех контейнеров на `Always` во всех неймспейсах, кроме `system`:
