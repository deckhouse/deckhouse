...
Модуль находится на критическом пути кластера: пока его валидирующий вебхук недоступен, API-сервер отклоняет запросы, которые вебхук перехватывает. В этом разделе описано, что зависит от каждого компонента, почему из проверки выведены только поды вебхука и какие алерты сообщают об отказе.
...
### Почему из проверки выведены только поды вебхука

В конфигурации вебхуков есть одно исключение, и оно охватывает только поды `gatekeeper-controller-manager`: каждый вебхук исключает через `objectSelector` объекты с лейблом `gatekeeper.sh/operation: webhook`. Исключение существует, чтобы разорвать циклическую зависимость, а не чтобы ослабить политики для модуля.

Вебхук, который проверяет обслуживающие его поды, не способен восстановиться после собственного отказа. Как только исчезает последняя реплика, API-серверу некуда доставить запрос, и он отклоняет создание замещающего пода: деплоймент остаётся с нулём реплик, пока оператор не удалит конфигурацию вебхуков вручную. Лейбл, который Gatekeeper проставляет собственным подам, — минимальное исключение, разрывающее этот цикл: деплоймент всегда может создать под, а все остальные объекты неймспейса проверяются как обычно.

Поды `gatekeeper-audit` несут лейбл `gatekeeper.sh/operation: audit` и намеренно не исключены. Admission от аудита не зависит, поэтому аудит не находится на пути восстановления, и исключение расширило бы брешь, не давая ничего взамен. Плата за это — пока вебхук недоступен, деплоймент аудита тоже не может создать под, и аудит возвращается только вслед за вебхуком.

Лейбл не является способом обойти политики. Три политики типа ValidatingAdmissionPolicy отклоняют объект с лейблом `gatekeeper.sh/operation: webhook`, если запрос пришёл не от сервисного аккаунта неймспейса `d8-*` или `kube-*` и не от `system:sudouser`: `deny-gatekeeper-webhook-operation-label` — для подов, `deny-gatekeeper-webhook-operation-label-controllers` — для создающих их контроллеров и `deny-gatekeeper-webhook-operation-label-controllers-cronjobs` — для CronJob. Пользователю, который проставит лейбл своему поду, чтобы пропустить проверку, откажут ещё до обращения к вебхуку, а контроллеру ReplicaSet платформы — нет, и именно это оставляет открытым путь восстановления. Эти же политики запрещают всем остальным перезапуск деплоймента командой `kubectl rollout restart`, поскольку перезапуск переписывает шаблон пода с этим лейблом.

Кроме того, Gatekeeper запускается с параметром `--exempt-namespace=d8-admission-policy-engine`, который допускает объекты собственного неймспейса модуля без проверки. Это исключение применяет сам Gatekeeper, поэтому оно действует только при запущенном Gatekeeper и во время отказа не помогает. Восстановление опирается только на лейбл.
