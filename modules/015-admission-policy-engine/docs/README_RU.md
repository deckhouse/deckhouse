...
Каждая политика хранит сводку своих нарушений в собственном статусе, поэтому состояние политики можно посмотреть, не выходя за пределы кластера. Модуль заполняет раздел `status.violations` ресурсов `SecurityPolicy` и `OperationPolicy` один раз за цикл аудита.

В разделе находятся агрегированные счётчики и короткая выборка, а не полный список нарушений. Статус лежит в одном объекте со спецификацией политики, и каждая запись сохраняет в хранилище целую новую ревизию этого объекта, поэтому полный список пришлось бы дорого перезаписывать в каждом цикле аудита. Полный список остаётся в метрике `d8_gatekeeper_exporter_constraint_violations`.

Чтобы узнать количество нарушений каждой политики, выведите список политик:

```bash
d8 k get securitypolicy
```

Колонка `Violations` показывает количество нарушений, зафиксированных последним циклом аудита:
...
Чтобы узнать, где находятся нарушения, прочитайте статус политики:
...
- `total` — общее количество нарушений; счётчик остаётся точным, даже если списки ниже сокращены;
- `truncated` — количество нарушений, которые аудит не зафиксировал, поскольку их число превысило его лимит;
- `byEnforcement` — количество нарушений в разбивке по действию, которое к ним применяется;
- `topNamespaces` — до 10 неймспейсов с наибольшим количеством нарушений;
- `sample` — до 10 нарушений для предварительного просмотра;
- `lastUpdateTime` — время последнего изменения сводки модулем.

{% alert level="info" %}
Модуль перезаписывает раздел только при изменении его содержимого. Пока набор нарушений остаётся прежним, значение `lastUpdateTime` не меняется, а политика не перезаписывается.
{% endalert %}
...
### Стандарты как объекты SecurityPolicy

Каждому стандарту в кластере соответствует собственный объект `SecurityPolicy` — `d8-pod-security-baseline` и `d8-pod-security-restricted`. Объекты показывают правила стандарта рядом с политиками, которые написал оператор кластера, и хранят нарушения стандарта в своём статусе.

Объекты принадлежат модулю, и он восстанавливает их при следующей сходимости, поэтому их изменение ни на что не влияет. Состав проверок стандарта задаёт модуль, а неймспейсы, на которые стандарт распространяется, определяются лейблами, описанными выше.

Объекты не описывают два случая:

- неймспейсы, которыми владеют модули Deckhouse: они проверяются отдельно и только тогда, когда владеющий неймспейсом модуль объявил его готовым к проверке;
- неймспейс с лейблом `security.deckhouse.io/pod-policy-action`: он проверяется с собственным действием, а не с действием из `spec.enforcementAction` объекта. Счётчики `byEnforcement` в статусе показывают, сколько нарушений пришлось на каждое действие.
