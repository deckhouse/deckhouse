...
Политики валидируют контроллеры, создающие поды, при операциях CREATE и UPDATE. Это обеспечивает ранний отклик при создании или обновлении рабочей нагрузки, до запуска подов, а также отображение запретов создания, при развертывании с помощью CI (в CI редко создаются поды, а ошибки создания подов от контроллеров чаще всего не отображаются).
...
{% alert level="warning" %}
Проверки на уровне контроллеров проходят по **исходному шаблону пода** (`spec.template`), а не по манифесту после всех мутаций. Тогда как проверка на уровне подов происходит после всех мутирующих действий (например, LimitRange добавляет `resources` по умолчанию, или мутирующий webhook добавляет `securityContext`);
Учитывайте это при использовании политик безопасности.
{% endalert %}

### Семантика labelSelector для контроллеров

При формировании политик, которые применяются на основе лейблов объектов (`match.labelSelector`) следует учитывать, что отбор происходит по `metadata.labels` проверяемого объекта. Для подов это лейблы пода. Для контроллеров (Deployment, StatefulSet и т.д.) это **верхнеуровневые** `metadata.labels` контроллера, а не `metadata.labels` шаблона пода (`spec.template.metadata.labels`).
Пользователям, создающих политики применяемые по лейблам следует учитывать, что:

- Если под labelSelector политики попадает создаваемые контроллером поды, то это не означает автоматическую проверку на уровне контроллера, если под labelSelector не подпадает сам контроллер.
- Исключающий селектор (`NotIn`/`DoesNotExist`), используемый для исключения из политики подов, может не сработать  на уровне контроллера, если под labelSelector не попадает сам контроллер.
...
### Мягкий режим для полей, добавляемых Kubernetes

При включённой проверке на уровне контроллеров некоторые ограничения используют **мягкий режим** (lenient mode) для полей, которые Kubernetes admission-контроллеры (LimitRange, PodSecurity, ServiceAccount admission) могут добавить при создании Pod. Это предотвращает ложные срабатывания, когда в шаблоне пода контроллера отсутствует поле, которое будет добавлено автоматически до создания Pod.

Следующие ограничения пропускают нарушения для отсутствующих полей при проверке контроллеров (но по-прежнему блокируют явно заданные недопустимые значения):

| Ограничение | Пропускаемые поля (при отсутствии в контроллере) | Компонент Kubernetes |
|---|---|---|
| `D8RequiredResources` | `container.resources` (при полном отсутствии) | LimitRange |
| `D8AllowedUsers` | `runAsUser`, `runAsGroup`, `fsGroup`, `supplementalGroups` (с правилами `MustRunAs` / `MustRunAsNonRoot`) | PodSecurity admission |
| `D8AllowedSeccompProfiles` | `seccompProfile.type` (при отсутствии во всех источниках) | PodSecurity admission |
