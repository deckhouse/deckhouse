...
- В кластерах, инициализированных с версией Deckhouse ниже v1.55, в качестве политики по-умолчанию для всех несистемных пространств имен используется `Privilged`;
- В кластерах, инициализированных с версией Deckhouse равной или выше v1.55, в качестве политики по-умолчанию для всех несистемных пространств имен используется `Baseline`;
- Обновление кластера на версию v1.55 не вызывает автоматической смены политики по-умолчанию.

Политики по-умолчанию можно переопределить как глобально, с помощью [configuration](configuration.html#parameters-podsecuritystandards-defaultProfile), так и для каждого пространства имен отдельно. Для точечного применения политики достаточно установить лейбл `security.deckhouse.io/pod-policy=<POLICY_NAME>` на соответствующее пространство имен.
...
По умолчанию, политики Pod Security Standards применяются в режиме "Deny" и поды приложений, не удовлетворяющие данным политикам, не смогут быть запущены. Режим работы политик может быть задан как глобально для кластера так и для каждого namespace отдельно. Что бы задать режим работы политик глобально используйте [configuration](configuration.html#parameters-podsecuritystandards-enforcementaction). В случае если необходимо переопределить глобальный режим политик для определенного namespace, допускается использовать лейбл `security.deckhouse.io/pod-policy-acion =<POLICY_ACTION>` на соответствующем namespace. Список допустимых режимом политик состоит из: "dryrun", "warn", "deny".
