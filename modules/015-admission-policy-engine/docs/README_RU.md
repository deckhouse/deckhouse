...
- Если поды создаются через Deployment, при этом поды потенциально нарушают политику, то создание такого Deployment будет отклонено.
- Если поды создаются через Deployment, который уже существовал до включения политики, то создаётся требуемое количество ReplicaSet, которые, в свою очередь, пытаются создать поды. В этом случае ошибка валидации не возвращается в ответе API, а отображается в событиях неймспейса или соответствующего ReplicaSet.

## Проверка на уровне контроллеров

Политики валидируют контроллеры, создающие поды, при операциях `CREATE` и `UPDATE`. Это обеспечивает ранний отклик при создании или обновлении рабочей нагрузки, до запуска подов, а также отображение запретов создания при развёртывании с помощью CI (в CI редко создаются поды, а ошибки создания подов от контроллеров чаще всего не отображаются).

Валидируются операции со следующими объектами:

| API-группа | Тип ресурса                        | Операции                     |
| ---------- | ---------------------------------- |------------------------------|
...
Операция `DELETE` перехватывается, чтобы политика могла запретить удаление ресурса:
правило, которое проверяет `input.review.operation`, выполняется только если запрос доходит до Gatekeeper.

{% alert level="warning" %}
Вебхук работает с `failurePolicy: Fail`.
Пока Gatekeeper недоступен, для перечисленных выше типов ресурсов блокируется и создание, и удаление.
Чтобы вернуть возможность работы с ресурсами, восстановите деплой `gatekeeper-controller-manager`
или удалите ValidatingWebhookConfiguration `d8-admission-policy-engine-config`.
{% endalert %}

{% alert level="warning" %}
Проверки на уровне контроллеров выполняются по **исходному шаблону пода** (`spec.template`),
а не по манифесту, полученному после всех мутаций.
Проверка на уровне подов, напротив, выполняется после всех мутирующих действий — например,
после того как LimitRange добавит `resources` по умолчанию, а мутирующий вебхук — `securityContext`.
Учитывайте это при использовании политик безопасности.
{% endalert %}

### Семантика labelSelector для контроллеров

При формировании политик, которые применяются на основе лейблов объектов (`match.labelSelector`),
учитывайте, что отбор происходит по `metadata.labels` проверяемого объекта.
Для подов это лейблы пода.
Для контроллеров (Deployment, StatefulSet и других) это **верхнеуровневые** `metadata.labels` контроллера,
а не `metadata.labels` шаблона пода (`spec.template.metadata.labels`).

Поэтому селектор, который отбирает поды, создаваемые контроллером, сам по себе не приводит к проверке
на уровне контроллера, а исключающий селектор (`NotIn`/`DoesNotExist`), исключающий такие поды из политики,
может не сработать на уровне контроллера.
В обоих случаях сам контроллер должен попадать под `labelSelector`.

То же относится к лейблу `security.deckhouse.io/skip-pss-check`, который исключает объект из ограничений Pod Security Standards.
Лейбл читается из `metadata.labels` проверяемого объекта, поэтому рабочая нагрузка, у которой он задан только в шаблоне пода
(`spec.template.metadata.labels`), исключается на уровне пода, но по-прежнему проверяется на уровне контроллера.
Чтобы исключить такую нагрузку полностью, задайте лейбл и в верхнеуровневых `metadata.labels` контроллера.
В отличие от SecurityPolicyException, разрешить этот лейбл из шаблона пода нельзя: исключение вычисляется
в `match.labelSelector` Gatekeeper, до выполнения кода политики.

### Разрешение лейблов SecurityPolicyException для контроллеров

Для контроллеров лейблы и аннотации SecurityPolicyException (SPE) берутся из **метаданных шаблона пода** (`spec.template.metadata`), а не из верхнеуровневых метаданных контроллера. Это гарантирует, что SPE применяется к подам, создаваемым контроллером, а не к самому объекту контроллера.

### Отключение проверки на уровне контроллеров

Проверку на уровне контроллеров можно отключить с помощью параметра [`controllerValidation`](configuration.html#parameters-podsecuritystandards-controllervalidation) в настройках модуля.
Несмотря на то что параметр находится в секции `podSecurityStandards`, он действует на все ограничения, которые читают спецификацию пода,
включая созданные из ресурсов OperationPolicy и SecurityPolicy.

При `controllerValidation: false` ограничения применяются только к подам. В этом случае:

- контроллеры (Deployment, StatefulSet, DaemonSet, Job, CronJob, ReplicationController) не проверяются при создании и обновлении;
- лейблы SecurityPolicyException на `spec.template.metadata.labels` контроллеров не разрешаются, так как ограничения не применяются к контроллерам;
- поды по-прежнему проверяются при запуске, как и при `true`.

### Мягкий режим для полей, которые могут быть заданы до создания пода

Некоторые ограничения используют **мягкий режим** (lenient mode) для полей, которые могут отсутствовать в шаблоне пода контроллера, но присутствовать в поде, создаваемом этим контроллером. Это не даёт отклонить нагрузку, поды которой в итоге соответствуют политике.

Следующие ограничения пропускают нарушения для отсутствующих полей при проверке контроллеров, но по-прежнему блокируют явно заданные недопустимые значения. Решение принимается для каждого контейнера отдельно: контейнер без поля обрабатывается мягко, даже если другой контейнер того же шаблона пода это поле задал.

| Ограничение | Пропускаемые поля (при отсутствии в контроллере) | Что может задать поле                                      |
|---|---|------------------------------------------------------------|
| `D8RequiredResources` | `container.resources` (когда не заданы ни `limits`, ни `requests`) | LimitRange                                                 |
| `D8AllowedUsers` | `runAsUser`, `runAsGroup`, `fsGroup`, `supplementalGroups` (с правилами `MustRunAs` / `MustRunAsNonRoot`) | Мутирующий вебхук, в том числе мутатор `Assign` Gatekeeper |
...
Некоторые проверки опираются на данные, которые появляются только после мутаций, поэтому не могут отклонить контроллер, хотя отклоняют его поды.

`D8AutomountServiceAccountToken` определяет монтирование токена либо по `spec.automountServiceAccountToken: true`, либо по контейнеру, который монтирует `/var/run/secrets/kubernetes.io/serviceaccount`. Плагин ServiceAccount admission добавляет это монтирование в под, а не в шаблон пода контроллера. Поэтому нагрузка, в которой `automountServiceAccountToken` просто не задан, проходит проверку на уровне контроллера, а отказ появляется при создании подов. Чтобы получить отказ на уровне контроллера, задайте в шаблоне пода `automountServiceAccountToken: false` явно.
...
## Доступность компонентов модуля

Модуль находится на критическом пути кластера: пока его валидирующий вебхук недоступен, API-сервер отклоняет запросы, которые вебхук перехватывает. В этом разделе описано, что зависит от каждого компонента и как компоненты размещаются, чтобы оставаться доступными.

### Почему валидирующий вебхук критичен для кластера

Деплоймент `gatekeeper-controller-manager` обслуживает ValidatingWebhookConfiguration с именем `d8-admission-policy-engine-config`. Его основной вебхук работает с `failurePolicy: Fail`: запрос, который API-сервер не смог доставить вебхуку, отклоняется, а не пропускается.

Это осознанный выбор. Политика, которая пропускается при недоступном вебхуке, политикой не является: объект, нарушающий её, был бы допущен ровно в тот момент, когда проверка отсутствует. Плата за такую гарантию — доступность вебхука становится доступностью admission в кластере.

Пока ни одна реплика `gatekeeper-controller-manager` не доступна, в неймспейсах, которые охватывает вебхук, перестаёт работать следующее:

- Создание и изменение подов и создающих их контроллеров: Deployment, StatefulSet, DaemonSet, ReplicationController, Job и CronJob.
- Удаление этих же ресурсов. Вебхук перехватывает и операцию `DELETE`, поскольку политика должна иметь возможность запретить удаление объекта; отклонённый `DELETE` — это тот же отказ, что и отклонённый `CREATE`.
- Создание и изменение Role, RoleBinding и ограничений Gatekeeper.
- Команды `kubectl exec` и `kubectl attach` в неймспейсах с именами, начинающимися на `d8-` и `kube-`. Их перехватывает отдельный вебхук, тоже с `failurePolicy: Fail`, поэтому отказ сужает возможности его же диагностики.

Мутирующий вебхук настроен иначе: его `failurePolicy` — `Ignore`, и недоступность деплоймента означает лишь то, что мутации не применяются.

### Как модуль сохраняет управление собой

Модуль способен перезапустить собственный вебхук во время отказа, потому что до обращения к Gatekeeper применяются два исключения:

- Поды `gatekeeper-controller-manager` несут лейбл `gatekeeper.sh/operation: webhook`, а каждый вебхук конфигурации исключает объекты с этим лейблом через `objectSelector`.
- Gatekeeper запускается с параметром `--exempt-namespace=d8-admission-policy-engine`, поэтому объекты собственного неймспейса модуля допускаются без проверки.

Второе исключение работает только при запущенном Gatekeeper. Когда он не запущен, запросы для этого неймспейса отклоняются на этапе доставки, и это затрагивает поды `gatekeeper-audit`: они несут лейбл `gatekeeper.sh/operation: audit` и не исключаются через `objectSelector`. Поэтому деплоймент аудита не может создать новый под, пока вебхук не восстановлен.

### Размещение компонентов

Оба деплоймента работают с классом приоритета `system-cluster-critical`, который ставит их выше любой прикладной нагрузки при вытеснении и защищает от выселения при нехватке ресурсов на узле.

В таблице собраны параметры, определяющие, сколько реплик переживёт отказ.

| Параметр | gatekeeper-controller-manager | gatekeeper-audit |
| --- | --- | --- |
| Количество реплик | 2 в режиме HA, иначе 1 | всегда 1 |
| Стратегия обновления | `RollingUpdate` с `maxSurge: 0` и `maxUnavailable: 1` в режиме HA, иначе значения Kubernetes по умолчанию | значения Kubernetes по умолчанию |
| Anti-affinity | обязательный, по `kubernetes.io/hostname`, в режиме HA | — |
| Узлы | обязательный node affinity: узлы модуля, системные узлы, узлы control-plane | node selector: узлы модуля или системные узлы, если такие узлы есть |
| Toleration | любой узел | системные узлы |
| PodDisruptionBudget | `minAvailable: 1` в режиме HA, иначе `minAvailable: 0` | `minAvailable: 0` |
| Класс приоритета | `system-cluster-critical` | `system-cluster-critical` |

Из таблицы следуют два вывода:

- Вне режима HA вебхук работает в одной реплике, а его PodDisruptionBudget разрешает её выселение. Вывод узла с этой репликой из эксплуатации блокирует admission в кластере до тех пор, пока под не будет запущен на другом узле и не станет готов. Чтобы получить вторую реплику на другом узле, включите для кластера режим HA.
- В режиме HA обновление деплоймента оставляет одну обслуживающую реплику, поскольку стратегия сочетает `maxSurge: 0` с `maxUnavailable: 1`. Правило anti-affinity задано как `requiredDuringSchedulingIgnoredDuringExecution`, поэтому в кластере, где подходящих узлов меньше, чем реплик, под остаётся в состоянии `Pending` постоянно.

### Мониторинг компонентов

О доступности компонентов сообщают три алерта:

- `D8AdmissionPolicyEngineWebhookUnavailable`, severity 1, срабатывает, когда у вебхука нет доступных реплик. Пока алерт активен, admission в кластере заблокирован.
- `D8AdmissionPolicyEngineWebhookDegraded`, severity 3, срабатывает, когда недоступна часть реплик. При обновлении деплоймента алерт возникает ненадолго.
- `D8AdmissionPolicyEngineAuditUnavailable`, severity 4, срабатывает, когда у деплоймента аудита нет доступных реплик. Нарушения среди уже работающих объектов не собираются, а алерты о нарушении политик перестают сообщать о проблемах.
