...
Для применения данных политик достаточно будет навесить лейбл `operation-policy.deckhouse.io/enabled: "true"` на желаемый namespace. Указанная политика является общей и рекомендованной командой Deckhouse. Если политики вашей компании отличаются, — вы можете сконфигурировать их по своему
