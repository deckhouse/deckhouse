...
Помимо подов, политики теперь валидируют контроллеры, создающие поды (Deployment, StatefulSet, DaemonSet, Job, CronJob, ReplicationController) при операциях CREATE и UPDATE. Это обеспечивает ранний отклик при создании или обновлении рабочей нагрузки, до запуска подов.

{% alert level="warning" %}
Проверки на уровне контроллеров оценивают **неизменённый шаблон пода** (`spec.template`), а не итоговый под. Под, попадающий в Gatekeeper, уже прошёл мутирующее допущение (например, LimitRange добавляет `resources` по умолчанию, или мутирующий webhook добавляет `securityContext`); шаблон пода в контроллере — нет.
{% endalert %}

Рабочая нагрузка, в шаблоне которой отсутствуют поля, обычно добавляемые LimitRange или мутирующим webhook, может быть **отклонена на уровне контроллера**, хотя итоговые поды будут валидными. Проверяйте политики против шаблонов подов в контроллерах, а не только против запущенных подов.

### Семантика labelSelector для контроллеров

`match.labelSelector` в Gatekeeper оценивается по `metadata.labels` проверяемого объекта. Для подов это лейблы пода. Для контроллеров (Deployment, StatefulSet и т.д.) это **верхнеуровневые** `metadata.labels` контроллера, а не `metadata.labels` шаблона пода (`spec.template.metadata.labels`). Пользователям, указывающим `labelSelector` pensando о лейблах подов, следует учитывать, что:

- Позитивный селектор, не продублированный на верхнем уровне контроллера, приведёт к отсутствию проверки на уровне контроллера.
- Исключающий селектор (`NotIn`/`DoesNotExist`), используемый для освобождения рабочей нагрузки, перестанет освобождать на уровне контроллера — ранее разрешённая рабочая нагрузка может быть отклонена.

### Разрешение лейблов SecurityPolicyException для контроллеров

Для контроллеров лейблы и аннотации SecurityPolicyException (SPE) разрешаются из **метаданных шаблона пода** (`spec.template.metadata`), а не из верхнеуровневых метаданных контроллера. Это гарантирует, что SPE применяется к подам, создаваемым контроллером, а не к самому объекту контроллера.
