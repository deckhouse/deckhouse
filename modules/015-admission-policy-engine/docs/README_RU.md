...
Помимо политик, которые запрещают использование параметров отличных от разрешенных, существуют ресурсы отвечающие за реализацию точечных исключений из  политик - [`security-policy-exceptions`](#security-policy-exceptions). С помощью данных ресурсов можно переопределить проверки для некоторых подов, без изменения всех политик накладываемых на неймспейс.
...
`SecurityPolicyException` — это механизм точечных исключений из проверок политик безопасности для отдельных подов и контейнеров. Он позволяет не исключать весь namespace из проверок, а описывать только необходимые точечные разрешения конкретного правила для Pod или контейнера внутри пода.
...
1. Создаётся объект `SecurityPolicyException` c нужными разрешениями в `spec`.
...
2. На Pod (обычно через `spec.template.metadata.labels` в `Deployment`/`StatefulSet`/`DaemonSet`) ставится ссылка на исключение:
   - `security.deckhouse.io/security-policy-exception: <exception-name>` — исключение для Pod в целом (pod-level);
...
- если container-specific лейбл отсутствует — используется исключение из `security.deckhouse.io/security-policy-exception`.

{% alert level="warning" %}
Если для контейнера задан container-specific лейбл, но он указывает на невалидный/несуществующий объект `SecurityPolicyException`, он всё равно имеет приоритет над global-лейблом и может привести к запрету размещения пода.
{% endalert %}
...
2. Разрешение на  privileged escalation только для контейнера с именем `sample-init`

При классическом подходе, без использования ресурсов `SecurityPolicyException`, для разрешения этих параметров потребовалось бы реализация пользовательской политики безопасности, где данные параметры были бы разрешены любым подом в кластере.   

При использовании `SecurityPolicyException` потребуется создаться следующие ресурсы:
...
А также выставить label для применения ресурсов в создаваемых подах
...
spec:
  template:
    metadata:
      labels:
        security.deckhouse.io/security-policy-exception: allow-hostnetwork-pod                                 # Общее исключение распространяемое на весь под
        security.deckhouse.io/security-policy-exception.container.sample-init: allow-privileged-init-container # Исключения для конкретного контейнера. В имени лейбла после .container. указно имя контейнера на который применяться исключение
    spec:
      hostNetwork: true         
    ...
    containers:
