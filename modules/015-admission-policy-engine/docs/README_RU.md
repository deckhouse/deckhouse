...
Модуль находится на критическом пути кластера: пока его валидирующий вебхук недоступен, API-сервер отклоняет запросы, которые вебхук перехватывает. В этом разделе описано, чем это грозит кластеру и почему из проверки выведены только поды вебхука.
...
Отключить модуль и разблокировать кластер можно, но вместе с модулем пропадает и контроль: политики больше не применяются, и объект, который политика запрещала, создаётся беспрепятственно. Порядок действий при недоступном вебхуке описан в [FAQ](faq.html#что-делать-если-валидирующий-вебхук-недоступен).

### Почему из проверки выведены только поды вебхука

В конфигурации вебхуков есть одно исключение, и оно охватывает только поды `gatekeeper-controller-manager`: вебхуки, перехватывающие создание объектов, исключают через `objectSelector` объекты с лейблом `gatekeeper.sh/operation: webhook`. Исключение существует, чтобы разорвать циклическую зависимость, а не чтобы ослабить политики для модуля. На вебхук, перехватывающий `kubectl exec` и `kubectl attach`, оно не распространяется: у него нет `objectSelector`, поэтому во время отказа exec в поды модуля блокируется наравне со всем остальным.
