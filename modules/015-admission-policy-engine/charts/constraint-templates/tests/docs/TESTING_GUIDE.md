...
Two additional `constraint_testgen` subcommands provide static analysis of constraint templates and Helm charts:

```bash
# Lint: detect Rego anti-patterns that cause false-positive violations
# Catches: object.get(..., "") default-to-empty, fail-open on unknown kind
go run $constraint_testgen lint

# Webhook-scope: detect Helm template issues in webhook/constraint definitions
# Catches: ReplicaSet in match.kinds, duplicated kinds blocks, DELETE on controllers
go run $constraint_testgen webhook-scope
```

Both commands scan the `templates/` directory by default. Use `--templates-root` to override.

#### Lint rules

| Rule | Pattern | Problem |
| ---- | ------- | ------- |
| `object-get-empty-default` | `object.get(x, "field", "")` in a violation context | Absent field becomes `""` — a concrete value that can trigger false violations. Use `has_field()` guard instead. |
| `list-contains-empty` | `list_contains(..., "")` | Undefined in OPA, so `not list_contains(..., "")` is always true. |
| `not-object-get-empty` | `not` combined with `object.get(..., "")` | Silently inverts to true when the field is absent. |
| `fail-open-unknown-kind` | `pod_spec := {}` when `not workload_kind(kind)` | Unknown kind yields empty containers — all container-level checks silently pass. Use `object.get(obj, "spec", {})` instead. |

`object-get-empty-default` accepts one pattern automatically: when the next line
rejects the empty value (`name != ""`), the absent case is handled explicitly.

Every other intentional use needs a directive on the line above, and the reason
after the rule name is mandatory:

```rego
# lint:allow=object-get-empty-default — "" is the value being tested for: a CONNECT without a subresource.
object.get(input.review, "requestSubResource", object.get(input.review, "subResource", "")) == ""
```

The directive covers exactly one line. Do not extend the checker with a
hardcoded list of blessed field names instead: matching on identifiers ties the
linter to names it has no business knowing, so renaming a variable silently
changes what is checked and a genuinely new bug next to a blessed name is
swallowed.
...
| Rule | Pattern | Problem |
| ---- | ------- | ------- |
| `replicaset-in-kinds` | `ReplicaSet` in match.kinds | RS is generated by Deployment — denial surfaces only in Deployment status. |
| `replicaset-in-test-fixture` | `ReplicaSet` in a test constraint fixture | The fixture no longer matches the shipped `workload_kinds`, so the suite validates a scope that is never deployed. |
| `duplicated-kinds-block` | Same kinds block appears 2+ times in a file | Extract into `{{- include "workload_kinds" . }}` helper. |
| `webhook-delete-on-controllers` | DELETE operation in webhook rules for controllers | While Gatekeeper is unavailable and `failurePolicy: Fail` is in effect, tearing down or rolling back a workload is blocked too. |

These rules take a file-level directive with the same mandatory reason:

```gotemplate
{{/* webhook-scope:allow=webhook-delete-on-controllers — DELETE interception is intended: policies must be able to forbid deleting a resource. */}}
```

`validatingwebhookconfiguration.yaml` carries exactly that directive. Intercepting
`DELETE` is a deliberate decision for this module — a rule that inspects
`input.review.operation == "DELETE"` only runs if the request reaches Gatekeeper —
so the rule stays available for new files without flagging the existing one.

### `workload_kinds` Helm helper

All constraint `match.kinds` blocks should use the shared helper instead of inline YAML:
...
1. **`labelSelector`** evaluates against the reviewed object's own `metadata.labels`. For controllers, this is the **top-level** metadata, not the pod template's `spec.template.metadata.labels`. See the comment on `constraint_selector` in `_helpers.tpl`.
...
4. **SPE label/annotation resolution**: for controllers, SPE-relevant labels and annotations are resolved from the pod template only (`spec.template.metadata`), not the controller's top-level metadata. This is handled by `object_labels` and `effective_metadata` in `common.rego`. This does not extend to `security.deckhouse.io/skip-pss-check`: that exclusion lives in Gatekeeper's `match.labelSelector` and is evaluated before any policy code runs, so it must be set on the controller's top-level metadata.

5. **Lenient mode**: name the component that actually fills an absent field in. LimitRange really does default `container.resources`; Pod Security Admission never modifies an object, so for `runAsUser` or `seccompProfile.type` the source is a mutating webhook, including a Gatekeeper `Assign` mutator. Decide leniency per container, never for the pod template as a whole.

6. **Violation messages**: use `lib.common.review_kind` instead of hardcoding "Pod", so a denied Deployment reports `Deployment <name>`.
