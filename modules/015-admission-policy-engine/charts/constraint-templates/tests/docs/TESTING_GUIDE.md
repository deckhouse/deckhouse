...
The lint checker suppresses known-intentional patterns:
- `object.get(..., "")` used in response object construction (`"errors"`, `"system_error"`, `"responses"`)
- Image retrieval (`object.get(c, "image", "")` followed by `!= ""`)
- Heritage label lookup, operation field lookup, subresource resolution
- `dnsPolicy` check (absent + `hostNetwork: true` is a real violation)
...
| `webhook-delete-on-controllers` | DELETE operation in webhook rules for controllers | During Gatekeeper degradation, cannot tear down/roll back workloads. |
...
4. **SPE label/annotation resolution**: for controllers, SPE-relevant labels and annotations are resolved from the pod template only (`spec.template.metadata`), not the controller's top-level metadata. This is handled by `object_labels` and `effective_metadata` in `common.rego`.
