...
func handleSP(input *go_hook.HookInput) error {
	...
	for _, sn := range snap {
		...
		// set observed status
		input.PatchCollector.Filter(set_cr_statuses.SetObservedStatus(sn, filterSP), "deckhouse.io/v1alpha1", "securitypolicy", "", sp.Metadata.Name, object_patch.FilterWithSubresource("/status"), object_patch.FilterWithIgnoreHookError(true))
