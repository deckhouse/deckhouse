...
func handleSP(input *go_hook.HookInput) error {
	...
	for _, sn := range snap {
		...
		// set observed status
		input.PatchCollector.PatchWithMutatingFunc(set_cr_statuses.SetObservedStatus(sn, filterSP), "deckhouse.io/v1alpha1", "securitypolicy", "", sp.Metadata.Name, object_patch.WithSubresource("/status"), object_patch.WithIgnoreHookError(true))
