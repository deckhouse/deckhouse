...
func updateSpStatus(input *go_hook.HookInput) error {
	...
	for _, sp := range securityPolicies {
		input.PatchCollector.Filter(set_cr_statuses.SetProcessedStatus(filterSP), "deckhouse.io/v1alpha1", "securitypolicy", "", sp.Metadata.Name, object_patch.FilterWithSubresource("/status"), object_patch.FilterWithIgnoreHookError(true))
	}
