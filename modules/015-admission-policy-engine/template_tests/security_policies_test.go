...
`)
	...
	Context("Cluster with security policies", func() {
		...
		It("A policy that only warns is not split by namespace scope", func() {
			...
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-warn-"+testPolicyName).Exists()).To(BeFalse())
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-enforce-"+testPolicyName).Exists()).To(BeFalse())
		...
		It("A denying policy that spans both scopes is split into three constraints", func() {
			...
			systemWarn := f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-warn-exprpolicy")
			systemEnforce := f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-enforce-exprpolicy")
		...
		It("A denying policy that names no system namespace stays a single constraint", func() {
			...
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-warn-minpolicy").Exists()).To(BeFalse())
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-enforce-minpolicy").Exists()).To(BeFalse())
		...
		It("A denying policy limited to system namespaces keeps its name on the enforcing constraint", func() {
			...
			systemWarn := f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-warn-syspolicy")

			Expect(systemEnforce.Exists()).To(BeTrue())
			Expect(systemWarn.Exists()).To(BeTrue())
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-enforce-syspolicy").Exists()).To(BeFalse())
		...
		It("A denying policy that excludes every system namespace stays a single constraint", func() {
			...
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-warn-excludedpolicy").Exists()).To(BeFalse())
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-enforce-excludedpolicy").Exists()).To(BeFalse())
		...
		It("A denying policy whose namespace list cannot be intersected is left alone", func() {
			...
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-warn-globpolicy").Exists()).To(BeFalse())
			Expect(f.KubernetesGlobalResource("D8PrivilegedContainer", "d8ape-system-enforce-globpolicy").Exists()).To(BeFalse())
		...
		It("Every variant keeps the parameters of the policy it came from", func() {
			...
			for _, name := range []string{"exprpolicy", "d8ape-system-warn-exprpolicy", "d8ape-system-enforce-exprpolicy"} {
