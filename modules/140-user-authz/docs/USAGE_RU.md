...
Такие ClusterRole не привязываются к пользователям по одной. Модуль поддерживает агрегированную ClusterRole `user-authz:<access-level>:custom` на каждый уровень доступа (например, `user-authz:editor:custom`), которая собирает правила всех аннотированных ClusterRole этого уровня и уровней ниже, и каждое правило `ClusterAuthorizationRule`/`AuthorizationRule` с указанным `accessLevel` получает один биндинг на эту агрегированную роль. Лейбл `user-authz.deckhouse.io/access-level` на аннотированные ClusterRole модуль выставляет автоматически; агрегацию выполняет контроллер агрегации Kubernetes, поэтому новые правила начинают действовать без переразвёртывания модуля. В журнале аудита доступ, выданный таким образом, приписывается агрегированной роли, а не отдельной аннотированной ClusterRole.
