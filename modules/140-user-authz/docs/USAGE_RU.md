...
{% alert level="info" %}
Пример использует [основную ролевую модель](./#основная-ролевая-модель).
{% endalert %}
...
Для назначения прав на управление ресурсами приложений в рамках неймспейса, но без возможности настройки модулей DKP, используйте роль `d8:namespace:admin` в RoleBinding в соответствующем неймспейсе.
...
{% alert level="info" %}
Пример использует [основную ролевую модель](./#основная-ролевая-модель) и модуль [multitenancy-manager](../multitenancy-manager/).
{% endalert %}
...
{% alert level="info" %}
Пример относится к [устаревшей ролевой модели](./#устаревшая-ролевая-модель).
{% endalert %}
...
{% alert level="warning" %}
Ограничения по неймспейсам из ClusterAuthorizationRule реализованы в цепочке авторизации с вебхуком. Если вебхук недоступен, эти ограничения **не применяются**, пока вебхук снова не станет доступен. Подробнее — в [описании модуля](./#устаревшая-ролевая-модель).
{% endalert %}

{% offtopic title="Основная ролевая модель" %}
