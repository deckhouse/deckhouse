...
{% alert level="info" %}
Субъект типа `User` или `Group` сопоставляется по имени с идентичностью из токена, а локально управляемая идентичность неотличима от полученной от внешнего провайдера аутентификации. Поэтому создание [User](/modules/user-authn/cr.html#user) с `spec.email` или [Group](/modules/user-authn/cr.html#group) с `spec.name`, совпадающим с субъектом существующего правила, запрещено — чтобы идентичность не получила права правила незаметно. Если совпадение сделано намеренно, например когда правило написано заранее, установите на User или Group аннотацию `user-authz.deckhouse.io/allow-authorization-rule-collision: "true"`.

Субъект типа `User` указывайте в нижнем регистре: email попадает в токен приведённым к нижнему регистру, поэтому субъект `Admin@Example.com` не совпадёт ни с кем. Субъект типа `Group` сравнивается точно, поскольку имена групп нигде не приводятся к нижнему регистру.

Добавление субъекта в правило подобным образом не ограничивается. Если совпадающий User или Group уже существует, права он получит сразу.
{% endalert %}
