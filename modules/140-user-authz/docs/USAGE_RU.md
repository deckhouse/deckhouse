...
Для назначения прав на управление ресурсами приложений в неймспейсе (включая некоторые namespaced-ресурсы модулей DKP, например DexAuthenticator) используйте роль `d8:namespace:admin` в RoleBinding в соответствующем неймспейсе. Кластерная настройка модулей в эту роль не входит.
