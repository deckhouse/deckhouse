...
{% alert level="info" %}
Пример относится к [устаревшей ролевой модели](readme.html#устаревшая-ролевая-модель).
{% endalert %}
...
{% alert level="warning" %}
Ограничения по неймспейсам из `ClusterAuthorizationRule` реализованы в цепочке авторизации с вебхуком. Если вебхук недоступен, эти ограничения **не применяются**, пока вебхук снова не станет доступен. Подробнее — в [описании модуля](readme.html#устаревшая-ролевая-модель).
{% endalert %}
