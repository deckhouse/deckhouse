...
Создать User на email, на котором грант уже есть, записать или удалить ClusterAuthorizationRule, сбросить такую идентичность через UserOperation либо подключить DexProvider, который может утвердить этот email или группу, — это раздача ролей. Запрос пройдёт, только если у запрашивающего уже есть покрывающие права или ему явно разрешено назначать эти роли.
