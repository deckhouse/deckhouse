...
{% alert level="warning" %}
Manage-роль ограничивает, к каким модулям и неймспейсам обращается субъект. Создать локального пользователя или группу, чей email или имя ещё не указаны субъектом существующего гранта, — обычное создание объекта. Создать пользователя на email, на котором уже висит грант, или записать ClusterAuthorizationRule — это назначение ролей: у запрашивающего должны быть покрывающие права либо явный диапазон can-assign на эти роли. Менеджер подсистемы `security` может выдать [уровни доступа](#текущая-ролевая-модель) ClusterAuthorizationRule (`User`, `PrivilegedUser`, `Editor`, `Admin`, `ClusterEditor`, `ClusterAdmin`) и роли подсистемы security до `d8:subsystem:security:admin`. Он не может выдать уровень `SuperAdmin` и кластерную роль Kubernetes `cluster-admin`. Права модуля `user-authn` на создание User и Group этого не дают: они не назначают роли, которые уже висят на этом email.
{% endalert %}
