...
Обратите внимание: устаревшая роль `d8:use:role:admin` соответствует роли `d8:namespace:admin` и, как и она, больше не даёт права выпускать токены ServiceAccount'ов и выполнять impersonation — теперь для этого нужен уровень `superadmin`.

Capabilities (роли-кирпичики `d8:manage:permission:*` и `d8:use:capability:*`) переименованы **без** псевдонимов совместимости: они предназначены для агрегации в роли, а не для прямой привязки. Если у вас есть `RoleBinding`/`ClusterRoleBinding` непосредственно на такую capability, после обновления он перестанет давать права — пересоздайте привязку на подходящую роль (или на собственную роль, агрегирующую новую capability).

Переведите существующие `RoleBinding` и `ClusterRoleBinding` на новые имена ролей. Поскольку поле `roleRef` неизменяемо, привязку нужно удалить и создать заново с новым именем роли. Найти привязки, использующие устаревшие имена, можно командой:
