...
- `d8:namespace:user` — дополнительно к роли `d8:namespace:viewer` позволяет в конкретном пространстве имён просматривать ресурсы RBAC, удалять поды (но не создавать или изменять их), выполнять `kubectl port-forward` и `kubectl proxy`, изменять количество реплик контроллеров;
- `d8:namespace:manager` — дополнительно к роли `d8:namespace:user` позволяет в конкретном пространстве имён просматривать секреты и подключаться к подам (`kubectl exec`, `kubectl attach`), а также управлять ресурсами модулей (например, `Certificate`, `PodLoggingConfig` и т. п.) и стандартными namespaced-ресурсами Kubernetes (`Pod`, `Deployment`, `ConfigMap`, `Secret`, `Service`, `Ingress`, `NetworkPolicy`, `CronJob` и т. п.);
...
Чтение всех секретов пространства имён и выполнение команд в его контейнерах — именно те два права, которые превращают доступ к пространству имён в контроль над всем, что в нём работает, поэтому они отнесены к уровню `manager`, который пространством имён управляет, а не к уровню `user`, который в нём просто работает.
...
Оба перечисленных ограничения не действуют на администраторов кластера. Администратором кластера считается тот, кому выдана стандартная роль Kubernetes `cluster-admin` или роль `user-authz:super-admin` из прежней ролевой модели (её получают субъекты `ClusterAuthorizationRule` с уровнем доступа `SuperAdmin`), а также участники групп `system:masters`, `kubeadm:cluster-admins`, `superadmins` и `system:sudousers`. Способ выдачи роли значения не имеет: подойдёт и `ClusterRoleBinding`, и `RoleBinding` в нужном пространстве имён. У таких пользователей и так есть полные права на кластер, поэтому за ними сохраняется аварийный доступ.

При этом права `superadmin` тоже не безграничны:

- Ресурсы, созданные из [шаблона проекта](../multitenancy-manager/) (лейбл `heritage: multitenancy-manager`), не может изменить **никто**, включая `superadmin` и администратора кластера, — они управляются исключительно контроллером проектов. Чтобы изменить такой ресурс, измените шаблон проекта или сам проект.
...
Обратите внимание: устаревшая роль `d8:use:role:admin` соответствует роли `d8:namespace:admin` и, как и она, [больше не даёт](#ограничения-уровня-admin-и-права-superadmin) права выпускать токены ServiceAccount'ов — теперь для этого нужен уровень `superadmin`. Точно так же устаревшие роли `d8:use:role:user` и `d8:use:role:user:kubernetes` соответствуют роли `d8:namespace:user` и больше не дают прав [просматривать секреты и подключаться к подам](#namespace-роли) — теперь для этого нужен уровень `manager`.
