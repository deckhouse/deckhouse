...
### Глобальные справочники ресурсов (`d8:use:dict`)

Помимо use- и manage-ролей, модуль создаёт специальную ClusterRole `d8:use:dict`. Она даёт
**read-only** доступ к кластерным «справочным» ресурсам, которые пользователям часто нужно просматривать
...
{% alert level="info" %}
Роль `d8:use:dict` независима от механизма грантов `multitenancy-manager`: она даёт только **чтение**
справочных ресурсов для их обнаружения. Механизм грантов контролирует, какие **значения** ресурсов
