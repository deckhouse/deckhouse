...
Ни одно из перечисленных ограничений не действует на администраторов кластера. Администратором кластера считается тот, кому выдана стандартная роль Kubernetes `cluster-admin` или роль `user-authz:super-admin` из прежней ролевой модели (её получают субъекты `ClusterAuthorizationRule` с уровнем доступа `SuperAdmin`), а также участники групп `system:masters`, `system:sudousers` и `cluster:admins`. Способ выдачи роли значения не имеет: подойдёт и `ClusterRoleBinding`, и `RoleBinding` в нужном пространстве имён. У таких пользователей и так есть полные права на кластер, поэтому за ними сохраняется аварийный доступ.

При этом права `superadmin` тоже не безграничны:

- Ресурсы, созданные из [шаблона проекта](../multitenancy-manager/) (лейбл `heritage: multitenancy-manager`), не может изменить ни `admin`, ни `superadmin` — они управляются исключительно контроллером проектов. Чтобы изменить такой ресурс, измените шаблон проекта или сам проект. Исключение — администраторы кластера, состоящие в перечисленных выше группах.
