...
Модели не совместимы по ресурсам — автоматическая конвертация невозможна, — но [могут использоваться одновременно](faq.html#можно-ли-использовать-упрощённую-и-гранулярную-ролевые-модели-одновременно): права из обеих моделей суммируются.
{% endalert %}
...
В отличие [от упрощённой ролевой модели](#упрощённая-ролевая-модель) DKP, гранулярная ролевая модель не использует ресурсы ClusterAuthorizationRule и AuthorizationRule. Права доступа настраиваются стандартным для RBAC Kubernetes способом: с помощью создания ресурсов RoleBinding или ClusterRoleBinding, с указанием в них одной из подготовленных модулем `user-authz` ролей. Для доступа сразу ко всем неймспейсам проекта дополнительно используются ресурсы [ProjectRoleBinding](/modules/multitenancy-manager/cr.html#projectrolebinding) и [ClusterProjectRoleBinding](/modules/multitenancy-manager/cr.html#clusterprojectrolebinding) модуля `multitenancy-manager`.

{% alert level="info" %}
Выдавать доступ можно не только вручную через YAML-манифесты: в [веб-интерфейсе Deckhouse Kubernetes Platform](/products/kubernetes-platform/documentation/latest/user/web/ui.html) есть мастер выдачи доступа. Он проводит по шагам (кому выдать доступ → где → с каким уровнем), сам выбирает правильный вид привязки (RoleBinding, ClusterRoleBinding, ProjectRoleBinding или ClusterProjectRoleBinding) и позволяет собрать собственную роль из готовых блоков без написания YAML.
{% endalert %}
...
- Управлять доступом к модулям определённой [подсистемы](#подсистемы-ролевой-модели) применения.
...
Namespace-роли предназначены для назначения прав пользователю **в конкретном неймспейсе**. Под пользователями понимаются, например, разработчики, которые используют настроенный администратором кластер для развёртывания своих приложений. Таким пользователям не нужно управлять модулями DKP или кластером, но им нужно иметь возможность, например, создавать свои Ingress-ресурсы, настраивать аутентификацию приложений и сбор логов с приложений.
...
{% alert level="warning" %}
Проектную роль нельзя назначить через ClusterRoleBinding — попытка будет отклонена. Для назначения роли на весь проект используйте [ProjectRoleBinding](../multitenancy-manager/cr.html#projectrolebinding) или [ClusterProjectRoleBinding](../multitenancy-manager/cr.html#clusterprojectrolebinding). Допускается также обычный RoleBinding в одном из неймспейсов проекта — тогда роль действует только в нём.
{% endalert %}

Проектные роли (`d8:project:<уровень>`) предназначены для работы с [проектами](../multitenancy-manager/) — изолированными окружениями, которые могут включать несколько неймспейсов. Уровни те же, что у namespace-ролей: `viewer`, `user`, `manager`, `admin`, `superadmin`.
...
{% alert level="warning" %}
Системные и подсистемные роли не дают доступа к неймспейсу пользовательских приложений.
...
- `ACCESS_LEVEL` — уровень доступа.
  ...
  - `d8:system:viewer` — доступ на просмотр конфигурации всех модулей DKP (ресурсы ModuleConfig), их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes (кроме секретов и ресурсов RBAC) во всех системных неймспейсах (начинающихся с `d8-` или `kube-`);
  - `d8:system:manager` — аналогично роли `d8:system:viewer`, только доступ на уровне `admin`, т. е. просмотр/создание/изменение/удаление конфигурации всех модулей DKP (ресурсы ModuleConfig), их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes во всех системных неймспейсах (начинающихся с `d8-` или `kube-`);
  - `d8:subsystem:observability:viewer` — доступ на просмотр конфигурации модулей DKP (ресурсы ModuleConfig) из подсистемы `observability`, их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes (кроме секретов и ресурсов RBAC) в системных неймспейсах `d8-log-shipper`, `d8-monitoring`, `d8-okmeter`, `d8-operator-prometheus`, `d8-upmeter`, `kube-prometheus-pushgateway`.

Модуль предоставляет три уровня доступа для системных и подсистемных ролей:
...
Принадлежность объектов к ролевой модели задаётся лейблами `rbac.deckhouse.io/*` — например, лейбл `rbac.deckhouse.io/aggregate-to-namespace-as: admin` включает capability в роль `d8:namespace:admin` (и, за счёт иерархии уровней, — во все уровни выше). Полный перечень лейблов и аннотаций — в [«Справочнике лейблов и аннотаций ролей»](#справочник-лейблов-и-аннотаций-ролей).
...
- Вы можете собирать собственные роли из готовых capabilities, не описывая RBAC-правила вручную. Как это сделать описано [в FAQ](faq.html#как-расширить-роли-или-создать-новую).
...
Лейблы, которые ролевая модель использует на объектах ClusterRole:
...
| `rbac.deckhouse.io/use-role: <уровень>` | Системные и подсистемные роли | Какой уровень namespace-роли автоматически выдаётся обладателю этой роли в системных неймспейсах её подсистемы. У встроенных ролей: `viewer` → `viewer`, `manager` → `admin`, `superadmin` → `superadmin`. Выдача выполняется автоматически создаваемыми RoleBinding (лейбл `rbac.deckhouse.io/automated: "true"`). |
| `rbac.deckhouse.io/namespace: <неймспейс>` | Capabilities | Дополнительный неймспейс, в котором обладателям системной/подсистемной роли будет автоматически создан RoleBinding ([пример в FAQ](faq.html#расширение-подсистемных-ролей-с-добавлением-нового-пространства-имён)). |
| `rbac.deckhouse.io/delegatable: "true"` | Роли `d8:namespace:*`, `d8:project:*` и пользовательские | Роль можно указывать в RoleBinding [внутри неймспейсов проектов](../multitenancy-manager/usage.html#какие-роли-доступны-в-rolebinding-внутри-проекта). Добавляйте лейбл на собственные роли, которые должны быть доступны в проектах. |
...
| Аннотация | Кто добавляет | Назначение |
|-----------|-----------|------------|
...
Перечисленные выше ограничения не действуют на администраторов кластера. Администратором кластера считается тот, кому выдана стандартная роль Kubernetes `cluster-admin` или роль `user-authz:super-admin` из прежней ролевой модели (её получают субъекты ClusterAuthorizationRule с уровнем доступа `SuperAdmin`), а также участники групп `system:masters`, `kubeadm:cluster-admins`, `superadmins` и `system:sudousers`. Способ выдачи роли значения не имеет: для этого можно использовать и ClusterRoleBinding, и RoleBinding в нужном неймспейсе. У таких пользователей и так есть полные права на кластер, поэтому за ними сохраняется аварийный доступ.

Для `superadmin` также есть некоторые ограничения:

- Ресурсы, созданные из [шаблона проекта](/modules/multitenancy-manager/) (лейбл `heritage: multitenancy-manager`), не может изменить **никто**, включая `superadmin` и администратора кластера, — они управляются исключительно контроллером проектов. Чтобы изменить такой ресурс, измените шаблон проекта или сам проект.
- Роль назначается через RoleBinding и действует только в том неймспейсе, где выдана: `superadmin` одного неймспейса не получает никаких особых прав в другом.
...
- стандартные права упрощённой ролевой модели (права k8s);
