...
Если вебхук авторизации станет недоступен, запросы будут **запрещаться**, а не разрешаться. Вебхук стоит первым в цепочке авторизации API-сервера и настроен с `failurePolicy: Deny`, поэтому запрос, на который он не смог ответить, до RBAC не доходит. Исключение — идентичности, перечисленные в `matchConditions` ресурса `AuthorizationConfiguration`: компоненты control plane, kubelet'ы, сервисные аккаунты `kube-system` и сервисные аккаунты `d8-*` через вебхук не проходят вовсе, поэтому кластер продолжает работать, а Deckhouse может восстановить вебхук.
