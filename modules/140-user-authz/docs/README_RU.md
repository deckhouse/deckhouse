...
{% alert level="warning" %}
Manage-роль ограничивает, к каким модулям и неймспейсам обращается субъект. Создать локального пользователя или группу с именем, которое ещё не является субъектом гранта, — обычный CRUD. Занять email или группу, на которых уже висит грант, или записать ClusterAuthorizationRule — это назначение: у запрашивающего должны быть покрывающие права либо явный диапазон can-assign на целевые роли. Менеджер подсистемы `security` может назначать basic-доступ до `ClusterAdmin` и роли security до названого admin и не может назначить SuperAdmin или `cluster-admin`. Одного права `user-authn` edit недостаточно, чтобы занять уже существующую SuperAdmin-идентичность.
{% endalert %}
