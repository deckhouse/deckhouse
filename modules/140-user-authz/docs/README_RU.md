...
Принадлежность объектов к ролевой модели задаётся лейблами `rbac.deckhouse.io/*`:

| Лейбл | Назначение |
|-------|-----------|
| `rbac.deckhouse.io/kind` | Тип объекта: `role`, `capability` (встроенные), `custom-role`, `custom-capability` (пользовательские) |
| `rbac.deckhouse.io/scope` | Область: `namespace`, `project`, `subsystem`, `system` |
| `rbac.deckhouse.io/aggregate-to-<область>-as: <уровень>` | Включает capability в роль указанной области и уровня. Например, лейбл `rbac.deckhouse.io/aggregate-to-namespace-as: admin` включает capability в роль `d8:namespace:admin` (и, за счёт лестницы уровней, в `d8:namespace:superadmin`) |
| `rbac.deckhouse.io/capability` | Уникальное имя capability — по нему capability можно адресно включить в собственную роль |
