...
{% alert level="warning" %}
В модуле реализованы две ролевые модели: [основная](#основная-ролевая-модель) (рекомендуется к использованию) и [устаревшая](#устаревшая-ролевая-модель), построенная на ресурсах ClusterAuthorizationRule и AuthorizationRule (поддержка будет прекращена в будущих релизах).
