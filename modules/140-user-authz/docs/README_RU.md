...
description: "Авторизация и управление доступом пользователей к ресурсам кластера Deckhouse Platform."
---
...
{% alert level="warning" %}
С версии Deckhouse Platform v1.64 в модуле реализована экспериментальная модель ролевого доступа.
...
Use-роли предназначены для назначения прав пользователю **в конкретном пространстве имён**. Под пользователями понимаются, например, разработчики, которые используют настроенный администратором кластер для развёртывания своих приложений. Таким пользователям не нужно управлять модулями DP или кластером, но им нужно иметь возможность, например, создавать свои Ingress-ресурсы, настраивать аутентификацию приложений и сбор логов с приложений.
...
{% alert level="warning" %}
Manage-роль сама по себе не даёт права раздавать доступ другим людям.

Создать User или Group, которых ещё нет среди субъектов грантов, можно как обычный объект.

Создать User на email, на котором грант уже есть, записать или удалить ClusterAuthorizationRule либо сбросить такую идентичность через UserOperation — это раздача ролей. Запрос пройдёт, только если у запрашивающего уже есть покрывающие права или ему явно разрешено назначать эти роли.

Подключение DexProvider проверяется так же, но относительно идентичностей, которые провайдер способен утвердить: `spec.allowedIdentities` (email, домены email, группы) и собственные фильтры групп провайдера ограничивают это множество, и учитываются только роли, уже выданные внутри него. Провайдер без ограничителя по email способен утвердить любой адрес, включая те, на которых висит SuperAdmin, поэтому его запись требует SuperAdmin. Подробности и список изменений, проходящих без проверки, — в документации модуля user-authn.

У ClusterAuthorizationRule поле `spec.accessLevel` задаёт уровень [текущей ролевой модели](#текущая-ролевая-модель): `User`, `PrivilegedUser`, `Editor`, `Admin`, `ClusterEditor`, `ClusterAdmin`, `SuperAdmin`. Менеджер подсистемы `security` может назначить любой из них, кроме `SuperAdmin`. Кластерную роль Kubernetes `cluster-admin` он тоже назначить не может. Роли экспериментальной модели подсистемы security — не выше `d8:subsystem:security:admin`.

В тексте отказа вебхука уровни `accessLevel` называются basic, чтобы не путать их с уровнями manage-ролей (`viewer` / `manager`).
...
- к управлению модулями DP (ресурсы ModuleConfig) в рамках [подсистемы](#подсистемы-ролевой-модели) роли, или всеми модулями DP для роли `d8:manage:all:*`;
- к управлению cluster-wide-ресурсами модулей DP в рамках [подсистемы](#подсистемы-ролевой-модели) роли или всеми ресурсами модулей DP для роли `d8:manage:all:*`;
...
- `ACCESS_LEVEL` — уровень доступа.
  ...
  - `d8:manage:all:viewer` — доступ на просмотр конфигурации всех модулей DP (ресурсы ModuleConfig), их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes (кроме секретов и ресурсов RBAC) во всех системных пространствах имён (начинающихся с `d8-` или `kube-`);
  - `d8:manage:all:manager` — аналогично роли `d8:manage:all:viewer`, только доступ на уровне `admin`, т. е. просмотр/создание/изменение/удаление конфигурации всех модулей DP (ресурсы ModuleConfig), их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes во всех системных пространствах имён (начинающихся с `d8-` или `kube-`);
  - `d8:manage:observability:viewer` — доступ на просмотр конфигурации модулей DP (ресурсы ModuleConfig) из подсистемы `observability`, их cluster-wide-ресурсов, их namespaced-ресурсов и стандартных объектов Kubernetes (кроме секретов и ресурсов RBAC) в системных пространствах имён `d8-log-shipper`, `d8-monitoring`, `d8-okmeter`, `d8-operator-prometheus`, `d8-upmeter`, `kube-prometheus-pushgateway`.
...
Помимо use- и manage-ролей, модуль создаёт специальную ClusterRole `d8:use:dict`. Она даёт
доступ для чтения кластерных «справочных» ресурсов, которые пользователям часто нужно просматривать
при создании объектов. Например, пользователю, создающему PersistentVolumeClaim, нужно видеть
доступные StorageClass, а создающему Ingress — IngressClass.
...
ClusterRoleBinding для `d8:use:dict` создаётся **автоматически**, когда RoleBinding ссылается на роль
`d8:use:role:*` (экспериментальная модель) или на устаревшую роль `user-authz:*` (текущая модель). Это
означает, что пользователи проекта могут просматривать справочные ресурсы без какой-либо ручной
настройки. Привязка появляется вместе с RoleBinding и удаляется при его удалении. Привязку создаёт и
управляет ею исключительно модуль `user-authz`. Самостоятельно её создавать не нужно.

{% alert level="info" %}
Роль `d8:use:dict` независима от механизма управления доступом к cluster-wide-ресурсам `multitenancy-manager`: она предоставляет права **только на чтение**
...
1. После обновления на DP 1.78 переведите эти объекты RoleBinding и ClusterRoleBinding на новые имена ролей в течение одного релизного цикла. Поскольку поле `roleRef` неизменяемо, привязку нужно удалить и создать заново с новым именем роли.

Порядок миграции кастомных ролей описан [в разделе «FAQ»](faq.html#как-перевести-кастомные-роли-на-новую-схему-в-dp-178).
