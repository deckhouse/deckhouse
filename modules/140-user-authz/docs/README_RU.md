...
<div style="height: 0;" id="миграция-на-новые-имена-ролей"></div>

### Миграция на новые имена ролей

{% alert level="warning" %}
Этот раздел описывает изменения, которые вступят в силу в одном из следующих релизов DKP. В текущем релизе роли продолжают работать под существующими именами.
{% endalert %}

В одном из следующих релизов DKP роли экспериментальной модели будут переименованы: текущие имена (`d8:manage:<подсистема>:<уровень>`, `d8:manage:all:<уровень>` и `d8:use:role:<уровень>`) станут устаревшими. Для обратной совместимости они будут сохранены как роли-псевдонимы ровно на один релиз: существующие привязки продолжат работать и давать те же права, что и новые роли, после чего псевдонимы будут удалены.
...
Обратите внимание: роль `d8:use:role:admin` будет соответствовать роли `d8:namespace:admin` и, как и она, перестанет давать права выпускать токены ServiceAccount'ов и выполнять impersonation — для этого понадобится уровень `superadmin`.

Capabilities (роли-кирпичики `d8:manage:permission:*` и `d8:use:capability:*`) будут переименованы **без** псевдонимов совместимости: они предназначены для агрегации в роли, а не для прямой привязки. Если у вас есть `RoleBinding`/`ClusterRoleBinding` непосредственно на такую capability, после обновления он перестанет давать права — пересоздайте привязку на подходящую роль (или на собственную роль, агрегирующую новую capability).
...
1. После обновления на релиз с переименованием переведите эти `RoleBinding` и `ClusterRoleBinding` на новые имена ролей в течение одного релизного цикла. Поскольку поле `roleRef` неизменяемо, привязку нужно удалить и создать заново с новым именем роли.

О миграции собственных (кастомных) ролей см. [в FAQ](faq.html#как-перевести-собственные-кастомные-роли-на-новую-схему).
