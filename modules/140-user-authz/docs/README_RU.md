...
Субъект, который может управлять модулем `user-authn`, по-прежнему создаёт локальные учётки. Удалить или пересоздать пользователя, чей email уже есть в правиле, можно только с правами не ниже этого гранта. Субъект, который может управлять модулем `user-authz`, меняет правила авторизации, но не может выдать accessLevel выше своего. При планировании доступа считайте manage-роль подсистемы `security` равной роли администратора кластера.
{% endalert %}
