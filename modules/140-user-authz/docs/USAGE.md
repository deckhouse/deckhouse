...
The module automatically sets the `user-authz.deckhouse.io/access-level` label on such ClusterRoles and includes their rules in the aggregated ClusterRole `user-authz:<access-level>:custom`. In audit logs, access granted through an annotated ClusterRole is attributed to that aggregated role.
