...
их, когда они исчезнут из манифеста, который отрисовал чарт. После этого движок делает один проход
по предыдущему манифесту, запрашивая каждый объект из него, чтобы увидеть метку. Этот проход
пропорционален числу биндингов и случается ровно один раз:
...
Чтобы применить ограничение по пространствам имён, вебхуку нужно знать, принадлежит ли ресурс
пространству имён, — он узнаёт это из discovery и запрашивает список API-группы не чаще раза в
10 секунд. Поэтому ресурс,
...
Раньше это был `fail` в чарте, останавливавший отрисовку шаблонов всего модуля: одно правило,
которое может написать любой, кому разрешено их создавать, замораживало все остальные изменения
модуля, а сообщение доходило только до того, кто читает логи релиза. Теперь об этом сообщается.
...
чём, что применял бы вебхук, и в метрику не попадает.
