...
| `<подсистема>`: `deckhouse`, `delivery`, `infrastructure`, `kubernetes`, `networking`, `observability`, `security`, `storage` | `viewer`, `manager`, `superadmin`                  | `ClusterRoleBinding`              | Кластерные ресурсы модуля и его `ModuleConfig`                      |
