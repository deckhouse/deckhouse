...
- вместо вида объекта у каждой capability есть **область** (лейбл `rbac.deckhouse.io/scope`): область
  `namespace` — это то же самое, что раньше было `use`; область `system` или имя подсистемы модуля
  (`kubernetes`, `networking` и так далее) — то же самое, что раньше было `manage`.

Capability — это всё тот же обычный `ClusterRole` с правилами, только с другими лейблами и именем. А
вот **роли** (`d8:namespace:viewer`, `d8:system:manager` и подобные) модуль в RBACv2 не поставляет и
не должен: их создают `user-authz` и `multitenancy-manager`. Роль — это пустой `ClusterRole`, в котором
...
{% alert level="info" %}
Если в старых шаблонах модуля где-то остался собственный объект-роль с полем `rules` — уберите это поле
...
У каждой capability есть область — в какую встроенную роль она агрегируется — и уровень внутри
области — сколько прав она даёт, от простого просмотра до полного управления. Уровни идут по
нарастанию: `viewer` → `user` → `manager` → `admin` → `superadmin`, но не каждая область содержит все
уровни.
...
1. **На своём ли уровне лежит право.** Раньше `use`-capability попадала в кластерную подсистемную роль,
   и правило на кластерный ресурс там всё равно работало. Теперь namespace-capability агрегируется
   только в namespace-роль, а правило на кластерный ресурс в такой роли ничего не даёт. Проверьте
   каждое правило в мигрированных файлах `use/`: если ресурс кластерный (а не namespaced), перенесите
   это правило в `manage/` — там оно попадёт в подсистемную область, где кластерные правила и должны
   быть.
1. **Уровень.** Скрипт сохраняет то соответствие, которое использовало большинство модулей: `view` →
...
1. **Два лейбла агрегации сразу.** У прежних `use`-capability иногда стояли одновременно и
   `aggregate-to-kubernetes-as`, и `aggregate-to-networking-as` — capability попадала в обе подсистемы.
...
1. **Формулировки.** Заголовки и описания, которые сгенерировал скрипт, следуют общей для платформы
