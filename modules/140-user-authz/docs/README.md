...
- **Connecting to system pods** — `kubectl exec`, `kubectl attach`, and `kubectl port-forward` into a pod labelled `deckhouse.io/system-resource: "true"` are available to `superadmin` only. This protects against reading foreign secrets and interfering with platform components from inside their pods.
  ...
  For module authors: the marker is a **label**, `deckhouse.io/system-resource: "true"`, on the object itself — not an annotation. The objects to protect are selected on the API server side, in the admission `matchConditions`, and those read labels; an object marked with an annotation is not seen by the webhook and stays writable, with nothing to say so. Put the label on every namespaced object your module places into user namespaces (pods, services, PVCs, secrets, custom resources): it then becomes writable by `superadmin` and cluster administrators only, and its pods reachable by `exec`, `attach` and `port-forward` only for them.
