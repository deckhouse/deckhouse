...
None of the restrictions above apply to cluster administrators. A cluster administrator is anyone granted the standard Kubernetes `cluster-admin` role or the `user-authz:super-admin` role of the previous role model (which the subjects of a `ClusterAuthorizationRule` with the `SuperAdmin` access level receive), as well as members of the `system:masters`, `system:sudousers`, and `cluster:admins` groups. How the role was granted does not matter: both a `ClusterRoleBinding` and a `RoleBinding` in the namespace in question work. Such users already hold full rights over the cluster, so they keep their break-glass access.

The `superadmin` rights are not unlimited either:

- Resources created from a [project template](../multitenancy-manager/) (the `heritage: multitenancy-manager` label) cannot be modified by `admin` or `superadmin` — they are managed exclusively by the project controller. To change such a resource, change the project template or the project itself. Cluster administrators who belong to the groups listed above are the exception.
