...
Если в кластере зарегистрирована API-группа `metrics.k8s.io` (например, установлен [metrics-server](https://github.com/kubernetes-sigs/metrics-server)), модуль автоматически обнаруживает её и включает поставщик метрик **KubernetesMetrics** в политике descheduler. Это позволяет стратегиям утилизации (`HighNodeUtilization`, `LowNodeUtilization`) использовать данные о **фактическом** потреблении ресурсов из Metrics API, а не полагаться исключительно на запросы и лимиты подов.

От пользователя не требуется никаких действий: модуль обнаруживает API-группу `metrics.k8s.io`, отслеживая ресурсы `APIService`, и настраивает политику автоматически. Если metrics-server установлен **после** запуска descheduler, под descheduler будет автоматически перезапущен с обновлённой политикой.
