...
- name: d8.cloud-instance-group
  ...
  - alert: CloudInstanceGroupReplicasUnavailable
    expr: |
      ...
      * on(name) group_left(instance_group) machine_deployment_instance_group_info
    for: 1h
    labels:
      severity_level: "8"
      tier: cluster
    annotations:
      ...
      plk_grouped_by__cluster_has_cloud_instance_groups_with_unavailable_replicas: "ClusterHasCloudInstanceGroupsWithUnavailableReplicas,tier=cluster,prometheus=deckhouse"
      plk_labels_as_annotations: "instance_group"
      summary: В cloud instance group {{`{{ $labels.instance_group }}`}} есть недоступные инстансы
      description: |
...
{{- template "todo_list" }}
  ...
  - alert: CloudInstanceGroupReplicasUnavailable
    ...
    annotations:
      ...
      plk_grouped_by__cluster_has_cloud_instance_groups_with_unavailable_replicas: "ClusterHasCloudInstanceGroupsWithUnavailableReplicas,tier=cluster,prometheus=deckhouse"
      plk_labels_as_annotations: "instance_group"
      summary: В cloud instance group {{`{{ $labels.instance_group }}`}} нет ни одного доступного инстанса
      description: |
{{- template "todo_list" }}
  ...
  - alert: CloudInstanceGroupReplicasUnavailable
    ...
    annotations:
      ...
      plk_grouped_by__cluster_has_cloud_instance_groups_with_unavailable_replicas: "ClusterHasCloudInstanceGroupsWithUnavailableReplicas,tier=cluster,prometheus=deckhouse"
      plk_labels_as_annotations: "instance_group"
      summary: В cloud instance group {{`{{ $labels.instance_group }}`}} количество одновременно недоступных инстансов превышает допустимое значение.
      description: |
...
{{- template "todo_list" }}
  ...
  - alert: ClusterHasCloudInstanceGroupsWithUnavailableReplicas
    expr: count(max by (instance_group) (ALERTS{alertname="CloudInstanceGroupReplicasUnavailable", alertstate="firing"})) > 0
    labels:
      tier: cluster
    annotations:
      plk_markup_format: markdown
