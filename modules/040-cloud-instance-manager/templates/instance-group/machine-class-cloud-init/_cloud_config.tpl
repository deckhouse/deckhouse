...
{{- if hasKey $context.Values.cloudInstanceManager.internal "cloudProvider" }}
  {{- range $path, $_ := $context.Files.Glob (printf "candi/cloud-providers/%s/bashible-bundles/*/bootstrap-networks.sh.tpl" $context.Values.cloudInstanceManager.internal.cloudProvider.type) }}
...
{{- $_ := set $bashible_bootstrap_script_tpl_context "normal" (dict "apiserverEndpoints" $context.Values.cloudInstanceManager.internal.clusterMasterAddresses) }}
...
- path: '/var/lib/bashible/bootstrap.sh'
  permissions: '0700'
  encoding: b64
  content: {{ include "instance_group_machine_class_bashible_bootstrap_script" $bashible_bootstrap_script_tpl_context | b64enc }}

- path: '/var/lib/bashible/ca.crt'
  permissions: '0644'
  encoding: b64
  content: {{ $context.Values.cloudInstanceManager.internal.kubernetesCA | b64enc }}
