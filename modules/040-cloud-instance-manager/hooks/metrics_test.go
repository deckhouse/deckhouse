...
var _ = Describe("Modules :: cloud-instance-manager :: hooks :: metrics ::", func() {
	const (
...
`)
	...
	Context("Cluster with proper machine deployment", func() {
		...
		It("Hook must not fail, metrics must render", func() {
			...
			Expect(f.BindingContexts.Get("0.snapshots.machine_deployments.0.filterResult.labels").String()).To(MatchJSON(`{"instance_group":"proper","name":"dev-proper-297926a1"}`))
		})
	})
})
