...
cat << "EOF" > /etc/systemd/system/docker-health-monitor.service
[Unit]
...
cat << "EOF" > /etc/systemd/system/kubelet-health-monitor.service
[Unit]
Description=Kubelet-health-monitor daemon
After=docker-health-monitor.service
[Install]
WantedBy=multi-user.target
[Service]
Restart=always
...
mkdir -p /opt/bin
cat << "EOF" > /opt/bin/health-monitor
#!/bin/bash
set -o nounset
set -o pipefail
...
function kubelet_monitoring {
  ...
  function kubectl {
    command kubectl --kubeconfig /etc/kubernetes/kubelet.conf "$@"
  }
...
chmod +x /opt/bin/health-monitor

systemctl daemon-reload

units="docker-health-monitor.service kubelet-health-monitor.service"
