...
var _ = Describe("Modules :: deckhouse :: hooks :: migrate :: alert on obsolete monitoring-deckhouse ModuleConfig", func() {
	const obsoleteMC = `
---
...
`
	...
	Context("When the obsolete monitoring-deckhouse ModuleConfig is absent", func() {
		...
		It("Does not set the alert metric and keeps other ModuleConfigs", func() {
			...
			for _, m := range f.MetricsCollector.CollectedMetrics() {
				if m.Name == "d8_deckhouse_obsolete_moduleconfig" {
	...
	Context("When the obsolete monitoring-deckhouse ModuleConfig exists", func() {
		...
		It("Sets the alert metric and keeps the ModuleConfig", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(f.KubernetesGlobalResource("ModuleConfig", "monitoring-deckhouse").Exists()).To(BeTrue())
			Expect(f.KubernetesGlobalResource("ModuleConfig", "deckhouse").Exists()).To(BeTrue())
			...
			for _, m := range f.MetricsCollector.CollectedMetrics() {
				if m.Name == "d8_deckhouse_obsolete_moduleconfig" {
					...
					Expect(m.Labels).To(HaveKeyWithValue("moduleconfig", "monitoring-deckhouse"))
				}
			}
