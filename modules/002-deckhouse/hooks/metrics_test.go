...
[
  ...
  {"from": "01:00", "to": "02:00", "days":["Mon", "Fri"]}
	...
	Context("Module version telemetry (d8_telemetry_deckhouse_module_version)", func() {
		moduleState := `
---
...
`
		BeforeEach(func() {
			...
			f.ValuesSet("deckhouse.update.mode", "Auto")
			f.ValuesSet("deckhouse.update.windows", []string{})
			f.BindingContexts.Set(f.KubeStateSet(moduleState))
			f.RunHook()
		})
		...
		It("Sets d8_telemetry_deckhouse_module_version for each module with version", func() {
			metrics := f.MetricsCollector.CollectedMetrics()
			var moduleVersionMetrics []struct{ Name, Version string }
			for _, m := range metrics {
				if m.Name == "d8_telemetry_deckhouse_module_version" {
					moduleVersionMetrics = append(moduleVersionMetrics, struct{ Name, Version string }{
						m.Labels["module_name"],
						m.Labels["module_version"],
					})
				}
			}
			Expect(moduleVersionMetrics).To(ContainElement(struct{ Name, Version string }{"monitoring-kubernetes", "v1.2.3"}))
			Expect(moduleVersionMetrics).To(ContainElement(struct{ Name, Version string }{"prometheus", "v2.0.1"}))
			Expect(moduleVersionMetrics).To(HaveLen(2)) // only modules with non-empty version
			for _, m := range moduleVersionMetrics {
				Expect(m.Name).ToNot(BeEmpty())
				Expect(m.Version).ToNot(BeEmpty())
			}
		})

		It("Sets metric value to 1", func() {
			metrics := f.MetricsCollector.CollectedMetrics()
			for _, m := range metrics {
				if m.Name == "d8_telemetry_deckhouse_module_version" {
					Expect(m.Value).To(Equal(ptr.To(1.0)))
