...
Управлять обновлением DP можно следующими способами:
...
После появления новой минорной версии DP на используемом канале обновлений, но до момента применения ее в кластере на адрес вебхука будет выполнен [POST-запрос](configuration.html#parameters-update-notification-webhook).
...
{% alert level="warning" %}
Если ваш вебхук возвращает любой код вне диапазона 2хх, DP повторяет отправку уведомления до пяти раз с экспоненциальной задержкой между попытками. Если все попытки окажутся неуспешными, выпуск блокируется до восстановления доступности вебхука.
{% endalert %}
...
Если вебхук возвращает успешный статус HTTP (2xx), DP считает уведомление успешным вне зависимости от содержимого ответа.
...
DP публикует собственные веб-интерфейсы по именам хостов, которые формируются из глобального параметра [`publicDomainTemplate`](/products/kubernetes-platform/documentation/v1/reference/api/global.html#parameters-modules-publicdomaintemplate): API, веб-интерфейс, Grafana, Dex, генератор kubeconfig и другие.
...
- `List` — резервируются только имена хостов сервисов, о публикации которых DP знает.
...
В режиме `Template` резервируются не только имена хостов, используемые DP, но и другие имена, соответствующие шаблону. Если приложению необходимо такое имя, его можно исключить из резервирования одним из следующих способов:
...
Gateway, к которому подключаются ресурсы ListenerSet модулей DP, автоматически исключается из резервирования независимо от того, какие имена заявлены в его listeners.
...
Без его исключения было бы невозможно изменить такой Gateway после включения резервирования, хотя от него продолжают зависеть подключённые маршруты DP.
...
Wildcard-имена (например, `*.example.com`), в `grandfatheredHosts` не добавляются. Иначе приложение могло бы использовать все имена хостов, соответствующие шаблону, в том числе имена сервисов, которые DP начнёт публиковать в дальнейшем.
...
Это ограничение не распространяется на Gateway, к которому подключается DP. Он автоматически исключается из проверки.
...
| `hosts` | Имена хостов, для которых используется точное сопоставление. В режиме `Template` содержит значения `additionalHosts` и wildcard-имя домена, а в режиме `List` — имена хостов сервисов платформы |
...
| `sharedGateway` | Gateway, к которому подключается DP, в формате `<NAMESPACE>/<NAME>`. Если такой Gateway не задан и не обнаружен, значение будет пустым |
| `platformHosts` | Имена хостов сервисов, публикуемых DP. Используется как справочный ключ для обоих режимов |
