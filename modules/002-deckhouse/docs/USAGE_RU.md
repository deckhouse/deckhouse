...
### Шлюз, к которому подключается платформа

Gateway, к которому подключаются ресурсы ListenerSet модулей Deckhouse, исключён из резервирования независимо от того, какие имена заявлены в его листенерах.
Это объект, указанный в глобальном параметре [`gatewayAPIGateway`](/products/kubernetes-platform/documentation/v1/reference/api/global.html#parameters-modules-gatewayapigateway), а если параметр не задан — обнаруженный в ConfigMap `default-gateway` неймспейса `d8-alb`.
Исключение действует по имени и неймспейсу и только для ресурсов Gateway: то же имя хоста на Ingress, HTTPRoute или любом другом Gateway остаётся зарезервированным.

Это важно, потому что такой Gateway обычно несёт листенер с wildcard-именем и wildcard-сертификатом, а живёт нередко в собственном неймспейсе, а не в помеченном лейблом `heritage: deckhouse`.
Без исключения его стало бы невозможно изменить после начала действия резервирования, тогда как подключённые к нему маршруты платформы продолжают от него зависеть.

Любой другой Gateway с листенером `*.example.com` отклоняется, и запись о том, что уже обслуживалось, его не покрывает.
Такому Gateway нужен лейбл на неймспейсе или режим `List`, установленные до обновления.

Если общий Gateway не задан и не обнаружен, не исключается ничего.
...
Шлюзу, к которому подключается платформа, не нужно ни то, ни другое: он исключён по имени.

Снимок делается один раз и повторно не снимается, поэтому переключение в `List` и обратно в `Template` сохраняет запись первого периода `Template`: то, что появилось за время действия `List`, отклоняется при следующем изменении.
Иначе режим стал бы способом обойти резервирование — переключиться в `List`, занять имена, вернуться обратно и получить их записанными как ранее существовавшие.
...
| `sharedGateway` | Шлюз, к которому подключается платформа, в виде `неймспейс/имя`. Пустой, если такой шлюз не задан и не обнаружен |
