...
Модуль обеспечивает мониторинг, оповещения и наблюдаемость самой платформы Deckhouse Kubernetes Platform. Он отслеживает состояние, производительность и правильную работу основных компонентов DKP и работает совместно с модулем [`prometheus`](/modules/prometheus/).

Модуль развертывает ресурсы мониторинга, которые:

- **Собирают метрики DKP** — скрейпит метрики из пода Deckhouse с помощью ресурса PodMonitor, включая:
  - Собственные метрики на порту `4222` через эндпоинт `/metrics`.
...
- **Определяют правила оповещений** — предоставляет правила оповещений Prometheus, организованные по категориям:
...
PodMonitor скрейпит два эндпоинта из пода Deckhouse:
...
PodMonitor создается только при включенном модуле `operator-prometheus`, а правила оповещений и дашборды обрабатываются модулем `prometheus`.
...
- `ClusterObservabilityMetricsRulesGroup` — для правил Prometheus.
- `ClusterObservabilityDashboard` — для дашбордов Grafana.
