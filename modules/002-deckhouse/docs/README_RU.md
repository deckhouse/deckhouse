...
- Установить режим автоматического обновления для патч-версий.
  ...
  Например: текущая версия DP `v1.70.1`, после установки режима автоматического обновления для патч-версий, Deckhouse сможет обновиться до версии `v1.70.1`, но не будет обновляться до версии `v1.71.*` и выше.
...
- Установить конкретный тег для Deployment `deckhouse` и удалить параметр [releaseChannel](configuration.html#parameters-releasechannel) из конфигурации модуля `deckhouse`.
  ...
  В таком случае DP останется на конкретной версии, никакой информации о новых доступных версиях (объекты DeckhouseRelease) в кластере появляться не будет.
...
Практические примеры создания и использования PriorityClass, демонстрация вытеснения и диагностика описаны [в разделе «Примеры»](examples.html).
...
Модуль обеспечивает мониторинг и наблюдаемость компонентов Deckhouse Platform (DP). Он собирает метрики работы платформы, предоставляет преднастроенные правила оповещений и дашборды для контроля состояния DP.

Для сбора и хранения метрик модуль использует возможности модулей [`prometheus`](/modules/prometheus/) и [`observability`](/modules/observability/).

Модуль развертывает ресурсы мониторинга, которые:

- **Собирают метрики DP** — собирает метрики из пода `deckhouse` с помощью ресурса PodMonitor, включая:
  - собственные метрики на порте `4222` через эндпоинт `/metrics`;
  - пользовательские метрики, генерируемые хуками, через эндпоинт `/metrics/hooks`;
  - метрики выполнения модулей, хуков и состояния системы.

- **Определяют правила оповещений** — предоставляет правила оповещений Prometheus, организованные по следующим категориям:
  - **доступность DP** — мониторинг состояния подов, готовности и времени работы;
  - **неисправности DP** — обнаружение чрезмерных перезапусков, проблем доступа к хранилищу образов контейнеров, а также зависших процессов;
...
- **Предоставляют дашборды Grafana** — включает предварительно созданные дашборды Grafana для визуализации:
  - метрик производительности DP;
...
[PodMonitor](/modules/operator-prometheus/cr.html#podmonitor) собирает метрики с двух эндпоинтов из пода `deckhouse`:
...
1. **Метрики хуков** (`/metrics/hooks`) — пользовательские метрики, генерируемые хуками DP с `honorLabels: true` для сохранения лейблов, специфичных для хуков.

PodMonitor создаётся только при включённом модуле [`operator-prometheus`](/modules/operator-prometheus/), а правила оповещений и дашборды обрабатываются модулем [`prometheus`](/modules/prometheus/).

### Интеграция с модулем observability

При включении модуля [`observability`](/modules/observability/) автоматически создаются следующие ресурсы:

- [ClusterObservabilityMetricsRulesGroup](/modules/observability/cr.html#clusterobservabilitymetricsrulesgroup) — для правил Prometheus;
- [ClusterObservabilityDashboard](/modules/observability/cr.html#clusterobservabilitydashboard) — для дашбордов Grafana.

Это позволяет централизованно управлять ресурсами мониторинга через интерфейсы модуля `observability`.
