...
Модуль обеспечивает мониторинг и наблюдаемость компонентов Deckhouse Kubernetes Platform (DKP). Он собирает метрики работы платформы, предоставляет преднастроенные правила оповещений и дашборды для контроля состояния DKP.
...
- **Собирают метрики DKP** — собирает метрики из пода `deckhouse` с помощью ресурса PodMonitor, включая:
...
- **Определяют правила оповещений** — предоставляет правила оповещений Prometheus, организованные по следующим категориям:
  - **доступность DKP** — мониторинг состояния подов, готовности и времени работы;
  - **неисправности DKP** — обнаружение чрезмерных перезапусков, проблем доступа к хранилищу образов контейнеров, а также зависших процессов;
...
1. **Метрики хуков** (`/metrics/hooks`) — пользовательские метрики, генерируемые хуками DKP с `honorLabels: true` для сохранения лейблов, специфичных для хуков.
