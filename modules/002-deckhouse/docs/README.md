...
The module provides monitoring, alerting, and observability for the Deckhouse Kubernetes Platform itself. It monitors the health, performance, and proper operation of DKP core components, working in conjunction with the [`prometheus`](/modules/prometheus/) module.

It deploys monitoring resources that:

- **Collect Deckhouse metrics**: scrapes metrics from the Deckhouse Pod using a PodMonitor resource, including:
...
A PodMonitor scrapes two endpoints from the Deckhouse Pod:
...
The PodMonitor is created only when the `operator-prometheus` module is enabled, and the alerting rules and dashboards are processed by the `prometheus` module.

### Integration with the Observability module

When the [`observability`](/modules/observability/) module is enabled, the following resources are created automatically:
