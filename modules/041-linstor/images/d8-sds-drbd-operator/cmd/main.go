...
import (
	...
	"d8-sds-drbd-operator/api/v1alpha1"
	"d8-sds-drbd-operator/config"
	"d8-sds-drbd-operator/pkg/controllers"
	kubutils "d8-sds-drbd-operator/pkg/kubeutils"
	"os"
	"os/signal"
	goruntime "runtime"
...
func main() {
	...
	// webhookOpt := webhook.Options{CertDir: "/tmp/d8-sds-drbd-operator-certs"}
	managerOpts := manager.Options{
		// LeaderElection:             false,
		// LeaderElectionNamespace:    "d8-linstor",
		// LeaderElectionID:           "d8-sds-drbd-operator-leader-election-helper",
	...
	go func() {
		defer cancel()
		err := controllers.NewLinstorNodesReconciler(ctx, mgr, log, cfgParams.ConfigSecretName, cfgParams.ScanInterval)
		if err != nil {
	...
	log.Info("controller DrbdOperator start")
