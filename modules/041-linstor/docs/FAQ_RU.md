...
Однако, иногда данная операция может завершиться с ошибкой или "зависнуть". В таких случаях ресурсы можно выселить вручную.
...
* Запустите скрипт и следуйте интерактивным инструкциям из него:
...
* Выполните очистку узла следующим образом:
  ...
  * удалите все логические тома(lv) и группы томов(vg) с узла, которые использовались для пулов хранения LINSTOR:
    ```shell
    vgremove -y <vg_name>
    ```

  * Следуйте [этой инструкции](https://deckhouse.ru/documentation/v1/modules/040-node-manager/faq.html#как-зачистить-узел-для-последующего-ввода-в-кластер), начиная со второго пункта для дальнейшей очистки.
