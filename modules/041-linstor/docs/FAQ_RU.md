...
- Через дашборд Grafana: перейдите **Dashboards --> Storage --> LINSTOR/DRBD**  
  В правом верхнем углу вы найдёте информацию об используемом пространсве в кластере.

  > **Внимание:** эта информация отражает состояние всего сырого пространства в кластере.
  > То есть если вы создаёте тома в двух репликах то эти значения стоит поделить на два, чтобы получить примерное представление того сколько таких томов может быть размещено в вашем кластере.

- Через коммандный интерфейс LINSTOR:

  ```shell
  kubectl exec -n d8-linstor deploy/linstor-controller -- linstor storage-pool list
  ```

  > **Внимание:** эта информация отражает состояние сырого пространства для каждой ноды в кластере.
  > То есть если вы создаёте тома в двух репликах то эти две реплики обязательно должны целиком поместиться на двух нодах вашего кластера.
