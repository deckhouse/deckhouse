  ...
  "crowd": {
	...
	Context("Fresh cluster", func() {
		...
		It("Should generate job with valid image", func() {
			...
			hash := f.ValuesGet("global.modulesImages.digests.userAuthn.cfssl").String()
			job := generateJob(registry, hash, "dGVzdAo=")
			Expect(job.Spec.Template.Spec.Containers).To(HaveLen(1))
			Expect(job.Spec.Template.Spec.Containers[0].Image).To(ContainSubstring("@"))
			Expect(job.Spec.Template.Spec.Containers[0].Image).To(Equal(registry + "@" + hash))
		})
	})
