...
func generateProxyAuthCert(input *go_hook.HookInput, dc dependency.Container) error {
	...
	hash := input.Values.Get("global.modulesImages.digests.userAuthn.cfssl").String()
	job := generateJob(registry, hash, base64.StdEncoding.EncodeToString(gcsr))
...
func generateJob(registry, hash, csrb64 string) *batchv1.Job {
	return &batchv1.Job{
			...
			Template: corev1.PodTemplateSpec{
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{
						{
							Name:    "generator",
							Image:   fmt.Sprintf("%s@%s", registry, hash),
