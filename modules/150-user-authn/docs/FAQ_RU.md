...
Удаление Secret'а к смене значения не приводит: пока значение есть в values модуля, владеющий им хук отрендерит его же обратно. Порядок смены:
...
1. Убедитесь, что значение изменилось:
   ...
   Если нет — повторите два предыдущих шага: модуль мог успеть отрендерить прежнее значение.

Потребители внутри кластера перерендерятся, и их поды перекатятся сами. Файлы kubeconfig, скачанные из генератора ранее, содержат прежний клиентский секрет и перестают обновлять токен, поэтому их нужно скачать заново. ID-токены, выданные до смены секрета, продолжают работать до истечения срока, максимум `settings.idTokenTTL` (по умолчанию 10 минут).
