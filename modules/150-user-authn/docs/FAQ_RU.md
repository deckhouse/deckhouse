...
Аутентификационные cookie устанавливаются с атрибутом `Secure`, что означает их передачу только через зашифрованные HTTPS-соединения. При попытке использовать DexAuthenticator с HTTP Ingress (без настройки TLS) возникнет бесконечный цикл редиректов после входа, поскольку браузер не будет отправлять аутентификационные cookie обратно на сервер.
