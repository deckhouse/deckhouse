...
Секрет привилегированного OAuth2-клиента `kubernetes` хранится в `Secret/kubernetes-dex-client-app-secret` в пространстве имён `d8-user-authn`. То же значение используют OAuth2-клиенты `kubeconfig-generator`, `kubeconfig-publish-api` и `kubeconfig-<slug>`, а также basic-auth-proxy, которому оно передаётся как `--ldap-client-secret`.

Удаление Secret'а к смене значения не приводит: пока значение есть в values модуля, владеющий им хук запишет его же обратно. Порядок смены:

1. Если пространство имён `d8-user-authn` синхронизирует GitOps-инструмент, сначала исключите `Secret/kubernetes-dex-client-app-secret` из синхронизации, иначе прежнее значение будет восстановлено.

1. Обнулите поле с секретом:

   ```shell
   d8 k -n d8-user-authn patch secret kubernetes-dex-client-app-secret --type merge -p '{"data":{"secret":""}}'
   ```

1. Перезапустите Deckhouse, чтобы хук увидел пустое поле и сгенерировал новое значение:

   ```shell
   d8 k -n d8-system rollout restart deployment/deckhouse
   ```

1. Убедитесь, что значение изменилось:

   ```shell
   d8 k -n d8-user-authn get secret kubernetes-dex-client-app-secret -o jsonpath='{.data.secret}'
   ```

   Если нет — повторите два предыдущих шага: модуль мог успеть записать прежнее значение обратно.

Конфигурация потребителей внутри кластера будет обновлена, и их поды перекатятся сами. Файлы kubeconfig, скачанные из генератора ранее, содержат прежний клиентский секрет и перестают обновлять токен, поэтому их нужно скачать заново. ID-токены, выданные до смены секрета, продолжают работать до истечения срока, максимум `settings.idTokenTTL` (по умолчанию 10 минут).
