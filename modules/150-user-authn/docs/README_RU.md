...
* SPNEGO (Kerberos) для LDAP‑коннектора. При включении Dex принимает билеты по заголовку `Authorization: Negotiate`, валидирует их по keytab, и завершает вход без рендера формы логина/пароля.
