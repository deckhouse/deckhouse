...
Пользователи, уже совпадающие с правилом, продолжают работать и вызывают только предупреждение. Учтите, что GitOps-процесс, который удаляет и заново создаёт такого пользователя, столкнётся с запретом при повторном создании, — в этом случае добавьте аннотацию в манифест.

Удаление пользователя не отзывает правило. Email остаётся с выданными правами, поэтому повторное создание пользователя с тем же email вернёт их, а до этого email доступен любому, кто может создавать пользователей. Если это нежелательно, удалите субъект и из правила.
...
Группы, уже совпадающие с правилом, продолжают работать и вызывают только предупреждение; для них действует та же оговорка про GitOps, что и для пользователей: цикл «удалить и создать заново» столкнётся с запретом, если аннотации нет в манифесте.
