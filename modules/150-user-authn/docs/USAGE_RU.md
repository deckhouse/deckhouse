...
Удалить, пересоздать или выполнить UserOperation (`ResetPassword`, `Reset2FA`, `Lock`, `Unlock`) над локальным пользователем, чей email или членство в группе уже несёт грант, можно только если вы можете назначить эти роли (покрывающие права или явный диапазон can-assign). `initiatorType: self` эту проверку не обходит. Встроенная страница самообслуживания пароля по-прежнему работает: UserOperation создаёт service account `user-api`.

Подключить DexProvider, который может утвердить уже занятый email или группу, — то же назначение. OpenID Connect, LDAP и другие провайдеры без фильтра claim groups на явный список могут утвердить любой существующий грант, включая SuperAdmin, поэтому создать или обновить такой провайдер может только тот, кто эти роли назначает. SAML с `filterGroups: true` и GitHub с перечисленными организациями и командами проверяются только по этим спискам.
