...
Ресурсы, у которых аннотация уже установлена, это не затрагивает: их владелец может продолжать их изменять, в том числе удалить аннотацию или сменить её значение на неистинное. Ограничено только появление гранта и расширение его у ресурса, который аннотацию уже содержит.
{% endalert %}
...
В отличие от email, имя группы сравнивается точно: по пути в токен его никто не приводит к нижнему регистру, поэтому `Admins` и `admins` — разные имена.
