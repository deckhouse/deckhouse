...
На странице провайдера в веб-интерфейсе DKP доступно действие «Проверить подключение». Оно создаёт ресурс [DexProviderCheck](cr.html#dexprovidercheck) и ждёт, пока контроллер `user-authn` запишет результат проверки в его `status`.
...
Подключить DexProvider, который может утвердить уже занятый email или группу, — то же назначение. OpenID Connect, LDAP и другие провайдеры без фильтра claim groups на явный список могут утвердить любой существующий грант, включая SuperAdmin, поэтому создать такой провайдер может только тот, кто эти роли назначает. SAML с `filterGroups: true` и GitHub с перечисленными организациями и командами ограничивают только claim groups. Claim email они не ограничивают: любой грант на субъект User, включая SuperAdmin, по-прежнему можно утвердить, поэтому создать такой провайдер можно только если эти роли можно назначить. Обновление провайдера без новых целевых ролей, например ротация client secret или bind-пароля, а также смена display name, новым назначением не является. Любое другое изменение настроек коннектора проверяется как новое подключение: `type` и адреса провайдера, параметры проверки подписи и сертификата, а также маппинг claim и атрибутов, определяющий, какое значение становится именем пользователя Kubernetes.
