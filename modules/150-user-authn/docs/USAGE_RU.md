...
Пользователи могут быть объединены в группы для управления правами доступа. Пример CR для группы:
...
* **lockout** - настройки блокировки при превышении лимита неудачных попыток входа.
  ...
  * **lockout.lockDuration** — время блокировки пользователя.
* **rotation** - настройки ротации паролей.
  * **rotation.interval** — период обязательной смены пароля.

### 2FA

2FA позволяет повысить уровень безопасности, требуя ввести код из приложения TOTP (например, Google Authenticator) при входе.
...
* **issuerName** — имя, которое будет отображаться в приложении TOTP при добавлении аккаунта.

_Примечание_: после включения 2FA каждый пользователь должен пройти процесс регистрации в TOTP приложении при первом входе.
