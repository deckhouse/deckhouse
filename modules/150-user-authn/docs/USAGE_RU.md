...
На странице провайдера в веб-интерфейсе DP доступно действие «Проверить подключение». Оно создаёт ресурс [DexProviderCheck](cr.html#dexprovidercheck) и ждёт, пока контроллер `user-authn` запишет результат проверки в его `status`.
