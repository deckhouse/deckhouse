...
На странице провайдера в веб-интерфейсе DKP доступно действие **Проверить подключение**. Оно создаёт ресурс [DexProviderCheck](cr.html#dexprovidercheck) и ждёт, пока хук запишет результат проверки в его status.
...
`DexClient` или [`DexAuthenticator`](cr.html#dexauthenticator) можно разрешить получать токены, которые принимает API-сервер Kubernetes. Для этого установите на ресурсе одну из аннотаций в значение `"true"`:

* `dexclient.deckhouse.io/allow-access-to-kubernetes` — на `DexClient`;
* `dexauthenticator.deckhouse.io/allow-access-to-kubernetes` — на `DexAuthenticator`.
...
Аннотация делает клиента доверенным (trusted peer) для привилегированного OAuth2-клиента `kubernetes`, что позволяет ему запрашивать ID-токены, предназначенные для API-сервера. В таком токене имя пользователя берётся из claim'а `email`, а группы — из claim'а `groups`, поэтому приложение действует в кластере от имени того пользователя, который прошёл аутентификацию, и с его правами.

{% alert level="warning" %}
Это грант уровня кластера, а не уровня пространства имён, хотя оба ресурса namespaced. Поэтому добавить аннотацию или изменить её значение на истинное может только субъект, которому разрешено изменять конфигурацию модуля `user-authn`, — например пользователь с ролью `d8:manage:permission:module:user-authn:edit`. Права на создание `DexClient` или `DexAuthenticator` в пространстве имён сами по себе недостаточны.

Добавление аннотации ограничено при любом её значении, включая `"false"`, поскольку в выпусках старше v1.77 грант выдаётся по одному наличию ключа. Отсутствие гранта выражайте отсутствием аннотации.

Ресурсы, у которых аннотация уже установлена, это не затрагивает: их владелец может продолжать их изменять, в том числе удалить аннотацию или сменить её значение на то, которое доступ не включает. Ограничено только появление гранта и расширение его у ресурса, который аннотацию уже содержит.
{% endalert %}
...
[Правило авторизации](/modules/user-authz/cr.html#authorizationrule) выдаёт права пользователю по email из выданного токена. Поэтому создание пользователя, у которого `spec.email` совпадает с субъектом типа `User` в существующем ресурсе [AuthorizationRule](/modules/user-authz/cr.html#authorizationrule) или [ClusterAuthorizationRule](/modules/user-authz/cr.html#clusterauthorizationrule), запрещено: такой пользователь незаметно получил бы права, выданные этим правилом. Укажите другой email или установите на пользователе аннотацию `user-authz.deckhouse.io/allow-authorization-rule-collision: "true"`, если совпадение сделано намеренно — например когда правило заранее написано под этого человека.

Email сравнивается после приведения к нижнему регистру, потому что именно в таком виде он попадает в токен: `Admin@Example.com` совпадает с субъектом `admin@example.com`.

Пользователи, уже совпадающие с правилом, продолжают работать и вызывают только предупреждение. Затронут любой декларативный процесс, который управляет и правилом, и пользователем, а не только тот, что удаляет и создаёт пользователя заново: при первом применении правило может примениться раньше пользователя, и тогда пользователь будет отклонён. В этом случае добавьте аннотацию в манифест.

Удаление пользователя не отзывает правило и вызывает предупреждение, пока email остаётся субъектом. Email остаётся с выданными правами, поэтому повторное создание пользователя с тем же email вернёт их, а до этого email доступен любому, кто может создавать пользователей. Если это нежелательно, удалите субъект и из правила.
...
Имя группы попадает в выданный токен без изменений, поэтому оно неотличимо от имени группы, полученного от внешнего провайдера аутентификации. Поэтому создание группы, у которой `spec.name` совпадает с субъектом типа `Group` в существующем ресурсе [AuthorizationRule](/modules/user-authz/cr.html#authorizationrule) или [ClusterAuthorizationRule](/modules/user-authz/cr.html#clusterauthorizationrule), запрещено: все участники такой группы незаметно получили бы права, выданные этим правилом. Переименуйте группу или установите на ней аннотацию `user-authz.deckhouse.io/allow-authorization-rule-collision: "true"`, если совпадение сделано намеренно.

В отличие от email, имя группы сравнивается точно: по пути в токен его никто не приводит к нижнему регистру, поэтому два имени, различающиеся только регистром букв, — разные имена.

Группы, уже совпадающие с правилом, продолжают работать и вызывают только предупреждение; для них действует та же оговорка, что и для пользователей: в любом декларативном процессе, управляющем и правилом, и группой, правило может примениться первым, и тогда группа будет отклонена. Удаление группы тоже вызывает предупреждение, пока её имя остаётся субъектом, поскольку правило продолжает выдавать права по этому имени любому, кто создаст группу заново.
