...
На странице провайдера в Console доступно действие **Проверить подключение**. Оно создаёт ресурс [DexProviderCheck](cr.html#dexprovidercheck) и ждёт, пока hook запишет результат проверки в его status.

Проверка убеждается, что:

- указанный [DexProvider](cr.html#dexprovider) существует и включён;
- in-cluster endpoint Dex discovery доступен;
- endpoint внешнего провайдера доступен.

Для OIDC-провайдеров дополнительно читаются discovery-документ и JWKS. Для LDAP-провайдеров проверяется доступность TCP/TLS/StartTLS и, если включён Kerberos, наличие Secret с keytab. Проверка не выполняет полный сценарий входа и не валидирует пароль тестового пользователя.
