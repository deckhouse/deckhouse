...
Для упрощения процесса миграции был написан скрипт [rbd-in-tree-to-ceph-csi-migration-helper.sh](https://github.com/deckhouse/deckhouse/blob/main/modules/099-ceph-csi/tools/rbd-in-tree-to-ceph-csi-migration-helper.sh).
Перед запуском необходимо удалить Pod (поскейлить в ноль StatefulSet/Deployment) использующий PVC. В процессе миграции будет необходимо вручную выполнить команду в Ceph-кластере для переименования RBD-образа (Ceph CSI использует другой формат имени).
...
* [Переименование RBD-образа в ceph-кластере](#%D0%BF%D0%B5%D1%80%D0%B5%D0%B8%D0%BC%D0%B5%D0%BD%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5-rbd-%D0%BE%D0%B1%D1%80%D0%B0%D0%B7%D0%B0-%D0%B2-ceph-%D0%BA%D0%BB%D0%B0%D1%81%D1%82%D0%B5%D1%80%D0%B5)
* [Удаление PVC и PV из кластера](#%D1%83%D0%B4%D0%B0%D0%BB%D0%B5%D0%BD%D0%B8%D0%B5-pvc-%D0%B8-pv-%D0%B8%D0%B7-%D0%BA%D0%BB%D0%B0%D1%81%D1%82%D0%B5%D1%80%D0%B0)
...
Используется StorageClass созданный модулем ceph-csi.
...
Образец `spec.csi` берём из созданного ранее PV (sample):
...
**Внимание!** Перед переходом на containerd убедитесь, что в кластере не используются сборщики логов, помимо log-shipper. Если такие имеются, то стоит либо от них отказаться в пользу модуля [log-shipper](https://deckhouse.io/documentation/v1/modules/460-log-shipper/), либо перенастроить их на работу с containerd. Containerd имеет другой формат логов и хранит файлы логов по другому пути.
