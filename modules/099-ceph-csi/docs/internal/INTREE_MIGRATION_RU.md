...
3. Переименование RBD-образа (выполняется вручную в ceph-кластере, скрипт выводит команду которую достаточно скопировать).
4. Удаление манифестов PVC и PV.
5. Генерация нового манифеста PVC и создание объекта в кластере.
6. Генерация нового манифеста PV и создание объекта в кластере.
...
Необходимо, поскольку Ceph CSI драйвер использует другой формат имени rbd-образа, переименуем его в ceph кластере:
```shell
