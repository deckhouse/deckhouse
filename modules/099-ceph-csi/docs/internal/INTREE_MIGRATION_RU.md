...
**Внимание!** Перед переходом на containerd убедитесь, что в кластере не используются сборщики логов, помимо log-shipper. Если такие имеются, то стоит либо от них отказаться в пользу модуля log-shipper, либо перенастроить их на работу с containerd. Containerd имеет другой формат логов и хранит файлы логов по другому пути.
