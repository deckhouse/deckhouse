	...
	tests := []struct {
	...
	}{
		{
			...
			buildFile: func() (apis.LogSource, apis.LogDestination) {
				...
				art := loglabels.BuildDestinationSinkArtifacts(loglabels.DestinationSinkInput{
					Spec: spec, SourceType: v1alpha1.SourceKubernetesPods, WithPodLabels: true,
				})
				dest := destination.NewLoki(destination.ComposeNameWithSourceType("testoutput", v1alpha1.SourceKubernetesPods), spec, art.LokiLabels)
				return src, dest
			},
		},
		{
			...
			buildFile: func() (apis.LogSource, apis.LogDestination) {
				...
				dest := destination.NewLogstash(destination.ComposeNameWithSourceType("testoutput", v1alpha1.SourceKubernetesPods), spec)
				return src, dest
			},
		},
		{
			...
			buildFile: func() (apis.LogSource, apis.LogDestination) {
				...
				dest := destination.NewElasticsearch(destination.ComposeNameWithSourceType("testoutput", v1alpha1.SourceFile), spec)
				return src, dest
			},
		},
		{
			...
			buildFile: func() (apis.LogSource, apis.LogDestination) {
				...
				dest := destination.NewVector(destination.ComposeNameWithSourceType("testoutput", v1alpha1.SourceKubernetesPods), spec)
				return src, dest
			},
		},
	}
