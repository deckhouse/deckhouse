...
3. `Secret` монтируется всем подам агентов log-shipper, конфигурация обновляется при ее изменении при помощи sidecar-контейнера `reloader`.
...
* Потребление ресурсов на узлах для приложений меньше
* Пользователи могут настроить в агрегаторе любые трансформации и слать логи в гораздо больше количество хранилищ
* Количество выделенных узлы под агрегаторы может увеличиваться вверх и вниз в зависимости от нагрузки
