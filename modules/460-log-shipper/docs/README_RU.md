...
Пайплайн обработки логов состоит из следующих этапов:
...
При сборе все источники обогащают логи метаданными. Обогащение происходит на этапе `Source`.

#### Kubernetes

Будут представлены следующие поля метаданных:
...
Есть специальное поле `@metadata`, которое содержит все лейблы из пространства имен (namespace_labels) и узла (node_labels).
Это поле автоматически не отправляется в хранилище логов. Основная цель его использования - фильтрация сообщений и добавление дополнительных полей в сообщение лога.
...
Этап `Destination` — последний в пайплайне. Он отвечает за отправку логов в хранилище.

#### Extra fields

Опция `extraFields` позволяет добавить дополнительные поля в сообщение лога. Поля добавляются в корень сообщения.

{% alert -%}
Extra labels добавляются на этапе `Destination`, поэтому невозможно фильтровать логи на их основании.
{%- endalert %}

Существуют четыре основных варианта использования `extraFields`:
* Чтобы добавить константное значение, используйте простой синтаксис ключ-значение: `key: "value"`
* Чтобы добавить значение из сообщения, используйте шаблон mustache с действительным [path expression](https://vector.dev/docs/reference/vrl/expressions/#path): `key: "{{ .app }}"`
* Для источника Kubernetes также возможно добавить метки из поля `@metadata`. Синтаксис такой же, как для сообщения, но путь всегда начинается с поля `@metadata`: `key: "{{ @metadata.namespace_labels.app }}"`.
* Для некоторых хранилищ логов, например, Elasticsearch или Splunk, стоит разобрать все сообщение и отправить его как вложенный объект. Чтобы заменить сообщение вложенным объектом, используйте ключ `message`: `message: "{{ @parsed_data }}"`.

{% alert -%}
Шаблоны работают только если сообщение является действительным объектом JSON.
{%- endalert %}

Вы можете найти примеры в разделе [Examples](examples.html) документации.
