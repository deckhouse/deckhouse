...
В приведенном выше примере для каждого неймспейса Kubernetes будет создан свой индекс в Elasticsearch.
...
* `labelFilter` — применяется к метаданным, например имени контейнера (`container`), неймспейсу (`namespace`) или имени пода (`pod_name`);
...
{% alert level="info" %}
Если замена нужна для поля внутри структурированного `message`, сначала примените [`ParseMessage`](#преобразование-логов-в-структурированный-объект).
{% endalert %}
...
## Настройка сборки логов с продуктовых неймспейсов, используя опцию namespace label selector
...
## Исключение подов и неймспейсов, используя label

Существует преднастроенный label для исключения определенных подов и неймспейсов: `log-shipper.deckhouse.io/exclude=true`.
Он помогает остановить сбор логов с подов и неймспейсов без изменения глобальной конфигурации.
