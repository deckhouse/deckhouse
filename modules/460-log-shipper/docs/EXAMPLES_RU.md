...
Вы можете использовать трансформацию `DropLabels`, чтобы удалить заданные лейблы из записей логов. Если задано поле `keepChildKeys`, каждый элемент списка `labels` указывает путь к **объекту**. Внутри этого объекта удаляются все вложенные ключи, кроме перечисленных в `keepChildKeys` (имена ключей без ведущей точки). Если `keepChildKeys` не задан — каждый путь из `labels` удаляется целиком.
