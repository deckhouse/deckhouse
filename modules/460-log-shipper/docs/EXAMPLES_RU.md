...
Если значение по указанному пути является массивом, условия работают следующим образом:

- для `==` и `=~` условие выполняется, если совпадает хотя бы один элемент;
- для `!=` и `!=~` — если ни один элемент не совпадает.
