...
- действия, проводимые при инсталляции DKP CSE, а также при инициализации DKP CSE, подлежат логированию (настройка сбора и отправки логов описана в п. 4.15);
...
<tr>
<td>4.1</td>
<td>Требования для установки</td>
<td>
<ul>
<li>Kubernetes+Virtualization</li>
<li>Virtualization</li>
...
### 4.1. Требования для установки

Перед установкой необходимо убедиться, что выполнены минимальные требования к программным (п. 4.1.1) и аппаратным средствам (п. 4.1.2), а сетевая инфраструктура отвечает требованиям, приведенным в п. 4.1.3 «Требования к обеспечению сетевого взаимодействия узлов кластера».

#### 4.1.1. Требования к программным средствам

Таблица 1 - Требования к программным средствам (для каждого исполнения DKP CSE)
<table>
<thead>
<tr>
<th>Исполнения DKP CSE</th>
<th>Требования к программным средствам</th>
</tr>
</thead>
<tbody>
<tr>
...
#### 4.1.2. Требования к аппаратным средствам

Требования к аппаратным средствам зависят от варианта размещения DKP CSE.
...
<tbody>
<tr>
<td>Одноузловой кластер<br/>(все компоненты на одном узле)</td>
<td>В одноузловой конфигурации все компоненты управления, служебные сервисы и пользовательские нагрузки размещаются на одном сервере. Такая архитектура может использоваться для тестовых окружений, а также для edge-сценариев.<br/><br/>
<b>Преимущества:</b>
<ul><li>возможность размещения всех компонентов на одном узле;</li>
<li>минимальные инфраструктурные затраты.</li>
</ul>
<b>Недостатки:</b>
<ul><li>отсутствует отказоустойчивость;</li>
<li>в случае переподписки платформы возможно влияние на пользовательские нагрузки.</li>
</ul>
</td>
</tr>
<tr>
<td>Кластер с одним master-узлом<br/>(1 master-узел + worker-узлы)</td>
<td>В данной архитектуре один узел выполняет функции управления, а пользовательские нагрузки размещаются на выделенных worker-узлах. Такой вариант подходит для небольших кластеров, где требуется разделение системных сервисов и пользовательских нагрузок.<br/><br/>
<b>Преимущества:</b>
<ul><li>разделение системных сервисов и полезной нагрузки.</li>
</ul>
<b>Недостатки:</b>
<ul><li>отсутствует отказоустойчивость;</li>
<li>требуются дополнительные ресурсы для компонентов управления и служебных сервисов.</li>
</ul>
</td>
</tr>
<tr>
<td>Кластер с тремя master-узлами<br/>(отказоустойчивый control plane)</td>
<td>Архитектура с тремя master-узлами используется в случаях, когда необходима высокая доступность. Компоненты управления распределяются по трём серверам, что обеспечивает отказоустойчивость компонентов управляющего слоя кластера (control plane) и продолжение работы при сбое одного из узлов. Пользовательская нагрузка выполняется на этих же серверах.<br/><br/>
<b>Преимущества:</b>
<ul>
<li>высокая доступность.</li>
</ul>
<b>Недостатки:</b>
<ul><li>в случае переподписки платформы возможно влияние на пользовательские нагрузки.</li>
</ul>
</td>
</tr>
<tr>
<td>Кластер с разделением ролей узлов<br/>(master / system / frontend / monitoring / worker)</td>
<td>Архитектура с разделением ролей узлов применяется в крупных кластерах. Данный вариант размещения предполагает распределение нагрузки по ролям узлов в соответствии с их назначением:
<ul>
<li>master-узла – управляющие узлы (control plane);</li>
<li>системные узлы – выделенные узлы для системных компонентов;</li>
<li>frontend-узлы – узлы для маршрутизации HTTP(S)-трафика;</li>
<li>узлы мониторинга – узлы для размещения компонентов мониторинга;</li>
<li>worker-узлы – узлы для пользовательской нагрузки и виртуальных машин (ВМ).</li>
</ul>
<b>Преимущества:</b>
<ul><li>высокая доступность;</li>
<li>изоляция пользовательских нагрузок от системных сервисов;</li>
<li>разделение доменов отказа.</li>
</ul>
<b>Недостатки:</b>
<ul>
<li>повышенные требования к ресурсам и инфраструктуре.</li>
</ul>
</td>
</tr>
</tbody>
...
Общим требованием к аппаратным средствам является архитектура процессора x86-64 и дисковая подсистема, способная отдавать от 400 IOPS. Приведённые далее требования указаны в расчёте на один узел и учитывают только нагрузку системных компонентов. Ресурсы для пользовательской нагрузки необходимо учитывать дополнительно.

- Для исполнения Kubernetes
  ...
  <tbody>
  <tr>
  <td>Одноузловой кластер</td>
  <td>Master-узел (1 шт.)</td>
  <td>12</td>
  <td>24</td>
  <td>350</td>
  </tr>
  <tr>
  <td rowspan="2">Кластер с одним master-узлом</td>
  <td>Master-узел (1 шт.)</td>
  <td>8</td>
  <td>16</td>
  <td>150</td>
  ...
  <tr>
  <td>Worker-узел (1+ шт.)</td>
  <td>8</td>
  <td>16</td>
  <td>250</td>
  </tr>
  <tr>
  <td>Кластер с тремя master-узлами</td>
  <td>Master-узел (3 шт.)</td>
  <td>6</td>
  <td>12</td>
  <td>150</td>
  </tr>
  <tr>
  <td rowspan="6">Кластер с разделением ролей узлов</td>
  <td>Master-узел (3 шт.)</td>
  <td>6</td>
  <td>12</td>
  <td>60</td>
  ...
  <tr>
  <td>Системный узел (2+ шт.)</td>
  <td>2</td>
  <td>4</td>
  <td>100</td>
  </tr>
  <tr>
  <td>Системный узел  (2+ шт.)<br/><i>(если нет выделенных узлов мониторинга)</i></td>
  <td>6</td>
  <td>12</td>
  <td>300</td>
  </tr>
  <tr>
  <td>Frontend-узел (2+ шт.)</td>
  <td>2</td>
  <td>4</td>
  <td>60</td>
  </tr>
  <tr>
  <td>Узел мониторинга (2+ шт.)</td>
  <td>4</td>
  <td>8</td>
  <td>200</td>
  </tr>
...
- Для исполнения Virtualization

  Для запуска виртуальных машин требуется поддержка инструкций Intel-VT (VMX) или AMD-V (SVM) на узлах с пользовательской нагрузкой.
  ...
  <tr>
  <td>Узел, предназначенный для запуска виртуальных машин (1+ шт.)</td>
  <td>12</td>
  <td>24</td>
  <td>350</td>
  </tr>
...
- Для исполнения Kubernetes+Virtualization
  ...
  <tr>
  <td>Узел, предназначенный для запуска контейнеров или виртуальных машин (1+ шт.)</td>
  <td>12</td>
  <td>24</td>
  <td>350</td>
  </tr>
  ...
  <tr>
  <td rowspan="7">Кластер с разделением ролей узлов</td>
  <td>Master-узел (3 шт.)</td>
  <td>6</td>
  <td>12</td>
  <td>60</td>
  ...
  <tr>
  <td>Frontend-узел (1+ шт.)</td>
  <td>2</td>
  <td>4</td>
  <td>60</td>
  </tr>
  <tr>
  <td>Узел мониторинга (1+ шт.)</td>
  <td>4</td>
  <td>8</td>
  <td>200</td>
  </tr>
...
#### 4.1.3. Требования к обеспечению сетевого взаимодействия узлов кластера
...
### 4.2. Проверки, выполняемые перед началом установки
...
1. Дополнительные требования для статического кластера:
   ...
   - Узел, выбранный под master:
     ...
     - соответствует минимальным системным требованиям (п. 4.1.2);
...
Для использования модуля `virtualization` необходимо установить DKP CSE. Работа с модулем описана в п. 4.16. данного Руководства.

### 4.3. Подготовка инфраструктуры установки
...
- TLS-сертификаты и ключи для HTTPS-доступа к registry и выпуска сертификатов сервисов DKP CSE. Подготовленные значения используются в `deckhouse.registryCA`, секрете `internal-ca-key-pair` и `ClusterIssuer` в файле конфигурации установки (п. 4.5.1).
- Хранилище образов контейнеров DKP CSE (container registry), путь к загруженным образам и данные аутентификации. Эти значения используются в ресурсе `InitConfiguration` (п. 4.4.1) и при запуске контейнера установщика (п. 4.5.2).
- SSH-доступ к узлам кластера. Подготовленные пользователь, публичный ключ и приватный ключ используются в ресурсе `SSHCredentials` файла конфигурации (п. 4.5.1) и в команде запуска установки (п. 4.5.2).
...
- Хранилище образов контейнеров DKP CSE (container registry), путь к загруженным образам и данные аутентификации. Эти значения используются в ресурсе `InitConfiguration` (п. 4.3.1) и при запуске контейнера установщика (п. 4.4.2).
  ...
  {{< alert level="info" >}}
  Поддерживаются только сертифицированные хранилища образов CSE. Добавление сторонних хранилищ не допускается.
  {{< /alert >}}
...
Если registry или сервисы DKP CSE будут использовать самоподписанные TLS-сертификаты, сначала подготовьте сертификаты и ключи (п. 4.3.1). Если у вас нет локального registry, разверните его и загрузите в него образы DKP CSE (п. 4.3.2) до подготовки YAML-файла конфигурации установки.

#### 4.3.1. Создание самоподписанного сертификата
...
#### 4.3.2. Подготовка registry и загрузка образов DKP CSE
...
При отсутствии TLS-сертификатов, можно сгенерировать самоподписанные сертификаты (п. 4.3.1).
...
После загрузки образов используйте адрес registry, путь к образам, данные аутентификации и сертификат registry в ресурсе `InitConfiguration` (п. 4.4.1):
...
### 4.4. Файл конфигурации установки DKP CSE

Для установки DKP CSE нужно подготовить YAML-файл конфигурации установки. Пример заполненного файла конфигурации приведен в п. 4.5.1.
...
#### 4.4.1. Ресурс InitConfiguration
...
#### 4.4.2. Ресурс ClusterConfiguration
...
#### 4.4.3. Ресурс StaticClusterConfiguration
...
#### 4.4.4. Список внутренних сетей узлов кластера
...
### 4.5. Установка DKP CSE

Установка возможна двумя способами: вручную из командной строки (разделы 4.5.1-4.5.4) и с помощью графического установщика (раздел 4.5.5).

#### 4.5.1. Пример конфигурации установки
...
- адрес registry и путь к образам DKP CSE: см. п. 4.3.2;
- данные аутентификации в registry в формате Base64: см. описание `deckhouse.registryDockerCfg` в п. 4.4.1;
- корневой сертификат registry, если используется HTTPS с самоподписанным сертификатом: см. п. 4.3.1;
- сертификат и ключ внутреннего CA для `internal-ca-key-pair`, если сертификаты сервисов DKP CSE выпускаются через `cert-manager`: см. п. 4.3.1;
- SSH-ключ и пользователь для подключения к узлам кластера: см. п. 4.3.3.
...
data:
 ...
 # Если используются файлы из п. 4.3.1, укажите содержимое intermediateCA.crt и intermediateCA.key.
...
spec:
 ...
 # Подготовка пользователя и SSH-ключа описана в п. 4.3.3.
 user: caps
...
spec:
 ...
 - name: falco-critical-alerts
   ...
   - alert: FalcoCriticalAlertsAreFiring
     ...
     expr: |
       sum by (node) (rate(falcosecurity_falcosidekick_falco_events_total{priority_raw=~"error|critical|warning|notice"}[5m]) > 0)
```

Замените параметры в примере:

- `<REGISTRY_HOST>` — DNS-имя или IP-адрес registry из п. 4.3.2;
...
По умолчанию для выпуска сертификатов используется `ClusterIssuer selfsigned`. Создание встроенных `ClusterIssuer letsencrypt` и `letsencrypt-staging` отключено. Если вам нужны  `letsencrypt` и `letsencrypt-staging`, задайте `disableLetsencrypt: false` в конфигурационном файле для модуля cert-manager:
...
#### 4.5.2. Установка DKP CSE

Подготовьте рабочую станцию (хост установки) и master-узел будущего кластера. Минимальные требования к программным и аппаратным средствам представлены в п 4.1. Порядок подготовки рабочего пространства к установке DKP CSE представлен в Разделе 4.

Перед запуском установки убедитесь, что у вас есть:

- файл `config.yml`, заполненный по примеру из п. 4.5.1;
...
#### 4.5.3. Дополнительные общие настройки кластера
...
#### 4.5.4. Настройка балансировки входящего трафика с помощью виртуальных IP-адресов
...
#### 4.5.5 Установка с помощью графического установщика

Запустите графический установщик в виде контейнера, использовав загруженный в хранилище образ. Пример запуска графического установщика:

```bash
docker run --rm --pull always -v $HOME/.d8installer:$HOME/.d8installer -v /var/run/docker.sock:/var/run/docker.sock -p 127.0.0.1:8080:8080 registry.company.int/deckhouse/cse/installer:latest -r $HOME/.d8installer
```

Дождитесь запуска и перейдите по ссылке `http://127.0.0.1:8080`.

На главном экране нажмите кнопку «Установить Deckhouse Kubernetes Platform».

![Главный экран графического установщика с кнопкой «Установить Deckhouse Kubernetes Platform»](images/admin-guide/image17.png)

На следующем экране создайте новый источник для установки DKP CSE. Для этого нажмите кнопку «Добавить источник» и в выпадающем списке выберите пункт «Свой источник».

![Экран добавления источника установки с пунктом «Свой источник»](images/admin-guide/image18.png)

В открывшемся диалоговом окне введите имя, адрес и параметры доступа к хранилищу образов контейнеров. Также в пункте «Используется самоподписанный сертификат» загрузите корневой сертификат, созданный ранее.

![Диалоговое окно параметров доступа к хранилищу образов контейнеров](images/admin-guide/image19.png)

Кнопкой «Проверить подключение» в самом низу убедитесь, что все настройки введены правильно.

![Проверка подключения к хранилищу образов контейнеров](images/admin-guide/image20.png)

Нажмите кнопку «Сохранить». На экране выбора источника появится пункт с именем созданного источника. Выберите его и нажмите кнопку «Инфраструктура» для перехода к настройкам узлов кластера.

![Экран выбора источника с кнопкой «Инфраструктура»](images/admin-guide/image21.png)

На экране «Новый кластер» уже выбран вариант «Статический кластер на существующие серверы», подразумевающий установку на ранее подготовленные машины для узлов будущего кластера. Нажмите кнопку «Параметры кластера».

![Экран «Новый кластер» с кнопкой «Параметры кластера»](images/admin-guide/image22.png)

В открывшемся окне с параметрами инфраструктуры задайте основные настройки:

* IP-адреса и количество узлов (в каждой группе можно ввести несколько IP-узлов):

  ![Параметры инфраструктуры: IP-адреса и количество узлов](images/admin-guide/image23.png)

* параметры подключения к узлам по SSH: пользователь, пароль для повышения привилегий (если необходимо), SSH-ключ:

  ![Параметры инфраструктуры: подключение к узлам по SSH](images/admin-guide/image24.png)

  Если есть необхдимость использовать отдельный SSH-бастион, включите его использование переключателем «Использовать SSH-бастион» и задайте параметры подключения.

* в разделе «Настройки хранилища» можно задать параметры используемого в кластере хранилища. Подробнее его настройка и включание в ручном режиме описаны в разделе 4.5.

  ![Параметры инфраструктуры: настройки хранилища](images/admin-guide/image25.png)

* в разделе «Входящий трафик» можно настроить работу ингресс-контроллера и параметры доступа к веб-интерфейсам кластера, включая настройку SSL-сертификатов.

  ![Параметры инфраструктуры: входящий трафик](images/admin-guide/image26.png)

* при необходимости включите автоматическое создание пользователся переключателем «Создать пользователя» и введите для него адрес электронной почты и пароль. Подробнее варианты аутентификации описаны в разделе 4.11.

  ![Параметры инфраструктуры: создание пользователя](images/admin-guide/image27.png)

После завершения настройки нажмите кнопку «Установить». Процесс установки будет отображён в виде графической диаграммы с описанием этапов.

![Процесс установки в виде графической диаграммы](images/admin-guide/image28.png)

При необходимости можно посмотреть полный лог установки. Для этого нажмите кнопку «Открыть лог».

![Лог установки](images/admin-guide/image29.png)

По окончанию установки все пункты будут подсвечены зеленым.

![Завершение установки](images/admin-guide/image30.png)

### 4.6. Настройка хранилищ
...
#### 4.6.1. Локальное хранилище Local Path Provisioner
...
#### 4.6.2. Локальное хранилище на основе Logical Volume Manager
...
#### 4.6.3. Распределённая система хранения Ceph
...
Внимание. Интеграция с Ceph-кластерами возможна только при использовании
  containerd v2. Работа совместно с containerd v1 не поддерживается.
...
#### 4.6.4. Сетевое файловое хранилище NFS
...
#### 4.6.5. Хранилище данных на основе протокола SCSI
...
#### 4.6.6. Хранилище данных Yadro Tatlin Unified Storage
...
### 4.7. Откат установки и удаление DKP CSE
...
### 4.11. Включение и отключение модуля
...
### 4.12. Механизмы аутентификации и авторизации

#### 4.12.1 Локальная аутентификация
...
##### 4.12.1.1 Создание статического пользователя
...
##### 4.12.1.2 Добавление пользователя в группу
...
После создания группы и добавления в неё пользователей, необходимо настроить авторизацию (п. 4.12.3).
...
##### 4.12.1.3 Настройка парольной политики
...
##### 4.12.1.4 Настройка двухфакторной аутентификации
...
#### 4.12.2 Подключение провайдера аутентификации
...
#### 4.15.1 Настройка сбора и доставки логов
...
#### 4.15.5 Отладка и расширенные возможности

4.15.5.1 Включение debug-логов агента `log-shipper`
...
4.15.5.2 Дополнительная информация о каналах передачи логов
...
4.15.5.3 Добавление поддержки нового source или sink
...
#### 4.16.1. Включение модуля `virtualization`
...
Настройка этих хранилищ подробно описана в п. 4.6.

#### 4.16.3. Параметры модуля
...
```bash
d8 k annotate moduleconfig virtualization modules.deckhouse.io/allow-disabling="true" --overwrite
```
...
#### 4.16.5. Классы виртуальных машин
...
#### 4.16.6. Механизмы обеспечения надежности
...
#### 4.16.7. Контроль целостности в модуле `virtualization`
...
Для проверки целостности образов в репозитории пользователя в DKP CSE используются следующие механизмы:

- Cosign — для проверки подписи образов;
- imagedigest (устаревший) — для проверки контрольной суммы образа.
...
1. Чтобы включить проверку подписи образов контейнеров в кластере DKP CSE, используйте параметр `policies.verifyImageSignatures` ресурса SecurityPolicy, указав публичный ключ, сгенерированный на шаге 1.

   Пример конфигурации SecurityPolicy для проверки подписи образов контейнеров в хранилище `registry.private.ru`, размещенных по пути `/labs/application/`:
...
1. Создайте ресурс OperationPolicy, ограничивающий запуск подов со сторонних хранилищ образов (registry):
