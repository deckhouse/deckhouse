...
lang: ru
search: администрирование, приемка средства, установка, настройка, файл конфигурации установки, подготовка инфраструктуры, откат установки, удаление, настройка хранилищ, Local Path Provisioner, LVM, Ceph, NFS, SCSI, Yadro Tatlin, аутентификация, авторизация, статические пользователи, экспорт данных, логирование, сбор логов, виртуализация, миграция etcd, сканирование уязвимостей, политики безопасности, события безопасности, журнал событий безопасности, контроль целостности, gost-integrity-controller, управление информационными потоками, фильтрация трафика, функции безопасности, параметры безопасности
searchBoost: 2
...
spec:
 ...
 # echo -n '<ПАРОЛЬ>' | htpasswd -BinC 10 "" | cut -d: -f2 | tr -d '\n' | base64 -w0; echo
 password: '<ЗАМЕНИТЕ_НА_ХЕШ_ПАРОЛЯ>'
---
apiVersion: deckhouse.io/v1alpha1
kind: Group
metadata:
...
spec:
  email: admin@deckhouse.ru
  # Сгенерируйте пароль для пользователя и подставьте его в команду ниже, чтобы получить bcrypt-хеш, закодированный в Base64, для указания в секции password:
  # echo -n '<ПАРОЛЬ>' | htpasswd -BinC 10 "" | cut -d: -f2 | tr -d '\n' | base64 -w0; echo
  password: '<ЗАМЕНИТЕ_НА_ХЕШ_ПАРОЛЯ>'
---
apiVersion: deckhouse.io/v1alpha1
...
##### 5.7.4.3. Проверка целостности образа с помощью ContainerdIntegrityPolicy

Механизм позволяет включить проверку подписи образов контейнеров в пользовательских пространствах имен на уровне контейнерного рантайма `containerd` v2. Для работы механизма на узлах кластера должен использоваться `ContainerdV2` (подробнее — в пункте 4.2.1.4 [«Обновление версии containerd»](update.html#обновление-версии-containerd) «Руководства по обновлению DKP CSE»).

Управление механизмом осуществляется с помощью ресурса `ContainerdIntegrityPolicy`, в котором указываются:

- `spec.ca` — сертификат удостоверяющего центра (CA) в формате PEM, с помощью которого будет проверяться подпись образов;
- `spec.protectedNamespaces.matchLabels` — метки пространств имен, образы в которых подлежат проверке.

За применение политик отвечают два компонента:

- `integrity-controller` — собирает список пространств имен, подпадающих под `matchLabels` всех созданных в кластере ресурсов `ContainerdIntegrityPolicy`;
- `integrity-containerd-configurator` — DaemonSet, который на каждом узле с `ContainerdV2` записывает итоговый список защищаемых пространств имен и CA-сертификатов в конфигурационный файл `containerd` (`/etc/containerd/integrity/integrity.toml`).

Если в кластере создано несколько ресурсов `ContainerdIntegrityPolicy`, их списки CA-сертификатов и пространств имен объединяются (мержатся): образ в защищаемом пространстве имен будет считаться подписанным корректно, если его подпись проходит проверку хотя бы по одному из указанных CA-сертификатов.

Чтобы включить проверку целостности образов в пользовательском пространстве имен, выполните следующие шаги:

1. Создайте ресурс `ContainerdIntegrityPolicy`, указав CA-сертификат и метку, по которой будут отбираться защищаемые пространства имен:
...
1. Присвойте нужному пространству имен метку, указанную в `protectedNamespaces.matchLabels`:

   ```bash
   d8 k label ns <NAMESPACE> integrity=enabled
   ```

1. Убедитесь, что конфигурация применена на узлах: на любом узле с `ContainerdV2` проверьте содержимое файла `/etc/containerd/integrity/integrity.toml` — он должен содержать имя размеченного пространства имен и указанный CA-сертификат. При этом сам файл хранит не исходный PEM, а его base64-представление, и список `ca_certs` может содержать несколько сертификатов — по одному от каждого применённого `ContainerdIntegrityPolicy`:

   ```toml
   namespaces = ["my-ns", "production", "kube-.+"]
   ca_certs = ["base64_ca_first", "base64_ca_second"]
   ```

1. Проверьте работу механизма, запустив в размеченном пространстве имен под с образом, подпись которого не проходит проверку по указанному CA-сертификату (или не подписанным вовсе). Запуск такого контейнера будет запрещен, а в событиях (Events) пода появится сообщение вида:

   ```console
   manifest verification failed: manifest signature verification failed: signature bundle creation: unmarshaling map to bundle: no signature. component=integrity
   ```

   Дополнительно ошибки проверки записываются в журнал службы `containerd` с компонентом `integrity` и сохраняются в файл `/var/log/containerd/containerd-integrity.log`.
