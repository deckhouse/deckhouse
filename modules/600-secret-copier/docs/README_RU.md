...
Это может привести к утечке чувствительных данных, если важные секреты окажутся в закрытом окружении пользователя, нарушая модель безопасности.

Если необходимо предоставить общий сертификат (например, WC-сертификат для внутреннего окружения) или общий токен доступа к registry, не используйте `secret-copier`.
