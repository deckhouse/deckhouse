...
func TestManifestsFanOutMultiNamespace(t *testing.T) {
	...
	// Namespaced objects fan out into BOTH project namespaces.
	require.Equal(t, []string{"proj", "proj-extra"}, byKind["NetworkPolicy"], "NetworkPolicy must render into every project namespace")
	require.Equal(t, []string{"proj", "proj-extra"}, byKind["PodLoggingConfig"], "PodLoggingConfig must render into every project namespace")
