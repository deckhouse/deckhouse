...
func (c *Client) ensureParametersStayValues(project *v1alpha2.Project, template *v1alpha1.ProjectTemplate) error {
	...
	sanitized := *project
	sanitized.Spec.Parameters, _ = replaceLineBreaks(project.Spec.Parameters).(map[string]interface{})
...
func objectDigests(manifests string) (map[string]string, error) {
	...
	for _, raw := range releaseutil.SplitManifests(manifests) {
		...
		stripped, _ := replaceLineBreaks(object.Object).(map[string]interface{})
...
func carriesLineBreak(value interface{}) bool {
...
func replaceLineBreaks(value interface{}) interface{} {
	switch typed := value.(type) {
	case string:
		for _, br := range lineBreaks {
