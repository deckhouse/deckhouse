...
func (m *Manager) Handle(ctx context.Context, project *v1alpha2.Project) (ctrl.Result, error) {
	...
	if err := m.updateProjectStatus(ctx, project, v1alpha2.ProjectStateDeploying, 0, nil); err != nil {
		m.logger.Error(err, "failed to set project status")
		return ctrl.Result{}, err
	}

	// set template label, finalizer and delete sync require annotation
	m.logger.Info("preparing the project", "project", project.Name, "projectTemplate", project.Spec.ProjectTemplateName)
	if err := m.prepareProject(ctx, project); err != nil {
		m.logger.Error(err, "failed to prepare project")
		return ctrl.Result{}, err
	}

	// get a project template for the project
	m.logger.Info("getting project template for project", "project", project.Name, "projectTemplate", project.Spec.ProjectTemplateName)
	...
	if statusErr := m.updateProjectStatus(ctx, project, "", projectTemplate.Generation, cond); statusErr != nil {
		m.logger.Error(statusErr, "failed to update the project status", "project", project.Name, "projectTemplate", project.Spec.ProjectTemplateName)
		return ctrl.Result{}, statusErr
	}

	// validate the project against the project template
	m.logger.Info("validating the project spec", "project", project.Name, "projectTemplate", projectTemplate.Name)
	...
	m.logger.Info("upgrading resources for the project", "project", project.Name, "projectTemplate", projectTemplate.Name)
	...
	m.logger.Info("the project reconciled", "project", project.Name, "projectTemplate", projectTemplate.Name)
	return ctrl.Result{}, nil
}
...
func (m *Manager) Delete(ctx context.Context, project *v1alpha2.Project) (ctrl.Result, error) {
	...
	m.logger.Info("successfully deleted the project", "project", project.Name)
	return ctrl.Result{}, nil
}
