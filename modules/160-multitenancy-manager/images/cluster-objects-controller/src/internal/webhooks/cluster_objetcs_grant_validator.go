...
func (v *ClusterObjectsGrantValidator) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if namespaces.IsSystem(review.Request.Namespace) {
	...
	for _, p := range policies {
		for _, ref := range p.Spec.UsageReferences {
			if ref.APIGroup == review.Request.Kind.Group && ref.Resource == review.Request.Resource.Resource {
	...
	for jsonPath, whitelist := range fieldsAndValuesToValidate {
		value, err := jsonpath.Get(jsonPath, obj.Object)
		if err != nil {
			log.Error(err, "invalid JSONPath", "JSONPath", jsonPath)
			continue
		}

		if !slices.Contains(whitelist, value.(string)) {
			err = v.deny(&review, w)
			if err != nil {
				log.Error(err, "admission response failed")
				return
