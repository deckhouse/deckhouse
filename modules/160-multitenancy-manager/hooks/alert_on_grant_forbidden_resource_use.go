	...
	for _, policyRef := range g.Spec.Policies {
		...
		for _, ref := range policy.Spec.UsageReferences {
			...
			for _, item := range list.Items {
				...
				if err != nil {
					log.Error("Invalid JSONPath expression", "expr", ref.FieldPath)
					continue
				}
				...
				switch {
				case len(values) == 0:
					log.Info("Skipping non-existing field reference",
						"expr", ref.FieldPath,
						"policy", policyObj.GetName(),
						"usage_reference", ref,
					)
				...
				case len(values) > 1:
					log.Info(
						"Skipping ambiguous field reference",
						"expr", ref.FieldPath,
						"policy", policyObj.GetName(),
						"usage_reference", ref,
					)
				...
				if !slices.Contains(policyRef.Allowed, values[0].(string)) {
