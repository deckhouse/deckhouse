...
// that used to be orphans as well. The legacy access-level RoleBindings (d8:user-authz:*) that the
// user-authz Helm release emits for CARs/AuthorizationRules targeting such a namespace do not
// deadlock on that: they are applied by the deckhouse controller, which systemWriterMatchConditions
// below excludes at the apiserver, before any call reaches this webhook.
