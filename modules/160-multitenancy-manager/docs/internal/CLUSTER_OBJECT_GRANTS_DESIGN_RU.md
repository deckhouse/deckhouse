...
- **A3 (админ)** — задать квоты на проект (например, 5 внешних LB, внутренних сколько угодно; 1 ТиБ
  ...
  [`ClusterResourceGrant`](#clusterresourcegrant) (см. [Квоты](#квоты)).
...
Покрытие: U1 → [`AvailableClusterResource`](#availableclusterresource); D1/D2 →
[`GrantableClusterResourceDefinition`](#grantableclusterresourcedefinition); A1/A2/A4/A5 →
[`ClusterResourceGrantPolicy`](#clusterresourcegrantpolicy); A3 (квота) → `Project.spec.quota.compute` +
[`ClusterResourceGrant`](#clusterresourcegrant) (см. [Квоты](#квоты)).
...
- **`AvailableClusterResource`** — синтетический per-project **каталог** (`get available`), namespaced —
  тенант узнаёт, *какие* ресурсы доступны проекту (имена + дефолт), обычным RBAC на namespace;
- **`ClusterResourceGrant`** — ресурс **квоты объектов**: `spec` — пул квоты объектов проекта (лимиты), `status`
  — живой расход. Namespaced, рендерится в каждый namespace проекта, чтобы расход был там виден. Квота
  compute остаётся нативной (`ResourceQuota`); `ClusterResourceGrant` — её аналог для granted-объектов;
- **реконсайлер** — материализует каталог, рендерит `ClusterResourceGrant` и считает эффективную доступность.
...
`AvailableClusterResource` (без неоднозначности user-identity), а не фильтрацией нативных cluster-scoped
списков.
...
flowchart TB
  subgraph MM["multitenancy-manager (один менеджер + webhook-сервер)"]
    subgraph CTRL["контроллеры"]
      ...
      C2[namespace]
      C3[template]
      C4["grants + quota реконсиляция (новый):<br/>AvailableClusterResource + ClusterResourceGrant"]
    end
    subgraph WH["вебхуки"]
      ...
      W6["/protect (новый): AvailableClusterResource и ClusterResourceGrant<br/>read-only; ClusterResourceGrant.spec только cluster-admin"]
    end
    subgraph SCH["scheme"]
      direction LR
      S1[Project]
...
|------|-------|------------------|------------|
| [`GrantableClusterResourceDefinition`](#grantableclusterresourcedefinition) | Cluster | разработчик модуля | регистрация ресурса: GVK, где ссылаются, источник дефолта, что измеримо |
| [`ClusterResourceGrantPolicy`](#clusterresourcegrantpolicy) | Cluster | админ | per-project (по селектору) allow-list + дефолт; пресет |
| [`AvailableClusterResource`](#availableclusterresource) | Namespaced | контроллер | per-project **каталог** — доступные имена + дефолт (read-only для пользователей) |
| [`ClusterResourceGrant`](#clusterresourcegrant) | Namespaced | cluster-admin (`spec`) / контроллер (`status`) | **пул** квоты объектов (`spec`) + живой **расход** (`status`); рендерится в каждый namespace проекта |

```mermaid
flowchart LR
  COG["ClusterResourceGrantPolicy<br/>(кому что — админ)"]
  CGR["GrantableClusterResourceDefinition<br/>(что и как — разработчик)"]
  AR["AvailableClusterResource<br/>(каталог; namespaced)"]
  GQ["ClusterResourceGrant<br/>(квота объектов: пул + расход; namespaced)"]
...
metadata:
  name: storageclasses           # на это имя ссылаются гранты; оно же — имя AvailableClusterResource
spec:
  ...
  # ОПЦИОНАЛЬНО — у многих ресурсов его нет (тогда ресурс только allow-list; записи в ClusterResourceGrant нет).
  # ЛИМИТЫ квоты живут в ClusterResourceGrant.spec — см. Квоты; грант (ниже) квоты не несёт.
status:
...
| `enforcement` | enum | нет | `Managed` (по умолчанию — энфорсят наши вебхуки) или `External` (энфорсит свой вебхук модуля; мы только материализуем `AvailableClusterResource`) — см. [Валидация на стороне модуля](#валидация-на-стороне-модуля-external-энфорсмент) |
...
Иначе на эти меры админ может задать лимиты в `ClusterResourceGrant.spec.objects.<resource>.<имя|"*">.<мера>` —
регистрация объявляет меры, а **лимиты несёт [`ClusterResourceGrant`](#clusterresourcegrant)** (см. [Квоты](#квоты)).

### ClusterResourceGrantPolicy

Создаётся админом; один грант — пресет, навешиваемый на класс проектов по метке. Делает **только
allow-list + default** — квоты в нём **нет** (квота объектов живёт в [`ClusterResourceGrant`](#clusterresourcegrant), см.
...
| `resources[].resourceName` | string | да | имя `GrantableClusterResourceDefinition` |
...
В гранте **нет поля `quota`** — лимиты квоты объектов задаются в [`ClusterResourceGrant`](#clusterresourcegrant). Скоуп
...
[`ClusterResourceGrant`](#clusterresourcegrant)). Namespaced, **только status**, `shortName: available`, по одному на
`(namespace проекта, GrantableClusterResourceDefinition)`, `metadata.name` = имя grantable-ресурса, поэтому:
...
`AvailableClusterResource` кому-либо, кроме service account контроллера — это поверхность статуса, а не
...
`AvailableClusterResource` (по одному на `(namespace, ресурс)`), `ClusterResourceGrant` — **один на namespace**, со
всеми мерами — ровно как один `ResourceQuota` перечисляет все compute-меры. Выступает в двух ролях,
один Kind:

- **пул** — один `ClusterResourceGrant` в **control namespace** проекта (в single-namespace проекте — в его
...
- **вид по namespace** — контроллер рендерит **read-only** `ClusterResourceGrant` в каждый рабочий namespace;
...
Защита: рендеримые (в рабочих namespace) `ClusterResourceGrant` несут метку `module: multitenancy-manager` и
управляются контроллером (read-only через ту же admission-политику, что и `AvailableClusterResource`). `status`
...
### Реконсиляция → материализация каталога + рендер ClusterResourceGrant

Триггеры: изменение любого гранта, grantable-ресурса, `Project` (метки), `ClusterResourceGrant.spec` пула,
...
- upsert `AvailableClusterResource` на `(namespace, resourceName)` — доступный набор (`allowed` ∪
  `allowedSelector`) и дефолт (**каталог**);
- отрендерить read-only `ClusterResourceGrant` на каждый рабочий namespace — эффективный `limit` (слайс
  `ProjectNamespace`, если задан, иначе пул из `ClusterResourceGrant.spec` в control namespace) и `used`,
  пересчитанный по живым usage-объектам, плюс итоги по проекту;
- свести итоги проекта в `ClusterResourceGrant.status` пула (objects) и `Project.status.quota` (compute, суммой
  по per-NS `ResourceQuota.status`).

```mermaid
flowchart LR
  G[ClusterResourceGrantPolicy] --> P[совпавшие Project]
  GQ["ClusterResourceGrant.spec (пул)"] --> P
  P --> N[namespaces проекта]
  N --> A["AvailableClusterResource на (ns, ресурс): каталог"]
  N --> Q["ClusterResourceGrant на ns: limit + used"]
...
flowchart TB
  ...
  QUOTA -- да --> ALLOW["ALLOW + резерв в пуле (и слайсе ns) ClusterResourceGrant.status"]
```

**Какие счётчики.** Расход объектов — это **пул проекта**, поэтому авторитетный ledger — `status`
**пула** `ClusterResourceGrant` в control namespace; рендеримый per-namespace `ClusterResourceGrant` несёт `used` своего
...
Периодический проход пересчитывает реальный `used`, корректирует `ClusterResourceGrant.status` и **алертит** на
...
`enforcement: External` в `GrantableClusterResourceDefinition`. Тогда:

- наши `/is-granted` и `/defaults` **не** перехватывают этот ресурс;
- контроллер **всё равно** материализует `AvailableClusterResource` (allow-list + дефолт) и `ClusterResourceGrant`
...
- если заданы `usageReferences` — контроллер всё равно пересчитывает `used` и отдаёт его в `ClusterResourceGrant`;
...
- **objects — наша, в `ClusterResourceGrant`.** Per-class лимиты по ключу **grantable-ресурс → имя granted (или
  ...
  в `ClusterResourceGrant` нет, он управляется только доступностью (allow-list + default).
...
per-class лимиты объектов — наши и единообразные: один блок `ClusterResourceGrant.spec.objects`, независимо от
...
- **Пул (итог по проекту, Уровень 1).** Один `ClusterResourceGrant` в **control namespace** проекта (в
...
|---|---|---|
| `ClusterResourceGrant.spec` (пул, control ns) | **только cluster-admin** | админ проекта |
| `ProjectNamespace.spec.quota` (слайс) | админ проекта (≤ пул) | админ проекта |
| рендеримый `ClusterResourceGrant` (рабочий ns) `status` | только контроллер | тенант |
...
`ClusterResourceGrant` несёт `projectUsed`/`projectLimit`, так что тенант видит итог по проекту из своего
namespace. `spec` пула namespaced, но роль админа проекта **не** даёт запись на `ClusterResourceGrant` (verb
...
рендерит read-only `ClusterResourceGrant` в каждый рабочий namespace, показывая расход (`used`) этого namespace,
его эффективный `limit` и итоги по проекту. Тенант в `team-a-backend` делает
`d8 k get clusterresourcegrant -n team-a-backend` (и `get resourcequota` для compute) — оба обычным namespace-RBAC.
...
2. `ClusterResourceGrant.spec.objects.loadbalancerclasses.external.services: 5` задаёт лимит на это имя;
3. на CREATE/UPDATE вебхук читает имя, берёт `used` для него из `ClusterResourceGrant` проекта и отклоняет, если
...
- **Single-namespace:** единственный namespace проекта держит и пул `ClusterResourceGrant` (spec — только
  cluster-admin, status = used), и нативный `ResourceQuota`. Слайсов нет (делить нечего).
- **Multi-namespace:** пул `ClusterResourceGrant` + `Project.spec.quota.compute` — при control namespace;
  опциональные per-NS слайсы через `ProjectNamespace`; контроллер рендерит `ClusterResourceGrant` +
...
optimistic check-and-update `used` в `ClusterResourceGrant.status` (retry на конфликте) — поэтому не менее
...
[`GrantableClusterResourceDefinition`](#grantableclusterresourcedefinition); остальное делают админы.
...
2. **Создайте** `GrantableClusterResourceDefinition` (поставляйте из шаблонов модуля) — см. пример выше.
...
Используется [`ClusterResourceGrantPolicy`](#clusterresourcegrantpolicy). Один грант — переиспользуемый пресет,
...
- **A3 — задать квоты.** Квоты задаются **не** в гранте. Compute → `Project.spec.quota.compute`;
  лимиты объектов → пул [`ClusterResourceGrant`](#clusterresourcegrant) в control namespace
...
- `defaultAvailability: None` или `excluded` — **breaking change** для проектов, уже использующих
  ...
  через alert-хук и каталог `AvailableClusterResource`), создать гранты, затем сузить до `None` по проектам,
...
- **Квота по механизму, не в гранте.** Compute → `Project.spec.quota.compute` (нативный `ResourceQuota`).
  Objects → [`ClusterResourceGrant`](#clusterresourcegrant): `spec` (пул, control namespace) + `status` (расход), по ключу
...
- **`ClusterResourceGrant` это spec + status, рендерится по namespace.** Пул задаётся в control namespace
  (`spec` пишет cluster-admin); контроллер рендерит read-only `ClusterResourceGrant` в каждый рабочий namespace
...
- **RBAC квоты**: тенант не может повысить себе квоту — `ClusterResourceGrant.spec` пишет только cluster-admin;
...
- **Гранёные имена бывают object-backed или value-backed.** `grantedResource` есть ⇒ object-backed
  ...
  `loadBalancerClass`; литеральный `allowed`). В любом случае per-name квота наша, в `ClusterResourceGrant`
...
- **Режим энфорсмента** на ресурс: `Managed` (наши вебхуки) или `External` (энфорсит свой вебхук
  модуля; мы только материализуем `AvailableClusterResource` + `ClusterResourceGrant` как read API). Закрывает историю D3.
...
- **Non-goal:** фильтрация native `kubectl get <cluster-resource>` по проекту. Прокси фильтрует по
  **пользователю**, а не по проекту, и ломается для юзеров в нескольких проектах — отдельная история.
  Discovery проекта — namespaced `AvailableClusterResource`.
- Лимиты квоты — **на проект** (пул `ClusterResourceGrant`), общие между его namespace (Уровень 1); опц.
  per-namespace слайс от админа проекта (Уровень 2) вложенный/аддитивный (`Σ слайсов ≤ пул`).
- `AvailableClusterResource`: namespaced, только status **каталог** (имена + дефолт), `shortName available`,
  метки `module: multitenancy-manager` + `projects.deckhouse.io/project`, защищён admission-политикой в
  дополнение к read-only RBAC. Квота — отдельный ресурс (`ClusterResourceGrant`).
...
- Пересекающиеся гранты: без hard-deny на создании (конфликты возникают динамически). Рантайм:
  ...
  в гранте нет, поэтому между грантами она не конфликтует; внутри `ClusterResourceGrant.spec.objects` `*` и
...
Задокументировать в USAGE: обнаружение через `get available` и `get clusterresourcegrant` для расхода; четыре
...
### StorageClass — allow-list, дефолт; квота storage в ClusterResourceGrant

Регистрация объявляет меры (число PVC, суммарный `requests.storage`); грант делает allow-list +
дефолт; **лимиты живут в пуле `ClusterResourceGrant`** (единообразные per-class ключи — `*` и именованный класс —
...
spec:
  ...
  - resourceName: storageclasses     # только allow-list + default
    ...
    default: standard
---
# Лимиты storage для проекта — пул ClusterResourceGrant в control namespace (только cluster-admin).
apiVersion: multitenancy.deckhouse.io/v1alpha1
kind: ClusterResourceGrant
metadata:
  name: objects
...
каноничный кейс «наша собственная квота»; лимиты идут в пул `ClusterResourceGrant`.
...
spec:
  ...
  - resourceName: loadbalancerclasses     # только allow-list + default
    ...
    default: internal
---
# Лимиты LB для проекта — пул ClusterResourceGrant (кейс «5 external»)
apiVersion: multitenancy.deckhouse.io/v1alpha1
kind: ClusterResourceGrant
metadata:
  name: objects
...
- **Обратная выборка — кластерный ресурс сам выбирает namespaced-объекты** (имени для гранта нет):
