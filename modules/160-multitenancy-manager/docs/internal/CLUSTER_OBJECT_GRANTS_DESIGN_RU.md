...
пропускают ссылку, путь которой не вычисляется. Оба вебхука работают с `failurePolicy: Fail`, и ответ
...
уже бывшие в старом объекте, повторно не проверяются. Метрика нарушений покажет это позже. Причины
отказа для namespaced-типа:
...
- Только для object-backed определений: у value-backed объектов нет, список там игнорируется.
...
Невалидная запись при проекции пропускается и больше ничего не ломает. Значения обновляются на каждом
реконсайле каталога; на сами выданные объекты вотчей нет, так что изменение доезжает за
`ResyncInterval` (2 минуты).
