...
metadata:
  name: team-a
  labels:
    environment: production        # используется в ClusterObjectGrant.projectSelector
spec:
  ...
  # Квота ОБЪЕКТОВ (per-class storage/LB/… лимиты) живёт в отдельном GrantQuota — см. Квоты.
  quota:
    compute:
      requests.cpu: "40"
...
| `quota` | общий **compute**-бюджет проекта (нативные ключи `ResourceQuota`), распределяемый по namespace; квота объектов живёт в `GrantQuota` (см. [Квоты](#квоты)) |
...
рендерит нативный `ResourceQuota` из `compute` и per-namespace `GrantQuota` из `objects`, держа
`Σ слайсов ≤ пул`; compute-пул: `Project.spec.quota.compute`; пул объектов: `GrantQuota` проекта; см.
[Квоты](#квоты)).
...
рендерит нативный `ResourceQuota` из `spec.quota.compute` и per-namespace `GrantQuota` из
...
- Ресурсы шаблона рендерятся в **каждый рабочий namespace** при его создании — шаблон
  параметризуется **namespace'ом**, а не проектом (`.namespace` вместо неявного единственного
  namespace).
- Разделение по «высоте»:
  ...
  - **пер-проект** (биндинги owner'ов, compute-квота, политика namespace) — переезжает в `Project.spec`
    (выше); квота объектов — в `GrantQuota` проекта, а не в шаблон.
...
- **validating-вебхук** разрешает в нём только белый список kinds — `ProjectNamespace`,
  `ProjectRoleBinding`, `GrantQuota` проекта (пул квоты объектов), read-only каталог `AvailableResource`
...
ровно модель «author once, match by label» как у `ClusterObjectGrant`. Задача контроллера — лишь
...
**На каждом управляемом контроллером объекте** (namespace проекта, `AvailableResource`, рендеримый
`GrantQuota`, рендеримые `RoleBinding`/`NetworkPolicy`/`ResourceQuota`):
...
- **objects — наша, в `GrantQuota`.** Per-class лимиты по ключу **grantable-ресурс → имя granted (или
  `*`) → мера** (storage по StorageClass, count по LoadBalancerClass/IngressClass, …). Пул — `GrantQuota`
  в control namespace; контроллер рендерит read-only `GrantQuota` в каждый рабочий namespace с расходом.
  Полная модель — в [дизайне грантов](./CLUSTER_OBJECT_GRANTS_DESIGN_RU.md#grantquota).
...
spec:
  quota:
    compute:
      ...
      pods: "400"
---
# пул объектов — GrantQuota в control namespace (пишет только cluster-admin)
...
единообразные, в `GrantQuota`, независимо от того, есть ли нативный ключ.

| Лимит | Где |
|-------|-----|
| cpu/memory (requests/limits), pods, сырой `count/<resource>` | `Project.spec.quota.compute` (нативно) |
| storage по StorageClass (`*` и по классу) | `GrantQuota.spec.objects.storageclasses` |
| count по LoadBalancerClass (напр. 5 external) | `GrantQuota.spec.objects.loadbalancerclasses` |
| count по IngressClass / по кастомному granted-объекту | `GrantQuota.spec.objects.<resource>` |
...
(`Project.spec.quota.compute` и `GrantQuota.spec` в control namespace — оба пишет только cluster-admin,
...
read-only per-namespace `GrantQuota` в каждый рабочий namespace (видно тенанту). Полная таблица RBAC —
...
- **по namespace (детализация)** — `compute` это нативный `ResourceQuota.status.used`; `objects` это
  отрендеренный `GrantQuota.status` этого namespace;
...
- **свод объектов** — `GrantQuota.status` в control namespace несёт итог по проекту (`projectUsed`
...
Расход объектов живёт в `GrantQuota` (итог по проекту — в объекте control namespace; по namespace — в
отрендеренном `GrantQuota` каждого рабочего namespace) — см.
[дизайн грантов](./CLUSTER_OBJECT_GRANTS_DESIGN_RU.md#grantquota).
...
Квота живёт на `Project` (compute) и `GrantQuota` (objects); **доступность** — *какие* кластерные
объекты можно проекту и per-project дефолт — задаётся отдельно как `ClusterObjectGrant` и
...
нечего), а запись в `GrantQuota` без гранта, делающего ресурс доступным, ничего не даёт.
...
spec:
  ...
  quota:
    compute:                        # нативные ключи ResourceQuota (объекты — в GrantQuota ниже)
      requests.cpu: "40"
      limits.cpu: "60"
      requests.memory: 80Gi
      limits.memory: 120Gi
      ...
      count/jobs.batch: "50"
---
# 2a. Пул квоты объектов — GrantQuota в control namespace (пишет только cluster-admin).
apiVersion: multitenancy.deckhouse.io/v1alpha1
kind: GrantQuota
metadata:
  name: objects
...
#     Грант делает только allow-list + default; per-class лимиты живут в GrantQuota выше.
...
- **Квоты, по механизму**: compute → `Project.spec.quota.compute` (нативный `ResourceQuota`, по
  namespace); objects → `GrantQuota` (`spec`-пул в control namespace + рендер read-only по namespace).
...
- **Доступность через гранты навешивается по лейблу**: `ClusterObjectGrant.projectSelector` матчит
...
- **Свод расхода**: compute — в `Project.status.quota` (`total` + `namespaces[]`, суммой по per-NS
  `ResourceQuota.status.used`); objects — в `GrantQuota.status` (итог по проекту в control namespace,
  по namespace — в каждом отрендеренном `GrantQuota`).
