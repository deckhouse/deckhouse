...
> Статус: **черновик / обсуждение — это не поставленный API.** Как проект Deckhouse эволюционирует от
> «проект == один namespace» к проекту, который его администратор может делить на несколько
> namespace — *опционально*, потому что self-service нужен не всем. Соседний документ —
> [Гранты на кластерные ресурсы](./CLUSTER_OBJECT_GRANTS_DESIGN_RU.md) (per-project гранты).
>
> Поле `owners` с `User | Group | ServiceAccount` ниже — предложение. Поставленный
> `Project.spec.administrators` принимает **только User или Group**; ServiceAccount — subject
> ProjectRoleBinding, не администратор проекта.
>
> **Не реализовано (на 2026-09), оставлено как история дизайна:** переключатель режима
> `namespaces.selfService` (дополнительные неймспейсы доступны любому проекту, режимов нет),
> закрытый **control namespace** и его белый список kinds (основной неймспейс — рабочий),
