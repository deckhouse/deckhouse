...
По умолчанию (параметр [`allowNamespacesWithoutProjects: true`](configuration.html#parameters-allownamespaceswithoutprojects)) пользователи по-прежнему могут создавать неймспейсы напрямую (`d8 k create namespace`). Чтобы такие неймспейсы не выпадали из-под управления, модуль автоматически «оборачивает» каждый из них в проект с тем же именем:

* проект помечается лейблом `multitenancy.deckhouse.io/project-managed-by-namespace`, источником истины остаётся неймспейс: его лейблы и аннотации синхронизируются в проект;
* неймспейс можно свободно редактировать и удалять — при удалении удаляется и проект;
...
Если параметр `allowNamespacesWithoutProjects` выключен, создание неймспейсов вне проектов запрещается — новые окружения создаются только через ресурс [Project](./cr.html#project).
