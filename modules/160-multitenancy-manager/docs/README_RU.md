...
В основе изоляции [Project](cr.html#project) используется механизм Namespace, расширенный дополнительными уровнями контроля и безопасности:
- `ResourceQuota`: механизм, ограничивающий использование ресурсов (CPU, RAM, количество объектов) внутри Namespace.
- `NetworkPolicy`: политики, которые управляют сетевыми взаимодействиями между Pod-ами (например, запрет на общение между Namespace) и внешними источниками.
