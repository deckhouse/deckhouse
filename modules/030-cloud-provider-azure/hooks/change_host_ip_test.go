...
var _ = Describe("Modules :: cloud-provider-azure :: hooks :: change host ip ::", func() {
...
`, 1))
		...
		It("Should run", func() {
			...
			pod := f.KubernetesResource("Pod", "d8-cloud-provider-azure", "ccm")
...
`, 2))
			...
			It("Should delete the pod", func() {
				Expect(f).To(ExecuteSuccessfully())
				Expect(f.KubernetesResource("Pod", "d8-cloud-provider-azure", "ccm").Exists()).To(BeFalse())
			})
		})
	})
