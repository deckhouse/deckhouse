...
- `AllowSsh` — входящий TCP/22 из CIDR в `sshAllowList` (если список не задан — из любого источника).

Дополнительные порты (например, HTTP/HTTPS) модуль в эту NSG не добавляет. Их нужно настроить вручную в Azure. Подключить заранее созданную собственную NSG к узлам через параметры модуля нельзя — доступен только `sshAllowList` для ограничения SSH.
