...
## Почему Ubuntu-узлы не могут подключиться к кластеру при использовании ContainerdV2?

Стандартные Ubuntu AMI, предоставляемые AWS, не включают пакет `linux-modules-extra`, поэтому на узле отсутствует модуль ядра `erofs`. Этот модуль необходим для поддержки containerd v2. При его отсутствии на узле устанавливается метка `node.deckhouse.io/containerd-v2-unsupported` и узел не может войти в кластер.

Начиная с Deckhouse v1.76, это обрабатывается автоматически: шаг Bashible устанавливает `linux-modules-extra-$(uname -r)` на Ubuntu-узлах при бутстрапе.

Если проблема возникла на более старой версии Deckhouse, установите пакет вручную на проблемном узле:
