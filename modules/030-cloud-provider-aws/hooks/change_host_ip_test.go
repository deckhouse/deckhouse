...
var _ = Describe("Modules :: cloud-provider-aws :: hooks :: change host ip ::", func() {
...
`, 1))
		...
		It("Should run", func() {
			...
			pod := f.KubernetesResource("Pod", "d8-cloud-provider-aws", "ccm-test")
...
`, 2))
			...
			It("Should delete the pod", func() {
				Expect(f).To(ExecuteSuccessfully())
				Expect(f.KubernetesResource("Pod", "d8-cloud-provider-aws", "ccm-test").Exists()).To(BeFalse())
			})
		})
	})
