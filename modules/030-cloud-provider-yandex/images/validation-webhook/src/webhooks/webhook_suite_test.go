...
func TestWebhooks(t *testing.T) {
	...
	RunSpecs(t, "Yandex Validation Webhook Suite")
}
...
var _ = BeforeSuite(func() {
	...
	// InstanceClassGVK is required: with an empty Kind the builder never loads InstanceClasses,
	// and every rule that inspects them silently passes.
	factory := ycval.NewAdmissionStateBuilderFactory(mgr.GetClient(), cpvaladmission.StateBuilderConfig{
...
func registerUnstructuredGVKs(scheme *runtime.Scheme) {
	for _, gvk := range []schema.GroupVersionKind{nodeGroupGVK(), instanceClassGVK(), moduleConfigGVK()} {
		listGVK := gvk
		listGVK.Kind += "List"
...
func instanceClassGVK() schema.GroupVersionKind {
	return schema.GroupVersionKind{Group: "deckhouse.io", Version: "v1alpha1", Kind: "YandexInstanceClass"}
}
