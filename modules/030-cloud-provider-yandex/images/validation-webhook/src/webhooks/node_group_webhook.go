...
func NewNodeGroupValidator(factory *ycval.AdmissionStateBuilderFactory, object *unstructured.Unstructured) *NodeGroupValidator {
...
func (v *NodeGroupValidator) ValidateCreate(ctx context.Context, obj *unstructured.Unstructured) (admission.Warnings, error) {
	if !v.shouldValidateNodeGroup(obj) {
		nodeGroupLog.V(2).Info("skipping validation", "reason", "not Yandex-relevant NodeGroup", "name", obj.GetName())
		return nil, nil
	}
...
func (v *NodeGroupValidator) ValidateUpdate(ctx context.Context, oldObj, newObj *unstructured.Unstructured) (admission.Warnings, error) {
	if !v.shouldValidateNodeGroupUpdate(oldObj, newObj) {
		nodeGroupLog.V(2).Info("skipping validation", "reason", "not Yandex-relevant NodeGroup update", "name", newObj.GetName())
		return nil, nil
	}
...
func (v *NodeGroupValidator) ValidateDelete(ctx context.Context, obj *unstructured.Unstructured) (admission.Warnings, error) {
	if !v.shouldValidateNodeGroup(obj) {
		nodeGroupLog.V(2).Info("skipping validation", "reason", "not Yandex-relevant NodeGroup delete", "name", obj.GetName())
		return nil, nil
	}
