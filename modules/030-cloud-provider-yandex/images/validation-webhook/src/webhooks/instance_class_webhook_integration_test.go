...
var _ = Describe("YandexInstanceClass webhook", func() {
	BeforeEach(func() {
	...
	It("rejects etcdDisk on worker instance class", func() {
		...
		Expect(unstructured.SetNestedField(worker.Object, int64(10), "spec", "etcdDiskSizeGB")).To(Succeed())

		err := testK8sClient.Update(testCtx, worker)
		Expect(err).To(HaveOccurred())
		Expect(apierrors.IsInvalid(err)).To(BeTrue())
		Expect(err.Error()).To(ContainSubstring(`worker.spec.etcdDisk: Invalid value: 10: InstanceClass.spec.etcdDisk can be used only when class is attached to NodeGroup master`))
	})
	...
	It("rejects removing etcdDisk from master instance class", func() {
		...
		Expect(testK8sClient.Get(testCtx, clientObjectKey("", "master-yandex"), master)).To(Succeed())

		unstructured.RemoveNestedField(master.Object, "spec", "etcdDiskSizeGB")

		err := testK8sClient.Update(testCtx, master)
		Expect(err).To(HaveOccurred())
		Expect(apierrors.IsInvalid(err)).To(BeTrue())
		Expect(err.Error()).To(ContainSubstring(`master-yandex.spec.etcdDisk: Invalid value: null: YandexInstanceClass for NodeGroup master must define spec.etcdDisk`))
	})
})
