...
`))
	...
	Context("Discovery data is wrong", func() {
		...
		It("All values should be gathered from discovered data", func() {
			...
			Expect(c.GoHookError.Error()).Should(ContainSubstring(`region in body is required`))
			Expect(c.GoHookError.Error()).Should(ContainSubstring(`routeTableID in body is required`))
			Expect(c.GoHookError.Error()).Should(ContainSubstring(`defaultLbTargetGroupNetworkId in body is required`))
			...
			Expect(c.GoHookError.Error()).Should(ContainSubstring(`shouldAssignPublicIPAddress in body is required`))
		})
	})
	...
	Context("Discovery data is wrong", func() {
		...
		It("All values should be gathered from discovered data", func() {
			...
			Expect(d.GoHookError.Error()).Should(ContainSubstring(`nodeNetworkCIDR in body is required`))
