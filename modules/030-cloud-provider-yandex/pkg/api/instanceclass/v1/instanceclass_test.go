...
func TestYandexInstanceClassImplementsContract(t *testing.T) {
	...
	if len(class.GetNodeGroupConsumers()) != 1 {
		t.Fatalf("GetNodeGroupConsumers() = %v, want [master]", class.GetNodeGroupConsumers())
	}

	class.Spec.EtcdDiskSizeGB = ptr.To(20)
	if class.GetEtcdDisk() == nil {
		t.Fatal("GetEtcdDisk() = nil, want the etcd disk with etcdDiskSizeGB")
	}
}
