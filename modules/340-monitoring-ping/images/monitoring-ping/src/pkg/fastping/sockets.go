...
// ReadPacket reads ICMP replies using non-blocking recv
func (s *socketConn) ReadPacket(timeout time.Duration) (string, time.Duration, error) {
	...
	for {
		...
		if err != nil {
			...
			return "", 0, fmt.Errorf("recv error: %w", err)
		}
		...
		if len(icmpData) < 16 {
			log.Warn(fmt.Sprintf("received packet from %s with too short payload for timestamp", ip))
			continue
		}
