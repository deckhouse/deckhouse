...
Артефакты Deckhouse CLI могут находиться в одном из двух мест. Официальный реестр публикует их один раз для всех редакций: в корне реестра, на уровень выше репозитория редакции кластера. Реестр, заполненный командой `d8 mirror push`, хранит их прямо под репозиторием кластера, ровно так, как они были загружены.

Прокси берёт репозиторий из секрета `d8-system/deckhouse-registry` и пробует кандидатов по очереди: сначала репозиторий как есть, затем репозиторий без завершающего сегмента редакции (`ce`, `be`, `se`, `se-plus`, `ee`, `fe`). Ответивший корень запоминается и в дальнейшем проверяется первым; ошибка 404 на запомненном корне или смена репозитория в секрете запускает повторный поиск. Например, кластер с репозиторием `registry.deckhouse.io/deckhouse/ee` читает образы из `registry.deckhouse.io/deckhouse/deckhouse-cli` и `registry.deckhouse.io/deckhouse/deckhouse-cli/plugins/<PLUGIN>`, а кластер, зеркалированный в `registry.local/dest/ee`, читает их из `registry.local/dest/ee/deckhouse-cli/plugins/<PLUGIN>`. Репозиторий без сегмента редакции используется как есть. Учётные данные во всех случаях берутся из того же секрета.
