...
- Предоставляет HTTPS API для иконок пакетов внутри кластера (без публикации через Ingress).
...
Если ни одного из этих файлов нет (или файл больше 4 MiB), прокси возвращает `404 Not Found`; вызывающий код должен использовать иконку по умолчанию. SVG предпочтительнее, так как не зависит от разрешения.
...
Модуль публикует ConfigMap `registry-packages-proxy-config` в неймспейсе `d8-cloud-instance-manager`. Клиент читает его и узнаёт, куда подключаться и какой CA подписывает предъявляемый сертификат, поэтому других запросов ему не нужно:

| Ключ | Описание |
|------|----------|
| `endpoints` | JSON-список адресов `<masterIP>:4219`. Есть в любом кластере. |
| `ca.crt` | CA, которым подписан сертификат на порту `4219`. Им проверяется соединение. |
| `publicEndpoint` | Базовый URL публичного Ingress. Присутствует только когда настроен публичный домен. |

Чтение входит в ClusterRole `d8:registry-packages-proxy:cli-download`, то есть одна привязка покрывает и поиск, и скачивание.

Адреса мастер-узлов работают в любом кластере, включая кластер без публичного домена, без ingress-контроллера и без внешнего центра сертификации. Публичный адрес остаётся полезен клиенту, у которого нет сетевого доступа к мастер-узлам.
...
- Иконки читаются из фиксированных путей внутри образа пакета (`docs/icon.svg`, `docs/icon.png`, `docs/icon.jpg`, `docs/icon.jpeg`); SVG предпочтительнее. Максимальный размер иконки — 4 MiB.
