...
Чтобы установить Deckhouse Kubernetes Platform вручную, используют утилиту dhctl. На вход она
...
3. `ClusterConfiguration` — конфигурация Kubernetes: версия, подсетей подов, сервисов и т.п.
   1. Параметры размещения в инфраструктуре
      1. `<Provider>ClusterConfiguration` — параметры размещения кластера в облаке или API
         виртуализации, если Deckhouse Kubernetes Platform устанавливается не на статические ресурсы;
      2. или `StaticClusterConfiguration` для статического кластера
4. `resources.yaml` — произвольная конфигурация Deckhouse Kubernetes Platform в виде манифестов Kubernetes
...
Deckhouse Commander делит конфигурацию Deckhouse Kubernetes Platform по принципу отслеживаемости. Причем
...
конфигурация должна будет удовлетворять особенностями dhctl и Deckhouse Kubernetes Platform той
