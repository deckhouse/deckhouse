...
Это пользовательская конфигурация шаблона для пользователя шаблона. См. [Входные параметры](usage.html#схема-параметров-кластера-и-ресурса).
...
Этот манифест содержит в реестр контейнеров, откуда будут взять образы Deckhouse Kubernetes Platform.
...
конфигурирует на них компоненты Deckhouse Kubernetes Platform, после чего создает заданные ресурсы
...
удаление. Статические ресурсы очищаются от компонентов Deckhouse Kubernetes Platform, облачные
...
занимал Deckhouse Kubernetes Platform, а кластер переместить в архив вручную.
...
кластеров можно предусмотреть выбор канала обновлений Deckhouse Kubernetes Platform или адрес
