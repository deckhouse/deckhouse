...
func purgeOrphanResources(input *go_hook.HookInput, dc dependency.Container) error {
	...
	if ns != nil {
		...
		if err != nil {
			input.Logger.Warnf("Failed to list IstioOperator resources: %v", err)
		} else {
			for _, iop := range iops.Items {
				...
				if err != nil {
					input.Logger.Warnf("Failed to remove finalizers from IstioOperator/%s in namespace %s: %v", iop.GetName(), istioSystemNs, err)
					continue
				}
				...
				if !iopDeletionTimestampExists {
					...
					if err != nil {
						input.Logger.Warnf("Failed to delete IstioOperator/%s from namespace %s: %v", iop.GetName(), istioSystemNs, err)
						continue
					}
