...
`))
	...
	Context("Secret cacerts is in cluster with CRL; values aren't set", func() {
		BeforeEach(func() {
			f.BindingContexts.Set(f.KubeStateSet(`
---
apiVersion: v1
...
`))
		...
		It("Should add existing ca certificate and CRL to values", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(f.BindingContexts.Array()).ShouldNot(BeEmpty())
			...
			Expect(f.ValuesGet("istio.internal.ca.chain").String()).To(Equal("ccc"))
			Expect(f.ValuesGet("istio.internal.ca.root").String()).To(Equal("eee"))
			Expect(f.ValuesGet("istio.internal.ca.crl").String()).To(Equal("crl"))
		})
	})

	Context("Secret cacerts is in cluster; values are set with CRL", func() {
		BeforeEach(func() {
			f.ValuesSetFromYaml("istio.ca", []byte(`
cert: xxx
key: yyy
...
`))
		...
		It("Should copy cert and CRL data from values", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(f.BindingContexts.Array()).ShouldNot(BeEmpty())
			...
			Expect(f.ValuesGet("istio.internal.ca.crl").String()).To(Equal("my-crl"))
		})
	})
})
