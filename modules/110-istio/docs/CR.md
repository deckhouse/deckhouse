...
This resource is cluster-wide.

* `spec`:
  * `trustDomain` — the TrustDomain of the remote cluster. A mandatory parameter. However, it isn't used currently since Istio cannot match TrustDomain with the root CA;
    * Format — a string;
    * Example — `mycluster.local`;
  * `metadataEndpoint` — an HTTPS-url where metadata of the remote cluster are available.
