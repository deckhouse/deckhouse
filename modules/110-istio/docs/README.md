...
* Федерация — это процесс установления взаимного доверия между двумя и более кластерами. Соответственно, для установления федерации, нужно в кластере A сделать кластер Б доверенным, и в кластере Б сделать кластер А доверенным. Технически это достигается взаимным обменом корневыми сертификатами.
...
* В кластер добавляется сервис, который экспортит метаданные кластера наружу (без аутентификации):
  * корневой сертификат Istio,
  * список публичных сервисов в кластере,
  * список публичных адресов сервиса ingressgateway.
...
Для автоматизации процесса федерации реализован специальный контроллер, экземпляр которого работает в каждом кластере-участнике. Алгоритм установления доверия с следующий:
...
* Каждый ресурс, который считается публичным, необходимо пометить специальным лейблом.
