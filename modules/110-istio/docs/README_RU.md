...
Основной ресурс для управления маршрутизацией — [VirtualService](istio-cr.html#virtualservice), он позволяет переопределять судьбу HTTP- или TCP-запроса. Доступны следующие аргументы для принятия решения о маршрутизации:
...
Основная цель активации — добавить sidecar-контейнер к подам приложения, после чего Istio сможет управлять трафиком. Рекомендованный способ добавления — использовать sidecar-injector. Istio умеет «подселять» к подам sidecar-контейнер с помощью механизма [Admission Webhook](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/), который настраивается с помощью лейблов и аннотаций:

{% alert level="info" %}
В Deckhouse версии >= 1.55 модуль [admission-policy-engine](../015-admission-policy-engine/) по умолчанию работает в режиме `Baseline`. Если включить автоматический примонтированный токен сервисного аккаунта, init-контейнерам Istio будет сложнее добавить необходимые правила для управления трафиком приложений. Чтобы решить эту проблему, можно установить политику `Restricted` для всего приложения или применить ее к отдельному приложению с помощью лейбла `security.deckhouse.io/pod-policy=privileged` в прикладном Namespace.
{% endalert %}

* Лейбл security.deckhouse.io/sidecar-injection=enabled для namespace указывает инжекторы sidecar на вашем namespace. После применения лейбла к новым подам, к ним будут добавлены соответствующие sidecar:
...
Чтобы подготовить Ingress-контроллер и Ingress-ресурсы приложения:
* Включите [`enableIstioSidecar`](../402-ingress-nginx/cr.html#ingressnginxcontroller-v1-spec-enableistiosidecar) у ресурса IngressNginxController.
* Добавьте аннотации на Ingress-ресурсы приложения:
  * `nginx.ingress.kubernetes.io/service-upstream: "true"` — Ingress-контроллер в качестве upstream использует ClusterIP вместо адресов подов. Балансировкой трафика между подами занимается sidecar-proxy. Используйте эту опцию в случае, если у вашего сервиса есть ClusterIP;
...
* Мультикластер объединяет созависимые кластеры, где пространство имен у кластеров общее — каждый сервис доступен для соседних кластеров так, словно он работает на локальном кластере (если это не запрещают правила авторизации).

### Федерация

#### Общие принципы федерации

* Для установления доверия между кластерами в федерации кластер A должен считать кластер Б надежным и наоборот. Это достигается путем взаимного обмена корневыми сертификатами.
...
Чтобы построить федерацию:
* В каждом кластере создайте набор ресурсов `IstioFederation`, которые описывают все остальные кластеры. После успешного автосогласования между кластерами, в ресурсе `IstioFederation` заполнятся разделы `status.metadataCache.public` и `status.metadataCache.private` служебными данными, необходимыми для работы федерации.
* Для каждого ресурса (`service`), который считается публичным в рамках федерации, поставьте пометки лейблом `federation.istio.deckhouse.io/public-service=`. В кластерах из состава федерации, для каждого `service` создадутся соответствующие `ServiceEntry`, ведущие на `ingressgateway` оригинального кластера.
...
* Мультикластер требует взаимного доверия между кластерами. Для построения мультикластера в кластере A необходимо кластер Б сделать доверенным, а в кластере Б сделать доверенным кластер А. Это достигается взаимным обменом корневыми сертификатами.
...
* В кластер добавляется прокси для публикации доступа к API-серверу при помощи стандартного Ingress-ресурса:
  - Доступ через этот публичный адрес ограничен авторизацией на основе Bearer-токенов, подписанных доверенными ключами. Обмен доверенными публичными ключами происходит автоматически средствами Deckhouse при взаимной настройке мультикластера.
  - Сам прокси имеет read-only-доступ к ограниченному набору ресурсов.
* В кластер добавлен сервис, который предоставляет метаданные кластера:
  - Корневой сертификат Istio (доступен без аутентификации).
