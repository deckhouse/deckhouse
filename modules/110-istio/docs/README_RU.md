...
|     1.21     |                                           1.26, 1.27, 1.28, 1.29, 1.30, 1.31                                          |  Устарела и будет удалена  |
...
- `istiod` — ключевой сервис, обеспечивающий решение следующих задач:
  ...
  - Компоновка конфигурации для каждого sidecar-proxy индивидуально:
    - генерация авторизации, маршрутизации, балансировки и прочих правил;
...
- Лейбл к `namespace` — обозначает ваш неймспейс для компонента sidecar-injector. После применения лейбла к новым подам будут добавлены sidecar-контейнеры:
  - `istio-injection=enabled` — использует глобальную версию Istio (`spec.settings.globalVersion` в `ModuleConfig`);
  - `istio.io/rev=v1x21` — использует конкретную версию Istio для этого неймспейсы;
...
Также существует возможность добавить sidecar к индивидуальному поду в неймспейсе без установленных лейблов `istio-injection=enabled` или `istio.io/rev=vXxYZ` путем установки лейбла `sidecar.istio.io/inject=true`.

{% alert level="warning" %}
Istio-proxy, который работает в качестве sidecar-контейнера, тоже потребляет ресурсы и добавляет накладные расходы:
{% endalert %}

- Любой входящий DNAT запрос принудительно перехватывается прокси-сервером Envoy. Он анализирует его и отправляет дальше, создавая новое соединение. Со стороны получателя происходит то же самое: трафик сначала попадает в «принимающий» Envoy, а только потом — в само приложение.
- Каждый Envoy хранит информацию обо всех сервисах в кластере, поэтому масштабирование кластера ведет к линейному росту потребления RAM в Envoy из-за хранения полной карты сервисов. С помощью ресурса CustomResource [Sidecar](istio-cr.html#sidecar), происходит фильтрация конфигурации и передача в Envoy только необходимого минимума данных.
...
- Федерация объединяет суверенные кластеры:
  - у каждого кластера собственный неймспейс (для namespace, Service и пр.);
