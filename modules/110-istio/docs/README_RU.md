...
* В каждом кластере создать набор ресурсов `IstioFederation`, которые описывают все остальные кластеры.
  * После успешного автосогласования между кластерами, в `IstioFederation` в секции `.status.metadataCache` заполнятся разделы `public`(появятся ключи соседнего кластера) и `private`(появится информация о `ingressGateways` соседнего кластера).
* Каждый ресурс(`service`), который считается публичным в рамках федерации, пометить лейблом `federation.istio.deckhouse.io/public-service=`.
  * После этого произойдут следующие события:
    * в том же кластере, в `IstioFederation` в раздел `.status.metadataCache.private.publicServices` добавится информация об этих `service`
    * в остальных кластерах федераций, для каждого `service` создадутся соответствующие `ServiceEntry`, ведущие на `ingressgateway` оригинального кластера
