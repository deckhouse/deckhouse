...
- Каждый ресурс (сервис), который считается публичным в рамках федерации, пометить лейблом `federation.istio.deckhouse.io/public-service: ""`.
  - В кластерах из состава федерации, для каждого сервиса создадутся соответствующие ServiceEntry, ведущие на ingressgateway оригинального кластера.
...
1. С помощью утилиты `istioctl` ([как установить...](https://istio.io/v1.25/docs/setup/additional-setup/download-istio-release/)):
...
- `ztunnel` — DaemonSet, который запускает один под на узел и прозрачно туннелирует трафик рабочих нагрузок ambient на этом узле по протоколу [HBONE](https://istio.io/latest/docs/ambient/architecture/) (HTTP-Based Overlay Network Encapsulation) с использованием Mutual TLS. Он обеспечивает функциональность L4 без сайдкара.
...
- Используйте Istio 1.25 или новее (режим ambient доступен начиная с Istio 1.25).
...
Модуль устанавливает и запускает инфраструктуру ambient (`ztunnel`, а также контроллер waypoint для ресурсов `WaypointInstance`), но **не** подключает рабочие нагрузки автоматически. Подключение выполняется с помощью стандартных лейблов Istio:
...
Пошаговая настройка описана в [примерах использования ambient mesh](examples.html#ambient-mesh).

{% alert level="warning" %}
Не отключайте режим ambient, пока в кластере существуют ресурсы `WaypointInstance`. При отключенном режиме ambient контроллер waypoint не запущен и не может согласовывать или удалять ресурсы waypoint. Удалите все ресурсы `WaypointInstance` перед отключением режима ambient.
{% endalert %}
