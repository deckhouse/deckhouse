...
### Перехват трафика на основе метки (mark-based) для подов с общим UID

Некоторые workload'и (например, `ingress-nginx` под Deckhouse) работают с тем же UID/GID, что и сайдкар Istio. В этом случае стандартные owner-based правила iptables не могут отличить трафик приложения от трафика Envoy, поэтому исходящий трафик обходит сайдкар, и mesh на него не влияет.

Для таких подов включите перехват на основе метки (mark-based), добавив аннотацию:
...
Envoy помечает собственные исходящие сокеты fwmark-меткой (`0x53a` = 1338), а init-контейнер исключает помеченные пакеты из редиректа. Обычные поды без аннотации продолжают использовать стандартные правила на основе UID и не затрагиваются.

{% alert level="warning" %}
Установка `SO_MARK` требует `CAP_NET_RAW` на Linux **>= 5.17** (или `CAP_NET_ADMIN` на более старых ядрах). На старых ядрах помеченный сайдкар не сможет открыть upstream-сокеты и получит `EPERM`.
{% endalert %}
