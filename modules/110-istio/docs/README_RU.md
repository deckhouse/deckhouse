...
Ресурс [AuthorizationPolicy](istio-cr_ru.html#authorizationpolicy) отвечает за управление авторизацией. После создания этого ресурса для сервиса для определения судьбы запроса используется следующий алгоритм:
...
Основной ресурс для управления маршрутизацией — [VirtualService](istio-cr_ru.html#virtualservice), он позволяет переопределять судьбу HTTP- или TCP-запроса. Доступные аргументы для принятия решения о маршрутизации:
...
Основной ресурс для управления балансировкой запросов — [DestinationRule](istio-cr_ru.html#destinationrule), он позволяет настроить нюансы исходящих из подов запросов:
...
В Deckhouse параметр — [`telemetryAPI.enabled`](configuration_ru.html#parameters-telemetryapi-enabled): `false` оставляет полностью включённым `telemetry.v2`, `true` переводит mesh на режим Telemetry API (`defaultProviders`, управляемые модулем `Telemetry` CR и формат строки доступа из [`dataPlane.accessLog`](configuration_ru.html#parameters-dataplane-accesslog)). Подробнее с примерами см. раздел [Telemetry API: метрики mesh, трассировка и журналы доступа](examples_ru.html#telemetry-api-mesh-observability).
...
#### Приложение с выключённым Istio
...
- Каждый Envoy хранит информацию обо всех сервисах в кластере, поэтому масштабирование кластера ведёт к линейному росту потребления RAM в Envoy из-за хранения полной карты сервисов. С помощью кастомного ресурса [Sidecar](istio-cr_ru.html#sidecar) происходит фильтрация конфигурации и передача в Envoy только необходимого минимума данных.
...
{% alert level="warning" %}
Если IP-адреса сервисов или подов пересекаются между кластерами, под правила маршрутизации, запрета или разрешения Istio могут попасть запросы подов из других кластеров. Пересечение подсетей сервисов и подов не рекомендуется (см. [сетевые модели Istio](https://istio.io/latest/docs/ops/deployment/deployment-models/#network-models)).
{% endalert %}
...
Если оба варианта отключены, модуль использует basic auth со сгенерированным паролем.
...
- горизонтальное масштабирование (настройка модуля [`controlPlane.replicasManagement`](configuration_ru.html#parameters-controlplane-replicasmanagement)) — чем больше экземпляров контроллеров, тем меньше экземпляров istio-сайдкаров обслуживать каждому из них и тем меньше нагрузка на CPU и на сеть.
- сегментация data-plane с помощью ресурса [Sidecar](istio-cr_ru.html#sidecar) (рекомендуемый подход) — чем меньше область видимости у отдельного istio-сайдкара, тем меньше требуется обновлять данных в data-plane и тем меньше нагрузка на CPU и на сеть.
