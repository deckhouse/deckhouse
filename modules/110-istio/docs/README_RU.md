...
- прокси waypoint — опциональный прокси уровня L7, разворачиваемый на неймспейс (или для части его рабочих нагрузок или сервисов). В Deckhouse он создается с помощью кастомного ресурса [`WaypointInstance`](cr.html#waypointinstance), который является абстракцией Deckhouse над встроенным механизмом создания waypoint в Istio. Он добавляет декларативное управление количеством реплик (`Static`/`HPA`), ресурсами (`Static`/`VPA`), размещением по узлам и бюджетами прерываний (PDB) для каждого экземпляра waypoint.

### Требования

- Используйте Istio 1.25 или новее (режим ambient доступен начиная с Istio 1.25).
- Установите параметру [`dataPlane.trafficRedirectionSetupMode`](configuration.html#parameters-dataplane-trafficredirectionsetupmode) значение `CNIPlugin`. Режим ambient требует настройки перенаправления трафика с помощью CNI-плагина.
...
См. [примеры использования ambient mesh](examples.html#ambient-mesh) для пошаговой настройки.
