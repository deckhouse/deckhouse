...
{% alert level="info" %}
В Deckhouse версии >= 1.55 модуль [admission-policy-engine](../015-admission-policy-engine/) по умолчанию работает в режиме `Baseline`. Если его включить, init-контейнеры Istio не могут добавить необходимые правила для перехвата прикладного трафика и Поды не могут запуститься. Решение — либо поменять политику по умолчанию на `Restricted`, либо выставить эту политику для отдельного приложения с помощью лейбла `security.deckhouse.io/pod-policy=privileged` на прикладной Namespace.
{% endalert %}
