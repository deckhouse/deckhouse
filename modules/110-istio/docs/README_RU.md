---
title: "Модуль istio"
description: "Реализует Service Mesh для централизованного управления сетевым трафиком в кластере: Mutual TLS, авторизация, маршрутизация трафика, балансировка нагрузки и наблюдаемость."
webIfaces:
- name: istio
---
...
В таблице ниже указаны версии Istio и их статус поддержки в Deckhouse Platform (DP).

| Версия Istio | Версии Kubernetes, поддерживаемые Istio | Статус в DP |
|:------------:|:------------------------------------------------------------------------------------------------------------------------------:|:--------------------------:|
...
{% alert level="warning" %}
Istio 1.21 больше не поддерживается. До обновления на этот релиз DP перенесите глобальную и все дополнительные ревизии Istio 1.21 на поддерживаемую версию, используя предыдущий релиз DP, который поддерживает обе версии. Обновление DP блокируется, пока в конфигурации остаётся Istio 1.21.
{% endalert %}
...
- [Управление балансировкой запросов между эндпоинтами сервиса](#управление-балансировкой-запросов-между-эндпоинтами-сервиса).
...
- [Подключение к сети без сайдкаров в каждом поде с помощью режима ambient](#ambient-mesh).

Также доступно [видео об архитектуре Istio и накладных расходах](https://www.youtube.com/watch?v=9CUfaeT3T-A).
...
Ресурс [AuthorizationPolicy](istio-cr.html#authorizationpolicy) отвечает за управление авторизацией. После создания этого ресурса для сервиса для определения судьбы запроса используется следующий алгоритм:
...
- **Istio Configuration Health** (EE) — результаты периодического анализа конфигурации: количество проблем по типу/severity/namespace, тренд во времени и текущий список issues (с фильтрами по revision и namespace). Требует [`configAnalysis.enabled`](configuration.html#parameters-configanalysis-enabled).
...
- `istiod` — ключевой сервис, обеспечивающий решение следующих задач:
  ...
  - Обработка и валидация с помощью механизма validating-вебхуков Kubernetes всех Custom Resources, которые связаны с Istio.
  ...
  - Автоматическая подстройка манифестов, определяющих прикладные поды через механизм mutating-вебхуков Kubernetes:
...
- К подам контроллера добавляется сайдкар-прокси, который обслуживает только трафик от контроллера в сторону прикладных сервисов (параметр [`enableIstioSidecar`](/modules/ingress-nginx/cr.html#ingressnginxcontroller-v1-spec-enableistiosidecar) ресурса IngressNginxController).
...
## Архитектура прикладного сервиса с включённым Istio
...
- Ресурсы типа Ingress требуют минимальной доработки в виде добавления аннотаций:
  - `nginx.ingress.kubernetes.io/service-upstream: "true"` — Ingress-контроллер в качестве upstream будет использовать ClusterIP сервиса вместо адресов подов. Балансировкой трафика между подами теперь занимается сайдкар-прокси. Используйте эту опцию, только если у вашего сервиса есть ClusterIP.
  - `nginx.ingress.kubernetes.io/upstream-vhost: "myservice.myns.svc"` — сайдкар-прокси Ingress-контроллера принимает решения о маршрутизации на основе заголовка Host. Без данной аннотации контроллер оставит заголовок с адресом сайта, например `Host: example.com`.
...
Рекомендованный способ добавления сайдкаров — использовать sidecar-injector. Istio умеет «подселять» к вашим подам сайдкар-контейнер с помощью механизма [Admission Webhook](https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/). Настраивается с помощью лейблов и аннотаций:
...
Также существует возможность добавить сайдкар к индивидуальному поду в неймспейсе без лейблов `istio-injection` и `istio.io/rev` путем установки лейбла `sidecar.istio.io/inject=true`. То же самое относится к лейблу пода `istio.io/rev=vXxYZ` — он задает версию Istio для отдельного пода, но только в неймспейсе, на котором нет ни лейбла `istio-injection`, ни лейбла `istio.io/rev`.

Лейблы неймспейса `istio-injection` и `istio.io/rev` взаимоисключающие. В неймспейс, на котором установлены оба лейбла или на котором у лейбла `istio-injection` значение отличается от `enabled`, сайдкары не добавляются вообще — ни на уровне неймспейса, ни на уровне отдельных подов.

Istio-proxy, работающий как сайдкар-контейнер, потребляет ресурсы и добавляет следующие накладные расходы:
...
- Каждый Envoy хранит информацию обо всех сервисах в кластере, поэтому масштабирование кластера ведёт к линейному росту потребления RAM в Envoy из-за хранения полной карты сервисов. С помощью кастомного ресурса [Sidecar](istio-cr.html#sidecar) происходит фильтрация конфигурации и передача в Envoy только необходимого минимума данных.
...
- Добавьте аннотации на Ingress-ресурсы приложения:
  - `nginx.ingress.kubernetes.io/service-upstream: "true"` — Ingress-контроллер в качестве upstream использует ClusterIP сервиса вместо адресов подов. Балансировкой трафика между подами теперь занимается сайдкар-прокси. Используйте эту опцию, только если у вашего сервиса есть ClusterIP;
  - `nginx.ingress.kubernetes.io/upstream-vhost: "myservice.myns.svc"` — сайдкар-прокси Ingress-контроллера принимает решения о маршрутизации на основе заголовка Host. Без этой аннотации контроллер оставит заголовок с адресом сайта, например `Host: example.com`.

## Федерация и мультикластер

{% alert level="warning" %}
Доступно в редакциях Enterprise Edition, DP Ultimate, Certified Security Edition Pro и DP Certified Pro.
{% endalert %}
...
{% alert level="warning" %}
В федерации могут участвовать только рабочие нагрузки в режиме сайдкаров. Подробнее — в разделе [Ограничения ambient mesh](#ограничения-ambient-mesh).
{% endalert %}
...
- Домены кластеров в параметре [`clusterDomain`](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#clusterconfiguration-clusterdomain) ресурса [ClusterConfiguration](/products/kubernetes-platform/documentation/v1/reference/api/cr.html#clusterconfiguration) должны быть одинаковыми для всех участников мультикластера. По умолчанию значение параметра — `cluster.local`.
...
- Для сбора информации о соседних сервисах Istio подключается напрямую к API-серверу соседнего кластера. Данный модуль DP берет на себя организацию соответствующего канала связи.
...
- В кластер добавляется прокси для публикации доступа к API-серверу посредством стандартного Ingress-ресурса:
  - Доступ через данный публичный адрес ограничен авторизацией на основе Bearer-токенов, подписанных доверенными ключами. Обмен доверенными публичными ключами происходит автоматически средствами DP при взаимной настройке мультикластера.
...
- компонент уровня узла (`ztunnel`) отвечает за функциональность уровня L4 (Mutual TLS, идентификация, базовая авторизация) для всех рабочих нагрузок сети на узле;
- опциональный компонент уровня неймспейса (прокси *waypoint*) отвечает за функциональность уровня L7 (маршрутизация HTTP, авторизация L7, телеметрия).

По сравнению с режимом сайдкаров, режим ambient снижает накладные расходы на каждый под (сайдкар-контейнер не добавляется в каждый под) и позволяет внедрять возможности сети постепенно — сначала L4 через `ztunnel`, а затем L7 только для тех неймспейсов, которым это нужно.

### Компоненты

В режиме ambient используются следующие компоненты:

- `ztunnel` — DaemonSet, который запускает один под на узел и прозрачно туннелирует трафик рабочих нагрузок, подключенных к ambient mesh на этом узле, по протоколу [HTTP-Based Overlay Network Encapsulation (HBONE)](https://istio.io/latest/docs/ambient/architecture/) с использованием Mutual TLS. Он обеспечивает функциональность L4 без сайдкара;
- прокси waypoint — опциональный прокси уровня L7, разворачиваемый на неймспейс (или для части его рабочих нагрузок или сервисов). В DP он создается с помощью кастомного ресурса [WaypointInstance](cr.html#waypointinstance), который является абстракцией DP над встроенным механизмом создания waypoint в Istio. Он добавляет декларативное управление количеством реплик (`Static`/`HPA`), ресурсами (`Static`/`VPA`), размещением по узлам и бюджетами прерываний (PDB) для каждого экземпляра waypoint.

### Требования

Для использования режима ambient выполните следующие требования:

- Используйте Istio 1.25 или более новой версии (режим ambient доступен начиная с Istio 1.25).
- Для параметра [`dataPlane.trafficRedirectionSetupMode`](configuration.html#parameters-dataplane-trafficredirectionsetupmode) установите значение `CNIPlugin`. Режим ambient требует настройки перенаправления трафика с помощью CNI-плагина.
- Включите режим ambient с помощью параметра модуля [`ambient.enabled`](configuration.html#parameters-ambient-enabled).

### Ограничения ambient mesh

Режим ambient совместим с [федерацией](#федерация) и [мультикластером](#мультикластер) на уровне кластера: его можно включить в кластере, который является участником федерации или мультикластера, — межкластерное взаимодействие продолжит работать.

Ограничение действует на уровне отдельных рабочих нагрузок: федерация и мультикластер работают только с рабочими нагрузками в режиме сайдкаров. Межкластерный трафик до рабочей нагрузки, подключенной к ambient mesh, и от неё не работает ни в одном из направлений.

{% alert level="warning" %}
Чтобы рабочая нагрузка могла участвовать в федерации или мультикластере, оставьте её в режиме сайдкаров — не добавляйте лейбл `istio.io/dataplane-mode=ambient` на саму рабочую нагрузку или её неймспейс.
{% endalert %}

Оба режима data plane могут одновременно использоваться в одном кластере, поэтому межкластерные рабочие нагрузки можно оставить в режиме сайдкаров, а остальные — подключить к ambient mesh.

### Подключение рабочих нагрузок

Модуль устанавливает и запускает компоненты инфраструктуры ambient (`ztunnel` и контроллер waypoint для ресурсов WaypointInstance), но **не** подключает рабочие нагрузки автоматически.
...
{% alert level="warning" %}
Перед отключением режима ambient удалите все ресурсы WaypointInstance. При отключенном режиме ambient контроллер waypoint не запускается и не может согласовывать или удалять ресурсы waypoint.
{% endalert %}

## Аутентификация

По умолчанию аутентификацию для Kiali обеспечивает модуль [`user-authn`](/modules/user-authn/). Также можно настроить аутентификацию через `externalAuthentication`.
Если оба варианта отключены, модуль использует базовую аутентификацию со сгенерированным паролем.

Чтобы посмотреть сгенерированный пароль, выполните:
