...
В таблице ниже указаны версии Istio и их статус поддержки в Deckhouse Kubernetes Platform (DKP).

| Версия Istio | Версии Kubernetes, поддерживаемые Istio | Статус в DKP |
|:------------:|:------------------------------------------------------------------------------------------------------------------------------:|:--------------------------:|
...
- Для сбора информации о соседних сервисах Istio подключается напрямую к API-серверу соседнего кластера. Данный модуль DKP берет на себя организацию соответствующего канала связи.
...
- В кластер добавляется прокси для публикации доступа к API-серверу посредством стандартного Ingress-ресурса:
  - Доступ через данный публичный адрес ограничен авторизацией на основе Bearer-токенов, подписанных доверенными ключами. Обмен доверенными публичными ключами происходит автоматически средствами DKP при взаимной настройке мультикластера.
...
- прокси waypoint — опциональный прокси уровня L7, разворачиваемый на неймспейс (или для части его рабочих нагрузок или сервисов). В DKP он создается с помощью кастомного ресурса [WaypointInstance](cr.html#waypointinstance), который является абстракцией DKP над встроенным механизмом создания waypoint в Istio. Он добавляет декларативное управление количеством реплик (`Static`/`HPA`), ресурсами (`Static`/`VPA`), размещением по узлам и бюджетами прерываний (PDB) для каждого экземпляра waypoint.
