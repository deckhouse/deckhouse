...
Перед запуском debug-контейнера создайте отдельный ServiceAccount и выдайте ему права, необходимые для команд `istioctl`, которые вы планируете запускать. Например, следующий манифест разрешает команды `istioctl proxy-config` для подов в одном прикладном неймспейсе:
...
Замените `<debug-namespace>` на неймспейс, в котором будет создан временный debug-под, а `<target-namespace>` — на неймспейс прикладного пода, который нужно проверить. Повторите `Role` и `RoleBinding` для каждого целевого неймспейса, где `istioctl` должен получать доступ к подам.
...
{% alert level="warning" %}
Создание подов в системных неймспейсах, таких как `d8-system`, и использование системных ServiceAccount, таких как `deckhouse`, обычно требует прав cluster-admin. Используйте отдельный ServiceAccount с минимально необходимыми правами.
{% endalert %}
