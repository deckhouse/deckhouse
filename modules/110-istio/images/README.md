...
### Building proxy-v1x21x6 image
  - final image based on `common/distroless` image
    - includes:
      - `libc.so.6`, `libm.so.6` and `ld-linux-x86-64.so.2` *(from the `-glibc-artifact` image)*
...
#### Building envoy for proxy-v1x21x6
  ...
  - `build-image-artifact` image based on `builder/golang-bookworm` image
    - clang 14, lld 14 and libc++ 14 from Debian packages
    - protoc, the protoc-gen-* plugins and bazel from internal mirrors
  ...
  Envoy links libc++ statically, so the final image needs only the three glibc files
  listed above.

## Envoy bazel inputs: deps and cache

The envoy build has no network access: it always runs `bazel build --nofetch`. Bazel
gets two prepared inputs from git instead.
...
go run ./build-envoy-artifacts -version 1.21.6 -artifact deps -out ~/envoy-artifacts
```
...
Versions that build LLVM from source (1.27.9) also need
