...
## 005-change-to-deckhouse-user.patch

Change sidecar UID/GID from 1337 to 64535 in istio product code (Go constants, envoy credential, inject/webhook, iptables constants) and in the istio charts that the classic istio-operator deploys from `/var/lib/istio/manifests` (injection-template.yaml, gateway-injection-template.yaml). The iptables `-u` arg keeps both UIDs (`64535,1337`) so both are excluded from redirect.

## 006-mark-interception.patch

Add mark-based outbound interception for pods where the sidecar must run with the same UID/GID as the application container (e.g. ingress-nginx under deckhouse, both UID 64535). With the dual-uid approach from 005 the app's own outbound traffic is also excluded from redirect, breaking interception.
