...
function bootstrap_static() {
 ...
 $cwd/opentofu apply -auto-approve -no-color | tee "$cwd/terraform.log" || return $?
  ...
  D8_MODULES_URL="registry.deckhouse.ru/deckhouse/ee"
  testRunAttempts=20
...
d8 mirror pull d8-modules \
  ...
  --include-module commander-agent --include-module commander --include-module prompp --no-platform  --no-security-db
